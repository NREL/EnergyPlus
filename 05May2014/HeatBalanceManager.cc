...
namespace HeatBalanceManager {
	...
	GetProjectControlData( bool & ErrorsFound ) // Set to true if errors detected during getting data
	{
    	...
    	std::string const Format_732( "('! <Zone Air Mass Flow Balance Simulation>, Simulation {Yes/No}')");
		std::string const Format_733("(' Zone Air Mass Flow Balance Simulation, ',A)");
		...
		// A new object is added by B. Nigusse, 02/14
		 CurrentModuleObject = "ZoneAirMassFlowBalance";
		 NumObjects = GetNumObjectsFound( CurrentModuleObject );
		...
		if (NumObjects > 0) {
			...
			if (NumAlpha > 1) {
				...
				if (SELECT_CASE_var == "ADDINFILTRATIONFLOW") {
					...
					ShowWarningError(trim(CurrentModuleObject) + ": Invalid input of " + trim(cAlphaFieldNames(2)) + ". The default choice is assigned = NO");
				} }
			}
			else {
				ZoneAirMassFlow.InfiltrationTreatment = 1;
		...
		if ( ZoneAirMassFlow.EnforceZoneMassBalance ) {
			gio::write(OutputFileInits, Format_733) << "Yes" << trim(AlphaName(1));
		}
		else {
