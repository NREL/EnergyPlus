...
//
		...
		SetZoneMassBalanceFlag()
	{
		...
		// PURPOSE OF THIS SUBROUTINE :
	    // This subroutine sets the zone mass balance flag to true.
        ...
        if (TotMixing > 0) {
            for (Loop = 1; Loop <= TotMixing; ++Loop) {
                ...
                if (ZoneMassBalanceRepVarFlag( Mixing( Loop ).ZonePtr ) ) {
                    ...
                    SetupOutputVariable("Zone Total Supply Air Mass Flow Rate [kg/s]", MassConservation( Mixing( Loop ).ZonePtr ).InMassFlowRate, "System", "Average", Zone( Mixing( Loop ).ZonePtr ).Name );
                    SetupOutputVariable("Zone Total Exhaust Air Mass Flow Rate [kg/s]", MassConservation( Mixing( Loop ).ZonePtr ).ExhMassFlowRate, "System", "Average", Zone( Mixing( Loop ).ZonePtr ).Name );
                ...
                if (ZoneMassBalanceRepVarFlag( Mixing( Loop ).FromZone ) ) {
                    ...
                    SetupOutputVariable("Zone Total Supply Air Mass Flow Rate [kg/s]", MassConservation( Mixing( Loop ).FromZone ).InMassFlowRate, "System", "Average", Zone( Mixing( Loop ).FromZone ).Name );
                    SetupOutputVariable("Zone Total Exhaust Air Mass Flow Rate [kg/s]", MassConservation( Mixing( Loop ).FromZone ).ExhMassFlowRate, "System", "Average", Zone( Mixing( Loop ).FromZone ).Name );
			...
			for ( ZoneNum = 1; ZoneNum <= NumOfZones; ++ZoneNum ) {
				  if ( MassConservation( ZoneNum ).IsOnlySourceZone ) {
					   if ( MassConservation( ZoneNum ).InfiltrationPtr == 0 ) {
						    ShowSevereError( RoutineName + ": Infiltration object is not defined for zone = " + trim( Zone( ZoneNum ).Name ) );
