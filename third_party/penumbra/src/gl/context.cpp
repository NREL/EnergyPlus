...
Context::Context(unsigned size)
  ...
  if (!glfwInit()) {
    ...
    showMessage(MSG_ERR,
                "Either there is no GPU, libraries are missing, or some other error happened.");
  }
  ...
  if (!window) {
    showMessage(MSG_ERR, "Unable to create OpenGL context. OpenGL 2.1+ is required to perform GPU "
