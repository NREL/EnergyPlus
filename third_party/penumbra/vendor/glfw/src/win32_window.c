...
//
static void getFullWindowSize(DWORD style, DWORD exStyle, int contentWidth, int contentHeight,
                              int *fullWidth, int *fullHeight, UINT dpi) {
...
static void updateFramebufferTransparency(const _GLFWwindow *window) {
  ...
  if (SUCCEEDED(DwmIsCompositionEnabled(&enabled)) && enabled) {
    ...
    if (SUCCEEDED(DwmEnableBlurBehindWindow(window->win32.handle, &bb))) {
      ...
      SetLayeredWindowAttributes(window->win32.handle, RGB(255, 0, 255), 255, LWA_COLORKEY);
    }
...
static int createNativeWindow(_GLFWwindow *window, const _GLFWwndconfig *wndconfig,
  ...
  window->win32.handle = CreateWindowExW(exStyle, _GLFW_WNDCLASSNAME, wideTitle, style, xpos, ypos,
