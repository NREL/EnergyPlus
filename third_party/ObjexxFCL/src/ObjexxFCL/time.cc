...
{
#ifdef _WIN32
	Sleep( static_cast< DWORD >( sec * 1000 ) ); // Non-POSIX GCC builds on Windows may be missing some thread support
#else
	std::this_thread::sleep_for( std::chrono::microseconds( static_cast< std::int64_t >( sec * 1.0e6 ) ) );
#endif
}
