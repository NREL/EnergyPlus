...
// Row-Major 4D Slice Array Proxy
//
			...
			for ( int j1 = a.l1(), e1 = a.u1(); j1 <= e1; ++j1 ) {
				for ( int j2 = a.l2(), e2 = a.u2(); j2 <= e2; ++j2 ) {
					for ( int j3 = a.l3(), e3 = a.u3(); j3 <= e3; ++j3 ) {
						for ( int j4 = a.l4(), e4 = a.u4(); j4 <= e4; ++j4, ++l ) {
							c[ l ] = a( j1, j2, j3, j4 );
						}
					}
				}
...
public: // MArray Generator
	...
	// MArray Generator
	template< typename M >
	MArray4< Array4S const, M >
	ma( M ClassT::* pmem ) const
	{
		return MArray4< Array4S const, M >( *this, pmem );
	}
	...
	ma( M ClassT::* pmem )
	{
		return MArray4< Array4S, M >( *this, pmem );
	}
...
inline
bool
conformable( Array4S< U > const & a, MArray4< A, M > const & b )
{
	return a.conformable( b );
}
...
inline
bool
conformable( MArray4< A, M > const & a, Array4S< V > const & b )
{
...
// Write an Array4S to a Binary File
template< typename T >
inline
std::ostream &
write_binary( std::ostream & stream, Array4S< T > const & a )
{
	std::size_t const n( a.size() );
	if ( stream && ( n > 0u ) ) {
