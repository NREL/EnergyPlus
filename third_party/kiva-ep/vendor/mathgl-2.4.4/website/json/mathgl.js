...
function mgl_rotate_left(obj,val)	{	obj.tet += val; }
function mgl_rotate_right(obj,val){	obj.tet -= val; }
function mgl_rotate_up(obj,val)	{	obj.bet += val; }
function mgl_rotate_down(obj,val)	{	obj.bet -= val; }
function mgl_restore(obj)		// restore transformation state
{	obj.tet=obj.phi=obj.bet=0;	obj.z=[0,1,0,1];	obj.pf=0;	}
function mgl_perspective(obj,val) // add perspective, def.value=0.1
{
	var x = obj.pf/(1-obj.pf) + val;
	obj.pf = x/(1+x);	// let use this formula for "smooth" changing
}
function mgl_zoom_in(obj,factor)		// zoom in picture
{	if(factor)	mgl_zoom_out(obj,1/factor);	}
function mgl_zoom_out(obj,factor)	// zoom out picture
{
	var d, c;
...
function mgl_shift_up(obj,val)	// shift up
{	mgl_shift_down(obj,-val);	}
function mgl_shift_down(obj,val)	// shift down
{
	var d=val/(obj.z[3]-obj.z[2]);
	obj.z[2] -= d; obj.z[3] -= d;
}
function mgl_shift_left(obj,val)	// shift left
{	mgl_shift_right(obj,-val);	}
function mgl_shift_right(obj,val)	// shift right
{
...
function mgl_draw_fast(obj, ctx, skip)
{
...
function mgl_draw_good(obj, ctx, skip)
{
	obj.fast = 0;
	mgl_prepare(obj,skip);	// update coordinates
...
function mgl_draw_prim(obj, ctx, prim, scl)
{
...
function mgl_draw_mark(ctx,x,y,st,size,d)
{
...
// This function for internal use only!!!
function mgl_wire_glyph(ctx, x,y, f,g,b)
{
	var xx,yy,j;
	var np=0;	ctx.beginPath();
	for(j=0;j<g[0];j++)
...
// This function for internal use only!!!
function mgl_line_glyph(ctx, x,y, f,solid,b)
{
	var xx,yy,j,xs,ys;
	var dy = 0.004;
