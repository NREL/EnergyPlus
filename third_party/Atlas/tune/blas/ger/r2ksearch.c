...
)
{
   ...
   mf = TimeR2Kernel(verb, FTIME, r1p, pre, M, N, lda, percL1, nrep,
                     fmflop[imf], cflush);
   while (mf == 0.0)
   {
      const int ft= (FTIME < 0) ? -1 : 1;
      fmflop[imf] = (fmflop[imf]) ? (fmflop[imf]<<1) : (ATL_nkflop+999)/1000;
      mf = TimeR2Kernel(verb, ft, r1p, pre, M, N, lda, percL1, nrep,
                        fmflop[imf], cflush);
   }
   return(mf);
}
 ...
 */
{
   ...
   for (kp=kq; kp; kp = kp->next)
   {
      ...
      percL1 = mf;
      mf = TimeR2Kernel(1, FTIME, kp, pre, M, N, lda, percL1, NREP,
                           Fflops, flushKB);
      kp->mflop[imf] = mf;
 ...
 */
{
   ...
   while(p)
   {
      ...
      if (R2KernelFailsTest(0, pre, m, n, lda, p))
      {
...
void PrintUsage(char *name, int ierr, char *flag)
{
   ...
   fprintf(stderr,
           "   -o <fname> : output filename [<pre>R2K.sum]\n");
   fprintf(stderr,
           "   -i <fnam> : kernel index file [R2CASES/<pre>cases.idx]\n");
   fprintf(stderr,
                       ...
                       char **fin, char **fout)
{
   ...
   *pre = 'd';
   ...
   for (i=1; i < nargs; i++)
   {
      ...
      switch(args[i][1])
      {
      case 'o': /* -o <fname> : output filename [<pre>R2K.sum] */
         if (++i >= nargs)
            PrintUsage(args[0], i, NULL);
         *fout = args[i];
         break;
      case 'i': /* -i <fnam> : kernel index file [R2CASES/<pre>cases.idx] */
         if (++i >= nargs)
            PrintUsage(args[0], i, NULL);
         *fin = args[i];
         break;
...
"# ------------------------------------------------------------------------\n");
      ...
      fprintf(fpout,
         "# Following %d GER2 kernels are optimized for %s\n",
...
)
{
   ...
   FillInR2ExtractGenStrings(pre, khi);
   ...
   {
      klo->minM = klo->MU = i;
      FillInR2ExtractGenStrings(pre, klo);
 ...
 */
   ...
   kp->exflags = DupString("R2Kdir=EXTDIR");
   kp->MU = kp->minM = CL;
 ...
 */
   ...
   kp->next = b2;
   FillInR2ExtractGenStrings(pre, kb);
   #ifndef DEBUG
 ...
 */
      ...
      {
         r1p->CacheElts = 0;
         r1p->mflop[2] = TimeR2Kernel(VERB, 0, r1p, pre, (Mooc/r1p->MU)*r1p->MU,
      ...
      for (r1p=r1b; r1p; r1p = r1p->next)
      {
         if (r1p->ID >= 900000 && r1p->ID < 1000000)
         {
            if (r1p->exflags)
            {
               ...
               r1p->exflags = DupString("R2Kdir=EXTDIR");
         }
      }
      if (r1b->mflop[3] <= 0.0)
      {
 ...
 */
   ...
   if (r1gen)
   {
      ...
      r1gen->ID = genID++;
      FillInR2ExtractGenStrings(pre, r1gen);
      r1gen->next = r1r;
      r1r = r1gen;
   }
   else r1gA = NULL;
