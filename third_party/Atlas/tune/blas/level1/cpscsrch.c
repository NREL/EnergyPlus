                 ...
                 int n, int *ix, int *iy, int *ia, int *ib)
{
   ...
   for (i=0; i < n; i++)
   {
      if (ia[i] == alpha)
      {
         ...
         fprintf(fpout, "%s      %s(N, alpha, X, incx, Y, incy);\n",
 ...
 */
   ...
   fprintf(fpout,
      "void ATL_%ccopy(const int, const TYPE*, const int, TYPE*, const int);\n",
           pre);
   if (pre == 'c' || pre == 'z')
      fprintf(fpout, "void Mjoin(PATLU,cpsc)\n   (const int, const TYPE, const TYPE*, const int, TYPE*, const int);\n");

   fprintf(fpout, "\nvoid ATL_%ccpsc(const int N, const SCALAR alpha, const TYPE *X, const int incX,\n", pre);
   ...
   if (pre == 'c' || pre == 'z')
   {
      ...
      fprintf(fpout,
         "%s{\n", spc);
      fprintf(fpout,
         "%s   Mjoin(PATLU,cpsc)(N+N, *alpha, X, 1, Y, 1);\n", spc);
      fprintf(fpout,
         "%s   return;\n", spc);
      fprintf(fpout,
         "%s}\n", spc);
 ...
 */
{
   ...
   for (i=0; i < n; i++)
   {
      ...
      fpout = fopen(ln, "w");
      assert(fpout);
      fprintf(fpout, "#define ATL_UCPSC %s\n\n", nam);
