...
namespace FenestrationCommon
{
                                                              ...
                                                              Combine const t_Combination) const
    {
        ...
        set_union(
          t_wv1.begin(), t_wv1.end(), t_wv2.begin(), t_wv2.end(), back_inserter(unionWavelengths));

        if(t_Combination == Combine::Interpolate)
        {
            // Remove extrapolated data. It is incorrect to have extrapolated wavelengths from one
            // sample
