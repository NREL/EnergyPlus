...
TEST_F(TestEnclosure2DBeam1, Enclosure2DBeam1) {
  ...
  double percentHit = 1;
  BeamViewFactor aVF1 =
      BeamViewFactor(enclosureIndex, segmentIndex, viewFactor, percentHit);
  ...
  for (size_t i = 0; i < correctResults.size(); ++i) {
    EXPECT_EQ(correctResults[i].enclosureIndex,
              (*aViewFactors)[i].enclosureIndex);
    ...
    EXPECT_NEAR(correctResults[i].percentHit, (*aViewFactors)[i].percentHit,
                1e-6);
  }
}
