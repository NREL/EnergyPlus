...
class TestMultiLayerOpticsSample : public testing::Test
{
private:
...
protected:
    virtual void SetUp()
    {
        CSeries solarRadiation({{0.50, 1026.70},
                                {0.51, 1066.70},
                                {0.52, 1011.50},
                                {0.53, 1084.90},
                                {0.54, 1082.40},
        ...
        m_MultiLayerOptics =
          std::make_shared<CMultiPaneSpectralSample>(multiSample, solarRadiation);
    }
...
TEST_F(TestMultiLayerOpticsSample, TestDoublePaneProperties)
{
    SCOPED_TRACE("Begin Test: Double pane equivalent layer properties.");
    ...
    double transmittance =
      MultiLayerOptics->getProperty(lowLambda, highLambda, Property::T, Side::Front);

    EXPECT_NEAR(0.6155426, transmittance, 1e-6);

    double frontReflectance =
      MultiLayerOptics->getProperty(lowLambda, highLambda, Property::R, Side::Front);

    EXPECT_NEAR(0.2378844, frontReflectance, 1e-6);

    double backReflectance =
      MultiLayerOptics->getProperty(lowLambda, highLambda, Property::R, Side::Back);

    EXPECT_NEAR(0.1858481, backReflectance, 1e-6);

    double totalAbsorptance =
      MultiLayerOptics->getProperty(lowLambda, highLambda, Property::Abs, Side::Front);

    EXPECT_NEAR(0.1465731, totalAbsorptance, 1e-6);
}

TEST_F(TestMultiLayerOpticsSample, TestDoublePaneLayerAbsorptances)
{
