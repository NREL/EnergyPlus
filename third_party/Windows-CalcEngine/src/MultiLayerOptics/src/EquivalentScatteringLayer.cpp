...
{
    CEquivalentScatteringLayer::CEquivalentScatteringLayer(const double Tf_dir_dir,
        ...
        m_DiffuseLayer(std::make_shared<CEquivalentLayerSingleComponent>(
          Tf_dif_dif, Rf_dif_dif, Tb_dif_dif, Rb_dif_dif)),
        m_BeamLayer(std::make_shared<CEquivalentLayerSingleComponent>(
    ...
    CEquivalentScatteringLayer::CEquivalentScatteringLayer(CScatteringLayer & t_Layer,
                                                           ...
                                                           const double t_Phi) :
        m_Layer(CScatteringLayer(t_Layer))
    {
    ...
    void CEquivalentScatteringLayer::addLayer(const double Tf_dir_dir,
                                              const double Rf_dir_dir,
                                              const double Tb_dir_dir,
                                              const double Rb_dir_dir,
                                              const double Tf_dir_dif,
                                              ...
                                              const Side t_Side)
    {
        CScatteringSurface aFrontSurface(
          Tf_dir_dir, Rf_dir_dir, Tf_dir_dif, Rf_dir_dif, Tf_dif_dif, Rf_dif_dif);

        CScatteringSurface aBackSurface(
          Tb_dir_dir, Rb_dir_dir, Tb_dir_dif, Rb_dir_dif, Tb_dif_dif, Rb_dif_dif);

        CScatteringLayer aLayer(aFrontSurface, aBackSurface);
        addLayer(aLayer, t_Side);
    }

    void CEquivalentScatteringLayer::addLayer(CScatteringLayer & t_Layer,
                                              const Side t_Side,
                                              const double t_Theta,
                                              const double t_Phi)
    {
    ...
    double CEquivalentScatteringLayer::getPropertySimple(const PropertySimple t_Property,
                                                         ...
                                                         const double t_Phi)
    {
        return m_Layer.getPropertySimple(m_Layer.getMinLambda(),
                                         m_Layer.getMaxLambda(),
                                         t_Property,
                                         t_Side,
                                         t_Scattering,
                                         t_Theta,
    ...
    CScatteringLayer CEquivalentScatteringLayer::getLayer() const
    {
        return m_Layer;
    }

    void CEquivalentScatteringLayer::calcEquivalentProperties(CScatteringLayer & t_First,
                                                              CScatteringLayer & t_Second)
    {
        ...
        m_Layer = CScatteringLayer(aFrontSurface, aBackSurface);
    }

    double CEquivalentScatteringLayer::getInterreflectance(const CScatteringSurface & t_First,
    ...
    std::shared_ptr<SimpleResults>
      CEquivalentScatteringLayer::calcDirectDiffuseTransAndRefl(const CScatteringSurface & f1,
    ...
    void CEquivalentScatteringLayer::addLayerComponents(CScatteringLayer & t_Layer,
                                                        const Side t_Side,
                                                        const double t_Theta,
