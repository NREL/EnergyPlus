...
namespace SpectralAveraging
{
    ...
    std::shared_ptr<CSpectralSample>
      CSingleAngularMeasurement::Interpolate(double const t_Angle,
                                             std::shared_ptr<CSpectralSample> const & t_Data1,
                                             double const t_Angle1,
    ...
    void
      CSingleAngularMeasurement::interpolate(std::vector<double> const & t_CommonWavelengths) const
    {
    ...
    CAngularMeasurements::CAngularMeasurements(
      std::vector<std::shared_ptr<CSingleAngularMeasurement>> const & t_Measurements) :
        m_Measurements(t_Measurements)
    {}

    // then you add function that will accept measurements at different angles
    void CAngularMeasurements::addMeasurement(
      std::shared_ptr<CSingleAngularMeasurement> const & t_SingleMeasurement)
    {
      ...
      CAngularMeasurements::getMeasurements(double const t_Angle)
    {
        ...
        if(m_Measurements.size() == 1)
        {
            throw std::runtime_error(
              "A single set is found. Spectral and angular sample must have 2 sets at least.");
        }
