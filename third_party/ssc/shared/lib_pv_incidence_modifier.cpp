...
/// Supporting function for IAM functions to calculate transmissance of a module
/// cover at a specific angle- reference: duffie & beckman, Ch 5.3
double transmittance(
    ...
    double n_incoming, /* refractive index of incoming material, typically n_air
                          = 1.0 */
    double k, /* proportionality constant assumed to be 4 (1/m) for derivation
                 of Bouguer's law (set to zero to skip bougeur's law */
    double l_thick, /* material thickness (set to zero to skip Bouguer's law */
    double *_theta2_deg) /* thickness of cover material (m), usually 2 mm for
                            typical module */
{
  double theta1 = theta1_deg * M_PI / 180.0;
  double theta2 = asin(n_incoming / n_cover *
                       sin(theta1)); // snell's law, assuming n_air = 1.0
                                     // fresnel's equation for non-reflected
                                     // unpolarized radiation as an average of
                                     // perpendicular and parallel components
  double tr =
      1 - 0.5 * (pow(sin(theta2 - theta1), 2) / pow(sin(theta2 + theta1), 2) +
                 pow(tan(theta2 - theta1), 2) / pow(tan(theta2 + theta1), 2));

  if (_theta2_deg)
    *_theta2_deg = theta2 * 180 / M_PI;

  return tr * exp(-k * l_thick / cos(theta2));
}

/// Incidence angle modifier not normalized relative to normal incidence (used
/// as a supporting function to normalized IAM function)
double iam_nonorm(double theta, bool ar_glass) {
  ...
  if (ar_glass) {
    double theta2 = 1;
    double tau_coating =
        transmittance(theta, n_arc, n_air, k_arc, l_arc, &theta2);
...
/// Incidence angle modifier normalized relative to normal incidence- used by
/// 61853 model and PVWatts
double
iam(double theta,
    bool ar_glass) // jmf- we should rename this to something more descriptive
{
  if (theta < AOI_MIN)
    theta = AOI_MIN;
  if (theta > AOI_MAX)
...
double iamSjerpsKoomen(double n2, double incidenceAngleRadians) {
  ...
  if (incidenceAngleRadians == 0) {
    cor = 1.0;
  } else if (incidenceAngleRadians > 0.0 &&
             incidenceAngleRadians <= M_PI / 2.0) {

    double refrAng = asin(sin(incidenceAngleRadians) / n2);
    double r1 = (pow(sin(refrAng - incidenceAngleRadians), 2.0) /
                 pow(sin(refrAng + incidenceAngleRadians), 2.0));
    double r2 = (pow(tan(refrAng - incidenceAngleRadians), 2.0) /
                 pow(tan(refrAng + incidenceAngleRadians), 2.0));
    cor = 1.0 - 0.5 * (r1 + r2);
    cor /= 1.0 - r0;
  }
  return cor;
...
double calculateIrradianceThroughCoverDeSoto(double theta, double theta_z,
                                             double tilt, double G_beam,
                                             double G_sky, double G_gnd,
  ...
  // transmittance at angle normal to surface (0 deg), use 1 (deg) to avoid
  // numerical probs.
  ...
  if (antiReflectiveGlass) {
    double tau_coating =
        transmittance(theta, n_arc, 1.0, k_arc, l_arc, &theta_after_coating);
    tau_beam *= tau_coating;
  }
  tau_beam *=
      transmittance(theta_after_coating, n_glass,
                    (antiReflectiveGlass ? n_arc : 1.0), k_glass, l_glass);
  ...
  // calculate component incidence angle modifiers, D&B Chap. 5 eqn 5.12.1,
  // DeSoto'04
