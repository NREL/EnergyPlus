...
cableFamily::cableFamily() { voltage = 0.0; }

cableFamily::cableFamily(const cableFamily &obj) {
  voltage = obj.voltage;
  cables.resize(obj.cables.size());
  for (size_t k = 0; k < cables.size(); k++)
    cables[k] = cable(obj.cables[k]);
  initializeFlag = true;
}

// Initialize individual cable objects in cable family
void cableFamily::initialize_cables(size_t ncable) {
  initialize_cables((int)ncable);
}
void cableFamily::initialize_cables(int ncable) {
  cables.resize(ncable);
  for (int k = 0; k < ncable; k++)
    cables[k] = cable();
  initializeFlag = true;
}

void cableFamily::check_size(size_t nval) {
  const std::string msg = "Size mismatch: " + std::to_string(cables.size()) +
                          " vs " + std::to_string(nval);
  if (!initializeFlag)
    initialize_cables(nval);
...
double vessel::get_mobilization_cost() {
  return (get_rate() * mobilization_time);
}
