...
Manual Dispatch
*/
dispatch_manual_t::dispatch_manual_t(
    battery_t *Battery, double dt, double SOC_min, double SOC_max,
    ...
    std::vector<bool> dm_charge, std::vector<bool> dm_discharge,
    std::vector<bool> dm_gridcharge, std::vector<bool> dm_fuelcellcharge,
    std::map<size_t, double> dm_percent_discharge,
    std::map<size_t, double> dm_percent_gridcharge)
    : dispatch_t(Battery, dt, SOC_min, SOC_max, current_choice, Ic_max, Id_max,
                 Pc_max_kwdc, Pd_max_kwdc, Pc_max_kwac, Pd_max_kwac, t_min,
                 mode, battMeterPosition) {
  init_with_vects(dm_dynamic_sched, dm_dynamic_sched_weekend, dm_charge,
                  dm_discharge, dm_gridcharge, dm_fuelcellcharge,
...
dispatch_manual_t::dispatch_manual_t(const dispatch_t &dispatch)
    : dispatch_t(dispatch) {
  const dispatch_manual_t *tmp =
      dynamic_cast<const dispatch_manual_t *>(&dispatch);
  init_with_vects(tmp->_sched, tmp->_sched_weekend, tmp->_charge_array,
                  tmp->_discharge_array, tmp->_gridcharge_array,
                  tmp->_fuelcellcharge_array, tmp->_percent_discharge_array,
                  tmp->_percent_charge_array);
}
...
void dispatch_manual_t::prepareDispatch(size_t hour_of_year, size_t) {
  ...
  if (m_batteryPower->canDischarge) {
    _percent_discharge = _percent_discharge_array[iprofile];
  }
  if (m_batteryPower->canSystemCharge || m_batteryPower->canFuelCellCharge) {
    _percent_charge = 100.;
  }
  if (m_batteryPower->canGridCharge) {
    _percent_charge = _percent_charge_array[iprofile];
  }
}
void dispatch_manual_t::dispatch(size_t year, size_t hour_of_year,
                                 size_t step) {
...
bool dispatch_manual_t::check_constraints(double &I, size_t count) {
  ...
  if (!iterate) {
    ...
    // Don't let system (PV) export to grid if can still charge battery
    // (increase charging)
    if (m_batteryPower->powerSystemToGrid > low_tolerance &&
        m_batteryPower
            ->canSystemCharge && // only do if battery is allowed to charge
        _Battery->SOC() < m_batteryPower->stateOfChargeMax -
                              1.0 && // and battery SOC is less than max
        fabs(I) < fabs(m_batteryPower
                           ->currentChargeMax) && // and battery current is less
                                                  // than max charge current
        fabs(m_batteryPower->powerBatteryDC) <
            (m_batteryPower->powerBatteryChargeMaxDC -
             1.0) && // and battery power is less than max charge power
        I <= 0)      // and battery was not discharging
    {
      double dI = 0;
      if (fabs(m_batteryPower->powerBatteryDC) < tolerance)
        dI = (m_batteryPower->powerSystemToGrid * util::kilowatt_to_watt /
              _Battery->V());
      else
        dI = (m_batteryPower->powerSystemToGrid /
              fabs(m_batteryPower->powerBatteryAC)) *
             fabs(I);

      // Main problem will be that this tends to overcharge battery maximum SOC,
      // so check
    ...
    else if (m_batteryPower->meterPosition == dispatch_t::BEHIND && I < 0 &&
             m_batteryPower->powerGridToLoad > tolerance &&
    ...
    // Don't let battery export to the grid if behind the meter
             ...
             m_batteryPower->powerBatteryToGrid > tolerance) {
      if (fabs(m_batteryPower->powerBatteryAC) < tolerance)
        I -= (m_batteryPower->powerBatteryToGrid * util::kilowatt_to_watt /
              _Battery->V());
      else
        I -= (m_batteryPower->powerBatteryToGrid /
              fabs(m_batteryPower->powerBatteryAC)) *
             fabs(I);
    } else
      iterate = false;
