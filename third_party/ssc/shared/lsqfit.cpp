...
static int mp_fdjac2(mp_func funct, int m, int n, int *ifree, int npar,
                     double *x, double *fvec, double *fjac, int ldfjac,
                     double epsfcn, double *wa, void *priv, int *nfev,
                     double *step, double *dstep, int *dside, int *qulimited,
                     double *ulimit, int *ddebug, double *ddrtol,
                     double *ddatol);
static void mp_qrfac(int m, int n, double *a, int lda, int pivot, int *ipvt,
...
int mpfit(mp_func funct, int m, int npar, double *xall, mp_par *pars,
  ...
  if ((npar <= 0) || (conf.ftol <= 0) || (conf.xtol <= 0) || (conf.gtol <= 0) ||
      (conf.maxiter < 0) || (conf.stepfactor <= 0)) {
    info = MP_ERR_PARAM;
    goto CLEANUP;
  }
...
OUTER_LOOP:
  ...
  iflag = mp_fdjac2(funct, m, nfree, ifree, npar, xnew, fvec, fjac, ldfjac,
                    conf.epsfcn, wa4, private_data, &nfev, step, dstep, mpside,
...
L200:
   ...
   */
  mp_lmpar(nfree, fjac, ldfjac, ipvt, ifree, diag, qtf, delta, &par, wa1, wa2,
           wa3, wa4);
  /*
  ...
  if (qanylim == 0) {
    ...
    for (j = 0; j < nfree; j++) {
      ...
      sgnu = (ulim[j] >= 0) ? (+1) : (-1);
      sgnl = (llim[j] >= 0) ? (+1) : (-1);
      ulim1 = ulim[j] * (1 - sgnu * MP_MACHEP0) -
              ((ulim[j] == 0) ? (MP_MACHEP0) : 0);
      llim1 = llim[j] * (1 + sgnl * MP_MACHEP0) +
              ((llim[j] == 0) ? (MP_MACHEP0) : 0);
  ...
  if ((fabs(actred) <= MP_MACHEP0) && (prered <= MP_MACHEP0) &&
      (p5 * ratio <= one)) {
    info = MP_FTOL;
  }
...
static int mp_fdjac2(mp_func funct, int m, int n, int *ifree, int npar,
  ...
  if (has_numerical_deriv)
    for (j = 0; j < n; j++) { /* Loop thru free parms */
      ...
      if ((dside && dsidei == -1) ||
          (dside && dsidei == 0 && qulimited && ulimit && qulimited[j] &&
           (temp > (ulimit[j] - h)))) {
        h = -h;
      }
...
struct lsq_vars_struct {
  double *x;
  double *y;
  double (*function)(double _x, double *par, void *user_data);
  void *user_data;
};
...
static int mpcall(int m, int, double *p, double *dy, double **, void *vars) {
  ...
  double *y = v->y;
  for (int i = 0; i < m; i++)
    dy[i] = y[i] - v->function(x[i], p, v->user_data);
  return 0;
}

int lsqfit(double (*function)(double _x, double *par, void *user_data),
           void *user_data, double par[], size_t npar, double *xdata,
           double *ydata, size_t len, double tol, int maxit, int maxfc) {
  ...
  int info =
      mpfit(mpcall, (int)len, (int)npar, par, 0, &cfg, (void *)&v, &result) > 0;
...
int linlsqfit(double *slope, double *intercept, double *xdata, double *ydata,
              size_t len) {
