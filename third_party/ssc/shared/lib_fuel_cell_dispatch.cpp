...
FuelCellDispatch::FuelCellDispatch(FuelCell *fuelCell, size_t numberOfUnits,
                                   int dispatchOption, int shutdownOption,
                                   double dt_hour, double fixed_percent,
    ...
    : m_powerTotal_kW(0), m_numberOfUnits(numberOfUnits),
      m_dispatchOption(dispatchOption), m_shutdownOption(shutdownOption),
      dt_hour(dt_hour), m_fixed_percent(fixed_percent * 0.01),
      m_dispatchInput_kW(dispatchInput_kW), m_canCharge(canCharge),
      m_canDischarge(canDischarge), m_discharge_percent(discharge_percent),
      m_discharge_units(discharge_units), m_scheduleWeekday(scheduleWeekday),
      m_scheduleWeekend(scheduleWeekend) {
  // Convert percentages to fractions
  for (auto percent = m_discharge_percent.begin();
       percent != m_discharge_percent.end(); percent++) {
    percent->second *= 0.01;
  }
  // Could have no units
  if (numberOfUnits > 0) {
  ...
  for (size_t fc = 0; fc < numberOfUnits; fc++) {
    ...
    if (!m_fuelCellVector[fc]->isInitialized()) {
      ...
      if (m_dispatchOption == FuelCellDispatch::FC_DISPATCH_OPTION::FIXED) {
        power_kW =
            m_fuelCellVector[fc]->getMaxPowerOriginal() * m_fixed_percent;
      }
      // Assume running at full
      else if (m_dispatchOption ==
               FuelCellDispatch::FC_DISPATCH_OPTION::LOAD_FOLLOW) {
        power_kW = m_fuelCellVector[fc]->getMaxPowerOriginal();
      }
      // Assume running at hour 1 level
      else if (m_dispatchOption ==
               FuelCellDispatch::FC_DISPATCH_OPTION::MANUAL) {
        ...
        power_kW = on * discharge_percent_init *
                   m_fuelCellVector[fc]->getMaxPowerOriginal();
      }
      // Assume running at hour 1 level
      else {
...
void FuelCellDispatch::runSingleTimeStep(size_t hour_of_year, size_t year_idx,
                                         double powerSystem_kWac,
                                         double powerLoad_kWac) {
  ...
  if (m_dispatchOption == FuelCellDispatch::FC_DISPATCH_OPTION::FIXED) {
    for (size_t fc = 0; fc < m_fuelCellVector.size(); fc++) {
      ...
      m_powerMaxPercentAverage_percent +=
          m_fuelCellVector[fc]->getPowerMaxPercent() / m_numberOfUnits;
      m_loadAverage_percent +=
          m_fuelCellVector[fc]->getPercentLoad() / m_numberOfUnits;
      m_efficiencyAverage_percent +=
          m_fuelCellVector[fc]->getElectricalEfficiency() * 100.0 /
          m_numberOfUnits;
      m_powerThermalTotal_kW += m_fuelCellVector[fc]->getPowerThermal();
      m_fuelConsumedTotal_MCf += m_fuelCellVector[fc]->getFuelConsumption();
    }
  }
  // Specified to follow load, fuel cell attempts to make up difference of
  // system - load
  else if (m_dispatchOption ==
           FuelCellDispatch::FC_DISPATCH_OPTION::LOAD_FOLLOW) {
    for (size_t fc = 0; fc < m_fuelCellVector.size(); fc++) {
  ...
  // Specified to follow manual dispatch input (add logic)
    ...
    for (size_t fc = 0; fc < m_numberOfUnits; fc++) {
      ...
      double on = fc < numberOfUnitsToRun ? 1.0 : 0.0;
      double power_kW =
          on * discharge_percent * m_fuelCellVector[fc]->getMaxPowerOriginal();
...
void FuelCellDispatch::setManualDispatchUnits(
    std::map<size_t, size_t> unitsByPeriod) {
  if (unitsByPeriod.size() == m_discharge_units.size()) {
    m_discharge_units = unitsByPeriod;
  }
}

double FuelCellDispatch::getPower() { return m_powerTotal_kW; }

double FuelCellDispatch::getPowerMaxPercent() {
  return m_powerMaxPercentAverage_percent;
}

double FuelCellDispatch::getPercentLoad() { return m_loadAverage_percent; }

double FuelCellDispatch::getElectricalEfficiencyPercent() {
  return m_efficiencyAverage_percent;
}

double FuelCellDispatch::getPowerThermal() { return m_powerThermalTotal_kW; }
