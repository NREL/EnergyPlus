...
UtilityRate::UtilityRate(bool useRealTimePrices,
  ...
  m_useRealTimePrices = useRealTimePrices, m_ecWeekday = ecWeekday;
...
UtilityRateCalculator::UtilityRateCalculator(UtilityRate *rate,
                                             size_t stepsPerHour)
    : UtilityRate(*rate) {
...
UtilityRateForecast::UtilityRateForecast(
    rate_data *util_rate, size_t stepsPerHour,
    const std::vector<double> &monthly_load_forecast,
    const std::vector<double> &monthly_gen_forecast,
    const std::vector<double> &monthly_avg_load_forecast,
    size_t analysis_period)
    : current_composite_buy_rates(), current_composite_sell_rates(),
      next_composite_buy_rates(), next_composite_sell_rates() {
...
UtilityRateForecast::UtilityRateForecast(UtilityRateForecast &tmp)
    : steps_per_hour(tmp.steps_per_hour), dt_hour(tmp.dt_hour),
      ...
      last_month_init(tmp.last_month_init), nyears(tmp.nyears) {
...
double UtilityRateForecast::forecastCost(std::vector<double> &predicted_loads,
                                         size_t year, size_t hour_of_year,
                                         size_t step) {
  double cost = 0;
  int month = util::month_of((double)hour_of_year) - 1;
  size_t lifeTimeIndex =
      util::lifetimeIndex(year, hour_of_year, step, steps_per_hour);
  ...
  int month_at_end =
      util::month_of((double)index_at_end / (double)steps_per_hour) - 1;
  ...
  if (rate->enable_nm) {
    previousEnergyCharge = getEnergyChargeNetMetering(
        month, current_composite_buy_rates, current_composite_sell_rates);
  }
  if (crossing_month) {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (current_month != month && !use_next_month) {
      use_next_month = true;
      if (rate->enable_nm) {
        // restartMonth will change the sign of energyUse from what
        // getEnergyCharge expects, so compute that first
        newEnergyCharge += getEnergyChargeNetMetering(
            month, current_composite_buy_rates, current_composite_sell_rates);
      }
    ...
    cost += getEnergyChargeNetBillingOrTimeSeries(
        energy, year_one_index, current_month, current_year, use_next_month);
  ...
  // If forecast length is 1, restartMonth won't be triggered on the next
  // forecast. Trigger it now
  ...
  if (crossing_month) {
    ...
    if (rate->enable_nm) {
      newEnergyCharge += getEnergyChargeNetMetering(
          month_at_end, next_composite_buy_rates, next_composite_sell_rates);
    }
  ...
  cost += newDemandCharge + newEnergyCharge - previousDemandCharge -
          previousEnergyCharge;
...
void UtilityRateForecast::compute_next_composite_tou(int month, size_t year) {
    ...
    for (size_t ir = 0; ir < num_per; ir++) {
      ...
      for (size_t ic = 0; ic < curr_month.ec_tou_ub.ncols() && !done; ic++) {
        ...
        if (expected_load > ub_tier) {
          periodCost += (ub_tier - prev_tier) / expected_load *
                        curr_month.ec_tou_br.at(ir, ic) * rate_esc;
        } else {
          periodCost += (expected_load - prev_tier) / expected_load *
                        curr_month.ec_tou_br.at(ir, ic) * rate_esc;
          done = true;
        }
      }
  ...
  if (expected_gen > 0) {
    for (size_t ir = 0; ir < num_per; ir++) {
      ...
      // Including the NM credits in the cost function can skew the price
      // signals, causing periods to appear to have higher cost than they
      // actually do. Assume $0 sell rate for NM
      if (!rate->nm_credits_w_rollover) {
        for (size_t ic = 0; ic < curr_month.ec_tou_ub.ncols() && !done; ic++) {
          ...
          if (expected_gen > ub_tier) {
            periodSellRate += (ub_tier - prev_tier) / expected_gen *
                              curr_month.ec_tou_sr.at(ir, ic) * rate_esc;
          } else {
            periodSellRate += (expected_gen - prev_tier) / expected_gen *
                              curr_month.ec_tou_sr.at(ir, ic) * rate_esc;
            done = true;
          }
        }
...
void UtilityRateForecast::initializeMonth(int month, size_t year) {
  if (last_month_init != month) {
    ...
    // Ignore any peak charges lower than the average gross load - this prevents
    // the price signal from showing demand charges on the first hour of each
    // month when the load is not really a peak
...
void UtilityRateForecast::copyTOUForecast() {
  ...
  std::copy(next_composite_buy_rates.begin(), next_composite_buy_rates.end(),
            std::back_inserter(current_composite_buy_rates));
  std::copy(next_composite_sell_rates.begin(), next_composite_sell_rates.end(),
            std::back_inserter(current_composite_sell_rates));
}

void UtilityRateForecast::restartMonth(int prevMonth, int currentMonth,
                                       size_t year) {
  ...
  bool skip_rollover =
      (currentMonth == 0 && year == 0) ||
      (currentMonth == rate->net_metering_credit_month + 1) ||
      (currentMonth == 0 && rate->net_metering_credit_month == 11);
...
double UtilityRateForecast::getEnergyChargeNetMetering(
    int month, std::vector<double> &buy_rates,
    std::vector<double> &sell_rates) {
  double cost = 0;
  ur_month &curr_month = rate->m_month[month];
...
double UtilityRateForecast::getEnergyChargeNetBillingOrTimeSeries(
    double energy, size_t year_one_index, int current_month, size_t year,
    bool use_next_month) {
  double cost = 0;
  ...
  size_t rate_index =
      year < rate->rate_scale.size() ? year : rate->rate_scale.size() - 1;
  ...
  if (energy < 0) {
    if (rate->en_ts_buy_rate) {
      ...
      if (use_next_month) {
        cost +=
            next_composite_buy_rates[tou_period] *
            -energy; // rate esclation is handled in compute_next_composite_tou
      } else {
