...
ChargeController::ChargeController(dispatch_t *dispatch,
                                   battery_metrics_t *battery_metrics)
    : m_batteryMetrics(battery_metrics), m_dispatch(dispatch) {}

ACBatteryController::ACBatteryController(dispatch_t *dispatch,
                                         battery_metrics_t *battery_metrics,
                                         double efficiencyACToDC,
                                         double efficiencyDCToAC)
    : ChargeController(dispatch, battery_metrics) {
  std::unique_ptr<BatteryBidirectionalInverter> tmp(
      new BatteryBidirectionalInverter(efficiencyACToDC, efficiencyDCToAC));
  ...
  m_batteryPower->singlePointEfficiencyACToDC =
      m_bidirectionalInverter->ac_dc_efficiency();
  m_batteryPower->singlePointEfficiencyDCToAC =
      m_bidirectionalInverter->dc_ac_efficiency();
}

void ACBatteryController::run(size_t year, size_t hour_of_year,
                              size_t step_of_hour, size_t) {
...
DCBatteryController::DCBatteryController(dispatch_t *dispatch,
                                         battery_metrics_t *battery_metrics,
                                         double efficiencyDCToDC,
                                         double inverterEfficiencyCutoff)
    : ChargeController(dispatch, battery_metrics) {
  std::unique_ptr<Battery_DC_DC_ChargeController> tmp(
      new Battery_DC_DC_ChargeController(efficiencyDCToDC, 100));
  m_DCDCChargeController = std::move(tmp);
  m_batteryPower = dispatch->getBatteryPower();
  m_batteryPower->connectionMode = ChargeController::DC_CONNECTED;
  m_batteryPower->singlePointEfficiencyDCToDC =
      m_DCDCChargeController->batt_dc_dc_bms_efficiency();
...
void DCBatteryController::run(size_t year, size_t hour_of_year,
                              size_t step_of_hour, size_t) {
  if (m_batteryPower->powerSystem < 0) {
    m_batteryPower->powerSystem = 0;
  }

  // For DC connected system, there is potentially full PV power going through
  // shared inverter
  m_batteryPower->powerSystemThroughSharedInverter =
      m_batteryPower->powerSystem;
