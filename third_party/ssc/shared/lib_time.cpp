 ...
 * as needed.  As an example, consider that solar generation is passed in as a
 * 15-minute, 25 year vector, and the electric load is currently a single-year
 ...
 * \param[in] singleyear_vector (the single year vector to scale to lifetime and
 * interpolate) \param[in] scale_factor (scaling factors for years 2 through n,
 * must be length n prior to calling this function) \param[in]
 * interpolation_factor (scaling as needed between single year records and the
 * interpolated records. Given annual data, should be 1 for power, and 1/dt_hour
 * for energy) \param[out] lifetime_from_singleyear_vector (the lifetime,
 * interpolated vector) \param[out] n_rec_single_year (the length of a single
 * year vector, interpolated at the lifetime vector timescale) \param[out]
 * dt_hour (the time step in hours)
 */
template <class T>
void single_year_to_lifetime_interpolated(
    bool is_lifetime, size_t n_years, size_t n_rec_lifetime,
    std::vector<T> singleyear_vector, std::vector<T> scale_factor,
    double interpolation_factor,
    std::vector<T> &lifetime_from_singleyear_vector, size_t &n_rec_single_year,
    double &dt_hour) {
  ...
  if (step_per_hour == 0)
    throw std::runtime_error("single_year_to_lifetime_interpolated error: "
                             "Calculated step_per_hour was 0.");

  // Parse single year properties
  double dt_hour_singleyear_input =
      (double)(util::hours_per_year) / (double)(singleyear_vector.size());
    ...
    // Downsample single year vector to dt_hour
      ...
      for (size_t h = 0; h < util::hours_per_year; h++) {
        for (size_t sy = 0; sy < step_per_hour; sy++) {
          // eventually add more sophisticated downsampling, ignoring
          // information
          singleyear_sampled.push_back(
              singleyear_vector[(size_t)(sy_idx / step_factor)] /
              interpolation_factor);
          sy_idx++;
        }
      }
...
template void single_year_to_lifetime_interpolated<double>(
    bool, size_t, size_t, std::vector<double>, std::vector<double>, double,
    std::vector<double> &, size_t &, double &);
template void single_year_to_lifetime_interpolated<float>(
    bool, size_t, size_t, std::vector<float>, std::vector<float>, double,
    std::vector<float> &, size_t &, double &);
 ...
 */
template <class T>
std::vector<T> flatten_diurnal(util::matrix_t<size_t> weekday_schedule,
                               util::matrix_t<size_t> weekend_schedule,
                               size_t steps_per_hour,
                               std::vector<T> period_values, T multiplier) {
...
flatten_diurnal(util::matrix_t<size_t> weekday_schedule,
                util::matrix_t<size_t> weekend_schedule, size_t steps_per_hour,
                std::vector<double> period_values, double multiplier);
 ...
 * multiplier and returns n output vector of the extrapolated values throughout
 * the entire year
 *
 * \param[in] steps_per_hour - Number of time steps per hour
 ...
 */
template <class T>
std::vector<T> extrapolate_timeseries(std::vector<T> input_values,
                                      size_t steps_per_hour, T multiplier) {
  ...
  for (size_t hour_of_year = 0; hour_of_year != 8760; hour_of_year++) {
    ...
    if (month > 0)
      month--; // month_of is 1 based and all other time functions are 0 based.
    week = util::week_of(hour_of_year);
    day = util::day_of(hour_of_year);
    hour = hour_of_year;
    for (size_t s = 0; s < steps_per_hour; s++) {
      minute_step =
          (size_t)((T)s * (T)input_steps_per_hour / (T)steps_per_hour);
      if (input_size == 12)
        extrapolated_value = input_values[month];
      else if (input_size == 52)
        extrapolated_value = input_values[week];
      else if (input_size == 365)
        extrapolated_value = input_values[day];
      else if (input_size == 8760)
        extrapolated_value = input_values[hour];
      else if (input_size > 8760 &&
               (hour * input_steps_per_hour + minute_step) < input_size)
        extrapolated_value =
            input_values[hour * input_steps_per_hour + minute_step];
      else // throw?
...
template std::vector<double>
extrapolate_timeseries(std::vector<double> input_values, size_t steps_per_hour,
                       double multiplier);
