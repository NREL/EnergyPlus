...
SharedInverter::SharedInverter(int inverterType, size_t numberOfInverters,
  ...
  if (m_inverterType == SANDIA_INVERTER ||
      m_inverterType == DATASHEET_INVERTER ||
      m_inverterType == COEFFICIENT_GENERATOR)
    m_nameplateAC_kW =
        m_numInverters * m_sandiaInverter->Paco * util::watt_to_kilowatt;
  else if (m_inverterType == PARTLOAD_INVERTER)
    m_nameplateAC_kW =
        m_numInverters * m_partloadInverter->Paco * util::watt_to_kilowatt;
  else if (m_inverterType == OND_INVERTER)
    m_nameplateAC_kW =
        m_numInverters * m_ondInverter->PMaxOUT * util::watt_to_kilowatt;
...
int SharedInverter::setTempDerateCurves(
  ...
  for (size_t r = 0; r < derateCurves.size(); r++) {
    ...
    for (size_t p = 0; p < tempSlopeEntries / 2; p++) {
      if (derateCurves[r][2 * p + 1] <= -273. ||
          derateCurves[r][2 * p + 2] > 0.)
        return (int)r + 1;
    }
  ...
  // Sort by DC voltage
  std::sort(m_thermalDerateCurves.begin(), m_thermalDerateCurves.end(),
            sortByVoltage);
...
void SharedInverter::findPointOnCurve(size_t idx, double T, double &startT,
                                      double &slope) {
  size_t p = 0;
  while (2 * p + 2 < m_thermalDerateCurves[idx].size() &&
         T >= m_thermalDerateCurves[idx][2 * p + 1]) {
    p++;
  }
...
void SharedInverter::calculateTempDerate(double V, double tempC,
                                         double &p_dc_rated, double &ratio,
                                         double &loss) {
  if (ratio == 0. || p_dc_rated == 0.)
    return;
  ...
  while (idx < m_thermalDerateCurves.size() &&
         V > m_thermalDerateCurves[idx][0]) {
    idx++;
  }
  ...
  // Use temp and slope of lower and upper curves for interpolation if they both
  // exist
    ...
    size_t n = std::max(m_thermalDerateCurves[idx].size() / 2,
                        m_thermalDerateCurves[idx - 1].size() / 2);
    size_t count = 0;
  ...
  // otherwise extrapolate using first start temps of each curve in order to
  // avoid inconsistent start temps
  else {
    if (idx == 0) {
      Vdc2 = m_thermalDerateCurves[idx][0];
      ...
      startTInterpolated =
          (startT2 - startT) / (Vdc2 - Vdc) * (V - Vdc2) + startT2;
...
double SharedInverter::getInverterDCMaxPower(double p_dc_rated) {
  ...
  if (m_inverterType == SANDIA_INVERTER ||
      m_inverterType == DATASHEET_INVERTER ||
      m_inverterType == COEFFICIENT_GENERATOR)
    // m_sandiaInverter->acpower(std::fabs(powerDC_Watts) / m_numInverters,
    // DCStringVoltage, &powerAC_Watts, &P_par, &P_lr, &efficiencyAC,
    // &powerClipLoss_kW, &powerConsumptionLoss_kW, &powerNightLoss_kW);
    inv_dc_max_power = m_sandiaInverter->Pdco;
  else if (m_inverterType == PARTLOAD_INVERTER)
    // m_partloadInverter->acpower(std::fabs(powerDC_Watts) / m_numInverters,
    // &powerAC_Watts, &P_lr, &P_par, &efficiencyAC, &powerClipLoss_kW,
    // &powerNightLoss_kW);
    inv_dc_max_power = m_partloadInverter->Pdco;
  else if (m_inverterType == OND_INVERTER)
    // m_ondInverter->acpower(std::fabs(powerDC_Watts) / m_numInverters,
    // DCStringVoltage, tempC, &powerAC_Watts, &P_par, &P_lr, &efficiencyAC,
    // &powerClipLoss_kW, &powerConsumptionLoss_kW, &powerNightLoss_kW,
    // &dcWiringLoss_ond_kW, &acWiringLoss_ond_kW);
...
// function that calculates AC power and inverter losses for a single inverter
// with one MPPT input
void SharedInverter::calculateACPower(const double powerDC_kW_in,
                                      const double DCStringVoltage,
                                      double tempC) {
  double P_par, P_lr;
  ...
  if (m_tempEnabled) {
    calculateTempDerate(DCStringVoltage, tempC, powerDC_Watts, power_ratio,
                        tempLoss);
  }
  ...
  if (m_inverterType == SANDIA_INVERTER ||
      ...
      m_inverterType == COEFFICIENT_GENERATOR)
    m_sandiaInverter->acpower(std::fabs(powerDC_Watts) / m_numInverters,
                              DCStringVoltage, &powerAC_Watts, &P_par, &P_lr,
                              &efficiencyAC, &powerClipLoss_kW,
                              &powerConsumptionLoss_kW, &powerNightLoss_kW);
  else if (m_inverterType == PARTLOAD_INVERTER)
    m_partloadInverter->acpower(std::fabs(powerDC_Watts) / m_numInverters,
                                &powerAC_Watts, &P_lr, &P_par, &efficiencyAC,
                                &powerClipLoss_kW, &powerNightLoss_kW);
  else if (m_inverterType == OND_INVERTER)
    m_ondInverter->acpower(std::fabs(powerDC_Watts) / m_numInverters,
                           DCStringVoltage, tempC, &powerAC_Watts, &P_par,
                           &P_lr, &efficiencyAC, &powerClipLoss_kW,
                           &powerConsumptionLoss_kW, &powerNightLoss_kW,
                           &dcWiringLoss_ond_kW, &acWiringLoss_ond_kW);
  else if (m_inverterType == NONE) {
    powerClipLoss_kW = 0.;
  ...
  // Convert units to kW- no need to scale to system size because passed in as
  // power to total number of inverters
  powerDC_kW = powerDC_Watts * util::watt_to_kilowatt;
  convertOutputsToKWandScale(tempLoss, powerAC_Watts);

  // In event shared inverter is charging a battery only, need to re-convert to
  // negative power
  if (negativePower) {
...
/* This function takes input inverter DC power (kW) per MPPT input for a SINGLE
 * multi-mppt inverter, DC voltage (V) per input, and ambient temperature (deg
 * C), and calculates output for the total number of inverters in the system */
void SharedInverter::calculateACPower(const std::vector<double> powerDC_kW_in,
                                      const std::vector<double> DCStringVoltage,
                                      double tempC) {
  double P_par, P_lr;
  ...
  for (size_t i = 0; i < powerDC_kW_in.size(); i++) {
    powerDC_Watts_one_inv.push_back(powerDC_kW_in[i] * util::kilowatt_to_watt /
                                    m_numInverters);
  }
  Tdry_C = tempC;
  ...
  double power_ratio = 1.0;
  if (m_tempEnabled) {
    // use average of the DC voltages to pick which temp curve to use- a
    // weighted average might be better but we don't have that information here
    double avgDCVoltage = 0;
    double avgDCPower_Watts = 0;
    for (size_t i = 0; i < powerDC_Watts_one_inv.size(); i++) {
      power_ratio = 1.0;
      calculateTempDerate(DCStringVoltage[i], tempC, powerDC_Watts_one_inv[i],
                          power_ratio, tempLoss[i]);
    }
  }
  // Power quantities go in and come out in units of W
  ...
  if (m_inverterType == SANDIA_INVERTER ||
      m_inverterType == DATASHEET_INVERTER ||
      m_inverterType == COEFFICIENT_GENERATOR)
    m_sandiaInverter->acpower(powerDC_Watts_one_inv, DCStringVoltage,
                              &powerAC_Watts, &P_par, &P_lr, &efficiencyAC,
                              &powerClipLoss_kW, &powerConsumptionLoss_kW,
                              &powerNightLoss_kW);
  else if (m_inverterType == PARTLOAD_INVERTER)
    m_partloadInverter->acpower(powerDC_Watts_one_inv, &powerAC_Watts, &P_lr,
                                &P_par, &efficiencyAC, &powerClipLoss_kW,
                                &powerNightLoss_kW);

  // Scale to total system size
  // Do not need to scale back up by m_numInverters because scaling them down
  // was a separate vector, powerDC_Watts_one_inv
  powerDC_kW = 0;
  double tempLoss_avg = 0;
  for (size_t i = 0; i < powerDC_Watts_one_inv.size(); i++) {
    powerDC_kW +=
        powerDC_Watts_one_inv[i] * util::watt_to_kilowatt * m_numInverters;
...
double SharedInverter::calculateRequiredDCPower(const double kwAC,
                                                const double DCStringV,
                                                double tempC) {
  ...
  newton<double, std::function<void(const double *, double *)>, 1>(
      x, resid, check, f, 100, 1e-6, 1e-6, 0.7);
...
double SharedInverter::getMaxPowerEfficiency() {
  if (m_inverterType == SANDIA_INVERTER ||
      m_inverterType == DATASHEET_INVERTER ||
      m_inverterType == COEFFICIENT_GENERATOR)
    calculateACPower(m_sandiaInverter->Paco * util::watt_to_kilowatt,
                     m_sandiaInverter->Vdco, 0.0);
  else if (m_inverterType == PARTLOAD_INVERTER)
    calculateACPower(m_partloadInverter->Paco * util::watt_to_kilowatt,
                     m_partloadInverter->Vdco, 0.0);
  else if (m_inverterType == OND_INVERTER)
    calculateACPower(m_ondInverter->PMaxOUT * util::watt_to_kilowatt,
                     m_ondInverter->VAbsMax, 0.0);

  return efficiencyAC;
}

double SharedInverter::getACNameplateCapacitykW() { return m_nameplateAC_kW; }
