...
dispatch_automatic_front_of_meter_t::dispatch_automatic_front_of_meter_t(
    ...
    double inverter_paco, std::vector<double> battReplacementCostPerkWh,
    int battCycleCostChoice, std::vector<double> battCycleCost,
    std::vector<double> forecast_price_series_dollar_per_kwh,
    UtilityRate *utilityRate, double etaPVCharge, double etaGridCharge,
    double etaDischarge)
  ...
  if (utilityRate) {
    std::unique_ptr<UtilityRateCalculator> tmp(
        new UtilityRateCalculator(utilityRate, _steps_per_hour));
  ...
  revenueToClipCharge = revenueToDischarge = revenueToGridCharge =
      revenueToPVCharge = 0;
...
dispatch_automatic_front_of_meter_t::
    ~dispatch_automatic_front_of_meter_t() { /* NOTHING TO DO */
}
void dispatch_automatic_front_of_meter_t::init_with_pointer(
    const dispatch_automatic_front_of_meter_t *tmp) {
...
dispatch_automatic_front_of_meter_t::dispatch_automatic_front_of_meter_t(
  ...
  const dispatch_automatic_front_of_meter_t *tmp =
      dynamic_cast<const dispatch_automatic_front_of_meter_t *>(&dispatch);
...
void dispatch_automatic_front_of_meter_t::update_dispatch(
    size_t year, size_t hour_of_year, size_t, size_t lifetimeIndex) {
  // Initialize
  ...
  if (_mode != dispatch_t::FOM_CUSTOM_DISPATCH) {
    ...
    auto max_ppa_cost = std::max_element(
        _forecast_price_rt_series.begin() + idx_year1,
        _forecast_price_rt_series.begin() + idx_year1 + idx_lookahead);
    auto min_ppa_cost = std::min_element(
        _forecast_price_rt_series.begin() + idx_year1,
        _forecast_price_rt_series.begin() + idx_year1 + idx_lookahead);
    ...
    if (m_utilityRateCalculator) {
      ...
      for (size_t i = hour_of_year; i < hour_of_year + _forecast_hours; i++) {
        for (size_t s = 0; s < _steps_per_hour; s++) {
          usage_cost_forecast.push_back(
              m_utilityRateCalculator->getEnergyRate(i % 8760));
        }
      }
    }
    ...
    if (_P_cliploss_dc.size() > lifetimeIndex + _forecast_hours) {
      energyToStoreClipped =
          std::accumulate(_P_cliploss_dc.begin() + lifetimeIndex,
                          _P_cliploss_dc.begin() + lifetimeIndex +
                              _forecast_hours * _steps_per_hour,
                          0.0) *
          _dt_hour;
    }

    /*! Economic benefit of charging from the grid in current time step to
     * discharge sometime in next X hours ($/kWh)*/
    revenueToGridCharge = *max_ppa_cost * m_etaDischarge -
                          usage_cost / m_etaGridCharge - m_cycleCost;
      ...
      for (size_t i = idx_year1; i < idx_year1 + idx_lookahead; i++) {
        if (m_utilityRateCalculator) {
          revenueToGridChargeForecast.push_back(
              *max_ppa_cost * m_etaDischarge -
              usage_cost_forecast[j] / m_etaGridCharge - m_cycleCost);
        } else {
          revenueToGridChargeForecast.push_back(
              *max_ppa_cost * m_etaDischarge -
              _forecast_price_rt_series[i] / m_etaGridCharge - m_cycleCost);
        }
        j++;
      }
      revenueToGridChargeMax =
          *std::max_element(std::begin(revenueToGridChargeForecast),
                            std::end(revenueToGridChargeForecast));
    }

    /*! Economic benefit of charging from regular PV in current time step to
     * discharge sometime in next X hours ($/kWh)*/
    revenueToPVCharge = _P_pv_ac[idx_year1] > 0
                            ? *max_ppa_cost * m_etaDischarge -
                                  ppa_cost / m_etaPVCharge - m_cycleCost
                            : 0;

    /*! Computed revenue to charge from PV in each of next X hours ($/kWh)*/
    size_t t_duration = static_cast<size_t>(
        ceilf((float)_Battery->energy_nominal() /
              (float)m_batteryPower->powerBatteryChargeMaxDC));
    size_t pv_hours_on;
    double revenueToPVChargeMax = 0;
    if (m_batteryPower->canSystemCharge) {
      std::vector<double> revenueToPVChargeForecast;
      for (size_t i = idx_year1; i < idx_year1 + idx_lookahead; i++) {
        // when considering grid charging, require PV output to exceed battery
        // input capacity before accepting as a better option
        bool system_on =
            _P_pv_ac[i] >= m_batteryPower->powerBatteryChargeMaxDC ? 1 : 0;
        if (system_on) {
          revenueToPVChargeForecast.push_back(
              system_on *
              (*max_ppa_cost * m_etaDischarge -
               _forecast_price_rt_series[i] / m_etaPVCharge - m_cycleCost));
        }
      }
      pv_hours_on = revenueToPVChargeForecast.size() / _steps_per_hour;
      revenueToPVChargeMax =
          pv_hours_on >= t_duration
              ? *std::max_element(std::begin(revenueToPVChargeForecast),
                                  std::end(revenueToPVChargeForecast))
              : 0;
    }

    /*! Economic benefit of charging from clipped PV in current time step to
     * discharge sometime in the next X hours (clipped PV is free) ($/kWh) */
    ...
    double energyNeededToFillBattery =
        _Battery->energy_to_fill(m_batteryPower->stateOfChargeMax);
    ...
    bool excessAcCapacity =
        _inverter_paco > m_batteryPower->powerSystemThroughSharedInverter;
    bool batteryHasDischargeCapacity =
        _Battery->SOC() >= m_batteryPower->stateOfChargeMin + 1.0;

    // Always Charge if PV is clipping
    if (m_batteryPower->canClipCharge &&
        m_batteryPower->powerSystemClipped > 0 && revenueToClipCharge > 0) {
      powerBattery = -m_batteryPower->powerSystemClipped;
    }

    // Increase charge from system (PV) if it is more valuable later than
    // selling now
    if (m_batteryPower->canSystemCharge &&
        // revenueToPVCharge >= revenueToGridChargeMax &&
        revenueToPVCharge > 0 && highChargeValuePeriod &&
      ...
      if (m_batteryPower->canClipCharge) {
        if (energyToStoreClipped < energyNeededToFillBattery) {
          double energyCanCharge =
              (energyNeededToFillBattery - energyToStoreClipped);
          if (energyCanCharge <= m_batteryPower->powerSystem * _dt_hour)
            powerBattery = -std::fmax(energyCanCharge / _dt_hour,
                                      m_batteryPower->powerSystemClipped);
          else
            powerBattery = -std::fmax(m_batteryPower->powerSystem,
                                      m_batteryPower->powerSystemClipped);

          energyNeededToFillBattery = std::fmax(
              0, energyNeededToFillBattery + (powerBattery * _dt_hour));
        }
    ...
    // Also charge from grid if it is valuable to do so, still leaving
    // EnergyToStoreClipped capacity in battery
    if (m_batteryPower->canGridCharge &&
        revenueToGridCharge >= revenueToPVChargeMax &&
        revenueToGridCharge > 0 && highChargeValuePeriod &&
    ...
    // Discharge if we are in a high-price period and have battery and inverter
    // capacity
    if (highDischargeValuePeriod && revenueToDischarge > 0 &&
        excessAcCapacity && batteryHasDischargeCapacity) {
      double loss_kw =
          _Battery->calculate_loss(m_batteryPower->powerBatteryTarget,
                                   lifetimeIndex); // Battery is responsible for
                                                   // covering discharge losses
      if (m_batteryPower->connectionMode == BatteryPower::DC_CONNECTED) {
        powerBattery = _inverter_paco + loss_kw - m_batteryPower->powerSystem;
      } else {
        powerBattery = _inverter_paco; // AC connected battery is already maxed
                                       // out by AC power limit, cannot increase
                                       // dispatch to ccover losses
      }
    }
    // save for extraction
  ...
  // Custom dispatch
    ...
    m_batteryPower->powerBatteryTarget =
        _P_battery_use[lifetimeIndex % (8760 * _steps_per_hour)];
    double loss_kw = _Battery->calculate_loss(
        m_batteryPower->powerBatteryTarget,
        lifetimeIndex); // Battery is responsible for covering discharge losses
