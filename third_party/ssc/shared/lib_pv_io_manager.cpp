...
PVIOManager::PVIOManager(compute_module *cm, std::string cmName) {
  ...
  std::unique_ptr<PVSystem_IO> pvSystem(
      new PVSystem_IO(cm, cmName, m_SimulationIO.get(), m_IrradianceIO.get(),
                      getSubarrays(), m_InverterIO.get()));
  m_PVSystemIO = std::move(pvSystem);

  // Allocate outputs, moved to here due to previous difficult to debug crashes
...
Subarray_IO *PVIOManager::getSubarrayIO(size_t subarrayNumber) {
  return m_SubarraysIO[subarrayNumber].get();
}
...
Irradiance_IO::Irradiance_IO(compute_module *cm, std::string cmName) {
  ...
  if (cm->is_assigned("solar_resource_file")) {
    weatherDataProvider = std::unique_ptr<weather_data_provider>(
        new weatherfile(cm->as_string("solar_resource_file")));
    weatherfile *weatherFile =
        dynamic_cast<weatherfile *>(weatherDataProvider.get());
    if (!weatherFile->ok())
      throw exec_error(cmName, weatherFile->message());
    if (weatherFile->has_message())
      cm->log(weatherFile->message(), SSC_WARNING);
  } else if (cm->is_assigned("solar_resource_data")) {
    weatherDataProvider = std::unique_ptr<weather_data_provider>(
        new weatherdata(cm->lookup("solar_resource_data")));
    if (weatherDataProvider->has_message())
      cm->log(weatherDataProvider->message(), SSC_WARNING);
  } else {
    throw exec_error(cmName, "No weather data supplied");
  }

  // assumes instantaneous values, unless hourly file with no minute column
  // specified
  tsShiftHours = 0.0;
  instantaneous = true;
  ...
  } else if (weatherDataProvider->annualSimulation() &&
             weatherDataProvider->nrecords() == 8760) {
    // hourly file with no minute data column.  assume
    // integrated/averaged values and use mid point convention for interpreting
    // results
    instantaneous = false;
    tsShiftHours = 0.5;
  } else
    throw exec_error(cmName, "subhourly and non-annual weather files must "
                             "specify the minute for each record");
  ...
  if (weatherDataProvider->annualSimulation() &&
      numberOfWeatherFileRecords % 8760 != 0)
    throw exec_error(cmName,
                     util::format("invalid number of data records (%zu): must "
                                  "be an integer multiple of 8760",
                                  numberOfWeatherFileRecords));
  if (weatherDataProvider->annualSimulation() &&
      (stepsPerHour < 1 || stepsPerHour > 60))
    throw exec_error(
        cmName,
        util::format(
            "%d timesteps per hour found. Weather data should be single year.",
            stepsPerHour));
...
void Irradiance_IO::checkWeatherFile(compute_module *cm, std::string cmName) {
  for (size_t idx = 0; idx < numberOfWeatherFileRecords; idx++) {
    if (!weatherDataProvider->read(&weatherRecord))
      throw exec_error(cmName, "could not read data line " +
                                   util::to_string((int)(idx + 1)) +
                                   " in weather file");

    // Check for missing data
    if ((weatherRecord.gh != weatherRecord.gh) &&
        (radiationMode == irrad::DN_GH || radiationMode == irrad::GH_DF)) {
      cm->log(util::format("missing global irradiance %lg W/m2 at time [y:%d "
                           "m:%d d:%d h:%d], exiting",
                           weatherRecord.gh, weatherRecord.year,
                           weatherRecord.month, weatherRecord.day,
                           weatherRecord.hour),
              SSC_ERROR, (float)idx);
      return;
    }
    if ((weatherRecord.dn != weatherRecord.dn) &&
        (radiationMode == irrad::DN_DF || radiationMode == irrad::DN_GH)) {
      cm->log(util::format("missing beam irradiance %lg W/m2 at time [y:%d "
                           "m:%d d:%d h:%d], exiting",
                           weatherRecord.dn, weatherRecord.year,
                           weatherRecord.month, weatherRecord.day,
                           weatherRecord.hour),
              SSC_ERROR, (float)idx);
      return;
    }
    if ((weatherRecord.df != weatherRecord.df) &&
        (radiationMode == irrad::DN_DF || radiationMode == irrad::GH_DF)) {
      cm->log(util::format("missing diffuse irradiance %lg W/m2 at time [y:%d "
                           "m:%d d:%d h:%d], exiting",
                           weatherRecord.df, weatherRecord.year,
                           weatherRecord.month, weatherRecord.day,
                           weatherRecord.hour),
              SSC_ERROR, (float)idx);
      return;
    }
    if ((weatherRecord.poa != weatherRecord.poa) &&
        (radiationMode == irrad::POA_R || radiationMode == irrad::POA_P)) {
      cm->log(util::format("missing POA irradiance %lg W/m2 at time [y:%d m:%d "
                           "d:%d h:%d], exiting",
                           weatherRecord.poa, weatherRecord.year,
                           weatherRecord.month, weatherRecord.day,
                           weatherRecord.hour),
              SSC_ERROR, (float)idx);
      return;
    ...
    if (weatherRecord.tdry != weatherRecord.tdry) {
      cm->log(util::format("missing temperature %lg W/m2 at time [y:%d m:%d "
                           "d:%d h:%d], exiting",
                           weatherRecord.tdry, weatherRecord.year,
                           weatherRecord.month, weatherRecord.day,
                           weatherRecord.hour),
              SSC_ERROR, (float)idx);
      return;
    ...
    if (weatherRecord.wspd != weatherRecord.wspd) {
      cm->log(util::format("missing wind speed %lg W/m2 at time [y:%d m:%d "
                           "d:%d h:%d], exiting",
                           weatherRecord.wspd, weatherRecord.year,
                           weatherRecord.month, weatherRecord.day,
                           weatherRecord.hour),
              SSC_ERROR, (float)idx);
      return;
    ...
    // Check for bad data
    if ((weatherRecord.gh < 0 || weatherRecord.gh > irrad::irradiationMax) &&
        (radiationMode == irrad::DN_GH || radiationMode == irrad::GH_DF)) {
      cm->log(util::format("out of range global irradiance %lg W/m2 at time "
                           "[y:%d m:%d d:%d h:%d], set to zero",
                           weatherRecord.gh, weatherRecord.year,
                           weatherRecord.month, weatherRecord.day,
                           weatherRecord.hour),
              SSC_WARNING, (float)idx);
      weatherRecord.gh = 0;
    }
    if ((weatherRecord.dn < 0 || weatherRecord.dn > irrad::irradiationMax) &&
        (radiationMode == irrad::DN_DF || radiationMode == irrad::DN_GH)) {
      cm->log(util::format("out of range beam irradiance %lg W/m2 at time "
                           "[y:%d m:%d d:%d h:%d], set to zero",
                           weatherRecord.dn, weatherRecord.year,
                           weatherRecord.month, weatherRecord.day,
                           weatherRecord.hour),
    ...
    if ((weatherRecord.df < 0 || weatherRecord.df > irrad::irradiationMax) &&
        (radiationMode == irrad::DN_DF || radiationMode == irrad::GH_DF)) {
      cm->log(util::format("out of range diffuse irradiance %lg W/m2 at time "
                           "[y:%d m:%d d:%d h:%d], set to zero",
                           weatherRecord.df, weatherRecord.year,
                           weatherRecord.month, weatherRecord.day,
                           weatherRecord.hour),
    ...
    if ((weatherRecord.poa < 0 || weatherRecord.poa > irrad::irradiationMax) &&
        (radiationMode == irrad::POA_R || radiationMode == irrad::POA_P)) {
      cm->log(util::format("out of range POA irradiance %lg W/m2 at time [y:%d "
                           "m:%d d:%d h:%d], set to zero",
                           weatherRecord.poa, weatherRecord.year,
                           weatherRecord.month, weatherRecord.day,
                           weatherRecord.hour),
              SSC_WARNING, (float)idx);
    ...
    // albedo is allowed to be missing in the weather file- will be filled in
    // from user-entered monthly array. only throw an error if there's a value
    // that isn't reasonable somewhere
    int month_idx = weatherRecord.month - 1;
    bool albedoError = true;
    if (useWeatherFileAlbedo && std::isfinite(weatherRecord.alb) &&
        weatherRecord.alb > 0 && weatherRecord.alb < 1) {
      albedoError = false;
    } else if (month_idx >= 0 && month_idx < 12) {
      if (userSpecifiedMonthlyAlbedo[month_idx] > 0 &&
          userSpecifiedMonthlyAlbedo[month_idx] < 1) {
        albedoError = false;
    ...
    if (albedoError) {
      throw exec_error(
          cmName,
          util::format("Error retrieving albedo value: Invalid month in "
                       "weather file or invalid albedo value in weather file"));
    }
  }
  weatherDataProvider->rewind();
}
...
void Irradiance_IO::AllocateOutputs(compute_module *cm) {
  ...
  p_weatherFileSnowDepth =
      cm->allocate("snowdepth", numberOfWeatherFileRecords);

  // If using input POA, must have POA for every subarray or assume POA applies
  // to each subarray
  for (size_t subarray = 0; subarray != numberOfSubarrays; subarray++) {
    std::string wfpoa =
        "wfpoa" + util::to_string(static_cast<int>(subarray + 1));
    p_weatherFilePOA.push_back(cm->allocate(wfpoa, numberOfWeatherFileRecords));
  }

  // set up the calculated components of irradiance such that they aren't
  // reported if they aren't assigned three possible calculated irradiance: gh,
  // df, dn
  if (radiationMode == irrad::DN_DF)
    p_IrradianceCalculated[0] = cm->allocate(
        "gh_calc",
        numberOfWeatherFileRecords); // don't calculate global for POA models
  if (radiationMode == irrad::DN_GH || radiationMode == irrad::POA_R ||
      radiationMode == irrad::POA_P)
    p_IrradianceCalculated[1] =
        cm->allocate("df_calc", numberOfWeatherFileRecords);
  if (radiationMode == irrad::GH_DF || radiationMode == irrad::POA_R ||
      radiationMode == irrad::POA_P)
    p_IrradianceCalculated[2] =
        cm->allocate("dn_calc", numberOfWeatherFileRecords);
...
Subarray_IO::Subarray_IO(compute_module *cm, const std::string &cmName,
                         size_t subarrayNumber) {
  ...
  if (enable) {
    ...
    if (n < 0) {
      throw exec_error(
          cmName, "invalid string allocation between subarrays.  all subarrays "
                  "must have zero or positive number of strings.");
    }

    nStrings = n;
    if (nStrings ==
        0) // subarrays with no strings need to be treated as if they are
           // disabled to avoid divide by zero issues in the subarray setup
    {
      enable = false;
      return;
    }
    ...
    tiltEqualLatitude = 0;
    if (cm->is_assigned(prefix + "tilt_eq_lat"))
      tiltEqualLatitude = cm->as_boolean(prefix + "tilt_eq_lat");

    // tilt required for fixed tilt, single axis, and azimuth axis- can't check
    // for this in variable table so check here
    tiltDegrees = std::numeric_limits<double>::quiet_NaN();
    if (trackMode == irrad::FIXED_TILT || trackMode == irrad::SINGLE_AXIS ||
        trackMode == irrad::AZIMUTH_AXIS)
      if (!tiltEqualLatitude && !cm->is_assigned(prefix + "tilt"))
        throw exec_error(cmName, "Subarray " +
                                     util::to_string((int)subarrayNumber) +
                                     " tilt required but not assigned.");
    if (cm->is_assigned(prefix + "tilt"))
      tiltDegrees = fabs(cm->as_double(prefix + "tilt"));
    // monthly tilt required if seasonal tracking mode selected- can't check for
    // this in variable table so check here
    if (trackMode == irrad::SEASONAL_TILT &&
        !cm->is_assigned(prefix + "monthly_tilt"))
      throw exec_error(cmName, "Subarray " +
                                   util::to_string((int)subarrayNumber) +
                                   " monthly tilt required but not assigned.");
    if (cm->is_assigned(prefix + "monthly_tilt"))
      monthlyTiltDegrees = cm->as_vector_double(prefix + "monthly_tilt");
    // azimuth required for fixed tilt, single axis, and seasonal tilt- can't
    // check for this in variable table so check here
    azimuthDegrees = std::numeric_limits<double>::quiet_NaN();
    if (trackMode == irrad::FIXED_TILT || trackMode == irrad::SINGLE_AXIS ||
        trackMode == irrad::SEASONAL_TILT)
      if (!cm->is_assigned(prefix + "azimuth"))
        throw exec_error(cmName, "Subarray " +
                                     util::to_string((int)subarrayNumber) +
                                     " azimuth required but not assigned.");
    if (cm->is_assigned(prefix + "azimuth"))
    ...
    // check that backtracking input is assigned here because cannot check in
    // the variable table
    ...
    if (trackMode == irrad::SINGLE_AXIS)
      if (!cm->is_assigned(prefix + "backtrack"))
        throw exec_error(cmName, "Subarray " +
                                     util::to_string((int)subarrayNumber) +
                                     " backtrack required but not assigned.");
    if (cm->is_assigned(prefix + "backtrack"))
    ...
    // losses
    rearIrradianceLossPercent =
        cm->as_double(prefix + "rear_irradiance_loss") / 100;
    ...
    dcLossTotalPercent =
        1 - ((1 - dcOptimizerLossPercent) * (1 - mismatchLossPercent) *
             (1 - diodesLossPercent) * (1 - dcWiringLossPercent) *
             (1 - trackingLossPercent) * (1 - nameplateLossPercent));

    if (groundCoverageRatio < 0.01)
      throw exec_error(cmName,
                       "array ground coverage ratio must obey 0.01 < gcr");

    monthlySoiling = cm->as_vector_double(prefix + "soiling");
    if (monthlySoiling.size() != 12)
      throw exec_error(cmName,
                       "soiling loss array must have 12 values: subarray " +
                           util::to_string((int)(subarrayNumber)));
    ...
    if (!shadeCalculator.setup(cm, prefix)) {
      throw exec_error(cmName,
                       prefix + "_shading: " + shadeCalculator.get_error());
    }

    shadeMode = cm->as_integer(prefix + "shade_mode");
    selfShadingInputs.mod_orient = cm->as_integer(
        prefix +
        "mod_orient"); // although these inputs are stored in self-shading
                       // structure, they are also used for snow model and
                       // bifacial model, so required for all enabled subarrays
    ...
    if (trackMode == irrad::FIXED_TILT || trackMode == irrad::SEASONAL_TILT ||
        (trackMode == irrad::SINGLE_AXIS)) {
      if (shadeMode != NO_SHADING) {
        // Calculate the number of rows given the module dimensions of each row.
        selfShadingInputs.nrows =
            (int)floor((nStrings * nModulesPerString) /
                       (selfShadingInputs.nmodx * selfShadingInputs.nmody));

        // if nrows comes out to be zero, this will cause a divide by zero
        // error. Give an error in this case.
        if (selfShadingInputs.nrows == 0 && nStrings != 0)
          throw exec_error(
              cmName, "Self shading: Number of rows calculated for subarray " +
                          util::to_string(int(subarrayNumber)) +
                          " was zero. Please check your inputs.");

        // Otherwise, if self-shading configuration does not have equal number
        // of modules as specified on system design page for that subarray,
        // compute dc derate using the self-shading configuration and apply it
        // to the whole subarray. Give warning.
        if ((size_t)(selfShadingInputs.nmodx * selfShadingInputs.nmody *
                     selfShadingInputs.nrows) !=
            (size_t)(nStrings * nModulesPerString))
          cm->log(util::format(
                      "The product of number of modules along side and bottom "
                      "for subarray %d is not equal to the number of modules "
                      "in the subarray. Check your inputs for self shading.",
                      int(subarrayNumber)),
                  SSC_WARNING);

        // assume aspect ratio of 1.7 (see variable "aspect_ratio" below to
        // change this assumption)
        selfShadingInputs.str_orient = 1; // assume horizontal wiring
        selfShadingInputs.mask_angle_calc_method =
            0; // assume worst case mask angle calc method
        selfShadingInputs.ndiode =
            3; // assume 3 diodes- maybe update this assumption based on number
    ...
    if (subarrayEnableSnow) {
      if (trackMode == irrad::SEASONAL_TILT)
        throw exec_error(cmName, "Time-series tilt input may not be used with "
                                 "the snow model at this time: subarray " +
                                     util::to_string((int)(subarrayNumber)));
    ...
    std::unique_ptr<Module_IO> tmp(
        new Module_IO(cm, cmName, 1 - dcLossTotalPercent));
    Module = std::move(tmp);
  }
}
...
void PVSystem_IO::SetupPOAInput() {
  ...
  // Check if a POA model is used, if so load all POA data into the poaData
  // struct
  if (Irradiance->radiationMode == irrad::POA_R ||
      Irradiance->radiationMode == irrad::POA_P) {
    for (size_t nn = 0; nn < Subarrays.size(); nn++) {
      ...
      Subarrays[nn]->poa.poaAll->POA.reserve(
          Irradiance->numberOfWeatherFileRecords);
      Subarrays[nn]->poa.poaAll->inc.reserve(
          Irradiance->numberOfWeatherFileRecords);
      Subarrays[nn]->poa.poaAll->tilt.reserve(
          Irradiance->numberOfWeatherFileRecords);
      Subarrays[nn]->poa.poaAll->zen.reserve(
          Irradiance->numberOfWeatherFileRecords);
      Subarrays[nn]->poa.poaAll->exTer.reserve(
          Irradiance->numberOfWeatherFileRecords);
      ...
      for (size_t ii = 0; ii < Irradiance->numberOfWeatherFileRecords; ii++) {
        ...
        if (!wdprov->read(&wf)) {
          throw exec_error("pvsamv1",
                           "could not read data line " +
                               util::to_string((int)(ii + 1)) +
                               " in weather file while loading POA data");
        }
        ...
        if (Subarrays[nn]->trackMode == irrad::SEASONAL_TILT)
          Subarrays[nn]->tiltDegrees =
              Subarrays[nn]
                  ->monthlyTiltDegrees[month_idx]; // overwrite the tilt input
                                                   // with the current tilt to be
        ...
        // Calculate sunrise and sunset hours in local standard time for the
        // current day
        double sun[9], angle[5];
        int tms[3];

        static const int dut1 = 0; // leap second, assumed not used for analysis
        solarpos_spa(wf.year, wf.month, wf.day, 12, 0.0, 0.0, hdr.lat, hdr.lon,
                     hdr.tz, dut1, hdr.elev, wf.pres, wf.tdry, 0, 180, sun);

        double t_sunrise = sun[4];
        double t_sunset = sun[5];

        if (t_sunset > 24) // sunset is legitimately the next day, so
                           // recalculate sunset from the previous day
        {
          double sunanglestemp[9];
          if (wf.day > 1) // simply decrement day during month
            solarpos_spa(wf.year, wf.month, wf.day - 1, 12, 0.0, 0.0, hdr.lat,
                         hdr.lon, hdr.tz, dut1, hdr.elev, wf.pres, wf.tdry, 0,
                         180, sunanglestemp);
          else if (wf.month > 1) // on the 1st of the month, need to switch to
                                 // the last day of previous month
            solarpos_spa(
                wf.year, wf.month - 1, __nday[wf.month - 2], 12, 0.0, 0.0,
                hdr.lat, hdr.lon, hdr.tz, dut1, hdr.elev, wf.pres, wf.tdry, 0,
                180,
                sunanglestemp); // month is 1-indexed and __nday is 0 indexed
          else // on the first day of the year, need to switch to Dec 31 of last
               // year
            solarpos_spa(wf.year - 1, 12, 31, 12, 0.0, 0.0, hdr.lat, hdr.lon,
                         hdr.tz, dut1, hdr.elev, wf.pres, wf.tdry, 0, 180,
                         sunanglestemp);
          // if sunset from yesterday WASN'T today, then it's ok to leave sunset
          // > 24, which will cause the sun to rise today and not set today
          if (sunanglestemp[5] >= 24)
            t_sunset = sunanglestemp[5] - 24.0;
        }

        if (t_sunrise < 0) // sunrise is legitimately the previous day, so
                           // recalculate for next day
        {
          double sunanglestemp[9];
          if (wf.day < __nday[wf.month -
                              1]) // simply increment the day during the month,
                                  // month is 1-indexed and __nday is 0-indexed
            solarpos_spa(wf.year, wf.month, wf.day + 1, 12, 0.0, 0.0, hdr.lat,
                         hdr.lon, hdr.tz, dut1, hdr.elev, wf.pres, wf.tdry, 0,
                         180, sunanglestemp);
          else if (wf.month < 12) // on the last day of the month, need to
                                  // switch to the first day of the next month
            solarpos_spa(wf.year, wf.month + 1, 1, 12, 0.0, 0.0, hdr.lat,
                         hdr.lon, hdr.tz, dut1, hdr.elev, wf.pres, wf.tdry, 0,
                         180, sunanglestemp);
          else // on the last day of the year, need to switch to Jan 1 of the
               // next year
            solarpos_spa(wf.year + 1, 1, 1, 12, 0.0, 0.0, hdr.lat, hdr.lon,
                         hdr.tz, dut1, hdr.elev, wf.pres, wf.tdry, 0, 180,
                         sunanglestemp);
          // if sunrise from tomorrow isn't today, then it's ok to leave sunrise
          // < 0, which will cause the sun to set at the right time and not rise
          // until tomorrow
        ...
        if (t_cur >= t_sunrise - ts_hour / 2.0 &&
          ...
          double t_calc = (t_sunrise + (t_cur + ts_hour / 2.0)) /
                          2.0; // midpoint of sunrise and end of timestep
          ...
          solarpos_spa(wf.year, wf.month, wf.day, hr_calc, min_calc, 0, hdr.lat,
                       hdr.lon, hdr.tz, dut1, hdr.elev, wf.pres, wf.tdry, 0,
                       180, sun);
        ...
        // timestep encompasses the sunset
          ...
          double t_calc = ((t_cur - ts_hour / 2.0) + t_sunset) /
                          2.0; // midpoint of beginning of timestep and sunset
          int hr_calc = (int)t_calc;
          double min_calc = (t_calc - hr_calc) * 60.0;
          ...
          solarpos_spa(wf.year, wf.month, wf.day, hr_calc, min_calc, 0.0,
                       hdr.lat, hdr.lon, hdr.tz, dut1, hdr.elev, wf.pres,
                       wf.tdry, 0, 180, sun);

          tms[2] = 3;
        }

        // timestep is not sunrise nor sunset, but sun is up  (calculate
        // position at provided t_cur)
        else if ((t_sunrise < t_sunset && t_cur >= t_sunrise &&
                  t_cur <= t_sunset) || // this captures normal daylight cases
                 (t_sunrise > t_sunset &&
                  (t_cur <= t_sunset ||
                   t_cur >= t_sunrise))) // this captures cases where sunset
                                         // (from previous day) is 1:30AM,
                                         // sunrise 2:30AM, in arctic circle
        {
          // timestep is not sunrise nor sunset, but sun is up  (calculate
          // position at provided t_cur)
          tms[0] = wf.hour;
          tms[1] = (int)wf.minute;
          solarpos_spa(wf.year, wf.month, wf.day, wf.hour, wf.minute, 0.0,
                       hdr.lat, hdr.lon, hdr.tz, dut1, hdr.elev, wf.pres,
                       wf.tdry, 0, 180, sun);
          tms[2] = 1;
        } else {
        ...
        if (tms[2] > 0) {
          incidence(Subarrays[nn]->trackMode, Subarrays[nn]->tiltDegrees,
                    Subarrays[nn]->azimuthDegrees,
                    Subarrays[nn]->trackerRotationLimitDegrees, sun[1], sun[0],
                    Subarrays[nn]->backtrackingEnabled,
                    Subarrays[nn]->groundCoverageRatio, false, 0.0, angle);
        } else {
          angle[0] = -999;
          angle[1] = -999;
          angle[2] = -999;
...
PVSystem_IO::PVSystem_IO(compute_module *cm, std::string cmName,
                         Simulation_IO *SimulationIO,
                         Irradiance_IO *IrradianceIO,
                         std::vector<Subarray_IO *> SubarraysAll,
                         Inverter_IO *InverterIO) {
  ...
  // The shared inverter of the PV array and a tightly-coupled DC connected
  // battery
  std::unique_ptr<SharedInverter> tmpSharedInverter(new SharedInverter(
      Inverter->inverterType, numberOfInverters, &Inverter->sandiaInverter,
      &Inverter->partloadInverter, &Inverter->ondInverter));
  ...
  // PV Degradation, place into intermediate variable since pointer outputs are
  // not allocated yet
  if (Simulation->useLifetimeOutput) {
    ...
    if (dc_degrad.size() == 1) {
      ...
      for (size_t i = 1; i < Simulation->numberOfYears && i < dc_degrad.size();
           i++)
        dcDegradationFactor.push_back(1.0 - dc_degrad[i] / 100.0);
    }

    // read in optional DC and AC lifetime daily losses, error check length of
    // arrays
    if (enableDCLifetimeLosses) {
      if (!Simulation->annualSimulation)
        throw exec_error(cmName, "Lifetime daily losses cannot be entered with "
                                 "non-annual weather data");

      dcLifetimeLosses = cm->as_vector_double("dc_lifetime_losses");
      if (dcLifetimeLosses.size() != Simulation->numberOfYears * 365)
        throw exec_error(cmName,
                         "Length of the lifetime daily DC losses array must be "
                         "equal to the analysis period * 365 days/year");
    }
    if (enableACLifetimeLosses) {
      ...
      if (acLifetimeLosses.size() != Simulation->numberOfYears * 365)
        throw exec_error(cmName,
                         "Length of the lifetime daily AC losses array must be "
                         "equal to the analysis period * 365 days/year");
    }
  }

  // Transformer losses
  transformerLoadLossFraction = cm->as_number("transformer_load_loss") *
                                (ssc_number_t)(util::percent_to_fraction);
  transformerNoLoadLossFraction = cm->as_number("transformer_no_load_loss") *
                                  (ssc_number_t)(util::percent_to_fraction);

  // Determine whether MPPT clipping should be enabled or not
  clipMpptWindow = false;

  if (InverterIO->mpptLowVoltage > 0 &&
      InverterIO->mpptHiVoltage > InverterIO->mpptLowVoltage) {
        ...
        || modulePowerModel == 5) // PVYield single diode
    {
      ...
      cm->log(
          "The simple efficiency and Sandia module models do not allow "
          "limiting module voltage to the MPPT tracking range of the inverter.",
          SSC_NOTICE);
    }
  } else {
    cm->log("Inverter MPPT voltage tracking window not defined - modules "
            "always operate at MPPT.",
            SSC_NOTICE);
  }
  ...
  for (size_t n_subarray = 0; n_subarray < numberOfSubarrays; n_subarray++) {
    if (Subarrays[n_subarray]->enable) {
      if (Subarrays[n_subarray]->mpptInput > (int)Inverter->nMpptInputs)
        throw exec_error(cmName, "Subarray " +
                                     util::to_string((int)n_subarray) +
                                     " MPPT input is greater than the number "
                                     "of inverter MPPT inputs.");
    }
  }
  for (size_t mppt = 1; mppt <= (size_t)Inverter->nMpptInputs;
       mppt++) // indexed at 1 to match mppt input numbering convention
  {
    std::vector<int> mppt_n; // create a temporary vector to hold which
                             // subarrays are on this mppt input
    // find all subarrays on this mppt input
    for (size_t n_subarray = 0; n_subarray < Subarrays.size();
         n_subarray++) // jmf update this so that all subarray markers are
                       // consistent, get rid of "enable" check
      if (Subarrays[n_subarray]->enable)
        if (Subarrays[n_subarray]->mpptInput == (int)mppt)
          mppt_n.push_back((int)n_subarray);
    if (mppt_n.size() < 1)
      throw exec_error(cmName, "At least one subarray must be assigned to each "
                               "inverter MPPT input.");
    mpptMapping.push_back(mppt_n); // add the subarrays on this input to the
                                   // total mppt mapping vector
  }

  // Only one multi-MPPT inverter is allowed at the moment
  if (Inverter->nMpptInputs > 1 && numberOfInverters > 1)
    throw exec_error(cmName,
                     "At this time, only one multiple-MPPT-input inverter may "
                     "be modeled per system. See help for details.");
  ...
  if (enableMismatchVoltageCalc &&
    ...
    throw exec_error(cmName,
                     "String level subarray mismatch can only be calculated "
                     "using a single-diode based module model.");
  }
  if (enableMismatchVoltageCalc && numberOfSubarrays <= 1)
    throw exec_error(cmName,
                     "Subarray voltage mismatch calculation requires more than "
                     "one subarray. Please check your inputs.");
...
void PVSystem_IO::AllocateOutputs(compute_module *cm) {
  ...
  for (size_t subarray = 0; subarray < Subarrays.size(); subarray++) {
    if (Subarrays[subarray]->enable) {
      ...
      p_angleOfIncidence.push_back(
          cm->allocate(prefix + "aoi", numberOfWeatherFileRecords));
      p_angleOfIncidenceModifier.push_back(
          cm->allocate(prefix + "aoi_modifier", numberOfWeatherFileRecords));
      p_surfaceTilt.push_back(
          cm->allocate(prefix + "surf_tilt", numberOfWeatherFileRecords));
      p_surfaceAzimuth.push_back(
          cm->allocate(prefix + "surf_azi", numberOfWeatherFileRecords));
      p_axisRotation.push_back(
          cm->allocate(prefix + "axisrot", numberOfWeatherFileRecords));
      p_idealRotation.push_back(
          cm->allocate(prefix + "idealrot", numberOfWeatherFileRecords));
      p_poaNominalFront.push_back(
          cm->allocate(prefix + "poa_nom", numberOfWeatherFileRecords));
      p_poaShadedFront.push_back(
          cm->allocate(prefix + "poa_shaded", numberOfWeatherFileRecords));
      p_poaShadedSoiledFront.push_back(cm->allocate(
          prefix + "poa_shaded_soiled", numberOfWeatherFileRecords));
      p_poaBeamFront.push_back(
          cm->allocate(prefix + "poa_eff_beam", numberOfWeatherFileRecords));
      p_poaDiffuseFront.push_back(
          cm->allocate(prefix + "poa_eff_diff", numberOfWeatherFileRecords));
      p_poaTotal.push_back(
          cm->allocate(prefix + "poa_eff", numberOfWeatherFileRecords));
      p_poaRear.push_back(
          cm->allocate(prefix + "poa_rear", numberOfWeatherFileRecords));
      p_poaFront.push_back(
          cm->allocate(prefix + "poa_front", numberOfWeatherFileRecords));
      p_derateSoiling.push_back(
          cm->allocate(prefix + "soiling_derate", numberOfWeatherFileRecords));
      p_beamShadingFactor.push_back(cm->allocate(prefix + "beam_shading_factor",
                                                 numberOfWeatherFileRecords));
      p_temperatureCell.push_back(
          cm->allocate(prefix + "celltemp", numberOfWeatherFileRecords));
      p_temperatureCellSS.push_back(
          cm->allocate(prefix + "celltempSS", numberOfWeatherFileRecords));
      p_moduleEfficiency.push_back(
          cm->allocate(prefix + "modeff", numberOfWeatherFileRecords));
      p_dcStringVoltage.push_back(
          cm->allocate(prefix + "dc_voltage", numberOfWeatherFileRecords));
      p_voltageOpenCircuit.push_back(
          cm->allocate(prefix + "voc", numberOfWeatherFileRecords));
      p_currentShortCircuit.push_back(
          cm->allocate(prefix + "isc", numberOfWeatherFileRecords));
      p_dcPowerGross.push_back(
          cm->allocate(prefix + "dc_gross", numberOfWeatherFileRecords));
      p_derateLinear.push_back(
          cm->allocate(prefix + "linear_derate", numberOfWeatherFileRecords));
      p_derateSelfShading.push_back(
          cm->allocate(prefix + "ss_derate", numberOfWeatherFileRecords));
      p_derateSelfShadingDiffuse.push_back(cm->allocate(
          prefix + "ss_diffuse_derate", numberOfWeatherFileRecords));
      p_derateSelfShadingReflected.push_back(cm->allocate(
          prefix + "ss_reflected_derate", numberOfWeatherFileRecords));

      if (enableSnowModel) {
        p_snowLoss.push_back(
            cm->allocate(prefix + "snow_loss", numberOfWeatherFileRecords));
        p_snowCoverage.push_back(
            cm->allocate(prefix + "snow_coverage", numberOfWeatherFileRecords));
      }

      if (Subarrays[subarray]->enableSelfShadingOutputs) {
        // ShadeDB validation
        p_shadeDB_GPOA.push_back(cm->allocate("shadedb_" + prefix + "gpoa",
                                              numberOfWeatherFileRecords));
        p_shadeDB_DPOA.push_back(cm->allocate("shadedb_" + prefix + "dpoa",
                                              numberOfWeatherFileRecords));
        p_shadeDB_temperatureCell.push_back(cm->allocate(
            "shadedb_" + prefix + "pv_cell_temp", numberOfWeatherFileRecords));
        p_shadeDB_modulesPerString.push_back(cm->allocate(
            "shadedb_" + prefix + "mods_per_str", numberOfWeatherFileRecords));
        p_shadeDB_voltageMaxPowerSTC.push_back(cm->allocate(
            "shadedb_" + prefix + "str_vmp_stc", numberOfWeatherFileRecords));
        p_shadeDB_voltageMPPTLow.push_back(cm->allocate(
            "shadedb_" + prefix + "mppt_lo", numberOfWeatherFileRecords));
        p_shadeDB_voltageMPPTHigh.push_back(cm->allocate(
            "shadedb_" + prefix + "mppt_hi", numberOfWeatherFileRecords));
      }
      p_shadeDBShadeFraction.push_back(cm->allocate(
          "shadedb_" + prefix + "shade_frac", numberOfWeatherFileRecords));
    }
  }

  for (size_t mppt_input = 0; mppt_input < Inverter->nMpptInputs;
       mppt_input++) {
    p_mpptVoltage.push_back(cm->allocate(
        "inverterMppt" + std::to_string(mppt_input + 1) + "_DCVoltage",
        numberOfLifetimeRecords));
    p_dcPowerNetPerMppt.push_back(cm->allocate(
        "inverterMppt" + std::to_string(mppt_input + 1) + "_NetDCPower",
        numberOfLifetimeRecords));
  }

  p_transformerNoLoadLoss =
      cm->allocate("xfmr_nll_ts", numberOfWeatherFileRecords);
  p_transformerLoadLoss =
      cm->allocate("xfmr_ll_ts", numberOfWeatherFileRecords);
  ...
  p_poaFrontBeamNominalTotal =
      cm->allocate("poa_beam_nom", numberOfWeatherFileRecords);
  p_poaFrontBeamTotal =
      cm->allocate("poa_beam_eff", numberOfWeatherFileRecords);
  p_poaFrontShadedTotal =
      cm->allocate("poa_shaded", numberOfWeatherFileRecords);
  p_poaFrontShadedSoiledTotal =
      cm->allocate("poa_shaded_soiled", numberOfWeatherFileRecords);
  ...
  p_inverterMPPTLoss =
      cm->allocate("dc_invmppt_loss", numberOfWeatherFileRecords);

  p_inverterPowerConsumptionLoss =
      cm->allocate("inv_psoloss", numberOfWeatherFileRecords);
  p_inverterNightTimeLoss =
      cm->allocate("inv_pntloss", numberOfWeatherFileRecords);
  p_inverterThermalLoss =
      cm->allocate("inv_tdcloss", numberOfWeatherFileRecords);
  p_inverterTotalLoss =
      cm->allocate("inv_total_loss", numberOfWeatherFileRecords);

  p_acWiringLoss = cm->allocate("ac_wiring_loss", numberOfWeatherFileRecords);
  p_transmissionLoss =
      cm->allocate("ac_transmission_loss", numberOfWeatherFileRecords);
  p_systemDCPower = cm->allocate("dc_net", numberOfLifetimeRecords);
  p_systemACPower = cm->allocate("gen", numberOfLifetimeRecords);

  if (Simulation->useLifetimeOutput) {
    p_dcDegradationFactor =
        cm->allocate("dc_degrade_factor", numberOfYears + 1);
  }
}
void PVSystem_IO::AssignOutputs(compute_module *cm) {
  cm->assign("ac_loss",
             var_data((ssc_number_t)(acLossPercent + transmissionLossPercent)));
}
...
Module_IO::Module_IO(compute_module *cm, std::string cmName, double dcLoss) {
  ...
  if (modulePowerModel == MODULE_SIMPLE_EFFICIENCY) {
    ...
    bifacialTransmissionFactor =
        cm->as_double("spe_bifacial_transmission_factor");
    groundClearanceHeight =
        cm->as_double("spe_bifacial_ground_clearance_height");
    for (int i = 0; i < 5; i++) {
      simpleEfficiencyModel.Rad[i] =
          cm->as_double(util::format("spe_rad%d", i));
      simpleEfficiencyModel.Eff[i] =
          0.01 * cm->as_double(util::format("spe_eff%d", i));
      if (i > 0 &&
          simpleEfficiencyModel.Rad[i] <= simpleEfficiencyModel.Rad[i - 1])
        throw exec_error(cmName, "simpleEfficiencyModel model radiation levels "
    ...
    default:
      throw exec_error(
          cmName,
          "invalid simpleEfficiencyModel module structure and mounting");
    }
    ...
    bifacialTransmissionFactor =
        cm->as_double("cec_bifacial_transmission_factor");
    groundClearanceHeight =
        cm->as_double("cec_bifacial_ground_clearance_height");
    ...
    selfShadingFillFactor =
        cecModel.Vmp * cecModel.Imp / cecModel.Voc / cecModel.Isc;
    ...
    if (cm->as_integer("cec_temp_corr_mode") == 0) {
      ...
      { // source for standoff adjustment constants:
        // https://prod-ng.sandia.gov/techlib-noauth/access-control.cgi/1985/850330.pdf
        // page 12
      case 2:
        nominalOperatingCellTemp.standoff_tnoct_adj = 2;
        break; // between 2.5 and 3.5 inches
      case 3:
        nominalOperatingCellTemp.standoff_tnoct_adj = 6;
        break; // between 1.5 and 2.5 inches
      case 4:
        nominalOperatingCellTemp.standoff_tnoct_adj = 11;
        break; // between 0.5 and 1.5 inches
      case 5:
        nominalOperatingCellTemp.standoff_tnoct_adj = 18;
        break; // less than 0.5 inches
      ...
      /*	int MC; // Mounting configuration
      (1=rack,2=flush,3=integrated,4=gap) int HTD; // Heat transfer dimension
      (1=Module,2=Array) int MSO; // Mounting structure orientation (1=does not
      impede flow beneath, 2=vertical supports, 3=horizontal supports) int
      Nrows, Ncols; // number of modules in rows and columns, when using array
      heat transfer dimensions double Length; // module length, along horizontal
      dimension, (m) double Width; // module width, along vertical dimension,
      (m) double Wgap;  // gap width spacing (m) double TbackInteg; */
      ...
      mountingSpecificCellTemp.MSO =
          cm->as_integer("cec_mounting_orientation") + 1;
    ...
    bifacialTransmissionFactor =
        cm->as_double("6par_bifacial_transmission_factor");
    groundClearanceHeight =
        cm->as_double("6par_bifacial_ground_clearance_height");

    int tech_id = module6par::monoSi;
    int type = cm->as_integer(
        "6par_celltech"); // "monoSi,multiSi,CdTe,CIS,CIGS,Amorphous"
    switch (type) {
    case 0:
    ...
    if (err != 0)
      throw exec_error(cmName,
                       "CEC 6 parameter model:  Could not solve for normalized "
                       "coefficients.  Please check your inputs.");
    ...
    selfShadingFillFactor = sandiaModel.Vmp0 * sandiaModel.Imp0 /
                            sandiaModel.Voc0 / sandiaModel.Isc0;
    ...
    moduleWattsSTC =
        elevenParamSingleDiodeModel.Vmp0 * elevenParamSingleDiodeModel.Imp0;
    referenceArea = elevenParamSingleDiodeModel.Area;
    selfShadingFillFactor =
        elevenParamSingleDiodeModel.Vmp0 * elevenParamSingleDiodeModel.Imp0 /
        elevenParamSingleDiodeModel.Voc0 / elevenParamSingleDiodeModel.Isc0;
    ...
    mlModuleModel.groundRelfectionFraction =
        cm->as_double("mlm_groundRelfectionFraction");
    ...
    mlModuleModel.IAM_c_cs_elements =
        (int)elementCount1; // as_integer("mlm_IAM_c_cs_elements");

    if (mlModuleModel.IAM_mode == 3) {
      if (elementCount1 != elementCount2) {
        throw exec_error(cmName, "Spline IAM: Number of entries for incidence "
                                 "angle and IAM value different.");
      }
    ...
    selfShadingFillFactor = mlModuleModel.V_mp_ref * mlModuleModel.I_mp_ref /
                            mlModuleModel.V_oc_ref / mlModuleModel.I_sc_ref;
...
void Module_IO::setupNOCTModel(compute_module *cm, const std::string &prefix) {
  ...
  if (cm->as_integer(prefix + "_mounting") == 1)
    nominalOperatingCellTemp.ffv_wind =
        0.61; // greater than 22ft high (2 story)

  int standoff = cm->as_integer(
      prefix +
      "_standoff"); // bipv,3.5in,2.5-3.5in,1.5-2.5in,0.5-1.5in,ground/rack
  nominalOperatingCellTemp.standoff_tnoct_adj = 0;
  switch (standoff) { // source for standoff adjustment constants:
                      // https://prod-ng.sandia.gov/techlib-noauth/access-control.cgi/1985/850330.pdf
                      // page 12
  case 2:
    nominalOperatingCellTemp.standoff_tnoct_adj = 2;
...
Inverter_IO::Inverter_IO(compute_module *cm, std::string cmName) {
  ...
  } else if (inverterType == 4) // PVYield (ondInverter)
  {
    ...
    ondInverter.doUseTemperatureLimit =
        cm->as_integer("ond_doUseTemperatureLimit");
    int matrixIndex;
...
void Inverter_IO::setupSharedInverter(compute_module *cm,
  ...
  if (err > 0) {
    throw exec_error("pvsamv1", "Inverter temperature derate curve row " +
                                    util::to_string((int)(err - 1)) +
                                    " is invalid.");
  }
}
