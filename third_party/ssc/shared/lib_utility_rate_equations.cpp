...
ur_month::ur_month()
    : ec_periods(), dc_periods(), ec_rollover_periods(), energy_net(),
      hours_per_month(), ec_energy_use(), ec_periods_tiers(),
      ec_energy_surplus(), dc_tou_peak(), dc_tou_peak_hour(), dc_flat_peak(),
      dc_flat_peak_hour(), ec_tou_ub_init(), ec_tou_br_init(), ec_tou_sr_init(),
      ec_tou_ub(), ec_tou_br(), ec_tou_sr(), ec_tou_units(), ec_charge(),
      dc_tou_ub(), dc_tou_ch(), dc_flat_ub(), dc_flat_ch(), dc_tou_charge(),
      dc_flat_charge() {}

ur_month::ur_month(const ur_month &tmp)
    : ec_periods(tmp.ec_periods), dc_periods(tmp.dc_periods),
      ec_rollover_periods(tmp.ec_rollover_periods), energy_net(tmp.energy_net),
      hours_per_month(tmp.hours_per_month), ec_energy_use(tmp.ec_energy_use),
      ec_periods_tiers(tmp.ec_periods_tiers),
      ec_energy_surplus(tmp.ec_energy_surplus), dc_tou_peak(tmp.dc_tou_peak),
      dc_tou_peak_hour(tmp.dc_tou_peak_hour), dc_flat_peak(tmp.dc_flat_peak),
      dc_flat_peak_hour(tmp.dc_flat_peak_hour),
      ec_tou_ub_init(tmp.ec_tou_ub_init), ec_tou_br_init(tmp.ec_tou_br_init),
      ec_tou_sr_init(tmp.ec_tou_sr_init), ec_tou_ub(tmp.ec_tou_ub),
      ec_tou_br(tmp.ec_tou_br), ec_tou_sr(tmp.ec_tou_sr),
      ec_tou_units(tmp.ec_tou_units), ec_charge(tmp.ec_charge),
      dc_tou_ub(tmp.dc_tou_ub), dc_tou_ch(tmp.dc_tou_ch),
      dc_flat_ub(tmp.dc_flat_ub), dc_flat_ch(tmp.dc_flat_ch),
      dc_tou_charge(tmp.dc_tou_charge), dc_flat_charge(tmp.dc_flat_charge) {}
...
rate_data::rate_data()
    : m_ec_tou_sched(), m_dc_tou_sched(), m_month(), m_ec_periods(),
      m_ec_ts_sell_rate(), m_ec_ts_buy_rate(), m_ec_periods_tiers_init(),
      m_dc_tou_periods(), m_dc_tou_periods_tiers(), m_dc_flat_tiers(),
      m_num_rec_yearly(), dc_hourly_peak(), monthly_dc_fixed(12),
      monthly_dc_tou(12), tou_demand_single_peak(false), enable_nm(false),
      nm_credits_w_rollover(false), net_metering_credit_month(11),
      nm_credit_sell_rate(0.0), rate_scale(), en_ts_buy_rate(false),
      en_ts_sell_rate(false) {}

rate_data::rate_data(const rate_data &tmp)
    : m_ec_tou_sched(tmp.m_ec_tou_sched), m_dc_tou_sched(tmp.m_dc_tou_sched),
      m_month(tmp.m_month), m_ec_periods(tmp.m_ec_periods),
      ...
      nm_credit_sell_rate(tmp.nm_credit_sell_rate), rate_scale(tmp.rate_scale),
      en_ts_buy_rate(tmp.en_ts_buy_rate), en_ts_sell_rate(tmp.en_ts_sell_rate) {
}
...
void rate_data::init_energy_rates(bool gen_only) {
  ...
  for (int m = 0; m < (int)m_month.size(); m++) {
    ...
    size_t num_tiers =
        (int)m_month[m].ec_tou_ub.ncols(); // Likely to be overwritten later

    if (!gen_only) // added for two meter no load scenarios to use load tier
                   // sizing
    {
      ...
      // 4. assumption is that all periods in same month have same tier
      // breakdown
      // 5. assumption is that tier numbering is correct for the kWh/kW
      // breakdown That is, first tier must be kWh/kW
      // See example at:
      // https://github.com/NREL/SAM-documentation/blob/master/Unit%20Testing/Utility%20Rates/block_step/GPC_PLL_Tiered_Bill_Calc_Example_v3_btm_tests.xlsx
      if ((m_month[m].ec_tou_units.ncols() > 0 &&
           m_month[m].ec_tou_units.nrows() > 0) &&
          check_for_kwh_per_kw_rate(m_month[m].ec_tou_units.at(0, 0))) {
        std::vector<double> kWh_per_kW_tiers; // Fill this first so we can see
                                              // where the kWh tiers break
        ...
        for (size_t i_tier = 0; i_tier < m_month[m].ec_tou_units.ncols();
             i_tier++) {
        ...
        for (size_t i_tier = 0; i_tier < total_tiers; i_tier++) {
          ...
          if (check_for_kwh_per_kw_rate(units)) {
            ...
            double kwh_per_kw = m_month[m].ec_tou_ub_init.at(0, i_tier);
            if (kWh_per_kW_tiers[block] < kwh_per_kw * flat_peak &&
                (block + 1 < kWh_per_kW_tiers.size())) {
              block++; // Move on to next block tier
            }
            // Check this block, if it has kWh steps skip it and move on,
            // otherwise add it
            ...
            if (max < kWh_per_kW_tiers[block]) {
              ...
              if (tier_kwh.empty() ||
                  (tier_kwh[tier_kwh.size() - 1] < kWh_per_kW_tiers[block])) {
                tier_kwh.push_back(kWh_per_kW_tiers[block]);
                tier_numbers.push_back(i_tier);
              }
            }
        ...
        for (size_t period = 0; period < num_periods; period++) {
          for (size_t tier = 0; tier < num_tiers; tier++) {
            br.at(period, tier) =
                m_month[m].ec_tou_br_init.at(period, tier_numbers[tier]);
            sr.at(period, tier) =
                m_month[m].ec_tou_sr_init.at(period, tier_numbers[tier]);
            ub.at(period, tier) = tier_kwh[tier];
            // update for correct tier number column headings
            m_month[m].ec_periods_tiers[period][tier] =
                (int)(m_ec_periods_tiers_init[period][tier_numbers[tier]]);
          }
        }
...
void rate_data::setup_time_series(size_t cnt, ssc_number_t *ts_sr,
                                  ssc_number_t *ts_br) {
  size_t i;
...
void rate_data::setup_energy_rates(ssc_number_t *ec_weekday,
                                   ssc_number_t *ec_weekend, size_t ec_tou_rows,
                                   ssc_number_t *ec_tou_in, bool sell_eq_buy) {
  ...
  if (!util::translate_schedule(ec_tod, ec_schedwkday, ec_schedwkend, 1, 12))
    throw general_error(
        "Could not translate weekday and weekend schedules for energy rates.");
  for (i = 0; i < 8760; i++) {
  ...
  for (r = 0; r < ec_tou_rows; r++) {
    ...
    if (std::find(m_ec_periods.begin(), m_ec_periods.end(), period) ==
        m_ec_periods.end())
  ...
  for (r = 0; r < ec_tou_rows; r++) {
    ...
    std::vector<int>::iterator result =
        std::find(m_ec_periods.begin(), m_ec_periods.end(), period);
  ...
  for (r = 0; r < m_ec_periods_tiers_init.size(); r++)
    std::sort(m_ec_periods_tiers_init[r].begin(),
              m_ec_periods_tiers_init[r].end());

  // find all periods for each month m through schedules
  for (m = 0; m < m_month.size(); m++) {
    // energy charges
    for (c = 0; c < ec_schedwkday.ncols(); c++) {
      if (std::find(m_month[m].ec_periods.begin(), m_month[m].ec_periods.end(),
                    ec_schedwkday.at(m, c)) == m_month[m].ec_periods.end())
        m_month[m].ec_periods.push_back((int)ec_schedwkday.at(m, c));

      // rollover periods considered from weekday schedule at 12a [0], 6a [5],
      // 12p [11], and 6p [17]
      if ((m_month[m].ec_rollover_periods.size() < 5) &&
          (c == 0 || c == 5 || c == 11 || c == 17)) {
    ...
    for (c = 0; c < ec_schedwkend.ncols(); c++) {
      if (std::find(m_month[m].ec_periods.begin(), m_month[m].ec_periods.end(),
                    ec_schedwkend.at(m, c)) == m_month[m].ec_periods.end())
    ...
    for (size_t pt_period = 0; pt_period < m_ec_periods_tiers_init.size();
         pt_period++)
      m_month[m].ec_periods_tiers.push_back(std::vector<int>());
    for (size_t pt_period = 0; pt_period < m_ec_periods_tiers_init.size();
         pt_period++) {
      for (size_t pt_tier = 0;
           pt_tier < m_ec_periods_tiers_init[pt_period].size(); pt_tier++)
        m_month[m].ec_periods_tiers[pt_period].push_back(
            m_ec_periods_tiers_init[pt_period][pt_tier]);
    }
  }

  // periods are rows and tiers are columns - note that columns can change based
  // on rows Initialize each month variables that are constant over the
  // simulation
  for (m = 0; m < m_month.size(); m++) {
    int num_periods = 0;
    int num_tiers = 0;
    ...
    for (i = 0; i < m_month[m].ec_periods.size(); i++) {
      // find all periods and check that number of tiers the same for all for
      // the month, if not through error
      std::vector<int>::iterator per_num = std::find(
          m_ec_periods.begin(), m_ec_periods.end(), m_month[m].ec_periods[i]);
      if (per_num == m_ec_periods.end()) {
        std::ostringstream ss;
        ss << "Period " << m_month[m].ec_periods[i] << " is in Month " << m
           << " but is not defined in the energy rate table. Rates for each "
              "period in the Weekday and Weekend schedules must be defined in "
              "the energy rate table.";
        throw exec_error("lib_utility_rate_equations", ss.str());
      }
      period = (*per_num);
      ...
      if (i == 0) {
        ...
        m_month[m].ec_tou_ub.resize_fill(num_periods, num_tiers,
                                         (ssc_number_t)1e+38);
        ...
        if ((int)m_ec_periods_tiers_init[ndx].size() != num_tiers) {
          std::ostringstream ss;
          ss << "The number of tiers in the energy rate table, "
             << m_ec_periods_tiers_init[ndx].size()
             << ", is incorrect for Month " << m << " and Period "
             << m_month[m].ec_periods[i]
             << ". The correct number of tiers for that month and period is "
             << num_tiers << ".";
          throw exec_error("lib_utility_rate_equations", ss.str());
        }
      }
        ...
        for (r = 0; (r < ec_tou_rows) && !found; r++) {
          if ((period == (int)ec_tou_mat.at(r, 0)) &&
            ...
            if ((m_month[m].ec_tou_units.at(i, j) == 2) ||
                (m_month[m].ec_tou_units.at(i, j) ==
                 3)) { // kWh daily or kWh/kW daily - adjust max usage by number
                       // of days in month (or billing cycle per Eric 12/14/15
...
void rate_data::setup_demand_charges(
    ssc_number_t *dc_weekday, ssc_number_t *dc_weekend, size_t dc_tou_rows,
    ssc_number_t *dc_tou_in, size_t dc_flat_rows, ssc_number_t *dc_flat_in) {
  ...
  if (!util::translate_schedule(dc_tod, dc_schedwkday, dc_schedwkend, 1, 12))
    throw general_error(
        "Could not translate weekday and weekend schedules for demand charges");
  ...
  for (m = 0; m < m_month.size(); m++) {
    ...
    for (c = 0; c < dc_schedwkday.ncols(); c++) {
      if (std::find(m_month[m].dc_periods.begin(), m_month[m].dc_periods.end(),
                    dc_schedwkday.at(m, c)) == m_month[m].dc_periods.end())
        m_month[m].dc_periods.push_back((int)dc_schedwkday.at(m, c));
    }
    for (c = 0; c < dc_schedwkend.ncols(); c++) {
      if (std::find(m_month[m].dc_periods.begin(), m_month[m].dc_periods.end(),
                    dc_schedwkend.at(m, c)) == m_month[m].dc_periods.end())
  ...
  for (r = 0; r < dc_tou_rows; r++) {
    ...
    if (std::find(m_dc_tou_periods.begin(), m_dc_tou_periods.end(), period) ==
        m_dc_tou_periods.end())
  ...
  m_dc_tou_periods_tiers =
      std::vector<std::vector<int>>(m_dc_tou_periods.size());

  for (r = 0; r < dc_tou_rows; r++) {
    period = (int)dc_tou_mat.at(r, 0);
    tier = (int)dc_tou_mat.at(r, 1);
    std::vector<int>::iterator result =
        std::find(m_dc_tou_periods.begin(), m_dc_tou_periods.end(), period);
  ...
  for (r = 0; r < m_dc_tou_periods_tiers.size(); r++)
    std::sort(m_dc_tou_periods_tiers[r].begin(),
              m_dc_tou_periods_tiers[r].end());
  ...
  for (m = 0; m < m_month.size(); m++) {
    ...
    for (i = 0; i < m_month[m].dc_periods.size(); i++) {
      // find all periods and check that number of tiers the same for all for
      // the month, if not through error
      std::vector<int>::iterator per_num =
          std::find(m_dc_tou_periods.begin(), m_dc_tou_periods.end(),
                    m_month[m].dc_periods[i]);
      if (per_num == m_dc_tou_periods.end()) {
        std::ostringstream ss;
        ss << "Period " << m_month[m].dc_periods[i] << " is in Month " << m
           << " but is not defined in the demand rate table.  Rates for each "
              "period in the Weekday and Weekend schedules must be defined in "
              "the demand rate table.";
        throw exec_error("utilityrate5", ss.str());
      }
      period = (*per_num);
    ...
    m_month[m].dc_tou_ub.resize_fill(num_periods, num_tiers,
                                     (ssc_number_t)1e38);
      ...
      for (j = 0; j < m_dc_tou_periods_tiers[ndx].size(); j++) {
        ...
        for (r = 0; (r < dc_tou_rows) && !found; r++) {
          if ((period == (int)dc_tou_mat.at(r, 0)) &&
            ...
            m_month[m].dc_tou_ch.at(i, j) =
                dc_tou_mat.at(r, 3); // demand charge;
            found = true;
          }
        }
      }
  ...
  // months are rows and tiers are columns - note that columns can change based
  // on rows Initialize each month variables that are constant over the
  // simulation
...
void rate_data::sort_energy_to_periods(int month, double energy, size_t step) {
  ...
  std::vector<int>::iterator per_num = std::find(
      curr_month.ec_periods.begin(), curr_month.ec_periods.end(), toup);
  if (per_num == curr_month.ec_periods.end()) {
    std::ostringstream ss;
    ss << "Energy rate TOU Period " << toup << " not found for Month "
       << util::schedule_int_to_month(month) << ".";
    throw exec_error("utilityrate5", ss.str());
  }
...
void rate_data::init_dc_peak_vectors(int month) {
  ...
  curr_month.dc_tou_peak =
      std::vector<ssc_number_t>(curr_month.dc_periods.size());
  curr_month.dc_tou_peak_hour =
      std::vector<size_t>(curr_month.dc_periods.size());
}
...
void rate_data::find_dc_tou_peak(int month, double power, size_t step) {
  ...
  if (curr_month.dc_periods.size() > 0) {
    ...
    std::vector<int>::iterator per_num = std::find(
        curr_month.dc_periods.begin(), curr_month.dc_periods.end(), todp);
  ...
  for (period = 0; period < (int)curr_month.dc_tou_ub.nrows(); period++) {
    ...
    if (tou_demand_single_peak) {
      ...
      if (curr_month.dc_flat_peak_hour != curr_month.dc_tou_peak_hour[period])
        continue; // only one peak per month.
    ...
    for (tier = 0; tier < (int)curr_month.dc_tou_ub.ncols() && !found; tier++) {
      if (demand < curr_month.dc_tou_ub.at(period, tier)) {
        found = true;
        charge += (demand - d_lower) * curr_month.dc_tou_ch.at(period, tier) *
                  rate_esc;
        curr_month.dc_tou_charge.push_back(charge);
      } else if (period < curr_month.dc_periods.size()) {
        charge += (curr_month.dc_tou_ub.at(period, tier) - d_lower) *
...
int rate_data::get_tou_row(size_t year_one_index, int month) {
  ...
  std::vector<int>::iterator per_num = std::find(
      curr_month.ec_periods.begin(), curr_month.ec_periods.end(), period);
  if (per_num == curr_month.ec_periods.end()) {
    std::ostringstream ss;
...
int rate_data::transfer_surplus(ur_month &curr_month, ur_month &prev_month) {
  ...
  for (size_t ir = 0; ir < prev_month.ec_energy_surplus.nrows(); ir++) {
    ...
    {
      int toup_source =
          prev_month.ec_periods[ir]; // number of rows of previous month - and
                                     // period with surplus
      // find source period in rollover map for previous month
      std::vector<int>::iterator source_per_num =
          std::find(prev_month.ec_rollover_periods.begin(),
                    prev_month.ec_rollover_periods.end(), toup_source);
      if (source_per_num == prev_month.ec_rollover_periods.end()) {
        ...
        int rollover_index =
            (int)(source_per_num - prev_month.ec_rollover_periods.begin());
        if (rollover_index < (int)curr_month.ec_rollover_periods.size()) {
          int toup_target = curr_month.ec_rollover_periods[rollover_index];
          std::vector<int>::iterator target_per_num =
              std::find(curr_month.ec_periods.begin(),
                        curr_month.ec_periods.end(), toup_target);
          if (target_per_num == curr_month.ec_periods.end()) {
            returnValue = 200 + toup_target;
          }
          int target_row =
              (int)(target_per_num - curr_month.ec_periods.begin());
...
void rate_data::compute_surplus(ur_month &curr_month) {
  ...
  for (size_t ir = 0; ir < curr_month.ec_energy_use.nrows(); ir++) {
    if (curr_month.ec_energy_use.at(ir, 0) > 0) {
      curr_month.ec_energy_surplus.at(ir, 0) =
          curr_month.ec_energy_use.at(ir, 0);
