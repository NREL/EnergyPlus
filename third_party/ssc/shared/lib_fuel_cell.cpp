...
FuelCell::FuelCell(
    double unitPowerMax_kW, double unitPowerMin_kW, double startup_hours,
    bool is_started, double shutdown_hours, double dynamicResponseUp_kWperHour,
    double dynamicResponseDown_kWperHour, double degradation_kWperHour,
    double degradationRestart_kW, size_t replacementOption,
    double replacement_percent, std::vector<size_t> replacementSchedule,
    util::matrix_t<size_t> shutdownTable, size_t efficiencyChoice,
    util::matrix_t<double> efficiencyTable, double lowerHeatingValue_BtuPerFt3,
    double higherHeatingValue_BtuPerFt3, double availableFuel_Mcf,
    int shutdownOption, double dt_hour)
    : dt_hour(dt_hour), m_unitPowerMax_kW(unitPowerMax_kW),
      m_unitPowerMin_kW(unitPowerMin_kW), m_startup_hours(startup_hours),
      m_is_started(is_started), m_shutdown_hours(shutdown_hours),
      ...
      m_efficiencyChoice(efficiencyChoice), m_efficiencyTable(efficiencyTable),
      m_lowerHeatingValue_BtuPerFt3(lowerHeatingValue_BtuPerFt3),
      m_higherHeatingValue_BtuPerFt3(higherHeatingValue_BtuPerFt3),
      m_availableFuel_MCf(availableFuel_Mcf), m_shutdownOption(shutdownOption),
      m_powerMax_kW(unitPowerMax_kW), m_power_kW(0),
      m_powerMaxPercentOfOriginal_percent(0), m_powerLoad_percent(0),
      m_powerPrevious_kW(0), m_replacementCount(0) {
  ...
  for (size_t r = 0; r < m_efficiencyTable.nrows(); r++) {
    ...
    if (m_efficiencyTable.at(r, FC_EFFICIENCY_COLUMN::PERCENT_MAX) > 0 &&
        m_efficiencyTable.at(
            r, FC_EFFICIENCY_COLUMN::PRECENT_ELECTRICAL_EFFICIENCY) > 0) {
      // Fuel consumption Btu = dt_hour * operating power kW / (efficiency at
      // LHV * Fuel HHV / Fuel LHV) * BTU_PER_KWH
      double fuelConsumption_Btu =
          BTU_PER_KWH * dt_hour *
          m_efficiencyTable.at(r, FC_EFFICIENCY_COLUMN::PERCENT_MAX) *
          m_unitPowerMax_kW /
          (m_efficiencyTable.at(
               r, FC_EFFICIENCY_COLUMN::PRECENT_ELECTRICAL_EFFICIENCY) *
           m_higherHeatingValue_BtuPerFt3 / m_lowerHeatingValue_BtuPerFt3);
      fuelConsumption_Mcf =
          BTU_TO_MCF(fuelConsumption_Btu, m_lowerHeatingValue_BtuPerFt3);
    }

    m_fuelConsumptionMap_MCf[m_efficiencyTable.at(
        r, FC_EFFICIENCY_COLUMN::PERCENT_MAX)] = (fuelConsumption_Mcf);
    m_efficiencyMap[m_efficiencyTable.at(r,
                                         FC_EFFICIENCY_COLUMN::PERCENT_MAX)] =
        m_efficiencyTable.at(
            r, FC_EFFICIENCY_COLUMN::PRECENT_ELECTRICAL_EFFICIENCY);
    m_heatRecoveryMap[m_efficiencyTable.at(r,
                                           FC_EFFICIENCY_COLUMN::PERCENT_MAX)] =
        m_efficiencyTable.at(r, FC_EFFICIENCY_COLUMN::PERCENT_HEAT_RECOVERY);
  }
...
FuelCell::FuelCell(const FuelCell &fuelCell)
    : dt_hour(fuelCell.dt_hour), m_unitPowerMax_kW(fuelCell.m_unitPowerMax_kW),
...
void FuelCell::init() {
  ...
  m_hoursSinceStop = 0;
  m_hoursRampUp =
      ceilf((float)(m_unitPowerMin_kW / m_dynamicResponseUp_kWperHour));
...
bool FuelCell::isInitialized() { return m_initialized; }

bool FuelCell::isStarting() { return m_startingUp; }

bool FuelCell::isRunning() { return m_startedUp; }

bool FuelCell::isShutDown() { return m_shutDown; }

bool FuelCell::isShuttingDown() { return m_shuttingDown; }

double FuelCell::interpolateMap(double key,
...
double FuelCell::getLoadFraction() { return getPercentLoad() * 0.01; }
...
void FuelCell::checkPowerResponse() {
  ...
  if (sign > 0) {
    m_power_kW =
        fmin(m_power_kW, (m_powerPrevious_kW + (dP_max * dt_hour * sign)));
  }
  // Limit output to the maximum of the power requested and ramp rate limit
  else {
    m_power_kW =
        fmax(m_power_kW, (m_powerPrevious_kW + (dP_max * dt_hour * sign)));
  }
}
double FuelCell::getPower() { return m_power_kW; }
double FuelCell::getPowerMaxPercent() {
  return m_powerMaxPercentOfOriginal_percent;
}
double FuelCell::getPowerThermal() { return m_powerThermal_kW; }
double FuelCell::getMaxPowerOriginal() { return m_unitPowerMax_kW; }
double FuelCell::getMaxPower() { return m_powerMax_kW; }
double FuelCell::getMinPower() { return m_unitPowerMin_kW; }
double FuelCell::getFuelConsumption() { return m_fuelConsumed_MCf; }
double FuelCell::getAvailableFuel() { return m_availableFuel_MCf; }
double FuelCell::getElectricalEfficiency() { return m_efficiency_percent; }
double FuelCell::getHeatRecoveryEfficiency() { return m_heatRecovery_percent; }
int FuelCell::getTotalReplacements() { return m_replacementCount; }
void FuelCell::resetReplacements() { m_replacementCount = 0; }

void FuelCell::setSystemProperties(double nameplate_kW, double min_kW,
...
void FuelCell::setStartupHours(double startup_hours, bool is_started) {
  m_startup_hours = startup_hours;
  // Assume that this function is only called at the beginning of a simulation
  // and implies that fuel cell already running
  //	if (startup_hours == 0) {
  if (is_started) {
...
void FuelCell::checkStatus(double power_kW) {
  ...
  if (!isShuttingDown() && !isRunning() && (power_kW > 0 || isStarting()) &&
      m_availableFuel_MCf > 0 && m_powerMax_kW > m_unitPowerMin_kW) {
    m_hoursSinceStart += dt_hour;

    // Fully started once past the startup hour criteria
    if ((m_hoursSinceStart > m_startup_hours) ||
        (m_hour <= m_startup_hours && m_is_started)) {
      m_startedUp = true;
      m_startingUp = false;
      m_power_kW = power_kW;
  ...
  // Scheduled shutdowns
    ...
    for (size_t r = 0; r < m_scheduledShutdowns.nrows(); r++) {
      double shutdown_hourOfYear = (double)m_scheduledShutdowns.at(
          r, FuelCell::FC_SHUTDOWN_COLUMN::HOUR);
      double duration_hours = (double)m_scheduledShutdowns.at(
          r, FuelCell::FC_SHUTDOWN_COLUMN::DURATION);
      ...
      if (duration_hours > 0) {
        ...
        if (m_hour >= shutdown_hourOfYear &&
            m_hour < shutdown_hourOfYear + duration_hours) {
          m_power_kW = 0;
          m_hoursSinceStop += dt_hour;
          break;
        }
...
void FuelCell::checkMinTurndown() {
  ...
  else if (m_power_kW < m_unitPowerMin_kW &&
           m_hoursSinceStart > m_startup_hours + m_hoursRampUp) {
...
void FuelCell::applyDegradation() {
  ...
  } else if (m_replacementOption ==
             FC_REPLACEMENT_OPTION::REPLACE_ON_SCHEDULE) {
