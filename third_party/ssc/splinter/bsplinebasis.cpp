...
BSplineBasis::BSplineBasis(std::vector<std::vector<double>> &knotVectors,
  ...
  if (knotVectors.size() != basisDegrees.size())
    throw Exception("BSplineBasis::BSplineBasis: Incompatible sizes. Number of "
                    "knot vectors is not equal to size of degree vector.");
  ...
  for (unsigned int i = 0; i < numVariables; i++) {
    ...
    if (numVariables > 2) {
      ...
      bases.at(i).setNumBasisFunctionsTarget((basisDegrees.at(i) + 1) +
                                             1); // Minimum degree+1
    }
  }
}
...
SparseVector BSplineBasis::eval(const DenseVector &x) const {
  // Evaluate basisfunctions for each variable i and compute the tensor product
...
SparseMatrix BSplineBasis::reduceSupport(std::vector<double> &lb,
  ...
  if (lb.size() != ub.size() || lb.size() != numVariables)
    throw Exception("BSplineBasis::reduceSupport: Incompatible dimension of "
                    "domain bounds.");
...
BSplineBasis1D BSplineBasis::getSingleBasis(int dim) { return bases.at(dim); }
