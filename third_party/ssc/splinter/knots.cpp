...
bool isKnotVectorRegular(const std::vector<double> &knots,
                         unsigned int degree) {
  // Check size
...
bool isKnotVectorClamped(const std::vector<double> &knots,
                         unsigned int degree) {
...
bool isKnotVectorRefinement(const std::vector<double> &knots,
  ...
  // Check that each element in knots occurs at least as many times in
  // refinedKnots
  for (std::vector<double>::const_iterator it = knots.begin();
       it != knots.end(); ++it) {
