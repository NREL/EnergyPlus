...
{
  //    if (degree <= 0)
  //        throw Exception("BSplineBasis1D::BSplineBasis1D: Cannot create
...
SparseMatrix BSplineBasis1D::insertKnots(double tau,
...
SparseMatrix BSplineBasis1D::refineKnots() {
  ...
  if (!isKnotVectorRefinement(knots, refinedKnots))
    throw Exception("BSplineBasis1D::refineKnots: New knot vector is not a "
                    "proper refinement!");
...
SparseMatrix BSplineBasis1D::refineKnotsLocally(double x) {
  ...
  if (!isKnotVectorRefinement(knots, refinedKnots))
    throw Exception("BSplineBasis1D::refineKnotsLocally: New knot vector is "
                    "not a proper refinement!");
...
SparseMatrix BSplineBasis1D::decomposeToBezierForm() {
  ...
  // Start at first knot and add knots until all knots have multiplicity degree
  // + 1
...
SparseMatrix BSplineBasis1D::buildKnotInsertionMatrix(
  ...
  if (!isKnotVectorRegular(refinedKnots, degree))
    throw Exception("BSplineBasis1D::buildKnotInsertionMatrix: New knot vector "
                    "is not regular!");
  ...
  for (unsigned int i = 0; i < m; i++) {
    ...
    if (R.rows() != 1 || R.cols() != (int)degree + 1) {
      throw Exception("BSplineBasis1D::buildKnotInsertionMatrix: Incorrect "
                      "matrix dimensions!");
    }
...
SparseMatrix BSplineBasis1D::reduceSupport(double lb, double ub) {
  ...
  if (numOld < numNew)
    throw Exception("BSplineBasis1D::reduceSupport: Number of basis functions "
                    "is increased instead of reduced!");
