...
DataPoint::DataPoint(std::vector<double> x, double y) { setData(x, y); }
...
bool DataPoint::operator<(const DataPoint &rhs) const {
  if (this->getDimX() != rhs.getDimX())
    throw Exception("DataPoint::operator<: Cannot compare data points of "
...
double dist(const std::vector<double> x, const std::vector<double> y) {
  if (x.size() != y.size())
    throw Exception("DataPoint::dist: Cannot measure distance between two "
