...
BSpline::Builder::Builder(const DataTable &data)
    : _data(data), _degrees(getBSplineDegrees(data.getNumVariables(), 3)),
      _numBasisFunctions(std::vector<unsigned int>(data.getNumVariables(), 0)),
      _knotSpacing(KnotSpacing::AS_SAMPLED), _smoothing(Smoothing::NONE),
      _alpha(0.1) {}
...
BSpline BSpline::Builder::build() const {
  ...
  if (!_data.isGridComplete())
    throw Exception("BSpline::Builder::build: Cannot create B-spline from "
                    "irregular (incomplete) grid.");
 ...
 * b = vector of m sample points y-values (or x-values when calculating knot
 * averages), x = B-spline coefficients (or knot averages), R = Regularization
 * matrix, alpha = regularization parameter.
 */
DenseVector
BSpline::Builder::computeCoefficients(const BSpline &bspline) const {
  ...
  if (_smoothing == Smoothing::IDENTITY) {
     ...
     * ||Ax-b||^2 + alpha*x^T*x
     *
     * NOTE: This corresponds to a Tikhonov regularization (or ridge regression)
     * with the Identity matrix. See:
    ...
    /*
     * The P-Spline is a smooting B-spline which relaxes the interpolation
     * constraints on the control points to allow smoother spline curves. It
     * minimizes an objective which penalizes both deviation from sample points
     * (to lower bias) and the magnitude of second derivatives (to lower
     * variance).
     *
  ...
  if (!solveAsDense) {
#ifndef NDEBUG
    std::cout << "BSpline::Builder::computeBSplineCoefficients: Computing "
                 "B-spline control points using sparse solver."
              << std::endl;
#endif // NDEBUG
  ...
  if (solveAsDense) {
#ifndef NDEBUG
    std::cout << "BSpline::Builder::computeBSplineCoefficients: Computing "
                 "B-spline control points using dense solver."
              << std::endl;
#endif // NDEBUG
    ...
    if (!s.solve(Ad, b, x)) {
      throw Exception("BSpline::Builder::computeBSplineCoefficients: Failed to "
                      "solve for B-spline coefficients.");
    }
  }
...
BSpline::Builder::computeBasisFunctionMatrix(const BSpline &bspline) const {
  ...
  // A.reserve(DenseVector::Constant(numSamples, nnzPrCol)); // TODO: should
  // reserve nnz per row!
...
/*
 * Function for generating second order finite-difference matrix, which is used
 * for penalizing the (approximate) second derivative in control point
 * calculation for P-splines.
 */
SparseMatrix BSpline::Builder::getSecondOrderFiniteDifferenceMatrix(
  ...
  for (unsigned int i = 0; i < numVariables; ++i)
    if (numBasisFunctions.at(i) < 3)
      throw Exception(
          "BSpline::Builder::getSecondOrderDifferenceMatrix: Need at least "
          "three coefficients/basis function per variable.");
...
std::vector<std::vector<double>> BSpline::Builder::computeKnotVectors() const {
  if (_data.getNumVariables() != _degrees.size())
    throw Exception("BSpline::Builder::computeKnotVectors: Inconsistent sizes "
                    "on input vectors.");
  ...
  for (unsigned int i = 0; i < _data.getNumVariables(); ++i) {
    // Compute knot vector
    auto knotVec =
        computeKnotVector(grid.at(i), _degrees.at(i), _numBasisFunctions.at(i));
...
std::vector<double>
BSpline::Builder::computeKnotVector(const std::vector<double> &values,
                                    unsigned int degree,
 ...
 * That is, samples (a,b,c,d,e,f) produces the knot vector (a,a,a,a,c,d,f,f,f,f)
 * for p = 3. For p = 1, (a,b,c,d,e,f) becomes (a,a,b,c,d,e,f,f).
 *
 * TODO:
 * Does not work well when number of knots is << number of samples! For such
 * cases almost all knots will lie close to the left samples. Try a bucket
 * approach, where the samples are added to buckets and the knots computed as
 * the average of these.
 */
std::vector<double>
BSpline::Builder::knotVectorMovingAverage(const std::vector<double> &values,
...
std::vector<double>
BSpline::Builder::knotVectorBuckets(const std::vector<double> &values,
                                    unsigned int degree,
  ...
  if (ni > unique.size() - degree - 1)
    throw Exception("BSpline::Builder::knotVectorBuckets: Invalid number of "
                    "internal knots!");
