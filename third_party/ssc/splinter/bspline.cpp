...
BSpline::BSpline(std::vector<std::vector<double>> knotVectors,
    ...
    : Function(knotVectors.size()),
      ...
      coefficients(DenseVector::Zero(1)), knotaverages(computeKnotAverages()) {
...
BSpline::BSpline(DenseVector coefficients,
    ...
    : Function(knotVectors.size()),
      basis(BSplineBasis(knotVectors, basisDegrees)),
      coefficients(coefficients), knotaverages(computeKnotAverages()) {
...
BSpline::BSpline(const std::string &fileName) : Function(1) { load(fileName); }
...
SparseMatrix BSpline::evalBasisJacobian(DenseVector x) const {
  ...
  // SparseMatrix Bi = basis.evalBasisJacobian(x);       // Sparse Jacobian
  // implementation SparseMatrix Bi = basis.evalBasisJacobian2(x);  // Sparse
...
void BSpline::setControlPoints(const DenseMatrix &controlPoints) {
  if (controlPoints.cols() != numVariables + 1)
    throw Exception("BSpline::setControlPoints: Incompatible size of control "
                    "point matrix.");
...
void BSpline::updateControlPoints(const DenseMatrix &A) {
  if (A.cols() != coefficients.rows() || A.cols() != knotaverages.rows())
    throw Exception("BSpline::updateControlPoints: Incompatible size of linear "
...
void BSpline::checkControlPoints() const {
  if (coefficients.rows() != knotaverages.rows())
    throw Exception("BSpline::checkControlPoints: Inconsistent size of "
                    "coefficients and knot averages matrices.");
  if (knotaverages.cols() != numVariables)
    throw Exception("BSpline::checkControlPoints: Inconsistent size of knot "
                    "averages matrix.");
}
...
void BSpline::reduceSupport(std::vector<double> lb, std::vector<double> ub,
  ...
  for (unsigned int dim = 0; dim < numVariables; dim++) {
    ...
    if (ub.at(dim) <= lb.at(dim) || lb.at(dim) >= su.at(dim) ||
        ub.at(dim) <= sl.at(dim))
      throw Exception("BSpline::reduceSupport: Cannot reduce B-spline domain "
                      "to empty set!");
  ...
  if (!removeUnsupportedBasisFunctions(sl, su)) {
    throw Exception("BSpline::reduceSupport: Failed to remove unsupported "
                    "basis functions!");
  }
}
...
DenseMatrix BSpline::computeKnotAverages() const {
  ...
  // Calculate vectors of ones (with same length as corresponding knot average
  // vector)
  ...
  for (unsigned int i = 0; i < numVariables; i++) {
    ...
    if (mu_ext.rows() != basis.getNumBasisFunctions())
      throw Exception("BSpline::computeKnotAverages: Incompatible size of knot "
                      "average matrix.");
...
void BSpline::regularizeKnotVectors(std::vector<double> &lb,
  ...
  // Add and remove controlpoints and knots to make the b-spline p-regular with
  // support [lb, ub]
  if (!(lb.size() == numVariables && ub.size() == numVariables))
    throw Exception(
  ...
  for (unsigned int dim = 0; dim < numVariables; dim++) {
    ...
    // Inserting many knots at the time (to save number of B-spline coefficient
    // calculations) NOTE: This method generates knot insertion matrices with
    // more nonzero elements than the method that inserts one knot at the time.
    // This causes the preallocation of kronecker product matrices to become too
    // small and the speed deteriorates drastically in higher dimensions because
    // reallocation is necessary. This can be prevented by precomputing the
    // number of nonzeros when preallocating memory (see myKroneckerProduct).
    int numKnotsLB =
...
bool BSpline::removeUnsupportedBasisFunctions(std::vector<double> &lb,
                                              std::vector<double> &ub) {
  if (lb.size() != numVariables || ub.size() != numVariables)
    throw Exception("BSpline::removeUnsupportedBasisFunctions: Incompatible "
                    "dimension of domain bounds.");
