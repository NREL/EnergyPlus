...
DataTable::DataTable(bool allowDuplicates, bool allowIncompleteGrid)
    : allowDuplicates(allowDuplicates),
      allowIncompleteGrid(allowIncompleteGrid), numDuplicates(0),
      numVariables(0) {}

DataTable::DataTable(const char *fileName) : DataTable(std::string(fileName)) {}

DataTable::DataTable(const std::string &fileName) { load(fileName); }

void DataTable::addSample(double x, double y) { addSample(DataPoint(x, y)); }
...
void DataTable::addSample(const DataPoint &sample) {
  ...
  if (sample.getDimX() != numVariables) {
    throw Exception("Datatable::addSample: Dimension of new sample is "
                    "inconsistent with previous samples!");
  }
...
DataTable operator+(const DataTable &lhs, const DataTable &rhs) {
  if (lhs.getNumVariables() != rhs.getNumVariables()) {
    throw Exception("operator+(DataTable, DataTable): trying to add two "
                    "DataTable's of different dimensions!");
  }
...
DataTable operator-(const DataTable &lhs, const DataTable &rhs) {
  if (lhs.getNumVariables() != rhs.getNumVariables()) {
    throw Exception("operator-(DataTable, DataTable): trying to subtract two "
                    "DataTable's of different dimensions!");
  }
