...
Serializer::Serializer() { stream = StreamType(0); }
...
void Serializer::loadFromFile(const std::string &fileName) {
  ...
  // Because we opened the file at the end, tellg() will give us the size of the
  // file
  ...
  // *result.data() can therefore be treated as an array of the same type as the
  // vector
