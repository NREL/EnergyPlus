...
static var_info _cm_vtab_sp_ty[] = {
    ...
    {SSC_INPUT, SSC_STRING, "solar_resource_file",
     "Weather file in TMY2, TMY3, EPW, or SMW.", "", "", "sp_ty", "*",
     "LOCAL_FILE", ""},
...
public:
  cm_sp_ty() { add_var_info(_cm_vtab_sp_ty); }

  double powerout(double dc_nameplate, double ac_nameplate,
                  double inv_eff_percent, double loss_percent, double ts_hour,
                  double poa, double wspd, double tdry) {
  ...
  void exec() {
    ...
    double eta_csp_parasitics =
        0.93; //[-] Receiver parasitics that scale with thermal power (HTF
              //pumping power, BOP, etc)
    double dispatch_price_multiplier =
        1.0; //[-] Annualized additional value of CSP realized by dispatching
             //during peak demand
    ...
    double eta_pv_losses =
        0.946; //[-] Other pv losses (from detailed PV model Losses page)
    double Q_delivered_CSP_min =
        990.0; //[kWh/m2] Minimum annual thermal energy delivered by heliostat
               //for 'standard' case
    double availability_csp =
        0.96; //[-] Fraction of annual energy calculated assuming continuous
              //operation that can be realized by actual plant considering O&M
              //and other shutdowns
    double A_hel = 20.0;           //[m2] Heliostat structure area
    double A_hel_refl_frac = 0.97; //[-] Reflective surface ratio
    double pv_cost_frac = 0.5; //[-] Cost of PV installed on heliostat relative
                               //to pv-only installation
    double annual_output_pv = 6009.0; //[kWh] PVWatts calculated output for
                                      //'pv_watts_nameplate' calculated below
    ...
    // But really, this floats depending on the PV efficiency for the given
    // conditions double eta_hel_breakeven =
    // (pv_area_frac*eta_pv*eta_inverter*eta_pv_losses) /
    // (eta_rec_therm*eta_cycle*eta_csp_parasitics / dispatch_price_multiplier);

    std::string file_dir =
        "C:/Users/tneises/Documents/2015 LPDP/csp pv/rotate vs static "
        "cavity/Analysis with helfieldpos 180 field";
    ...
    for (int i = 0; i < n_eta_files; i++) {
      ...
      if (!fp) {
        throw exec_error("SP_ty",
                         util::format("there was a problem opening the file"));
      }
      ...
      if (comma1 == std::string::npos) {
        throw exec_error(
            "SP_ty", util::format("file does not seem to have zenith angle"));
      }
      ...
      double zenith =
          90.0 - std::atof(opt_line.substr(comma1 + 1, comma2 - comma1 - 1));
      ...
      if (comma1 == std::string::npos) {
        throw exec_error(
            "SP_ty", util::format("file does not seem to have azimuth angle"));
      }
      ...
      double azimuth =
          std::atof(opt_line.substr(comma1 + 1, comma2 - comma1 - 1));
      ...
      // Now want to count the number of remaining rows to get the number of
      // heliostats
      int n_heliostats = 0;
      while (true) {
    ...
    for (int i = 1; i < n_sol_pos; i++) {
      if (n_heliostats_per_sol_pos[i] != n_heliostats_per_sol_pos[0]) {
        throw exec_error(
            "SP_ty",
            util::format(
                "All files do not contain the same number of heliostats"));
      }
    }
    int n_heliostats_field = n_heliostats_per_sol_pos[0];

    // Check that there are the same number of zenith positions for each azimuth
    // position
    ...
    // Need AT LEAST 2 zenith and 3 azimuth positions, with +/- 180 azimuth and
    // 0 zenith covered
    ...
    if (!found_zenith_90) {
      throw exec_error("SP_ty",
                       util::format("Need a zenith angle = 0 degrees"));
    }
    ...
    if (!found_az_180 || !found_az_n_180) {
      throw exec_error("SP_ty",
                       util::format("Need azimuth angles =  +/- 180 degrees"));
    }
    ...
    // If we're saving the optical efficiency for each heliostat for each solar
    // position, then n_az * n_zen * n_heliostats entries vector<double>
    // combined_eta_opt([n_az*n_zen]*n_heliostats); for each file,
    // combined_eta_opt[*index* + row (helio id)] = eta_opt_row so reference
    // index of listed_zenith and listed_azimuth to get index
    ...
    for (int i = 0; i < n_eta_files; i++) {
      // Copied from above - but can delete lines that read in solar position -
      ...
      while (true) {
        if (!lk::read_line(fp, opt_line)) {
          ...
          if (i == 0) // heliostat positions are the same in each optical
                      // efficiency file, so only need to do this once...
          {
    ...
    if (test_radius != test_radius)
      throw exec_error("sp_ty",
                       "couldn't find radius within band that had az = 0");
    ...
    for (int i = 0; i < n_heliostats_test_radius; i++) {
      for (int j = 0; j < n_heliostats_test_radius; j++) {
        if (az_hel[hel_id_test_radius[j]] < local_max &&
            az_hel[hel_id_test_radius[j]] > min_az) {
    ...
    for (int index_rec_rot = 18; index_rec_rot < 19; index_rec_rot++) {
      ...
      // For EACH heliostat at 'test_radius', run an annual simulation with the
      // generic csp model
      for (int index_nh = 0; index_nh < n_heliostats_test_radius; index_nh++) {
        ...
        if (step_per_hour < 1 || step_per_hour > 60 ||
          ...
          throw exec_error("sp_ty",
                           util::format("invalid number of data records (%d): "
        ...
        while (hour < 8760) {
          ...
          for (size_t jj = 0; jj < step_per_hour; jj++) {
            if (!wf.read())
              throw exec_error("sp_ty", "could not read data line " +
                                            util::to_string((int)(idx + 1)) +
                                            " in weather file");

            if (wf.dn < 0 || wf.dn > 1500.0) {
              log(util::format("invalid beam irradiance %lg W/m2 at time [y:%d "
                               "m:%d d:%d h:%d], set to zero",
                               wf.dn, wf.year, wf.month, wf.day, wf.hour),
            ...
            if (code != 0)
              throw exec_error(
                  "pvsamv1",
                  util::format("failed to process irradiation on surface %d "
                               "(code: %d) [y:%d m:%d d:%d h:%d]",
            ...
            if (sunup == 1) {
              ...
              double az_hel_local360 =
                  az_hel[hel_id_test_radius_az_sorted[index_nh]] + 180.0;
              ...
              for (int i = 0; i < n_heliostats_test_radius; i++) {
                if (az_hel[hel_id_test_radius_az_sorted[i]] >
                    delta_az_hel_rec) {
              ...
              if (index_hel_proxy > -1) {
                ...
                if (az_sp360 < min_az360)
                  az_solar_proxy360 =
                      az_sp360 - min_az360; // should be negative azimuth angle

                if (az_sp360 > max_az360)
                  az_solar_proxy360 =
                      az_sp360 - max_az360; // should be positive azimuth angle
                ...
                for (int i = 0; i < n_sol_pos; i++) {
                  ...
                  data[i_local + (n_azimuth)*j_local] =
                      combined_eta_opt(i, index_hel_proxy);
                }
              ...
              // Get PV output
              pv_available_relval_ts =
                  powerout(pv_watts_nameplate, ac_nameplate, inv_eff_percent,
                           loss_percent, ts_hour, poa_pv, wf.wspd, wf.tdry) /
                  1000.0; // kWe
              // double powerout(double dc_nameplate, double ac_nameplate,
              // double inv_eff_percent, double loss_percent, double ts_hour,
              // double poa, double wspd, double tdry)
              ...
              if (wf.dn > DNI_cutoff) {
                ...
                csp_available_relval_ts =
                    opt_eta_point * wf.dn * 0.001 * eta_rec_therm * eta_cycle *
                    eta_csp_parasitics * dispatch_price_multiplier * A_hel *
                    A_hel_refl_frac; // kWe
              ...
              opt_eta_point =
                  opt_eta_dni_product /
                  (950.0 * 0.001); //[-] Normalized output compared to perfect
                                   //optical efficiency at design DNI
            ...
            sum_csp_available_relval_ts +=
                csp_available_relval_ts * ts_hour; // kWeh/m2
            sum_csp_produced_relval_ts +=
                csp_produced_relval_ts * ts_hour; // kWeh/m2
            sum_pv_available_relval_ts +=
                pv_available_relval_ts * ts_hour; // kWeh/m2
            sum_pv_produced_relval_ts +=
                pv_produced_relval_ts * ts_hour; // kWeh/m2

            idx++; // Add to weatherfile line counter and repeat
          }

          // hel_csp_to_rec_hourly(index_nh, hour) = sum_opt_eta_dni_product;
          // // kW/m2
          hel_csp_to_rec_annual[index_nh] += sum_opt_eta_dni_product; // kW/m

          // csp_available_relval_hourly(index_nh, hour) =
          // sum_csp_available_relval_ts;
          csp_available_relval_annual[index_nh] += sum_csp_available_relval_ts;

          // csp_produced_relval_hourly(index_nh, hour) =
          // sum_csp_produced_relval_ts;
          csp_produced_relval_annual[index_nh] += sum_csp_produced_relval_ts;

          // pv_available_relval_hourly(index_nh, hour) =
          // sum_pv_available_relval_ts;
          pv_available_relval_annual[index_nh] += sum_pv_available_relval_ts;

          // pv_produced_relval_hourly(index_nh, hour) =
          // sum_pv_produced_relval_ts;
      ...
      // Write outputs
      std::string hel_performance_processed =
          file_dir + "/hel_performance_processed_rotate_" +
          to_string(rec_rotate_span) + ".txt";

      ofstream hel_output_file(hel_performance_processed.c_str());

      std::string out_line =
          "heliostat id,azimuth,radius,Q_deliver_hel "
          "(kWh_per_m2),CSP_avail_relval (kWeh),CSP_produced_relval "
          "(kWeh),PV_avail_relval (kWeh),PV_produced_relval "
          "(kWeh),bin0-11,bin11-20,bin21-30,bin31-40,bin41-50,bin51-60,bin61-"
          "70,bin71-80,bin81-90,bin91-100\n";

      hel_output_file << out_line;

      for (int i = 0; i < n_heliostats_test_radius; i++) {
        hel_output_file
            << to_string(hel_id_test_radius_az_sorted[i]) + "," +
                   to_string(az_hel[hel_id_test_radius_az_sorted[i]]) + "," +
                   to_string(r_hel[hel_id_test_radius_az_sorted[i]]) + "," +
                   to_string(hel_csp_to_rec_annual[i]) + "," +
                   to_string(csp_available_relval_annual[i] *
                             availability_csp) +
                   "," +
                   to_string(csp_produced_relval_annual[i] * availability_csp) +
                   "," + to_string(pv_available_relval_annual[i]) + "," +
                   to_string(availability_csp *
                                 (pv_produced_relval_annual[i] -
                                  pv_available_relval_annual[i]) +
                             pv_available_relval_annual[i]) +
    ...
    // util::matrix_t<double> hel_csp_to_rec_hourly(n_heliostats_field, 8760,
    // 0.0); util::matrix_t<double>
    // csp_available_relval_hourly(n_heliostats_field, 8760, 0.0);
    // util::matrix_t<double> csp_produced_relval_hourly(n_heliostats_field,
    // 8760, 0.0); util::matrix_t<double>
    // pv_available_relval_hourly(n_heliostats_field, 8760, 0.0);
    // util::matrix_t<double> pv_produced_relval_hourly(n_heliostats_field,
    // 8760, 0.0);
    ...
    for (int index_nh = 0; index_nh < n_heliostats_test_radius; index_nh++) {
      ...
      if (index_nh % (n_heliostats_test_radius) == 0) {
        float percent =
            100.0f * ((float)index_nh + 1) / ((float)n_heliostats_test_radius);
      ...
      for (int i = 0; i < n_sol_pos; i++) {
        ...
        data[i_local + (n_azimuth)*j_local] =
            combined_eta_opt(i, hel_id_test_radius_az_sorted[index_nh]);
      }
          ...
          if (sunup == 1) {
            ...
            pv_available_relval_ts =
                powerout(pv_watts_nameplate, ac_nameplate, inv_eff_percent,
                         loss_percent, ts_hour, poa_pv, wf.wspd, wf.tdry) /
                1000.0; // kWe
            // double powerout(double dc_nameplate, double ac_nameplate, double
            // inv_eff_percent, double loss_percent, double ts_hour, double poa,
    ...
    // Write outputs
    std::string hel_performance_processed =
        file_dir + "/hel_performance_processed_static.txt";
    ...
    std::string out_line =
        ...
        "(kWh_per_m2),CSP_avail_relval (kWeh),CSP_produced_relval "
        "(kWeh),PV_avail_relval (kWeh),PV_produced_relval "
        "(kWeh),bin0-11,bin11-20,bin21-30,bin31-40,bin41-50,bin51-60,bin61-70,"
        "bin71-80,bin81-90,bin91-100\n";
    ...
    for (int i = 0; i < n_heliostats_test_radius; i++) {
      hel_output_file
          << to_string(hel_id_test_radius_az_sorted[i]) + "," +
                 ...
                 to_string(csp_produced_relval_annual[i] * availability_csp) +
                 "," + to_string(pv_available_relval_annual[i]) + "," +
                 to_string(availability_csp * (pv_produced_relval_annual[i] -
                                               pv_available_relval_annual[i]) +
                           pv_available_relval_annual[i]) +
                 "," + to_string(annual_bin0_10[i]) + "," +
                 to_string(annual_bin11_20[i]) + "," +
