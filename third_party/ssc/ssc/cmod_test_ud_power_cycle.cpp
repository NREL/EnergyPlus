...
public:
  cm_test_ud_power_cycle() { add_var_info(_cm_vtab_test_ud_power_cycle); }
  ...
  void exec() override {
    ...
    c_pc.init(a_table, a_ref, a_low, a_high, b_table, b_ref, b_low, b_high,
    ...
    for (int i = 0; i < N_runs; i++) {
      for (int j = 0; j < N_runs; j++) {
        for (int k = 0; k < N_runs; k++) {
          ...
          Y_actual[index] =
              three_var_eqn(a_table(i, 0), b_table(j, 0), c_table(k, 0));

          Y_reg[index] = c_pc.get_W_dot_gross_ND(a_table(i, 0), b_table(j, 0),
                                                 c_table(k, 0)) *
                         Y_ref;

          E_reg_less_act[index] =
              (Y_reg[index] - Y_actual[index]) / fmax(Y_actual[index], 0.0001);
...
DEFINE_MODULE_ENTRY(test_ud_power_cycle, "Test user-defined power cylce model",
                    0)
