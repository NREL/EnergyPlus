...
void map_input(var_table *vt, const std::string &sam_name,
               var_table *reopt_table, const std::string &reopt_name,
  ...
  if (var_data *vd = reopt_table->lookup(reopt_name)) {
    if (sum) {
      ...
      throw std::runtime_error(reopt_name +
                               " variable already exists in 'reopt_table'.");
  } else {
    if (to_ratio)
...
void map_optional_input(var_table *vt, const std::string &sam_name,
                        var_table *reopt_table, const std::string &reopt_name,
...
SSCEXPORT void Reopt_size_battery_params(ssc_data_t data) {
  ...
  var_table reopt_scenario, reopt_site, reopt_electric, reopt_utility,
      reopt_load, reopt_fin, reopt_pv, reopt_batt, reopt_wind;
  ...
  map_optional_input(vt, "degradation", &reopt_pv, "degradation_pct", 0.5,
                     true);
  ...
  auto assign_matching_pv_vars = [&vt](var_table &dest, std::string pvwatts_var,
                                       std::string pvsam_var,
                                       bool ratio = false) {
    try {
  ...
  assign_matching_pv_vars(reopt_pv, "losses", "annual_total_loss_percent",
                          true);

  // Get appropriate inverter efficiency input and transform to ratio from
  // percent
  int inv_model = 0;
  ...
  if (vd) {
    std::vector<std::string> inv_eff_names = {"inv_snl_eff_cec", "inv_ds_eff",
                                              "inv_pd_eff", "inv_cec_cg_eff"};
    double eff;
    ...
    std::vector<std::string> inv_power_names = {
    ...
    map_input(vt, "inv_eff", &reopt_batt, "inverter_efficiency_pct", false,
              true);
    map_input(vt, "dc_ac_ratio", &reopt_pv, "dc_ac_ratio");
  }

  // financial inputs
  map_optional_input(vt, "itc_fed_percent", &reopt_pv, "federal_itc_pct", 0.,
                     true);
  map_optional_input(vt, "pbi_fed_amount", &reopt_pv, "pbi_us_dollars_per_kwh",
                     0.);
  map_optional_input(vt, "pbi_fed_term", &reopt_pv, "pbi_years", 0.);
  vd = reopt_pv.lookup("pbi_years");
  if (vd->num[0] < 1)
    vd->num[0] = 1; // minimum is 1, set pbi_fed_amount to 0 to disable

  map_optional_input(vt, "ibi_sta_percent", &reopt_pv, "state_ibi_pct", 0.,
                     true);
  map_optional_input(vt, "ibi_sta_percent_maxvalue", &reopt_pv,
                     "state_ibi_max_us_dollars", 10000000000);
  vd = reopt_pv.lookup("state_ibi_max_us_dollars");
  if (vd->num[0] > 10000000000)
    vd->num[0] = 10000000000;

  map_optional_input(vt, "ibi_uti_percent", &reopt_pv, "utility_ibi_pct", 0.,
                     true);
  map_optional_input(vt, "ibi_uti_percent_maxvalue", &reopt_pv,
                     "utility_ibi_max_us_dollars", 10000000000);
  ...
  if (vd && !vd2) {
    ...
    reopt_pv.assign("om_cost_us_dollars_per_kw",
                    (vd->num[0] / system_cap) + vd2->num[0]);
  }

  vd = vt->lookup("total_installed_cost");
  if (vd) {
    reopt_pv.assign("installed_cost_us_dollars_per_kw",
                    vd->num[0] / system_cap);
  }
  ...
  if (vd && vd2) {
    // ReOpt's internal_efficient_pct = SAM's (batt_dc_ac_efficiency +
    // batt_ac_dc_efficiency)/2
    reopt_batt.assign("internal_efficiency_pct",
                      (vd->num[0] + vd2->num[0]) / 200.);
  } else if (vd && !vd2) {
  ...
  if ((vd = vt->lookup("batt_replacement_schedule"))) {
    vec = vd->arr_vector();
    if (vec.size() > 1)
      log += "Warning: only first value of 'batt_replacement_schedule' array "
             "is used for the ReOpt input 'battery_replacement_year'.\n";
  ...
  map_optional_input(vt, "value_of_lost_load", &reopt_fin,
  ...
  if (vd && vd2) {
    reopt_fin.assign("offtaker_tax_pct",
                     vd->num[0] / 100. + vd2->num[0] / 100.);
  }
  ...
  if (vd)
    ...
    val2 = 6.4;
  reopt_fin.assign("offtaker_discount_pct",
                   (1 + val1 / 100.) * (1 + val2 / 100.) - 1);
  ...
  if (vd && !vd2) {
    ...
    reopt_pv.assign("om_cost_escalation_pct",
                    (vd->num[0] / system_cap) + vd2->num[0]);
  }
  ...
  if (sim_len != 8760 && sim_len != 8760 * 2 && sim_len != 8760 * 4) {
    throw std::runtime_error("Load profile must be hourly, 30 min or 15 min "
  ...
  if (vec.size() != sim_len) {
    throw std::runtime_error(
        "Critical load profile's length must be same as for load.");
  }
  ...
  reopt_scenario.assign_match_case("time_steps_per_hour",
                                   var_data((int)(sim_len / 8760)));
