...
bool try_get_rate_schedule(var_table *vt, const std::string &ssc_name,
                           util::matrix_t<double> &schedule_matrix) {
...
bool try_get_rate_structure(
    var_table *vt, const std::string &ssc_name, bool power_units,
  ...
  size_t n_periods = (size_t)(*(std::max_element(
      rate_matrix.begin(), rate_matrix.end(),
    ...
    if (!power_units) {
      ...
      if (unit_type == 0 || ((unit_type == -1 && max > 1e36)))
        ...
        throw(std::runtime_error(
            "ElectricityRates_format_as_URDBv7 error. Unit type in " +
            ssc_name + " not allowed."));
...
SSCEXPORT void ElectricityRates_format_as_URDBv7(ssc_data_t data) {
  ...
  case 3:
    throw(
        std::runtime_error("ElectricityRates_format_as_URDBv7 error. "
                           "ur_net_metering_option not available in URDBv7."));
  case 2:
  ...
  case 4:
    ...
    break;
  default:
    throw(std::runtime_error("ElectricityRates_format_as_URDBv7 error. "
                             "ur_net_metering_option not recognized."));
  }
  ...
  if (vt->is_assigned("ur_dc_flat_mat")) {
    ...
    for (size_t i = 0; i < n_rows; i++) {
      ...
      for (size_t j = 0; j < flat_demand_structure.size(); j++) {
        ...
        double j_charge =
            flat_demand_structure[j][0].table.lookup("rate")->num[0];
