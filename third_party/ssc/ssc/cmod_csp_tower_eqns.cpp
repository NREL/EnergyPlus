...
void MSPT_System_Design_Equations(ssc_data_t data) {
  ...
  double P_ref, gross_net_conversion_factor, nameplate, design_eff, solarm,
      q_pb_design, q_rec_des, tshours, tshours_sf;

  // nameplate
  ssc_data_t_get_number(data, "P_ref", &P_ref);
  ssc_data_t_get_number(data, "gross_net_conversion_factor",
  ...
  ssc_data_t_get_number(
      data, "P_ref",
      &P_ref); // repeat assessors because the values may have changed
...
void Tower_SolarPilot_Solar_Field_Equations(ssc_data_t data) {
  ...
  double land_max, h_tower, land_max_calc, helio_height, helio_width,
      dens_mirror, csp_pt_sf_heliostat_area, land_min, land_min_calc,
      csp_pt_sf_fixed_land_area, land_area_base, csp_pt_sf_land_overhead_factor,
      csp_pt_sf_total_land_area, a_sf_ui, helio_area_tot,
      csp_pt_sf_tower_height, c_atm_0, c_atm_1, c_atm_2, c_atm_3, c_atm_info,
      helio_optical_error_mrad, error_equiv, field_model_type, q_rec_des,
      q_design, dni_des, dni_des_calc, opt_flux_penalty, n_hel, override_opt,
      is_optimize, override_layout, opt_algorithm;
  ...
  csp_pt_sf_heliostat_area =
      Csp_pt_sf_heliostat_area(helio_height, helio_width, dens_mirror);
  ssc_data_t_set_number(data, "csp.pt.sf.heliostat_area",
                        csp_pt_sf_heliostat_area);
  ...
  // ssc_data_t_get_number(data, "csp_pt_sf_heliostat_area",
  // &csp_pt_sf_heliostat_area); csp_pt_sf_total_reflective_area =
  // Csp_pt_sf_total_reflective_area(n_hel, csp_pt_sf_heliostat_area);
  // ssc_data_t_set_number(data, "csp_pt_sf_total_reflective_area",
  // csp_pt_sf_total_reflective_area);
  ...
  ssc_data_t_get_number(data, "csp.pt.sf.fixed_land_area",
                        &csp_pt_sf_fixed_land_area);
  ssc_data_t_get_number(data, "land_area_base", &land_area_base);
  ssc_data_t_get_number(data, "csp.pt.sf.land_overhead_factor",
                        &csp_pt_sf_land_overhead_factor);
  csp_pt_sf_total_land_area =
      Csp_pt_sf_total_land_area(csp_pt_sf_fixed_land_area, land_area_base,
                                csp_pt_sf_land_overhead_factor);
  ssc_data_t_set_number(data, "csp.pt.sf.total_land_area",
                        csp_pt_sf_total_land_area);
  ...
  // c_atm_info
  // ssc_data_t_get_matrix(vt, "helio_positions", helio_positions);        //
  // THIS IS A PROBLEM: (getting the same ssc_data_t matrix data must also be
  // changing it)
  ...
  ssc_data_t_get_number(data, "h_tower", &h_tower);
  c_atm_info =
      C_atm_info(helio_positions, c_atm_0, c_atm_1, c_atm_2, c_atm_3, h_tower);
  ssc_data_t_set_number(data, "c_atm_info", c_atm_info);

  // error_equiv
  ssc_data_t_get_number(data, "helio_optical_error_mrad",
  ...
  success = ssc_data_t_get_number(data, "field_model_type",
                                  &assigned_field_model_type);
  if (!success) {
    assigned_field_model_type = -1.;
  }
  field_model_type =
      Field_model_type(is_optimize, override_layout,
                       static_cast<int>(assigned_field_model_type));
...
void MSPT_Receiver_Equations(ssc_data_t data) {
  ...
  double csp_pt_rec_max_oper_frac, q_rec_des, csp_pt_rec_htf_c_avg,
      t_htf_hot_des, t_htf_cold_des, csp_pt_rec_max_flow_to_rec,
      csp_pt_rec_htf_t_avg, rec_d_spec, csp_pt_rec_cav_ap_hw_ratio,
      csp_pt_rec_cav_ap_height, d_rec, rec_height, rec_aspect, h_tower,
      piping_length_mult, piping_length_const, piping_length, piping_loss,
      piping_loss_tot, rec_htf;
  ...
  // ssc_data_t_set_number(data, "csp_pt_rec_cav_lip_height",
  // csp_pt_rec_cav_lip_height);
  ...
  // ssc_data_t_set_number(data, "csp_pt_rec_cav_panel_height",
  // csp_pt_rec_cav_panel_height);
  ...
  csp_pt_rec_htf_c_avg =
      Csp_pt_rec_htf_c_avg(csp_pt_rec_htf_t_avg, rec_htf, field_fl_props);
  ssc_data_t_set_number(data, "csp.pt.rec.htf_c_avg", csp_pt_rec_htf_c_avg);

  // csp_pt_rec_max_flow_to_rec
  ssc_data_t_get_number(data, "csp.pt.rec.max_oper_frac",
                        &csp_pt_rec_max_oper_frac);
  ssc_data_t_get_number(data, "q_rec_des", &q_rec_des);
  ssc_data_t_get_number(data, "csp.pt.rec.htf_c_avg", &csp_pt_rec_htf_c_avg);
  ssc_data_t_get_number(data, "t_htf_hot_des", &t_htf_hot_des);
  ssc_data_t_get_number(data, "t_htf_cold_des", &t_htf_cold_des);
  csp_pt_rec_max_flow_to_rec = Csp_pt_rec_max_flow_to_rec(
      csp_pt_rec_max_oper_frac, q_rec_des, csp_pt_rec_htf_c_avg, t_htf_hot_des,
      t_htf_cold_des);
  ssc_data_t_set_number(data, "csp.pt.rec.max_flow_to_rec",
                        csp_pt_rec_max_flow_to_rec);

  // This one doesn't seem to be used
  // csp_pt_rec_cav_ap_height
  ssc_data_t_get_number(data, "rec_d_spec", &rec_d_spec);
  ssc_data_t_get_number(data, "csp.pt.rec.cav_ap_hw_ratio",
                        &csp_pt_rec_cav_ap_hw_ratio);
  csp_pt_rec_cav_ap_height =
      Csp_pt_rec_cav_ap_height(rec_d_spec, csp_pt_rec_cav_ap_hw_ratio);
  ssc_data_t_set_number(data, "csp.pt.rec.cav_ap_height",
                        csp_pt_rec_cav_ap_height);
  ...
  piping_length =
      Piping_length(h_tower, piping_length_mult, piping_length_const);
...
void MSPT_System_Control_Equations(ssc_data_t data) {
  ...
  double bop_par, bop_par_f, bop_par_0, bop_par_1, bop_par_2, p_ref,
      csp_pt_par_calc_bop, aux_par, aux_par_f, aux_par_0, aux_par_1, aux_par_2,
      csp_pt_par_calc_aux, disp_wlim_maxspec, constant, disp_wlim_max;
  ...
  ssc_data_t_get_number(data, "p_ref", &p_ref);
  csp_pt_par_calc_bop = Csp_pt_par_calc_bop(bop_par, bop_par_f, bop_par_0,
                                            bop_par_1, bop_par_2, p_ref);
  ...
  ssc_data_t_get_number(data, "p_ref", &p_ref);
  csp_pt_par_calc_aux = Csp_pt_par_calc_aux(aux_par, aux_par_f, aux_par_0,
                                            aux_par_1, aux_par_2, p_ref);
  ...
  ssc_data_t_get_number(data, "disp_wlim_maxspec",
                        &disp_wlim_maxspec); // if coming from UI
  ...
  if (std::isnan(constant)) {
    ssc_data_t_get_number(data, "adjust:constant",
                          &constant); // if coming from LK script
  }
  ...
  if (!vt->is_assigned("wlim_series")) {
    ...
    ssc_data_t_set_array(data, "wlim_series", wlim_series.data(),
                         wlim_series.ncells());
  }
}
...
void Tower_SolarPilot_Capital_Costs_MSPT_Equations(ssc_data_t data) {
  ...
  double d_rec, rec_height, receiver_type_double, rec_d_spec,
      csp_pt_rec_cav_ap_height, csp_pt_cost_receiver_area, p_ref, design_eff,
      tshours, csp_pt_cost_storage_mwht, demand_var,
      csp_pt_cost_power_block_mwe;
  int receiver_type;
  ...
  if (std::isnan(receiver_type_double)) {
    receiver_type =
        0; // not passed to LK script, assume a external, non-cavity receiver
  } else {
    receiver_type = static_cast<int>(receiver_type_double);
  }
  ssc_data_t_get_number(data, "rec_d_spec", &rec_d_spec);
  ssc_data_t_get_number(data, "csp.pt.rec.cav_ap_height",
                        &csp_pt_rec_cav_ap_height);
  csp_pt_cost_receiver_area = Csp_pt_cost_receiver_area(
      tower_type, d_rec, rec_height, static_cast<int>(receiver_type),
      rec_d_spec, csp_pt_rec_cav_ap_height);
  ssc_data_t_set_number(data, "csp.pt.cost.receiver.area",
  ...
  csp_pt_cost_storage_mwht =
      Csp_pt_cost_storage_mwht(tower_type, p_ref, design_eff, tshours);
  ssc_data_t_set_number(data, "csp.pt.cost.storage_mwht",
                        csp_pt_cost_storage_mwht);

  ssc_data_t_get_number(data, "p_ref", &p_ref);
  demand_var = std::numeric_limits<double>::quiet_NaN();
  csp_pt_cost_power_block_mwe =
      Csp_pt_cost_power_block_mwe(tower_type, p_ref, demand_var);
  ssc_data_t_set_number(data, "csp.pt.cost.power_block_mwe",
                        csp_pt_cost_power_block_mwe);
...
void Tower_SolarPilot_Capital_Costs_DSPT_Equations(ssc_data_t data) {
  ...
  double d_rec, rec_height, receiver_type, rec_d_spec, csp_pt_rec_cav_ap_height,
      csp_pt_cost_receiver_area, p_ref, design_eff, tshours,
      csp_pt_cost_storage_mwht, demand_var, csp_pt_cost_power_block_mwe;
  ...
  csp_pt_cost_receiver_area = Csp_pt_cost_receiver_area(
      tower_type, d_rec, rec_height, static_cast<int>(receiver_type),
      rec_d_spec, csp_pt_rec_cav_ap_height);
  ssc_data_t_set_number(data, "csp.pt.cost.receiver_area",
                        csp_pt_cost_receiver_area);
