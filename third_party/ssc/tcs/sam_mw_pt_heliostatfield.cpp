...
public:
  struct RUN_TYPE {
    enum A { AUTO, USER_FIELD, USER_DATA };
  };
  ...
  virtual int init() {
    ...
    case sam_mw_pt_heliostatfield::RUN_TYPE::USER_DATA:
      ...
      if (nfluxmap % nfluxpos != 0) {
        message(TCS_ERROR,
                "The number of flux maps provided does not match the number of "
                "flux map sun positions provided. Please "
                "ensure that the dimensionality of each flux map is consistent "
                "and that one sun position is provided for "
    ...
    case sam_mw_pt_heliostatfield::RUN_TYPE::USER_FIELD: {
      ...
      default: {
        stringstream msg;
        msg << "Invalid Cant Type specified in SSC Heliostat Field Module. "
      ...
      if (run_type == sam_mw_pt_heliostatfield::RUN_TYPE::AUTO) {
        ...
        */
        ...
        for (int i = 0; i < 8760; i++) {
          if (!wFile.read(&wf)) {
            string msg = "solarpilot: could not read data line " +
                         util::to_string(i + 1) + " of 8760 in weather file";
            message(TCS_WARNING, msg.c_str());
          }

          mysnprintf(buf, 1023, "%d,%d,%d,%.2lf,%.1lf,%.1lf,%.1lf", wf.day,
                     wf.hour, wf.month, wf.dn, wf.tdry, wf.pres / 1000.,
                     wf.wspd);
      ...
      for (int i = 0; i < npos; i++) {
        ...
        eta_map[i * 3] = m_flux_positions.at(i).at(0) =
            fluxtab.azimuths.at(i) * 180. / pi;
        eta_map[i * 3 + 1] = m_flux_positions.at(i).at(1) =
            fluxtab.zeniths.at(i) * 180. / pi;
      ...
      for (int i = 0; i < nfl; i++) {
        for (int j = 0; j < n_flux_y; j++) {
          for (int k = 0; k < n_flux_x; k++) {
            TCS_MATRIX_INDEX(var(P_flux_maps), i * n_flux_y + j, k) =
                f->at(j, k, i);
          }
        }
      }
    ...
    case sam_mw_pt_heliostatfield::RUN_TYPE::USER_DATA: {
      ...
      for (int i = 0; i < nrows; i++) {
        sunpos.at(i).at(0) =
            TCS_MATRIX_INDEX(var(P_eta_map), i, 0) / az_scale * pi / 180.;
        sunpos.at(i).at(1) =
            TCS_MATRIX_INDEX(var(P_eta_map), i, 1) / zen_scale * pi / 180.;
  ...
  virtual int call(double time, double step, int ncall) {
    ...
    if (solzen > (pi / 2 - .001 - hel_stow_deploy) || v_wind > v_wind_max ||
      ...
      eta_field = min(max(eta_field, 0.0), 1.0) *
                  field_control; // Ensure physical behavior
        ...
        for (int j = 0; j < n_flux_y; j++) {
          for (int i = 0; i < n_flux_x; i++) {
            TCS_MATRIX_INDEX(var(O_flux_map), j, i) +=
                TCS_MATRIX_INDEX(var(P_flux_maps), imap * n_flux_y + j, i) *
                weights.at(k);
          }
        }
      }
    ...
    value(O_pparasi,
          pparasi /
              1.E3); // [MW], convert from kWe: Parasitic power for tracking
