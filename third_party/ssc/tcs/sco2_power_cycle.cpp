...
double HeatExchanger::hxr_DP(int stream, double m_dot, bool scale_DP /*kPa*/) {
  /*Returns the pressure drop(in kPa) of the specified stream(1 or 2) for the
     heat exchanger specified by 'hxr'.
          If scale_DP is .true., scales the design - point pressure drop with
     mass flow rate according to the Darcy friction factor and Blasius
     correlation. */

  if (scale_DP)
    return m_HX_des_par.m_DP_design[stream] *
           pow((m_dot / m_HX_des_par.m_m_dot_design[stream]), 1.75);
  else
    return m_HX_des_par.m_DP_design[stream];
}

double HeatExchanger::hxr_UA(double m_dot_0, double m_dot_1,
                             bool scale_UA /*kW/K*/) {
  /*Returns the UA(kW / K) of the heat exchanger specified by 'hxr'.
          If scale_UA is .true., scales the design - point UA with mass flow
     rate according to the Dittus - Boelter heat transfer correlation.*/

  if (scale_UA) {
    double m_dot_ratio = (m_dot_0 / m_HX_des_par.m_m_dot_design[0] +
                          m_dot_1 / m_HX_des_par.m_m_dot_design[1]) /
                         2.0;
...
void calculate_turbomachinery_outlet(
    double T_in /*K*/, double P_in /*kPa*/, double P_out /*kPa*/,
    double eta /*-*/, bool is_comp, int &error_code, double &enth_in /*kJ/kg*/,
    double &entr_in /*kJ/kg-K*/, double &dens_in /*kg/m3*/,
    double &temp_out /*K*/, double &enth_out /*kJ/kg*/,
    double &entr_out /*kJ/kg-K*/, double &dens_out /*kg/m3*/,
    double &spec_work /*kJ/kg*/) {
  /*Calculates the outlet state of a compressor or turbine using its isentropic
     efficiency. is_comp = .true.means the turbomachine is a compressor(w = w_s
     / eta)
  ...
  int prop_error_code =
      CO2_TP(T_in, P_in, &co2_props); // properties at the inlet conditions
  ...
  prop_error_code = CO2_PS(
      P_out, s_in,
      &co2_props); // outlet enthalpy if compression/expansion is isentropic
  if (prop_error_code != 0) {
    error_code = 2;
    return;
  }
  ...
  double w_s =
      h_in - h_s_out; // specific work if process is isentropic (negative for
                      // compression, positive for expansion)
...
void calculate_hxr_UA(int N_hxrs, double Q_dot /*units?*/, double m_dot_c,
                      double m_dot_h, double T_c_in, double T_h_in,
                      double P_c_in, double P_c_out, double P_h_in,
                      double P_h_out, int &error_code, double &UA,
                      double &min_DT) {
  /*Calculates the UA of a heat exchanger given its mass flow rates, inlet
  temperatures, and a heat transfer rate. Note: the heat transfer rate must be
  positive.*/
  ...
  for (int i = 0; i < N_nodes; i++) {
    ...
    if (i > 0) {
      double C_dot_h = m_dot_h * (h_h_prev - h_h) /
                       (T_h_prev - T_h); // [kW/K] hot stream capacitance rate
      double C_dot_c = m_dot_c * (h_c_prev - h_c) /
                       (T_c_prev - T_c); // [kW/K] cold stream capacitance rate
      double C_dot_min =
          min(C_dot_h, C_dot_c); // [kW/K] Minimum capacitance stream
      double C_dot_max =
          max(C_dot_h, C_dot_c); // [kW/K] Maximum capacitance stream
      double C_R =
          C_dot_min / C_dot_max; // [-] Capacitance ratio of sub-heat exchanger
      double eff =
          (Q_dot / (double)N_hxrs) /
          (C_dot_min *
           (T_h_prev - T_c)); // [-] Effectiveness of each sub-heat exchanger
      double NTU = 0.0;
      if (C_R != 1.0)
        NTU = log((1.0 - eff * C_R) / (1.0 - eff)) /
              (1.0 - C_R); // [-] NTU if C_R does not equal 1
      else
        NTU = eff / (1.0 - eff);
...
void compressor::solve_compressor(double T_in, double P_in, double m_dot,
                                  double N_in, int &error_code, double &T_out,
                                  double &P_out) {
  ...
  m_N = N_in;
  // Calculate the modified flow and head coefficients and efficiency for the
  // SNL compressor
  ...
  {
    m_surge = true;
    phi = m_phi_min; // reset phi to its minimum value; this sets psi and eta to
                     // be fixed at the values at the surge limit
  }

  double phi_star =
      phi * pow(m_N / m_N_design, 0.2); // modified flow coefficient
  double psi_star =
      compressor_psi_polynomial_fit(m_comp_des_par.m_type, phi_star);
  double eta_star =
      compressor_eta_polynomial_fit(m_comp_des_par.m_type, phi_star);
  double psi = psi_star / (pow(m_N_design / m_N, pow(20.0 * phi_star, 3.0)));
  double eta_0 =
      eta_star * 1.47528 /
      (pow(m_N_design / m_N,
           pow(20.0 * phi_star,
               5.0))); // Efficiency is normalized so it equals 1.0 at phi_star
                       // = 0.0297 (the compressor's design point)
  m_eta =
      max(eta_0 * m_comp_des_par.m_eta_design,
          0.0); // The actual compressor efficiency, not allowed to go negative

  // Check that the specified mass flow rate is possible with the compressor's
  // current shaft speed
  if (psi <= 0.0) {
    error_code = 16;
    return;
  ...
  double dh_s =
      psi * pow(U_tip, 2) * 0.001; // ideal enthalpy rise in compressor, from
                                   // definition of head coefficient (kJ/kg)
  ...
  prop_error_code = CO2_HS(h_s_out, s_in,
                           &co2_props); // determines compressor outlet pressure
  if (prop_error_code !=
      0) // most likely case is that the outlet pressure is above the high
         // pressure limit of the property routine
  {
    error_code = 17;
    return;
  }
  ...
  prop_error_code =
      CO2_PH(P_out, h_out, &co2_props); // determines compressor outlet
...
void turbine::solve_turbine(double T_in, double P_in, double P_out, double N_in,
                            int &error_code, double &T_out, double &m_dot_out) {
  m_N = N_in;
  ...
  double C_s = sqrt(
      (h_in - h_s_out) *
      2000.0); // Spouting velocity (m/s) [note: there is a mutiple of 2 in the
               // equation that was folded into the *1000 unit conversion]
  ...
  if (nu < 1.0)
    eta_0 =
        2.0 * nu *
        sqrt(1.0 - pow(nu, 2)); // Efficiency from Baines (1.0 at design point)
  else
    eta_0 = 0.0; // catches nu values just over 1, which leads to sqrt of
                 // negative number
  ...
  // Determine the allowed mass flow rate through the turbine and the total
  // power.
...
bool RecompCycle::design() {
  ...
  if (m_cycle_des_par.m_DP_LT[1 - cpp_offset] < 0.0)
    m_pres_last[3 - cpp_offset] =
        m_pres_last[2 - cpp_offset] -
        m_pres_last[2 - cpp_offset] *
            abs(m_cycle_des_par
                    .m_DP_LT[1 -
                             cpp_offset]); // Relative pressure drop specified
                                           // for LT recuperator (cold stream)
  else
    m_pres_last[3 - cpp_offset] =
        m_pres_last[2 - cpp_offset] -
        m_cycle_des_par
            .m_DP_LT[1 - cpp_offset]; // Absolute pressure drop specified for LT
                                      // recuperator (cold stream)

  double UA_LT = m_cycle_des_par.m_UA_rec_total * m_cycle_des_par.m_LT_frac;
  double UA_HT =
      m_cycle_des_par.m_UA_rec_total * (1 - m_cycle_des_par.m_LT_frac);

  if (UA_LT < 1.E-12)
    m_pres_last[3 - cpp_offset] =
        m_pres_last[2 - cpp_offset]; // if there is no LT recuperator, there is
                                     // no pressure drop

  m_pres_last[4 - cpp_offset] =
      m_pres_last[3 - cpp_offset]; // No pressure drop in mixing value
  m_pres_last[10 - cpp_offset] =
      m_pres_last[3 - cpp_offset]; // No pressure drop in mixing value

  if (m_cycle_des_par.m_DP_HT[1 - cpp_offset] < 0.0)
    m_pres_last[5 - cpp_offset] =
        m_pres_last[4 - cpp_offset] -
        m_pres_last[4 - cpp_offset] *
            abs(m_cycle_des_par
                    .m_DP_HT[1 -
                             cpp_offset]); // relative pressure drop specified
                                           // for HT recuperator (cold stream)
  else
    m_pres_last[5 - cpp_offset] =
        m_pres_last[4 - cpp_offset] -
        m_cycle_des_par
            .m_DP_HT[1 - cpp_offset]; // absolute pressure drop specified for HT
                                      // recuperator (cold stream)

  if (UA_HT < 1.E-12)
    m_pres_last[5 - cpp_offset] =
        m_pres_last[4 - cpp_offset]; // if there is no HT recuperator, there is
                                     // no pressure drop

  if (m_cycle_des_par.m_DP_PHX[1 - cpp_offset] < 0.0)
    m_pres_last[6 - cpp_offset] =
        m_pres_last[5 - cpp_offset] -
        m_pres_last[5 - cpp_offset] *
            abs(m_cycle_des_par
                    .m_DP_PHX[1 - cpp_offset]); // relative pressure drop
                                                // specified for PHX
  else
    m_pres_last[6 - cpp_offset] =
        m_pres_last[5 - cpp_offset] -
        m_cycle_des_par.m_DP_PHX[1 - cpp_offset]; // absolute pressure drop
                                                  // specified for PHX

  if (m_cycle_des_par.m_DP_PC[2 - cpp_offset] < 0.0)
    m_pres_last[9 - cpp_offset] =
        m_pres_last[1 - cpp_offset] /
        (1.0 -
         abs(m_cycle_des_par
                 .m_DP_PC[2 - cpp_offset])); // relative pressure drop specified
                                             // for precooler [P1 = P9 -
                                             // P9*rel_DP => P1 = P9*(1-rel_DP)
  else
    m_pres_last[9 - cpp_offset] =
        m_pres_last[1 - cpp_offset] +
        m_cycle_des_par.m_DP_PC[2 - cpp_offset]; // absolute pressure drop
                                                 // specified for precooler

  if (m_cycle_des_par.m_DP_LT[2 - cpp_offset] < 0.0)
    m_pres_last[8 - cpp_offset] =
        m_pres_last[9 - cpp_offset] /
        (1.0 -
         abs(m_cycle_des_par
                 .m_DP_LT[2 - cpp_offset])); // relative pressure drop specified
                                             // for LT recuperator (hot stream)
  else
    m_pres_last[8 - cpp_offset] =
        m_pres_last[9 - cpp_offset] +
        m_cycle_des_par
            .m_DP_LT[2 - cpp_offset]; // absolute pressure drop specified for LT
                                      // recuperator (hot stream)

  if (UA_LT < 1.E-12)
    m_pres_last[8 - cpp_offset] =
        m_pres_last[9 - cpp_offset]; // if there is no LT recup, there is no
                                     // pressure drop

  if (m_cycle_des_par.m_DP_HT[2 - cpp_offset] < 0.0)
    m_pres_last[7 - cpp_offset] =
        m_pres_last[8 - cpp_offset] /
        (1.0 - abs(m_cycle_des_par
                       .m_DP_HT[2 - cpp_offset])); // relative pressure drop
                                                   // specified for HT recup
  else
    m_pres_last[7 - cpp_offset] =
        m_pres_last[8 - cpp_offset] +
        m_cycle_des_par.m_DP_HT[2 - cpp_offset]; // absolute pressure drop
                                                 // specified for HT recup
  ...
  // Determine the outlet states of the main compressor and turbine and their
  // specific works
  calculate_turbomachinery_outlet(
      m_temp_last[1 - cpp_offset], m_pres_last[1 - cpp_offset],
      m_pres_last[2 - cpp_offset], m_cycle_des_par.m_eta_mc, true,
      sub_error_code, m_enth_last[1 - cpp_offset], m_entr_last[1 - cpp_offset],
      m_dens_last[1 - cpp_offset], m_temp_last[2 - cpp_offset],
  ...
  calculate_turbomachinery_outlet(
      m_temp_last[6 - cpp_offset], m_pres_last[6 - cpp_offset],
      m_pres_last[7 - cpp_offset], m_cycle_des_par.m_eta_t, false,
      sub_error_code, m_enth_last[6 - cpp_offset], m_entr_last[6 - cpp_offset],
      m_dens_last[6 - cpp_offset], m_temp_last[7 - cpp_offset],
  ...
  // Check to ensure this cycle can produce power under the best conditions(ie,
  // temp(9) = temp(2) if there is a recompressing compressor).
  w_rc = 0.0;
  ...
  if (m_cycle_des_par.m_recomp_frac >= 1.E-12) {
    ...
    calculate_turbomachinery_outlet(
        m_temp_last[2 - cpp_offset], m_pres_last[9 - cpp_offset],
        m_pres_last[10 - cpp_offset], m_cycle_des_par.m_eta_rc, true,
        sub_error_code, dummy[0], dummy[1], dummy[2], dummy[3], dummy[4],
        dummy[5], dummy[6], w_rc);
  ...
  if (UA_HT < 1.0E-12) // No high-temp recuperator
  {
    ...
    T8_lower_bound =
        m_temp_last[2 - cpp_offset]; // The lower possible value of temp(8)
    T8_upper_bound =
        m_temp_last[7 - cpp_offset]; // The highest possible value of temp(8)
    m_temp_last[8 - cpp_offset] = (T8_lower_bound + T8_upper_bound) *
                                  0.5; // Bisect bounds for first guess
    UA_HT_calc = -1.0;
    last_HT_residual = UA_HT; // know a priori that with T8 = T7, UA_calc = 0
                              // therefore residual is UA_HT-0
    last_T8_guess = m_temp_last[7 - cpp_offset];
  }
  ...
  for (T8_iter = 1; T8_iter <= max_iter; T8_iter++) {
    property_error_code =
        CO2_TP(m_temp_last[8 - cpp_offset], m_pres_last[8 - cpp_offset],
               &co2_props); // fully define state 8
    ...
    if (UA_LT < 1.E-12) // no low-temp recuperator
    {
      ...
      T9_lower_bound =
          m_temp_last[2 - cpp_offset]; // the lower possible value for T9
      T9_upper_bound =
          m_temp_last[8 - cpp_offset]; // the highest possible value for T9
      m_temp_last[9 - cpp_offset] = (T9_lower_bound + T9_upper_bound) *
                                    0.5; // bisect bounds for first guess
      UA_LT_calc = -1.0;
      last_LT_residual = UA_LT; // know a priori that with T9=T8, UA_calc = 0
                                // therefore residual is UA_LT - 0
      last_T9_guess = m_temp_last[8 - cpp_offset];
    }
    ...
    for (T9_iter = 1; T9_iter <= max_iter; T9_iter++) {
      ...
      if (m_cycle_des_par.m_recomp_frac >= 1.E-12) {
        calculate_turbomachinery_outlet(
            m_temp_last[9 - cpp_offset], m_pres_last[9 - cpp_offset],
            m_pres_last[10 - cpp_offset], m_cycle_des_par.m_eta_rc, true,
            sub_error_code, m_enth_last[9 - cpp_offset],
            m_entr_last[9 - cpp_offset], m_dens_last[9 - cpp_offset],
            m_temp_last[10 - cpp_offset], m_enth_last[10 - cpp_offset],
            m_entr_last[10 - cpp_offset], m_dens_last[10 - cpp_offset], w_rc);
        ...
        property_error_code = CO2_TP(m_temp_last[9 - cpp_offset],
                                     m_pres_last[9 - cpp_offset], &co2_props);
        if (property_error_code != 0) {
          m_errors.SetError(28);
          return false;
        }
        m_temp_last[10 - cpp_offset] =
            m_temp_last[9 -
                        cpp_offset]; // Assume state(10) is the same as state(9)
        m_enth_last[9 - cpp_offset] = m_enth_last[10 - cpp_offset] =
            co2_props.enth;
        m_entr_last[9 - cpp_offset] = m_entr_last[10 - cpp_offset] =
            co2_props.entr;
        m_dens_last[9 - cpp_offset] = m_dens_last[10 - cpp_offset] =
            co2_props.dens;
      }

      // Knowing the specific work of the the recompressing compressor, the
      // required mass flow rate can be determined.
      m_dot_t = m_cycle_des_par.m_W_dot_net /
                (w_mc * (1.0 - m_cycle_des_par.m_recomp_frac) +
                 w_rc * m_cycle_des_par.m_recomp_frac +
                 w_t); // total mass flow rate(through turbine)
      if (m_dot_t <
          0.0) // positive power output is not possible with these inputs
      {
      ...
      if (UA_LT <
          1.E-12) // no low-temp recuperator (this check is necessary to prevent
                  // pressure drops with UA=0 from causing problems)
        Q_dot_LT = 0.0;
      else
        Q_dot_LT = m_dot_t *
                   (m_enth_last[8 - cpp_offset] - m_enth_last[9 - cpp_offset]);

      calculate_hxr_UA(m_cycle_des_par.m_N_sub_hxrs, Q_dot_LT, m_dot_mc,
                       m_dot_t, m_temp_last[2 - cpp_offset],
                       m_temp_last[8 - cpp_offset], m_pres_last[2 - cpp_offset],
                       m_pres_last[3 - cpp_offset], m_pres_last[8 - cpp_offset],
                       m_pres_last[9 - cpp_offset], sub_error_code, UA_LT_calc,
                       min_DT_LT);

      if (sub_error_code > 0) {
        if (sub_error_code ==
            11) // second - law violation in hxr, therefore temp(9) is too low
        {
          T9_lower_bound = m_temp_last[9 - cpp_offset];
          m_temp_last[9 - cpp_offset] = (T9_lower_bound + T9_upper_bound) *
                                        0.5; // bisect bounds for next guess
      ...
      double secant_guess =
          ...
          UA_LT_residual * (last_T9_guess - m_temp_last[9 - cpp_offset]) /
              (last_LT_residual -
               UA_LT_residual); // next guess predicted using secant method

      if (UA_LT_residual <
          0.0) // UA_LT_calc is too big, temp(9) needs to be higher
      {
      ...
      last_T9_guess =
          m_temp_last[9 - cpp_offset]; // reset last stored guess value

      // Check if the secant method overshoots and fall back to bisection if it
      // does.
      if (secant_guess <= T9_lower_bound || secant_guess >= T9_upper_bound ||
    ...
    m_enth_last[3 - cpp_offset] =
        ...
        Q_dot_LT /
            m_dot_mc; // energy balance on cold stream of low-temp recuperator
    property_error_code = CO2_PH(m_pres_last[3 - cpp_offset],
                                 m_enth_last[3 - cpp_offset], &co2_props);
    if (property_error_code != 0) {
    ...
    if (m_cycle_des_par.m_recomp_frac >= 1.E-12) {
      m_enth_last[4 - cpp_offset] =
          (1.0 - m_cycle_des_par.m_recomp_frac) * m_enth_last[3 - cpp_offset] +
          m_cycle_des_par.m_recomp_frac *
              m_enth_last[10 - cpp_offset]; // conservation of energy (both
                                            // sides divided by m_dot_t
      property_error_code = CO2_PH(m_pres_last[4 - cpp_offset],
                                   m_enth_last[4 - cpp_offset], &co2_props);
      if (property_error_code != 0) {
    ...
    // recuperator.
    if (m_temp_last[4 - cpp_offset] >=
        m_temp_last[8 - cpp_offset]) // temp(8) is not valid; it must be higher
                                     // than it is
    {
    ...
    if (UA_HT <
        1.E-12) // no high-temp recuperator (this check is necessary to prevent
                // pressure drops with UA=0 from causing problems)
      Q_dot_HT = 0.0;
    else
      Q_dot_HT =
    ...
    calculate_hxr_UA(m_cycle_des_par.m_N_sub_hxrs, Q_dot_HT, m_dot_t, m_dot_t,
                     m_temp_last[4 - cpp_offset], m_temp_last[7 - cpp_offset],
                     m_pres_last[4 - cpp_offset], m_pres_last[5 - cpp_offset],
                     m_pres_last[7 - cpp_offset], m_pres_last[8 - cpp_offset],
                     sub_error_code, UA_HT_calc, min_DT_HT);

    if (sub_error_code > 0) {
      if (sub_error_code ==
          1) // 2nd law violation in hxr, therefore temp(8) is too low
      {
        T8_lower_bound = m_temp_last[8 - cpp_offset];
        m_temp_last[8 - cpp_offset] = (T8_lower_bound + T8_upper_bound) *
                                      0.5; // bisect bounds for next guess
    ...
    if (UA_HT_residual <
        0.0) // UA_HT_calc is too big, temp(8) needs to be higher
    {
    ...
    last_T8_guess =
        m_temp_last[8 - cpp_offset]; // reset last stored guess value

    // Check if the secant method overshoots and fall back to bisection if it
    // does.
    if (secant_guess <= T8_lower_bound ||
        secant_guess >= T8_upper_bound) // secant method overshot, use bisection
      m_temp_last[8 - cpp_offset] = (T8_lower_bound + T8_upper_bound) * 0.5;
    else
  ...
  m_enth_last[5 - cpp_offset] =
      m_enth_last[4 - cpp_offset] +
      Q_dot_HT /
          m_dot_t; // Energy balance on cold stream of high-temp recuperator
  property_error_code = CO2_PH(m_pres_last[5 - cpp_offset],
                               m_enth_last[5 - cpp_offset], &co2_props);
  if (property_error_code != 0) {
  ...
  // ****************************************************************
  // LT Recuperator
  property_error_code = CO2_TP(
      m_temp_last[2 - cpp_offset], m_pres_last[9 - cpp_offset],
      &co2_props); // enthalpy of hot stream outlet if temp is cold stream inlet
  if (property_error_code != 0) {
  ...
  double eff = Q_dot_LT / (m_dot_t * (m_enth_last[8 - cpp_offset] -
  h_hot_ideal)); double C_dot_cold =
  m_dot_mc*(m_enth_last[3-cpp_offset]-m_enth_last[2-cpp_offset])/(m_temp_last[3-cpp_offset]-m_temp_last[2-cpp_offset]);
  double C_dot_hot =
  m_dot_t*(m_enth_last[8-cpp_offset]-m_enth_last[9-cpp_offset])/(m_temp_last[8-cpp_offset]-m_temp_last[9-cpp_offset]);
  */
  ...
  LT_des_par.m_DP_design[0] =
      m_pres_last[2 - cpp_offset] - m_pres_last[3 - cpp_offset];
  LT_des_par.m_DP_design[1] =
  ...
  // HT recuperator
  property_error_code = CO2_TP(
      m_temp_last[4 - cpp_offset], m_pres_last[4 - cpp_offset],
      &co2_props); // enthalpy of hot steram outlet if temp is cold stream inlet
  if (property_error_code != 0) {
  ...
  C_dot_cold = m_dot_t * (m_enth_last[5-cpp_offset] -
  m_enth_last[4-cpp_offset])/(m_temp_last[5-cpp_offset]-m_temp_last[4-cpp_offset]);
  C_dot_hot = m_dot_t * (m_enth_last[7-cpp_offset] -
  m_enth_last[8-cpp_offset])/(m_temp_last[7-cpp_offset]-m_temp_last[8-cpp_offset]);
  */
  ...
  HT_des_par.m_DP_design[0] =
      m_pres_last[4 - cpp_offset] - m_pres_last[5 - cpp_offset];
  HT_des_par.m_DP_design[1] =
      m_pres_last[7 - cpp_offset] - m_pres_last[8 - cpp_offset];
  ...
  // Resize?
  PHX_des_par.m_DP_design[0] =
      m_pres_last[5 - cpp_offset] - m_pres_last[6 - cpp_offset];
...
bool RecompCycle::optimal_design() {
  // Use booleans to determine whether to optimize the design for any
  // combination of: compressor outlet pressure, pressure ratio, recompression
  // fraction, and UA distribution These boolean parameters and associated fixed
  // values (if boolean indicate no optimization of parameter) or guess values
  // (if boolean indicates optimization) should
  // ... be set during the constructor call for the RecompCycle class. These and
  // the constant cycle parameters are stored in 'cycle_design_parameters'
  // structure named 'm_cycle_des_par'
  ...
  if (index > 0) {
    ...
    // After optimization solves, get back the parameters that result in the
    // maximum efficiency
    index = 0;
  ...
  // Solve a final time with the design parameters to set all component
  // design-point information and check that it solves
  if (design()) {
    set_des_data();
    return true;
  } else {
...
double RecompCycle::design_point_eta(const std::vector<double> &x) {
  // Called by nlopt optimization routine, which varies the values in the 'x'
  // vector to maximize the thermal efficiency of the design cycle by calling
  // 'design' method
  //
...
bool RecompCycle::auto_optimal_design() {
  ...
  // Calls opt_eta, which is the outer optimization loop that optimizes the
  // upper pressure opt_eta then call optimal_design which optimizes the
  // pressure ratio, recompression fraction, and UA distribution
  double best_P_high =
      fminbr(m_cycle_des_par.m_P_high_limit * 0.2,
             m_cycle_des_par.m_P_high_limit, &fmin_callback_opt_eta, this, 1.0);

  double recomp_local = std::numeric_limits<double>::quiet_NaN();
  // Compare high pressure to limit
  if (m_cycle_des_par.m_P_high_limit - best_P_high >
      1.0) { // If the optimized high pressure is not at the limit, check
  ...
  // Run design() with optimized design parameters to get final performance
  // metrics
  if (recomp_local != m_eta_thermal_autodes) {
...
double RecompCycle::opt_eta(double P_high_opt) {
  // fmin_callback_opt_eta uses 'fmin' routine to optimize the high side
  // pressure. Each time f_min chooses a pressure, this routine sets up calls to
  // 'optimal_design' to optimize the pressure ratio, recomp fraction, and UA
  // distribution for both a simple and recompression cycle

  double P_pseudo = P_pseudocritical(m_cycle_des_par.m_T_mc_in);

  // Set information for optimization variables in cycle class member structure
  // for cycle design parameters 1) Set high side pressure to P_high_opt:
  // optimized by fmin
  ...
  // 3-4) LT_frac & recomp_frac: set depending on whether evaluating simple or
  // recomp cycle Set up Simple Cycle Configuration
  m_cycle_des_par.m_fixed_recomp_frac = true;
  m_cycle_des_par.m_recomp_frac = 0.0;
  m_cycle_des_par.m_fixed_LT_frac = true;
  m_cycle_des_par.m_LT_frac = 0.5;
  ...
  return -max(simple_local,
              recomp_local); // Minimizer, so return a negative value
}

bool RecompCycle::optimal_off_design(
    const cycle_opt_off_des_inputs &cycle_opt_off_des_in_in) {
  ...
  if (!off_design_target_power()) {
    ...
    m_q_dot_in_od = 0.0;
    m_errors.SetError(
        9999); // Need to add this error message "Error at optimal off-design
               // solution - perhaps target is not possible"
    return false;
  }
...
double
RecompCycle::off_design_target_power_function(const std::vector<double> &x) {
  int index = 0;
...
bool RecompCycle::off_design_target_power() {
  ...
  // First check the net power output using the design-point compressor inlet
  // pressure
  int cpp_offset = 1;
  ...
  if (!off_design(cycle_off_des_in)) {
    m_errors.SetError(63); // This is going to display orders in opposite order
                           // than John's code: shouldn't be a problem?
    return false;
  }
  ...
  for (iter = 0; iter < max_iter; iter++) {
    ...
    double residual =
        cycle_off_des_in.m_S.m_W_dot_net_target - m_W_dot_net_od_last;
    double secant_guess =
        P_low - residual * (last_P_low - P_low) / (last_residual - residual);
    ...
    if (abs(residual) / cycle_off_des_in.m_S.m_W_dot_net_target <=
        (1 + iter / 10) * cycle_off_des_in.m_S.m_tol)
      break;
    last_P_low = P_low;
...
bool RecompCycle::off_design(const cycle_off_des_inputs &cycle_off_des_in_in) {
  ...
  int max_iter = 100;
  double temperature_tol = 1.E-9; // Temperature differences below this value
                                  // are assumed to be zero (K)
  ...
  CO2_state co2_props;
  int prop_error_code = CO2_TP(m_temp_od_last[1 - cpp_offset],
                               m_pres_od_last[1 - cpp_offset], &co2_props);
  if (prop_error_code != 0) {
  ...
  double m_dot_mc_max = m_mc.calculate_m_dot_max(
      rho_in, N_mc_local); // Highest possible mass flow rate in main compressor
  m_dot_mc_max *= 2.0;     // Add a safety factor (helps with convergence)

  double m_dot_upper_bound =
      m_dot_mc_max /
      (1.0 -
       m_cycle_off_des_in.m_S.m_recomp_frac); // Largest possible total mass
                                              // flow rate (through turbine)
  double m_dot_lower_bound =
      0.0; // Allow surge conditions, will check during iteration and give a
           // warning if final state is surging

  double last_m_dot_guess =
      m_dot_upper_bound; // know a priori that at this mass flow rate the
                         // pressure ratio is zero
  double last_m_dot_residual =
      m_dot_upper_bound; // with zero pressure ratio there is zero mass flow
                         // rate through the turbine

  double m_dot_t_guess =
      (m_dot_lower_bound + m_dot_upper_bound) *
      0.5; // Use bisection for initial turbine mass flow rate guess

  double m_dot_mc, m_dot_rc, m_dot_t;
  m_dot_mc = m_dot_rc = m_dot_t = std::numeric_limits<double>::quiet_NaN();

  int m_dot_iter = 0;
  for (m_dot_iter = 1; m_dot_iter <= max_iter;
       m_dot_iter++) // Enter the mass flow rate iteration loop
  {
    m_dot_rc =
        m_dot_t_guess *
        m_cycle_off_des_in.m_S
            .m_recomp_frac; // mass flow rate through recompressing compressor
    m_dot_mc = m_dot_t_guess - m_dot_rc; // mass flow rate through compressor

    // Calculate the pressure rise through the main compressor
    int sub_error_code = 0;
    m_mc.solve_compressor(m_temp_od_last[1 - cpp_offset],
                          m_pres_od_last[1 - cpp_offset], m_dot_mc, N_mc_local,
                          sub_error_code, m_temp_od_last[2 - cpp_offset],
                          m_pres_od_last[2 - cpp_offset]);

    if (sub_error_code != 0) {
      if (sub_error_code ==
          16) // m_dot is too high because the given shaft speed is not possible
      {
        m_dot_upper_bound = m_dot_t_guess;
        m_dot_t_guess = (m_dot_lower_bound + m_dot_upper_bound) *
                        0.5; // Use bisection for new mass flow rate guess
        continue;
      } else if (sub_error_code == 17) // m_dot is too low because (likely)
                                       // P_out is above properties limits
      {
    ...
    // Set all state-point pressures, taking into account (possibly scaled)
    // pressure drops through heat exchangers
    m_pres_od_last[3 - cpp_offset] =
        m_pres_od_last[2 - cpp_offset] -
        m_LT.hxr_DP(1 - cpp_offset, m_dot_mc,
                    true); // through cold-side of low-temp recuperator
    m_pres_od_last[4 - cpp_offset] = m_pres_od_last[3 - cpp_offset];
    m_pres_od_last[10 - cpp_offset] = m_pres_od_last[3 - cpp_offset];
    m_pres_od_last[5 - cpp_offset] =
        m_pres_od_last[4 - cpp_offset] -
        m_HT.hxr_DP(1 - cpp_offset, m_dot_t_guess,
                    true); // through cold-side of high-temp recuperator
    m_pres_od_last[6 - cpp_offset] =
        m_pres_od_last[5 - cpp_offset] -
        m_PHX.hxr_DP(1 - cpp_offset, m_dot_t_guess,
                     true); // through CO2 (cold) side of primary heat exchanger
    m_pres_od_last[9 - cpp_offset] =
        m_pres_od_last[1 - cpp_offset] +
        m_PC.hxr_DP(2 - cpp_offset, m_dot_mc,
                    true); // through CO2 side of precooler (starting from known
                           // outlet pressure)
    m_pres_od_last[8 - cpp_offset] =
        m_pres_od_last[9 - cpp_offset] +
        m_LT.hxr_DP(2 - cpp_offset, m_dot_t_guess,
                    true); // through hot-side of low-temp recuperator (starting
                           // from known outlet pressure)
    m_pres_od_last[7 - cpp_offset] =
        m_pres_od_last[8 - cpp_offset] +
        m_HT.hxr_DP(2 - cpp_offset, m_dot_t_guess,
                    true); // through hot-side of high-temp recuperator
                           // (starting from known outlet pressure)

    // Calculate the allowable mass flow rate through the turbine
    m_dot_t = std::numeric_limits<double>::quiet_NaN();
    m_t.solve_turbine(m_temp_od_last[6 - cpp_offset],
                      m_pres_od_last[6 - cpp_offset],
                      m_pres_od_last[7 - cpp_offset], N_t_local, sub_error_code,
                      m_temp_od_last[7 - cpp_offset], m_dot_t);
    if (sub_error_code > 0) {
    ...
    double m_dot_residual =
        m_dot_t_guess - m_dot_t; // residual between guessed total mass flow
                                 // rate and turbine mass flow rates
    double secant_guess =
        m_dot_t_guess -
        m_dot_residual * (last_m_dot_guess - m_dot_t_guess) /
            (last_m_dot_residual -
             m_dot_residual); // next guess predicted using secant method
    if (m_dot_residual > 0.0) {
      if (m_dot_residual / m_dot_t <
          m_cycle_off_des_in.m_S
              .m_tol) // residual is positive; check for convergence
        break;
      m_dot_upper_bound = m_dot_t_guess; // reset lower bound
    } else {
      if (-m_dot_residual / m_dot_t <
          m_cycle_off_des_in.m_S
              .m_tol) // residual is negative; check for convergence
        break;
    ...
    if (m_dot_upper_bound - m_dot_lower_bound <
        1.E-12) // interval is getting too tight
      break;

    // Check if the secant method overshoots and fall back to bisection if it
    // does.
    if (secant_guess < m_dot_lower_bound ||
        secant_guess >
            m_dot_upper_bound) // secant method overshot, use bisection
  ...
  prop_error_code = CO2_TP(m_temp_od_last[1 - cpp_offset],
                           m_pres_od_last[1 - cpp_offset], &co2_props);
  if (prop_error_code != 0) {
  ...
  prop_error_code = CO2_TP(m_temp_od_last[2 - cpp_offset],
                           m_pres_od_last[2 - cpp_offset], &co2_props);
  if (prop_error_code != 0) {
  ...
  prop_error_code = CO2_TP(m_temp_od_last[6 - cpp_offset],
                           m_pres_od_last[6 - cpp_offset], &co2_props);
  if (prop_error_code != 0) {
  ...
  prop_error_code = CO2_TP(m_temp_od_last[7 - cpp_offset],
                           m_pres_od_last[7 - cpp_offset], &co2_props);
  if (prop_error_code != 0) {
    m_errors.SetError(44);
    return false;
  }
  ...
  // Outer iteration loop: temp(8) checking against UA_HT
  double T8_lower_bound, T8_upper_bound, UA_HT_calc, last_HT_residual,
      last_T8_guess;
  T8_lower_bound = T8_upper_bound = UA_HT_calc = last_HT_residual =
      last_T8_guess = std::numeric_limits<double>::quiet_NaN();
  if (UA_HT < 1.E-12) {
    ...
    T8_lower_bound =
        m_temp_od_last[2 - cpp_offset]; // the absolute lowest temp(8) could be
    T8_upper_bound =
        m_temp_od_last[7 -
                       cpp_offset]; // the absolutely highest temp(8) could be
    m_temp_od_last[8 - cpp_offset] = (T8_lower_bound + T8_upper_bound) *
                                     0.5; // bisect bounds for first guess
    UA_HT_calc = -1.0;
    last_HT_residual = UA_HT; // know a priori that with T8 = T7, UA_calc = 0
                              // therefore residual is UA_HT-0
  ...
  for (T8_iter = 1; T8_iter <= max_iter; T8_iter++) {
    ...
    prop_error_code =
        CO2_TP(m_temp_od_last[8 - cpp_offset], m_pres_od_last[8 - cpp_offset],
               &co2_props); // fully define state
    if (prop_error_code != 0) {
    ...
    double T9_lower_bound, T9_upper_bound, last_LT_residual, last_T9_guess;
    T9_lower_bound = T9_upper_bound = UA_LT_calc = last_LT_residual =
    ...
    if (UA_LT < 1.E-12) // no low-temp recuperator
    {
      ...
      T9_lower_bound =
          m_temp_od_last[2 -
                         cpp_offset]; // the absolute lowest temp(9) could be
      T9_upper_bound =
          m_temp_od_last[8 -
                         cpp_offset]; // the absolutely highest temp(9) could be
      m_temp_od_last[9 - cpp_offset] = (T9_lower_bound + T9_upper_bound) *
                                       0.5; // bisect bounds for first guess
      UA_LT_calc = -1.0;
      last_LT_residual = UA_LT; // know a priori that with T9 = T8, UA_calc = 0
                                // therefore residual is UA_LT-0
    ...
    for (T9_iter = 1; T9_iter <= max_iter; T9_iter++) {
      ...
      prop_error_code =
          CO2_TP(m_temp_od_last[9 - cpp_offset], m_pres_od_last[9 - cpp_offset],
                 &co2_props); // fully define state(9)
      if (prop_error_code != 0) {
      ...
      if (m_cycle_off_des_in.m_S.m_recomp_frac >= 1.0E-12) {
        ...
        m_rc.solve_compressor(m_temp_od_last[9 - cpp_offset],
                              m_pres_od_last[9 - cpp_offset], m_dot_rc,
                              last_N_rc_guess, sub_error_code,
                              m_temp_od_last[10 - cpp_offset], pres_10_calc);
        if (sub_error_code != 0) {
          m_errors.SetError(49);
          m_errors.SetError(sub_error_code);
          return false;
        }
        double last_pres_10_residual =
            m_pres_od_last[10 - cpp_offset] - pres_10_calc;
        ...
        for (N_rc_iter = 1; N_rc_iter <= max_iter; N_rc_iter++) {
          ...
          double pres_10_residual =
              m_pres_od_last[10 - cpp_offset] - pres_10_calc;

          if (abs(pres_10_residual) / m_pres_od_last[10 - cpp_offset] <=
              m_cycle_off_des_in.m_S.m_tol)
            break;

          double secant_guess =
              N_rc_guess - pres_10_residual * (last_N_rc_guess - N_rc_guess) /
                               (last_pres_10_residual -
                                pres_10_residual); // next guess predicted using
                                                   // secant method
        ...
        prop_error_code = CO2_TP(m_temp_od_last[10 - cpp_offset],
                                 m_pres_od_last[10 - cpp_offset], &co2_props);
        if (prop_error_code != 0) {
      ...
      {
        prop_error_code = CO2_TP(m_temp_od_last[9 - cpp_offset],
                                 m_pres_od_last[9 - cpp_offset], &co2_props);
        if (prop_error_code != 0) {
          m_errors.SetError(53);
          return false;
        }
        m_temp_od_last[10 - cpp_offset] = m_temp_od_last[9 - cpp_offset];
        m_enth_od_last[10 - cpp_offset] = m_enth_od_last[9 - cpp_offset] =
            co2_props.enth;
        m_entr_od_last[10 - cpp_offset] = m_entr_od_last[9 - cpp_offset] =
            co2_props.entr;
        m_dens_od_last[10 - cpp_offset] = m_dens_od_last[9 - cpp_offset] =
            co2_props.dens;
      }

      // Calculate the UA value of the low-temperature recuperator.
      if (UA_LT <
          1.E-12) // then no low-temp recuperator (this check is necessary to
                  // prevent pressure drops with UA=0 from causing problems)
        Q_dot_LT = 0.0;
      else
        Q_dot_LT = m_dot_t * (m_enth_od_last[8 - cpp_offset] -
                              m_enth_od_last[9 - cpp_offset]);

      int sub_error_code = 0;
      min_DT_LT = std::numeric_limits<double>::quiet_NaN();
      calculate_hxr_UA(
          m_cycle_off_des_in.m_S.m_N_sub_hxrs, Q_dot_LT, m_dot_mc, m_dot_t,
          m_temp_od_last[2 - cpp_offset], m_temp_od_last[8 - cpp_offset],
          m_pres_od_last[2 - cpp_offset], m_pres_od_last[3 - cpp_offset],
          m_pres_od_last[8 - cpp_offset], m_pres_od_last[9 - cpp_offset],
      ...
      double secant_guess =
          m_temp_od_last[9 - cpp_offset] -
          UA_LT_residual * (last_T9_guess - m_temp_od_last[9 - cpp_offset]) /
              (last_LT_residual -
               UA_LT_residual); // next guess predicted using secant method
      ...
      } else // UA_LT_calc is too small, temp(9) needs to be lower
      {
        ...
        if (min_DT_LT < temperature_tol)
          break; // exit T9_loop            !UA_calc is still too low but there
                 // isn't anywhere to go so it's ok(catches huge UA values)
        T9_upper_bound = m_temp_od_last[9 - cpp_offset];
      }
      last_LT_residual = UA_LT_residual; // reset last stored residual value
      last_T9_guess =
          m_temp_od_last[9 - cpp_offset]; // reset last stored guess value

      // Check if the secant method overshoots and fall back to bisection if it
      // does.
      if (secant_guess <= T9_lower_bound || secant_guess >= T9_upper_bound ||
          secant_guess != secant_guess) // then  ! secant method overshot (or is
                                        // NaN), use bisection
        m_temp_od_last[9 - cpp_offset] =
            (T9_lower_bound + T9_upper_bound) * 0.5;
      else
    ...
    // State 3 can now be fully defined
    m_enth_od_last[3 - cpp_offset] =
        m_enth_od_last[2 - cpp_offset] +
        Q_dot_LT /
            m_dot_mc; // Energy balance on cold stream of low-temp recuperator
    prop_error_code = CO2_PH(m_pres_od_last[3 - cpp_offset],
                             m_enth_od_last[3 - cpp_offset], &co2_props);
    if (prop_error_code != 0) {
    ...
    // Go through mixing valve.
    if (m_cycle_off_des_in.m_S.m_recomp_frac >= 1.0E-12) {
      m_enth_od_last[4 - cpp_offset] =
          (1.0 - m_cycle_off_des_in.m_S.m_recomp_frac) *
              m_enth_od_last[3 - cpp_offset] +
          m_cycle_off_des_in.m_S.m_recomp_frac *
              m_enth_od_last[10 - cpp_offset]; // conservation of energy (both
                                               // sides divided by m_dot_t)
      prop_error_code = CO2_PH(m_pres_od_last[4 - cpp_offset],
                               m_enth_od_last[4 - cpp_offset], &co2_props);
      if (prop_error_code != 0) {
    ...
    // Check for a second law violation at the outlet of the high-temp
    // recuperator.
    if (m_temp_od_last[4 - cpp_offset] >=
        m_temp_od_last[8 - cpp_offset]) // temp(8) is not valid; it must be
                                        // higher than it is
    {
    ...
    if (UA_HT <
      ...
      Q_dot_HT = m_dot_t * (m_enth_od_last[7 - cpp_offset] -
                            m_enth_od_last[8 - cpp_offset]);

    int sub_error_code = 0;
    min_DT_HT = std::numeric_limits<double>::quiet_NaN();
    calculate_hxr_UA(
        m_cycle_off_des_in.m_S.m_N_sub_hxrs, Q_dot_HT, m_dot_t, m_dot_t,
        m_temp_od_last[4 - cpp_offset], m_temp_od_last[7 - cpp_offset],
        m_pres_od_last[4 - cpp_offset], m_pres_od_last[5 - cpp_offset],
        m_pres_od_last[7 - cpp_offset], m_pres_od_last[8 - cpp_offset],
        sub_error_code, UA_HT_calc, min_DT_HT);
    ...
    double secant_guess =
        m_temp_od_last[8 - cpp_offset] -
        UA_HT_residual * (last_T8_guess - m_temp_od_last[8 - cpp_offset]) /
            (last_HT_residual -
             UA_HT_residual); // next guess predicted using secant method

    if (UA_HT_residual <
        0.0) // UA_HT calc is too big, temp(8) needs to be higher
    {
    ...
    } else // UA_HT_calc is too small, temp(8) needs to be lower
    {
      ...
      if (min_DT_HT < temperature_tol)
        break; // exit T8_loop            !UA_calc is still too low but there
               // isn't anywhere to go so it's ok(catches huge UA values)
      T8_upper_bound = m_temp_od_last[8 - cpp_offset];
    }
    last_HT_residual = UA_HT_residual; // reset last stored residual value
    last_T8_guess =
        m_temp_od_last[8 - cpp_offset]; // reset last stored guess value

    // Check if the secant method overshoots and fall back to bisection if it
    // does.
    if (secant_guess <= T8_lower_bound ||
        secant_guess >=
            T8_upper_bound) // then  ! secant method overshot, use bisection
      m_temp_od_last[8 - cpp_offset] = (T8_lower_bound + T8_upper_bound) * 0.5;
    else
    ...
    // State 5 can now be fully defined
    m_enth_od_last[5 - cpp_offset] =
        m_enth_od_last[4 - cpp_offset] +
        Q_dot_HT /
            m_dot_t; // energy balance on cold stream of high-temp recuperator
    prop_error_code = CO2_PH(m_pres_od_last[5 - cpp_offset],
                             m_enth_od_last[5 - cpp_offset], &co2_props);
    if (prop_error_code != 0) {
  ...
  m_enth_od_last[5 - cpp_offset] =
      m_enth_od_last[4 - cpp_offset] +
      Q_dot_HT / m_dot_t; // Energy balance on cold stream of HT recup
  prop_error_code = CO2_PH(m_pres_od_last[5 - cpp_offset],
                           m_enth_od_last[5 - cpp_offset], &co2_props);
  if (prop_error_code != 0) {
    m_errors.SetError(60);
  ...
  LT_outputs.m_DP_calc[0] =
      m_pres_od_last[3 - cpp_offset] - m_pres_od_last[2 - cpp_offset];
  LT_outputs.m_DP_calc[1] =
      m_pres_od_last[9 - cpp_offset] - m_pres_od_last[8 - cpp_offset];
  ...
  double h_hot_ideal =
      co2_props
          .enth; // enthalpy of hot stream outlet if temp is cold stream inlet

  // assumes hot stream is minimum capacitance stream(should probably check
  // which stream is min cap.)
  LT_outputs.m_eff =
      Q_dot_LT / (m_dot_t * (m_enth_od_last[8 - cpp_offset] - h_hot_ideal));
  LT_outputs.m_min_DT = min_DT_LT;
  //	cycle%LT%C_dot_cold = m_dot_mc * (enth(3) - enth(2)) / (temp(3) -
  //temp(2)) 	cycle%LT%C_dot_hot = m_dot_t  * (enth(8) - enth(9)) / (temp(8) -
  //temp(9))
  m_LT.set_od_outputs(LT_outputs);

  HX_off_design_outputs HT_outputs;
  HT_outputs.m_UA_calc = UA_HT_calc;
  HT_outputs.m_DP_calc[0] =
      m_pres_od_last[5 - cpp_offset] - m_pres_od_last[4 - cpp_offset];
  HT_outputs.m_DP_calc[1] =
      m_pres_od_last[8 - cpp_offset] - m_pres_od_last[7 - cpp_offset];
  ...
  prop_error_code = CO2_TP(m_temp_od_last[4 - cpp_offset],
                           m_pres_od_last[8 - cpp_offset], &co2_props);
  if (prop_error_code != 0) {
  ...
  h_hot_ideal = co2_props.enth;
  HT_outputs.m_eff =
      Q_dot_HT / (m_dot_t * (m_enth_od_last[7 - cpp_offset] - h_hot_ideal));
  HT_outputs.m_min_DT = min_DT_HT;
  //	cycle%HT%C_dot_cold = m_dot_t * (enth(5) - enth(4)) / (temp(5) -
  //temp(4)) 	cycle%HT%C_dot_hot = m_dot_t  * (enth(7) - enth(8)) / (temp(7) -
  //temp(8))
  m_HT.set_od_outputs(HT_outputs);

  HX_off_design_outputs PHX_outputs;
  double Q_dot_PHX = m_dot_t * (m_enth_od_last[6 - cpp_offset] -
                                m_enth_od_last[5 - cpp_offset]);
  ...
  PC_outputs.m_Q_dot_calc = m_dot_mc * (m_enth_od_last[9 - cpp_offset] -
                                        m_enth_od_last[1 - cpp_offset]);

  m_W_dot_net_od_last =
      m_mc.get_w() * m_dot_mc + m_rc.get_w() * m_dot_rc + m_t.get_w() * m_dot_t;
...
double nlopt_callback_opt_des(const std::vector<double> &x,
...
double nlopt_callback_opt_off_des(const std::vector<double> &x,
                                  std::vector<double> &grad, void *data) {
  RecompCycle *frame = static_cast<RecompCycle *>(data);
  if (frame != NULL)
