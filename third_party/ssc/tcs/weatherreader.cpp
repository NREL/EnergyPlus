...
public:
  ...
  virtual int call(double time, double step, int ncall) {
    ...
    {
      // If the start time does not correspond to the first record in the
      // weather file, loop to the correct record
      int nread = 1;
      if (m_first) {
        nread = (int)time / step;
        m_first = false;
    ...
    if (trackmode < 0 || trackmode > 2) {
      message(TCS_ERROR, "invalid tracking mode specified %d [0..2]",
              trackmode);
      return -1;
    }
...
TCS_IMPLEMENT_TYPE(weatherreader, "Standard Weather File format reader",
                   "Aron Dobos", 1, weatherreader_variables, NULL, 0)
