...
void calculate_turbomachinery_outlet_1(double T_in /*K*/, double P_in /*kPa*/,
                                       double P_out /*kPa*/,
                                       double eta_isen /*-*/, bool is_comp,
                                       int &error_code,
  ...
  calculate_turbomachinery_outlet_1(
      T_in, P_in, P_out, eta_isen, is_comp, error_code, enth_in, entr_in,
      dens_in, temp_out, enth_out, entr_out, dens_out, spec_work);
}

void calculate_turbomachinery_outlet_1(
  ...
  eta) is_comp = .false.means the turbomachine is a turbine(w = w_s * eta) */
...
void isen_eta_from_poly_eta(double T_in /*K*/, double P_in /*kPa*/,
                            double P_out /*kPa*/, double poly_eta /*-*/,
                            bool is_comp, int &error_code, double &isen_eta) {
  /* 9.3.14: code written by John Dyreby, translated to C++ by Ty Neises
  ! Calculate the isentropic efficiency that corresponds to a given polytropic
  efficiency ! for the expansion or compression from T_in and P_in to P_out.
  !
  ! Inputs:
  ...
  !   is_comp -- if .true., model a compressor (w = w_s / eta); if .false.,
  model a turbine (w = w_s * eta)
  !
  ! Outputs:
  ...
  !
  ! Notes:
  !   1) Integration of small DP is approximated numerically by using 200
  stages. !   2) No error checking is performed on the inlet and outlet
  pressures; valid pressure ratios are assumed. */
  ...
  for (int i = 1; i <= N_stages; i++) {
    ...
    double w_s =
        stage_h_in - stage_h_s_out; // specific work if process is isentropic
  ...
  // Note: last stage outlet enthalpy is equivalent to turbomachinery outlet
  // enthalpy
  if (is_comp)
...
int calc_turbomachinery_eta_isen(double T_in /*K*/, double P_in /*kPa*/,
  ...
  prop_error_code =
      CO2_TP(T_out, P_out, &co2_props); // properties at the inlet conditions
  if (prop_error_code != 0) {
    return prop_error_code;
  }
...
int Ts_data_over_linear_dP_ds(double P_in /*kPa*/, double s_in /*kJ/kg-K*/,
                              double P_out /*kPa*/, double s_out /*kJ/kg-K*/,
                              std::vector<double> &T_data /*C*/,
                              std::vector<double> &s_data /*kJ/kg-K*/,
                              int N_points /*=30*/) {
  CO2_state co2_props;
...
int Ph_data_over_turbomachinery(double T_in /*K*/, double P_in /*kPa*/,
                                double T_out /*K*/, double P_out /*kPa*/,
                                std::vector<double> &P_data /*MPa*/,
                                std::vector<double> &h_data /*kJ/kg*/,
                                int N_points /*30*/) {
  CO2_state co2_props;
  ...
  for (int i = 1; i < N_points; i++) {
    ...
    calculate_turbomachinery_outlet_1(
        T_in, P_in, P_local, eta_isen, is_comp, err_code, h_in_local,
        s_in_local, rho_in_local, T_out_local, h_out_local, s_out_local,
        rho_out_local, spec_work_local);
...
int sco2_cycle_plot_data_TS(int cycle_config,
  ...
  // Get LTR HP data
  int err_code = Ts_data_over_linear_dP_ds(
      pres[C_sco2_cycle_core::MC_OUT], entr[C_sco2_cycle_core::MC_OUT],
  ...
  // Get HTR HP data
  err_code = Ts_data_over_linear_dP_ds(
      pres[C_sco2_cycle_core::MIXER_OUT], entr[C_sco2_cycle_core::MIXER_OUT],
  ...
  err_code = Ts_data_over_linear_dP_ds(
      pres[C_sco2_cycle_core::HTR_HP_OUT], entr[C_sco2_cycle_core::HTR_HP_OUT],
      pres[C_sco2_cycle_core::TURB_IN], entr[C_sco2_cycle_core::TURB_IN], T_PHX,
      s_PHX, 25);
  if (err_code != 0)
    return err_code;
  ...
  // Get HTR HP data
  err_code = Ts_data_over_linear_dP_ds(
      pres[C_sco2_cycle_core::TURB_OUT], entr[C_sco2_cycle_core::TURB_OUT],
  ...
  if (cycle_config != 2) // Recompression Cycle
  {
    ...
    err_code = Ts_data_over_linear_dP_ds(
        pres[C_sco2_cycle_core::LTR_LP_OUT],
        entr[C_sco2_cycle_core::LTR_LP_OUT], pres[C_sco2_cycle_core::MC_IN],
        entr[C_sco2_cycle_core::MC_IN], T_main_cooler, s_main_cooler, 25);
    if (err_code != 0)
      return err_code;
  ...
  } else // Partial Cooling Cycle
  {
    ...
    err_code = Ts_data_over_linear_dP_ds(
        pres[C_sco2_cycle_core::LTR_LP_OUT],
        entr[C_sco2_cycle_core::LTR_LP_OUT], pres[C_sco2_cycle_core::PC_IN],
        entr[C_sco2_cycle_core::PC_IN], T_pre_cooler, s_pre_cooler, 25);
    if (err_code != 0)
      return err_code;

    // Get main cooler data
    err_code = Ts_data_over_linear_dP_ds(
        pres[C_sco2_cycle_core::PC_OUT], entr[C_sco2_cycle_core::PC_OUT],
...
int sco2_cycle_plot_data_PH(
    int cycle_config, const std::vector<double> temp /*K*/,
    const std::vector<double> pres /*kPa*/, std::vector<double> &P_t /*MPa*/,
    std::vector<double> &h_t /*kJ/kg*/, std::vector<double> &P_mc /*MPa*/,
    std::vector<double> &h_mc /*kJ/kg*/, std::vector<double> &P_rc /*MPa*/,
    std::vector<double> &h_rc /*kJ/kg*/, std::vector<double> &P_pc /*MPa*/,
    std::vector<double> &h_pc /*kJ/kg*/) {
  int n_pres = pres.size();
  int n_temp = temp.size();

  int err_code = Ph_data_over_turbomachinery(
      temp[C_sco2_cycle_core::TURB_IN], pres[C_sco2_cycle_core::TURB_IN],
      temp[C_sco2_cycle_core::TURB_OUT], pres[C_sco2_cycle_core::TURB_OUT], P_t,
      h_t, 25);

  if (err_code != 0)
    return err_code;

  err_code = Ph_data_over_turbomachinery(
      temp[C_sco2_cycle_core::MC_IN], pres[C_sco2_cycle_core::MC_IN],
      temp[C_sco2_cycle_core::MC_OUT], pres[C_sco2_cycle_core::MC_OUT], P_mc,
      h_mc, 25);
  ...
  if (cycle_config != 2) // Recompression Cycle
  {
    ...
    // Recompressor
    err_code = Ph_data_over_turbomachinery(
        temp[C_sco2_cycle_core::LTR_LP_OUT],
        pres[C_sco2_cycle_core::LTR_LP_OUT], temp[C_sco2_cycle_core::RC_OUT],
        pres[C_sco2_cycle_core::RC_OUT], P_rc, h_rc, 25);
  ...
  } else // Partial Cooling Cycle
  {
    ...
    err_code = Ph_data_over_turbomachinery(
        ...
        temp[C_sco2_cycle_core::RC_OUT], pres[C_sco2_cycle_core::RC_OUT], P_rc,
        h_rc, 25);
    ...
    // Precompressor
    err_code = Ph_data_over_turbomachinery(
        temp[C_sco2_cycle_core::PC_IN], pres[C_sco2_cycle_core::PC_IN],
        temp[C_sco2_cycle_core::PC_OUT], pres[C_sco2_cycle_core::PC_OUT], P_pc,
        h_pc, 25);
...
int Ph_arrays_over_constT(double P_low /*MPa*/, double P_high /*MPa*/,
                          std::vector<double> T_consts /*C*/,
                          std::vector<std::vector<double>> &P_data /*MPa*/,
...
int Ts_arrays_over_constP(double T_cold /*C*/, double T_hot /*C*/,
                          std::vector<double> P_consts /*kPa*/,
                          std::vector<std::vector<double>> &T_data /*C*/,
  ...
  for (int i = 0; i < n_P; i++) {
    ...
    Ts_data_over_linear_dP_ds(P_consts[i], s_cold, P_consts[i], s_hot,
                              T_data[i], s_data[i], n_points);
  }
...
int Ts_dome(double T_cold /*C*/, std::vector<double> &T_data /*C*/,
...
int Ts_full_dome(double T_cold /*C*/, std::vector<double> &T_data /*C*/,
                 std::vector<double> &s_data /*kJ/kg-K*/,
                 std::vector<double> &P_data /*MPa*/,
                 std::vector<double> &h_data /*kJ/kg*/) {
  CO2_state t_co2_props;
...
int Ph_dome(double P_low /*MPa*/, std::vector<double> &P_data /*MPa*/,
            std::vector<double> &h_data) {
  CO2_info t_co2_info;
  get_CO2_info(&t_co2_info);
  ...
  int T_P_target_code = P_x0_solver.solve(
      T_crit - 10.0, T_crit - 20.0, P_low * 1.E3, T_P_target_solved,
  ...
  return Ts_full_dome(T_P_target_solved - 273.15, T_data, s_data, P_data,
                      h_data);
}

int C_MEQ_CO2_props_at_2phase_P::operator()(double T_co2 /*K*/,
                                            double *P_calc /*kPa*/) {
...
void C_HeatExchanger::hxr_pressure_drops(const std::vector<double> &m_dots,
                                         std::vector<double> &hxr_deltaP) {
  int N = (int)m_dots.size();
  hxr_deltaP.resize(N);
  for (int i = 0; i < N; i++)
    hxr_deltaP[i] = ms_des_par.m_DP_design[i] *
                    pow((m_dots[i] / ms_des_par.m_m_dot_design[i]), 1.75);
}

void C_HeatExchanger::hxr_conductance(const std::vector<double> &m_dots,
                                      double &hxr_UA) {
  double m_dot_ratio = 0.5 * (m_dots[0] / ms_des_par.m_m_dot_design[0] +
                              m_dots[1] / ms_des_par.m_m_dot_design[1]);
  hxr_UA = ms_des_par.m_UA_design * pow(m_dot_ratio, 0.8);
}

double C_turbine::calculate_cost(double T_in /*K*/, double P_in /*kPa*/,
                                 double m_dot /*kg/s*/, double T_out /*K*/,
                                 double P_out /*kPa*/, double W_dot /*kWe*/) {
  switch (m_cost_model) {
...
void C_turbine::turbine_sizing(const S_design_parameters &des_par_in,
                               int &error_code) {
  /* 9.4.14: code from John Dyreby, converted to C++ by Ty Neises
  ! Determine the turbine rotor diameter, effective nozzle area, and
  design-point shaft ! speed and store values in recomp_cycle%t.
  !
  ! Arguments:
  !   recomp_cycle -- a RecompCycle object that defines the simple/recompression
  cycle at the design point !   error_trace -- an ErrorTrace object
  !
  ! Notes:
  !   1) The value for recomp_cycle%t%N_design is required to be set.  If it is
  <= 0.0 then !      the value for recomp_cycle%mc%N_design is used (i.e., link
  the compressor and turbine !      shafts).  For this reason, turbine_sizing
  must be called after compressor_sizing if
  ...
  int prop_error_code =
      CO2_TD(ms_des_par.m_T_in, ms_des_par.m_D_in, &co2_props);
  if (prop_error_code != 0) {
    error_code = prop_error_code;
    return;
  }
  ...
  double w_i =
      ms_des_par.m_h_in - h_s_out; //[kJ/kg] Isentropic specific work of turbine
  double C_s = sqrt(2.0 * w_i * 1000.0);          //[m/s] Spouting velocity
  double U_tip = ms_des_solved.m_nu_design * C_s; //[m/s] Tip speed
  ms_des_solved.m_D_rotor =
      U_tip / (0.5 * ms_des_solved.m_N_design * 0.104719755); //[m]
  ms_des_solved.m_A_nozzle = (ms_des_par.m_m_dot / m_r_W_dot_scale) /
                             (C_s * ms_des_par.m_D_in); //[m^2]
  ...
  ms_des_solved.m_eta = (ms_des_par.m_h_in - ms_des_par.m_h_out) /
                        w_i; //[-] Isentropic efficiency

  ms_des_solved.m_W_dot =
      ms_des_par.m_m_dot * (ms_des_par.m_h_in - ms_des_par.m_h_out);

  ms_des_solved.m_cost =
      calculate_cost(ms_des_par.m_T_in, ms_des_par.m_P_in, ms_des_par.m_m_dot,
                     T_out, ms_des_par.m_P_out, ms_des_solved.m_W_dot);
}

void C_turbine::off_design_turbine(double T_in, double P_in, double P_out,
                                   double N, int &error_code,
                                   double &m_dot_cycle, double &T_out) {
  /* 9.4.14: code from John Dyreby, converted to C++ by Ty Neises
  ! Solve for the outlet state of 'turb' given its inlet conditions, outlet
  ...
  !
  ! Notes:
  !   1) This subroutine also sets the following values in 'turb': nu, eta,
  m_dot, w, w_tip_ratio */
  ...
  double U_tip =
      ms_des_solved.m_D_rotor * 0.5 * N * 0.104719755; //[m/s] tip speed
  ms_od_solved.m_nu = U_tip / C_s; //[-] ratio of tip speed to spouting velocity

  double eta_0 =
      (((1.0626 * ms_od_solved.m_nu - 3.0874) * ms_od_solved.m_nu + 1.3668) *
           ms_od_solved.m_nu +
       1.3567) *
          ms_od_solved.m_nu +
      0.179921180;
  ...
  ms_od_solved.m_eta =
      eta_0 * ms_des_solved.m_eta; //[-] Actual turbine efficiency

  // Calculate the outlet state and allowable mass flow rate
  double h_out =
      h_in - ms_od_solved.m_eta *
                 (h_in - h_s_out); //[kJ/kg] Enthalpy at turbine outlet
  prop_error_code = CO2_PH(P_out, h_out, &co2_props);
  if (prop_error_code != 0) {
    error_code = prop_error_code;
    return;
  ...
  double m_dot_basis = C_s * ms_des_solved.m_A_nozzle *
                       D_in; //[kg/s] Mass flow rate through turbine
  ms_od_solved.m_w_tip_ratio =
      U_tip / ssnd_in; //[-] Ratio of the tip speed to the local (turbine inlet)
                       //speed of sound
  ms_od_solved.m_N = N;
  m_dot_cycle = m_dot_basis * m_r_W_dot_scale; //[kg/s]
  ms_od_solved.m_W_dot_out =
      m_dot_cycle * (h_in - h_out);   //[kW] Turbine power output
...
void C_turbine::od_turbine_at_N_des(double T_in, double P_in, double P_out,
                                    int &error_code, double &m_dot_cycle,
                                    double &T_out) {
...
int C_comp__psi_eta_vs_phi::design_given_shaft_speed(
  ...
  double D_rotor =
      std::pow(m_dot / (phi_design * rho_in * 0.5 * N_rad_s), 1.0 / 3.0); //[m]
  ...
  ms_des_solved.m_psi_max_at_N_des = calc_psi_isen(
      ms_des_solved.m_phi_surge, 1.0, T_in,
      P_in); //[-] max ideal head coefficient at design shaft speed

  return 0;
}

int C_comp__psi_eta_vs_phi::design_given_performance(double T_in /*K*/,
                                                     double P_in /*kPa*/,
                                                     double m_dot /*kg/s*/,
                                                     double T_out /*K*/,
                                                     double P_out /*K*/) {
  ...
  double w_i = h_isen_out -
               h_in; //[kJ/kg] positive isentropic specific work of compressor
...
int C_comp__psi_eta_vs_phi::off_design_given_N(
    double T_in /*K*/, double P_in /*kPa*/, double m_dot /*kg/s*/,
    double N_rpm /*rpm*/, double &T_out /*K*/, double &P_out /*kPa*/) {
  CO2_state co2_props;
  ...
  double U_tip =
      ms_des_solved.m_D_rotor * 0.5 * ms_od_solved.m_N * 0.104719755; //[m/s]
  ...
  ms_od_solved.m_eta = std::max(
      eta_ND_od * ms_des_solved.m_eta_design,
  ...
  double dh_s = psi * pow(U_tip, 2.0) *
                0.001; //[kJ/kg] Ideal enthalpy rise in compressor, from
                       //definition of head coefficient
  double dh =
      dh_s / ms_od_solved.m_eta; //[kJ/kg] Actual enthalpy rise in compressor
...
int C_comp__psi_eta_vs_phi::calc_N_from_phi(double T_in /*K*/,
                                            double P_in /*kPa*/,
                                            double m_dot /*kg/s*/,
                                            double phi_in /*-*/,
                                            double &N_rpm /*rpm*/) {
  ...
  double rho_in = co2_props.dens; //[kg/m^3]
  double U_tip =
      m_dot / (phi_in * rho_in * std::pow(ms_des_solved.m_D_rotor, 2)); //[m/s]
  N_rpm = (U_tip * 2.0 / ms_des_solved.m_D_rotor) * 9.549296590;        //[rpm]

  return 0;
}

int C_comp__psi_eta_vs_phi::calc_m_dot__phi_des(double T_in /*K*/,
                                                double P_in /*kPa*/,
                                                double N_rpm /*rpm*/,
                                                double &m_dot /*kg/s*/) {
  CO2_state co2_props;
  ...
  m_dot = ms_des_solved.m_phi_des * U_tip * rho_in *
          pow(ms_des_solved.m_D_rotor, 2); //[kg/s]
...
C_comp__psi_eta_vs_phi::construct_derived_C_comp__psi_eta_vs_phi(
  ...
  if (comp_model_code == E_snl_radial_via_Dyreby) {
    return std::unique_ptr<C_comp__snl_radial_via_Dyreby>(
        new C_comp__snl_radial_via_Dyreby());
  } else {
    throw(C_csp_exception("C_comp__psi_eta_vs_phi::construct_derived_C_comp__"
                          "psi_eta_vs_phi unrecognized compressor model code"));
  }
}

void C_comp__snl_radial_via_Dyreby::set_design_solution(
    double phi /*-*/, double T_comp_in_des /*K*/, double P_comp_in /*kPa*/) {}

void C_comp__snl_radial_via_Dyreby::report_phi_psi_eta_vectors(
    std::vector<double> &phi, std::vector<double> &psi,
    std::vector<double> &eta, double &eta_norm_design) {
...
double
C_comp__snl_radial_via_Dyreby::adjust_phi_for_N(double phi /*-*/,
                                                double N_des_over_N_od /*-*/) {
  return phi * pow(1.0 / N_des_over_N_od, 0.2); //[-] modified flow coefficient
}

double C_comp__snl_radial_via_Dyreby::calc_phi_min(double T_comp_in /*K*/,
                                                   double P_comp_in /*kPa*/) {
  return m_phi_min;
}

double
C_comp__snl_radial_via_Dyreby::calc_phi_design(double T_comp_in /*K*/,
                                               double P_comp_in /*kPa*/) {
  return m_phi_design;
}

double C_comp__snl_radial_via_Dyreby::calc_phi_max(double T_comp_in /*K*/,
                                                   double P_comp_in /*kPa*/) {
  return m_phi_max;
}

double
C_comp__snl_radial_via_Dyreby::calc_psi_isen_design(double T_comp_in /*K*/,
                                                    double P_comp_in /*kPa*/) {
...
double C_comp__snl_radial_via_Dyreby::calc_psi_isen(
    double phi_in /*-*/, double N_des_over_N_od /*-*/, double T_comp_in /*K*/,
    double P_comp_in /*kPa*/) {
  ...
  if (phi >= m_phi_min) {
    psi = ((((-498626.0 * phi) + 53224.0) * phi - 2505.0) * phi + 54.6) * phi +
          0.04049; // from dimensionless modified head curve(at design - point,
                   // psi and modified psi are equal)
  } else {
    double psi_at_surge =
        ((((-498626.0 * m_phi_min) + 53224.0) * m_phi_min - 2505.0) *
             m_phi_min +
         54.6) *
            m_phi_min +
        0.04049; // from dimensionless modified head curve(at design - point,
...
double C_comp__snl_radial_via_Dyreby::calc_eta_OD_normalized(
    double phi_in /*-*/, double N_des_over_N_od /*-*/, double T_comp_in /*K*/,
    double P_comp_in /*kPa*/) {
  double phi = adjust_phi_for_N(phi_in, N_des_over_N_od);

  double eta_star =
      ((((-1.638e6 * phi) + 182725.0) * phi - 8089.0) * phi + 168.6) * phi -
      0.7069; // from dimensionless modified efficiency curve

  return eta_star * 1.47528 / pow(N_des_over_N_od, pow(20.0 * phi, 5.0));
}

void C_comp__compA__PT_map_template::set_design_solution(
    double phi /*-*/, double T_comp_in_des /*K*/, double P_comp_in /*kPa*/) {}

void C_comp__compA__PT_map_template::report_phi_psi_eta_vectors(
    std::vector<double> &phi, std::vector<double> &psi,
    std::vector<double> &eta, double &eta_norm_design) {
  std::vector<double> phi_temp = mc_data_at_PT.get_column_data(0);
  int n_pts = phi_temp.size() - 1; //[-] Don't want first value, which only
                                   //helps converge during surge conditions
  phi.resize(n_pts);
  psi.resize(n_pts);
  eta.resize(n_pts);
...
double C_comp__compA__PT_map_template::calc_phi_min(double T_comp_in /*K*/,
                                                    double P_comp_in /*kPa*/) {
  return m_phi_min;
}

double
C_comp__compA__PT_map_template::calc_phi_design(double T_comp_in /*K*/,
                                                double P_comp_in /*kPa*/) {
  return m_phi_design;
}

double C_comp__compA__PT_map_template::calc_phi_max(double T_comp_in /*K*/,
                                                    double P_comp_in /*kPa*/) {
  return m_phi_max;
}

double
C_comp__compA__PT_map_template::calc_psi_isen_design(double T_comp_in /*K*/,
                                                     double P_comp_in /*kPa*/) {
  double phi_design = calc_phi_design(T_comp_in, P_comp_in);
...
double C_comp__compA__PT_map_template::calc_psi_isen(
    double phi /*-*/, double N_des_over_N_od /*-*/, double T_comp_in /*K*/,
    double P_comp_in /*kPa*/) {
  return mc_data_at_PT.linear_1D_interp(0, 1, phi);
}

double C_comp__compA__PT_map_template::calc_eta_OD_normalized(
    double phi /*-*/, double N_des_over_N_od /*-*/, double T_comp_in /*K*/,
    double P_comp_in /*kPa*/) {
  return mc_data_at_PT.linear_1D_interp(0, 2, phi);
}

int C_comp_multi_stage::C_MEQ_eta_isen__h_out::operator()(
    double eta_isen /*-*/, double *h_comp_out /*kJ/kg*/) {
  C_MEQ_N_rpm__P_out c_stages(mpc_multi_stage, m_T_in, m_P_in, m_m_dot_basis,
                              eta_isen);
  ...
  int N_rpm_code = 0;
  try {
    N_rpm_code = c_solver.solve(N_rpm_guess_1, N_rpm_guess_2, m_P_out,
                                N_rpm_solved, tol_solved, iter_solved);
  } catch (C_csp_exception) {
    throw(C_csp_exception(
        "C_comp_multi_stage::C_MEQ_eta_isen__h_out threw an exception"));
  }

  if (N_rpm_code != C_monotonic_eq_solver::CONVERGED) {
    if (!(N_rpm_code > C_monotonic_eq_solver::CONVERGED &&
          fabs(tol_solved) < 0.01)) {
      throw(C_csp_exception("C_comp_multi_stage::C_MEQ_eta_isen__h_out failed "
                            "to converge within a reasonable tolerance"));
    }
  }

  int n_stages = mpc_multi_stage->mv_c_stages.size(); //[-]

  *h_comp_out = mpc_multi_stage->mv_c_stages[n_stages - 1]
                    ->ms_des_solved.m_h_out; //[kJ/kg]

  return 0;
}

int C_comp_multi_stage::C_MEQ_N_rpm__P_out::operator()(
    double N_rpm /*rpm*/, double *P_comp_out /*kPa*/) {
  int n_stages = mpc_multi_stage->mv_c_stages.size(); //[-]
  ...
  for (int i = 0; i < n_stages; i++) {
    ...
    mpc_multi_stage->mv_c_stages[i] =
        C_comp__psi_eta_vs_phi::construct_derived_C_comp__psi_eta_vs_phi(
            mpc_multi_stage->m_compressor_model);
    comp_err_code = mpc_multi_stage->mv_c_stages[i]->design_given_shaft_speed(
        T_in, P_in, m_m_dot_basis, N_rpm, m_eta_isen, P_out, T_out, tip_ratio);
...
double
C_comp_multi_stage::calculate_cost(double T_in /*K*/, double P_in /*kPa*/,
                                   double m_dot /*kg/s*/, double T_out /*K*/,
                                   double P_out /*kPa*/, double W_dot /*kWe*/) {
  switch (m_cost_model) {
...
int C_comp_multi_stage::design_given_outlet_state(
    int comp_model_code, double T_in /*K*/, double P_in /*kPa*/,
    double m_dot_cycle /*kg/s*/, double T_out /*K*/, double P_out /*K*/,
    double tol /*-*/) {
  ...
  mv_c_stages[0] =
      C_comp__psi_eta_vs_phi::construct_derived_C_comp__psi_eta_vs_phi(
          m_compressor_model);
  mv_c_stages[0]->design_given_performance(T_in, P_in, m_dot_basis, T_out,
                                           P_out);
  ...
  if (mv_c_stages[0]->ms_des_solved.m_tip_ratio > tip_speed_limit) {
    ...
    while (is_add_stages) {
      tip_speed_limit =
          0.9; //[-] If multi-stage, increase tip speed limit to 0.9
      ...
      // Now solve for the isentropic efficiency for each stage that results in
      // the total compressor design isentropic efficiency
      ...
      try {
        eta_isen_code =
            c_solver.solve(eta_isen_guess_1, eta_isen_guess_2, h_out,
                           eta_isen_solved, tol_solved, iter_solved);
      } catch (C_csp_exception) {
        throw(C_csp_exception("C_comp_multi_stage::design_given_outlet_state "
                              "threw an exception"));
      }
      ...
      if (eta_isen_code != C_monotonic_eq_solver::CONVERGED) {
        if (!(eta_isen_code > C_monotonic_eq_solver::CONVERGED &&
              fabs(tol_solved) < 0.01)) {
          throw(C_csp_exception(
              "C_comp_multi_stage::design_given_outlet_state failed to "
              "converge within a reasonable tolerance"));
        }
      }

      max_calc_tip_speed = 0.0;
      for (int i = 0; i < n_stages; i++) {
        max_calc_tip_speed = std::max(
            max_calc_tip_speed, mv_c_stages[i]->ms_des_solved.m_tip_ratio);
      }
  ...
  ms_des_solved.m_T_out =
      mv_c_stages[n_stages - 1]->ms_des_solved.m_T_out; //[K]
  ms_des_solved.m_P_out =
      mv_c_stages[n_stages - 1]->ms_des_solved.m_P_out; //[kPa]
  ms_des_solved.m_h_out =
      mv_c_stages[n_stages - 1]->ms_des_solved.m_h_out; //[kJ/kg]
  ms_des_solved.m_D_out =
      mv_c_stages[n_stages - 1]->ms_des_solved.m_D_out; //[kg/m^3]

  ms_des_solved.m_isen_spec_work = h_out_isen - h_in; //[kJ/kg]

  ms_des_solved.m_m_dot = m_dot_cycle; //[kg/s]
  ms_des_solved.m_W_dot =
      ms_des_solved.m_m_dot *
      (ms_des_solved.m_h_out - ms_des_solved.m_h_in); //[kWe]

  ms_des_solved.m_N_design =
      mv_c_stages[n_stages - 1]->ms_des_solved.m_N_design;               //[rpm]
  ...
  ms_des_solved.m_psi_max_at_N_des =
      mv_c_stages[0]
          ->ms_des_solved.m_psi_max_at_N_des; //[-] Max ideal head coefficient
                                              //at design shaft speed
  ...
  for (int i = 0; i < n_stages; i++) {
    ...
    ms_des_solved.mv_tip_speed_ratio[i] =
        mv_c_stages[i]->ms_des_solved.m_tip_ratio; //[-]
    ms_des_solved.mv_eta_stages[i] =
        mv_c_stages[i]->ms_des_solved.m_eta_design; //[-]
  }

  ms_des_solved.m_cost = calculate_cost(
      ms_des_solved.m_T_in, ms_des_solved.m_P_in, ms_des_solved.m_m_dot,
...
void C_comp_multi_stage::off_design_at_N_des(
...
int C_comp_multi_stage::calc_m_dot__N_des__phi_des_first_stage(
    double T_in /*K*/, double P_in /*kPa*/, double &m_dot_cycle /*kg/s*/) {
  double N_des = mv_c_stages[0]->ms_des_solved.m_N_design; //[rpm]

  double m_dot_basis = std::numeric_limits<double>::quiet_NaN();
  int stage_err_code =
      mv_c_stages[0]->calc_m_dot__phi_des(T_in, P_in, N_des, m_dot_basis);
...
void C_comp_multi_stage::off_design_given_N(
    double T_in /*K*/, double P_in /*kPa*/, double m_dot_in /*kg/s*/,
    double N_rpm /*rpm*/, int &error_code, double &T_out /*K*/,
    double &P_out /*kPa*/) {
  ...
  for (int i = 0; i < n_stages; i++) {
    ...
    error_code = mv_c_stages[i]->off_design_given_N(
        T_stage_in, P_stage_in, m_dot, N_rpm, T_stage_out, P_stage_out);
    if (error_code != 0) {
      return;
    }
  ...
  ms_od_solved.m_eta =
      (h_out_isen - h_in) / (h_out - h_in); //[-] Overall compressor efficiency

  ms_od_solved.m_phi_min = phi_min; //[-] Min (all stages) flow coefficient

  ms_od_solved.m_tip_ratio_max =
      tip_ratio_max; //[-] Max (all stages) tip ratio overall

  ms_od_solved.m_N = N_rpm;

  ms_od_solved.m_W_dot_in =
      m_dot_in * (h_out - h_in); //[kg/s] (scaled to cycle, not basis)
  ms_od_solved.m_surge_safety = surge_safety_min;

  for (int i = 0; i < n_stages; i++) {
    ms_od_solved.mv_tip_speed_ratio[i] =
        mv_c_stages[i]->ms_od_solved.m_w_tip_ratio;              //[-]
...
int C_comp_multi_stage::C_MEQ_phi_od__P_out::operator()(
    double phi_od /*-*/, double *P_comp_out /*kPa*/) {
  double m_dot_basis =
      m_m_dot_cycle / mpc_multi_stage->m_r_W_dot_scale; //[kg/s]

  int error_code = 0;
  double N_rpm = std::numeric_limits<double>::quiet_NaN();
  error_code = mpc_multi_stage->mv_c_stages[0]->calc_N_from_phi(
      m_T_in, m_P_in, m_dot_basis, phi_od, N_rpm);
  if (error_code != 0) {
    *P_comp_out = std::numeric_limits<double>::quiet_NaN();
    return error_code;
  }
  ...
  double T_out = std::numeric_limits<double>::quiet_NaN();
  error_code = 0;
  mpc_multi_stage->off_design_given_N(m_T_in, m_P_in, m_m_dot_cycle, N_rpm,
                                      error_code, T_out, *P_comp_out);
...
void C_comp_multi_stage::off_design_given_P_out(
    double T_in /*K*/, double P_in /*kPa*/, double m_dot_cycle /*kg/s*/,
    double P_out /*kPa*/, double tol /*-*/, int &error_code,
    double &T_out /*K*/) {
  ...
  int test_code = c_rd_od_solver.test_member_function(
      phi_guess_lower, &P_solved_phi_guess_lower);
  if (test_code != 0) {
    for (int i = 1; i < 9; i++) {
      phi_guess_lower = ms_des_solved.m_phi_des * (10 - i) / 10.0 +
                        mv_c_stages[0]->ms_des_solved.m_phi_max * i / 10.0;
      test_code = c_rd_od_solver.test_member_function(
          phi_guess_lower, &P_solved_phi_guess_lower);
      if (test_code == 0)
        break;
    }
  ...
  double phi_guess_upper =
      phi_guess_lower * 0.5 + mv_c_stages[0]->ms_des_solved.m_phi_max * 0.5;
  double P_solved_phi_guess_upper = std::numeric_limits<double>::quiet_NaN();
  test_code = c_rd_od_solver.test_member_function(phi_guess_upper,
                                                  &P_solved_phi_guess_upper);
  if (test_code != 0) {
    for (int i = 6; i < 10; i++) {
      phi_guess_upper =
          phi_guess_lower * i / 10.0 +
          mv_c_stages[0]->ms_des_solved.m_phi_max * (10 - i) / 10.0;
      test_code = c_rd_od_solver.test_member_function(
          phi_guess_upper, &P_solved_phi_guess_upper);
      if (test_code == 0)
        break;
    }
    if (test_code != 0 && phi_guess_lower == ms_des_solved.m_phi_des) {
      for (int i = 6; i < 10; i++) {
        phi_guess_upper = phi_guess_lower * i / 10.0 +
                          ms_des_solved.m_phi_surge * (10 - i) / 10.0;
        // phi_guess_upper = phi_guess_lower * i / 10.0 + phi_lower*(10 - i)
        // / 10.0;
        test_code = c_rd_od_solver.test_member_function(
            phi_guess_upper, &P_solved_phi_guess_upper);
        if (test_code == 0)
  ...
  if (test_code != 0) {
    // Can't find a RC 2nd guess value (which, if we've found a first, means the
    // solution space is really small?)
    error_code = -20;
    return;
  }
  ...
  int phi_code = 0;
  try {
    phi_code = c_rd_od_solver.solve(phi_pair_lower, phi_pair_upper, P_out,
                                    phi_solved, tol_solved, iter_solved);
  } catch (C_csp_exception) {
    error_code = -1;
    return;
  }
  ...
  if (phi_code != C_monotonic_eq_solver::CONVERGED) {
    ...
    if (n_call_history > 0)
      error_code =
          -(*(c_rd_od_solver.get_solver_call_history()))[n_call_history - 1]
               .err_code;
