...
private:
  ...
  int tech_type;       // Flag indicating which coef. set to use.
                 // (1=tower,2=trough,3=user)
...
public:
  ...
  virtual int init() {
    ...
    */
    ...
    eta_ref = value(P_ETA_REF); // Reference conversion efficiency at design
                                // condition [none]
    T_htf_hot_ref =
        value(P_T_HTF_HOT_REF); // Reference HTF inlet temperature at design [C]
    T_htf_cold_ref = value(
        P_T_HTF_COLD_REF); // Reference HTF outlet temperature at design [C]
    dT_cw_ref = value(P_DT_CW_REF); // Reference condenser cooling water
                                    // inlet/outlet T diff [C]
    T_amb_des =
        value(P_T_AMB_DES); // Reference ambient temperature at design point [C]
    HTF =
        (int)value(P_HTF); // Integer flag identifying HTF in power block [none]
    q_sby_frac = value(P_Q_SBY_FRAC); // Fraction of thermal power required for
                                      // standby mode [none]
    ...
    startup_frac = value(P_STARTUP_FRAC); // Fraction of design thermal power
                                          // needed for startup [none]
    tech_type = (int)value(P_TECH_TYPE); // Flag indicating which coef. set to
                                         // use. (1=tower,2=trough,3=user) [none]
    ...
    n_pl_inc = (int)value(P_N_PL_INC); // Number of part-load increments for the
                                       // heat rejection system [none]
    F_wc = value(P_F_WC, &nval_F_wc); // Fraction indicating wet cooling use for
                                      // hybrid system [none]
    ...
    ---
    This type uses the power block class from SSC/recore/lib_powerblock.h. The
    code was already converted for the geothermal hourly model, so rather than
    reconvert it again, we just implement the existing class in the TCS type
    framework.
  ...
  virtual int call(double time, double step, int ncall) {
    ...
    mode = (int)value(
        I_MODE); // Cycle part load control, from plant controller [none]
    T_htf_hot =
    ...
    demand_var = value(
        I_DEMAND_VAR); // Control signal indicating operational mode [none]
    standby_control = (int)value(
        I_STANDBY_CONTROL); // Control signal indicating standby mode [none]
    ...
    */
    ...
    value(O_T_HTF_COLD,
          T_htf_cold); //[C] Heat transfer fluid outlet temperature
    value(O_M_DOT_MAKEUP,
          m_dot_makeup); //[kg/hr] Cooling water makeup flow rate
    value(O_M_DOT_DEMAND,
          m_dot_demand); //[kg/hr] HTF required flow rate to meet power load
    value(O_M_DOT_HTF_OUT, m_dot_htf_out); //[kg/hr] Actual HTF flow rate
                                           //passing through the power cycle
    value(O_M_DOT_HTF_REF,
          m_dot_htf_ref); //[kg/hr] Calculated reference HTF flow rate at design
    value(O_W_COOL_PAR, W_cool_par); //[MWe] Cooling system parasitic load
    value(O_P_REF_OUT, P_ref_out);   //[MWe] Reference power level output at
                                     //design (mirror param)
