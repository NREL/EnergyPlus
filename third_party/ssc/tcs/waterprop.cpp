...
///  common operations
static int VaporNonGap(int pIndex, double pFraction, property_info *data,
                       int edgeInd, double indVar);
static void LiquidNonGap(int pIndex, double pFraction, property_info *data,
                         int edgeInd, double indVar);
static void VaporGap(int firstCoefIndex, double TSat, double sFraction,
                     property_info *data);
static void LiquidGap(int lastCoefIndex, double TSat, double sFraction,
                      property_info *data);

/// common helper functions
static double cubic_solution(double coef_1, double coef_2, double coef_3,
                             double coef_4, double value);
...
static double vaporGrid(int k, int lastCoefIndex, double pFraction,
                        double sFraction = 1.);
static double liquidGrid(int j, int k, int firstCoefIndex, double pFraction);
static double liquidGrid(int k, int lastCoefIndex, double pFraction,
                         double sFraction = 1.);
static double propDome(int j, int k, int TSatIndex, double dT);
static double VHSDome(int i, int j, int TSatIndex, double dT);
static int maxloc(int firstIndex, int lastIndex, double indexVector[],
                  double S);
static int maxloc(std::vector<double> &valueVector, double S);

/// gap functions
/// Returns the temperature in the vapor gap given the entropy fractional
/// position
inline double vapor_gap_temp(double sFraction, double pFraction, double TSat,
                             int firstCoefIndex) {
...
inline double vapor_gap_dens(double sFraction, double pFraction, double dT,
                             int TSatIndex, int firstCoefIndex) {
...
inline double vapor_gap_enth(double sFraction, double pFraction, double dT,
                             int TSatIndex, int firstCoefIndex) {
...
inline double vapor_gap_cv(double sFraction, double pFraction, double dT,
                           int TSatIndex, int firstCoefIndex) {
...
inline double vapor_gap_cp(double sFraction, double pFraction, double dT,
                           int TSatIndex, int firstCoefIndex) {
...
inline double vapor_gap_ssnd(double sFraction, double pFraction, double dT,
                             int TSatIndex, int firstCoefIndex) {
...
inline double vapor_gap_cond(double sFraction, double pFraction, double dT,
                             int TSatIndex, int firstCoefIndex) {
...
inline double vapor_gap_visc(double sFraction, double pFraction, double dT,
                             int TSatIndex, int firstCoefIndex) {
...
inline double vapor_gap_entr(double sFraction, double dT, int TSatIndex,
                             int pIndex) {
  ...
  return (
      dome +
      (water_vapor_entr_values[water_vapor_entr_index_vector[pIndex]] - dome) *
          sFraction);
}
/// Returns temp in the liquid gap given the entropy fractional position
inline double liquid_gap_temp(double sFraction, double pFraction, double TSat,
                              int lastCoefIndex) {
...
inline double liquid_gap_dens(double sFraction, double pFraction, double dT,
                              int TSatIndex, int lastCoefIndex) {
...
inline double liquid_gap_enth(double sFraction, double pFraction, double dT,
                              int TSatIndex, int lastCoefIndex) {
...
inline double liquid_gap_cv(double sFraction, double pFraction, double dT,
                            int TSatIndex, int lastCoefIndex) {
...
inline double liquid_gap_cp(double sFraction, double pFraction, double dT,
                            int TSatIndex, int lastCoefIndex) {
...
inline double liquid_gap_ssnd(double sFraction, double pFraction, double dT,
                              int TSatIndex, int lastCoefIndex) {
...
inline double liquid_gap_cond(double sFraction, double pFraction, double dT,
                              int TSatIndex, int lastCoefIndex) {
...
inline double liquid_gap_visc(double sFraction, double pFraction, double dT,
                              int TSatIndex, int lastCoefIndex) {
...
inline double liquid_gap_entr(double sFraction, double dT, int TSatIndex,
                              int pIndex) {
  ...
  return (water_liquid_entr_values[lastIndex] +
          (dome - water_liquid_entr_values[lastIndex]) * sFraction);
}
...
/// 2D indexing
static inline int Index2D(int i, int j, int R) { return (i + R * j); }
/// 4D indexing
...
WPEXPORT int water_TQ(double T, double Q, property_info *data) {
  ...
  *************************************************************/
  ...
  double vhs_values[3];
  for (int i = 0; i < 3; i++) {
    vhs_values[i] = Q * (sat_vhs_values[i][1] - sat_vhs_values[i][0]) +
                    sat_vhs_values[i][0];
  }
  ...
  (*data).P = ((water_sat_pres_coef_array[ind[0]] * dT +
                water_sat_pres_coef_array[ind[1]]) *
                   dT +
               water_sat_pres_coef_array[ind[2]]) *
                  dT +
  ...
  *************************************************************/
  ...
  if (P <= water_max_sat_pres) {
    ...
    if (T > TSat) {
      ...
      /// --------------------- LIQUID ----------------------------------------
      int lastCoefIndex =
          water_liquid_entr_index_vector[pIndex + 1] - pIndex - 2;
  ...
  *************************************************************/
  ...
  if (P <= water_max_sat_pres) {
    ...
    if (H > HSat[1]) {
      ...
      /// check if temperature lies in gap between vapor dome and left-most
      /// element
      ...
      for (int i = 0; i < 3; i++) {
        ...
        vhs_values[i] =
            (*data).Q * (sat_vhs_values[i][1] - sat_vhs_values[i][0]) +
            sat_vhs_values[i][0];
      }
      ...
      /// check to see if properties are requested that are only valid if Q = 0
      /// or 1
  ...
  if (P <= water_max_sat_pres) {
    ...
    if (S > SSat[1]) {
      ...
      if (S < water_vapor_entr_values[firstIndex]) {
        /// in gap
        double sFraction =
            (S - SSat[1]) / (water_vapor_entr_values[firstIndex] - SSat[1]);
        int firstCoefIndex = water_vapor_entr_index_vector[pIndex] - pIndex;
        VaporGap(firstCoefIndex, TSat, sFraction, data);
      } else {
        /// fully vapor
        ...
        double sFraction = (S - water_vapor_entr_values[sIndex]) /
                           (water_vapor_entr_values[sIndex + 1] -
                            water_vapor_entr_values[sIndex]);
      ...
      if (S > water_liquid_entr_values[lastIndex]) {
        double sFraction = (S - water_liquid_entr_values[lastIndex]) /
                           (SSat[0] - water_liquid_entr_values[lastIndex]);
        int lastCoefIndex =
            water_liquid_entr_index_vector[pIndex + 1] - pIndex - 2;
        LiquidGap(lastCoefIndex, TSat, sFraction, data);
      } else {
        ...
        double sFraction = (S - water_liquid_entr_values[sIndex]) /
                           (water_liquid_entr_values[sIndex + 1] -
                            water_liquid_entr_values[sIndex]);
        int coefIndex =
            water_liquid_entr_index_vector[pIndex] - pIndex + rowInd;
...
int VaporNonGap(int pIndex, double pFraction, property_info *data, int edgeInd,
                double indVar) {
  int error_code = 0;
  ...
  int fortranOffset = -1;
  int firstCoefIndex =
      water_vapor_entr_index_vector[pIndex] - pIndex + 1 + fortranOffset;
  int lastCoefIndex =
      water_vapor_entr_index_vector[pIndex + 1] - pIndex - 1 + fortranOffset;

  std::vector<double> edge;
  edge.reserve(lastCoefIndex - firstCoefIndex + 1);

  for (int i = firstCoefIndex; i <= lastCoefIndex; i++)
    edge.push_back(vaporGrid(0, edgeInd, i, pFraction));
  ...
  double sFraction =
      cubic_solution(pAdjustedCoefs[0], pAdjustedCoefs[1], pAdjustedCoefs[2],
                     pAdjustedCoefs[3], indVar);
  if (fabs(sFraction) > 1.)
    return error_code = 9;
  ...
  (*data).S = water_vapor_entr_values[sIndex] +
              sFraction * (water_vapor_entr_values[sIndex + 1] -
                           water_vapor_entr_values[sIndex]);
...
void LiquidNonGap(int pIndex, double pFraction, property_info *data,
                  int edgeInd, double indVar) {
  /// fortranOffset
  int fortranOffset = -1;
  ...
  int firstCoefIndex =
      water_liquid_entr_index_vector[pIndex] - pIndex + 1 + fortranOffset;
  int lastCoefIndex =
      water_liquid_entr_index_vector[pIndex + 1] - pIndex - 1 + fortranOffset;

  std::vector<double> edge;
  edge.reserve(lastCoefIndex - firstCoefIndex + 1);

  for (int i = firstCoefIndex; i <= lastCoefIndex; i++)
    edge.push_back(liquidGrid(0, edgeInd, i, pFraction));
  int rowInd = maxloc(edge, indVar);
  int coefIndex = firstCoefIndex + rowInd;
  ...
  (*data).S = water_liquid_entr_values[sIndex] +
              sFraction * (water_liquid_entr_values[sIndex + 1] -
                           water_liquid_entr_values[sIndex]);
...
void VaporGap(int firstCoefIndex, double TSat, double sFraction,
              property_info *data) {
  ...
  (*data).dens =
      vapor_gap_dens(sFraction, pFraction, dT, TSatIndex, firstCoefIndex);
  (*data).V = 1. / (*data).dens;
  (*data).H =
      vapor_gap_enth(sFraction, pFraction, dT, TSatIndex, firstCoefIndex);
  (*data).S = vapor_gap_entr(sFraction, dT, TSatIndex, pIndex);
  (*data).U = (*data).H - (*data).P * (*data).V;
  (*data).Cv =
      vapor_gap_cv(sFraction, pFraction, dT, TSatIndex, firstCoefIndex);
  (*data).Cp =
      vapor_gap_cp(sFraction, pFraction, dT, TSatIndex, firstCoefIndex);
  (*data).ssnd =
      vapor_gap_ssnd(sFraction, pFraction, dT, TSatIndex, firstCoefIndex);
  (*data).cond =
      vapor_gap_cond(sFraction, pFraction, dT, TSatIndex, firstCoefIndex);
  (*data).visc =
      vapor_gap_visc(sFraction, pFraction, dT, TSatIndex, firstCoefIndex);
  (*data).Q = 10.;
}
/// Generic function for computing liquid gap properties
void LiquidGap(int lastCoefIndex, double TSat, double sFraction,
               property_info *data) {
  /// find temperature index and fractional position
  int TSatIndex;
  double dT;
  ...
  (*data).dens =
      liquid_gap_dens(sFraction, pFraction, dT, TSatIndex, lastCoefIndex);
  (*data).V = 1. / (*data).dens;
  (*data).H =
      liquid_gap_enth(sFraction, pFraction, dT, TSatIndex, lastCoefIndex);
  (*data).S = liquid_gap_entr(sFraction, dT, TSatIndex, pIndex);
  (*data).U = (*data).H - (*data).P * (*data).V;
  (*data).Cv =
      liquid_gap_cv(sFraction, pFraction, dT, TSatIndex, lastCoefIndex);
  (*data).Cp =
      liquid_gap_cp(sFraction, pFraction, dT, TSatIndex, lastCoefIndex);
  (*data).ssnd =
      liquid_gap_ssnd(sFraction, pFraction, dT, TSatIndex, lastCoefIndex);
  (*data).cond =
      liquid_gap_cond(sFraction, pFraction, dT, TSatIndex, lastCoefIndex);
  (*data).visc =
      liquid_gap_visc(sFraction, pFraction, dT, TSatIndex, lastCoefIndex);
  (*data).Q = -10.;
}

/// Calculate the root (that should lie between 0 and 1) of a cubic equation.
double cubic_solution(double coef_1, double coef_2, double coef_3,
                      double coef_4, double value) {
    ...
    if ((root_1 >= 0.0) && (root_1 <= 1.0)) {
      ...
      if ((root_2 >= 0.0) && (root_2 <= 1.0)) {
        ...
        if ((root_3 >= 0.0) && (root_3 <= 1.0)) {
          ...
          if ((froot_1 < froot_2) && (froot_1 < froot_3))
            root = root_1;
          else if ((froot_2 < froot_1) && (froot_2 < froot_3))
            root = root_2;
          else
            root = root_3;
...
double t_sat(double P) {
  /// No bounds checking is performed and the function will not adjust the
  /// highest index -
  ///	it assumes the pressure vector extends beyond the maximum pressure that
  ///will be given. 	Required Inputs: 	    pres = pressure (kPa) 	 Returns: 	    temp =
  ...
  double TSat = ((water_sat_temp_coef_array[ind[0]] * dP +
                  water_sat_temp_coef_array[ind[1]]) *
                     dP +
                 water_sat_temp_coef_array[ind[2]]) *
                    dP +
...
void sat_find(double T, int *TSatIndex, double *dT) {
  ...
  /// Note: will return 0 if the temperature is less than the lowest value, and
  /// if the temperature is exactly equal to the highest value then it will
  /// return one less than expected
  ///	Required Inputs:
  ///		T = saturation temperature
  ///	 Required Outputs:
  ///	    TSatIndex = the element containing the value, referencing the
  ///smaller index value (left side of element) 	    dT = the temperature difference
  ///between the given temperature and the smaller element bound */
  ...
  if (T >= water_sat_temp_vector[index + 1])
    index = index + 1;
  if (index == 126 && T == water_sat_temp_vector[126])
    index = 125;
...
void pres_find(double P, int *PSatIndex, double *pFraction) {
  ...
  if (P >= water_pres_vector[index + 1])
    index = index + 1;
  if (index == 126 && P == water_pres_vector[126])
    index = 125;

  /// Calculate the fractional position within the element.
  *pFraction = (P - water_pres_vector[index]) /
               (water_pres_vector[index + 1] - water_pres_vector[index]);
  *PSatIndex = index;
}
...
double vaporGrid(int j, int k, int firstCoefIndex, double pFraction) {
  ...
  double grid = ((water_vapor_coef_array[ind[0]] * pFraction +
                  water_vapor_coef_array[ind[1]]) *
                     pFraction +
                 water_vapor_coef_array[ind[2]]) *
                    pFraction +
...
double vaporGrid(int k, int lastCoefIndex, double pFraction, double sFraction) {
  ...
  double grid = (((((water_vapor_coef_array[ind[0]] * pFraction +
                  ...
                  ((water_vapor_coef_array[ind[4]] * pFraction +
                    water_vapor_coef_array[ind[5]]) *
                       pFraction +
                   water_vapor_coef_array[ind[6]]) *
                      pFraction +
                  water_vapor_coef_array[ind[7]]) *
                     sFraction +
                 ((water_vapor_coef_array[ind[8]] * pFraction +
                   water_vapor_coef_array[ind[9]]) *
                      pFraction +
                  water_vapor_coef_array[ind[10]]) *
                     pFraction +
                 water_vapor_coef_array[ind[11]]) *
                    sFraction +
                ((water_vapor_coef_array[ind[12]] * pFraction +
                  water_vapor_coef_array[ind[13]]) *
                     pFraction +
                 water_vapor_coef_array[ind[14]]) *
                    pFraction +
...
double liquidGrid(int j, int k, int firstCoefIndex, double pFraction) {
  ...
  double grid = ((water_liquid_coef_array[ind1] * pFraction +
                  water_liquid_coef_array[ind2]) *
                     pFraction +
                 water_liquid_coef_array[ind3]) *
                    pFraction +
                water_liquid_coef_array[ind4];
  return grid;
}
/// One parameter model of liquidGrid (overloaded). sFraction option input
double liquidGrid(int k, int lastCoefIndex, double pFraction,
                  double sFraction) {
  ...
  double grid = (((((water_liquid_coef_array[ind[0]] * pFraction +
                     water_liquid_coef_array[ind[1]]) *
                        pFraction +
                    water_liquid_coef_array[ind[2]]) *
                       pFraction +
                   water_liquid_coef_array[ind[3]] * sFraction) +
                  ((water_liquid_coef_array[ind[4]] * pFraction +
                    water_liquid_coef_array[ind[5]]) *
                       pFraction +
                   water_liquid_coef_array[ind[6]]) *
                      pFraction +
                  water_liquid_coef_array[ind[7]]) *
                     sFraction +
                 ((water_liquid_coef_array[ind[8]] * pFraction +
                   water_liquid_coef_array[ind[9]]) *
                      pFraction +
                  water_liquid_coef_array[ind[10]]) *
                     pFraction +
                 water_liquid_coef_array[ind[11]]) *
                    sFraction +
                ((water_liquid_coef_array[ind[12]] * pFraction +
                  water_liquid_coef_array[ind[13]]) *
                     pFraction +
                 water_liquid_coef_array[ind[14]]) *
                    pFraction +
...
double propDome(int j, int k, int TSatIndex, double dT) {
  ...
  double dome = ((water_sat_prop_coef_array[ind[0]] * dT +
                  water_sat_prop_coef_array[ind[1]]) *
                     dT +
                 water_sat_prop_coef_array[ind[2]]) *
                    dT +
...
double VHSDome(int i, int j, int TSatIndex, double dT) {
  ...
  double dome = ((water_sat_vhs_coef_array[ind[0]] * dT +
                  water_sat_vhs_coef_array[ind[1]]) *
                     dT +
                 water_sat_vhs_coef_array[ind[2]]) *
                    dT +
