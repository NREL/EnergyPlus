...
TEST_F(voltage_dynamic_lib_battery_voltage_test,
       NickelMetalHydrideFromPaperTest) {
  CreateModel(1);

  // Figure 3 from A Generic Battery Model for the Dynamic Simulation of Hybrid
  // Electric Vehicles
  cap = std::unique_ptr<capacity_lithium_ion_t>(
      new capacity_lithium_ion_t(6.5, 100, 100, 0, 1));

  model = std::unique_ptr<voltage_t>(new voltage_dynamic_t(
      1, 1, 1.2, 1.4, 1.25, 1.2, 6.5, 1.3, 5.2, 0.2, 0.0046, 1));
  ...
  for (size_t i = 0; i < 3; i++) {
    ...
    EXPECT_NEAR(model->battery_voltage(), voltages[i], 0.05)
        << "NickelMetalHydrideFromPaperTest: " + std::to_string(i);
  }
}
...
TEST_F(voltage_dynamic_lib_battery_voltage_test, calculateMaxChargeHourly) {
  ...
  double max_current;
  double power =
      model->calculate_max_charge_w(cap->q0(), cap->qmax(), 0, &max_current);
  EXPECT_NEAR(power, -2989, 1);
  double max_current_calc =
      model->calculate_current_for_target_w(power, cap->q0(), cap->qmax(), 0);
...
TEST_F(voltage_dynamic_lib_battery_voltage_test, calculateMaxDischargeHourly) {
  ...
  double max_current;
  double power =
      model->calculate_max_discharge_w(cap->q0(), cap->qmax(), 0, &max_current);
...
TEST_F(voltage_dynamic_lib_battery_voltage_test,
       calculateMaxDischargeSubHourly) {
  double dt_hour = 0.5;
  CreateModel(dt_hour);
...
TEST_F(voltage_dynamic_lib_battery_voltage_test,
       calculateMaxDischargeSubMinute) {
  double dt_hour = 1. / 200;
  CreateModel(dt_hour);
...
TEST_F(voltage_table_lib_battery_voltage_test, calculateMaxChargeHourly1) {
  ...
  max_current_calc = model->calculate_current_for_target_w(power + 1, cap->q0(),
                                                           cap->qmax(), 0);
...
TEST_F(voltage_table_lib_battery_voltage_test, calculateMaxDischargeHourly) {
  ...
  double max_current_calc = model->calculate_current_for_target_w(
      power - 1, cap->q0(), cap->qmax(), 0);
...
TEST_F(voltage_table_lib_battery_voltage_test,
       calculateMaxDischargeHourly_table_2) {
  double dt_hour = 1;
  CreateModel_SSC_412(dt_hour);

  // start at half SOC
  double max_current;
  double power = model->calculate_max_discharge_w(cap->q0(), cap->qmax(), 293,
                                                  &max_current);
  EXPECT_NEAR(power, 3618.3, 1); // current ~4
  double max_current_calc =
      model->calculate_current_for_target_w(power, cap->q0(), cap->qmax(), 293);
  ...
  // start at empty SOC
  power = model->calculate_max_discharge_w(cap->q0(), cap->qmax(), 293,
                                           &max_current);
...
TEST_F(voltage_table_lib_battery_voltage_test,
       calculate_discharging_past_limits) {
  double dt_hour = 1;
  CreateModel_SSC_412(dt_hour);
  ...
  max_current_calc = model->calculate_current_for_target_w(1000.0, cap->q0(),
                                                           cap->qmax(), 293);
...
TEST_F(voltage_vanadium_lib_battery_voltage_test,
       calculateMaxDischargeSubHourly) {
  double dt_hour = 0.5;
  CreateModel(dt_hour);
