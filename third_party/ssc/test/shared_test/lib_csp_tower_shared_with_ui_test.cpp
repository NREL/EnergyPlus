...
//=======Testing Molten Salt Power Tower UI
//Equations=============================================
...
NAMESPACE_TEST(csp_common, TowerSharedWithUi, SolarPilotField) {
  ...
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_sf_heliostat_area"), 144.375,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_sf_total_land_area"), 1892.,
                   kErrorToleranceHi);
  // ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_sf_total_reflective_area"), 1269056.25,
  // kErrorToleranceHi);			//  This one is not being read in the UI
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_sf_tower_height"), 193.458,
                   kErrorToleranceHi);
...
NAMESPACE_TEST(csp_common, TowerSharedWithUi, Receiver) {
  ...
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_rec_htf_c_avg"), 1.5066,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_rec_max_flow_to_rec"), 1853.5,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_rec_cav_ap_height"), 18.,
                   kErrorToleranceHi);
...
NAMESPACE_TEST(csp_common, TowerSharedWithUi, Tes) {
  ...
  ASSERT_NEAR_FRAC(GetNum(data, "csp_pt_tes_tank_diameter"), 38.8,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(data, "q_dot_tes_est"), 0.73, kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(data, "csp_pt_tes_htf_density"), 1808.48,
                   kErrorToleranceHi);
}
...
NAMESPACE_TEST(csp_common, TowerSharedWithUi, SystemControl) {
  ...
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_par_calc_aux"), 2.78783,
                   kErrorToleranceHi);
...
NAMESPACE_TEST(csp_common, TowerSharedWithUi, CapitalCosts) {
  ...
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_receiver_area"), 1197.86,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_storage_mwht"), 2791.26,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_power_block_mwe"), 115.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_site_improvements"), 20304872.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_heliostats"), 177667632.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_tower"), 25319156.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_receiver"), 85191944.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_storage"), 61407768.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_power_block"), 119600000.,
                   kErrorToleranceHi);
  ...
  ASSERT_NEAR_FRAC(GetNum(vd, "ui_direct_subtotal"), 522841376.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_contingency"), 36598896.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "total_direct_cost"), 559440256.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_epc_total"), 72727232.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_plm_total"), 18920378.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_sales_tax_total"), 22377610.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "total_indirect_cost"), 114025224.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "total_installed_cost"), 673465472.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "csp_pt_cost_installed_per_capacity"), 6506.91,
                   kErrorToleranceHi);
}
...
NAMESPACE_TEST(csp_common, TowerSharedWithUi, FinancialCase1) {
  ...
  ASSERT_NEAR_FRAC(GetNum(vd, "const_per_principal1"), 673465472.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "const_per_interest1"), 26938618.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "const_per_total1"), 33673272.,
                   kErrorToleranceHi);
  ...
  ASSERT_NEAR_FRAC(GetNum(vd, "const_per_percent_total"), 100.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "const_per_principal_total"), 673465472.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "const_per_interest_total"), 26938618.,
                   kErrorToleranceHi);
  ASSERT_NEAR_FRAC(GetNum(vd, "construction_financing_cost"), 33673272.,
                   kErrorToleranceHi);
}
...
//	ASSERT_THROW(Tower_SolarPilot_Solar_Field_Equations(nullptr),
//std::runtime_error);
//	//ASSERT_THROW(MSPT_Receiver_Equations(nullptr), std::runtime_error);
//	//ASSERT_THROW(MSPT_System_Control_Equations(nullptr),
//std::runtime_error);
//	//ASSERT_THROW(Tower_SolarPilot_Capital_Costs_MSPT_Equations(nullptr),
//std::runtime_error);
//	//ASSERT_THROW(Tower_SolarPilot_Capital_Costs_DSPT_Equations(nullptr),
//std::runtime_error);
//	//ASSERT_THROW(Tower_SolarPilot_Capital_Costs_ISCC_Equations(nullptr),
//std::runtime_error);
//}
...
//	ASSERT_THROW(Tower_SolarPilot_Solar_Field_Equations(vd),
//std::runtime_error);
//	//ASSERT_THROW(MSPT_Receiver_Equations(vd), std::runtime_error);
//	//ASSERT_THROW(MSPT_System_Control_Equations(vd), std::runtime_error);
//	//ASSERT_THROW(Tower_SolarPilot_Capital_Costs_MSPT_Equations(vd),
//std::runtime_error);
//	//ASSERT_THROW(Tower_SolarPilot_Capital_Costs_DSPT_Equations(vd),
//std::runtime_error);
//	//ASSERT_THROW(Tower_SolarPilot_Capital_Costs_ISCC_Equations(vd),
//std::runtime_error);
//}
