...
TEST_F(lib_battery_thermal_test, updateTemperatureTest) {
  CreateModel(Cp);
  // battery which adjusts quickly to temp {16.85, 16.85, 21.85, 21.85, 16.85,
  // -3.15, -3.15};
  double I = 50;
  size_t idx = 0;
...
TEST_F(lib_battery_losses_test, MonthlyLossesTest) {
  model = std::unique_ptr<losses_t>(
      new losses_t(chargingLosses, dischargingLosses, chargingLosses));
...
TEST_F(lib_battery_losses_test, TimeSeriesLossesTest) {
  ...
  int charge_mode = -1; // not used - still need to test charge/discharge/idle
                        // loss projections since those will be used in dispatch
  double dt_hr = 1;
...
TEST_F(lib_battery_test, SetUpTest) { ASSERT_TRUE(1); }
...
TEST_F(lib_battery_test, runTestCycleAt1C) {
  ...
  //    std::cerr <<  idx << ": soc " << batteryModel->SOC() << ", cap " <<
  //    capacity_passed << "\n";
  // the SOC isn't at 5 so it means the controller is not able to calculate a
  // current/voltage at which to discharge to 5
...
TEST_F(lib_battery_test, RoundtripEffModel) {
  ...
  while (current < fabs(max_current)) {
    ...
    while (capacityModel->SOC() < 100) {
      ...
      voltageModel->updateVoltage(capacityModel->q0(), capacityModel->qmax(),
                                  capacityModel->I(), 0, 1);
    ...
    while (voltageModel->calculate_max_discharge_w(
               capacityModel->q0(), capacityModel->qmax(), 0, nullptr) > 0) {
  ...
  std::vector<double> eff_expected = {
      0.99, 0.99, 0.98, 0.98, 0.97, 0.97, 0.96, 0.96,
      0.95, 0.95, 0.94, 0.95, 0.93, // i = 12
      0.93, 0.93, 0.93, 0.95, 0.92, 0.90, 0.93, 0.88,
      0.92, 0.90, 0.91, 0.90, 0.89, // i = 25
      0.89, 0.90, 0.90, 0.84, 0.87, 0.88, 0.89, 0.89,
      0.81, 0.85, 0.85, 0.86, 0.88, // i = 38
      0.88, 0.87, 0.78, 0.81, 0.81, 0.83, 0.84, 0.85,
      0.86, 0.87, 0.87, 0.85, 0.79, // i = 51
      0.75, 0.75, 0.76, 0.77, 0.78, 0.79, 0.80, 0.81,
      0.82, 0.83, 0.84, 0.85, 0.85, // i = 64
      0.84, 0.84, 0.82, 0.76, 0.66, 0.66, 0.66, 0.67,
      0.68, 0.69, 0.70, 0.70, 0.71, // i = 77
      0.72, 0.73, 0.74, 0.74, 0.75, 0.76, 0.77, 0.77,
      0.78, 0.78, 0.79, 0.80, 0.81,                   // i = 90
...
TEST_F(lib_battery_test, RoundtripEffTable) {
  std::vector<double> vals = {0, Vfull, 1.78, Vexp, 88.9, Vnom, 99, 0};
  util::matrix_t<double> table(4, 2, &vals);

  auto capacityModel =
      new capacity_lithium_ion_t(q, SOC_init, SOC_max, SOC_min, dtHour);
  auto voltageModel = new voltage_table_t(n_series, n_strings, Vnom_default,
                                          table, resistance, 1);
  capacityModel->change_SOC_limits(0, 100);

  double full_current = 1000;
  double max_current;
  voltageModel->calculate_max_charge_w(capacityModel->q0(),
                                       capacityModel->qmax(), 0, &max_current);
  ...
  std::vector<double> eff_expected = {
      0.99, 0.99, 0.98, 0.98, 0.97, 0.96, 0.96, 0.95, 0.95, 0.94, 0.93,
      0.94, 0.93, 0.92, 0.92, 0.92, 0.91, 0.90, 0.90, 0.89, 0.88, 0.87,
      0.88, 0.87, 0.86, 0.87, 0.86, 0.84, 0.86, 0.87, 0.85, 0.83, 0.81,
      0.84, 0.85, 0.86, 0.84, 0.82, 0.79, 0.78, 0.80, 0.82, 0.84, 0.85,
      0.85, 0.82, 0.80, 0.77, 0.74, 0.73, 0.74, 0.76, 0.77, 0.78, 0.79,
      0.81, 0.82, 0.83, 0.84, 0.83, 0.80, 0.77, 0.74, 0.71, 0.67, 0.64,
      0.65, 0.65, 0.66, 0.67, 0.68, 0.69, 0.70, 0.71, 0.71, 0.72, 0.73,
      0.74, 0.75, 0.75, 0.76, 0.77, 0.78, 0.79, 0.79, 0.80, 0.81, 0.82,
      0.82, 0.79, 0.76, 0.73, 0.69, 0.66, 0.62, 0.59, 0.55, 0.51, 0.47};
  for (size_t i = 0; i < eff_expected.size(); i++)
...
TEST_F(lib_battery_test, RoundtripEffVanadiumFlow) {
  ...
  while (current < fabs(max_current)) {
    ...
    current *= -1;
    double output_power = 0.;
    while (vol->calculate_max_discharge_w(cap->q0(), cap->qmax(), 300,
                                          nullptr) > 0) {
      double output_current = current;
...
TEST_F(lib_battery_test, HourlyVsSubHourly) {
  auto cap_hourly =
      new capacity_lithium_ion_t(q, SOC_init, SOC_max, SOC_min, dtHour);
  auto volt_hourly =
      new voltage_dynamic_t(n_series, n_strings, Vnom_default, Vfull, Vexp,
                            Vnom, Qfull, Qexp, Qnom, C_rate, resistance, 1);

  auto cap_subhourly =
      new capacity_lithium_ion_t(q, SOC_init, SOC_max, SOC_min, dtHour);
  auto volt_subhourly =
      new voltage_dynamic_t(n_series, n_strings, Vnom_default, Vfull, Vexp,
                            Vnom, Qfull, Qexp, Qnom, C_rate, resistance, .5);
  ...
  while (cap_hourly->SOC() > 16) {
    double I_hourly = volt_hourly->calculate_current_for_target_w(
        discharge_watts, cap_hourly->q0(), cap_hourly->qmax(), 0);
    cap_hourly->updateCapacity(I_hourly, 1);
    volt_hourly->updateVoltage(cap_hourly->q0(), cap_hourly->qmax(),
                               cap_hourly->I(), 0, 1);
    EXPECT_NEAR(cap_hourly->I() * volt_hourly->battery_voltage(),
                discharge_watts, 0.1);

    double I_subhourly = volt_subhourly->calculate_current_for_target_w(
        discharge_watts, cap_subhourly->q0(), cap_subhourly->qmax(), 0);
    cap_subhourly->updateCapacity(I_subhourly, 0.5);
    volt_subhourly->updateVoltage(cap_subhourly->q0(), cap_subhourly->qmax(),
                                  cap_subhourly->I(), 0, 0.5);
    EXPECT_NEAR(cap_subhourly->I() * volt_subhourly->battery_voltage(),
                discharge_watts, 0.1);
  }
}
  ...
  while (count < 2000) {
    ...
    while (batteryModel->SOC() > 15) {
      ...
      EXPECT_NEAR(batteryModel->get_state().lifetime->day_age_of_battery,
                  batt_subhourly->get_state().lifetime->day_age_of_battery,
                  1e-3);
      EXPECT_NEAR(batteryModel->get_state().lifetime->day_age_of_battery,
                  batt_adaptive->get_state().lifetime->day_age_of_battery,
                  1e-3);
    }
    ...
    // max energy throughput error is about 10% from hourly runs and 15% from 15
    // min runs
    EXPECT_NEAR(hourly_E, adaptive_E, hourly_E * 0.10) << "At count " << count;
    EXPECT_NEAR(subhourly_E, adaptive_E, subhourly_E * 0.15)
        << "At count " << count;

    // max lifetime degradation error is about 15, out of charge max of ~600 ->
    // 20 / 600 = 3.3 % error
    EXPECT_NEAR(batteryModel->charge_maximum(), batt_adaptive->charge_maximum(),
                20)
        << "At count " << count;
    EXPECT_NEAR(batt_subhourly->charge_maximum(),
                batt_adaptive->charge_maximum(), 20)
        << "At count " << count;
  }
...
TEST_F(lib_battery_test, AugmentCapacity) {
  ...
  // Correct future approach for augmenting batteries, by treating as separate
  // entities
...
TEST_F(lib_battery_test, NMCLifeModel) {
  ...
  auto thermalModelNMC =
      new thermal_t(dtHour, mass, surface_area, resistance, Cp, h, T_room);
  auto capacityModelNMC =
      new capacity_lithium_ion_t(q, SOC_init, SOC_max, SOC_min, dtHour);
  auto voltageModelNMC = new voltage_dynamic_t(
      n_series, n_strings, Vnom_default, Vfull, Vexp, Vnom, Qfull, Qexp, Qnom,
      C_rate, resistance, dtHour);
  auto lossModelNMC = new losses_t(monthlyLosses, monthlyLosses, monthlyLosses);

  auto batteryNMC = std::unique_ptr<battery_t>(
      new battery_t(dtHour, chemistry, capacityModelNMC, voltageModelNMC,
                    lifetimeModelNMC, thermalModelNMC, lossModelNMC));
  double I = Qfull * n_strings * 2;
...
TEST_F(lib_battery_test, AdaptiveTimestepNMC) {
  ...
  batteryModel = std::unique_ptr<battery_t>(
      new battery_t(dtHour, chemistry, capacityModelNMC, voltageModelNMC,
                    lifetimeModelNMC, thermalModelNMC, lossModelNMC));
  ...
  while (count < 2000) {
    ...
    ASSERT_NEAR(subhourly_E, adaptive_E, subhourly_E * 0.20)
        << "At count " << count;

    // max lifetime degradation error is about 15, out of charge max of ~600 ->
    // 20 / 600 = 3.3 % error
    ASSERT_NEAR(batteryModel->charge_maximum(), batt_adaptive->charge_maximum(),
                20)
        << "At count " << count;
    ASSERT_NEAR(batt_subhourly->charge_maximum(),
                batt_adaptive->charge_maximum(), 20)
        << "At count " << count;
  }
...
TEST_F(lib_battery_test, AdaptiveTimestepNonIntegerStep) {
  ...
  auto batt_adaptive = std::unique_ptr<battery_t>(
      new battery_t(dtHour, chemistry, capacityModelNMC, voltageModelNMC,
                    lifetimeModelNMC, thermalModelNMC, lossModelNMC));
