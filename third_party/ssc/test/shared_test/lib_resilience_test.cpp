              ...
              for (auto Qnom : {0.8, 0.9}) {
                ...
                for (auto C_rate : {0.05, 0.1, 0.2}) {
                  for (auto resistance : {0.05, 0.1, 0.2}) {
                    char buf[300];
                    sprintf(buf,
                            "dtHour, %f, Vfull, %f, Vexp, %f, Vnom, %f, Qfull, "
                            "%f, Qexp, %f, Qnom, %f, C rate, %f, res, %f",
                            dtHour, Vfull, Vexp, Vnom, Qfull, Qexp, Qnom,
                            C_rate, resistance);
                    auto voltageModel = new voltage_dynamic_t(
                        n_series, n_strings, Vnom * 0.98, Vfull, Vexp, Vnom,
                        Qfull, Qexp, Qnom, C_rate, resistance, dtHour);
                    try {
                      double current1;
                      for (auto q_ratio : {0.25, 0.5, 0.75}) {
                        double q = n_strings * Qfull * q_ratio;
                        double qmax = n_strings * Qfull;
                        auto max_1 = voltageModel->calculate_max_discharge_w(
                            q, qmax, 0, &current1);
                        auto power1 =
                            voltageModel->calculate_voltage_for_current(
                                current1, q - current1 * dtHour, qmax, 0) *
                            current1;
                        EXPECT_NEAR(max_1, power1, 1e-3)
                            << buf << ", q_ratio, " << q_ratio;
                      }
                      delete voltageModel;
...
TEST_F(ResilienceTest_lib_resilience, DischargeBatteryModelSubHourly) {
  ...
  double max_current;
  double max_power =
      batt->battery_model->calculate_max_discharge_kw(&max_current);
...
TEST_F(ResilienceTest_lib_resilience, PVWattsSetUp) {
  ...
  //    auto power_model =
  //    batt->dispatch_model->getBatteryPowerFlow()->getBatteryPower();
  ...
  while (count < 100.) {
    ...
    //               cap->I(), vol->V(),
    //               batt->battery_model->losses_model()->getLoss(count),
    //               power_model->powerBatteryDC);
...
TEST_F(ResilienceTest_lib_resilience, DischargeVoltageTable) {
  ...
  double max_p =
      volt.calculate_max_discharge_w(cap.q0(), cap.qmax(), 0, &req_cur);
  cap.updateCapacity(req_cur, 1);
  volt.updateVoltage(cap.q0(), cap.qmax(), cap.I(), 0, 1);
  ...
  cap.updateCapacity(req_cur, 1);
  volt.updateVoltage(cap.q0(), cap.qmax(), cap.I(), 0, 1);
  EXPECT_GT(max_p, cap.I() * volt.cell_voltage())
      << "resulting power should be less than max";

  double overmax_I =
      volt.calculate_current_for_target_w(max_p * 1.1, cap.q0(), cap.qmax(), 0);
  cap = capacity_lithium_ion_t(2.25, 50, 100, 0, 1);
...
TEST_F(ResilienceTest_lib_resilience, ChargeVoltageTable) {
  ...
  cap.updateCapacity(current, 1);
  double req_cur =
      volt.calculate_current_for_target_w(-1.5, cap.q0(), cap.qmax(), 0);
  cap.updateCapacity(req_cur, 1);
  volt.updateVoltage(cap.q0(), cap.qmax(), cap.I(), 0, 1);
  double v = volt.cell_voltage();
...
TEST_F(ResilienceTest_lib_resilience, VoltageVanadium) {
  ...
  req_cur =
      volt.calculate_current_for_target_w(5, cap.q0(), cap.qmax(), 306.25);
  cap.updateCapacity(req_cur, 1);
  volt.updateVoltage(cap.q0(), cap.qmax(), cap.I(), 33, 1);
  v = volt.cell_voltage();
  EXPECT_NEAR(req_cur * v, 5, 1e-2);
  ...
  double max_p =
      volt.calculate_max_charge_w(cap.q0(), cap.qmax(), 306.15, &req_cur);
  cap.updateCapacity(req_cur, 1);
  volt.updateVoltage(cap.q0(), cap.qmax(), cap.I(), 33, 1);
  EXPECT_NEAR(max_p, cap.I() * volt.cell_voltage(), 1e-3);

  max_p =
      volt.calculate_max_discharge_w(cap.q0(), cap.qmax(), 306.15, &req_cur);
  cap.updateCapacity(req_cur, 1);
  volt.updateVoltage(cap.q0(), cap.qmax(), cap.I(), 33, 1);
  EXPECT_NEAR(max_p, cap.I() * volt.cell_voltage(), 1e-3);
}
...
TEST_F(ResilienceTest_lib_resilience, PVWattsACHourly_Discharge) {
  ...
  std::vector<double> correct_charge_total = {13.86, 11.96, 10.05, 8.10, 6.10,
...
TEST_F(ResilienceTest_lib_resilience, PVWattsACHalfHourly_Discharge) {
  ...
  for (size_t i = 0; i < 10; i++) {
    for (size_t j = 0; j < 2; j++) {
      ...
      resilience.run_surviving_batteries(load[i], 0, 0, 0, 0, 0);
      batt->advance(vartab, ac[i], voltage, load[i]);
      EXPECT_NEAR(batt->outBatteryPower[i], 1., 1e-3)
          << "timestep " << i * 2 + j;
    }
    charge_total.emplace_back(batt->battery_model->charge_total());
  }
...
// for dc-connected battery, the dispatch array units are kwDC. This doesn't
// affect dispatch_resiliency which decides the dispatch based on the ac load,
// but it will affect batt::advance, which will be dispatching less power per
// time step than in the AC case where the dispatch kWAC was converted to kWDC
TEST_F(ResilienceTest_lib_resilience, PVWattsDCHourly_Discharge) {
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    charge_total.emplace_back(batt->battery_model->charge_total());
    if (i < 5)
      EXPECT_NEAR(batt->outBatteryPower[i],
                  1. * inverter->efficiencyAC / 100. *
                      batt_vars->batt_dc_dc_bms_efficiency / 100.,
                  1e-3)
          << "timestep " << i << " battery discharging";
    else if (i == 5)
      EXPECT_NEAR(batt->outBatteryPower[i], 0.855, 1e-3)
          << "timestep 5 battery SOC limits";
    else
      EXPECT_NEAR(batt->outBatteryPower[i], 0, 1e-3)
          << "timestep " << i << " battery at min SOC";
  }
  std::vector<double> correct_charge_total = {13.94, 12.12, 10.28, 8.42, 6.51,
                                              4.72,  4.72,  4.72,  4.72, 4.72};
...
TEST_F(ResilienceTest_lib_resilience, PVWattsACHourly_Charge) {
  ...
  std::vector<double> correct_charge_total = {16.61, 17.46, 18.32, 19.17,
                                              20.02};
