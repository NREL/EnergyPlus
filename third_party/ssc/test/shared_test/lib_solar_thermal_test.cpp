...
// Basic test of expected power gain and outlet temperature of a single flat
// plate collector
NAMESPACE_TEST(solar_thermal, FlatPlateCollectorTest,
               TestFlatPlateCollectorNominalOperation) {
  DefaultFpcFactory default_fpc_factory = DefaultFpcFactory();
  std::unique_ptr<FlatPlateCollector> flat_plate_collector =
      default_fpc_factory.MakeCollector();
  std::unique_ptr<TimeAndPosition> time_and_position =
      default_fpc_factory.MakeTimeAndPosition();
  std::unique_ptr<ExternalConditions> external_conditions =
      default_fpc_factory.MakeExternalConditions();

  double useful_power_gain = flat_plate_collector->UsefulPowerGain(
      *time_and_position, *external_conditions); // [W]
  double T_out = flat_plate_collector->T_out(*time_and_position,
                                             *external_conditions); // [C]

  EXPECT_NEAR(useful_power_gain, 1.659e3, 1.659e3 * kErrorToleranceHi);
  EXPECT_NEAR(T_out, 50.26, 50.26 * kErrorToleranceHi);
}

// Basic test of expected power gain and outlet temperature of a flat plate
// collector array Uses a factory (abstract factory pattern) to create the
// different physical and non-physical components
NAMESPACE_TEST(solar_thermal, FlatPlateArrayTest,
               TestFlatPlateArrayOfOneNominalOperation) {
  DefaultFpcFactory default_fpc_factory = DefaultFpcFactory();
  std::unique_ptr<FlatPlateArray> flat_plate_array =
      default_fpc_factory.MakeFpcArray();
  ...
  double useful_power_gain =
      flat_plate_array->UsefulPowerGain(timestamp, *external_conditions); // [W]
  double T_out =
      flat_plate_array->T_out(timestamp, *external_conditions); // [C]
...
//========FpcFactory (super
//class)================================================================
std::unique_ptr<FlatPlateArray>
FpcFactory::MakeFpcArray(FlatPlateCollector *flat_plate_collector,
                         CollectorLocation *collector_location,
                         CollectorOrientation *collector_orientation,
                         ArrayDimensions *array_dimensions, Pipe *inlet_pipe,
                         Pipe *outlet_pipe) const {
  return std::unique_ptr<FlatPlateArray>(new FlatPlateArray(
      *flat_plate_collector, *collector_location, *collector_orientation,
      *array_dimensions, *inlet_pipe, *outlet_pipe));
}

std::unique_ptr<FlatPlateCollector> FpcFactory::MakeCollector(
    CollectorTestSpecifications *collector_test_specifications) const {
  return std::unique_ptr<FlatPlateCollector>(
      new FlatPlateCollector(*collector_test_specifications));
}

std::unique_ptr<TimeAndPosition> FpcFactory::MakeTimeAndPosition() const {
  auto time_and_position =
      std::unique_ptr<TimeAndPosition>(new TimeAndPosition);
...
std::unique_ptr<FlatPlateCollector> DefaultFpcFactory::MakeCollector() const {
  std::unique_ptr<CollectorTestSpecifications> collector_test_specifications =
      this->MakeTestSpecifications();
  return std::unique_ptr<FlatPlateCollector>(
      new FlatPlateCollector(*collector_test_specifications));
}

std::unique_ptr<CollectorTestSpecifications>
DefaultFpcFactory::MakeTestSpecifications() const {
  auto collector_test_specifications =
      std::unique_ptr<CollectorTestSpecifications>(
          new CollectorTestSpecifications());
...
std::unique_ptr<Pipe> DefaultFpcFactory::MakePipe() const {
  ...
  return std::unique_ptr<Pipe>(new Pipe(inner_diameter, insulation_conductivity,
                                        insulation_thickness, length));
}

std::unique_ptr<ExternalConditions>
DefaultFpcFactory::MakeExternalConditions() const {
  auto external_conditions =
      std::unique_ptr<ExternalConditions>(new ExternalConditions);
  ...
  external_conditions->weather.wind_speed =
      std::numeric_limits<double>::quiet_NaN();
  external_conditions->weather.wind_direction =
      std::numeric_limits<double>::quiet_NaN();
...
tm DefaultFpcFactory::MakeTime() const {
  tm time;
  // TODO - The timestamp should be generated from a string so all attributes
  // are valid
