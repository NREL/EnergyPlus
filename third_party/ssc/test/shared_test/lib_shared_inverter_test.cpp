...
TEST_F(sharedInverterTest_lib_shared_inverter,
       tempDerateTest_lib_shared_inverter) {
  /// First set of curves
  ...
  // V less than that of first curve, extrapolated startT is 10. and slope is
  // -0.1
  V = 100.;
  T = 11.;
  inv->calculateTempDerate(V, T, pDC, ratio, loss);
  ...
  // V more than that of first curve, interpolated startT is 25. and slope is
  // -0.25
  V = 250.;
  T = 26.;
  reset();
  ...
  // V more than that of second curve, interpolated startT is 40. and slope is
  // -0.4
  V = 400.;
  T = 41.;
  reset();
...
TEST_F(sharedInverterTest_lib_shared_inverter, calculateEffForACPower) {
  ...
  EXPECT_LT(inv->powerAC_kW, p_kwac)
      << "inverter efficiency too low to produce required ac";
  ...
  for (auto p : {0.05, 0.95, 1.}) {
    ...
    inv->calculateACPower(p_kwdc, sandia.Vdco, 25);
    EXPECT_NEAR(inv->powerAC_kW, p_kwac, 1e-3)
        << "inverter should produce required ac of Paco * " << p;
    // test negative ac values
    p_kwac *= -1;
    p_kwdc = inv->calculateRequiredDCPower(p_kwac, sandia.Vdco, 25);
    inv->calculateACPower(p_kwdc, sandia.Vdco, 25);
    EXPECT_NEAR(inv->powerAC_kW, p_kwac, 1e-3)
        << "inverter should produce required (negative) ac of Paco * " << p;
  }
  ...
  for (auto p : {1.05, 1.1}) {
    ...
    p_kwdc = inv->calculateRequiredDCPower(p_kwac, sandia.Vdco, 25);
    inv->calculateACPower(p_kwdc, sandia.Vdco, 25);
    EXPECT_NEAR(inv->powerAC_kW, sandia.Paco / 1000., 1e-3)
        << "inverter cannot produce more than max Paco";
    // test negative ac values
    p_kwac *= -1;
    p_kwdc = inv->calculateRequiredDCPower(p_kwac, sandia.Vdco, 25);
    inv->calculateACPower(p_kwdc, sandia.Vdco, 25);
    EXPECT_NEAR(inv->powerAC_kW, -sandia.Paco / 1000., 1e-3)
        << "inverter cannot produce more than max (negative) Paco";
  }
}
