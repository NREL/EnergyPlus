...
TEST_F(FuelCellTest, Startup_lib_fuel_cell) {
  ...
  EXPECT_EQ(fuelCell->getPower(),
            unitPowerMin_kW + dynamicResponseUp_kWperHour);
  fuelCell->runSingleTimeStep(100);
  fuelCell->runSingleTimeStep(100);
  fuelCell->runSingleTimeStep(100);
  fuelCell->runSingleTimeStep(100);
  ...
  EXPECT_NEAR(fuelCell->getPower(),
              fuelCell->getMaxPower() - dynamicResponseDown_kWperHour, 0.1);
}
...
TEST_F(FuelCellTest, Shutdown_lib_fuel_cell) {
  ...
  // Initiate shutdown.  Should produce heat but no electricity for shutdown
  // hours
...
TEST_F(FuelCellTest, Idle_lib_fuel_cell) {
  ...
  // Initiate shutdown.  Should produce heat and idle at minimum turndown
  // electricity for shutdown hours
  for (size_t h = 0; h < (size_t)shutdown_hours; h++) {
    fuelCell->runSingleTimeStep(0);
...
TEST_F(FuelCellTest, AvailableFuel_lib_fuel_cell) {
  ...
  // Run for startup_hours, assume no fuel consumed from available stock during
  // startup
  for (size_t h = 0; h < (size_t)startup_hours; h++) {
    fuelCell->runSingleTimeStep(20);
  ...
  for (size_t h = (size_t)startup_hours; h < (size_t)startup_hours + 10; h++) {
    fuelCell->runSingleTimeStep(20);
    EXPECT_EQ(fuelCell->getAvailableFuel(),
...
TEST_F(FuelCellTest, HeatCalculation_lib_fuel_cell) {
  ...
  for (size_t h = (size_t)startup_hours; h < (size_t)startup_hours + 10; h++) {
    fuelCell->runSingleTimeStep(20);
    EXPECT_EQ(fuelCell->getPowerThermal(),
              20 * fuelCell->getHeatRecoveryEfficiency());
  }
}
...
TEST_F(FuelCellTest, Replacements_lib_fuel_cell) {
  ...
  fuelCell->setReplacementOption(
      FuelCell::FC_REPLACEMENT_OPTION::REPLACE_AT_CAPACITY);
...
TEST_F(FuelCellTest, DispatchFixedSubhourly_lib_fuel_cell_dispatch) {
  ...
  fuelCellDispatchSubhourly->setDispatchOption(
      FuelCellDispatch::FC_DISPATCH_OPTION::FIXED);
...
TEST_F(FuelCellTest, DispatchFixedMultiple_lib_fuel_cell_dispatch) {
  ...
  for (size_t h = sh + 1; h < sh + 10; h++) {
    ...
    EXPECT_EQ(fuelCellDispatchMultiple->getBatteryPower()->powerFuelCellToLoad,
              0);
    EXPECT_EQ(fuelCellDispatchMultiple->getBatteryPower()->powerFuelCellToGrid,
              n_multipleFuelCells * 40);
    EXPECT_EQ(fuelCellDispatchMultiple->getBatteryPower()->powerSystemToLoad,
              10);
    EXPECT_EQ(fuelCellDispatchMultiple->getBatteryPower()->powerSystemToGrid,
              10);
  }
}
...
TEST_F(FuelCellTest, DispatchLoadFollow_lib_fuel_cell_dispatch) {
  ...
  fuelCellDispatch->setDispatchOption(
  ...
  // Dispatch fuel cell for net load of 60 kW, dynamic response should limit to
  // 40 kW
...
TEST_F(FuelCellTest, DispatchManual_lib_fuel_cell_dispatch) {
  ...
  // Dispatch fuel cell at 40% of max output (40 kW, limited by dynamic
  // response, and min turndown)
  for (size_t s = 0; s < stepsPerHour; s++) {
...
TEST_F(FuelCellTest, DispatchManualUnits_lib_fuel_cell_dispatch) {
  ...
  fuelCellDispatchMultiple->setDispatchOption(
      FuelCellDispatch::FC_DISPATCH_OPTION::MANUAL);
  ...
  // Dispatch fuel cells at 40% of max output (40 kW per unit, limited by
  // dynamic response, and min turndown)
...
TEST_F(FuelCellTest, DispatchInput_lib_fuel_cell_dispatch) {
  ...
  fuelCellDispatch->setDispatchOption(
      FuelCellDispatch::FC_DISPATCH_OPTION::INPUT);
  ...
  // Dispatch fuel cell at 50% of max output (50 kW, limited by dynamic
  // response)
