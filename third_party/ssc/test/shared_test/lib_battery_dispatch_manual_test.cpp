...
TEST_F(ManualTest_lib_battery_dispatch, PowerLimitsDispatchManualAC) {
  dispatchManual = new dispatch_manual_t(
      batteryModel, dtHour, SOC_min, SOC_max, currentChoice, currentChargeMax,
      currentDischargeMax, powerChargeMax, powerDischargeMax, powerChargeMax,
      powerDischargeMax, minimumModeTime, dispatchChoice, meterPosition,
      scheduleWeekday, scheduleWeekend, canCharge, canDischarge, canGridcharge,
      canGridcharge, percentDischarge, percentGridcharge);
  ...
  EXPECT_LT(
      dispatchManual->battery_power_to_fill(),
...
TEST_F(ManualTest_lib_battery_dispatch, CurrentLimitsDispatchManualAC) {
  dispatch_t::CURRENT_CHOICE testChoice =
      dispatch_t::CURRENT_CHOICE::RESTRICT_CURRENT;
  double testChargeMax = 20;
  double testDischargeMax = 20;
  dispatchManual = new dispatch_manual_t(
      batteryModel, dtHour, SOC_min, SOC_max, testChoice, testChargeMax,
  ...
  batteryPower->voltageSystem = 600;
  dispatchManual->dispatch(year, hour_of_year, step_of_hour);
  double current = batteryPower->powerBatteryDC * util::kilowatt_to_watt /
  ...
  batteryPower->powerLoad = 1000;
  dispatchManual->dispatch(year, hour_of_year, step_of_hour);
  current = batteryPower->powerBatteryDC * util::kilowatt_to_watt /
            batteryPower->voltageSystem;
...
TEST_F(ManualTest_lib_battery_dispatch, BothLimitsDispatchManualAC) {
  dispatch_t::CURRENT_CHOICE testChoice =
      dispatch_t::CURRENT_CHOICE::RESTRICT_BOTH;
  double testChargeMax = 20;
  double testDischargeMax = 20;
...
TEST_F(ManualTest_lib_battery_dispatch, BothLimitsDispatchManualDC) {
  ...
  dispatchManual = new dispatch_manual_t(
      batteryModel, dtHour, SOC_min, SOC_max, testChoice, testChargeMax,
      testDischargeMax, testChargeMaxPower, testDischargeMaxPower,
      powerChargeMax, powerDischargeMax, minimumModeTime, dispatchChoice,
      meterPosition, scheduleWeekday, scheduleWeekend, canCharge, canDischarge,
      canGridcharge, canGridcharge, percentDischarge, percentGridcharge);
...
TEST_F(ManualTest_lib_battery_dispatch, DispatchChangeFrequency) {
  ...
  dispatchManual = new dispatch_manual_t(
      batteryModel, testTimestep, SOC_min, SOC_max, currentChoice,
      currentChargeMax, currentDischargeMax, powerChargeMax, powerDischargeMax,
      powerChargeMax, powerDischargeMax, testMinTime, dispatchChoice,
      meterPosition, scheduleWeekday, scheduleWeekend, canCharge, canDischarge,
      canGridcharge, canGridcharge, percentDischarge, percentGridcharge);
  ...
  // Abruptly cut off the PV and increase the load. Power should go to zero (1
  // minute)
  step_of_hour += 1;
  batteryPower->powerSystem = 0;
  batteryPower->voltageSystem = 600;
...
TEST_F(ManualTest_lib_battery_dispatch, ManualGridChargingOffTest) {
  ...
  batteryPower->powerSystem = 0;
  batteryPower->voltageSystem = 600;
  batteryPower->powerGridToBattery = 1000;
  dispatchManual->dispatch(year, hour_of_year, step_of_hour);
...
TEST_F(ManualTest_lib_battery_dispatch, ManualGridChargingOnTest) {
  ...
  dispatchManual = new dispatch_manual_t(
      ...
      currentDischargeMax, powerChargeMax, powerDischargeMax, powerChargeMax,
      powerDischargeMax, minimumModeTime, dispatchChoice, meterPosition,
      scheduleWeekday, scheduleWeekend, canCharge, canDischarge,
      testCanGridcharge, canGridcharge, percentDischarge,
      testPercentGridCharge);
  ...
  batteryPower->powerGridToBattery = 1000;
  dispatchManual->dispatch(year, hour_of_year, step_of_hour);
  EXPECT_NEAR(batteryPower->powerBatteryDC,
              -powerChargeMax * batteryPower->singlePointEfficiencyACToDC, 1.0);
...
TEST_F(ManualTest_lib_battery_dispatch, InverterEfficiencyCutoffDC) {
  ...
  dispatchManual = new dispatch_manual_t(
      ...
      powerDischargeMax, minimumModeTime, dispatchChoice, meterPosition,
      scheduleWeekday, scheduleWeekend, canCharge, canDischarge,
      testCanGridcharge, canGridcharge, testPercentGridCharge,
      testPercentGridCharge);
  ...
  batteryPower->powerSystem = 0;
  batteryPower->voltageSystem = 600;
  batteryPower->powerGridToBattery = 7;
  dispatchManual->dispatch(year, 0, step_of_hour);
  EXPECT_NEAR(batteryPower->sharedInverter->efficiencyAC, 0.0,
              0.1); // Efficiency is 0 when system is not running
  EXPECT_NEAR(batteryPower->powerBatteryDC, 0.0, 0.1);
  ...
  batteryPower->powerSystem = 0;
  batteryPower->voltageSystem = 600;
  batteryPower->powerGridToBattery = 0;
  batteryPower->powerLoad = 7;
  dispatchManual->dispatch(year, 0, step_of_hour);
  EXPECT_NEAR(batteryPower->sharedInverter->efficiencyAC, 35.82,
              0.1); // Not enforced becasue no PV
  EXPECT_NEAR(batteryPower->powerBatteryDC, 4.43, 0.1);

  batteryPower->powerSystem = 770;
  batteryPower->voltageSystem = 600;
  batteryPower->powerGridToBattery = 0;
  batteryPower->powerLoad = 1000;
  dispatchManual->dispatch(year, 12, step_of_hour);
  EXPECT_NEAR(batteryPower->sharedInverter->efficiencyAC, 93.9, 0.1);
  EXPECT_NEAR(batteryPower->powerBatteryDC, 49.9,
              0.1); // Dispatch both battery and PV

  batteryPower->powerSystem = 1000;
  batteryPower->voltageSystem = 600;
  batteryPower->powerGridToBattery = 0;
  batteryPower->powerLoad = 1100;
  dispatchManual->dispatch(year, 12, step_of_hour);
  EXPECT_NEAR(batteryPower->sharedInverter->efficiencyAC, 77, 0.1);
  EXPECT_NEAR(batteryPower->powerBatteryDC, 0.0,
              2.0); // Overwhelm inverter with PV, back off battery
}
...
TEST_F(ManualTest_lib_battery_dispatch_losses, TestLossesWithDispatch) {
  ...
  // Test max charge power constraint
  batteryPower->powerSystem = 40;
  batteryPower->voltageSystem = 600;
  dispatchManual->dispatch(year, hour_of_year, step_of_hour);
  EXPECT_NEAR(batteryPower->powerSystemToBattery,
              batteryPower->powerSystem - batteryPower->powerSystemLoss, 0.1);

  // Test max discharge power constraint
  batteryPower->powerSystem = 0;
  batteryPower->voltageSystem = 600;
  batteryPower->powerLoad = 40;
  dispatchManual->dispatch(year, hour_of_year, step_of_hour);
  EXPECT_NEAR(batteryPower->powerGeneratedBySystem, batteryPower->powerLoad,
              0.5); // Constraints drive efficiency lower, meaning some grid
                    // power is used to meet load (<0.5 kW)
