...
TEST_F(windTurbineTest, turbinePowerTest_lib_windwakemodel) {
  ...
  EXPECT_NEAR(thrustCoeff, 0.3725, e)
      << "At 11.25m/s, the thrust coeff should be 0.3725";
...
TEST_F(simpleWakeModelTest, wakeCalcNoInterference_lib_windwakemodel) {
  ...
  swm.wakeCalculations(seaLevelAirDensity, &distDownwind[0], &distCrosswind[0],
                       &power[0], &eff[0], &thrust[0], &windSpeed[0],
                       &turbIntensity[0]);
  for (int i = 0; i < numberTurbines; i++) {
    ...
    EXPECT_NEAR(windSpeed[i], 10, e)
        << "No change expected in windspeed at index " << i;
    EXPECT_NEAR(turbIntensity[i], 0.1, e)
        << "Turb intensity calculated at index " << i;
  }
}

/// All turbines are in a line at same crosswind distance: lot of power
/// reduction
TEST_F(simpleWakeModelTest, wakeCalcAllInterference_lib_windwakemodel) {
  ...
  for (int i = 0; i < numberTurbines; i++) {
    EXPECT_NEAR(thrust[i], newThrust[i], e)
        << "Thrust calculated at index " << i;
    EXPECT_NEAR(power[i], newPower[i], e) << "Power calculated at index " << i;
    EXPECT_NEAR(eff[i], newEff[i], e) << "Eff calculated at index " << i;
    EXPECT_NEAR(windSpeed[i], newWindSpeed[i], e)
        << "windSpeeds at turbine " << i << " should be reduced.";
    if (i >= 1)
      EXPECT_GT(turbIntensity[i], 0.1)
          << "Turb intensity at turbine " << i << " should be increased.";
  }
}

/// Turbines form a triangle with two downwind turbines: little change due to
/// crosswind distance
TEST_F(simpleWakeModelTest, wakeCalcTriangleInterference_lib_windwakemodel) {
  ...
  for (int i = 0; i < numberTurbines; i++) {
    ...
    EXPECT_NEAR(eff[i], 100, e) << "Eff calculated at index " << i;
    EXPECT_NEAR(windSpeed[i], 10, e)
        << "Minor wind reduction expected at turbine " << i;
    if (i >= 1)
      EXPECT_NEAR(turbIntensity[i], 0.10031, e)
          << "Turb intensity should be increased at turbine " << i;
  }
...
TEST_F(parkWakeModelTest, wakeCalcNoInterference_lib_windwakemodel) {
  ...
  pm.wakeCalculations(seaLevelAirDensity, &distDownwind[0], &distCrosswind[0],
                      &power[0], &eff[0], &thrust[0], &windSpeed[0],
                      &turbIntensity[0]);
  for (int i = 0; i < numberTurbines; i++) {
    EXPECT_NEAR(thrust[i], .47669, e) << "Thrust calculated at index " << i;
...
TEST_F(parkWakeModelTest, wakeCalcTriangleInterference_lib_windwakemodel) {
  ...
  for (int i = 1; i < numberTurbines; i++) {
    ...
    EXPECT_NEAR(windSpeed[i], 9.03, e)
        << "Minor wind reduction expected at turbine " << i;
    EXPECT_NEAR(turbIntensity[i], 0.1, e) << "Turb intensity at turbine " << i;
  }
  EXPECT_EQ(turbIntensity[1], turbIntensity[2]);
...
TEST_F(eddyViscosityWakeModelTest, wakeCalcNoInterference_lib_windwakemodel) {
  ...
  evm.wakeCalculations(seaLevelAirDensity, &distDownwind[0], &distCrosswind[0],
                       &power[0], &eff[0], &thrust[0], &windSpeed[0],
                       &turbIntensity[0]);
  for (int i = 0; i < numberTurbines; i++) {
    EXPECT_NEAR(thrust[i], .47669, e) << "Thrust calculated at index " << i;
...
/// Turbines form a narrow triangle with two downwind turbines: some overlap
TEST_F(eddyViscosityWakeModelTest,
       wakeCalcTriangleInterference_lib_windwakemodel) {
  distDownwind = {0, 5, 5};
  distCrosswind = {0, -1, 1};
  ...
  for (int i = 1; i < numberTurbines; i++) {
    ...
    EXPECT_NEAR(windSpeed[i], 8.04, e)
        << "Minor wind reduction expected at turbine " << i;
    EXPECT_NEAR(turbIntensity[i], 0.1, e) << "Turb intensity at turbine " << i;
  }
  EXPECT_EQ(turbIntensity[1], turbIntensity[2]);
