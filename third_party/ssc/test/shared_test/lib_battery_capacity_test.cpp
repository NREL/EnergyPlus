...
TEST_F(LiIon_lib_battery_capacity_test, updateCapacityThermalTest) {
  ...
  auto s1 = capacity_state({500, 1000, 800, 0, 0, 62.5, 50, 2});
...
TEST_F(LiIon_lib_battery_capacity_test, replaceBatteryTest) {
  ...
  s1 = {0, 0, 1000, 0, 500, 0, 50, 2};
  compareState(old_cap->get_state(), s1,
               "replaceBatteryTest: init degradation");
...
TEST_F(LiIon_lib_battery_capacity_test, runSequenceTest) {
  double I = 400;
  old_cap->updateCapacity(I, dt_hour);
  auto s1 = capacity_state({150, 1000, 1000, 350, 0, 15, 50, 2});
...
// battery is overcharged so capacity gets reduced while we want to discharge
// some current
TEST_F(LiIon_lib_battery_capacity_test, OverchargedSOCLimits) {
  ...
  auto cap =
      std::make_shared<capacity_lithium_ion_t>(19984, 90, SOC_max, SOC_min, dt);
...
// battery is overcharged so capacity gets reduced while we want to charge some
// current
...
// battery is undercharged so capacity gets increased while we want to discharge
// some current
TEST_F(LiIon_lib_battery_capacity_test, UnderchargedSOCLimits) {
  ...
  auto cap =
      std::make_shared<capacity_lithium_ion_t>(19984, 10, SOC_max, SOC_min, dt);
  cap->updateCapacityForThermal(90);
...
// battery is undercharged so capacity gets increased while we want to charge
// some current
...
TEST_F(KiBam_lib_battery_capacity_test, updateCapacityTest) {
  double I = 1.5;
  old_cap->updateCapacity(I, dt_hour);
  auto s1 = capacity_state({52.58, 108.16, 108.16, 1.5, 0, 48.613, 50, 2});
  compareState(old_cap->get_state(), s1, "updateCapacityTest: 1");

  I = 3;
  old_cap->updateCapacity(I, dt_hour);

  s1 = {49.58, 108.16, 108.16, 3, 0, 45.839, 48.613, 2};
  compareState(old_cap->get_state(), s1, "updateCapacityTest: 2");

  I = 490;
  old_cap->updateCapacity(I, dt_hour);
  s1 = {22.927, 108.16, 108.16, 26.65, 0, 21.19, 45.839, 2};
  compareState(old_cap->get_state(), s1, "updateCapacityTest: 3");

  I = 490;
  old_cap->updateCapacity(I, dt_hour);
  s1 = {16.67, 108.16, 108.16, 6.25, 0, 15.413, 21.19, 2};
  compareState(old_cap->get_state(), s1, "updateCapacityTest: 4");
}
...
TEST_F(KiBam_lib_battery_capacity_test, updateCapacityThermalTest) {
  double percent = 80;
  old_cap->updateCapacityForThermal(percent);
  auto s1 = capacity_state({54.07, 108.15, 86.53, 0, 0, 62.5, 50, 2});
  compareState(old_cap->get_state(), s1, "updateCapacityThermalTest: 1");
...
TEST_F(KiBam_lib_battery_capacity_test, runSequenceTest) {
  double I = 30;
  old_cap->updateCapacity(I, dt_hour);
  auto s1 = capacity_state({24.07, 108.16, 108.16, 30, 0, 22.26, 50, 2});
  compareState(old_cap->get_state(), s1, "runSequenceTest: 1");
  I = -30;
  old_cap->updateCapacity(I, dt_hour);
