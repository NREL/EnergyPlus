...
TEST_F(lib_battery_lifetime_cycle_test, runCycleLifetimeTestWithNoise) {
  int seed = 100;
  double tol_high =
      1.6; // Randomness will generate different results on different platforms
...
TEST_F(lib_battery_lifetime_test, updateCapacityTest) {
  ...
  while (idx < 876) {
    ...
    EXPECT_EQ(state.calendar->q_relative_calendar,
              model->capacity_percent_calendar());
...
TEST_F(lib_battery_lifetime_test, runCycleLifetimeTestWithRestPeriod) {
  ...
  std::vector<double> DOD = {5,  50, 95, 50, 5,  5,  5,  50, 95,
                             50, 5,  5,  5,  50, 95, 50, 5}; // 3 cycles 90% DOD
  std::vector<bool> charge_changed = {true,  false, false, true, false, false,
  ...
  while (idx < DOD.size()) {
    ...
    model->runLifetimeModels(idx, charge_changed[idx], DOD_prev, DOD[idx],
                             T_battery);
    idx++;
  }
...
TEST_F(lib_battery_lifetime_nmc_test, StorageDays) {
  ...
  for (size_t i = 0; i < days.back() + 1; i++) {
    ...
    if (pos != days.end()) {
      auto state = model->get_state();
      EXPECT_NEAR(state.nmc_li_neg->q_relative_li,
                  expected_q_li[pos - days.begin()], 0.5);
    }
  }
...
TEST_F(lib_battery_lifetime_nmc_test, CyclingCRate) {
  ...
  std::vector<double> DODs_day = {50., 56.67, 63.33, 70., 76.67, 83.33,
                                  90., 83.33, 76.67, 70., 63.33, 56.67,
                                  50., 43.33, 36.67, 30., 23.33, 16.67,
...
/// There's less accuracy since the degradation coefficients from the first day
/// are lost when doing computation on day 2
...
/** Test focusing on how different time steps affect the integration of a day's
 *degradation in the NMC life model. The integration of degradation is done at
 *the end of each day when the elapsed time, `cum_dt` is exactly 1. Check that
 *if a simulation step has a timestep large enough that `cum_dt` passes from <1
 *to >1, that the effects on lifetime are the same by breaking that timestep up
 *and accruing the degradation and `cum_dt` correctly
 **/
TEST_F(lib_battery_lifetime_nmc_test, IrregularTimeStep) {
  ...
  auto subhourly_model =
      std::unique_ptr<lifetime_nmc_t>(new lifetime_nmc_t(b_params, b_state));
  ...
  printf("\n");
  // run 30min timesteps for 23.5 hours then hourly for 24, then 1 0.5 hr time
  // idx
  day = 0;
  while (day < 87) {
    size_t idx = 0;
