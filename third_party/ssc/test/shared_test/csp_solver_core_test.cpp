...
/**
 * This class tests the C_csp_weatherreader's functions and ensures that the
 * interface is the same using weatherfile & weatherdata as weather inputs. The
 * test also tests for variable access and memory.
 */
...
protected:
  void SetUp() {
    ...
    char hourly[256];
    int a = sprintf(hourly, "%s/test/input_docs/weather.csv",
                    std::getenv("SSCDIR"));
...
protected:
  ...
  void TearDown() { free_weatherdata_array(data); }
};

/**
 * Integration tests for CSP's weatherreader class test that the interface &
 * outputs are the same whether the data is taken from a file or as var_data.
 * The test weather data is for Buenos Aires and taken from SAM's solar
 * resource.
 */
...
TEST_F(UsingFileCaseWeatherReader, IntegrationTest_csp_solver_core) {
  wr.init();
  // check header values
  EXPECT_NEAR(wr.m_weather_data_provider->lat(), -34.82, e)
      << "Values in weather file's m_hdr\n";
  EXPECT_NEAR(wr.m_weather_data_provider->lon(), -58.53, e)
      << "Values in weather file's m_hdr\n";
  EXPECT_NEAR(wr.m_weather_data_provider->tz(), -3, e)
      << "Values in weather file's m_hdr\n";
  EXPECT_NEAR(wr.m_weather_data_provider->elev(), 20, e)
      << "Values in weather file's m_hdr\n";
  EXPECT_EQ(wr.m_weather_data_provider->step_sec(), 3600)
      << "Values in weather file's m_hdr\n";
  EXPECT_EQ(wr.m_weather_data_provider->nrecords(), 8760)
      << "Values in weather file's m_hdr\n";

  // check reading first timestep
  sim_info.ms_ts.m_time = 3600;
  wr.timestep_call(sim_info);
  EXPECT_NEAR(wr.ms_outputs.m_twet, 19.1422, e)
      << "Twet should be calculated in weather_data_provider.read()\n";
  EXPECT_NEAR(wr.ms_outputs.m_tdry, 20.9, e)
      << "Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_pres, 1010, e)
      << "Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_beam, 0.0, e)
      << "Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_wspd, 2.1, e)
      << "Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_aod, 0.291, e)
      << "Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_solazi, 187.367892, e)
      << "Members specific to CSP weather\n";
  EXPECT_NEAR(wr.ms_outputs.m_solzen, 121.750884, e)
      << "Members specific to CSP weather\n";
  EXPECT_NEAR(wr.ms_solved_params.m_shift, -13.530000, e)
      << "Members specific to CSP weather\n";
  EXPECT_FALSE(wr.ms_solved_params.m_leapyear)
      << "Members specific to CSP weather\n";
  ...
  EXPECT_NEAR(wr.ms_outputs.m_twet, 27.4906, e)
      << "Twet should be calculated in weather_data_provider.read()\n";
  EXPECT_NEAR(wr.ms_outputs.m_tdry, 29.6, e)
      << "Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_pres, 1007, e)
      << "Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_beam, 566, e)
      << "Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_wspd, 1.5, e)
      << "Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_aod, 0.291, e)
      << "Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_solazi, 79.817137, e)
      << "Members specific to CSP weather\n";
  EXPECT_NEAR(wr.ms_outputs.m_solzen, 34.102205, e)
      << "Members specific to CSP weather\n";
  EXPECT_NEAR(wr.ms_solved_params.m_shift, -13.530000, e)
      << "Members specific to CSP weather\n";
  EXPECT_FALSE(wr.ms_solved_params.m_leapyear)
  ...
  EXPECT_EQ(wr.ms_outputs.m_minute, 30)
      << "Originally empty, minute column should be set to 30 by weatherfile";

  EXPECT_TRUE(std::isnan(wr.ms_outputs.m_global))
      << "Global not in weatherfile\n";
  EXPECT_NEAR(wr.ms_outputs.m_beam, 602, e);
  EXPECT_NEAR(wr.ms_outputs.m_diffuse, 315, e);
  EXPECT_NEAR(wr.ms_outputs.m_tdry, 30.6, e);
  EXPECT_NEAR(wr.ms_outputs.m_twet, 28.4516, e)
      << "Twet should be calculated in weather_data_provider.read()\n";
  EXPECT_NEAR(wr.ms_outputs.m_tdew, 19.8, e);
  EXPECT_NEAR(wr.ms_outputs.m_wspd, 2.6, e)
      << "Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_wdir, 180, e)
      << "Values copied from weather file\n";
  ...
  EXPECT_NEAR(wr.ms_outputs.m_albedo, 0.17, e)
      << "Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_aod, 0.291, e)
      << "Values copied from weather file\n";

  EXPECT_NEAR(wr.ms_outputs.m_poa, 871.628310, e)
      << "Calculated in timestep_call()\n";
  EXPECT_NEAR(wr.ms_outputs.m_solazi, 64.094861, e)
      << "Calculated in timestep_call()\n";
  EXPECT_NEAR(wr.ms_outputs.m_solzen, 22.387110, e)
      << "Calculated in timestep_call()\n";
  EXPECT_NEAR(wr.ms_outputs.m_hor_beam, 556.628310, e)
      << "Calculated in timestep_call()\n";
...
TEST_F(UsingDataCaseWeatherReader, IntegrationTest_csp_solver_core) {
  wr.init();
  // check header values
  ASSERT_NEAR(wr.m_weather_data_provider->lat(), -34.82, e)
      << "Values in weather data's m_hdr";
  ASSERT_NEAR(wr.m_weather_data_provider->lon(), -58.53, e)
      << "Values in weather data's m_hdr";
  ASSERT_NEAR(wr.m_weather_data_provider->tz(), -3, e)
      << "Values in weather data's m_hdr";
  ASSERT_NEAR(wr.m_weather_data_provider->elev(), 20, e)
      << "Values in weather data's m_hdr";
  ASSERT_EQ(wr.m_weather_data_provider->step_sec(), 3600)
      << "Values in weather data's m_hdr";
  ASSERT_EQ(wr.m_weather_data_provider->nrecords(), 8760)
      << "Values in weather data's m_hdr";
  ...
  wr.timestep_call(sim_info);
  EXPECT_NEAR(wr.ms_outputs.m_twet, 19.1422, e)
      << "1st time step: Twet should be calculated in "
         "weather_data_provider.read()\n";
  EXPECT_NEAR(wr.ms_outputs.m_tdry, 20.9, e)
      << "1st time step: Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_pres, 1010, e)
      << "1st time step: Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_beam, 0.0, e)
      << "1st time step: Values copied from weather file\n";
  ...
  EXPECT_NEAR(wr.ms_outputs.m_aod, 0.291, e)
      << "1st time step: Values copied from weather file\n";
  EXPECT_NEAR(wr.ms_outputs.m_solazi, 187.367892, e)
      << "1st time step: Members specific to CSP weather\n";
  EXPECT_NEAR(wr.ms_outputs.m_solzen, 121.750884, e)
      << "1st time step: Members specific to CSP weather\n";
  EXPECT_NEAR(wr.ms_solved_params.m_shift, -13.530000, e)
      << "1st time step: Members specific to CSP weather\n";
  ...
  EXPECT_NEAR(wr.ms_outputs.m_solazi, 79.817137, e)
      << "Members specific to CSP weather\n";
  EXPECT_NEAR(wr.ms_outputs.m_solzen, 34.102206, e)
      << "Members specific to CSP weather\n";
  EXPECT_NEAR(wr.ms_solved_params.m_shift, -13.530000, e)
      << "Members specific to CSP weather\n";
  EXPECT_FALSE(wr.ms_solved_params.m_leapyear)
  ...
  EXPECT_TRUE(std::isnan(wr.ms_outputs.m_global))
      << "11th hour, Global not in weatherfile\n";
  ...
  EXPECT_TRUE(std::isnan(wr.ms_outputs.m_snow))
      << "11th hour, Snow not in weatherfile\n";
...
protected:
  ...
  void SetUp() {
    ...
    solver = new C_csp_solver(wr, *cr, *pc, tes, tou, system, ssc_cmod_update,
                              (void *)0);
  }
};
