...
/**
 * Tests windfile's interpolation of measurement height's pres, tmp, speed, &
 * dir data points to required hub height.
 */
...
TEST_F(windDataProviderCalculatorTest, FindClosestUsingData_lib_windfile_test) {
  ...
  double pres, temp, spd, dir, heightOfClosestMeasuredSpd,
      heightOfClosestMeasuredDir;
  windDataProvider->read(85, &spd, &dir, &temp, &pres,
                         &heightOfClosestMeasuredSpd,
                         &heightOfClosestMeasuredDir, true);
  ...
  EXPECT_NEAR(heightOfClosestMeasuredSpd, 85, e)
      << "case 1: hub height can be interpolated.";

  //// Case 2: hubheight: 95, cannot interpolate, gives closest
  windDataProvider->read(95, &spd, &dir, &temp, &pres,
                         &heightOfClosestMeasuredSpd,
                         &heightOfClosestMeasuredDir, true);
