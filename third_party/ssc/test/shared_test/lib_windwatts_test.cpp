...
/**
 * windPowerCalculatorTest requires an initialized windTurbine and wakeModel,
 * and XCoords & YCoords. SetUp() allocates the vectors for input and output
 * variables for windPowerUsingResource.
 */
...
protected:
  ...
  std::vector<double> turbulenceCoeff, distX, distY, distDownwind,
      distCrosswind;
...
TEST_F(windPowerCalculatorTest, windPowerUsingResource_lib_windwatts) {
  ...
  int run = wpc.windPowerUsingResource(
      windSpeedData, windDirData, pressureData, tempData, &farmPower,
      &farmPowerGross, &power[0], &thrust[0], &eff[0], &windSpeed[0],
      &turbulenceCoeff[0], &distDownwind[0],
...
TEST_F(windPowerCalculatorTest, windPowerUsingWeibull_lib_windwatts) {
  ...
  double energyTotal = wpc.windPowerUsingWeibull(
      weibullK, avgSpeed, refHeight, &energy[0]); // runs method we want to test
...
TEST_F(windPowerCalculatorTest, windPowerUsingDistribution_lib_windwatts) {
  ...
  std::vector<std::vector<double>> dst = {
      {1.5, 180, .12583}, {5, 180, .3933},     {8, 180, .18276},
