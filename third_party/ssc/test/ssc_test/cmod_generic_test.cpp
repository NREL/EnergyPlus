...
TEST_F(CMGeneric, SingleOwnerWithBattery_cmod_generic) {
  ...
  for (size_t i = 0; i < dispatch_options.size(); i++) {
    ssc_data_set_number(data, "batt_dispatch_choice",
                        (ssc_number_t)dispatch_options[i]);
  ...
  set_array(data, "batt_custom_dispatch", generictest::batt_dispatch_path_30min,
            8760 * 2);
  set_array(data, "batt_room_temperature_celsius",
            generictest::temperature_path_30min, 8760 * 2);
  for (size_t i = 0; i < dispatch_options.size(); i++) {
    ssc_data_set_number(data, "batt_dispatch_choice",
                        (ssc_number_t)dispatch_options[i]);
    EXPECT_FALSE(run_module(data, "generic_system"));
  ...
  set_array(data, "batt_custom_dispatch", generictest::batt_dispatch_path_30min,
            8760 * 2); // 8760 or 8760 * 3 fails with execution error on Linux
                       // and windows
  EXPECT_FALSE(run_module(data, "generic_system"));
  EXPECT_FALSE(run_module(data, "battery"));
}
