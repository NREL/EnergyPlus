...
/// Test standalone battery compute modeule with a input lifetime generation and
/// commercial load
TEST_F(CMBattery_cmod_battery, CommercialLifetimePeakShaving) {
  ...
  if (!errors) {
    ...
    ssc_data_get_number(data, "average_battery_roundtrip_efficiency",
                        &roundtripEfficiency);
    ...
    int replacements =
        std::accumulate(calculated_array, calculated_array + n, 0);
...
TEST_F(CMBattery_cmod_battery, ResilienceMetricsFullLoadLifetime) {
  ...
  data_vtab->assign(
      "gen", var_data(data_vtab->as_array("gen", nullptr), 8760 * nyears));
  data_vtab->assign("batt_replacement_option", 0);
