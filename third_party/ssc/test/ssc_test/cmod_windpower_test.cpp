...
TEST_F(CMWindPowerIntegration, HubHeightInterpolation_cmod_windpower) {
  ...
  EXPECT_GT(annual_energy, 4e06)
      << "Annual energy should be higher than height at 90";
...
TEST_F(CMWindPowerIntegration, WakeModelsUsingFile_cmod_windpower) {
  ...
  ssc_number_t monthly_energy =
      ssc_data_get_array(data, "monthly_energy", nullptr)[0];
...
TEST_F(CMWindPowerIntegration, UsingInterpolatedSubhourly_cmod_windpower) {
  ...
  ssc_number_t hourly_january_energy =
      ssc_data_get_array(data, "monthly_energy", nullptr)[0];

  // Using 15 min File
  sprintf(file, "%s/test/input_docs/AR Northwestern-Flat Lands-15min.srw",
          SSCDIR);
  ...
  EXPECT_NEAR(check_annual_energy, hourly_annual_energy,
              0.005 * check_annual_energy);

  ssc_number_t check_january_energy =
      ssc_data_get_array(data, "monthly_energy", nullptr)[0];
  EXPECT_NEAR(check_january_energy, hourly_january_energy,
              0.005 * check_january_energy);

  size_t nEntries = static_cast<var_table *>(data)->lookup("gen")->num.ncols();
  EXPECT_EQ(nEntries, 8760 * 4);

  // Using 5 min File
  sprintf(file, "%s/test/input_docs/AR Northwestern-Flat Lands-5min.srw",
          SSCDIR);
...
TEST_F(CMWindPowerIntegration, WindDist_cmod_windpower) {
  ...
  double dist[18] = {1.5, 180, .12583, 5,    180, .3933,  8,  180, .18276,
                     10,  180, .1341,  13.5, 180, .14217, 19, 180, .0211};
...
TEST_F(CMWindPowerIntegration, WindDist2_cmod_windpower) {
  ssc_data_set_number(data, "wind_resource_model_choice", 2);
  // mimic a weibull with k factor 2 and avg speed 7.25 for comparison -> scale
  // param : 8.181
  double dst[18] = {1.5, 180, .12583, 5,    180, .3933,  8,  180, .18276,
                    10,  180, .1341,  13.5, 180, .14217, 19, 180, .0211};
...
TEST(Turbine_powercurve_cmod_windpower_eqns, Case1) {
  ...
  util::matrix_t<ssc_number_t> ws =
      vd->lookup("wind_turbine_powercurve_windspeeds")->num;
  util::matrix_t<ssc_number_t> power =
      vd->lookup("wind_turbine_powercurve_powerout")->num;
...
#ifdef __WINDOWS__
  auto python_dir =
      std::string(std::getenv("SAMNTDIR")) + "\\deploy\\runtime\\python\\";
#else
  if (!std::getenv("CMAKEBLDDIR"))
    return false;
  auto python_dir = std::string(std::getenv("CMAKEBLDDIR")) +
                    "/sam/SAM.app/Contents/runtime/python/";
...
TEST(windpower_landbosse, SetupPython) {
  ...
  std::string configPath =
      std::string(get_python_path()) + "python_config.json";
  ...
  if (!python_config_root.isMember("miniconda_version"))
    throw std::runtime_error("Missing key 'miniconda_version' in " +
                             configPath);
  ...
  std::vector<std::string> config = {
      python_config_root["python_version"].asString(),
...
TEST(windpower_landbosse, RunSuccess) {
  ...
  EXPECT_NEAR(vd->lookup("total_gridconnection_cost")->num[0], 5.61774e+06,
              1e2);
  ...
  std::vector<std::string> all_outputs = {
      ...
      "development_labor_usd",
      "development_material_usd",
      "development_mobilization_usd",
      "engineering_usd",
...
TEST(windpower_landbosse, SubhourlyFail) {
  ...
  EXPECT_EQ(
      err,
      "Error in Weather_Data: Length of values does not match length of index");
}
...
TEST(windpower_landbosse, NegativeInputFail) {
  ...
  auto err = vd->lookup("errors")->str;
  EXPECT_EQ(err, "Error in NegativeInputError: User entered a negative value "
                 "for depth. This is an invalid entry");
}
