...
TEST_F(CMFuelCell, NoFinancialModelFixed_cmod_fuelcell) {
  ...
  if (!errors) {
    ...
    ssc_data_get_number(data, "fuelcell_dynamic_response_up",
                        &dynamic_response);
    ...
    EXPECT_NEAR(calculated_array[(size_t)(startup_hours + 1)],
                2 * dynamic_response, 0.1);
  ...
  if (!errors) {
    ...
    if (!errors_battery) {
      ...
      for (int i = 0; i < n; i += (int)interval) {
        EXPECT_LT(fabs(load[i] - (fc_to_load[i] + pv_to_load[i] +
                                  batt_to_load[i] + grid_to_load[i])),
                  1.0);
      }
    }
  }
