...
TEST_F(CMPvwattsV5Integration_cmod_pvwattsv5, DefaultNoFinancialModel) {
  ...
  EXPECT_NEAR((double)monthly_energy[0], 435.198, error_tolerance)
      << "Monthly energy of January";
  EXPECT_NEAR((double)monthly_energy[1], 482.681, error_tolerance)
      << "Monthly energy of February";
  EXPECT_NEAR((double)monthly_energy[2], 593.864, error_tolerance)
      << "Monthly energy of March";
  EXPECT_NEAR((double)monthly_energy[3], 673.452, error_tolerance)
      << "Monthly energy of April";
  EXPECT_NEAR((double)monthly_energy[4], 715.823, error_tolerance)
      << "Monthly energy of May";
  EXPECT_NEAR((double)monthly_energy[5], 665.008, error_tolerance)
      << "Monthly energy of June";
  EXPECT_NEAR((double)monthly_energy[6], 665.698, error_tolerance)
      << "Monthly energy of July";
  EXPECT_NEAR((double)monthly_energy[7], 647.621, error_tolerance)
      << "Monthly energy of August";
  EXPECT_NEAR((double)monthly_energy[8], 594.314, error_tolerance)
      << "Monthly energy of September";
  EXPECT_NEAR((double)monthly_energy[9], 568.281, error_tolerance)
      << "Monthly energy of October";
  EXPECT_NEAR((double)monthly_energy[10], 453.305, error_tolerance)
      << "Monthly energy of November";
  EXPECT_NEAR((double)monthly_energy[11], 412.782, error_tolerance)
...
TEST_F(CMPvwattsV5Integration_cmod_pvwattsv5, DifferentTechnologyInputs) {
  //	std::vector<double> annual_energy_expected = { 6909.79, 7123.32,
  //7336.478, 6909.79, 6804.376, 8711.946, 8727.704, 9690.735 };
  // single axis tracking reduction due to pull request 280
  std::vector<double> annual_energy_expected = {
      6908.02, 7121.52, 7334.71, 6908.02, 6802.62, 8584.29, 8721.18, 9687.18};
  ...
  for (int module_type = 0; module_type < 3; module_type++) {
    ...
    int pvwatts_errors =
        modify_ssc_data_and_run_module(data, "pvwattsv5", pairs);
...
TEST_F(CMPvwattsV5Integration_cmod_pvwattsv5, LargeSystem_cmod_pvwattsv5) {
  std::vector<double> annual_energy_expected = {1727006, 1700656, 2146072,
                                                2180297, 2421795};
  std::map<std::string, double> pairs;
  size_t count = 0;
  ...
  for (int array_type = 0; array_type < 5; array_type++) {
    ...
    if (!pvwatts_errors) {
      ssc_number_t annual_energy;
      ssc_data_get_number(data, "annual_energy", &annual_energy);
      EXPECT_NEAR(annual_energy, annual_energy_expected[count], 1)
          << "Annual energy.";
    }
    count++;
  }
