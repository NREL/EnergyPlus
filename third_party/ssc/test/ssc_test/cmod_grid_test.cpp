...
TEST_F(CMGrid_cmod_grid, SingleYearHourly_cmod_grid) {
  ...
  EXPECT_FALSE(errors);
  if (!errors) {
    ssc_number_t capacity_factor, annual_energy_pre_interconnect,
        annual_energy_interconnect;
    int gen_size;
    ssc_data_get_number(data, "capacity_factor_interconnect_ac",
                        &capacity_factor);
    ssc_data_get_number(data, "annual_energy_pre_interconnect_ac",
    ...
    ssc_number_t *system_pre_interconnect_kwac =
        GetArray("system_pre_interconnect_kwac", gen_size);
...
TEST_F(CMGrid_cmod_grid, SingleYearSubHourly_cmod_grid) {
  ...
  EXPECT_FALSE(errors);
  if (!errors) {
    ssc_number_t capacity_factor, annual_energy_pre_interconnect, annual_energy,
    ...
    ssc_data_get_number(data, "annual_energy_pre_curtailment_ac",
                        &annual_energy_pre_curtailment_ac);
    ssc_number_t *system_kwac = GetArray("gen", gen_size);
    ssc_number_t *system_pre_interconnect_kwac =
        GetArray("system_pre_interconnect_kwac", gen_size);
