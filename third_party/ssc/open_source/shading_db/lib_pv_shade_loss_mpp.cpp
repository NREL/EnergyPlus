...
bool ShadeDB8_mpp::get_index(const size_t &N, const size_t &d, const size_t &t,
                             const size_t &S, const db_type &DB_TYPE,
                             size_t *ret_ndx) {
  bool ret_val = false;
  // size_t ret_ndx=-1;
...
std::vector<double> ShadeDB8_mpp::get_vector(const size_t &N, const size_t &d,
                                             const size_t &t, const size_t &S,
  ...
  if (get_index(N, d, t, S, DB_TYPE, &ndx)) {
    for (size_t i = 0; i < length; i++) { // could replace with single get!
      if (DB_TYPE == VMPP)
...
void ShadeDB8_mpp::init() {
  ...
  p_vmpp = (unsigned char *)malloc(
      p_vmpp_uint8_size); // malloc(12091680); uint8 size
  p_impp = (unsigned char *)malloc(
      p_impp_uint8_size);      // malloc(12091680); uint8 size
...
bool ShadeDB8_mpp::decompress_file_to_uint8() {
  ...
  status = tinfl_decompress_mem_to_mem((void *)pTmp_data, mem_size, pCmp_data,
                                       p_compressed_size,
...
double ShadeDB8_mpp::get_shade_loss(double &gpoa, double &dpoa,
                                    std::vector<double> &shade_frac,
                                    bool use_pv_cell_temp, double pv_cell_temp,
                                    int mods_per_str, double str_vmp_stc,
                                    double mppt_lo, double mppt_hi) {
  double shade_loss = 0;
  ...
  if (num_strings > 0) {
    ...
    if ((s_sum > 0) && (gpoa > 0)) {
      ...
      std::vector<double> vmpp = get_vector(num_strings, diffuse_frac, s_max,
                                            counter, ShadeDB8_mpp::VMPP);
      std::vector<double> impp = get_vector(num_strings, diffuse_frac, s_max,
                                            counter, ShadeDB8_mpp::IMPP);
      double p_max_frac = 0;
      ...
      if (use_pv_cell_temp) {
        ...
        */
        double n = 1.263;
        double BetaVmp =
            -0.137 * mods_per_str; // mult by ModsPerString because it's in V
        ...
        double TcVmpMax = vmpp[p_max_ind] * VMaxSTCStrUnshaded +
        ...
        */
        ...
        for (size_t i = 0; i < TcVmps.size() && i < pmp_fracs.size(); i++) {
          outm << i << "," << vmpp[i] << "," << impp[i] << "," << pmp_fracs[i]
               << "," << TcVmps[i] << "\n";
        }
...
#ifdef SHADE_DB_DEBUG
      std::stringstream outm;
      outm << "\nglobal = " << gpoa << " and shade fraction = " << s_sum
           << " and shade loss = " << shade_loss << "\n";
      p_warning_msg = outm.str();
#endif
    }
  }
