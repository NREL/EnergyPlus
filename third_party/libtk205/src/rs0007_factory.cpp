...
std::shared_ptr<RSInstanceBase> RS0007Factory::create_instance(const char* RS_instance_file, std::shared_ptr<Courierr::Courierr> logger) const
{
    ...
    else if (j["metadata"]["schema"] == "RS0007")
    {
        ...
        rs0007_ns::RS0007::logger = logger;
        p_rs->initialize(j);
    }
    else
    {
