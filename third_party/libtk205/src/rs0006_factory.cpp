...
std::shared_ptr<RSInstanceBase> RS0006Factory::create_instance(const char* RS_instance_file, std::shared_ptr<Courierr::Courierr> logger) const
{
    ...
    else if (j["metadata"]["schema"] == "RS0006")
    {
        ...
        rs0006_ns::RS0006::logger = logger;
        p_rs->initialize(j);
    }
    else
    {
