...
namespace EnergyPlus {
	...
	TEST_F( HVACFixture, Beam_calcOneZone ) 
	{
			std::string const idf_objects = delimited_string( { 
		"    SimulationControl,",
	...
	//	DataZoneEquipment::ZoneEquipConfig( 1 ).InletNode( 1 ) = 3;
		bool ErrorsFound =  false;
	//	DataZoneEquipment::ZoneEquipConfig( 1 ).ZoneNode = NodeInputManager::GetOnlySingleNode( "Zone One Node", ErrorsFound, "Zone", "BeamTest", DataLoopNode::NodeType_Air, DataLoopNode::NodeConnectionType_ZoneNode, 1, DataLoopNode::ObjectIsNotParent, "Test zone node" );

	//	DataDefineEquip::NumAirDistUnits = 1;
	//	DataDefineEquip::AirDistUnit.allocate( 1 );
	//	DataDefineEquip::AirDistUnit( 1 ).EquipName( 1 ) = "ZONE ONE 4PIPE BEAM"; // needs to be uppercased, or item will not be found at line 2488 in IP
	//	DataDefineEquip::AirDistUnit( 1 ).OutletNodeNum = 3;
	...
	//	DataDefineEquip::AirDistUnit( 1 ).airTerminalPtr = FourPipeBeam::HVACFourPipeBeam::fourPipeBeamFactory( DataDefineEquip::SingleDuctConstVolFourPipeBeam, DataDefineEquip::AirDistUnit( 1 ).EquipName( 1 ) );
		...
		EXPECT_FLOAT_EQ( DataLoopNode::Node( 15 ).Temp, 19.251915 );
		EXPECT_FLOAT_EQ( DataLoopNode::Node( 15 ).MassFlowRate, 0.047231779 ); 
		...
		EXPECT_FLOAT_EQ( DataLoopNode::Node( 39 ).Temp, 34.960255);
		EXPECT_FLOAT_EQ( DataLoopNode::Node( 39 ).MassFlowRate, 0.19061226);
