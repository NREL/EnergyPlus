...
TEST_F( HVACFixture, VRFTest ) {
		...
		std::string const idf_objects = delimited_string( {
			...
			"  TU1 VRF DX Cool Coil,  !- Cooling Coil Object Name",
			"  COIL:Heating:DX:VariableRefrigerantFlow,  !- Heating Coil Object Type",
			"  TU1 VRF DX Heat Coil,  !- Heating Coil Object Name",
		...
		EXPECT_EQ( 0.0, VRF( VRFCond ).WaterCondVolFlowRate );
		EXPECT_EQ( 0.0, VRF( VRFCond ).WaterCondenserDesignMassFlow );
	...
	TEST_F( HVACFixture, VRFTest_WaterCooled ) {
		...
		Node( VRF( VRFCond ).CondenserNodeNum ).Temp = 7.0; // water inlet temperature
