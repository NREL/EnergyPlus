...
- Don’t worry compliance parameters for now, just ignore them. In future maybe merge a JSON file that contains them.
...
A new Python utility will be created separate from EnergyPlus that can eventually be packaged 
with the EnergyPlus installer. It will be developed in its own repository but eventually 
this may be merged or linked from the EnergyPlus repository.
