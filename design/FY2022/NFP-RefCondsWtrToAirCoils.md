...
- `PeakTotCapTempModFac` would be the capacity modifier at peak design conditions. Right now, the peak design conditions corresponds to the mixed air wet-bulb temperature and an entering water temperature of 85 deg. F. We propose to keep the former but to change the latter to be based on the user-specified reference entering water temperature (default to 30 deg. C, 85 deg. F). 
...
Coil:Cooling:WaterToAirHeatPump:EquationFit,
   N*,  \field Reference Entering Water Temperature
        ...
        \note applications, the rated temperature is 30 deg. C.
        \units C
        \type real
        \minimum> 0
        \default 30
   N*,  \field Reference Entering Air Wet-Bulb Temperature 
        ...
        \note water loop applications, the rated temperature is 19 deg. C.
        \units C
        \type real
        \minimum> 0
        \default 19
...
Coil:Heating:WaterToAirHeatPump:EquationFit,
   N*,  \field Reference or Rated Entering Water Temperature
        \note Reference or rated entering water temperature corresponding to the water-to
        \note -air application for which this coil is used. For example: for water loop 
        \note applications, the rated temperature is 20 deg. C.
        \units C
        \type real
        \minimum> 0
        \default 20
   N*,  \field Reference or Rated Entering Air Wet-Bulb Temperature 
        \note Reference or rated entering air wet-bulb temperature corresponding to the
        \note water-to-air application for which this coil is used. For example: for
        \note water loop applications, the rated temperature is 15 deg. C.
        \units C
        \type real
        \minimum> 0
        \default 15
...
This new feature proposal doesn't include any new outputs.
...
> 
> \begin{equation}
>   T_{WB,ratio,ref} = \frac{T_{WB,air,in,~~des~~<ins>ref</ins>}+273.15C}{283.15C}
> \end{equation}
> 
> \begin{equation}
>   T_{S,ratio<ins>,ref</ins>} = \frac{<ins>T_{water,in,ref}<\ins>+273.15C}{283.15C}
> \end{equation}
>
> \begin{equation}
>   <ins>T_{WB,ratio,peak} = \frac{T_{WB,air,in,peak}+273.15C}{283.15C}</ins>
> \end{equation}
> 
> \begin{equation}
>   <ins>T_{S,ratio,peak} = \frac{T_{water,in,peak}<\ins>+273.15C}{283.15C}</ins>
> \end{equation}
> 
> where:
> 
> ~~$T_{WB,ratio} = $ ratio of <ins>reference</ins> load-side inlet air wet-bulb temperature in Kelvin to a reference temperature~~
> 
> ~~$T_{S,ratio} = $ ratio of <ins>reference</ins> source-side inlet water temperature in Kelvin to a reference temperature~~
>
><ins>
> $T_{WB,ratio,peak} = $ ratio of load-side inlet air wet-bulb temperature at peak design conditions in Kelvin to a reference temperature
> 
> $T_{S,ratio,peak} = $ ratio of peak source-side inlet water temperature at peak design conditions in Kelvin to a reference temperature
></ins>
>
> <ins>$T_{WB,air,in,ref} = $  ratio of reference load-side inlet air wet-bulb temperature air wet-bulb temperature, if not specified by the user, 19 deg. C (66.2 deg. F) is used which corresponds to the rating conditions of a water loop water-source heat pump according to ISO-13256-1</ins>
>
> <ins>$T_{WB,air,in,ref} = $ ratio of reference source-side inlet water temperature, if not specified by the user, 30 deg. C (86 deg. F) is used which corresponds to the rating conditions of a water loop water-source heat pump according to ISO-13256-1</ins>
>
> \begin{equation}
> TotCapTempModFac<ins>Ref</ins> = \,TCC1 + TCC2\left( {{T_{WB,ratio<ins>,ref</ins>}}} \right) + TCC3\left( {{T_{S,ratio<ins>,ref</ins>}}} \right) + TCC4 + TCC5
> TotCapTempModFac<ins>Peak</ins> = \,TCC1 + TCC2\left( {{T_{WB,ratio<ins>,peak</ins>}}} \right) + TCC3\left( {{T_{S,ratio<ins>,peak</ins>}}} \right) + TCC4 + TCC5
> \end{equation}
>
> [...]
>
>\begin{equation}
>   \dot{Q}_{coil,des,total}   = \frac{\dot{m}_{air,des}\PB{H_{in}-H_{out}}<ins>TotCapTempModFacRef</ins>}{TotCapTempModFac<ins>Peak</ins>} + \dot{Q}_{fan,heat,des}
>\end{equation}

## Example File and Transition Changes ##

Two of the current example files will be modified to illustrate the proposed changes: one for a GSHP application and one for a WSHP application.
