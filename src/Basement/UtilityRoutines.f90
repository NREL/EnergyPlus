SUBROUTINE AbortEnergyPlus
  ...
  CALL ShowMessage('GroundTempCalc:Basement Terminated--Fatal Error Detected. '//TRIM(NumWarnings)//' Warning; '//  &
                           TRIM(NumSevere)//' Severe Errors')
  tempfl=GetNewUnitNumber()
  open(tempfl,file='eplusout.end')
  write(tempfl,'(A)') 'GroundTempCalc:Basement Terminated--Fatal Error Detected. '//TRIM(NumWarnings)//' Warning; '//  &
                           TRIM(NumSevere)//' Severe Errors'
  close(tempfl)
...
SUBROUTINE EndEnergyPlus
  ...
  CALL ShowMessage('GroundTempCalc:Basement Completed Successfully-- '//TRIM(NumWarnings)//' Warning; '//TRIM(NumSevere)//  &
                      ' Severe Errors;'//' Elapsed Time='//TRIM(Elapsed))
  tempfl=GetNewUnitNumber()
  open(tempfl,file='eplusout.end')
  write(tempfl,'(A)') 'GroundTempCalc:Basement Completed Successfully-- '//TRIM(NumWarnings)//' Warning; '//TRIM(NumSevere)//  &
                        ' Severe Errors'
  close(tempfl)
  CALL CloseMiscOpenFiles
...
SUBROUTINE ShowFatalError(ErrorMessage,OutUnit1,OutUnit2)
  ...
  IF (PRESENT(OutUnit1)) THEN
    IF (OutUnit1 == EPObjects) THEN
      EPOWritten=.true.
      CALL writePreprocessorObject(EPObjects,ProgramName,'Fatal',ErrorMessage)
!      WRITE(EPObjects,'(A)') 'Output:PreprocessorMessage,'//ProgramName//',Fatal ,'
!      IF (len_trim(ErrorMessage) > 100) THEN
!        WRITE(EPObjects,'(A)')   trim(ErrorMessage(1:100))//';'
!      ELSE
!        WRITE(EPObjects,'(A)')   trim(ErrorMessage)//';'
!      ENDIF
    ENDIF
  ENDIF
  IF (PRESENT(OutUnit2)) THEN
    IF (OutUnit2 == EPObjects) THEN
      EPOWritten=.true.
      CALL writePreprocessorObject(EPObjects,ProgramName,'Fatal',ErrorMessage)
!      WRITE(EPObjects,'(A)') 'Output:PreprocessorMessage,'//ProgramName//',Fatal ,'
!      IF (len_trim(ErrorMessage) > 100) THEN
...
SUBROUTINE ShowSevereError(ErrorMessage,OutUnit1,OutUnit2)
  ...
  IF (PRESENT(OutUnit1)) THEN
    IF (OutUnit1 == EPObjects) THEN
      ...
      CALL writePreprocessorObject(EPObjects,ProgramName,'Severe',ErrorMessage)
!      WRITE(EPObjects,'(A)') 'Output:PreprocessorMessage,'//ProgramName//',Severe,'
!      IF (len_trim(ErrorMessage) > 100) THEN
!        WRITE(EPObjects,'(A)')   trim(ErrorMessage(1:100))//';'
!      ELSE
!        WRITE(EPObjects,'(A)')   trim(ErrorMessage)//';'
...
SUBROUTINE ShowWarningError(ErrorMessage,OutUnit1,OutUnit2)
  ...
  IF (PRESENT(OutUnit1)) THEN
    IF (OutUnit1 == EPObjects) THEN
      ...
      CALL writePreprocessorObject(EPObjects,ProgramName,'Warning',ErrorMessage)
!      WRITE(EPObjects,'(A)') 'Output:PreprocessorMessage,'//ProgramName//',Warning,'
!      IF (len_trim(ErrorMessage) > 100) THEN
!        WRITE(EPObjects,'(A)')   trim(ErrorMessage(1:100))//';'
!      ELSE
!        WRITE(EPObjects,'(A)')   trim(ErrorMessage)//';'
