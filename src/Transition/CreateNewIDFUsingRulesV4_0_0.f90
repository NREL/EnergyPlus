...
SUBROUTINE SetThisVersionVariables()
      VerString='Conversion 3.1 => 4.0'
      VersionNum=4.0
      IDDFileNameWithPath=TRIM(ProgramPath)//'V3-1-0-Energy+.idd'
      NewIDDFileNameWithPath=TRIM(ProgramPath)//'V4-0-0-Energy+.idd'
      RepVarFileNameWithPath=TRIM(ProgramPath)//'Report Variables 3-1-0-027 to 4-0-0.csv'
END SUBROUTINE
...
!
     ...
     ! deleted objects.  no transition.
            ...
            IF (.not. MakingPretty) THEN
              ...
              CASE ('VERSION')
                IF (InArgs(1)(1:3) == '4.0' .and. ArgFile) THEN
...
!    !!!    Changes for this version
              ...
              CASE('COIL:COOLING:DX:SINGLESPEED')
                ...
                OutArgs(14:CurArgs-1)=InArgs(15:CurArgs)
                CurArgs=CurArgs-1
              ...
              CASE('COILPERFORMANCE:DX:COOLING')
                ...
                OutArgs(12:CurArgs-1)=InArgs(13:CurArgs)
                CurArgs=CurArgs-1
              ...
              CASE('COIL:COOLING:WATERTOAIRHEATPUMP:EQUATIONFIT')
                ...
                ! calculate COP
                ErrFlag=.false.
                Fielda=ProcessNumber(InArgs(8),ErrFlag)
                IF (ErrFlag) THEN
                  CALL ShowSevereError('Invalid Number, Coil:Cooling:WaterToAirHeatPump:EquationFit field 8 ['//   &
                     trim(InArgs(8))//'], Name='//TRIM(OutArgs(1)),Auditf)
                  WRITE(DifLfn,fmta) '  ! Invalid Number, field 8 {'//TRIM(FldNames(8))//'} value='//  &
                     TRIM(InArgs(8))
                ENDIF
                Fieldb=ProcessNumber(InArgs(10),ErrFlag)
                IF (ErrFlag) THEN
                  CALL ShowSevereError('Invalid Number, Coil:Cooling:WaterToAirHeatPump:EquationFit field 10 ['//  &
                     trim(InArgs(10))//'], Name='//TRIM(OutArgs(1)),Auditf)
                  WRITE(DifLfn,fmta) '  ! Invalid Number, field 10 {'//TRIM(FldNames(10))//'} value='//  &
                     TRIM(InArgs(10))
                ENDIF
                OutFieldc=SafeDivide(Fielda,Fieldb)
                OutArgs(10)=RoundSigDigits(OutFieldc,4)
                OutArgs(11:CurArgs)=InArgs(11:CurArgs)
              ...
              CASE('COIL:HEATING:WATERTOAIRHEATPUMP:EQUATIONFIT')
                ...
                IF (ErrFlag) THEN
                  ...
                  WRITE(DifLfn,fmta) '  ! Invalid Number, field 8 {'//TRIM(FldNames(8))//'} value='//  &
                     TRIM(InArgs(8))
                ENDIF
                Fieldb=ProcessNumber(InArgs(9),ErrFlag)
                IF (ErrFlag) THEN
                  CALL ShowSevereError('Invalid Number, Coil:Cooling:WaterToAirHeatPump:EquationFit field 9 ['//  &
                     trim(InArgs(9))//'], Name='//TRIM(OutArgs(1)),Auditf)
                  WRITE(DifLfn,fmta) '  ! Invalid Number, field 9 {'//TRIM(FldNames(9))//'} value='//  &
                     TRIM(InArgs(9))
                ENDIF
                OutFieldc=SafeDivide(Fielda,Fieldb)
                OutArgs(9)=RoundSigDigits(OutFieldc,4)
              ...
              CASE('REFRIGERATION:COMPRESSOR')
                ...
                IF (SameString(InArgs(4),'RatedSuperHeat')) THEN
                  OutArgs(4)=InArgs(5)
                  OutArgs(5)=Blank
                ELSEIF (SameString(InArgs(4),'RatedReturnGasTemperature')) THEN
                  OutArgs(4)=Blank
                  OutArgs(5)=InArgs(5)
                ELSE
                  CALL ShowSevereError('Invalid phrase, Refrigeration:Compressor field 4, Name='//TRIM(OutArgs(1)),Auditf)
                  WRITE(DifLfn,fmta) '  ! Invalid phrase, field 4 {'//TRIM(FldNames(4))//'} value='//  &
                     TRIM(InArgs(4))
                ENDIF
                IF (SameString(InArgs(6),'RatedSubcooling')) THEN
                  OutArgs(6)=Blank
                  OutArgs(7)=InArgs(7)
                ELSEIF (SameString(InArgs(6),'RatedLiquidTemperature')) THEN
                  OutArgs(6)=InArgs(7)
                  OutArgs(7)=Blank
                ELSE
                  CALL ShowSevereError('Invalid phrase, Refrigeration:Compressor field 6, Name='//TRIM(OutArgs(1)),Auditf)
                  WRITE(DifLfn,fmta) '  ! Invalid phrase, field 6 {'//TRIM(FldNames(6))//'} value='//  &
                     TRIM(InArgs(6))
                ENDIF
                OutArgs(8)=InArgs(8)
              ...
              CASE('REFRIGERATION:CASELIST')
                ObjectName='Refrigeration:CaseAndWalkInList'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                nodiff=.false.
              ...
              CASE('SCHEDULETYPELIMITS')
                ...
                ELSE
                  pos=INDEX(InArgs(2),':')
                  IF (pos > 1) THEN
                    OutArgs(2)=InArgs(2)(1:pos-1)
                    OutArgs(3)=adjustl(InArgs(2)(pos+1:))
                  ELSE
                    CALL ShowSevereError('Invalid range, ScheduleTypeLimits field 2, Name='//TRIM(OutArgs(1)),Auditf)
                    WRITE(DifLfn,fmta) '  ! Invalid range, field 2 {'//TRIM(FldNames(2))//'} value='//  &
                       TRIM(InArgs(2))
                    OutArgs(2:3)=Blank
                  ENDIF
                ENDIF
                ...
                SELECT CASE (MakeUPPERCase(InArgs(1)))
                  ...
                  CASE ('SURFACES')
                    ...
                    SELECT CASE(MakeUPPERCase(InArgs(2)))
                      ...
                      CASE DEFAULT
                        CALL ShowSevereError('Output:Reports object cannot transition.  See resultant IDF.',Auditf)
