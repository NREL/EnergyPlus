!SUBROUTINE CreateNewIDFUsingRulesV1_0_2(EndOfFile,DiffOnly,InLfn,AskForInput,InputFileName,ArgFile,ArgIDFExtension)
  ...
  LOGICAL :: WildMatch
  CHARACTER(len=MaxNameLength), ALLOCATABLE, DIMENSION(:) :: COMISFacadeNames
  INTEGER NumCOMISFacades
  INTEGER NIndex
  INTEGER :: Var
  INTEGER :: CurVar
    ...
    DO WHILE (.not. EndOfFile)
      ...
      IF (FullFileName /= Blank) THEN
        ...
        IF (LocalFileExtension == 'idf' .or. LocalFileExtension == 'imf') THEN
          ...
          NumCOMISFacades=GetNumObjectsFound('COMIS EXTERNAL NODE')
          ALLOCATE(COMISFacadeNames(NumCOMISFacades))
          COMISFacadeNames=Blank
          DO Num=1,NumCOMISFacades
            CALL GetObjectItem('COMIS EXTERNAL NODE',Num,Alphas,NumAlphas,Numbers,NumNumbers,Status)
            COMISFacadeNames(Num)=Alphas(1)
          ENDDO
          ...
          DO Num=1,NumIDFRecords
            ...
            IF (.not. MakingPretty) THEN
              ...
              SELECT CASE (MakeUPPERCase(TRIM(IDFRecords(Num)%Name)))
                ...
                CASE ('VERSION')
                  IF (InArgs(1)(1:5) == '1.0.2' .and. ArgFile) THEN
                ...
                CASE ('COMIS SURFACE DATA')
                  ...
                  OutArgs(4)=InArgs(4)
                  READ(InArgs(3),*) NIndex
                  IF (NIndex == 0) THEN
                    OutArgs(3)=Blank
                  ELSE
                    OutArgs(3)=COMISFacadeNames(NIndex)
                  ENDIF
                  Nodiff=.false.
  ...
  !!!   Changes for report variables, meters, tables -- update new names
      ...
      IF (ALLOCATED(COMISFacadeNames)) DEALLOCATE(COMISFacadeNames)
