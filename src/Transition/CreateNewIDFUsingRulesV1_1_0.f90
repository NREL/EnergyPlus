...
!                CYCLE
          ...
          DO Num=1,NumIDFRecords
            ...
            IF (.not. MakingPretty) THEN
              ...
              SELECT CASE (MakeUPPERCase(TRIM(IDFRecords(Num)%Name)))
                ...
                CASE ('VERSION')
                  IF (InArgs(1)(1:3) == '1.1' .and. ArgFile) THEN
        ...
        !!!    Changes for this version
                ...
                CASE ('SKY RADIANCE DISTRIBUTION')
                  IF (DiffOnly) THEN
                    CALL ShowWarningError('Sky Radiance Distribution Object is Obsolete -- is deleted from new file.',DifLfn,Auditf)
                  ENDIF
                  Written=.true.
                  !CYCLE

                CASE('SURFACE:SHADING:DETACHED')
                  ObjectName='SURFACE:SHADING:DETACHED:FIXED'
                  CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  ...
                  DO Arg=3,CurArgs,5
                    SELECT CASE (MakeUPPERCase(OutArgs(Arg)))
                      CASE('CHILLER:GAS TURBINE')
                        OutArgs(Arg)='CHILLER:COMBUSTION TURBINE'
                        nodiff=.false.
                      CASE('CHILLER:GAS ABSORPTION')
                        OutArgs(Arg)='CHILLER:DIRECT FIRED ABSORPTION'
                        nodiff=.false.
                      CASE DEFAULT
                    END SELECT
                  ENDDO
                  ...
                  DO Arg=3,CurArgs,4
                    SELECT CASE (MakeUPPERCase(OutArgs(Arg)))
                      CASE('GENERATOR:DIESEL')
                        OutArgs(Arg)='GENERATOR:IC ENGINE'
                        nodiff=.false.
                      CASE('GENERATOR:GAS TURBINE')
                        OutArgs(Arg)='GENERATOR:COMBUSTION TURBINE'
                        nodiff=.false.
                      CASE DEFAULT
                    END SELECT
                  ENDDO
                ...
                CASE ('CHILLER:GAS TURBINE')
                  ObjectName='CHILLER:COMBUSTION TURBINE'
                  CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  IF (CurArgs < 54) THEN
                    OutArgs(CurArgs+1:54)=NwFldDefaults(CurArgs+1:54)
                  ENDIF
                  OutArgs(55)=NwFldDefaults(55)
                  CurArgs=55
                  nodiff=.false.

                CASE ('CHILLER:GAS ABSORPTION')
                  ObjectName='CHILLER:DIRECT FIRED ABSORPTION'
                  CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  IF (CurArgs < 33) THEN
                    OutArgs(CurArgs+1:33)=NwFldDefaults(CurArgs+1:33)
                  ENDIF
                  OutArgs(34)=NwFldDefaults(34)
                  CurArgs=34
                  nodiff=.false.

                CASE ('GENERATOR:DIESEL')
                  ObjectName='GENERATOR:IC ENGINE'
                  CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  IF (CurArgs < 19) THEN
                    OutArgs(CurArgs+1:19)=NwFldDefaults(CurArgs+1:19)
                  ENDIF
                  OutArgs(20)=NwFldDefaults(20)
                  CurArgs=20
                  nodiff=.false.

                CASE ('GENERATOR:GAS TURBINE')
                  ObjectName='GENERATOR:COMBUSTION TURBINE'
                  CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  IF (CurArgs < 21) THEN
                    OutArgs(CurArgs+1:21)=NwFldDefaults(CurArgs+1:21)
                  ENDIF
                  OutArgs(22)=NwFldDefaults(22)
                  CurArgs=22
                  nodiff=.false.
