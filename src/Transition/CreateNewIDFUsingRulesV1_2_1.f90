    ...
    DO WHILE (.not. EndOfFile)
          ...
          DO Num=1,NumIDFRecords
            ...
            IF (.not. MakingPretty) THEN
              ...
              SELECT CASE (MakeUPPERCase(TRIM(IDFRecords(Num)%Name)))
                ...
                CASE ('VERSION')
                  IF (InArgs(1)(1:5) == '1.2.1' .and. ArgFile) THEN
      ...
      !!!    Changes for this version
                ...
                CASE ('DESICCANT DEHUMIDIFIER:SOLID')
                  ...
                  IF (MakeUPPERCase(OutArgs(14)) == 'FAN:SIMPLE:CONSTVOLUME' .and. MakeUPPERCase(OutArgs(16)) == 'DEFAULT') THEN
                    WRITE(DifLfn,FmtA) ' ! Following Object (field 14) needs to be "Fan:Simple:VariableVolume" with '// &
                                    'coefficients (0,1,0,0,0) or changed'
                    CALL ShowWarningError('Desiccant Dehumidifier:Solid '//TRIM(OutArgs(1))//' needs change.  '// &
                                          'See resultant input file for explanation.',Auditf)
                  ENDIF
                  nodiff=.false.
                ...
                CASE ('GROUND HEAT EXCHANGER:POND')
                  CALL GetNewObjectDefInIDD(ObjectName,  &
                                            NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  IF (MakeUPPERCase(OutArgs(4)) /= 'WATER') THEN
                    WRITE(DifLfn,FmtA) ' Following Object may need manual manipulation to work'
                    CALL ShowWarningError('Ground Heat Exchanger:Pond="'//TRIM(OutArgs(1))//'" may need manual change to work.',Auditf)
                    nodiff=.false.
                  ENDIF
                ...
                CASE ('GROUND HEAT EXCHANGER:SURFACE')
                  CALL GetNewObjectDefInIDD(ObjectName,  &
                                            NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  IF (MakeUPPERCase(OutArgs(5)) /= 'WATER') THEN
                    WRITE(DifLfn,FmtA) ' Following Object may need manual manipulation to work'
                    CALL ShowWarningError('Ground Heat Exchanger:Surface="'//TRIM(OutArgs(1))//'" may need manual change to work.',Auditf)
                    nodiff=.false.
                  ENDIF
                ...
                CASE ('HEAT EXCHANGER:HYDRONIC:FREE COOLING')
                  CALL GetNewObjectDefInIDD(ObjectName,  &
                                            NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  IF (MakeUPPERCase(OutArgs(9)) /= 'WATER' .or. MakeUPPERCase(OutArgs(10)) /= 'WATER') THEN
                    WRITE(DifLfn,FmtA) ' Following Object may need manual manipulation to work'
                    CALL ShowWarningError('Heat Exchanger:Hydronic:Free Cooling="'//TRIM(OutArgs(1))//'" may need manual change to work.',Auditf)
                    nodiff=.false.
                  ENDIF
                ...
                CASE ('HEAT EXCHANGER:PLATE:FREE COOLING')
                  CALL GetNewObjectDefInIDD(ObjectName,  &
                                            NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  IF (MakeUPPERCase(OutArgs(8)) /= 'WATER' .or. MakeUPPERCase(OutArgs(9)) /= 'WATER') THEN
                    WRITE(DifLfn,FmtA) ' Following Object may need manual manipulation to work'
                    CALL ShowWarningError('Heat Exchanger:Plate:Free Cooling="'//TRIM(OutArgs(1))//'" may need manual change to work.',Auditf)
                    nodiff=.false.
                  ENDIF
              ...
              CASE('SET POINT MANAGER:SINGLE ZONE MIN HUM', 'SET POINT MANAGER:SINGLE ZONE MAX HUM')
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                nodiff=.false.
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                OutArgs(2:3)=blank
