...
SUBROUTINE SetThisVersionVariables()
      VerString='Conversion 7.1 => 7.2'
      VersionNum=7.2
      IDDFileNameWithPath=TRIM(ProgramPath)//'V7-1-0-Energy+.idd'
      NewIDDFileNameWithPath=TRIM(ProgramPath)//'V7-2-0-Energy+.idd'
      RepVarFileNameWithPath=TRIM(ProgramPath)//'Report Variables 7-1-0-012 to 7-2-0.csv'
END SUBROUTINE
    ...
    DO WHILE (.not. EndOfFile)
     ...
     ! deleted objects.  no transition.
            ...
            IF (.not. MakingPretty) THEN
              ...
              CASE ('VERSION')
                IF ((InArgs(1)(1:3) == '7.2').and. ArgFile) THEN
...
!    !!!    Changes for this version
              ...
              CASE('EXTERNALINTERFACE')
                ...
                IF (SameString(OutArgs(1),'FunctionalMockupUnit')) OutArgs(1)='FunctionalMockupUnitImport'

              CASE('EXTERNALINTERFACE:FUNCTIONALMOCKUPUNIT')
                nodiff=.false.
                ObjectName='ExternalInterface:FunctionalMockupUnitImport'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)

              CASE('EXTERNALINTERFACE:FUNCTIONALMOCKUPUNIT:FROM:VARIABLE')
                nodiff=.false.
                ObjectName='ExternalInterface:FunctionalMockupUnitImport:From:Variable'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                nodiff=.true.
              ...
              CASE('EXTERNALINTERFACE:FUNCTIONALMOCKUPUNIT:TO:SCHEDULE')
                nodiff=.false.
                ObjectName='ExternalInterface:FunctionalMockupUnitImport:To:Schedule'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)

              CASE('EXTERNALINTERFACE:FUNCTIONALMOCKUPUNIT:TO:ACTUATOR')
                nodiff=.false.
                ObjectName='ExternalInterface:FunctionalMockupUnitImport:To:Actuator'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)

              CASE('EXTERNALINTERFACE:FUNCTIONALMOCKUPUNIT:TO:VARIABLE')
                nodiff=.false.
                ObjectName='ExternalInterface:FunctionalMockupUnitImport:To:Variable'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                nodiff=.true.
                IF (OutArgs(1) == Blank) THEN
              ...
              CASE('WINDOWMATERIAL:GAS')
                ...
                IF (CurArgs>6) THEN
                  CurArgs=CurArgs+1
                  OutArgs(7:8)=InArgs(6:7)
                  OutArgs(9)='0.0'
                ENDIF
                IF (CurArgs>9) THEN
                  CurArgs=CurArgs+1
                  OutArgs(10:11)=InArgs(8:9)
                  OutArgs(12)='0.0'
                ENDIF
                IF (CurArgs >= 13) OutArgs(13)=InArgs(10)
                IF (CurArgs >= 14) OutArgs(14)=InArgs(11)
              ...
              CASE('COIL:COOLING:DX:MULTISPEED')
                ...
                OutArgs(22)=blank
                OutArgs(23:40)=InArgs(22:39)
                OutArgs(41)=blank
                OutArgs(42:55)=InArgs(40:53)
                If (CurArgs > 53) THEN
                  OutArgs(56:59)=InArgs(54:57)
                  OutArgs(60)=blank
                  OutArgs(61:74)=InArgs(58:71)
                ENDIF
                IF (CurArgs > 71) THEN
                  OutArgs(75:78)=InArgs(72:75)
                  OutArgs(79)=blank
                  OutArgs(80:93)=InArgs(76:89)
                  CurArgs=CurArgs+4
                ELSE
                  IF (CurArgs > 53) THEN
                    CurArgs=CurArgs+3
              ...
              CASE('SIZING:ZONE')
                nodiff=.false.
                ! minfields goes from 18 to 22
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1)=InArgs(1)
                OutArgs(2)='SupplyAirTemperature'
                OutArgs(3)=InArgs(2)
                OutArgs(4)=blank
                OutArgs(5)='SupplyAirTemperature'
                OutArgs(6)=InArgs(3)
                OutArgs(7)=blank
                OutArgs(8:CurArgs+4)=InArgs(4:CurArgs)
                CurArgs=CurArgs+4

              CASE('ZONECONTAMINANTSOURCEANDSINK:GENERICCONTAMINANT:CONSTANT')
                nodiff=.false.
                ObjectName='ZoneContaminantSourceAndSink:Generic:Constant'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)

              CASE('SURFACECONTAMINANTSOURCEANDSINK:GENERICCONTAMINANT:PRESSUREDRIVEN')
                nodiff=.false.
                ObjectName='SurfaceContaminantSourceAndSink:Generic:PressureDriven'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)

              CASE('ZONECONTAMINANTSOURCEANDSINK:GENERICCONTAMINANT:CUTOFFMODEL')
                nodiff=.false.
                ObjectName='ZoneContaminantSourceAndSink:Generic:CutoffModel'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)

              CASE('ZONECONTAMINANTSOURCEANDSINK:GENERICCONTAMINANT:DECAYSOURCE')
                nodiff=.false.
                ObjectName='ZoneContaminantSourceAndSink:Generic:DecaySource'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)

              CASE('SURFACECONTAMINANTSOURCEANDSINK:GENERICCONTAMINANT:BOUDARYLAYERDIFFUSION')
                nodiff=.false.
                ObjectName='SurfaceContaminantSourceAndSink:Generic:BoundaryLayerDiffusion'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)

              CASE('SURFACECONTAMINANTSOURCEANDSINK:GENERICCONTAMINANT:DEPOSITIONVELOCITYSINK')
                nodiff=.false.
                ObjectName='SurfaceContaminantSourceAndSink:Generic:DepositionVelocitySink'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)

              CASE('ZONECONTAMINANTSOURCEANDSINK:GENERICCONTAMINANT:DEPOSITIONRATESINK')
                nodiff=.false.
                ObjectName='ZoneContaminantSourceAndSink:Generic:DepositionRateSink'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)

              CASE('CONSTRUCTIONPROPERTY:USEHBALGORITHMCONDFDDETAILED')
                nodiff=.false.
                ObjectName='SurfaceProperty:HeatTransferAlgorithm:Construction'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1)=blank
                OutArgs(2)='ConductionFiniteDifference'
                OutArgs(3)=InArgs(1)
                CurArgs=3
              ...
              CASE('HEATBALANCEALGORITHM')
                ...
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                IF (SameString(InArgs(1),'ConductionFiniteDifferenceSimplified')) THEN
              ...
              CASE('AIRCONDITIONER:VARIABLEREFRIGERANTFLOW')
                ...
                OutArgs(18)=blank
                IF (CurArgs <= 55) THEN
                  OutArgs(19:CurArgs+1)=InArgs(18:CurArgs)
                  CurArgs=CurArgs+1
                ELSE  ! > 55 args
                  OutArgs(19:56)=InArgs(18:55)
                  OutArgs(57)=InArgs(56)
                  OutArgs(58)=blank
                  OutArgs(59)=blank
                  OutArgs(60:CurArgs+4)=InArgs(57:CurArgs)
                  CurArgs=CurArgs+4
                ENDIF
