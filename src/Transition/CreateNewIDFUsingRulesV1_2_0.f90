!SUBROUTINE CreateNewIDFUsingRulesV1_2_0(EndOfFile,DiffOnly,InLfn,AskForInput,InputFileName,ArgFile,ArgIDFExtension)
  ...
  TYPE CoilData
    ...
    CHARACTER(len=MaxNameLength) :: cType=blank
  END TYPE
  ...
  LOGICAL :: ErrFlag
  INTEGER :: numCoils
  TYPE(CoilData), ALLOCATABLE, DIMENSION(:) :: Coils
    ...
    DO WHILE (.not. EndOfFile)
      ...
      IF (FullFileName /= Blank) THEN
        ...
        IF (LocalFileExtension == 'idf' .or. LocalFileExtension == 'imf') THEN
          ...
          DO Num=1,NumIDFRecords
            IF (MakeUPPERCase(IDFRecords(Num)%Name(1:5)) /= 'COIL:') CYCLE
            numCoils=numCoils+1
          ENDDO

          ALLOCATE(Coils(numCoils))
          numCoils=0
          DO Num=1,NumIDFRecords
            IF (MakeUPPERCase(IDFRecords(Num)%Name(1:5)) /= 'COIL:') CYCLE
            numCoils=numCoils+1
            Coils(numCoils)%cType=IDFRecords(Num)%Name
            Coils(numCoils)%Name=IDFRecords(Num)%Alphas(1)
          ENDDO
          ...
          DO Num=1,NumIDFRecords
            ...
            IF (.not. MakingPretty) THEN
              ...
              SELECT CASE (MakeUPPERCase(TRIM(IDFRecords(Num)%Name)))
                ...
                CASE ('VERSION')
                  IF (InArgs(1)(1:3) == '1.2' .and. ArgFile) THEN
      ...
      !!!    Changes for this version
                ...
                CASE ('ELECTRIC LOAD CENTER:GENERATORS')
                  CALL GetNewObjectDefInIDD('ELECTRIC LOAD CENTER:GENERATORS',  &
                                            NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  nodiff=.false.
                  DO xCount=3,CurArgs,4
                    IF (MakeUPPERCase(OutArgs(xCount)) == 'GENERATOR:PHOTOVOLTAICS') THEN
                      OutArgs(xCount)='GENERATOR:PV:EQUIVALENT ONE-DIODE'
                    ENDIF
                  ENDDO


                CASE ('COIL:DX:COOLINGBYPASSFACTOREMPIRICAL')
                  CALL GetNewObjectDefInIDD(ObjectName,  &
                                            NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  IF (MakeUPPERCase(OutArgs(14)) == 'VARFANVARCOMP' .or. MakeUPPERCase(OutArgs(14)) == 'VARFANUNLOADCOMP') THEN
                    OutArgs(14)='ContFanCycComp'
                    nodiff=.false.
                  ELSE
                    nodiff=.true.
                  ENDIF
                ...
                CASE ('AIR CONDITIONER:WINDOW:CYCLING')
                  ...
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  OutArgs(15)='Unknown'
                  xcount=FindItemInList(OutArgs(11),Coils%Name,numCoils)
                  nodiff=.false.
                  CurArgs=15

                CASE ('GENERATOR:PHOTOVOLTAICS')
                  CALL GetNewObjectDefInIDD('GENERATOR:PV:EQUIVALENT ONE-DIODE',  &
                                            NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  ObjectName='GENERATOR:PV:EQUIVALENT ONE-DIODE'
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  nodiff=.false.

                CASE ('WATERHEATER:SIMPLE')
                  CALL GetNewObjectDefInIDD('WATER HEATER:SIMPLE',  &
                                            NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  ObjectName='WATER HEATER:SIMPLE'
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  nodiff=.false.
                ...
                CASE ('DESICCANT DEHUMIDIFIER:SOLID')
                  ...
                  OutArgs(14)=' unknown '
                  OutArgs(15)=InArgs(18)
                  OutArgs(16)=InArgs(19)
                  IF (MakeUPPERCase(InArgs(19)) /= 'DEFAULT') THEN
                    VArgS=17
                    DO xCount=20,CurArgs
                      OutArgs(VArgS)=InArgs(xCount)
                      VArgS=VArgs+1
                    ENDDO
                    CurArgs=CurArgs-5+2
                  ELSE
                    CurArgs=16
                  ENDIF
                  DO xCount=1,NumIDFRecords
                    IF (MakeUPPERCase(IDFRecords(xCount)%Name(1:4)) /= 'COIL') CYCLE
                    IF (SameString(IDFRecords(xCount)%Alphas(1),InArgs(17))) THEN
                      OutArgs(12)=IDFRecords(xCount)%Name
                      EXIT
                    ENDIF
                  ENDDO
                  DO xCount=1,NumIDFRecords
                    IF (MakeUPPERCase(IDFRecords(xCount)%Name(1:3)) /= 'FAN') CYCLE
                    IF (SameString(IDFRecords(xCount)%Alphas(1),InArgs(18))) THEN
                      OutArgs(14)=IDFRecords(xCount)%Name
                      EXIT
                    ENDIF
                  ENDDO
                  nodiff=.false.
                ...
                CASE ('PLANT LOOP')
                  ...
                  CALL GetNewObjectDefInIDD('SET POINT MANAGER:SCHEDULED',NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  CurArgs=4
                  OutArgs(1)=TRIM(InArgs(1))//' Setpoint Manager'
                  OutArgs(2)='TEMP'
                  OutArgs(3)=InArgs(4)
                  OutArgs(4)=InArgs(11)
                  CALL WriteOutIDFLines(DifLfn,'SET POINT MANAGER:SCHEDULED',CurArgs,OutArgs,NwFldNames,NwFldUnits)
                  Written=.true.
                  !CYCLE
                ...
                CASE ('CONDENSER LOOP')
                  CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                  IF (InArgs(4) == 'AIR' .or. InArgs(4) == 'GROUND') THEN
                    nodiff=.true.
                  ELSE
                    OutArgs(4)=InArgs(11)
                    nodiff=.false.
