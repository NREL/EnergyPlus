...
!
     ...
     ! deleted objects.  no transition.
            ...
            IF (MakeUPPERCase(TRIM(IDFRecords(Num)%Name)) == 'WATER HEATER:SIMPLE') THEN
              ...
              WRITE(DifLfn,fmta) 'Output:PreprocessorMessage,Transition,'//trim(ProgNameConversion)//  &
            ...
            IF (.not. MakingPretty) THEN
              ...
              CASE ('VERSION')
                IF (InArgs(1)(1:3) == '6.0' .and. ArgFile) THEN
...
!    !!!    Changes for this version
              ...
              CASE('AIRLOOPHVAC:UNITARYHEATPUMP:AIRTOAIR:MULTISPEED')
                ...
                OutArgs(6:CurArgs-1)=InArgs(7:CurArgs)
                CurArgs=CurArgs-1
              ...
              CASE('COOLINGTOWER:SINGLESPEED')
                ...
                IF (CurArgs <= 9) OutArgs(10)='UFactorTimesAreaAndDesignWaterFlowRate'
                IF (CurArgs >=25) THEN
                  OutArgs(25:28)=Blank
              ...
              CASE('COOLINGTOWER:TWOSPEED')
                ...
                IF (CurArgs <= 12) OutArgs(13)='UFactorTimesAreaAndDesignWaterFlowRate'
                IF (CurArgs >= 28) THEN
                  OutArgs(28:31)=Blank
                  OutArgs(32:CurArgs+4)=InArgs(28:CurArgs)
                  CurArgs=CurArgs+4
                ENDIF

              CASE('COOLINGTOWER:VARIABLESPEED')
                nodiff=.false.
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:25)=InArgs(1:25)
                IF (CurArgs >= 26) THEN
                  OutArgs(26:29)=Blank
                  OutArgs(30:CurArgs+4)=InArgs(26:CurArgs)
                  CurArgs=CurArgs+4
                ENDIF
              ...
              CASE('COIL:WATERHEATING:DESUPERHEATER')
                ...
                IF (SameString(InArgs(14),'Refrigeration:Condenser')) THEN
                  OutArgs(14)='Refrigeration:Condenser:AirCooled'
                ENDIF
              ...
              CASE('COIL:HEATING:DESUPERHEATER')
                ...
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                IF (SameString(InArgs(6),'Refrigeration:Condenser')) THEN
                  OutArgs(6)='Refrigeration:Condenser:AirCooled'
                ENDIF
              ...
              CASE('ZONEHVAC:FOURPIPEFANCOIL')
                ...
                OutArgs(3)='ConstantFanVariableFlow'
                OutArgs(4)=InArgs(3)
                OutArgs(5:6)=blank
                OutArgs(7)=InArgs(4)
                OutArgs(8)=blank
              ...
              CASE('PLANTEQUIPMENTOPERATION:OUTDOORDRYBULBDIFFERENCE')
                ...
                COutArgs=2
                DO Arg=3,CurArgs,4
                  IF (SameString(InArgs(Arg+2),'ON'))THEN
                    COutArgs=COutArgs+1
              ...
              CASE('SURFACEPROPERTY:CONVECTIONCOEFFICIENTS','SURFACEPROPERTY:CONVECTIONCOEFFICIENTS:MULTIPLESURFACE')
                nodiff=.false.
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:2)=InArgs(1:2)
                OutArgs(3)=InArgs(3)
                IF (SameString(OutArgs(3),'Detailed')) THEN
                  OutArgs(3)='TARP'
                ELSEIF (SameString(OutArgs(3),'BLAST')) THEN
                  OutArgs(3)='TARP'
                ELSEIF (SameString(OutArgs(2),'Outside') .and. SameString(OutArgs(3),'Simple')) THEN
                  OutArgs(3)='SimpleCombined'
                ENDIF
                ...
                IF (CurArgs > 5) THEN
                  OutArgs(7)=InArgs(6)
                  OutArgs(8)=InArgs(7)
                  IF (SameString(OutArgs(8),'Detailed')) THEN
                    OutArgs(8)='TARP'
                  ELSEIF (SameString(OutArgs(8),'BLAST')) THEN
                    OutArgs(8)='TARP'
                  ELSEIF (SameString(OutArgs(7),'Outside') .and. SameString(OutArgs(8),'Simple')) THEN
                    OutArgs(8)='SimpleCombined'
                  ENDIF
                  OutArgs(9)=InArgs(8)
                  OutArgs(10)=Blank
                  CurArgs=CurArgs+1
              ...
              CASE('ZONE')
                ...
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                IF (CurArgs >= 10) THEN
                  IF (SameString(OutArgs(10),'Detailed')) THEN
                    OutArgs(10)='TARP'
                  ENDIF
                ENDIF
                IF (CurArgs >= 11) THEN
                  IF (SameString(OutArgs(11),'Detailed')) THEN
                    OutArgs(11)='TARP'
                  ELSEIF (SameString(OutArgs(11),'BLAST')) THEN
                    OutArgs(11)='TARP'
                  ELSEIF (SameString(OutArgs(11),'Simple')) THEN
                    OutArgs(11)='SimpleCombined'
                  ENDIF
                ENDIF

              CASE('SURFACECONVECTIONALGORITHM:INSIDE')
                nodiff=.false.
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                IF (SameString(OutArgs(1),'Detailed')) THEN
                  OutArgs(1)='TARP'
                ENDIF

              CASE('SURFACECONVECTIONALGORITHM:OUTSIDE')
                nodiff=.false.
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                IF (SameString(OutArgs(1),'Detailed')) THEN
                  OutArgs(1)='TARP'
                ELSEIF (SameString(OutArgs(1),'BLAST')) THEN
                  OutArgs(1)='TARP'
                ELSEIF (SameString(OutArgs(1),'Simple')) THEN
                  OutArgs(1)='SimpleCombined'
                ENDIF
              ...
              CASE('COMPONENTCOST:LINEITEM')
                ...
                IF (CurArgs > 13) THEN
                  DO Arg=14,CurArgs
                    IF (InArgs(Arg) /= Blank) THEN
                      WRITE(DifLfn,fmta)   &
                         'Output:PreprocessorMessage,'//trim(ProgNameConversion)//  &
                            ',warning,Non-used Fields in the ComponentCost:LineItem have been deleted;'
                      EXIT
                    ENDIF
                  ENDDO
                ENDIF
                CurArgs=MIN(13,CurArgs)

              CASE('ZONECAPACITANCEMULTIPLIER')
                nodiff=.false.
                ObjectName='ZoneCapacitanceMultiplier:ResearchSpecial'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:3)=InArgs(1)
                CurArgs=3
              ...
              CASE('HVACTEMPLATE:ZONE:FANCOIL')
                nodiff=.false.
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
              ...
              CASE('HVACTEMPLATE:ZONE:UNITARY')
                nodiff=.false.
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:5)=InArgs(1:5)
                OutArgs(6)=InArgs(5)
              ...
              CASE('HVACTEMPLATE:ZONE:VAV:FANPOWERED')
                nodiff=.false.
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:5)=InArgs(1:5)
                OutArgs(6)=InArgs(5)
              ...
              CASE('HVACTEMPLATE:ZONE:VAV')
                ...
                OutArgs(8)=InArgs(6)
                OutArgs(9:10)=Blank
                OutArgs(11:17)=InArgs(7:13)
                OutArgs(18:21)=Blank
                OutArgs(22:26)=InArgs(14:18)
                CurArgs=26

              CASE('CONSTRUCTION:USEHBALGORITHMCONDFDDETAILED')
                nodiff=.false.
                ObjectName='ConstructionProperty:UseHBAlgorithmCondFDDetailed'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1)=InArgs(1)
