...
SUBROUTINE SetThisVersionVariables()
      ...
      VersionNum=7.1
      IDDFileNameWithPath=TRIM(ProgramPath)//'V7-0-0-Energy+.idd'
      NewIDDFileNameWithPath=TRIM(ProgramPath)//'V7-1-0-Energy+.idd'
      RepVarFileNameWithPath=TRIM(ProgramPath)//'Report Variables 7-0-0-036 to 7-1-0.csv'
END SUBROUTINE
    ...
    DO WHILE (.not. EndOfFile)
     ...
     ! deleted objects.  no transition.
            ...
            IF (.not. MakingPretty) THEN
              ...
              CASE ('VERSION')
                IF (InArgs(1)(1:3) == '7.1' .and. ArgFile) THEN
...
!    !!!    Changes for this version
              ...
              CASE('AIRLOOPHVAC:UNITARYCOOLONLY')
                nodiff=.false.
                ObjectName='CoilSystem:Cooling:DX'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)

              CASE('BRANCH')
                nodiff=.false.  ! Must change AirLoopHVAC:UnitaryCoolOnly to CoilSystem:Cooling:DX
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)
 ! field 4 and every 5 thereafter is object type
                do Arg=4,CurArgs,5
                  if (.not. SameString(OutArgs(Arg),'AirLoopHVAC:UnitaryCoolOnly')) CYCLE
                  OutArgs(Arg)='CoilSystem:Cooling:DX'
                enddo

              CASE('ZONEHVAC:OUTDOORAIRUNIT:EQUIPMENTLIST')
                nodiff=.false.  ! Must change AirLoopHVAC:UnitaryCoolOnly to CoilSystem:Cooling:DX
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                do Arg=2,CurArgs,2
              ...
              CASE('AIRLOOPHVAC:OUTDOORAIRSYSTEM:EQUIPMENTLIST')
                nodiff=.false.  ! Must change AirLoopHVAC:UnitaryCoolOnly to CoilSystem:Cooling:DX
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                do Arg=2,CurArgs,2
              ...
              CASE('SIZINGPERIOD:DESIGNDAY')
                ...
                ENDIF
                OutArgs(9)=InArgs(15)
                IF (SameString(OutArgs(9),'Wetbulb') .or. SameString(OutArgs(9),'Wet-bulb') .or. OutArgs(9) == blank) THEN
                  OutArgs(9)='Wetbulb'
                  OutArgs(10)=InArgs(4)
                  OutArgs(11)=InArgs(16)
                  OutArgs(12)=blank  ! new field
                  OutArgs(13)=blank  ! new field
                ELSEIF (SameString(OutArgs(9),'Dewpoint')) THEN
                  OutArgs(10)=InArgs(4)
                  OutArgs(11)=InArgs(16)
                  OutArgs(12)=blank  ! new field
                  OutArgs(13)=blank  ! new field
                ELSEIF (SameString(OutArgs(9),'HumidityRatio')) THEN
                  OutArgs(10)=blank
                  OutArgs(11)=InArgs(16)
                  OutArgs(12)=InArgs(4)  ! new field
                  OutArgs(13)=blank  ! new field
                ELSEIF (SameString(OutArgs(9),'Enthalpy')) THEN
                  OutArgs(10)=blank
                  OutArgs(11)=InArgs(16)
                  OutArgs(12)=blank  ! new field
                  TestValue=ProcessNumber(InArgs(4),ErrFlag)
                  IF (.not. ErrFlag) THEN
                    TestValue=TestValue*1000.
                    OutArgs(13)=RoundSigDigits(TestValue,2)
                  ELSE
                    OutArgs(13)='invalid'
                  ENDIF
                ELSEIF (SameString(OutArgs(9),'Schedule')) THEN
                  OutArgs(9)='RelativeHumiditySchedule'
                  OutArgs(10:13)=blank
                  OutArgs(11)=InArgs(16)
                ELSEIF (SameString(OutArgs(9),'WetBulbProfileMultiplierSchedule') .or.   &
                        SameString(OutArgs(9),'WetBulbProfileDifferenceSchedule') .or.   &
                        SameString(OutArgs(9),'WetBulbProfileDefaultMultipliers') ) THEN
                  OutArgs(10)=InArgs(4)
                  OutArgs(11)=InArgs(16)
                  OutArgs(12:13)=blank
                ENDIF
                IF (CurArgs >= 24) THEN
                  OutArgs(14)=InArgs(24)
                ELSE
                  OutArgs(14)=blank
                ENDIF
                OutArgs(15)=InArgs(5)
                OutArgs(16)=InArgs(6)
                OutArgs(17)=InArgs(7)
                TestValue=ProcessNumber(InArgs(9),ErrFlag)
                IF (TestValue == 0.0) THEN
                  OutArgs(18)='No'
                ELSEIF (TestValue == 1.0) THEN
                  OutArgs(18)='Yes'
                ELSEIF (InArgs(9) == blank) THEN
                  OutArgs(18)=blank
                ELSE
                  OutArgs(18)='invalid'
                ENDIF
                TestValue=ProcessNumber(InArgs(10),ErrFlag)
                IF (TestValue == 0.0) THEN
                  OutArgs(19)='No'
                ELSEIF (TestValue == 1.0) THEN
                  OutArgs(19)='Yes'
                ELSEIF (InArgs(10) == blank) THEN
                  OutArgs(19)=blank
                ELSE
                  OutArgs(19)='invalid'
                ENDIF
                TestValue=ProcessNumber(InArgs(14),ErrFlag)
                IF (TestValue == 0.0) THEN
                  OutArgs(20)='No'
                ELSEIF (TestValue == 1.0) THEN
                  OutArgs(20)='Yes'
                ELSEIF (InArgs(14) == blank) THEN
                  OutArgs(20)=blank
                ELSE
                  OutArgs(20)='invalid'
                ENDIF
                IF (CurArgs >= 19) THEN
                  OutArgs(21)=InArgs(19)
                ELSE
                  OutArgs(21)='ASHRAEClearSky'
                ENDIF
                IF (CurArgs >= 20) THEN
                  OutArgs(22)=InArgs(20)
                ELSE
                  OutArgs(22)=blank
                ENDIF
                IF (CurArgs >= 21) THEN
                  OutArgs(23)=InArgs(21)
                ELSE
                  OutArgs(23)=blank
                ENDIF
                IF (CurArgs >= 22) THEN
                  OutArgs(24)=InArgs(22)
                ELSE
                  OutArgs(24)=blank
                ENDIF
                IF (CurArgs >= 23) THEN
                  OutArgs(25)=InArgs(23)
                ELSE
                  OutArgs(25)=blank
                ENDIF
                IF (SameString(OutArgs(21),'ASHRAEClearSky') .or. SameString(OutArgs(21),'ZhangHuang')) THEN
                  OutArgs(26)=InArgs(8)
                  CurArgs=26
                ELSE
                  Arg=25
                  IF (InArgs(8) /= blank) THEN
                    CALL writePreProcessorObject(DifLfn,ProgNameConversion,'Warning',  &
                       'SizingPeriod:DesignDay object="'//trim(OutArgs(1))//  &
                       '" '//trim(NwFldNames(21))//'="'//trim(OutArgs(21))//'" does not use '//  &
                       trim(NwFldNames(26))//' but prior input was not blank. Copied to new field.')
                    OutArgs(26)=InArgs(8)
                    Arg=26
                  ENDIF
                  DO CurArgs=Arg,21,-1
              ...
              CASE('CONTROLLER:MECHANICALVENTILATION')
                ...
                COutArgs=6
                do arg=5,NArgs,5
                  OutArgs(COutArgs)=InArgs(arg)
                  COutArgs=COutArgs+1
                  OutArgs(COutArgs)=InArgs(arg+1)
                  COutArgs=COutArgs+1
                  OutArgs(COutArgs)='CM DSZAD '//trim(InArgs(arg))
                  COutArgs=COutArgs+1
                enddo
                COutArgs=COutArgs-1
                ...
                ! create new object DesignSpecification:ZoneAirDistribution
                ObjectName='DesignSpecification:ZoneAirDistribution'
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                do arg=5,NArgs,5
                  OutArgs(1)='CM DSZAD '//trim(InArgs(arg))
                  OutArgs(2)=InArgs(arg+2)
                  OutArgs(3)=InArgs(arg+3)
                  OutArgs(4)=InArgs(arg+4)
                  COutArgs=4
                  CALL WriteOutIDFLines(DifLfn,ObjectName,COutArgs,OutArgs,NwFldNames,NwFldUnits)
                enddo
                Written=.true.
              ...
              CASE('SIZING:ZONE')
                ...
                IF (InArgs(19) /= blank .or. InArgs(20) /= blank) THEN
                  OutArgs(19)='SZ DZAD '//trim(InArgs(1))
                  COutArgs=19
                ELSE
                  COutArgs=18
                ENDIF
                ...
                IF (InArgs(19) /= blank .or. InArgs(20) /= blank) THEN
                  ObjectName='DesignSpecification:ZoneAirDistribution'
                  CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  OutArgs(1)='SZ DZAD '//trim(InArgs(1))
                  OutArgs(2)=InArgs(19)
                  OutArgs(3)=InArgs(20)
                  IF (OutArgs(3) == blank) THEN
                    COutArgs=2
                  ELSE
                    COutArgs=3
                  ENDIF
              ...
              CASE('COIL:HEATING:DX:SINGLESPEED')
                ...
                OutArgs(6)=blank
                OutArgs(7:15)=InArgs(6:14)
                OutArgs(16)=blank
                OutArgs(17:23)=InArgs(15:21)
                IF (CurArgs > 21) THEN
                  OutArgs(24)=blank
              ...
              CASE('WINDOWMATERIAL:SHADE')
                nodiff=.false.
                CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                OutArgs(1:CurArgs)=InArgs(1:CurArgs)
                IF (OutArgs(7) == blank) OutArgs(7)='0.0'
                IF (OutArgs(8) == blank) OutArgs(8)='0.003'
                IF (OutArgs(9) == blank) OutArgs(9)='0.1'
