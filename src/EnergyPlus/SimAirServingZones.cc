...
namespace SimAirServingZones {
	...
	GetAirPathData()
	{
		...
		static Fstring const RoutineName( "GetAirPathData: " );
		...
		int MaxNumbers;
		int MaxAlphas;
		FArray1D_Fstring Alphas( sFstring( MaxNameLength ) ); // alpha strings returned by GetObjectItem
		...
		Fstring ControllerName( MaxNameLength ); // controller name
		Fstring ControllerType( MaxNameLength ); // controller type
		Fstring BranchListName( MaxNameLength ); // Name of a Branch List object
		Fstring ControllerListName( MaxNameLength ); // Name of a controller list object
		Fstring AvailManagerListName( MaxNameLength ); // Name of an availability manager list object
		Fstring ConnectorListName( MaxNameLength ); // Name of a connector list object
		static FArray1D_Fstring BranchNames( sFstring( MaxNameLength ) ); // Branch names from GetBranchList call
		static FArray1D_Fstring CompTypes( sFstring( MaxNameLength ) ); // Component types from GetBranchList call
		static FArray1D_Fstring CompNames( sFstring( MaxNameLength ) ); // Component names from GetBranchList call
		static FArray1D_Fstring InletNodeNames( sFstring( MaxNameLength ) ); // Component inlet node names from GetBranchData call
		static FArray1D_Fstring OutletNodeNames( sFstring( MaxNameLength ) ); // Component outlet node names from GetBranchData call
		static FArray1D_Fstring NodeNames( sFstring( MaxNameLength ) ); // Outlet node names from GetLoopSplitter call
		...
		FArray1D_Fstring MatchNodeName( 3, sFstring( MaxNameLength ) );
		...
		for ( AirSysNum = 1; AirSysNum <= NumPrimaryAirSys; ++AirSysNum ) {
			...
			IsNotOK = false;
			IsBlank = false;
			VerifyName( Alphas( 1 ), PrimaryAirSystem.Name(), AirSysNum - 1, IsNotOK, IsBlank, trim( CurrentModuleObject ) + " Name" );
			if ( IsNotOK ) {
				ErrorsFound = true;
			...
			if ( NumAlphas < 9 ) {
				ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( Alphas( 1 ) ) + "\", insufficient information." );
			...
			AirToZoneNodeInfo( AirSysNum ).AirLoopReturnNodeNum( 1 ) = GetOnlySingleNode( Alphas( 6 ), ErrorsFound, trim( CurrentModuleObject ), Alphas( 1 ), NodeType_Air, NodeConnectionType_Inlet, 1, ObjectIsParent );
			AirToZoneNodeInfo( AirSysNum ).ZoneEquipReturnNodeNum( 1 ) = GetOnlySingleNode( Alphas( 7 ), ErrorsFound, trim( CurrentModuleObject ), Alphas( 1 ), NodeType_Air, NodeConnectionType_Outlet, 1, ObjectIsParent );
			...
			if ( test == 0 ) {
				...
				ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( Alphas( 1 ) ) + "\", duplicate node name." );
				ShowContinueError( "...used for " + trim( cAlphaFields( 6 ) ) + "=\"" + trim( Alphas( 6 ) ) + "\"" );
				ShowContinueError( "...first used in " + trim( CurrentModuleObject ) + "=\"" + trim( TestUniqueNodes( test ).AirLoopName ) + "\" for " + trim( TestUniqueNodes( test ).FieldName ) );
				ErrorsFound = true;
			}
			...
			if ( test == 0 ) {
				...
				ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( Alphas( 1 ) ) + "\", duplicate node name." );
				ShowContinueError( "...used for " + trim( cAlphaFields( 7 ) ) + "=\"" + trim( Alphas( 7 ) ) + "\"" );
				ShowContinueError( "...first used in " + trim( CurrentModuleObject ) + "=\"" + trim( TestUniqueNodes( test ).AirLoopName ) + "\" for " + trim( TestUniqueNodes( test ).FieldName ) );
				ErrorsFound = true;
			}
			...
			if ( test == 0 ) {
				...
				ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( Alphas( 1 ) ) + "\", duplicate node name/list." );
				ShowContinueError( "...used for " + trim( cAlphaFields( 8 ) ) + "=\"" + trim( Alphas( 8 ) ) + "\"" );
				ShowContinueError( "...first used in " + trim( CurrentModuleObject ) + "=\"" + trim( TestUniqueNodes( test ).AirLoopName ) + "\" for " + trim( TestUniqueNodes( test ).FieldName ) );
				ErrorsFound = true;
			}
			...
			if ( test == 0 ) {
				...
				ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( Alphas( 1 ) ) + "\", duplicate node name/list." );
				ShowContinueError( "...used for " + trim( cAlphaFields( 9 ) ) + "=\"" + trim( Alphas( 9 ) ) + "\"" );
				ShowContinueError( "...first used in " + trim( CurrentModuleObject ) + "=\"" + trim( TestUniqueNodes( test ).AirLoopName ) + "\" for " + trim( TestUniqueNodes( test ).FieldName ) );
				ErrorsFound = true;
			}
			...
			if ( test == 0 ) {
				...
				ShowContinueError( trim( cAlphaFields( 7 ) ) + " (Return Air Path or ZoneHVAC:EquipmentConnections) not valid = \"" + trim( Alphas( 7 ) ) + "\"." );
				ErrorsFound = true;
			}
			// Get the supply nodes
			ErrInList = false;
			GetNodeNums( Alphas( 8 ), NumNodes, NodeNums, ErrInList, NodeType_Air, trim( CurrentModuleObject ), PrimaryAirSystem( AirSysNum ).Name, NodeConnectionType_Inlet, 1, ObjectIsParent, _, cAlphaFields( 8 ) );
			if ( ErrInList ) {
			...
			if ( NumNodes > 3 ) {
				ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\", too many nodes." );
				ShowContinueError( "Only 1st 3 Nodes will be used from " + trim( cAlphaFields( 8 ) ) + "=\"" + trim( Alphas( 8 ) ) + "\"." );
				ErrorsFound = true;
			}
			if ( NumNodes == 0 ) {
				ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\", too few nodes." );
			...
			ErrInList = false;
			GetNodeNums( Alphas( 9 ), NumNodes, NodeNums, ErrInList, NodeType_Air, trim( CurrentModuleObject ), PrimaryAirSystem( AirSysNum ).Name, NodeConnectionType_Outlet, 1, ObjectIsParent, _, cAlphaFields( 9 ) );
			if ( ErrInList ) {
				//      CALL ShowContinueError(RoutineName//TRIM(CurrentModuleObject)//'="'//TRIM(PrimaryAirSystem(AirSysNum)%Name)//  &
			...
			if ( NumNodes != AirToZoneNodeInfo( AirSysNum ).NumSupplyNodes ) {
				ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( Alphas( 1 ) ) + "\", node mismatch." );
				ShowContinueError( "...number of air system exit nodes [" + trim( RoundSigDigits( NumNodes ) ) + "] must match number of zone equip inlet nodes [" + trim( RoundSigDigits( AirToZoneNodeInfo( AirSysNum ).NumSupplyNodes ) ) + "]." );
				ErrorsFound = true;
			}
			for ( I = 1; I <= AirToZoneNodeInfo( AirSysNum ).NumSupplyNodes; ++I ) {
			...
			if ( PrimaryAirSystem( AirSysNum ).NumBranches == 0 ) {
				ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\", insufficient information." );
			...
			for ( BranchNum = 1; BranchNum <= PrimaryAirSystem( AirSysNum ).NumBranches; ++BranchNum ) {
				...
				if ( NumCompsOnBranch <= 0 ) {
					ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\", insufficient information." );
					ShowContinueError( "...Branch=\"" + trim( BranchNames( BranchNum ) ) + "\", no components on branch." );
					ErrorsFound = true;
					continue;
				}
				...
				for ( CompNum = 1; CompNum <= PrimaryAirSystem( AirSysNum ).Branch( BranchNum ).TotalComponents; ++CompNum ) {
					...
					if ( SameString( CompTypes( CompNum ), "AirLoopHVAC:OutdoorAirSystem" ) ) {
						if ( PrimaryAirSystem( AirSysNum ).OASysExists ) {
							ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\", too many outdoor air systems." );
						...
						if ( OANum > 0 ) {
							...
							if ( OAMixNum > 0 ) {
								...
								ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\", item not found." );
								ShowContinueError( "OutdoorAir:Mixer for AirLoopHVAC:OutdoorAirSystem=\"" + trim( CompNames( CompNum ) ) + "\" not found." );
								ErrorsFound = true;
							}
						} else {
							ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\", item not found." );
							ShowContinueError( "AirLoopHVAC:OutdoorAirSystem=\"" + trim( CompNames( CompNum ) ) + "\" not found." );
							ShowContinueError( "  referenced in Branch=\"" + trim( PrimaryAirSystem( AirSysNum ).Branch( BranchNum ).Name ) + "\"." );
							ErrorsFound = true;
						}
					}
			...
			for ( OutBranchNum = 1; OutBranchNum <= PrimaryAirSystem( AirSysNum ).NumOutletBranches; ++OutBranchNum ) {
				...
				ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\", branch in error." );
				ShowContinueError( "Probable missing or misspelled node referenced in the branch(es):" );
				for ( BranchNum = 1; BranchNum <= PrimaryAirSystem( AirSysNum ).NumBranches; ++BranchNum ) {
					ShowContinueError( "Possible Error in Branch Object=\"" + trim( PrimaryAirSystem( AirSysNum ).Branch( BranchNum ).Name ) + "\"." );
				}
				ShowContinueError( "...looking to match to Node=\"" + trim( MatchNodeName( OutBranchNum ) ) + "\"." );
				ErrorsFound = true;
			}
			...
			for ( InBranchNum = 1; InBranchNum <= PrimaryAirSystem( AirSysNum ).NumInletBranches; ++InBranchNum ) {
				...
				if ( PrimaryAirSystem( AirSysNum ).InletBranchNum( InBranchNum ) == 0 ) {
					ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\", connection to zone." );
					ShowContinueError( "No Connection found for Return Air from Zone" );
					ShowContinueError( "Expected node name =\"" + trim( NodeID( AirToZoneNodeInfo( AirSysNum ).AirLoopReturnNodeNum( InBranchNum ) ) ) + "\"." );
					ErrorsFound = true;
				}
			}
			...
			if ( ConnectorListName != " " ) {
				...
				if ( ConListNum > 0 ) {
					...
					ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\", connector list object." );
					ShowContinueError( "ConnectorList object=\"" + trim( ConnectorListName ) + "\" not found in input." );
				}
				errFlag = false;
			...
			if ( ControllerListName != " " ) { // If not blank, then must be there and valid
				...
				if ( ControllerListNum > 0 ) {
					...
					for ( ControllerNum = NumOASysSimpControllers + 1; ControllerNum <= NumOASysSimpControllers + NumControllers; ++ControllerNum ) {
						...
						IsNotOK = false;
						ValidateComponent( ControllerType, ControllerName, IsNotOK, trim( CurrentModuleObject ) );
						if ( IsNotOK ) {
							ShowContinueError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\", for ControllerList=\"" + trim( ControllerListName ) + "\"." );
							ErrorsFound = true;
						}
					...
					ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\", controller list object." );
					ShowContinueError( "ControllerList object=\"" + trim( ControllerListName ) + "\" not found in input." );
					ErrorsFound = true;
				}
			}
			...
			if ( NumControllers + NumOASysSimpControllers == 0 ) {
				if ( ! PackagedUnit( AirSysNum ) ) {
					ShowWarningError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\" has no Controllers." );
				}
			...
			if ( errFlag ) {
				ShowContinueError( "Occurs in " + trim( CurrentModuleObject ) + " = " + trim( PrimaryAirSystem( AirSysNum ).Name ) );
				ErrorsFound = true;
			}
			...
			for ( BranchNum = 1; BranchNum <= PrimaryAirSystem( AirSysNum ).NumBranches; ++BranchNum ) {
				for ( CompNum = 1; CompNum <= PrimaryAirSystem( AirSysNum ).Branch( BranchNum ).TotalComponents; ++CompNum ) {
					...
					if ( SELECT_CASE_var == "AIRLOOPHVAC:OUTDOORAIRSYSTEM" ) {
						...
						ShowSevereError( RoutineName + trim( CurrentModuleObject ) + " = \"" + trim( PrimaryAirSystem( AirSysNum ).Name ) + "\"." );
						ShowContinueError( "..Invalid Air Loop Component Type = \"" + trim( PrimaryAirSystem( AirSysNum ).Branch( BranchNum ).Comp( CompNum ).TypeOf ) + "\"." );
						ShowContinueError( "..Air Loop Component Name = \"" + trim( PrimaryAirSystem( AirSysNum ).Branch( BranchNum ).Comp( CompNum ).Name ) + "\"." );
						ShowContinueError( "..reference Branch = \"" + trim( PrimaryAirSystem( AirSysNum ).Branch( BranchNum ).Name ) + "\"." );
			...
			for ( BranchNum = 1; BranchNum <= PrimaryAirSystem( AirSysNum ).NumBranches; ++BranchNum ) {
				for ( CompNum = 1; CompNum <= PrimaryAirSystem( AirSysNum ).Branch( BranchNum ).TotalComponents; ++CompNum ) {
					...
					if ( CompType_Num == WaterCoil_DetailedCool || CompType_Num == WaterCoil_SimpleHeat || CompType_Num == WaterCoil_Cooling ) {
						...
						if ( NodeNotFound ) {
							ErrorsFound = true;
							ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( PrimaryAirSystem( AirSysNum ).Branch( BranchNum ).Comp( CompNum ).Name ) + "\", invalid actuator." );
		...
		for ( OASysNum = 1; OASysNum <= OANum; ++OASysNum ) {
			...
			for ( OACompNum = 1; OACompNum <= NumInList; ++OACompNum ) {
				...
				if ( CompType_Num == WaterCoil_DetailedCool || CompType_Num == WaterCoil_SimpleHeat || CompType_Num == WaterCoil_Cooling ) {
					...
					if ( NodeNotFound ) {
						ErrorsFound = true;
						ShowSevereError( RoutineName + trim( CurrentModuleObject ) + "=\"" + trim( GetOACompName( OASysNum, OACompNum ) ) + "\", invalid actuator." );
						ShowContinueError( "...this coil requires a water coil controller " "and the inlet node of a water coil must also be an actuator node of a water coil controller." );
					}
				}
			}
		...
		if ( ErrorsFound ) {
			ShowFatalError( RoutineName + "Errors found retrieving input for " + trim( CurrentModuleObject ) + "." );
		}
		...
		if ( MyOneTimeFlag ) {
			...
			SupplyAirPathLoop: for ( SupAirPath = 1; SupAirPath <= NumSupplyAirPaths; ++SupAirPath ) {
				...
				for ( CompNum = 1; CompNum <= SupplyAirPath( SupAirPath ).NumOfComponents; ++CompNum ) {
					if ( SameString( SupplyAirPath( SupAirPath ).ComponentType( CompNum ), "AirLoopHVAC:ZoneSplitter" ) ) {
						SplitterNum = FindItemInList( SupplyAirPath( SupAirPath ).ComponentName( CompNum ), SplitterCond.SplitterName(), NumSplitters );
						if ( SplitterNum == 0 ) {
							ShowSevereError( "AirLoopHVAC:ZoneSplitter not found=" + trim( SupplyAirPath( SupAirPath ).ComponentName( CompNum ) ) );
							ShowContinueError( "Occurs in AirLoopHVAC:SupplyPath=" + trim( SupplyAirPath( SupAirPath ).Name ) );
							ErrorsFound = true;
						}
						...
						PlenumNum = FindItemInList( SupplyAirPath( SupAirPath ).ComponentName( CompNum ), ZoneSupPlenCond.ZonePlenumName(), NumZoneSupplyPlenums );
						if ( PlenumNum == 0 ) {
							ShowSevereError( "AirLoopHVAC:SupplyPlenum not found=" + trim( SupplyAirPath( SupAirPath ).ComponentName( CompNum ) ) );
							ShowContinueError( "Occurs in AirLoopHVAC:SupplyPath=" + trim( SupplyAirPath( SupAirPath ).Name ) );
							ErrorsFound = true;
						}
			...
			PrimaryAirSysLoop: for ( AirLoopNum = 1; AirLoopNum <= NumPrimaryAirSys; ++AirLoopNum ) {
				...
				AirSysOutletsLoop: for ( OutNum = 1; OutNum <= AirToZoneNodeInfo( AirLoopNum ).NumSupplyNodes; ++OutNum ) {
					...
					SupplyAirPathOutletLoop: for ( SupAirPathOutNodeNum = 1; SupAirPathOutNodeNum <= NumSupAirPathOutNodes; ++SupAirPathOutNodeNum ) {
						...
						if ( ! FoundSupPathZoneConnect ) {
							ShowSevereError( "Node " + trim( NodeID( SupplyAirPath( SupAirPathNum ).OutletNode( SupAirPathOutNodeNum ) ) ) + " connects to no component" );
							ShowContinueError( "Occurs in Supply Air Path=" + trim( SupplyAirPath( SupAirPathNum ).Name ) );
				...
				if ( ( NumZonesCool + NumZonesHeat ) == 0 ) {
					ShowSevereError( "An outlet node in AirLoopHVAC=\"" + trim( PrimaryAirSystem( AirLoopNum ).Name ) + "\" is not connected to any zone" );
					ShowContinueError( "Could not match ZoneEquipGroup Inlet Node=\"" + trim( NodeID( ZoneSideNodeNum ) ) + "\" to any Supply Air Path or controlled zone" );
					ErrorsFound = true;
				}
		...
		for ( AirLoopNum = 1; AirLoopNum <= NumPrimaryAirSys; ++AirLoopNum ) {
			...
			if ( FirstHVACIteration ) {
				...
				for ( InNum = 1; InNum <= PrimaryAirSystem( AirLoopNum ).NumInletBranches; ++InNum ) {
					...
					if ( InBranchNum == 0 ) {
						ShowFatalError( "Missing Inlet Branch on Primary Air System=" + trim( PrimaryAirSystem( AirLoopNum ).Name ) );
					}
	...
	)
	{
		...
		static Fstring ErrEnvironmentName( MaxNameLength * 2 );
		// A character string equivalent of ErrCount
		Fstring CharErrOut( 20 );
					...
					if ( Iter > MaxIter ) {
						...
						if ( ! WarmupFlag ) {
							...
							if ( ErrCount < 15 ) {
								...
								ShowWarningError( "SolveAirLoopControllers: Maximum iterations (" + trim( CharErrOut ) + ") exceeded for " + trim( PrimaryAirSystem( AirLoopNum ).Name ) + ", at " + trim( EnvironmentName ) + ", " + trim( CurMnDy ) + " " + trim( CreateSysTimeIntervalString() ) );
							} else {
								if ( EnvironmentName != ErrEnvironmentName ) {
									MaxErrCount = 0;
									ErrEnvironmentName = EnvironmentName;
								}
								ShowRecurringWarningErrorAtEnd( "SolveAirLoopControllers: Exceeding Maximum iterations for " + trim( PrimaryAirSystem( AirLoopNum ).Name ) + " during " + trim( EnvironmentName ) + " continues", MaxErrCount );
							}
						}
	...
	)
	{
		...
		Fstring CompType( MaxNameLength ); // Component type
		Fstring CompName( MaxNameLength ); // Component name
		Fstring CoilName( MaxNameLength );
		Fstring CoilType( MaxNameLength );
		int CompType_Num; // Numeric equivalent for CompType
		int CompNum;
		bool ErrorsFound;
		...
		if ( BranchNum == 1 ) {
			...
			if ( PrimaryAirSystem( AirLoopNum ).DesignVolFlowRate < SmallAirVolFlow ) {
				ShowSevereError( "AirLoopHVAC " + trim( PrimaryAirSystem( AirLoopNum ).Name ) + " has no air flow" );
	...
	SetUpSysSizingArrays()
	{
		...
		for ( SysSizIndex = 1; SysSizIndex <= NumSysSizInput; ++SysSizIndex ) {
			...
			if ( PrimAirIndex == 0 ) {
				ShowSevereError( "Sizing:System: " + trim( SysSizInput( SysSizIndex ).AirPriLoopName ) + " references unknown AirLoopHVAC" );
				ErrorsFound = true;
			} else {
		...
		SysSizing.allocate( NumPrimaryAirSys, TotDesDays + TotRunDesPersDays );
		...
		for ( DesDayEnvrnNum = 1; DesDayEnvrnNum <= TotDesDays + TotRunDesPersDays; ++DesDayEnvrnNum ) {
			for ( AirLoopNum = 1; AirLoopNum <= NumPrimaryAirSys; ++AirLoopNum ) {
				...
				SysSizing( AirLoopNum, DesDayEnvrnNum ).HeatFlowSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).CoolFlowSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SensCoolCapSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).HeatCapSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).PreheatCapSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysCoolRetTempSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysCoolRetHumRatSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysHeatRetTempSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysHeatRetHumRatSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysCoolOutTempSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysCoolOutHumRatSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysHeatOutTempSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysHeatOutHumRatSeq.allocate( NumOfTimeStepInDay );
				SysSizing( AirLoopNum, DesDayEnvrnNum ).HeatFlowSeq = 0.0;
				SysSizing( AirLoopNum, DesDayEnvrnNum ).CoolFlowSeq = 0.0;
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SensCoolCapSeq = 0.0;
				SysSizing( AirLoopNum, DesDayEnvrnNum ).HeatCapSeq = 0.0;
				SysSizing( AirLoopNum, DesDayEnvrnNum ).PreheatCapSeq = 0.0;
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysCoolRetTempSeq = 0.0;
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysCoolRetHumRatSeq = 0.0;
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysHeatRetTempSeq = 0.0;
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysHeatRetHumRatSeq = 0.0;
				SysSizing( AirLoopNum, DesDayEnvrnNum ).SysCoolOutTempSeq = 0.0;
		...
		for ( AirLoopNum = 1; AirLoopNum <= NumPrimaryAirSys; ++AirLoopNum ) {
			...
			if ( SysSizNum > 0 ) { // move data from system sizing input
				...
				ShowWarningError( "SetUpSysSizingArrays: Sizing for System (HVACAirLoop)=\"" + trim( FinalSysSizing( AirLoopNum ).AirPriLoopName ) + "\" will use Sizing:System specifications listed for System=\"" + trim( SysSizInput( 1 ).AirPriLoopName ) + "\"." );
			...
			if ( AnyEnergyManagementSystemInModel ) {
				...
				SetupEMSInternalVariable( "Air System Cooling Design Mixed Air Temperature", FinalSysSizing( AirLoopNum ).AirPriLoopName, "[C]", FinalSysSizing( AirLoopNum ).CoolMixTemp );
				SetupEMSInternalVariable( "Air System Cooling Design Mixed Air Humidity Ratio", FinalSysSizing( AirLoopNum ).AirPriLoopName, "[kgWater/kgDryAir]", FinalSysSizing( AirLoopNum ).CoolMixHumRat );
				SetupEMSInternalVariable( "Air System Cooling Design Return Air Temperature", FinalSysSizing( AirLoopNum ).AirPriLoopName, "[C]", FinalSysSizing( AirLoopNum ).CoolRetTemp );
				SetupEMSInternalVariable( "Air System Cooling Design Return Air Humidity Ratio", FinalSysSizing( AirLoopNum ).AirPriLoopName, "[kgWater/kgDryAir]", FinalSysSizing( AirLoopNum ).CoolRetHumRat );
				SetupEMSInternalVariable( "Air System Cooling Design Outdoor Air Temperature", FinalSysSizing( AirLoopNum ).AirPriLoopName, "[C]", FinalSysSizing( AirLoopNum ).CoolOutTemp );
				SetupEMSInternalVariable( "Air System Cooling Design Outdoor Air Humidity Ratio", FinalSysSizing( AirLoopNum ).AirPriLoopName, "[kgWater/kgDryAir]", FinalSysSizing( AirLoopNum ).CoolOutHumRat );
				...
				for ( ZonesCooledNum = 1; ZonesCooledNum <= NumZonesCooled; ++ZonesCooledNum ) {
					...
					if ( SysSizNum > 0 ) {
						if ( SysSizInput( SysSizNum ).SystemOAMethod == SOAM_ZoneSum ) { // ZoneSum Method
							...
							if ( ! ( ZoneOAUnc > 0.0 ) ) {
								ShowSevereError( "Sizing:System - The system outdoor air method is set to VRP in " + trim( FinalSysSizing( AirLoopNum ).AirPriLoopName ) );
								ShowContinueError( "But zone \"" + trim( FinalZoneSizing( CtrlZoneNum ).ZoneName ) + "\" associated with system does not have OA flow/person" );
	...
	UpdateSysSizing( int const CallIndicator )
	{
		...
		static Fstring const SSizeFmt10( "('Time')" );
		static Fstring const SSizeFmt11( "(A1,A,A,A1,A,A,A1,A,A,A1,A,A)" );
		static Fstring const SSizeFmt20( "(I2.2,':',I2.2,':00')" );
		static Fstring const SSizeFmt21( "(A1,ES12.6,A1,ES12.6,A1,ES12.6,A1,ES12.6)" );
		static Fstring const SSizeFmt30( "('Coinc Peak   ')" );
		static Fstring const SSizeFmt31( "(A1,ES12.6,A1,ES12.6,A1,ES12.6,A1,ES12.6)" );
		static Fstring const SSizeFmt40( "('NonCoinc Peak')" );
		static Fstring const SSizeFmt41( "(A1,ES12.6,A1,ES12.6,A1,ES12.6,A1,ES12.6 )" );
		...
		if ( SELECT_CASE_var == BeginDay ) {
			...
			for ( CtrlZoneNum = 1; CtrlZoneNum <= NumOfZones; ++CtrlZoneNum ) {
				...
				RetTempRise = ZoneSizing( CtrlZoneNum, CurOverallSimDay ).ZoneRetTempAtCoolPeak - ZoneSizing( CtrlZoneNum, CurOverallSimDay ).ZoneTempAtCoolPeak;
				if ( RetTempRise > 0.01 ) {
					...
					ZoneSizing( CtrlZoneNum, CurOverallSimDay ).ZoneRetTempAtCoolPeak = ZoneSizing( CtrlZoneNum, CurOverallSimDay ).ZoneTempAtCoolPeak + RetTempRise * termunitsizingtempfrac;
				}
				RetTempRise = ZoneSizing( CtrlZoneNum, CurOverallSimDay ).ZoneRetTempAtHeatPeak - ZoneSizing( CtrlZoneNum, CurOverallSimDay ).ZoneTempAtHeatPeak;
				if ( RetTempRise > 0.01 ) {
					//avoid possible compiler bug
					...
					ZoneSizing( CtrlZoneNum, CurOverallSimDay ).ZoneRetTempAtHeatPeak = ZoneSizing( CtrlZoneNum, CurOverallSimDay ).ZoneTempAtHeatPeak + RetTempRise * termunitsizingtempfrac;
				}
			}
			...
			for ( AirLoopNum = 1; AirLoopNum <= NumPrimaryAirSys; ++AirLoopNum ) { // start of begin day loop over primary air systems
				...
				for ( ZonesCooledNum = 1; ZonesCooledNum <= NumZonesCooled; ++ZonesCooledNum ) { // loop over cooled zones
					CtrlZoneNum = AirToZoneNodeInfo( AirLoopNum ).CoolCtrlZoneNums( ZonesCooledNum );
					SysSizing( AirLoopNum, CurOverallSimDay ).NonCoinCoolMassFlow += ZoneSizing( CtrlZoneNum, CurOverallSimDay ).DesCoolMassFlow / ( 1.0 + TermUnitSizing( CtrlZoneNum ).InducRat );
				...
				if ( NumZonesHeated > 0 ) { // if there are zones supplied with central hot air
					for ( ZonesHeatedNum = 1; ZonesHeatedNum <= NumZonesHeated; ++ZonesHeatedNum ) { // loop over heated zones
						CtrlZoneNum = AirToZoneNodeInfo( AirLoopNum ).HeatCtrlZoneNums( ZonesHeatedNum );
						SysSizing( AirLoopNum, CurOverallSimDay ).NonCoinHeatMassFlow += ZoneSizing( CtrlZoneNum, CurOverallSimDay ).DesHeatMassFlow / ( 1.0 + TermUnitSizing( CtrlZoneNum ).InducRat );
			...
			for ( CtrlZoneNum = 1; CtrlZoneNum <= NumOfZones; ++CtrlZoneNum ) {
				...
				termunitsizingtemp = ( 1.0 + TermUnitSizing( CtrlZoneNum ).InducRat );
				termunitsizingtempfrac = ( 1.0 / termunitsizingtemp );
				RetTempRise = ZoneSizing( CtrlZoneNum, CurOverallSimDay ).CoolZoneRetTempSeq( TimeStepInDay ) - ZoneSizing( CtrlZoneNum, CurOverallSimDay ).CoolZoneTempSeq( TimeStepInDay );
				if ( RetTempRise > 0.01 ) {
					//avoid possible compiler bug
					...
					//           (1.0d0/(1.0d0+TermUnitSizing(CtrlZoneNum)%InducRat))
					ZoneSizing( CtrlZoneNum, CurOverallSimDay ).CoolZoneRetTempSeq( TimeStepInDay ) = ZoneSizing( CtrlZoneNum, CurOverallSimDay ).CoolZoneTempSeq( TimeStepInDay ) + RetTempRise * termunitsizingtempfrac;
				}
			...
			for ( AirLoopNum = 1; AirLoopNum <= NumPrimaryAirSys; ++AirLoopNum ) { // start of zone time step loop over primary air systems
				...
				for ( ZonesCooledNum = 1; ZonesCooledNum <= NumZonesCooled; ++ZonesCooledNum ) { // loop over zones cooled by central system
					...
					SysSizing( AirLoopNum, CurOverallSimDay ).CoolFlowSeq( TimeStepInDay ) += ZoneSizing( CtrlZoneNum, CurOverallSimDay ).CoolFlowSeq( TimeStepInDay ) / ( 1.0 + TermUnitSizing( CtrlZoneNum ).InducRat );
					// calculate the return air temperature for this time step
					SysCoolRetTemp += ZoneSizing( CtrlZoneNum, CurOverallSimDay ).CoolZoneRetTempSeq( TimeStepInDay ) * ZoneSizing( CtrlZoneNum, CurOverallSimDay ).CoolFlowSeq( TimeStepInDay ) / ( 1.0 + TermUnitSizing( CtrlZoneNum ).InducRat );
					SysCoolRetHumRat += ZoneSizing( CtrlZoneNum, CurOverallSimDay ).CoolZoneHumRatSeq( TimeStepInDay ) * ZoneSizing( CtrlZoneNum, CurOverallSimDay ).CoolFlowSeq( TimeStepInDay ) / ( 1.0 + TermUnitSizing( CtrlZoneNum ).InducRat );
				} // end of loop over zones cooled by central system
				// check that there is system mass flow
				if ( SysSizing( AirLoopNum, CurOverallSimDay ).CoolFlowSeq( TimeStepInDay ) > 0.0 ) {
					// complete return air temp calc
					SysCoolRetTemp /= SysSizing( AirLoopNum, CurOverallSimDay ).CoolFlowSeq( TimeStepInDay );
					SysCoolRetHumRat /= SysSizing( AirLoopNum, CurOverallSimDay ).CoolFlowSeq( TimeStepInDay );
					...
					RhoAir = StdRhoAir;
					if ( SysSizing( AirLoopNum, CurOverallSimDay ).CoolOAOption == MinOA ) {
						OutAirFrac = RhoAir * SysSizing( AirLoopNum, CurOverallSimDay ).DesOutAirVolFlow / SysSizing( AirLoopNum, CurOverallSimDay ).CoolFlowSeq( TimeStepInDay );
					...
					SysSensCoolCap = PsyCpAirFnWTdb( constant_zero, constant_twenty ) * SysSizing( AirLoopNum, CurOverallSimDay ).CoolFlowSeq( TimeStepInDay ) * ( SysCoolMixTemp - SysSizing( AirLoopNum, CurOverallSimDay ).CoolSupTemp );
				...
				if ( NumZonesHeated > 0 ) { // IF there are centrally heated zones
					...
					for ( ZonesHeatedNum = 1; ZonesHeatedNum <= NumZonesHeated; ++ZonesHeatedNum ) { // loop over the heated zones
						...
						SysSizing( AirLoopNum, CurOverallSimDay ).HeatFlowSeq( TimeStepInDay ) += ZoneSizing( CtrlZoneNum, CurOverallSimDay ).HeatFlowSeq( TimeStepInDay ) / ( 1.0 + TermUnitSizing( CtrlZoneNum ).InducRat );
						// calculate the return air temperature for this time step
						SysHeatRetTemp += ZoneSizing( CtrlZoneNum, CurOverallSimDay ).HeatZoneRetTempSeq( TimeStepInDay ) * ZoneSizing( CtrlZoneNum, CurOverallSimDay ).HeatFlowSeq( TimeStepInDay ) / ( 1.0 + TermUnitSizing( CtrlZoneNum ).InducRat );
						SysHeatRetHumRat += ZoneSizing( CtrlZoneNum, CurOverallSimDay ).HeatZoneHumRatSeq( TimeStepInDay ) * ZoneSizing( CtrlZoneNum, CurOverallSimDay ).HeatFlowSeq( TimeStepInDay ) / ( 1.0 + TermUnitSizing( CtrlZoneNum ).InducRat );
					} // end heated zones loop
					// check that the system flow rate is nonzero
					if ( SysSizing( AirLoopNum, CurOverallSimDay ).HeatFlowSeq( TimeStepInDay ) > 0.0 ) {
						// complete return air temp calc
						SysHeatRetTemp /= SysSizing( AirLoopNum, CurOverallSimDay ).HeatFlowSeq( TimeStepInDay );
						SysHeatRetHumRat /= SysSizing( AirLoopNum, CurOverallSimDay ).HeatFlowSeq( TimeStepInDay );
						...
						// calculate the outside air fraction for this time step
						RhoAir = StdRhoAir;
						if ( SysSizing( AirLoopNum, CurOverallSimDay ).HeatOAOption == MinOA ) {
							OutAirFrac = RhoAir * SysSizing( AirLoopNum, CurOverallSimDay ).DesOutAirVolFlow / SysSizing( AirLoopNum, CurOverallSimDay ).HeatFlowSeq( TimeStepInDay );
							OutAirFrac = min( 1.0, max( 0.0, OutAirFrac ) );
						} else {
							OutAirFrac = 1.0;
						}
						...
						SysHeatCap = PsyCpAirFnWTdb( constant_zero, constant_twenty ) * SysSizing( AirLoopNum, CurOverallSimDay ).HeatFlowSeq( TimeStepInDay ) * ( SysSizing( AirLoopNum, CurOverallSimDay ).HeatSupTemp - SysHeatMixTemp );
					...
					if ( SysHeatCap > SysSizing( AirLoopNum, CurOverallSimDay ).HeatCap ) {
					...
					SysSizing( AirLoopNum, CurOverallSimDay ).CoinHeatMassFlow = max( SysSizing( AirLoopNum, CurOverallSimDay ).CoinHeatMassFlow, SysSizing( AirLoopNum, CurOverallSimDay ).HeatFlowSeq( TimeStepInDay ) );
			...
			for ( AirLoopNum = 1; AirLoopNum <= NumPrimaryAirSys; ++AirLoopNum ) {
				...
				{ auto const SELECT_CASE_var1( SysSizing( AirLoopNum, CurOverallSimDay ).SizingOption );
				if ( SELECT_CASE_var1 == Coincident ) {
					if ( FinalSysSizing( AirLoopNum ).SystemOAMethod == SOAM_ZoneSum ) {
						SysSizing( AirLoopNum, CurOverallSimDay ).DesCoolVolFlow = SysSizing( AirLoopNum, CurOverallSimDay ).CoinCoolMassFlow / StdRhoAir;
						SysSizing( AirLoopNum, CurOverallSimDay ).DesHeatVolFlow = SysSizing( AirLoopNum, CurOverallSimDay ).CoinHeatMassFlow / StdRhoAir;
					} else if ( FinalSysSizing( AirLoopNum ).SystemOAMethod == SOAM_VRP ) { // Ventilation Rate Procedure
						// cooling
						SysSizing( AirLoopNum, CurOverallSimDay ).DesCoolVolFlow = SysSizing( AirLoopNum, CurOverallSimDay ).CoinCoolMassFlow / StdRhoAir;
						if ( SysSizing( AirLoopNum, CurOverallSimDay ).DesCoolVolFlow > 0 ) {
							OutAirFrac = SysSizing( AirLoopNum, CurOverallSimDay ).DesOutAirVolFlow / SysSizing( AirLoopNum, CurOverallSimDay ).DesCoolVolFlow;
						} else {
							OutAirFrac = 0.0;
						}
						OutAirFrac = min( 1.0, max( 0.0, OutAirFrac ) );
						...
						if ( FinalSysSizing( AirLoopNum ).OAAutoSized && SysSizing( AirLoopNum, CurOverallSimDay ).DesCoolVolFlow > 0 ) {
							NumZonesCooled = AirToZoneNodeInfo( AirLoopNum ).NumZonesCooled;
							MinCoolingEvz = 1.0;
						...
						// heating
						SysSizing( AirLoopNum, CurOverallSimDay ).DesHeatVolFlow = SysSizing( AirLoopNum, CurOverallSimDay ).CoinHeatMassFlow / StdRhoAir;
						if ( SysSizing( AirLoopNum, CurOverallSimDay ).DesHeatVolFlow > 0 ) {
							OutAirFrac = SysSizing( AirLoopNum, CurOverallSimDay ).DesOutAirVolFlow / SysSizing( AirLoopNum, CurOverallSimDay ).DesHeatVolFlow;
						} else {
							OutAirFrac = 0.0;
						}
						OutAirFrac = min( 1.0, max( 0.0, OutAirFrac ) );
						...
						if ( SysSizing( AirLoopNum, CurOverallSimDay ).DesHeatVolFlow > 0 ) {
							...
							Xs = min( 1.0, FinalSysSizing( AirLoopNum ).SysUncOA / max( SysSizing( AirLoopNum, CurOverallSimDay ).DesHeatVolFlow, SysHtgPeakAirflow ) );
						} else {
							Xs = 0.0;
						}

						if ( FinalSysSizing( AirLoopNum ).OAAutoSized && SysSizing( AirLoopNum, CurOverallSimDay ).DesHeatVolFlow > 0 ) {
							NumZonesHeated = AirToZoneNodeInfo( AirLoopNum ).NumZonesHeated;
							MinHeatingEvz = 1.0;
					...
					SysSizing( AirLoopNum, CurOverallSimDay ).DesMainVolFlow = max( SysSizing( AirLoopNum, CurOverallSimDay ).DesCoolVolFlow, SysSizing( AirLoopNum, CurOverallSimDay ).DesHeatVolFlow );
				} else if ( SELECT_CASE_var1 == NonCoincident ) {
					if ( FinalSysSizing( AirLoopNum ).SystemOAMethod == SOAM_ZoneSum ) {
						SysSizing( AirLoopNum, CurOverallSimDay ).DesCoolVolFlow = SysSizing( AirLoopNum, CurOverallSimDay ).NonCoinCoolMassFlow / StdRhoAir;
						SysSizing( AirLoopNum, CurOverallSimDay ).DesHeatVolFlow = SysSizing( AirLoopNum, CurOverallSimDay ).NonCoinHeatMassFlow / StdRhoAir;
					} else if ( FinalSysSizing( AirLoopNum ).SystemOAMethod == SOAM_VRP ) { // Ventilation Rate Procedure
						// cooling
						SysSizing( AirLoopNum, CurOverallSimDay ).DesCoolVolFlow = SysSizing( AirLoopNum, CurOverallSimDay ).NonCoinCoolMassFlow / StdRhoAir;
						if ( SysSizing( AirLoopNum, CurOverallSimDay ).DesCoolVolFlow > 0 ) {
			...
			for ( AirLoopNum = 1; AirLoopNum <= NumPrimaryAirSys; ++AirLoopNum ) {
				...
				for ( DDNum = 1; DDNum <= TotDesDays + TotRunDesPersDays; ++DDNum ) {
					...
					if ( SysSizing( AirLoopNum, DDNum ).HeatCap > CalcSysSizing( AirLoopNum ).HeatCap ) {
			...
			for ( AirLoopNum = 1; AirLoopNum <= NumPrimaryAirSys; ++AirLoopNum ) {
				...
				if ( CalcSysSizing( AirLoopNum ).InpDesCoolAirFlow > 0.0 && CalcSysSizing( AirLoopNum ).DesCoolVolFlow > 0.0 && CalcSysSizing( AirLoopNum ).CoolAirDesMethod == InpDesAirFlow ) {
				...
				if ( CalcSysSizing( AirLoopNum ).InpDesHeatAirFlow > 0.0 && CalcSysSizing( AirLoopNum ).DesHeatVolFlow > 0.0 && CalcSysSizing( AirLoopNum ).HeatAirDesMethod == InpDesAirFlow ) {
				...
				if ( std::abs( SysCoolSizingRat - 1.0 ) > .00001 ) {
					...
					if ( FinalSysSizing( AirLoopNum ).DesCoolVolFlow > 0.0 ) {
						...
						FinalSysSizing( AirLoopNum ).CoolMixTemp = FinalSysSizing( AirLoopNum ).CoolOutTemp * OutAirFrac + FinalSysSizing( AirLoopNum ).CoolRetTemp * ( 1.0 - OutAirFrac );
						FinalSysSizing( AirLoopNum ).CoolMixHumRat = FinalSysSizing( AirLoopNum ).CoolOutHumRat * OutAirFrac + FinalSysSizing( AirLoopNum ).CoolRetHumRat * ( 1.0 - OutAirFrac );
						FinalSysSizing( AirLoopNum ).SensCoolCap = PsyCpAirFnWTdb( constant_zero, constant_twenty ) * RhoAir * FinalSysSizing( AirLoopNum ).DesCoolVolFlow * ( FinalSysSizing( AirLoopNum ).CoolMixTemp - FinalSysSizing( AirLoopNum ).CoolSupTemp );
				...
				if ( std::abs( SysHeatSizingRat - 1.0 ) > .00001 ) {
					...
					if ( NumZonesHeated > 0 ) { // IF there are centrally heated zones
						...
						for ( ZonesCooledNum = 1; ZonesCooledNum <= NumZonesCooled; ++ZonesCooledNum ) { // loop over the cooled zones
							CtrlZoneNum = AirToZoneNodeInfo( AirLoopNum ).CoolCtrlZoneNums( ZonesCooledNum );
							if ( ( SysHeatSizingRat != 1.0 ) && ( FinalSysSizing( AirLoopNum ).LoadSizeType == Ventilation ) && ( FinalZoneSizing( CtrlZoneNum ).MinOA <= 0.0 ) ) {
								ShowWarningError( "FinalSystemSizing: AirLoop=\"" + trim( AirToZoneNodeInfo( AirLoopNum ).AirLoopName ) + "\", Requested sizing on Ventilation," );
								ShowContinueError( "but Zone has no design OA Flow. Zone=\"" + trim( ZoneEquipConfig( CtrlZoneNum ).ZoneName ) + "\"." );
							}
							if ( ( SysHeatSizingRat != 1.0 ) && ( FinalSysSizing( AirLoopNum ).LoadSizeType == Ventilation ) && ( FinalZoneSizing( CtrlZoneNum ).MinOA > 0.0 ) ) {
								ZoneOARatio = FinalZoneSizing( CtrlZoneNum ).MinOA / max( FinalZoneSizing( CtrlZoneNum ).DesHeatVolFlow, FinalZoneSizing( CtrlZoneNum ).MinOA );
								TermUnitFinalZoneSizing( CtrlZoneNum ).DesHeatVolFlow = FinalZoneSizing( CtrlZoneNum ).DesHeatVolFlow * ZoneOARatio * ( 1.0 + TermUnitSizing( CtrlZoneNum ).InducRat );
			...
			for ( I = 1; I <= NumPrimaryAirSys; ++I ) {
				{ IOFlags flags; flags.ADVANCE( "No" ); gio::write( OutputFileSysSizing, SSizeFmt11, flags ) << SizingFileColSep << trim( CalcSysSizing( I ).AirPriLoopName ) << ":Des Heat Mass Flow [kg/s]" << SizingFileColSep << trim( CalcSysSizing( I ).AirPriLoopName ) << ":Des Cool Mass Flow [kg/s]" << SizingFileColSep << trim( CalcSysSizing( I ).AirPriLoopName ) << ":Des Heat Cap [W]" << SizingFileColSep << trim( CalcSysSizing( I ).AirPriLoopName ) << ":Des Sens Cool Cap [W]"; }
			}
			gio::write( OutputFileSysSizing, fmta ) << " ";
			//      HourFrac = 0.0
			Minutes = 0;
			TimeStepIndex = 0;
