...
namespace PlantCentralGSHP {
	...
	SizeWrapper( int const WrapperNum )
	{
		...
		if ( Wrapper( WrapperNum ).ControlMode == SmartMixing ) {
			for ( NumChillerHeater = 1; NumChillerHeater <= Wrapper( WrapperNum ).ChillerHeaterNums; ++NumChillerHeater ) {
				...
				if ( PltSizNum > 0 ) {
					...
					if ( PlantSizesOkayToFinalize ) {
						if ( IsAutoSize ) {
							...
							if ( Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).EvapVolFlowRate > 0.0 && tmpEvapVolFlowRate > 0.0 ) {
								...
								ReportSizingOutput( "ChillerHeaterPerformance:Electric:EIR", Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).Name, "Design Size Reference Chilled Water Flow Rate [m3/s]", tmpEvapVolFlowRate, "User-Specified Reference Chilled Water Flow Rate [m3/s]", EvapVolFlowRateUser );
					...
					if ( IsAutoSize ) {
						...
						if ( Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).EvapVolFlowRate > 0.0 ) {
							ReportSizingOutput( "ChillerHeaterPerformance:Electric:EIR", Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).Name, "User-Specified Reference Chilled Water Flow Rate [m3/s]", Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).EvapVolFlowRate );
						}
					}
				}
				...
				IsAutoSize = false;
				if ( Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).RefCapCooling == AutoSize ) {
					IsAutoSize = true;
				}
				if ( PltSizNum > 0 ) {
					if ( PlantSizData( PltSizNum ).DesVolFlowRate >= SmallWaterVolFlow && tmpEvapVolFlowRate > 0.0 ) {
						Cp = GetSpecificHeatGlycol( PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidName, InitConvTemp, PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidIndex, "SizeCGSHPChillerHeater" );

						rho = GetDensityGlycol( PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidName, InitConvTemp, PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidIndex, "SizeCGSHPChillerHeater" );
					...
					if ( PlantSizesOkayToFinalize ) {
						if ( IsAutoSize ) {
							...
							Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).RefCapClgHtg = Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).RefCapCooling * Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).ClgHtgToCoolingCapRatio;
						} else {
							if ( Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).RefCapCooling > 0.0 && tmpNomCap > 0.0 ) {
								...
								ReportSizingOutput( "ChillerHeaterPerformance:Electric:EIR", Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).Name, "Design Size Reference Capacity [W]", tmpNomCap, "User-Specified Reference Capacity [W]", NomCapUser );
					...
					if ( IsAutoSize ) {
						...
						if ( Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).RefCapCooling > 0.0 ) {
							ReportSizingOutput( "ChillerHeaterPerformance:Electric:EIR", Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).Name, "User-Specified Reference Capacity [W]", Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).RefCapCooling );
						}
					}
				}
				...
				if ( PltSizCondNum > 0 ) {
					if ( PlantSizData( PltSizNum ).DesVolFlowRate >= SmallWaterVolFlow ) {
						rho = GetDensityGlycol( PlantLoop( Wrapper( WrapperNum ).GLHELoopNum ).FluidName, InitConvTemp, PlantLoop( Wrapper( WrapperNum ).GLHELoopNum ).FluidIndex, "SizeCGSHPChillerHeater" );
						Cp = GetSpecificHeatGlycol( PlantLoop( Wrapper( WrapperNum ).GLHELoopNum ).FluidName, Wrapper( WrapperNum ).ChillerHeater( NumChillerHeater ).TempRefCondIn, PlantLoop( Wrapper( WrapperNum ).GLHELoopNum ).FluidIndex, "SizeCGSHPChillerHeater" );
	...
	)
	{
		...
		if ( MyWrapperEnvrnFlag( WrapperNum ) && BeginEnvrnFlag && ( PlantSizesOkayToFinalize ) ) {
			...
			if ( Wrapper( WrapperNum ).ControlMode == SmartMixing ) {
				...
				rho = GetDensityGlycol( PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidName, InitConvTemp, PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidIndex, "InitCGSHPHeatPump" );
	...
	)
	{
		...
		Real64 EvaporatorLoad; // Cooling load evaporator should meet
		Real64 HeatingPower; // Electric power use for heating
		Real64 CHWInletMassFlowRate; // Chilled water inlet mass flow rate
		Real64 CurAvailCHWMassFlowRate; // Maximum available mass flow rate for current chiller heater
		...
		for ( ChillerHeaterNum = 1; ChillerHeaterNum <= Wrapper( WrapperNum ).ChillerHeaterNums; ++ChillerHeaterNum ) {
			...
			if ( EvaporatorLoad > 0.0 && ( GetCurrentScheduleValue( Wrapper( WrapperNum ).WrapperComp( CompNum ).CHSchedPtr ) > 0.0 ) ) {
				...
				InitDensity = GetDensityGlycol( PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidName, InitConvTemp, PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidIndex, "CalcChillerHeaterModel" );
				EvapDensity = GetDensityGlycol( PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidName, EvapInletTemp, PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidIndex, "CalcChillerHeaterModel" );
				CondDensity = GetDensityGlycol( PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidName, CondInletTemp, PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidIndex, "CalcChillerHeaterModel" );
			...
			if ( IsLoadCoolRemaining && ( EvapMassFlowRate > 0 ) && ( GetCurrentScheduleValue( Wrapper( WrapperNum ).WrapperComp( CompNum ).CHSchedPtr ) > 0 ) ) {
				...
				Cp = GetSpecificHeatGlycol( PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidName, EvapInletTemp, PlantLoop( Wrapper( WrapperNum ).CWLoopNum ).FluidIndex, "CalcChillerHeaterModel" );
	...
	)
	{
		...
		Real64 CurAvailHWMassFlowRate; // Maximum available hot water mass within the wrapper bank
		...
		for ( ChillerHeaterNum = 1; ChillerHeaterNum <= Wrapper( WrapperNum ).ChillerHeaterNums; ++ChillerHeaterNum ) {
			...
			if ( IsLoadHeatRemaining && CondMassFlowRate > 0.0 && ( GetCurrentScheduleValue( Wrapper( WrapperNum ).WrapperComp( CompNum ).CHSchedPtr ) > 0 ) ) { // System is on
				...
				if ( SimulClgDominant || CurrentMode == 3 ) {
					CurrentMode = 3;
					Cp = GetSpecificHeatGlycol( PlantLoop( Wrapper( WrapperNum ).HWLoopNum ).FluidName, CondInletTemp, PlantLoop( Wrapper( WrapperNum ).HWLoopNum ).FluidIndex, "CalcChillerHeaterModel" );
					...
					Cp = GetSpecificHeatGlycol( PlantLoop( Wrapper( WrapperNum ).HWLoopNum ).FluidName, Wrapper( WrapperNum ).ChillerHeater( ChillerHeaterNum ).EvapInletNode.Temp, PlantLoop( Wrapper( WrapperNum ).HWLoopNum ).FluidIndex, "CalcChillerHeaterModel" );
					...
					Cp = GetSpecificHeatGlycol( PlantLoop( Wrapper( WrapperNum ).HWLoopNum ).FluidName, CondInletTemp, PlantLoop( Wrapper( WrapperNum ).HWLoopNum ).FluidIndex, "CalcElectricEIRChillerModel" );
	...
	)
	{
		...
		Real64 WrapperHeatRate; // Chiller heater bank total heating rate [W]
		Real64 WrapperGLHERate; // Chiller heater bank total condenser heat transfer rate [W]
		Real64 WrapperElecEnergyCool; // Chiller heater bank total electric cooling energy [J]
		Real64 WrapperElecEnergyHeat; // Chiller heater bank total electric heating energy [J]
		Real64 WrapperCoolEnergy; // Chiller heater bank total cooling energy [J]
		Real64 WrapperHeatEnergy; // Chiller heater bank total heating energy [J]
		Real64 WrapperGLHEEnergy; // Chiller heater bank total condenser heat transfer energy [J]
