...
namespace HWBaseboardRadiator {
	...
	Fstring const cCMO_BBRadiator_Water( "ZoneHVAC:Baseboard:RadiantConvective:Water" );
	...
	)
	{
		...
		if ( CompIndex == 0 ) {
			...
			if ( BaseboardNum == 0 ) {
				ShowFatalError( "SimHWBaseboard: Unit not found=" + trim( EquipName ) );
			}
			CompIndex = BaseboardNum;
		} else {
			BaseboardNum = CompIndex;
			if ( BaseboardNum > NumHWBaseboards || BaseboardNum < 1 ) {
				ShowFatalError( "SimHWBaseboard:  Invalid CompIndex passed=" + trim( TrimSigDigits( BaseboardNum ) ) + ", Number of Units=" + trim( TrimSigDigits( NumHWBaseboards ) ) + ", Entered Unit name=" + trim( EquipName ) );
			}
			if ( CheckEquipName( BaseboardNum ) ) {
				if ( EquipName != HWBaseboard( BaseboardNum ).EquipID ) {
					ShowFatalError( "SimHWBaseboard: Invalid CompIndex passed=" + trim( TrimSigDigits( BaseboardNum ) ) + ", Unit name=" + trim( EquipName ) + ", stored Unit Name for that index=" + trim( HWBaseboard( BaseboardNum ).EquipID ) );
				}
		...
		if ( CompIndex > 0 ) {
			...
			if ( SELECT_CASE_var == TypeOf_Baseboard_Rad_Conv_Water ) { // 'ZoneHVAC:Baseboard:RadiantConvective:Water'
				...
				ShowSevereError( "SimBaseboard: Errors in Baseboard=" + trim( HWBaseboard( BaseboardNum ).EquipID ) );
				ShowContinueError( "Invalid or unimplemented equipment type=" + trim( TrimSigDigits( HWBaseboard( BaseboardNum ).EquipType ) ) );
	...
	GetHWBaseboardInput()
	{
		...
		// SUBROUTINE PARAMETER DEFINITIONS:
		static Fstring const RoutineName( "GetHWBaseboardInput:" );
		...
		for ( BaseboardNum = 1; BaseboardNum <= NumHWBaseboards; ++BaseboardNum ) {
			...
			if ( lAlphaFieldBlanks( 2 ) ) {
				...
				if ( HWBaseboard( BaseboardNum ).SchedPtr == 0 ) {
					ShowSevereError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cAlphaFieldNames( 2 ) ) + "=\"" + trim( cAlphaArgs( 2 ) ) + "\" not found." );
					ErrorsFound = true;
				}
			}
			...
			if ( HWBaseboard( BaseboardNum ).WaterTempAvg > MaxWaterTempAvg + 0.001 ) {
				ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cNumericFieldNames( 1 ) ) + " was higher than the allowable maximum." );
				ShowContinueError( "...reset to maximum value=[" + trim( RoundSigDigits( MaxWaterTempAvg, 2 ) ) + "]." );
				HWBaseboard( BaseboardNum ).WaterTempAvg = MaxWaterTempAvg;
			} else if ( HWBaseboard( BaseboardNum ).WaterTempAvg < MinWaterTempAvg - 0.001 ) {
				ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cNumericFieldNames( 1 ) ) + " was lower than the allowable minimum." );
				ShowContinueError( "...reset to minimum value=[" + trim( RoundSigDigits( MinWaterTempAvg, 2 ) ) + "]." );
			...
			if ( HWBaseboard( BaseboardNum ).WaterMassFlowRateStd < LowWaterMassFlowRate - 0.0001 || HWBaseboard( BaseboardNum ).WaterMassFlowRateStd > HighWaterMassFlowRate + 0.0001 ) {
				ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cNumericFieldNames( 2 ) ) + " is an invalid Standard Water mass flow rate." );
				ShowContinueError( "...reset to a default value=[" + trim( RoundSigDigits( WaterMassFlowDefault, 1 ) ) + "]." );
			...
			if ( std::abs( HWBaseboard( BaseboardNum ).WaterVolFlowRateMax ) <= MinWaterFlowRate ) {
				ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cNumericFieldNames( 4 ) ) + " was less than the allowable minimum." );
				ShowContinueError( "...reset to minimum value=[" + trim( RoundSigDigits( MinWaterFlowRate, 2 ) ) + "]." );
				...
				ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cNumericFieldNames( 4 ) ) + " was higher than the allowable maximum." );
				ShowContinueError( "...reset to maximum value=[" + trim( RoundSigDigits( MaxWaterFlowRate, 2 ) ) + "]." );
			...
			if ( HWBaseboard( BaseboardNum ).Offset <= 0.0 ) {
				ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cNumericFieldNames( 5 ) ) + " was less than the allowable minimum." );
				ShowContinueError( "...reset to a default value=[" + trim( RoundSigDigits( MaxFraction, 2 ) ) + "]." );
			...
			if ( HWBaseboard( BaseboardNum ).FracRadiant < MinFraction ) {
				ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cNumericFieldNames( 6 ) ) + " was lower than the allowable minimum." );
			...
			if ( HWBaseboard( BaseboardNum ).FracRadiant > MaxFraction ) {
				ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cNumericFieldNames( 6 ) ) + " was higher than the allowable maximum." );
			...
			if ( AllFracsSummed > MaxFraction ) {
				ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", Fraction Radiant was higher than the allowable maximum." );
				HWBaseboard( BaseboardNum ).FracRadiant = MaxFraction;
			...
			if ( HWBaseboard( BaseboardNum ).FracDistribPerson < MinFraction ) {
				ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cNumericFieldNames( 7 ) ) + " was lower than the allowable minimum." );
			...
			if ( HWBaseboard( BaseboardNum ).FracDistribPerson > MaxFraction ) {
				ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cNumericFieldNames( 7 ) ) + " was higher than the allowable maximum." );
			...
			if ( ( HWBaseboard( BaseboardNum ).TotSurfToDistrib < MinDistribSurfaces ) && ( HWBaseboard( BaseboardNum ).FracRadiant > MinFraction ) ) {
				ShowSevereError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", the number of surface/radiant fraction groups entered was less than the allowable minimum." );
			...
			for ( SurfNum = 1; SurfNum <= HWBaseboard( BaseboardNum ).TotSurfToDistrib; ++SurfNum ) {
				...
				if ( HWBaseboard( BaseboardNum ).SurfacePtr( SurfNum ) == 0 ) {
					ShowSevereError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cAlphaFieldNames( SurfNum + 4 ) ) + "=\"" + trim( cAlphaArgs( SurfNum + 4 ) ) + "\" invalid - not found." );
					ErrorsFound = true;
				}
				if ( HWBaseboard( BaseboardNum ).FracDistribToSurf( SurfNum ) > MaxFraction ) {
					ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cNumericFieldNames( SurfNum + 7 ) ) + "was greater than the allowable maximum." );
					ShowContinueError( "...reset to maximum value=[" + trim( RoundSigDigits( MaxFraction, 2 ) ) + "]." );
					HWBaseboard( BaseboardNum ).TotSurfToDistrib = MaxFraction;
				}
				if ( HWBaseboard( BaseboardNum ).FracDistribToSurf( SurfNum ) < MinFraction ) {
					ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", " + trim( cNumericFieldNames( SurfNum + 7 ) ) + "was less than the allowable minimum." );
			...
			if ( AllFracsSummed > ( MaxFraction + 0.01 ) ) {
				ShowSevereError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", Summed radiant fractions for people + surface groups > 1.0" );
				ErrorsFound = true;
			}
			if ( ( AllFracsSummed < ( MaxFraction - 0.01 ) ) && ( HWBaseboard( BaseboardNum ).FracRadiant > MinFraction ) ) { // User didn't distribute all of the | radiation warn that some will be lost
				ShowWarningError( RoutineName + cCMO_BBRadiator_Water + "=\"" + trim( cAlphaArgs( 1 ) ) + "\", Summed radiant fractions for people + surface groups < 1.0" );
	...
	)
	{
		...
		if ( ! ZoneEquipmentListChecked && ZoneEquipInputsFilled ) {
			...
			for ( Loop = 1; Loop <= NumHWBaseboards; ++Loop ) {
				...
				ShowSevereError( "InitBaseboard: Unit=[" + trim( cCMO_BBRadiator_Water ) + "," + trim( HWBaseboard( Loop ).EquipID ) + "] is not on any ZoneHVAC:EquipmentList.  It will not be simulated." );
			}
		}
	...
	SizeHWBaseboard( int const BaseboardNum )
	{
		...
		if ( PltSizHeatNum > 0 ) {
			if ( CurZoneEqNum > 0 ) {
						...
						if ( HWBaseboard( BaseboardNum ).WaterVolFlowRateMax > 0.0 && WaterVolFlowRateMaxDes > 0.0 ) {
							...
							if ( DisplayExtraWarnings ) {
								if ( ( std::abs( WaterVolFlowRateMaxDes - WaterVolFlowRateMaxUser ) / WaterVolFlowRateMaxUser ) > AutoVsHardSizingThreshold ) {
									ShowMessage( "SizeHWBaseboard: Potential issue with equipment sizing for " "ZoneHVAC:Baseboard:RadiantConvective:Water=\"" + trim( HWBaseboard( BaseboardNum ).EquipID ) + "\"." );
				...
				if ( DesCoilLoad >= SmallLoad ) {
					...
					if ( AirOutletTempStd >= WaterInletTempStd ) {
						ShowSevereError( "SizeHWBaseboard: ZoneHVAC:Baseboard:RadiantConvective:Water=\"" + trim( HWBaseboard( BaseboardNum ).EquipID ) + "\"." );
						ShowContinueError( "...Air Outlet temperature must be below the Water Inlet temperature" );
						ShowContinueError( "...Air Outlet Temperature=[" + trim( RoundSigDigits( AirOutletTempStd, 2 ) ) + "], Water Inlet Temperature=[" + trim( RoundSigDigits( WaterInletTempStd, 2 ) ) + "]." );
						AirOutletTempStd = WaterInletTempStd - 0.01;
						ShowContinueError( "...Air Outlet Temperature set to [" + trim( RoundSigDigits( AirOutletTempStd, 2 ) ) + "]." );
					}
					// Check Tw,out < Ta,in
					if ( AirInletTempStd >= WaterOutletTempStd ) {
						ShowSevereError( "SizeHWBaseboard: ZoneHVAC:Baseboard:RadiantConvective:Water=\"" + trim( HWBaseboard( BaseboardNum ).EquipID ) + "\"." );
						ShowContinueError( "...Water Outlet temperature must be below the Air Inlet temperature" );
						ShowContinueError( "...Air Inlet Temperature=[" + trim( RoundSigDigits( AirInletTempStd, 2 ) ) + "], Water Outlet Temperature=[" + trim( RoundSigDigits( WaterOutletTempStd, 2 ) ) + "]." );
						WaterOutletTempStd = AirInletTempStd + 0.01;
						ShowContinueError( "...Water Outlet Temperature set to [" + trim( RoundSigDigits( WaterOutletTempStd, 2 ) ) + "]." );
					}
					// LMTD calculation
			...
			if ( HWBaseboard( BaseboardNum ).WaterVolFlowRateMax == AutoSize || HWBaseboard( BaseboardNum ).RatedCapacity == AutoSize || HWBaseboard( BaseboardNum ).RatedCapacity == 0.0 ) {
				...
				ShowContinueError( "Occurs in Hot Water Baseboard Heater=" + trim( HWBaseboard( BaseboardNum ).EquipID ) );
				ErrorsFound = true;
			}
			...
			for ( RadSurfNum = 1; RadSurfNum <= HWBaseboard( BaseboardNum ).TotSurfToDistrib; ++RadSurfNum ) {
				...
				if ( Surface( SurfNum ).Area > SmallestArea ) {
					...
					if ( ThisSurfIntensity > MaxRadHeatFlux ) {
						...
						ShowContinueError( "Occurs in " + cCMO_BBRadiator_Water + " = " + trim( HWBaseboard( BaseboardNum ).EquipID ) );
	...
	)
	{
		...
		if ( CompIndex == 0 ) {
			...
			if ( BaseboardNum == 0 ) {
				ShowFatalError( "UpdateHWBaseboardPlantConnection: Specified baseboard not valid =" + trim( BaseboardName ) );
			}
			CompIndex = BaseboardNum;
		} else {
			BaseboardNum = CompIndex;
			if ( BaseboardNum > NumHWBaseboards || BaseboardNum < 1 ) {
				ShowFatalError( "UpdateHWBaseboardPlantConnection:  Invalid CompIndex passed=" + trim( TrimSigDigits( BaseboardNum ) ) + ", Number of baseboards=" + trim( TrimSigDigits( NumHWBaseboards ) ) + ", Entered baseboard name=" + trim( BaseboardName ) );
			}
			if ( KickOffSimulation ) {
				if ( BaseboardName != HWBaseboard( BaseboardNum ).EquipID ) {
					ShowFatalError( "UpdateHWBaseboardPlantConnection: Invalid CompIndex passed=" + trim( TrimSigDigits( BaseboardNum ) ) + ", baseboard name=" + trim( BaseboardName ) + ", stored baseboard Name for that index=" + trim( HWBaseboard( BaseboardNum ).EquipID ) );
				}
				if ( BaseboardTypeNum != TypeOf_Baseboard_Rad_Conv_Water ) {
					ShowFatalError( "UpdateHWBaseboardPlantConnection: Invalid CompIndex passed=" + trim( TrimSigDigits( BaseboardNum ) ) + ", baseboard name=" + trim( BaseboardName ) + ", stored baseboard Name for that index=" + trim( ccSimPlantEquipTypes( BaseboardTypeNum ) ) );
				}
			}
		}
