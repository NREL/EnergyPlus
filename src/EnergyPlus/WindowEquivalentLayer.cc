	...
	)
	{
		...
		if ( CalcCondition == noCondition ) {
			...
			if ( SurfNumAdj > 0 ) {
				...
				outir = SurfaceWindow( SurfNumAdj ).IRfromParentZone + QHTRadSysSurf( SurfNumAdj ) + QCoolingPanelSurf( SurfNumAdj ) + QHWBaseboardSurf( SurfNumAdj ) + QSteamBaseboardSurf( SurfNumAdj ) + QElecBaseboardSurf( SurfNumAdj ) + QRadThermInAbs( SurfNumAdj ) + QRadThermInAbs( SurfNumAdj );
