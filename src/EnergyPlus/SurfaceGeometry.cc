...
namespace EnergyPlus {
  ...
  namespace SurfaceGeometry {
	...
	for ( Loop = 1; Loop <= ItemsToGet; ++Loop ) {
	  ...
	  if ( TmpConstruction[ SurfNum  - 1 ] > 0 ) {
	    if ( SurfaceTmp( SurfNum ).Class == SurfaceClass_Wall && Construct( TmpConstruction[ SurfNum  - 1 ] ).TypeIsCfactorWall && SurfaceTmp( SurfNum ).ExtBoundCond == Ground ) {
	      SurfaceTmp( SurfNum ).ExtBoundCond = GroundFCfactorMethod;
	    } else if ( Construct( TmpConstruction[ SurfNum  - 1 ] ).TypeIsCfactorWall ) {  //SurfaceTmp( SurfNum ).Construction ).TypeIsCfactorWal						ErrorsFound = true;
	    ...
	    if ( SurfaceTmp( SurfNum ).Class == SurfaceClass_Floor && Construct( TmpConstruction[ SurfNum - 1 ] ).TypeIsFfactorFloor && SurfaceTmp( SurfNum ).ExtBoundCond == Ground ) {
	      SurfaceTmp( SurfNum ).ExtBoundCond = GroundFCfactorMethod;
	    } else if ( Construct( TmpConstruction[ SurfNum  - 1 ] ).TypeIsFfactorFloor ) {
	      ErrorsFound = true;
