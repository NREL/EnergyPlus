	...
	GetMTGeneratorInput()
	{
		...
		for ( GeneratorNum = 1; GeneratorNum <= NumMTGenerators; ++GeneratorNum ) {
			...
			if ( MTGenerator( GeneratorNum ).RefCombustAirInletHumRat <= 0.0 ) {
				...
				RefBaroPressure = 101325.0 * power( ( 1.0 - 2.25577e-05 * MTGenerator( GeneratorNum ).RefElevation ), 5.2559 );
	...
	)
	{
		...
		if ( MTGenerator( GeneratorNum ).ExhAirCalcsActive ) {
			...
			if ( ExhAirMassFlowRate <= 0.0 ) {
				...
				H2OHtOfVap = PsyHfgAirFnWTdb( 1.0, 16.0 ); // W not used, passing 1.0 as dummy. // *&^unique^&* "CalcMTGeneratorModel"
