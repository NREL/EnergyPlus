			...
			for ( FacadeNum = 1; FacadeNum <= 5; ++FacadeNum ) {
				...
				for ( WindDirNum = 1; WindDirNum <= 12; ++WindDirNum ) {
					...
					if ( SameString( AirflowNetworkSimu.BldgType, "LowRise" ) && FacadeNum <= 4 ) {
						...
						MultizoneCPValueData( FacadeNum ).CPValue( WindDirNum ) = 0.6 * std::log( 1.248 - 0.703 * std::sin( IncRad / 2. ) - 1.175 * pow2( std::sin( IncRad ) ) + 0.131 * pow3( std::sin( 2. * IncRad * SideRatioFac ) ) + 0.769 * std::cos( IncRad / 2. ) + 0.07 * pow2( ( SideRatioFac * std::sin( IncRad / 2. ) ) ) + 0.717 * pow2( std::cos( IncRad / 2. ) ) );
					}
	...
	CalcSingleSidedCps()
	{
		...
		for ( ZnNum = 1; ZnNum <= AirflowNetworkNumOfZones; ++ZnNum ) {
			if ( MultizoneZoneData( ZnNum ).SingleSidedCpType == "ADVANCED" ) {
				...
				Sprime( ZnNum ) = std::sqrt( pow2( std::abs( X1 - X2 ) ) + pow2( std::abs( Y1 - Y2 ) ) ) / MultizoneZoneData( ZnNum ).BuildWidth;
			...
			if ( MultizoneZoneData( ZnNum ).SingleSidedCpType == "ADVANCED" ) {
				...
				for ( ExtOpenNum = 1; ExtOpenNum <= AFNNumOfExtOpenings; ++ExtOpenNum ) {
					if ( AFNExtSurfaces( ExtOpenNum ).MZDZoneNum == ZnNum ) {
						...
						if ( OpenNuminZone == 1 ) {
							for ( WindDirNum = 1; WindDirNum <= MultizoneCPArrayData( 1 ).NumWindDir; ++WindDirNum ) {
								MultizoneCPValueDataTempUnMod( SrfNum ).CPValue( WindDirNum ) = MultizoneCPValueData( AFNExtSurfaces( ExtOpenNum ).CPVNum ).CPValue( WindDirNum ) + 0.5 * ( 1 / pow2( AFNExtSurfaces( ExtOpenNum ).DischCoeff ) ) * DeltaCp( ZnNum ).WindDir( WindDirNum );
								MultizoneCPValueDataTempUnMod( SrfNum ).Name = AFNExtSurfaces( ExtOpenNum ).SurfName;
								MultizoneCPValueDataTemp( SrfNum ).CPValue( WindDirNum ) = ( pow2( VelRatio ) ) * MultizoneCPValueDataTempUnMod( SrfNum ).CPValue( WindDirNum );
