...
namespace PlantPressureSystem {
	...
	)
	{
		...
		if ( NodeMassFlow > 0.0 ) {
			PlantLoop( LoopNum ).LoopSide( LoopSideNum ).Branch( BranchNum ).PressureEffectiveK = BranchDeltaPress / ( pow2( NodeMassFlow ) );
		} else {
			PlantLoop( LoopNum ).LoopSide( LoopSideNum ).Branch( BranchNum ).PressureEffectiveK = 0.0;
		}
	...
	UpdatePressureDrop( int const LoopNum )
	{
		...
		for ( LoopSideNum = DemandSide; LoopSideNum <= SupplySide; ++LoopSideNum ) {
			...
			if ( TempVal_SumOfOneByRootK > 0.0 ) EffectiveLoopSideKValue += ( 1.0 / ( pow2( TempVal_SumOfOneByRootK ) ) );
	...
	)
	{
		...
		SystemPressureDrop = LoopEffectiveK * pow2( SystemMassFlow );
		...
		for ( Iteration = 1; Iteration <= MaxIters; ++Iteration ) {
			...
			PumpPressureRise = PsiPump * NodeDensity * ( pow2( PumpSpeed ) ) * ( pow2( PumpImpellerDia ) );
