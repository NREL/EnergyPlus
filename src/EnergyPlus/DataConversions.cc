...
namespace DataConversions {
	...
	Real64 const DELTMP( -32. ); // Temperature:    (F + DELTMP) * CFT = C
	Real64 const CFA( CFL * CFL ); // Area:           FT**2 * CFA = M**2
	Real64 const CFT( 5. / 9. ); // Temperature:    R * CFT = K
	...
	Real64 const CFMF( CFM / 3600. ); // Mass Flow:      LB/HR * CFMF = KG/SEC
	Real64 const CFQ( CFE ); // Power:          BTU/HR * CFQ = W
	Real64 const CFU( CFK / CFL ); // U-Value:        BTU/(HR*FT**2*R) * CFU = W/(M**2*K)
	// Note:  R-Value = 1/U-Value
	Real64 const CFS( CFL / 60. ); // Speed:          FT/MIN * CFS = M/SEC
	Real64 const CFVF( CFV / 60. ); // Volume Flow:    FT**3/MIN * CFVF = M**3/SEC
