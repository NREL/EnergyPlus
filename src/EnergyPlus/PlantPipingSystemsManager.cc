...
namespace PlantPipingSystemsManager {
	...
	RadialCellInfo_XY_CrossSectArea( RadialCellInformation const & r )
	{
		...
		RetVal = 3.14159 * ( ( power( r.OuterRadius, 2 ) ) - ( power( r.InnerRadius, 2 ) ) );
	...
	)
	{
		...
		c.InterfaceVolume = ( 1.0 - ( 3.1415926535 / 4.0 ) ) * ( power( GridCellWidth, 2 ) ) * CellDepth;
	...
	)
	{
		...
		c.Volume = 3.1415926535 * ( power( m_PipeInnerRadius, 2 ) ) * m_CellDepth;
	...
	)
	{
		...
		if ( ThisMesh.MeshDistribution == MeshDistribution_Uniform ) {
			...
			for ( I = 1; I <= NumCellsOnEachSide; ++I ) {
				SummationTerm += power( ThisMesh.GeometricSeriesCoefficient, ( I - 1 ) );
			}
	...
	)
	{
		...
		X_sunset = 1.0 - power( std::tan( Latitude_Radians ), 2 ) * power( std::tan( Declination ), 2 );
		X_sunset = max( X_sunset, 0.000001 );

		// Find sunset angle
		Sunset_Angle = Pi / 2.0 - std::atan( -std::tan( Latitude_Radians ) * std::tan( Declination ) / power( X_sunset, 0.5 ) );
		...
		QRAD_NL = 2.042E-10 * power( CurAirTempK, 4 ) * ( 0.34 - 0.14 * std::sqrt( VaporPressureActual_kPa ) ) * ( 1.35 * Ratio_SO - 0.35 );
		...
		Slope_S = 2503.0 * std::exp( 17.27 * PipingSystemDomains( DomainNum ).Cur.CurAirTemp / ( PipingSystemDomains( DomainNum ).Cur.CurAirTemp + 237.3 ) ) / power( ( PipingSystemDomains( DomainNum ).Cur.CurAirTemp + 237.3 ), 2 );
		Pressure = 98.0;
	...
	)
	{
		...
		//Flow calculations
		Area_c = ( Pi / 4.0 ) * power( PipingSystemCircuits( CircuitNum ).PipeSize.InnerDia, 2 );
		...
		if ( Velocity > 0 ) {
			...
			Nusselt = 0.023 * power( Reynolds, ( 4.0 / 5.0 ) ) * power( Prandtl, ExponentTerm );
