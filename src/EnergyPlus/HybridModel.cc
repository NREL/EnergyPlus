	...
	CheckAndReadHybridModelZone()
	{
		...
		if (NumOfHybridModelZones > 0) {
			...
			for (i = 1; i <= NumOfHybridModelZones; ++i) {
				...
				HybridModelZone(i).ZonePtr = FindItemInList(cAlphaArgs(2), Zone);
				HybridModelZone( i ).InternalThermalMassCalc = cAlphaArgs(3);
				HybridModelZone( i ).InfiltrationCalc = cAlphaArgs(4);
				HybridModelZone( i ).ZoneMeasuredTemperatureSchedulePtr = GetScheduleIndex(cAlphaArgs(5));
				HybridModelZone( i ).ZoneMeasuredTemperatureStartMonth = rNumericArgs(1);
				HybridModelZone( i ).ZoneMeasuredTemperatureStartDate = rNumericArgs(2);
				HybridModelZone( i ).ZoneMeasuredTemperatureEndMonth = rNumericArgs(3);
				HybridModelZone( i ).ZoneMeasuredTemperatureEndDate = rNumericArgs(4);
