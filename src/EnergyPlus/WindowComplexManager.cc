...
namespace WindowComplexManager {
		...
		BTW: if ( Construct( IConst ).BSDFInput.BasisType == BasisType_WINDOW ) {
			...
			BST: if ( Construct( IConst ).BSDFInput.BasisSymmetryType == BasisSymmetry_None ) {
				...
				ThLoop: for ( I = 1; I <= NThetas; ++I ) {
					...
					DPhi = 2.0 * Pi / NPhis( I );
					if ( I == 1 ) {
						Lamda = Pi * power( ( std::sin( UpperTheta ) ), 2 );
						SolAng = 2.0 * Pi * ( 1.0 - std::cos( UpperTheta ) );
					} else {
						Lamda = 0.5 * DPhi * ( power( ( std::sin( UpperTheta ) ), 2 ) - power( ( std::sin( LowerTheta ) ), 2 ) ); //For W6 basis, lamda is funct of Theta and
	...
	)
	{
		...
		if ( SELECT_CASE_var == Front_Incident ) {
			...
			Cost = -RdotZ;
			Sint = std::sqrt( 1.0 - power( Cost, 2 ) );
			Theta = std::acos( Cost );
	...
	)
	{
		...
		if ( CalcCondition == noCondition ) {
			...
			if ( SELECT_CASE_var == ZoneMeanAirTemp ) {
				...
				for ( NodeNum = 1; NodeNum <= ZoneEquipConfig( ZoneEquipConfigNum ).NumInletNodes; ++NodeNum ) {
					...
					CpAir = PsyCpAirFnWTdb( ZoneAirHumRat( ZoneNum ), NodeTemp ); // *&^unique^&* "CalcComplexWindowThermal"
			...
			if ( SurfNumAdj > 0 ) { // Interzone window
				...
				if ( SELECT_CASE_var == ZoneMeanAirTemp ) {
					...
					for ( NodeNum = 1; NodeNum <= ZoneEquipConfig( ZoneEquipConfigNum ).NumInletNodes; ++NodeNum ) {
						NodeTemp = Node( ZoneEquipConfig( ZoneEquipConfigNum ).InletNode( NodeNum ) ).Temp;
						MassFlowRate = Node( ZoneEquipConfig( ZoneEquipConfigNum ).InletNode( NodeNum ) ).MassFlowRate;
						CpAir = PsyCpAirFnWTdb( ZoneAirHumRat( ZoneNumAdj ), NodeTemp ); // *&^unique^&* "CalcComplexWindowThermal"
						SumSysMCp += MassFlowRate * CpAir;
						SumSysMCpT += MassFlowRate * CpAir * NodeTemp;
					}
					// a weighted average of the inlet temperatures.
				...
				outir = Surface( SurfNum ).ViewFactorSkyIR * ( AirSkyRadSplit( SurfNum ) * sigma * power( tsky, 4 ) + ( 1.0 - AirSkyRadSplit( SurfNum ) ) * Ebout ) + Surface( SurfNum ).ViewFactorGroundIR * Ebout;
		...
		if ( ( nperr > 0 ) && ( nperr < 1000 ) ) { // process error signal from tarcog
			...
			if ( ShadeFlag == IntShadeOn || ShadeFlag == IntBlindOn ) {
				...
				NetIRHeatGainShade = ShadeArea * EpsShIR2 * ( sigma * power( theta( nglfacep ), 4 ) - rmir ) + EpsShIR1 * ( sigma * power( theta( nglfacep - 1 ), 4 ) - rmir ) * RhoGlIR2 * TauShIR / ShGlReflFacIR;
				NetIRHeatGainGlass = ShadeArea * ( emis( 2 * ngllayer ) * TauShIR / ShGlReflFacIR ) * ( sigma * power( theta( 2 * ngllayer ), 4 ) - rmir );
				...
				NetIRHeatGainGlass = Surface( SurfNum ).Area * emis( 2 * nlayer ) * ( sigma * power( theta( 2 * nlayer ), 4 ) - rmir );
