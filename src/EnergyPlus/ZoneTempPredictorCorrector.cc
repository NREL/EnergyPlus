...
namespace ZoneTempPredictorCorrector {
	...
	)
	{
		...
		for ( ZoneNum = 1; ZoneNum <= NumOfZones; ++ZoneNum ) {
			...
			AIRRAT( ZoneNum ) = Zone( ZoneNum ).Volume * ZoneVolCapMultpSens * PsyRhoAirFnPbTdbW( OutBaroPress, MAT( ZoneNum ), ZoneAirHumRat( ZoneNum ), "CorrectZoneAirTemp" ) * PsyCpAirFnWTdb( ZoneAirHumRat( ZoneNum ), MAT( ZoneNum ) ) / ( TimeStepSys * SecInHour );
	...
	)
	{
		...
		ControlledZoneAirFlag = false;
		for ( ZoneEquipConfigNum = 1; ZoneEquipConfigNum <= NumOfZones; ++ZoneEquipConfigNum ) {
			if ( ! ZoneEquipConfig( ZoneEquipConfigNum ).IsControlled ) continue;
			if ( ZoneEquipConfig( ZoneEquipConfigNum ).ActualZoneNum != ZoneNum ) continue;
			ControlledZoneAirFlag = true;
			break;
		} // ZoneEquipConfigNum
	...
	VerifyControlledZoneForThermostat( std::string const & ZoneName ) // Zone to verify
	{
		...
		// Return value
		bool IsControlled; // True if does, false if not.
		...
		if ( FindItemInList( ZoneName, ZoneEquipConfig.ZoneName(), NumOfZones ) > 0 ) {
			IsControlled = true;
		} else {
			IsControlled = false;
		}
	...
	PMVResidual(
		Real64 const Tset,
		Optional< FArray1S< Real64 > const > Par // par(1) = PMV set point
	)
	{
		// FUNCTION INFORMATION:
