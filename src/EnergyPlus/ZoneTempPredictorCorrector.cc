...
namespace ZoneTempPredictorCorrector {
	...
	)
	{
		...
		if ( ControlledZoneAirFlag ) { // If there is system flow then calculate the flow rates
			...
			ExhMassFlowRate -= ZoneEquipConfig( ZoneEquipConfigNum ).ZoneExhBalanced; // Balanced exhaust flow assumes there are other flows providing makeup air such as mixing or infiltration
