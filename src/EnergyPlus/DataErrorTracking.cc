...
namespace DataErrorTracking {
	...
	FArray1D_Fstring const MessageSearch( SearchCounts, sFstring( 23 ), { "InterZone Surface Areas", "CAUTION -- Interzone   ", "Node Connection Error  ", "InterZone Surface Azimu", "InterZone Surface Tilts", "Suspected non-planar   ", "Deprecated             ", "Floor Tilt=            ", "Roof/Ceiling Tilt=     ", "View factors not       ", "Unbalanced exhaust air ", "Loads Initialization   ", "CalcDaylightMapPoints: ", "Zone Air Heat Balance  ", "occupant density is ext", "Temperature (low) out o", "Temperature (high) out ", "nominally unused       ", "InfraredTransparent    ", "No reporting elements  " } );
	FArray1D_Fstring const Summaries( SearchCounts, sFstring( 38 ), { "InterZone Surface Areas -- mismatch   ", "Interzone surfaces - different zones  ", "Node Connection Errors                ", "InterZone Surface Azimuths -- mismatch", "InterZone Surface Tilts -- mismatch   ", "Likely non-planar surfaces            ", "Deprecated Features or Key Values     ", "Incorrect Floor Tilt                  ", "Incorrect Roof/Ceiling Tilt           ", "Incomplete View factors               ", "Unbalanced exhaust air flow           ", "Loads Initialization did not Converge ", "CalcDaylightMapPoints: Window         ", "Zone Air Heat Balance Warnings        ", "Occupant density is extremely high    ", "Temperature (low) out of bounds       ", "Temperature (high) out of bounds      ", "Nominally Unused Constructions        ", "Material:InfraredTransparent usage    ", "No Reporting Elements requested       " } );
	// in below -- simple line end <CR>.  End of Whole message <CRE>
	int const iDTEstrLeng( 693 );
	Fstring const MoreDetails_1( iDTEstrLeng, "Area mismatch errors happen when the interzone surface in zone A is<CR>" "not the same size as it's companion in zone B.<CRE>" ); // InterZone Surface Areas -- mismatch
	Fstring const MoreDetails_2( iDTEstrLeng ); // Interzone surfaces - different zones
	Fstring const MoreDetails_3( iDTEstrLeng, "Node connection errors are often caused by spelling mistakes in a node name field.<CR>" "To track down the problem, search the idf file for each node name listed to see if it<CR>" "occurs in the expected input fields.<CRE>" ); // Node Connection Errors
	Fstring const MoreDetails_4( iDTEstrLeng, "The azimuths (outward facing angle) of two interzone surfaces should not be the same.<CR>" "Normally, the absolute difference between the two azimuths will be 180 degrees.<CR>" "You can turn on the report: Output:Surfaces:List,Details; to inspect your surfaces.<CRE>" ); // InterZone Surface Azimuths -- mismatch
	Fstring const MoreDetails_5( iDTEstrLeng ); // InterZone Surface Tilts -- mismatch
	Fstring const MoreDetails_6( iDTEstrLeng, "EnergyPlus Surfaces should be planar. If the error indicates a small increment for the<CR>" "out of planar bounds, then the calculations are likely okay though you should try to fix<CR>" "the problem. If a greater increment, the calculations will likely be incorrect.<CRE>" ); // Likely non-planar surfaces
	Fstring const MoreDetails_7( iDTEstrLeng, "A deprecated feature warning/severe error indicates that you are using a feature which will be<CR>" "removed in a future release. The new feature is likely included in the EnergyPlus version you are<CR>" "using.  Consider switching now to avoid future problems.<CR>" "A deprecated key value message indicates you are using an out-dated key value in your input file.<CR>" "While EnergyPlus may continue to accept these values, some other input file readers may not.<CR>" "Consider changing to values that are included as valid in the Energy+.idd for these objects.<CRE>" ); // Deprecated Features or Key Values
	Fstring const MoreDetails_8( iDTEstrLeng, "Floors are usually flat and \"tilted\" 180 degrees.  If you get this error message,<CR>" "it's likely that you need to reverse the vertices of the surface to remove the error.<CR>" "EnergyPlus will attempt to fix the vertices for the running simulation.<CR>" "You can turn on the report: Output:Surfaces:List,Details; to inspect your surfaces.<CRE>" ); // Incorrect Floor Tilt
	Fstring const MoreDetails_9( iDTEstrLeng, "Flat roofs/ceilings are \"tilted\" 0 degrees. Pitched roofs should be \"near\" 0 degrees.<CR>" "If you get this error message, it's likely that you need to reverse the vertices of <CR>" "the surface to remove the error. EnergyPlus will attempt to fix the vertices for the <CR\"" "running simulation. You can turn on the report: Output:Surfaces:List,Details; <CR>" "to inspect your surfaces.<CRE>" ); // Incorrect Roof/Ceiling Tilt
	Fstring const MoreDetails_10( iDTEstrLeng, "Incomplete view factors can result from incorrect floor specifications (such as tilting 0<CR>" "instead of 180) or not enough surfaces in a zone to make an enclosure.  The error message<CR>" "also shows an enforced repciprocity value.  You can decide if you need to make geometry<CR>" "changes based on that value.<CRE>" ); // Incomplete View factors
	Fstring const MoreDetails_11( iDTEstrLeng, "Unbalanced exhaust air flow errors can occur when exhaust fans are running but there is no<CR>" "supply air. Turn off exhaust fans when the system is not running may help resolve the problem.<CR>" "Time shown is first occurrence of error.<CRE>" ); // Unbalanced exhaust air flow
	Fstring const MoreDetails_12( iDTEstrLeng, "1) very high thermal mass such as very thick concrete (solution: increase max number of warmup<CR>" "   days in the BUILDING object);<CR>" "2) moderate mass and inadequate space conditioning such that the building keeps getting warmer<CR>" "   and warmer on successive days (solution: add HVAC, check building thermal properties,<CR>" "   check if infiltration is included, make sure HVAC properly controlled);<CR>" "3) a soil layer modeled below the concrete slab - (solution remove this layer and read about<CR>" "   ground temperatures in the Auxiliary Programs document).<CR>" "4) unreasonable (too small) limits in the BUILDING object for temperature (.4 default) or<CR>" "   loads tolerances (.04 default)<CRE>" ); // Loads Initialization did not Converge
	Fstring const MoreDetails_13( iDTEstrLeng, "Window is too close to map points for accurate calculation.  Suggested change is to create<CR>" "Output:IlluminanceMap coordinates (x,y,z) that are more \"inside\" the zone<CRE>" ); // CalcDaylightMapPoints: Window
	Fstring const MoreDetails_14( iDTEstrLeng, "Zone Air Heat Balance out of Balance warnings are currently used by developers.<CR>" "Users can safely ignore these warnings.<CRE>" ); // Zone Air Heat Balance Warnings
	Fstring const MoreDetails_15( iDTEstrLeng, "The occupant density warning is provided to alert you to potential conditions that can cause<CR>" "problems with the heat balance calculations. Too high a density could be cause for severe<CR>" "temperature out of bounds errors in a zone leading to program termination.<CRE>" ); // Occupant density is extremely high
	Fstring const MoreDetails_16( iDTEstrLeng, "A temperature out of bounds problem can be caused by several things. The user should check:<CR>" "1) the weather environment (including the horizontal IR from sky)<CR>" "2) the level of interal gains with respect to the zone<CR>" "3) the thermal properties of their materials.  And other things.<CR>" "A common cause is a building with no or little thermal mass - all materials with Material:NoMass" "definitions.<CRE>" ); // Temperature (low) out of bounds AND Temperature (high) out of bounds
	Fstring const MoreDetails_18( iDTEstrLeng, "The nominally unused constructions warning is provided to alert you to potential conditions" " that can cause<CR>" "extra time during simulation. Each construction is calculated by the algorithm indicated in the " "HeatBalanceAlgorithm<CR>" "object. You may remove the constructions indicated (when you use the DisplayExtraWarnings option).<CRE>" ); // Nominally unused constructions
	Fstring const MoreDetails_19( iDTEstrLeng, "Using Material:InfraredTransparent materials in constructions are correctly used " "in interzone surface<CR>" "constructions. Warnings are given if they are used in other kinds of surfaces.<CR>" "They CANNOT currently be used with ConductionFiniteDifference algorithms.<CRE>" ); // InfraredTransparent constructions in non-interzone surfaces
	Fstring const MoreDetails_20( iDTEstrLeng, "No Reporting elements have been requested. You will see no output values from your run.<CR>" "Add Output:Variable, Output:Meter, Output:Table:SummaryReports, Output:Table:Monthly, " "Output:Table:TimeBins<CR>" "objects to your input file to receive output values from the simulation.<CRE>" ); // No reporting elements requested
	FArray1D_Fstring const MoreDetails( SearchCounts, sFstring( iDTEstrLeng ), { MoreDetails_1, MoreDetails_2, MoreDetails_3, MoreDetails_4, MoreDetails_5, MoreDetails_6, MoreDetails_7, MoreDetails_8, MoreDetails_9, MoreDetails_10, MoreDetails_11, MoreDetails_12, MoreDetails_13, MoreDetails_14, MoreDetails_15, MoreDetails_16, MoreDetails_16, MoreDetails_18, MoreDetails_19, MoreDetails_20 } ); // Details 16 applies to both temperature out of bounds | errors.
