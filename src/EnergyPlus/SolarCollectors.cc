...
namespace SolarCollectors {
	...
	InitSolarCollector( int const CollectorNum )
	{
		...
		if ( SetDiffRadFlag( CollectorNum ) && Collector( CollectorNum ).InitICS ) {
			...
			Theta = ( 59.68 - 0.1388 * Tilt + 0.001497 * second_power( Tilt ) ) * DegToRadians;
			CalcTransRefAbsOfCover( CollectorNum, Theta, TransSys, RefSys, AbsCover1, AbsCover2 );
			...
			Theta = ( 90.0 - 0.5788 * Tilt + 0.002693 * second_power( Tilt ) ) * DegToRadians;
			CalcTransRefAbsOfCover( CollectorNum, Theta, TransSys, RefSys, AbsCover1, AbsCover2 );
	...
	CalcSolarCollector( int const CollectorNum )
	{
		...
		if ( QRadSWOutIncident( SurfNum ) > 0.0 ) {
			...
			Tilt = Surface( SurfNum ).Tilt;
			ThetaSky = ( 59.68 - 0.1388 * Tilt + 0.001497 * second_power( Tilt ) ) * DegToRadians;
			ThetaGnd = ( 90.0 - 0.5788 * Tilt + 0.002693 * second_power( Tilt ) ) * DegToRadians;
		...
		while ( std::abs( OutletTemp - OutletTempPrev ) > TempConvergTol ) { // Check for temperature convergence
			...
			if ( MassFlowRate > 0.0 ) { // Calculate efficiency and heat transfer with flow
				...
				C = -FRULT * second_power( Surface( SurfNum ).OutDryBulbTemp ) + FRUL * Surface( SurfNum ).OutDryBulbTemp - FRTAN * IncidentAngleModifier * QRadSWOutIncident( SurfNum );
				qEquation = ( second_power( B ) - 4.0 * A * C );
				if ( qEquation < 0.0 ) {
	...
	)
	{
		...
		if ( std::abs( IncidentAngle ) > CutoffAngle ) { // cut off, model curves not robust beyond cutoff
			...
			IAM = 1.0 + Parameters( ParamNum ).iam1 * s + Parameters( ParamNum ).iam2 * ( second_power( s ) );
	...
	)
	{
		...
		if ( AbsorberPlateHasMass ) {
			...
			b = -( a1 + b2 );
			c = a1 * b2 - a2 * b1;
			BSquareM4TimesATimesC = second_power( b ) - 4.0 * a * c;
	...
	)
	{
		...
		for ( nCover = 1; nCover <= Parameters( ParamNum ).NumOfCovers; ++nCover ) {
			...
			if ( IncAngle == 0.0 ) {
				ParaRad = second_power( ( ( CoverRefrIndex - AirRefIndex ) / ( CoverRefrIndex + AirRefIndex ) ) );
				PerpRad = second_power( ( ( CoverRefrIndex - AirRefIndex ) / ( CoverRefrIndex + AirRefIndex ) ) );
			} else {
				ParaRad = second_power( std::tan( RefrAngle - IncAngle ) ) / second_power( std::tan( RefrAngle + IncAngle ) );
				PerpRad = second_power( std::sin( RefrAngle - IncAngle ) ) / second_power( std::sin( RefrAngle + IncAngle ) );
			}

			// parallel and perpendicular transmitted components:
			TransPerp( nCover ) = TransAbsOnly( nCover ) * ( ( 1.0 - PerpRad ) / ( 1.0 + PerpRad ) ) * ( ( 1.0 - second_power( PerpRad ) ) / ( 1.0 - second_power( ( PerpRad * TransAbsOnly( nCover ) ) ) ) );
			TransPara( nCover ) = TransAbsOnly( nCover ) * ( ( 1.0 - ParaRad ) / ( 1.0 + ParaRad ) ) * ( ( 1.0 - second_power( ParaRad ) ) / ( 1.0 - second_power( ( ParaRad * TransAbsOnly( nCover ) ) ) ) );

			ReflPerp( nCover ) = ( PerpRad + ( ( second_power( ( 1.0 - PerpRad ) ) ) * ( second_power( TransAbsOnly( nCover ) ) ) * PerpRad ) / ( 1.0 - second_power( ( PerpRad * TransAbsOnly( nCover ) ) ) ) );
			ReflPara( nCover ) = ( ParaRad + ( ( second_power( ( 1.0 - ParaRad ) ) ) * ( second_power( TransAbsOnly( nCover ) ) ) * ParaRad ) / ( 1.0 - second_power( ( ParaRad * TransAbsOnly( nCover ) ) ) ) );
	...
	CalcHeatTransCoeffAndCoverTemp( int const ColleNum ) // Collector object number
	{
		...
		if ( SELECT_CASE_var == 1 ) {
			...
			tempnom = StefanBoltzmann * ( ( TempAbsPlate + KelvinConv ) + ( TempOuterCover + KelvinConv ) ) * ( second_power( ( TempAbsPlate + KelvinConv ) ) + second_power( ( TempOuterCover + KelvinConv ) ) );
			...
			for ( CoverNum = 1; CoverNum <= NumCovers; ++CoverNum ) {
				if ( CoverNum == 1 ) {
					// calc linearized radiation coefficient
					tempnom = StefanBoltzmann * ( ( TempAbsPlate + KelvinConv ) + ( TempInnerCover + KelvinConv ) ) * ( second_power( ( TempAbsPlate + KelvinConv ) ) + second_power( ( TempInnerCover + KelvinConv ) ) );
					...
					tempnom = StefanBoltzmann * ( ( TempInnerCover + KelvinConv ) + ( TempOuterCover + KelvinConv ) ) * ( second_power( ( TempInnerCover + KelvinConv ) ) + second_power( ( TempOuterCover + KelvinConv ) ) );
		...
		tempnom = Surface( SurfNum ).ViewFactorSky * EmissOfOuterCover * StefanBoltzmann * ( ( TempOuterCover + KelvinConv ) + SkyTempKelvin ) * ( second_power( ( TempOuterCover + KelvinConv ) ) + second_power( SkyTempKelvin ) );
		...
		tempnom = Surface( SurfNum ).ViewFactorGround * EmissOfOuterCover * StefanBoltzmann * ( ( TempOuterCover + KelvinConv ) + GroundTempKelvin ) * ( second_power( ( TempOuterCover + KelvinConv ) ) + second_power( GroundTempKelvin ) );
	...
	)
	{
		...
		RaNum = gravity * ( second_power( DensOfAir ) ) * VolExpAir * PrOfAir * DeltaT * ( third_power( AirGap ) ) / ( second_power( VisDOfAir ) );
	...
	)
	{
		...
		GrNum = gravity * VolExpWater * DensOfWater * DensOfWater * PrOfWater * DeltaT * ( third_power( Lc ) ) / ( second_power( VisOfWater ) );
