...
namespace SolarShading {
	...
	AllocateModuleArrays()
	{
		...
		FenLaySurfTempFront.allocate( TotSurfaces, MaxSolidWinLayers );
		FenLaySurfTempFront = 0.0;
		...
		XTEMP.allocate( ( MaxVerticesPerSurface + 1 ) * 2 );
		XTEMP = 0.0;
		YTEMP.allocate( ( MaxVerticesPerSurface + 1 ) * 2 );
		YTEMP = 0.0;
		XVC.allocate( MaxVerticesPerSurface + 1 );
		XVC = 0.0;
		XVS.allocate( MaxVerticesPerSurface + 1 );
		XVS = 0.0;
		YVC.allocate( MaxVerticesPerSurface + 1 );
		YVC = 0.0;
		YVS.allocate( MaxVerticesPerSurface + 1 );
		YVS = 0.0;
		ZVC.allocate( MaxVerticesPerSurface + 1 );
		ZVC = 0.0;

		//Sutherland-Hodgman
		ATEMP.allocate( 2 * ( MaxVerticesPerSurface + 1 ) );
		ATEMP = 0.0;
		BTEMP.allocate( 2 * ( MaxVerticesPerSurface + 1 ) );
		BTEMP = 0.0;
		CTEMP.allocate( 2 * ( MaxVerticesPerSurface + 1 ) );
		CTEMP = 0.0;
		XTEMP1.allocate( 2 * ( MaxVerticesPerSurface + 1 ) );
		XTEMP1 = 0.0;
		YTEMP1.allocate( 2 * ( MaxVerticesPerSurface + 1 ) );
		YTEMP1 = 0.0;
		...
		for ( SurfLoop = 1; SurfLoop <= TotSurfaces; ++SurfLoop ) {
			...
			if ( Surface( SurfLoop ).Class == SurfaceClass_Window ) {
				...
				if ( Surface( SurfLoop ).ExtSolar ) {
					...
					if ( Construct( Construction[ SurfLoop  - 1] ).TCFlag == 1 ) {
					...
					if ( DisplayAdvancedReportVariables ) {
						...
						if ( SurfaceRadiantWin[ SurfLoop  - 1].OriginalClass != SurfaceClass_TDD_Diffuser ) SetupOutputVariable( "Surface Window Total Glazing Layers Absorbed Solar Radiation Rate [W]", QRadSWwinAbsTot( SurfLoop ), "Zone", "Average", Surface( SurfLoop ).Name );
						SetupOutputVariable( "Surface Window Total Glazing Layers Absorbed Shortwave Radiation Rate [W]", SWwinAbsTotalReport( SurfLoop ), "Zone", "Average", Surface( SurfLoop ).Name );
						if ( SurfaceRadiantWin[ SurfLoop  - 1].OriginalClass != SurfaceClass_TDD_Diffuser ) SetupOutputVariable( "Surface Window Transmitted Solar Radiation Rate [W]", WinTransSolar( SurfLoop ), "Zone", "Average", Surface( SurfLoop ).Name );
						SetupOutputVariable( "Surface Window Transmitted Beam Solar Radiation Rate [W]", WinBmSolar( SurfLoop ), "Zone", "Average", Surface( SurfLoop ).Name );
						...
						// Energy
						if ( SurfaceRadiantWin[ SurfLoop  - 1].OriginalClass != SurfaceClass_TDD_Diffuser ) SetupOutputVariable( "Surface Window Total Glazing Layers Absorbed Solar Radiation Energy [J]", QRadSWwinAbsTotEnergy( SurfLoop ), "Zone", "Sum", Surface( SurfLoop ).Name );
						if ( SurfaceRadiantWin[ SurfLoop  - 1].OriginalClass != SurfaceClass_TDD_Diffuser ) SetupOutputVariable( "Surface Window Transmitted Solar Radiation Energy [J]", WinTransSolarEnergy( SurfLoop ), "Zone", "Sum", Surface( SurfLoop ).Name );
						SetupOutputVariable( "Surface Window Transmitted Beam Solar Radiation Energy [J]", WinBmSolarEnergy( SurfLoop ), "Zone", "Sum", Surface( SurfLoop ).Name );
			...
			if ( SurfaceRadiantWin[ SurfLoop  - 1].OriginalClass == SurfaceClass_TDD_Diffuser ) {
	...
	)
	{
		...
		// SUBROUTINE LOCAL VARIABLE DECLARATIONS:
		int N; // Loop Control (vertex number)
		int NABOVE; // Number of vertices of shadow casting surface. above the plane of receiving surface
		int NEXT; // First vertex above plane of receiving surface
		int NON; // Number of vertices of shadow casting surface. on plane of receiving surface
		Real64 XIN; // X of entry point of shadow casting surface. into plane of receiving surface
		Real64 XOUT; // X of exit point of shadow casting surface. from plane of receiving surface
		Real64 YIN; // Y of entry point of shadow casting surface. into plane of receiving surface
		Real64 YOUT; // Y of exit point of shadow casting surface. from plane of receiving surface
	...
	)
	{
		...
		if ( I != 0 ) { // Transform vertices of figure ns.
			...
			for ( N = 1; N <= NumVertices; ++N ) {
				ITEMP = nint64( XVS( N ) * HCMULT );
				HCX( N, NS ) = ITEMP;
				ITEMP = nint64( YVS( N ) * HCMULT );
				HCY( N, NS ) = ITEMP;
			}
	...
	)
	{
		...
		for ( N = 1; N <= NV1; ++N ) {
			for ( M = 1; M <= NV2; ++M ) {
				...
				if ( KK != 0 ) {
					for ( K = 1; K <= KK; ++K ) {
						if ( std::abs( XTEMP( NV3 ) - XTEMP( K ) ) > 2.0 ) continue;
						if ( std::abs( YTEMP( NV3 ) - YTEMP( K ) ) > 2.0 ) continue;
						NV3 = KK;
						break; // K DO loop
					}
				}
	...
	)
	{
		...
		for ( E = 1; E <= NV2; ++E ) { // Loop over edges of the clipping polygon
			for ( P = 1; P <= NVOUT; ++P ) {
				XTEMP1( P ) = XTEMP( P );
				YTEMP1( P ) = YTEMP( P );
			}
			...
			for ( P = 1; P <= NVOUT; ++P ) {
				...
				if ( HFunct <= 0.0 ) { // Vertex is not in the clipping plane
					...
					if ( HFunct > 0.0 ) { // Test vertex is in the clipping plane
						...
						if ( W != 0.0 ) {
							XTEMP( NVTEMP ) = nint64( ( HCC( E, NS2 ) * BTEMP( S ) - HCB( E, NS2 ) * CTEMP( S ) ) / W );
							YTEMP( NVTEMP ) = nint64( ( HCA( E, NS2 ) * CTEMP( S ) - HCC( E, NS2 ) * ATEMP( S ) ) / W );
						} else {
						...
						if ( E == NV2 ) { // Remove near-duplicates on last edge
							if ( KK != 0 ) {
								for ( K = 1; K <= KK; ++K ) {
									if ( std::abs( XTEMP( NVTEMP ) - XTEMP( K ) ) > 2.0 ) continue;
									if ( std::abs( YTEMP( NVTEMP ) - YTEMP( K ) ) > 2.0 ) continue;
									NVTEMP = KK;
									break; // K DO loop
								}
							}
	...
	)
	{
		...
		if ( NV3 < MaxHCV && NS3 <= MaxHCS ) {
			...
			if ( ! SutherlandHodgman ) {
				...
				for ( N = 1; N <= NV3; ++N ) {
					HCX( N, NS3 ) = nint64( XTEMP( N ) );
					HCY( N, NS3 ) = nint64( YTEMP( N ) );
				}
			}
	...
	DetermineShadowingCombinations()
	{
		...
		HCA = 0;
		HCB.allocate( MaxHCV + 1, MaxHCS * 2 );
		HCB = 0;
		HCC.allocate( MaxHCV + 1, MaxHCS * 2 );
		HCC = 0;
		HCX.allocate( MaxHCV + 1, MaxHCS * 2 );
		HCX = 0;
		HCY.allocate( MaxHCV + 1, MaxHCS * 2 );
		HCY = 0;
		HCAREA.allocate( MaxHCS * 2 );
		HCAREA = 0.0;
		HCNS.allocate( MaxHCS * 2 );
		HCNS = 0;
		HCNV.allocate( MaxHCS * 2 );
		HCNV = 0;
		HCT.allocate( MaxHCS * 2 );
		HCT = 0.0;
		...
		for ( GRSNR = 1; GRSNR <= TotSurfaces; ++GRSNR ) { // Loop through all surfaces (looking for potential receiving surfaces)...
			...
			for ( SBSNR = 1; SBSNR <= TotSurfaces; ++SBSNR ) { // Loop through the surfaces yet again (looking for subsurfaces of GRSNR)...
				...
				if ( Construct( Construction[ SBSNR  - 1] ).TransDiff > 0.0 ) HasWindow = true; // Check for window
	...
	)
	{
		...
		if ( NGSS <= 0 ) { // IF NO S.S., receiving surface FULLY SUNLIT.
			...
			ShadowingSurfaces: for ( I = 1; I <= NGSS; ++I ) { // Loop through all shadowing surfaces...
				...
				if ( ! Surface( GSSNR ).HeatTransSurf ) {
					...
					if ( Surface( GSSNR ).SchedShadowSurfIndex > 0 ) {
						if ( LookUpScheduleValue( Surface( GSSNR ).SchedShadowSurfIndex, iHour ) == 1.0 ) continue;
					}
				}

				//commented OUT by Geof -- why are we checking this stuff AGAIN??!? RECHECK
				//      No shadow if shading surface is transparent
				// if (! CalcSkyDifShading ) {
				// 	if ( ! Surface( GSSNR ).HeatTransSurf ) {
				// 		if ( Surface( GSSNR ).IsTransparent ) continue;
				// 		if ( Surface( GSSNR ).SchedShadowSurfIndex > 0 ) {
				// 			if ( LookUpScheduleValue( Surface( GSSNR ).SchedShadowSurfIndex, iHour, TS ) == 1.0 ) continue;
				// 		}
				// 	}
				// } else {
				// 	if ( ! Surface( GSSNR ).HeatTransSurf ) {
				...
				if ( ( Surface( GSSNR ).BaseSurf != 0 ) && ( ! Surface( GSSNR ).HeatTransSurf ) ) {
				...
				{ auto const SELECT_CASE_var( MainOverlapStatus );

				if ( SELECT_CASE_var == NoOverlap ) { // No overlap of general surface shadow and receiving surface
					continue;

				} else if ( ( SELECT_CASE_var == FirstSurfWithinSecond ) || ( SELECT_CASE_var == TooManyVertices ) || ( SELECT_CASE_var == TooManyFigures ) ) {
					goto ShadowingSurfaces_exit;

				} else if ( ( SELECT_CASE_var == SecondSurfWithinFirst ) || ( SELECT_CASE_var == PartialOverlap ) ) {
			...
			{ auto const SELECT_CASE_var( ExitLoopStatus );

			if ( SELECT_CASE_var == FirstSurfWithinSecond ) { // Surface fully shaded
				SAREA( HTS ) = 0.0;
				LOCHCA = FGSSHC;

			} else if ( ( SELECT_CASE_var == TooManyVertices ) || ( SELECT_CASE_var == TooManyFigures ) ) { // Array limits exceeded, estimate
	...
	CalcInteriorSolarDistribution()
	{
		...
		FArray1D< Real64 > AbsBeamWin( MaxSolidWinLayers ); // Glass layer beam solar absorptance of a window
		FArray1D< Real64 > AbsBeamWinSh( MaxSolidWinLayers ); // Glass layer beam solar absorptance of a shaded window
		FArray1D< Real64 > AbsBeamWinEQL( CFSMAXNL+1 ); // layers beam solar absorptance of a window
		...
		FArray1D< Real64 > AbWin( MaxSolidWinLayers ); // Factor for front beam radiation absorbed in window glass layers
		Real64 AbWinBack; // Factor for back beam radiation absorbed in window glass layers
		FArray1D< Real64 > AbWinSh( MaxSolidWinLayers ); // Like AbWin, but for shaded window
		FArray1D< Real64 > AbWinEQL( CFSMAXNL+1 ); // Factor for front beam radiation absorbed for equivalent layer window model
		FArray1D< Real64 > AdWinEQL( CFSMAXNL+1 ); // Factor for front diffuse radiation absorbed for equivalent layer window model
		...
		bool VarSlatsBack;
		FArray1D< Real64 > ADiffWin( 5 ); // Diffuse solar absorptance of glass layers, bare window
		FArray1D< Real64 > ADiffWinSh( 5 ); // Diffuse solar absorptance of glass layers, window with shading device
		...
		FArray1D< Real64 > tempVec1( 3 ); // temporary vector for performing dot_product
		FArray1D< Real64 > tempVec2( 3 ); // temporary vector for performing dot_product
		...
		for ( ZoneNum = 1; ZoneNum <= NumOfZones; ++ZoneNum ) {
			...
			for ( SurfNum = ZoneSpecs[ZoneNum - 1].SurfaceFirst; SurfNum <= ZoneSpecs[ZoneNum - 1].SurfaceLast; ++SurfNum ) {
				if ( ( ( Surface( SurfNum ).ExtBoundCond != ExternalEnvironment ) && ( Surface( SurfNum ).ExtBoundCond != OtherSideCondModeledExt ) ) && SurfaceRadiantWin[ SurfNum  - 1].OriginalClass != SurfaceClass_TDD_Diffuser ) continue;
				if ( ! Surface( SurfNum ).HeatTransSurf ) continue;
				...
				ShadeFlag = SurfaceRadiantWin[ SurfNum  - 1].getShadingFlag(); // Set in subr. WindowShadingManager
				ProfAng = 0.0;
				if ( ShadeFlag != ExtScreenOn && BlNum > 0 ) ProfileAngle( SurfNum, SOLCOS, Blind( BlNum ).SlatOrientation, ProfAng );
				SlatAng = SurfaceRadiantWin[ SurfNum - 1 ].SlatAngThisTS;
				VarSlats = SurfaceRadiantWin[ SurfNum  - 1].MovableSlats;

				if ( SurfaceRadiantWin[ SurfNum  - 1].OriginalClass == SurfaceClass_TDD_Diffuser ) {
				...
				if ( Surface( SurfNum ).Class == SurfaceClass_TDD_Dome || SurfaceRadiantWin[ SurfNum  - 1].OriginalClass == SurfaceClass_TDD_Diffuser ) SunlitFracWithoutReveal( SurfNum, HourOfDay, TimeStep ) = SunLitFract; // Frames/dividers not allowed
				...
				if ( SunlitFracWithoutReveal( SurfNum, HourOfDay, TimeStep ) > 0.0 ) {
					...
					if ( SurfaceWindow( SurfNum ).WindowModelType != WindowBSDFModel && SurfaceWindow( SurfNum ).WindowModelType != WindowEQLModel ) {
						...
						for ( Lay = 1; Lay <= NGlass; ++Lay ) {
							AbWin( Lay ) = POLYF( CosInc, Construct( ConstrNum ).AbsBeamCoef( Lay, {1,6} ) ) * CosInc * SunLitFract * SurfaceWindow( SurfNum ).OutProjSLFracMult( HourOfDay );
							...
							if ( ShadeFlag <= 0 || ShadeFlag >= 10 ) {
								...
								AWinSurf( SurfNum, Lay ) = AbWin( Lay ) + SurfaceWindow( SurfNum ).OutsRevealDiffOntoGlazing * Construct( ConstrNum ).AbsDiff( Lay ) + SurfaceWindow( SurfNum ).InsRevealDiffOntoGlazing * Construct( ConstrNum ).AbsDiffBack( Lay );
								...
								if ( ShadeFlag == IntShadeOn || ShadeFlag == ExtShadeOn || ShadeFlag == BGShadeOn || ShadeFlag == SwitchableGlazing ) {
									...
									if ( ShadeFlag == IntBlindOn ) {
										...
										ADiffWinSh( Lay ) = ADiffWin( Lay ) + Construct( ConstrNum ).TransDiff * AGlDiffBack * RhoBlDiffFront / ( 1.0 - RhoBlDiffFront * RGlDiffBack );
										...
										ADiffWinSh( Lay ) = ADiffWin( Lay ) * TBlDifDif / ( 1.0 - RGlDifFr * RhoBlDifDifBk );
										...
										AbWinSh( Lay ) = TScBmBm * AbWin( Lay ) + ( TScBmBm * RGlFront * RScBack + TScBmDiff ) * Construct( ConstrNum ).AbsDiff( Lay ) / ( 1.0 - RGlDiffFront * RScDifBack ) * CosInc * FracSunLit;

										ADiffWinSh( Lay ) = ADiffWin( Lay ) * TScDifDif / ( 1.0 - RGlDifFr * RScDifBack );
								...
								if ( ShadeFlag != SwitchableGlazing ) {
									...
									if ( ShadeFlag == IntShadeOn || ShadeFlag == IntBlindOn || ShadeFlag == BGShadeOn || ShadeFlag == BGBlindOn ) AWinSurf( SurfNum, Lay ) += ADiffWinSh( Lay ) * SurfaceWindow( SurfNum ).OutsRevealDiffOntoGlazing;
									...
									AWinSurf( SurfNum, Lay ) += InterpSw( SwitchFac, ADiffWin( Lay ), ADiffWinSh( Lay ) ) * SurfaceWindow( SurfNum ).OutsRevealDiffOntoGlazing + InterpSw( SwitchFac, Construct( ConstrNum ).AbsDiffBack( Lay ), Construct( ConstrNumSh ).AbsDiffBack( Lay ) ) * SurfaceWindow( SurfNum ).InsRevealDiffOntoGlazing;
								}
						...
						if ( FenSolAbsPtr == 0 ) {
							...
							for ( Lay = 1; Lay <= SurfaceWindow( SurfNum ).ComplexFen.State( SurfaceWindow( SurfNum ).ComplexFen.CurrentState ).NLayers; ++Lay ) {
								AbWin( Lay ) = SurfaceWindow( SurfNum ).ComplexFen.State( SurfaceWindow( SurfNum ).ComplexFen.CurrentState ).WinBmFtAbs( Lay, HourOfDay, TimeStep ) * CosInc * SunLitFract * SurfaceWindow( SurfNum ).OutProjSLFracMult( HourOfDay );

								// Add contribution of beam reflected from outside and inside reveal
								AWinSurf( SurfNum, Lay ) = AbWin( Lay ) + SurfaceWindow( SurfNum ).OutsRevealDiffOntoGlazing * SurfaceWindow( SurfNum ).ComplexFen.State( SurfaceWindow( SurfNum ).ComplexFen.CurrentState ).WinFtHemAbs( Lay ) + SurfaceWindow( SurfNum ).InsRevealDiffOntoGlazing * SurfaceWindow( SurfNum ).ComplexFen.State( SurfaceWindow( SurfNum ).ComplexFen.CurrentState ).WinBkHemAbs( Lay );
							}
						}
						...
						for ( Lay = 1; Lay <= CFS( EQLNum ).NL + 1; ++Lay ) {
							...
							if ( CFS( EQLNum ).L( 1 ).LTYPE != ltyGLAZE ) {
								...
								if ( CFS( EQLNum ).L( Lay ).LTYPE == ltyGLAZE ) {
									AWinSurf( SurfNum, Lay ) = AbWinEQL( Lay ) + SurfaceWindow( SurfNum ).OutsRevealDiffOntoGlazing * AbsSolBeamEQL( Lay, 1 ) + SurfaceWindow( SurfNum ).InsRevealDiffOntoGlazing * AbsSolDiffEQL( Lay, 2 );
								} else {
									AWinSurf( SurfNum, Lay ) = AbWinEQL( Lay ) + SurfaceWindow( SurfNum ).OutsRevealDiffOntoGlazing * AbsSolBeamEQL( Lay, 1 );
								}
							}
						}
				...
				if ( SurfaceWindow( SurfNum ).WindowModelType != WindowBSDFModel && SurfaceWindow( SurfNum ).WindowModelType != WindowEQLModel ) { // Regular window
				...
				} else if ( SurfaceRadiantWin[ SurfNum  - 1].OriginalClass == SurfaceClass_TDD_Diffuser ) {
					...
					if ( FenSolAbsPtr == 0 ) {
						...
						Construct( Construction[ SurfNum - 1 ] ).TransDiff = NomDiffTrans;
					} else {
						DSZoneWin = 0.0;
						DGZoneWin = 0.0;
						DiffTrans = 0.0;
				...
				if ( SunLitFract > 0.0 ) {
				  ...
				  if ( ShadeFlag == IntShadeOn || ShadeFlag == ExtShadeOn || ShadeFlag == BGShadeOn || SurfaceWindow( SurfNum ).SolarDiffusing || SurfaceRadiantWin[ SurfNum  - 1].OriginalClass == SurfaceClass_TDD_Diffuser || Surface( SurfNum ).Class == SurfaceClass_TDD_Dome ) continue;
					...
					if ( SolarDistribution == FullInteriorExterior ) { // Full interior solar distribution
						...
						if ( SurfaceWindow( SurfNum ).WindowModelType != WindowBSDFModel && SurfaceWindow( SurfNum ).WindowModelType != WindowEQLModel ) {
							...
							for ( IBack = 1; IBack <= MaxBkSurf; ++IBack ) {
								...
								if ( Construct( ConstrNumBack ).TransDiff <= 0.0 ) {
									...
									ShadeFlagBack = SurfaceRadiantWin[ BackSurfNum  - 1].getShadingFlag();
									SlatAngBack = SurfaceRadiantWin[ BackSurfNum  - 1].SlatAngThisTS;
									VarSlatsBack = SurfaceRadiantWin[ BackSurfNum  - 1].MovableSlats;
									...
									if ( ShadeFlagBack == IntShadeOn || ShadeFlagBack == ExtShadeOn || ShadeFlagBack == BGShadeOn ) {
										for ( Lay = 1; Lay <= ConstrWin[ ConstrNumBackSh  - 1 ].TotGlassLayers; ++Lay ) {
							...
							if ( FenSolAbsPtr == 0 ) {
								...
								for ( IBack = 1; IBack <= NBkSurf; ++IBack ) {
									...
									ConstrNumBack = Construction[ BackSurfaceNumber - 1 ];
						...
						for ( FloorNum = ZoneSpecs[ ZoneNum - 1 ].SurfaceFirst; FloorNum <= ZoneSpecs[ ZoneNum - 1 ].SurfaceLast; ++FloorNum ) {
	...
	WindowShadingManager()
	{
		...
		for ( ISurf = 1; ISurf <= TotSurfaces; ++ISurf ) {
		        SurfaceWindow( ISurf ).ExtIntShadePrevTS = SurfaceRadiantWin[ ISurf  - 1].getShadingFlag();
			SurfaceRadiantWin[ ISurf  - 1].setShadingFlag(NoShade);
			...
			SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( ShadeOff ); // Initialize shading flag to off
			...
			if ( SELECT_CASE_var == WSCT_AlwaysOn ) { // 'ALWAYSON'
			  SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( ShType );

			} else if ( SELECT_CASE_var == WSCT_AlwaysOff ) { // 'ALWAYSOFF'
			  SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( ShadeOff );

			} else if ( SELECT_CASE_var == WSCT_OnIfScheduled ) { // 'ONIFSCHEDULEALLOWS'
			  if ( SchedAllowsControl ) SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( ShType );
				...
				if ( SunIsUp ) {
					if ( SolarOnWindow > SetPoint && SchedAllowsControl ) {
					  SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( ShType );
						  ...
						  SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( 10 * ShType );
					}
				}
				...
				// by control schedule.
			  if ( SunIsUp ) SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( 10 * ShType );
			  ...
			  if ( SunIsUp && SchedAllowsControl ) SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( GlassConditionallyLightened );
				...
				if ( ! BeginSimFlag ) {
					if ( ! SunIsUp ) { // Night
					  if ( Surface( ISurf ).OutDryBulbTemp < SetPoint && SchedAllowsControl ) SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( ShType );
					} else { // Day
				...
				if ( ! BeginSimFlag ) {
					if ( ! SunIsUp ) { // Night
					  if ( SNLoadHeatRate( IZone ) > SetPoint && SchedAllowsControl ) SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( ShType );
					} else { // Day
						if ( SNLoadCoolRate( IZone ) > 0.0 && SchedAllowsControl ) {
						  SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( ShType );
						} else if ( GlareControlIsActive ) {
				...
				if ( ! BeginSimFlag ) {
					if ( SunIsUp && SNLoadCoolRate( IZone ) > 0.0 && SchedAllowsControl ) {
					  if ( SolarOnWindow > SetPoint ) SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( ShType );
					} else if ( GlareControlIsActive ) {
					  SurfaceRadiantWin[ ISurf  - 1].setShadingFlag( 10 * ShType );
					}
				}
			...
			if ( SurfaceRadiantWin[ ISurf  - 1].getShadingFlag() == SwitchableGlazing ) {
			...
			SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS = 0.0;
			SurfaceWindow( ISurf ).SlatAngThisTSDeg = 0.0;
			SurfaceWindow( ISurf ).SlatsBlockBeam = false;
			if ( SurfaceRadiantWin[ ISurf  - 1].getShadingFlag() == IntBlindOn || 
			     SurfaceRadiantWin[ ISurf  - 1].getShadingFlag() == 10 * IntBlindOn || 
			     SurfaceRadiantWin[ ISurf  - 1].getShadingFlag() == ExtBlindOn || 
			     SurfaceRadiantWin[ ISurf  - 1].getShadingFlag() == 10 * ExtBlindOn || 
			     SurfaceRadiantWin[ ISurf  - 1].getShadingFlag() == BGBlindOn || 
			     SurfaceRadiantWin[ ISurf  - 1].getShadingFlag() == 10 * BGBlindOn ) {
				...
				if ( BlNum > 0 ) {
					...
					if ( SELECT_CASE_var == WSC_SAC_FixedSlatAngle ) { // 'FIXEDSLATANGLE'
           				        SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS = InputSlatAngle;
						if ( ( SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS <= ThetaSmall || SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS >= ThetaBig ) && ( Blind( BlNum ).SlatWidth > Blind( BlNum ).SlatSeparation ) && ( BeamSolarOnWindow > 0.0 ) ) SurfaceWindow( ISurf ).SlatsBlockBeam = true;

					} else if ( SELECT_CASE_var == WSC_SAC_ScheduledSlatAngle ) { // 'SCHEDULEDSLATANGLE'
					        SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS = GetCurrentScheduleValue( WindowShadingControl( IShadingCtrl ).SlatAngleSchedule );
						SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS = max( Blind( BlNum ).MinSlatAngle, min( SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS, Blind( BlNum ).MaxSlatAngle ) ) * DegToRadians;
						if ( ( SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS <= ThetaSmall || SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS >= ThetaBig ) && ( Blind( BlNum ).SlatWidth > Blind( BlNum ).SlatSeparation ) && ( BeamSolarOnWindow > 0.0 ) ) SurfaceWindow( ISurf ).SlatsBlockBeam = true;
						...
						if ( BeamSolarOnWindow > 0.0 ) {
							if ( Blind( BlNum ).SlatSeparation >= Blind( BlNum ).SlatWidth ) {
								...
								if ( ThetaSmall >= ThetaMin && ThetaSmall <= ThetaMax ) {
									SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS = ThetaSmall;
								} else if ( ThetaBig >= ThetaMin && ThetaBig <= ThetaMax ) {
									SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS = ThetaBig;
								} else if ( ThetaSmall < ThetaMin && ThetaBig < ThetaMin ) {
									SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS = ThetaMin;
								} else if ( ThetaSmall > ThetaMax && ThetaBig > ThetaMax ) {
									SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS = ThetaMax;
					...
					SurfaceWindow( ISurf ).SlatAngThisTSDeg = SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS / DegToRadians;
					if ( SurfaceWindow( ISurf ).SlatAngThisTSDegEMSon ) {
					  ...
					  SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS = DegToRadians * SurfaceWindow( ISurf ).SlatAngThisTSDeg;
					}
					// Air flow permeability for calculation of convective air flow between blind and glass
					SlatAng = SurfaceRadiantWin[ ISurf - 1].SlatAngThisTS;
			...
			if ( SurfaceRadiantWin[ ISurf  - 1].getShadingFlag() == ExtScreenOn && SunIsUp ) {
				CalcScreenTransmittance( ISurf );
			}

			// EMS Actuator Point: override setting if ems flag on
			if ( SurfaceWindow( ISurf ).ShadingFlagEMSOn ) {
			  SurfaceRadiantWin[ ISurf  - 1].setShadingFlag(SurfaceWindow( ISurf ).ShadingFlagEMSValue);
			}
	...
	CalcWindowProfileAngles()
	{
		...
		FArray1D< Real64 > WinNorm( 3 ); // Unit vector normal to window
		...
		ElevSun = PiOvr2 - std::acos( SOLCOS( 3 ) );
		AzimSun = std::atan2( SOLCOS( 1 ), SOLCOS( 2 ) );
			...
			if ( Surface( SurfNum ).Class == SurfaceClass_Window && Surface( SurfNum ).WindowShadingControlPtr > 0 ) {
				...
				if ( WindowShadingControl( WinShadeCtrlNum ).ShadingType == WSC_ST_InteriorShade || WindowShadingControl( WinShadeCtrlNum ).ShadingType == WSC_ST_ExteriorShade || WindowShadingControl( WinShadeCtrlNum ).ShadingType == WSC_ST_BetweenGlassShade ) {
					...
					if ( WindowShadingControl( WinShadeCtrlNum ).ShadingType == WSC_ST_InteriorShade ) {
						...
						if ( ConstrWin[ ConstrNumSh  - 1 ].TotGlassLayers == 2 ) {
		...
		for ( ZoneNum = 1; ZoneNum <= NumOfZones; ++ZoneNum ) {
			...
			for ( DifTransSurfNum = ZoneSpecs[ZoneNum - 1].SurfaceFirst; DifTransSurfNum <= ZoneSpecs[ZoneNum - 1].SurfaceLast; ++DifTransSurfNum ) {
				...
				for ( HeatTransSurfNum = ZoneSpecs[ZoneNum - 1].SurfaceFirst; HeatTransSurfNum <= ZoneSpecs[ZoneNum - 1].SurfaceLast; ++HeatTransSurfNum ) {
					...
					ViewFactor = ZoneInfo[ ZoneNum - 1 ].F( DifTransSurfNum - FirstZoneSurf + 1, HeatTransSurfNum - FirstZoneSurf + 1 );
					...
					ConstrNum = Construction[ HeatTransSurfNum  - 1];
					if ( Construct( ConstrNum ).TransDiff <= 0.0 ) { // Interior Opaque Surface
						...
						ShadeFlag = SurfaceRadiantWin[ HeatTransSurfNum - 1 ].getShadingFlag();
						...
						if ( SurfaceWindow( HeatTransSurfNum ).WindowModelType != WindowEQLModel ) {
							if ( ShadeFlag <= 0 ) { // No window shading
								...
								if ( AdjSurfNum > 0 ) { // this is an interior window surface
									...
									AdjConstrNum = Construction[ AdjSurfNum - 1 ];
	...
	)
	{
		...
		for ( HeatTransSurfNum = ZoneSpecs[ZoneNum - 1].SurfaceFirst; HeatTransSurfNum <= ZoneSpecs[ZoneNum - 1].SurfaceLast; ++HeatTransSurfNum ) {
			...
			ViewFactor = ZoneInfo[ ZoneNum - 1 ].F( IntWinSurfNum - FirstZoneSurf + 1, HeatTransSurfNum - FirstZoneSurf + 1 );
			// debug ViewFactorTotal
			ViewFactorTotal += ViewFactor; // debug
			...
			if ( Construct( ConstrNum ).TransDiff <= 0.0 ) { // Interior Opaque Surface
				...
				//    * view factor from current (sending) window IntWinSurfNum to current (receiving) surface HeatTransSurfNum
				//    * current surface inside solar absorptance
				DifSolarAbsW = IntWinDifSolarTransW * ViewFactor * InsideDifAbsorptance; // [W]
				...
				//    * view factor from current (sending) window IntWinSurfNum to current (receiving) surface HeatTransSurfNum
				//    * current window inside solar reflectance
				DifSolarReflW = IntWinDifSolarTransW * ViewFactor * InsideDifReflectance;
				...
				if ( ShadeFlag <= 0 ) { // No window shading
					...
					//    * view factor from current (sending) window IntWinSurfNum to current (receiving) surface HeatTransSurfNum
					//    * current window inside solar reflectance
					DifSolarReflW = IntWinDifSolarTransW * ViewFactor * Construct( ConstrNum ).ReflectSolDiffBack;
					...
					// This is not very effective since it assigns whatever distributed diffuse solar has not been
					// absorbed or reflected to transmitted.
					DifSolarTransW = IntWinDifSolarTransW * ViewFactor - DifSolarAbsW - DifSolarReflW;
					...
					for ( IGlass = 1; IGlass <= TotGlassLayers; ++IGlass ) {
						// Calc diffuse solar absorbed in each window glass layer
						WinDifSolLayAbsW = IntWinDifSolarTransW * ViewFactor * InterpSw( SurfaceWindow( HeatTransSurfNum ).SwitchingFactor, Construct( ConstrNum ).AbsDiffBack( IGlass ), Construct( ConstrNumSh ).AbsDiffBack( IGlass ) );
					...
					// Calc diffuse solar reflected back to zone
					DifSolarReflW = IntWinDifSolarTransW * ViewFactor * InterpSw( SurfaceWindow( HeatTransSurfNum ).SwitchingFactor, Construct( ConstrNum ).ReflectSolDiffBack, Construct( ConstrNumSh ).ReflectSolDiffBack );
				...
				//            ELSE IF(ShadeFlag == IntBlindOn) THEN
				//              DividerSolAbs = DividerSolAbs * InterpSlatAng(SurfaceWindow(HeatTransSurfNum)%SlatAngThisTS, &
				//                  SurfaceRadiantWin[HeatTransSurfNum - 1].MovableSlats,Blind(BlNum)%SolBackDiffDiffTrans)
				//            END IF
				// Note that DividerQRadInAbs is initially calculated in InitSolarHeatGains
