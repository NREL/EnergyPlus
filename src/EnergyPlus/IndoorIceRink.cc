                         ...
                         int const SysNum)                // Index to the refrigeration system
{
    ...
    QResurfacing = 0.001 * RhoWater * ResurfacerTank_capacity * ((CpWater * ResurfacingHWTemperature) + (QFusion) - (CpIce * IceSurfaceTemperature));
    ...
    AH_preResurfacing = ((6.112 * exp((17.67 * T_air_preResurfacing) / (T_air_preResurfacing + 243.5)) * RH_air_preResurfacing * MolarMassWater) /
                        ...
                        (1 / RhoWater);
    AH_postResurfacing = ((6.112 * exp((17.67 * T_air_postResurfacing) / (T_air_postResurfacing + 243.5)) * RH_air_postResurfacing * MolarMassWater) /
                             ...
                             Real64 TubeDiameter)
{
    ...
    static Array1D<Real64> const Mu(
        NumOfPropDivisions, {0.0001903, 0.0001881, 0.000186, 0.0001839, 0.0001818, 0.0001798, 0.0001778, 0.0001759, 0.000174, 0.0001721, 0.0001702});
...
Real64 BOTC(int SystemType,           // Type of refrigeration system
            ...
            Real64 const Temperature) // Temperature of inlet refrigerant
{
    // Using/ Aliasing
    using DataGlobals::Pi;
    using DataHeatBalFanSys::CTFTsrcConstPart;
    ...
    if (SystemType == DirectSystem) {
        ...
        if (Index == 1) {
            ...
            InterpFrac = (Temperature - Temps(Index - 1)) / (Temps(Index) - Temps(Index - 1));
            CpRef = Cp(Index - 1) + InterpFrac * (Cp(Index) - Cp(Index - 1));
        }
