		...
		for ( Loop = 1; Loop <= TotThermalChimney; ++Loop ) {
			...
			for ( IterationLoop = 1; IterationLoop <= 10; ++IterationLoop ) {
				...
				if ( ThermChimSubTemp( NTC ) <= RoomAirTemp ) {
					...
					TempTCVolumeAirFlowRate( IterationLoop ) = DischargeCoeffTC * AirOutletCrossAreaTC * ( power( ( 2.0 * ( ( ThermChimSubTemp( NTC ) - RoomAirTemp ) / RoomAirTemp ) * 9.8 * OverallThermalChimLength / ( power( ( 1.0 + AirRelativeCrossArea ), 2 ) ) ), 0.5 ) );
				}
			...
			if ( ThermChimSubTemp( NTC ) <= RoomAirTemp ) {
				...
				TCVolumeAirFlowRate = DischargeCoeffTC * AirOutletCrossAreaTC * ( power( ( 2.0 * ( ( ThermChimSubTemp( NTC ) - RoomAirTemp ) / RoomAirTemp ) * 9.8 * OverallThermalChimLength / ( power( ( 1.0 + AirRelativeCrossArea ), 2 ) ) ), 0.5 ) );
			}
