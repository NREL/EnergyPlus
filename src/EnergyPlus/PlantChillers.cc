...
namespace PlantChillers {
	...
	)
	{
		...
		AvailNomCapRat = CapacityRat( 1 ) + CapacityRat( 2 ) * DeltaTemp + CapacityRat( 3 ) * power( DeltaTemp, 2 );

		AvailChillerCap = ChillerNomCap * AvailNomCapRat;

		// from BLAST...G=ADJEC(1)+ADJEC(2)*RCAV+ADJEC(3)*RCAV**2.
		FullLoadPowerRat = PowerRat( 1 ) + PowerRat( 2 ) * AvailNomCapRat + PowerRat( 3 ) * power( AvailNomCapRat, 2 );
		...
		FracFullLoadPower = FullLoadFactor( 1 ) + FullLoadFactor( 2 ) * PartLoadRat + FullLoadFactor( 3 ) * power( PartLoadRat, 2 );
	...
	)
	{
		...
		if ( PartLoadRat != 0 ) {
			...
			UA = EngineDrivenChiller( ChillerNum ).UACoef( 1 ) * power( ChillerNomCap, EngineDrivenChiller( ChillerNum ).UACoef( 2 ) );
