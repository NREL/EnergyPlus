...
namespace ConvectionCoefficients {
	...
	Real64 const OneFifth( ( 1.0 / 5. ) ); // 1/5 in highest precision
	Real64 const OneSixth( ( 1.0 / 6. ) ); // 1/6 in highest precision
	Real64 const FourFifths( ( 4. / 5. ) ); // 4/5 in highest precision
	...
	)
	{
		...
		if ( ( SELECT_CASE_var <= -1 ) ) { // Set by user using one of the standard algorithms...
			...
			if ( SELECT_CASE_var1 == ASHRAESimple ) {
				...
				HExt = std::sqrt( second_power( ( MoWiTTTurbulentConstant * std::pow( ( std::abs( TAir - TSurf ) ), OneThird ) ) ) + second_power( ( ConstantA * std::pow( SurfWindSpeed, ConstantB ) ) ) );
				...
				Hn = CalcHnASHRAETARPExterior( TSurf, TAir, Surface( SurfNum ).CosTilt );
				HcGlass = std::sqrt( second_power( Hn ) + second_power( ( ConstantA * std::pow( SurfWindSpeed, ConstantB ) ) ) );
			...
			if ( TSurf == SkyTempKelvin || std::abs( Surface( SurfNum ).ExtConvCoeff ) == ASHRAESimple ) {
				...
				HSky = StefanBoltzmann * AbsExt * Surface( SurfNum ).ViewFactorSkyIR * AirSkyRadSplit( SurfNum ) * ( ( fourth_power( TSurf ) ) - ( fourth_power( SkyTempKelvin ) ) ) / ( TSurf - SkyTempKelvin );
			}
			...
			if ( TSurf == TAir || std::abs( Surface( SurfNum ).ExtConvCoeff ) == ASHRAESimple ) {
				...
				HGround = StefanBoltzmann * AbsExt * Surface( SurfNum ).ViewFactorGroundIR * ( ( fourth_power( TSurf ) ) - ( fourth_power( TAir ) ) ) / ( TSurf - TAir );

				// Compute air radiation coefficient
				HAir = StefanBoltzmann * AbsExt * Surface( SurfNum ).ViewFactorSkyIR * ( 1.0 - AirSkyRadSplit( SurfNum ) ) * ( ( fourth_power( TSurf ) ) - ( fourth_power( TAir ) ) ) / ( TSurf - TAir );
			}
	...
	)
	{
		...
		if ( std::abs( CosTilt ) < 0.98 ) { // Surface is not horizontal
			...
			if ( ( Diff - 180. ) > .001 ) Diff -= 360.0;
			if ( ( std::abs( Diff ) - 100. ) > .001 ) AgainstWind = false; // Surface is leeward
		}
	...
	)
	{
		...
		// FLOW:
		CalcASHRAESimpExtConvectCoeff = D( Roughness ) + E( Roughness ) * SurfWindSpeed + F( Roughness ) * second_power( SurfWindSpeed );
	...
	)
	{
		...
		if ( Surf1 > 0 && Surf2 > 0 ) {
			...
			Gr = ( g * beta * std::abs( Tsi - Tso ) * third_power( gapW ) ) / ( second_power( v ) ); // curve fit for v = v(T)?
	...
	)
	{
		...
		RaH = ( second_power( rho ) * third_power( Height ) * g * Cp * ( std::abs( SurfTempKelvin - AirTempKelvin ) ) ) / ( TmeanFilmKelvin * mu * lambda ); // eq 132 in ISO 15099
	...
	SetupAdaptiveConvectionStaticMetaData()
	{
		...
		for ( ZoneLoop = 1; ZoneLoop <= NumOfZones; ++ZoneLoop ) {
			...
			thisZoneSimplePerim = 2. * ( Zone( ZoneLoop ).MaximumY - Zone( ZoneLoop ).MinimumY ) + 2. * ( Zone( ZoneLoop ).MaximumX - Zone( ZoneLoop ).MinimumX );
		...
		for ( SurfLoop = 1; SurfLoop <= TotSurfaces; ++SurfLoop ) {
			...
			if ( ( Surface( SurfLoop ).Tilt >= 45. ) && ( Surface( SurfLoop ).Tilt < 135. ) ) {
		...
		NorthFacade.Perimeter = 2. * ( std::pow( ( second_power( ( NorthFacade.Xmax - NorthFacade.Xmin ) ) + second_power( ( NorthFacade.Ymax - NorthFacade.Ymin ) ) ), 0.5 ) ) + 2. * ( NorthFacade.Zmax - NorthFacade.Zmin );
		NorthFacade.Height = NorthFacade.Zmax - NorthFacade.Zmin;

		NorthEastFacade.Perimeter = 2. * ( std::pow( ( second_power( ( NorthEastFacade.Xmax - NorthEastFacade.Xmin ) ) + second_power( ( NorthEastFacade.Ymax - NorthEastFacade.Ymin ) ) ), 0.5 ) ) + 2. * ( NorthEastFacade.Zmax - NorthEastFacade.Zmin );
		NorthEastFacade.Height = NorthEastFacade.Zmax - NorthEastFacade.Zmin;

		EastFacade.Perimeter = 2. * ( std::pow( ( second_power( ( EastFacade.Xmax - EastFacade.Xmin ) ) + second_power( ( EastFacade.Ymax - EastFacade.Ymin ) ) ), 0.5 ) ) + 2. * ( EastFacade.Zmax - EastFacade.Zmin );
		EastFacade.Height = EastFacade.Zmax - EastFacade.Zmin;

		SouthEastFacade.Perimeter = 2. * ( std::pow( ( second_power( ( SouthEastFacade.Xmax - SouthEastFacade.Xmin ) ) + second_power( ( SouthEastFacade.Ymax - SouthEastFacade.Ymin ) ) ), 0.5 ) ) + 2. * ( SouthEastFacade.Zmax - SouthEastFacade.Zmin );
		SouthEastFacade.Height = SouthEastFacade.Zmax - SouthEastFacade.Zmin;

		SouthFacade.Perimeter = 2. * ( std::pow( ( second_power( ( SouthFacade.Xmax - SouthFacade.Xmin ) ) + second_power( ( SouthFacade.Ymax - SouthFacade.Ymin ) ) ), 0.5 ) ) + 2. * ( SouthFacade.Zmax - SouthFacade.Zmin );
		SouthFacade.Height = SouthFacade.Zmax - SouthFacade.Zmin;

		SouthWestFacade.Perimeter = 2. * ( std::pow( ( second_power( ( SouthWestFacade.Xmax - SouthWestFacade.Xmin ) ) + second_power( ( SouthWestFacade.Ymax - SouthWestFacade.Ymin ) ) ), 0.5 ) ) + 2. * ( SouthWestFacade.Zmax - SouthWestFacade.Zmin );
		SouthWestFacade.Height = SouthWestFacade.Zmax - SouthWestFacade.Zmin;

		WestFacade.Perimeter = 2. * ( std::pow( ( second_power( ( WestFacade.Xmax - WestFacade.Xmin ) ) + second_power( ( WestFacade.Ymax - WestFacade.Ymin ) ) ), 0.5 ) ) + 2. * ( WestFacade.Zmax - WestFacade.Zmin );
		WestFacade.Height = WestFacade.Zmax - WestFacade.Zmin;

		NorthWestFacade.Perimeter = 2. * ( std::pow( ( second_power( ( NorthWestFacade.Xmax - NorthWestFacade.Xmin ) ) + second_power( ( NorthWestFacade.Ymax - NorthWestFacade.Ymin ) ) ), 0.5 ) ) + 2. * ( NorthWestFacade.Zmax - NorthWestFacade.Zmin );
		...
		if ( surfacearea > .001 ) { // Roof is not flat
	...
	)
	{
		...
		if ( SELECT_CASE_var == HcInt_UserCurve ) {
			...
			ZoneNum = Surface( SurfNum ).Zone;
			tmpHc = CalcWaltonUnstableHorizontalOrTilt( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).CosTilt ); //TODO verify CosTilt in vs out
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
		} else if ( SELECT_CASE_var == HcInt_WaltonStableHorizontalOrTilt ) {
			ZoneNum = Surface( SurfNum ).Zone;
			tmpHc = CalcWaltonStableHorizontalOrTilt( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).CosTilt ); //TODO verify CosTilt in vs out
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
			...
			ZoneNum = Surface( SurfNum ).Zone;
			tmpHc = CalcAlamdariHammondStableHorizontal( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).IntConvZoneHorizHydrDiam, SurfNum );
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
		} else if ( SELECT_CASE_var == HcInt_AlamdariHammondVerticalWall ) {
			ZoneNum = Surface( SurfNum ).Zone;
			tmpHc = CalcAlamdariHammondVerticalWall( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).IntConvZoneWallHeight, SurfNum );
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
		} else if ( SELECT_CASE_var == HcInt_AlamdariHammondUnstableHorizontal ) {
			ZoneNum = Surface( SurfNum ).Zone;
			tmpHc = CalcAlamdariHammondUnstableHorizontal( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).IntConvZoneHorizHydrDiam, SurfNum );
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
			...
			ZoneNum = Surface( SurfNum ).Zone;
			tmpHc = CalcAwbiHattonHeatedFloor( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).IntConvZoneHorizHydrDiam );
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
		} else if ( SELECT_CASE_var == HcInt_AwbiHattonHeatedWall ) {
			ZoneNum = Surface( SurfNum ).Zone;
			tmpHc = CalcAwbiHattonHeatedWall( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).IntConvZoneHorizHydrDiam );
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
			...
			tmpHc = CalcBeausoleilMorrisonMixedAssistedWall( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).IntConvZoneWallHeight, TH( SurfNum, 1, 2 ), SupplyAirTemp, AirChangeRate, ZoneNum );
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
			...
			tmpHc = CalcBeausoleilMorrisonMixedOpposingWall( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).IntConvZoneWallHeight, TH( SurfNum, 1, 2 ), SupplyAirTemp, AirChangeRate, ZoneNum );
			...
			tmpHc = CalcBeausoleilMorrisonMixedStableCeiling( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).IntConvZoneHorizHydrDiam, TH( SurfNum, 1, 2 ), SupplyAirTemp, AirChangeRate, ZoneNum );
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
			...
			tmpHc = CalcBeausoleilMorrisonMixedUnstableCeiling( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).IntConvZoneHorizHydrDiam, TH( SurfNum, 1, 2 ), SupplyAirTemp, AirChangeRate, ZoneNum );
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
			...
			tmpHc = CalcBeausoleilMorrisonMixedStableFloor( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).IntConvZoneHorizHydrDiam, TH( SurfNum, 1, 2 ), SupplyAirTemp, AirChangeRate, ZoneNum );
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
			...
			tmpHc = CalcBeausoleilMorrisonMixedUnstableFloor( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).IntConvZoneHorizHydrDiam, TH( SurfNum, 1, 2 ), SupplyAirTemp, AirChangeRate, ZoneNum );
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
		} else if ( SELECT_CASE_var == HcInt_FohannoPolidoriVerticalWall ) {
			ZoneNum = Surface( SurfNum ).Zone;
			tmpHc = CalcFohannoPolidoriVerticalWall( ( TH( SurfNum, 1, 2 ) - MAT( ZoneNum ) ), Surface( SurfNum ).IntConvZoneWallHeight, TH( SurfNum, 1, 2 ), -QdotConvInRepPerArea( SurfNum ), SurfNum );
			Surface( SurfNum ).TAirRef = ZoneMeanAirTemp;
			...
			ZoneNum = Surface( SurfNum ).Zone;
			CalcISO15099WindowIntConvCoeff( SurfNum, TH( SurfNum, 1, 2 ), MAT( ZoneNum ) );
	...
	)
	{
		...
		if ( SELECT_CASE_var == HcExt_None ) {
			...
			Hn = CalcWaltonUnstableHorizontalOrTilt( ( TH( SurfNum, 1, 1 ) - Surface( SurfNum ).OutDryBulbTemp ), Surface( SurfNum ).CosTilt ); //TODO verify CosTilt in vs out
		} else if ( SELECT_CASE_var == HcExt_NaturalWaltonStableHorizontalOrTilt ) {
			Hn = CalcWaltonStableHorizontalOrTilt( ( TH( SurfNum, 1, 1 ) - Surface( SurfNum ).OutDryBulbTemp ), Surface( SurfNum ).CosTilt ); //TODO verify CosTilt in vs out
		} else if ( SELECT_CASE_var == HcExt_AlamdariHammondVerticalWall ) {

			Hn = CalcAlamdariHammondVerticalWall( ( TH( SurfNum, 1, 1 ) - Surface( SurfNum ).OutDryBulbTemp ), Surface( SurfNum ).OutConvFaceHeight, SurfNum );
		} else if ( SELECT_CASE_var == HcExt_FohannoPolidoriVerticalWall ) {
			Hn = CalcFohannoPolidoriVerticalWall( ( TH( SurfNum, 1, 1 ) - Surface( SurfNum ).OutDryBulbTemp ), Surface( SurfNum ).OutConvFaceHeight, TH( SurfNum, 1, 1 ), -QdotConvOutRepPerArea( SurfNum ), SurfNum );
			...
			Hn = CalcAlamdariHammondStableHorizontal( ( TH( SurfNum, 1, 1 ) - Surface( SurfNum ).OutDryBulbTemp ), HydraulicDiameter, SurfNum );
			...
			Hn = CalcAlamdariHammondUnstableHorizontal( ( TH( SurfNum, 1, 1 ) - Surface( SurfNum ).OutDryBulbTemp ), HydraulicDiameter, SurfNum );
		}}
	...
	DynamicIntConvSurfaceClassification( int const SurfNum ) // surface number
	{
		...
		if ( ( FinalFlowRegime == InConvFlowRegime_C ) || ( FinalFlowRegime == InConvFlowRegime_D ) ) {
			...
			GrH = ( g * ( Tmax - Tmin ) * ( third_power( Zone( ZoneNum ).CeilingHeight ) ) ) / ( ( MAT( ZoneNum ) + KelvinConv ) * ( second_power( v ) ) );
			...
			if ( Re > 0.0 ) {
				Ri = GrH / ( second_power( Re ) ); //Richardson Number
		...
		if ( SELECT_CASE_var == InConvFlowRegime_A1 ) {
			...
			if ( Surface( SurfNum ).Class == SurfaceClass_Wall || Surface( SurfNum ).Class == SurfaceClass_Door ) {
				...
				if ( ( Surface( SurfNum ).Tilt > 85. ) && ( Surface( SurfNum ).Tilt < 95. ) ) { //vertical wall
					Surface( SurfNum ).IntConvClassification = InConvClass_A1_VertWalls;
				} else if ( Surface( SurfNum ).Tilt >= 95. ) { //tilted upwards
					if ( DeltaTemp > 0.0 ) {
				...
				} else if ( Surface( SurfNum ).Tilt <= 85. ) { //tilted downwards
					if ( DeltaTemp < 0.0 ) {
						Surface( SurfNum ).IntConvClassification = InConvClass_A1_UnstableTilted;
					} else {
						Surface( SurfNum ).IntConvClassification = InConvClass_A1_StableTilted;
				...
				} else if ( ( Surface( SurfNum ).Tilt >= 5. ) && ( ( Surface( SurfNum ).Tilt < 95. ) ) ) { //tilted downwards
					if ( DeltaTemp < 0.0 ) {
						Surface( SurfNum ).IntConvClassification = InConvClass_A1_UnstableTilted;
					} else {
						Surface( SurfNum ).IntConvClassification = InConvClass_A1_StableTilted;
					}
				} else if ( ( Surface( SurfNum ).Tilt > 85. ) && ( Surface( SurfNum ).Tilt < 95. ) ) { //vertical wall
					Surface( SurfNum ).IntConvClassification = InConvClass_A1_VertWalls;
				} else if ( Surface( SurfNum ).Tilt >= 95. ) { //tilted upwards
					if ( DeltaTemp > 0.0 ) {
						Surface( SurfNum ).IntConvClassification = InConvClass_A1_UnstableTilted;
				...
				} else if ( Surface( SurfNum ).Tilt > 175. ) { //floor
					if ( DeltaTemp > 0.0 ) {
						Surface( SurfNum ).IntConvClassification = InConvClass_A1_UnstableHoriz;
					} else {
						Surface( SurfNum ).IntConvClassification = InConvClass_A1_StableHoriz;
					}
				} else if ( ( Surface( SurfNum ).Tilt <= 175. ) && ( Surface( SurfNum ).Tilt >= 95. ) ) {
					if ( DeltaTemp > 0.0 ) {
						Surface( SurfNum ).IntConvClassification = InConvClass_A1_UnstableTilted;
					} else {
						Surface( SurfNum ).IntConvClassification = InConvClass_A1_StableTilted;
			...
			if ( Surface( SurfNum ).Class == SurfaceClass_Wall || Surface( SurfNum ).Class == SurfaceClass_Door ) {
				...
				} else if ( ( Surface( SurfNum ).Tilt > 5. ) && ( ( Surface( SurfNum ).Tilt < 85. ) ) ) { //tilted downwards
					if ( DeltaTemp < 0.0 ) {
						Surface( SurfNum ).IntConvClassification = InConvClass_A3_UnstableTilted;
					} else {
						Surface( SurfNum ).IntConvClassification = InConvClass_A3_StableTilted;
			...
			if ( Surface( SurfNum ).Class == SurfaceClass_Wall || Surface( SurfNum ).Class == SurfaceClass_Door ) {
				...
				if ( Surface( SurfNum ).Tilt > 175. ) { //floor
					if ( DeltaTemp > 0.0 ) {
						Surface( SurfNum ).IntConvClassification = InConvClass_D_UnstableHoriz;
					} else {
						Surface( SurfNum ).IntConvClassification = InConvClass_D_StableHoriz;
	...
	)
	{
		...
		if ( HcInsideUserCurve( UserCurveNum ).HcFnTempDiffCurveNum > 0 ) {
			tmpHc = CurveValue( HcInsideUserCurve( UserCurveNum ).HcFnTempDiffCurveNum, std::abs( TH( SurfNum, 1, 2 ) - tmpAirTemp ) );
		}

		if ( HcInsideUserCurve( UserCurveNum ).HcFnTempDiffDivHeightCurveNum > 0 ) {
			tmpHc += CurveValue( HcInsideUserCurve( UserCurveNum ).HcFnTempDiffDivHeightCurveNum, ( std::abs( TH( SurfNum, 1, 2 ) - tmpAirTemp ) / Surface( SurfNum ).IntConvZoneWallHeight ) );
		}
	...
	)
	{
		...
		if ( HcOutsideUserCurve( UserCurveNum ).HnFnTempDiffCurveNum > 0 ) {
			tmpHc += CurveValue( HcOutsideUserCurve( UserCurveNum ).HnFnTempDiffCurveNum, std::abs( TH( SurfNum, 1, 1 ) - Surface( SurfNum ).OutDryBulbTemp ) );
		}

		if ( HcOutsideUserCurve( UserCurveNum ).HnFnTempDiffDivHeightCurveNum > 0 ) {
			if ( Surface( SurfNum ).OutConvFaceHeight > 0.0 ) {
				tmpHc += CurveValue( HcOutsideUserCurve( UserCurveNum ).HnFnTempDiffDivHeightCurveNum, ( ( std::abs( TH( SurfNum, 1, 1 ) - Surface( SurfNum ).OutDryBulbTemp ) ) / Surface( SurfNum ).OutConvFaceHeight ) );
			}
		}
	...
	)
	{
		...
		if ( HydraulicDiameter > 0.0 ) {
			Hn = std::pow( ( ( sixth_power( ( 1.4 * ( std::pow( ( std::abs( DeltaTemp ) / HydraulicDiameter ), OneFourth ) ) ) ) ) + ( ( 1.63 * sixth_power( ( std::pow( ( std::abs( DeltaTemp ) ), OneThird ) ) ) ) ) ), OneSixth );
		} else {
			Hn = 9.999;
			if ( ErrorIndex == 0 ) {
	...
	)
	{
		...
		if ( HydraulicDiameter > 0.0 ) {
			Hn = 0.6 * ( std::pow( ( std::abs( DeltaTemp ) / ( second_power( HydraulicDiameter ) ) ), OneFifth ) );
		} else {
			Hn = 9.999;
			if ( ErrorIndex == 0 ) {
	...
	)
	{
		...
		if ( Height > 0.0 ) {
			Hn = std::pow( ( ( sixth_power( ( 1.5 * ( std::pow( ( std::abs( DeltaTemp ) / Height ), OneFourth ) ) ) ) ) + ( ( 1.23 * sixth_power( ( std::pow( ( std::abs( DeltaTemp ) ), OneThird ) ) ) ) ) ), OneSixth );
		} else {
			Hn = 9.999;
			if ( ErrorIndex == 0 ) {
	...
	)
	{
		...
		if ( ( DeltaTemp != 0.0 ) && ( Height != 0.0 ) ) {
			Hc = std::pow( ( ( std::pow( ( ( sixth_power( ( 1.5 * ( std::pow( ( std::abs( DeltaTemp ) / Height ), OneFourth ) ) ) ) ) + std::pow( ( ( 1.23 * sixth_power( ( std::pow( ( std::abs( DeltaTemp ) ), OneThird ) ) ) ) ), OneSixth ) ), 0.5 ) + ( third_power( ( ( ( SurfTemp - SupplyAirTemp ) / std::abs( DeltaTemp ) ) * ( -0.199 + 0.190 * ( std::pow( AirChangeRate, 0.8 ) ) ) ) ) ) ) ), OneThird );
		} else {
			Hc = 9.999;
			if ( Height == 0.0 ) {
	...
	)
	{
		...
		if ( ( DeltaTemp != 0.0 ) ) { // protect divide by zero
			...
			if ( Height != 0.0 ) {
				HcTmp1 = std::pow( ( ( std::pow( ( ( sixth_power( ( 1.5 * ( std::pow( ( std::abs( DeltaTemp ) / Height ), OneFourth ) ) ) ) ) + std::pow( ( ( 1.23 * sixth_power( ( std::pow( ( std::abs( DeltaTemp ) ), OneThird ) ) ) ) ), OneSixth ) ), 0.5 ) - ( third_power( ( ( ( SurfTemp - SupplyAirTemp ) / std::abs( DeltaTemp ) ) * ( -0.199 + 0.190 * ( std::pow( AirChangeRate, 0.8 ) ) ) ) ) ) ) ), OneThird );

				HcTmp2 = 0.8 * ( std::pow( ( ( sixth_power( ( 1.5 * ( std::pow( ( std::abs( DeltaTemp ) / Height ), OneFourth ) ) ) ) ) + ( ( 1.23 * sixth_power( ( std::pow( ( std::abs( DeltaTemp ) ), OneThird ) ) ) ) ) ), OneSixth ) );
			} else {
				HcTmp1 = 9.999;
				HcTmp2 = 9.999;
				if ( ErrorIndex2 == 0 ) {
	...
	)
	{
		...
		if ( ( HydraulicDiameter != 0.0 ) && ( DeltaTemp != 0.0 ) ) {
			Hc = std::pow( ( third_power( ( 0.6 * std::pow( ( std::abs( DeltaTemp ) / HydraulicDiameter ), OneFifth ) ) ) + third_power( ( ( ( SurfTemp - SupplyAirTemp ) / std::abs( DeltaTemp ) ) * ( 0.159 + 0.116 * ( std::pow( AirChangeRate, 0.8 ) ) ) ) ) ), OneThird );
		} else {
			Hc = 9.999;
			if ( HydraulicDiameter == 0.0 ) {
	...
	)
	{
		...
		if ( ( HydraulicDiameter != 0.0 ) && ( DeltaTemp != 0.0 ) ) {
			Hc = std::pow( ( std::pow( ( sixth_power( ( 1.4 * std::pow( ( std::abs( DeltaTemp ) / HydraulicDiameter ), OneFourth ) ) ) + sixth_power( ( 1.63 * ( std::pow( std::abs( DeltaTemp ), OneThird ) ) ) ) ), 0.5 ) + third_power( ( ( ( SurfTemp - SupplyAirTemp ) / std::abs( DeltaTemp ) ) * ( 0.159 + 0.116 * ( std::pow( AirChangeRate, 0.8 ) ) ) ) ) ), OneThird );
		} else {
			Hc = 9.999;
			if ( HydraulicDiameter == 0.0 ) {
	...
	)
	{
		...
		if ( ( HydraulicDiameter != 0.0 ) && ( DeltaTemp != 0.0 ) ) {
			Hc = std::pow( ( third_power( ( 0.6 * std::pow( ( std::abs( DeltaTemp ) / HydraulicDiameter ), OneFifth ) ) ) + third_power( ( ( ( SurfTemp - SupplyAirTemp ) / std::abs( DeltaTemp ) ) * ( -0.166 + 0.484 * ( std::pow( AirChangeRate, 0.8 ) ) ) ) ) ), OneThird );
		} else {
			Hc = 9.999;
			if ( HydraulicDiameter == 0.0 ) {
	...
	)
	{
		...
		if ( ( HydraulicDiameter != 0.0 ) && ( DeltaTemp != 0.0 ) ) {
			Hc = std::pow( ( std::pow( ( sixth_power( ( 1.4 * std::pow( ( std::abs( DeltaTemp ) / HydraulicDiameter ), OneFourth ) ) ) + sixth_power( ( 1.63 * ( std::pow( std::abs( DeltaTemp ), OneThird ) ) ) ) ), 0.5 ) + third_power( ( ( ( SurfTemp - SupplyAirTemp ) / std::abs( DeltaTemp ) ) * ( -0.166 + 0.484 * ( std::pow( AirChangeRate, 0.8 ) ) ) ) ) ), OneThird );
		} else {
			Hc = 9.999;
			if ( HydraulicDiameter == 0.0 ) {
	...
	)
	{
		...
		if ( Height > 0.0 ) {
			RaH = ( g * BetaFilm * QdotConv * ( fourth_power( Height ) ) * Pr ) / ( k * second_power( v ) );
	...
	)
	{
		...
		Hc = std::pow( ( second_power( ( 0.84 * ( std::pow( ( std::abs( DeltaTemp ) ), OneThird ) ) ) ) + second_power( ( 3.26 * ( std::pow( WindAtZ, 0.89 ) ) ) ) ), 0.5 );
	...
	)
	{
		...
		Hc = std::pow( ( second_power( ( 0.84 * ( std::pow( ( std::abs( DeltaTemp ) ), OneThird ) ) ) ) + second_power( ( 3.55 * ( std::pow( WindAtZ, 0.617 ) ) ) ) ), 0.5 );
	...
	)
	{
		...
		HcSmooth = std::sqrt( second_power( Hn ) + second_power( ( 3.26 * std::pow( WindAtZ, 0.89 ) ) ) );
	...
	)
	{
		...
		HcSmooth = std::sqrt( second_power( Hn ) + second_power( ( 3.55 * std::pow( WindAtZ, 0.617 ) ) ) );
	...
	)
	{
		...
		x = std::sqrt( RoofArea ) / 2.; // quick simplification, geometry routines to develop
		...
		GrLn = g * ( second_power( AirDensity ) ) * ( third_power( Ln ) ) * std::abs( DeltaTemp ) * BetaFilm / second_power( v );
		RaLn = GrLn * Pr;

		Rex = WindAtZ * AirDensity * x / v;

		if ( Rex > 0.1 ) { //avoid zero and crazy small denominators
			eta = ( std::log( 1.0 + GrLn / second_power( Rex ) ) ) / ( 1.0 + std::log( 1.0 + GrLn / ( second_power( Rex ) ) ) );
		} else {
