	...
	InitialInitHeatBalFiniteDiff()
	{
		...
		for ( ConstrNum = 1; ConstrNum <= TotConstructs; ++ConstrNum ) {
			...
			for ( Layer = 1; Layer <= Construct( ConstrNum ).TotLayers; ++Layer ) { // Begin layer loop ...
				...
				StabilityTemp = Alpha * Delt / pow2( dxn );
				StabilityMoist = mAlpha * Delt / pow2( dxn );
				...
				if ( Material( MatLay ).ROnly || Material( MatLay ).Group == 1 ) { // R Layer or Air Layer  **********
					...
					if ( HMovInsul <= 0.0 ) { //  regular  case
						...
						if ( SELECT_CASE_var == CrankNicholsonSecondOrder ) {
							...
							//   First Order
							TDT( i ) = ( 2.0 * Delt * DelX * QRadSWOutFD + Cp * pow2( DelX ) * RhoS * TD( i ) + 2.0 * Delt * kt * TDT( i + 1 ) + 2.0 * Delt * DelX * hgnd * Tgnd + 2.0 * Delt * DelX * hconvo * Toa + 2.0 * Delt * DelX * hrad * Toa + 2.0 * Delt * DelX * hsky * Tsky ) / ( 2.0 * Delt * DelX * hconvo + 2.0 * Delt * DelX * hgnd + 2.0 * Delt * DelX * hrad + 2.0 * Delt * DelX * hsky + 2.0 * Delt * kt + Cp * pow2( DelX ) * RhoS );
						...
						{ auto const SELECT_CASE_var( CondFDSchemeType );
						if ( SELECT_CASE_var == CrankNicholsonSecondOrder ) {
							TDT( i ) = ( 2 * Delt * DelX * QRadSWOutFD + Cp * pow2( DelX ) * RhoS * TD( i ) + 2 * Delt * kt * TDT( i + 1 ) + 2 * Delt * DelX * HMovInsul * TInsulOut ) / ( 2 * Delt * DelX * HMovInsul + 2 * Delt * kt + Cp * pow2( DelX ) * RhoS );
		...
		if ( Surface( Surf ).HeatTransferAlgorithm == HeatTransferModel_CondFD ) { //HT Algo issue
			...
			if ( RLayerPresent && RLayer2Present ) {
				...
				if ( SELECT_CASE_var == CrankNicholsonSecondOrder ) {
					...
					TDT( i ) = ( 2. * Delt * Delx2 * QSSFlux * Rlayer - Delt * Delx2 * TD( i ) - Delt * kt2 * Rlayer * TD( i ) + Cp2 * pow2( Delx2 ) * RhoS2 * Rlayer * TD( i ) + Delt * Delx2 * TD( i - 1 ) + Delt * kt2 * Rlayer * TD( i + 1 ) + Delt * Delx2 * TDT( i - 1 ) + Delt * kt2 * Rlayer * TDT( i + 1 ) ) / ( Delt * Delx2 + Delt * kt2 * Rlayer + Cp2 * pow2( Delx2 ) * RhoS2 * Rlayer );

				} else if ( SELECT_CASE_var == FullyImplicitFirstOrder ) {

					TDT( i ) = ( 2. * Delt * Delx2 * QSSFlux * Rlayer + Cp2 * pow2( Delx2 ) * RhoS2 * Rlayer * TD( i ) + 2. * Delt * Delx2 * TDT( i - 1 ) + 2. * Delt * kt2 * Rlayer * TDT( i + 1 ) ) / ( 2. * Delt * Delx2 + 2. * Delt * kt2 * Rlayer + Cp2 * pow2( Delx2 ) * RhoS2 * Rlayer );
				...
				if ( SELECT_CASE_var == CrankNicholsonSecondOrder ) {
					TDT( i ) = ( 2. * Delt * Delx1 * QSSFlux * Rlayer2 - Delt * Delx1 * TD( i ) - Delt * kt1 * Rlayer2 * TD( i ) + Cp1 * pow2( Delx1 ) * RhoS1 * Rlayer2 * TD( i ) + Delt * kt1 * Rlayer2 * TD( i - 1 ) + Delt * Delx1 * TD( i + 1 ) + Delt * kt1 * Rlayer2 * TDT( i - 1 ) + Delt * Delx1 * TDT( i + 1 ) ) / ( Delt * Delx1 + Delt * kt1 * Rlayer2 + Cp1 * pow2( Delx1 ) * RhoS1 * Rlayer2 );
				} else if ( SELECT_CASE_var == FullyImplicitFirstOrder ) {
					TDT( i ) = ( 2. * Delt * Delx1 * QSSFlux * Rlayer2 + Cp1 * pow2( Delx1 ) * RhoS1 * Rlayer2 * TD( i ) + 2. * Delt * kt1 * Rlayer2 * TDT( i - 1 ) + 2. * Delt * Delx1 * TDT( i + 1 ) ) / ( 2. * Delt * Delx1 + 2. * Delt * kt1 * Rlayer2 + Cp1 * pow2( Delx1 ) * RhoS1 * Rlayer2 );
				...
				if ( SELECT_CASE_var == CrankNicholsonSecondOrder ) {
					...
					TDT( i ) = ( 2. * Delt * Delx1 * Delx2 * QSSFlux - Delt * Delx2 * kt1 * TD( i ) - Delt * Delx1 * kt2 * TD( i ) + Cp1 * pow2( Delx1 ) * Delx2 * RhoS1 * TD( i ) + Cp2 * Delx1 * pow2( Delx2 ) * RhoS2 * TD( i ) + Delt * Delx2 * kt1 * TD( i - 1 ) + Delt * Delx1 * kt2 * TD( i + 1 ) + Delt * Delx2 * kt1 * TDT( i - 1 ) + Delt * Delx1 * kt2 * TDT( i + 1 ) ) / ( Delt * Delx2 * kt1 + Delt * Delx1 * kt2 + Cp1 * pow2( Delx1 ) * Delx2 * RhoS1 + Cp2 * Delx1 * pow2( Delx2 ) * RhoS2 );

				} else if ( SELECT_CASE_var == FullyImplicitFirstOrder ) {
					// first order adams moulton
					TDT( i ) = ( 2. * Delt * Delx1 * Delx2 * QSSFlux + Cp1 * pow2( Delx1 ) * Delx2 * RhoS1 * TD( i ) + Cp2 * Delx1 * pow2( Delx2 ) * RhoS2 * TD( i ) + 2. * Delt * Delx2 * kt1 * TDT( i - 1 ) + 2. * Delt * Delx1 * kt2 * TDT( i + 1 ) ) / ( 2. * Delt * Delx2 * kt1 + 2. * Delt * Delx1 * kt2 + Cp1 * pow2( Delx1 ) * Delx2 * RhoS1 + Cp2 * Delx1 * pow2( Delx2 ) * RhoS2 );
		...
		if ( Surface( Surf ).HeatTransferAlgorithm == HeatTransferModel_CondFD ) {
			...
			if ( Material( MatLay ).ROnly || Material( MatLay ).Group == 1 ) { // R Layer or Air Layer
				...
				if ( Surface( Surf ).ExtBoundCond > 0 && i == 1 ) { //this is for an adiabatic or interzone partition
					...
					if ( SELECT_CASE_var == CrankNicholsonSecondOrder ) {
						// Adams-Moulton second order
						TDT( i ) = ( 2. * Delt * DelX * NetLWRadToSurfFD + 2. * Delt * DelX * QHtRadSysSurfFD + 2. * Delt * DelX * QHWBaseboardSurfFD + 2. * Delt * DelX * QSteamBaseboardSurfFD + 2. * Delt * DelX * QElecBaseboardSurfFD + 2. * Delt * DelX * QRadSWInFD + 2. * Delt * DelX * QRadThermInFD - Delt * DelX * hconvi * TD( i ) - Delt * kt * TD( i ) + Cp * pow2( DelX ) * RhoS * TD( i ) + Delt * kt * TD( i + 1 ) + Delt * kt * TDT( i + 1 ) + 2. * Delt * DelX * hconvi * Tia ) / ( Delt * DelX * hconvi + Delt * kt + Cp * pow2( DelX ) * RhoS );

					} else if ( SELECT_CASE_var == FullyImplicitFirstOrder ) {
						// Adams-Moulton First order
						TDT( i ) = ( 2. * Delt * DelX * NetLWRadToSurfFD + 2. * Delt * DelX * QHtRadSysSurfFD + 2. * Delt * DelX * QHWBaseboardSurfFD + 2. * Delt * DelX * QSteamBaseboardSurfFD + 2. * Delt * DelX * QElecBaseboardSurfFD + 2. * Delt * DelX * QRadSWInFD + 2. * Delt * DelX * QRadThermInFD + Cp * pow2( DelX ) * RhoS * TD( i ) + 2. * Delt * kt * TDT( i + 1 ) + 2. * Delt * DelX * hconvi * Tia ) / ( 2. * Delt * DelX * hconvi + 2. * Delt * kt + Cp * pow2( DelX ) * RhoS );
					}}
