				...
				if ( Operand( 1 ).TrendVariable ) {
					...
					if ( thisIndex >= 1 ) {
						...
						if ( thisIndex <= TrendVariable( thisTrend ).LogDepth ) {
							...
							thisSlope = ( sum( TrendVariable( thisTrend ).TimeARR( {1,thisIndex} ) ) * sum( TrendVariable( thisTrend ).TrendValARR( {1,thisIndex} ) ) - thisIndex * sum( ( TrendVariable( thisTrend ).TimeARR( {1,thisIndex} ) * TrendVariable( thisTrend ).TrendValARR( {1,thisIndex} ) ) ) ) / ( second_power( sum( TrendVariable( thisTrend ).TimeARR( {1,thisIndex} ) ) ) - thisIndex * sum( pow( TrendVariable( thisTrend ).TimeARR( {1,thisIndex} ), 2 ) ) );
