...
namespace ManageElectricPower {
	...
	)
	{
		...
		if ( ElecStorage( ElecStorNum ).StorageModelMode == KiBaMBattery ) {
			...
			ElecStorage( ElecStorNum ).ThermLossRate = InternalR * second_power( I0 ) * Numbattery;
			ElecStorage( ElecStorNum ).ThermLossEnergy = InternalR * second_power( I0 ) * TimeStepSys * SecInHour * Numbattery;
	...
	ManageTransformers()
	{
		...
		if ( MyOneTimeFlag ) {
			...
			for ( TransfNum = 1; TransfNum <= NumTransformers; ++TransfNum ) {
				if ( Transformer( TransfNum ).PerformanceInputMode == EfficiencyMethod ) {
					...
					Denominator = Transformer( TransfNum ).RatedEfficiency * ( 1.0 + second_power( ( Transformer( TransfNum ).RatedPUL / Transformer( TransfNum ).MaxPUL ) ) );

					Transformer( TransfNum ).RatedNL = Numerator / Denominator;
					Transformer( TransfNum ).RatedLL = Transformer( TransfNum ).RatedNL / ( FactorTempCorr * second_power( ( Transformer( TransfNum ).MaxPUL ) ) );
		...
		for ( TransfNum = 1; TransfNum <= NumTransformers; ++TransfNum ) {
			...
			if ( GetCurrentScheduleValue( Transformer( TransfNum ).AvailSchedPtr ) > 0.0 ) {
				...
				Transformer( TransfNum ).LoadLossRate = Transformer( TransfNum ).RatedLL * ( second_power( PUL ) ) * FactorTempCorr;
