	...
	GetPowerManagerInput()
	{
		...
		if ( NumTransformers > 0 ) {
			...
			for ( TransfNum = 1; TransfNum <= NumTransformers; ++TransfNum ) {
				...
				SetupOutputVariable( "Transformer Distribution Electric Loss Energy [J]", Transformer( TransfNum ).ElecUseUtility, "System", "Sum", Transformer( TransfNum ).Name, _, "Electricity", "COGENERATION", _, "System" );
