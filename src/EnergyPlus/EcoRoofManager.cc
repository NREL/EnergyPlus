...
namespace EcoRoofManager {
	...
	)
	{
		...
		Latm = 1.0 * Sigma * 1.0 * Surface( SurfNum ).ViewFactorGround * ( fourth_power( ( GroundTempKelvin ) ) ) + 1.0 * Sigma * 1.0 * Surface( SurfNum ).ViewFactorSky * fourth_power( ( SkyTempKelvin ) );
		...
		if ( SurfNum == FirstEcoSurf ) {
			...
			Cfhn = second_power( ( Kv / std::log( ( Za - Zd ) / Zo ) ) ); //Equation 12, page 7, FASST Model
			...
			Lef = 1.91846e6 * second_power( ( ( Tif + KelvinConv ) / ( Tif + KelvinConv - 33.91 ) ) );
			...
			dqf = ( ( 0.622 * Pa ) / second_power( ( Pa - esf ) ) ) * Desf; //Derivative of saturation specific humidity
			...
			Leg = 1.91846e6 * second_power( ( Tgk / ( Tgk - 33.91 ) ) );
			//Check to see if ice is sublimating or frost is forming.
			...
			dqg = ( 0.622 * Pa / second_power( ( Pa - esg ) ) ) * Desg;
			...
			Rhoag = ( Rhoa + Rhog ) / 2.;
			Rib = 2.0 * g1 * Za * ( Taf - Tg ) / ( ( Tafk + Tgk ) * second_power( Waf ) ); //Richardson Number
			...
			Chng = second_power( ( Kv / std::log( Za / Zog ) ) ) / rch; // bulk transfer coefficient near ground
			...
			Chne = second_power( ( Kv / std::log( Za / Zog ) ) ) / rche;
			...
			for ( EcoLoop = 1; EcoLoop <= 3; ++EcoLoop ) {
				P1 = sigmaf * ( RS * ( 1.0 - Alphaf ) + epsilonf * Latm ) - 3.0 * sigmaf * epsilonf * epsilong * Sigma * fourth_power( ( SoilTK ) ) / EpsilonOne - 3.0 * ( -sigmaf * epsilonf * Sigma - sigmaf * epsilonf * epsilong * Sigma / EpsilonOne ) * fourth_power( ( LeafTK ) ) + sheatf * ( 1.0 - 0.7 * sigmaf ) * ( Ta + KelvinConv ) + LAI * Rhoaf * Cf * Lef * Waf * rn * ( ( 1.0 - 0.7 * sigmaf ) / dOne ) * qa + LAI * Rhoaf * Cf * Lef * Waf * rn * ( ( ( 0.6 * sigmaf * rn ) / dOne ) - 1.0 ) * ( qsf - ( LeafTK ) * dqf ) + LAI * Rhoaf * Cf * Lef * Waf * rn * ( ( 0.1 * sigmaf * Mg ) / dOne ) * ( qsg - ( SoilTK ) * dqg );
				...
				P3 = 4.0 * ( -sigmaf * epsilonf * Sigma - ( sigmaf * epsilonf * Sigma * epsilong ) / EpsilonOne ) * third_power( ( LeafTK ) ) + ( 0.6 * sigmaf - 1.0 ) * sheatf + LAI * Rhoaf * Cf * Lef * Waf * rn * ( ( ( 0.6 * sigmaf * rn ) / dOne ) - 1.0 ) * dqf;
				...
				T1G = ( 1.0 - sigmaf ) * ( RS * ( 1.0 - Alphag ) + epsilong * Latm ) - ( 3.0 * ( sigmaf * epsilonf * epsilong * Sigma ) / EpsilonOne ) * fourth_power( ( LeafTK ) ) - 3.0 * ( -( 1.0 - sigmaf ) * epsilong * Sigma - sigmaf * epsilonf * epsilong * Sigma / EpsilonOne ) * fourth_power( ( SoilTK ) ) + sheatg * ( 1.0 - 0.7 * sigmaf ) * ( Ta + KelvinConv ) + Rhoag * Ce * Leg * Waf * Mg * ( ( 1.0 - 0.7 * sigmaf ) / dOne ) * qa + Rhoag * Ce * Leg * Waf * Mg * ( 0.1 * sigmaf * Mg / dOne - Mg ) * ( qsg - ( SoilTK ) * dqg ) + Rhoag * Ce * Leg * Waf * Mg * ( 0.6 * sigmaf * rn / dOne ) * ( qsf - ( LeafTK ) * dqf ) + Qsoilpart1 + Qsoilpart2 * ( KelvinConv ); //finished by T1G

				T2G = 4.0 * ( -( 1.0 - sigmaf ) * epsilong * Sigma - sigmaf * epsilonf * epsilong * Sigma / EpsilonOne ) * third_power( ( SoilTK ) ) + ( 0.1 * sigmaf - 1.0 ) * sheatg + Rhoag * Ce * Leg * Waf * Mg * ( 0.1 * sigmaf * Mg / dOne - Mg ) * dqg - Qsoilpart2;

				T3G = ( 4.0 * ( sigmaf * epsilong * epsilonf * Sigma ) / EpsilonOne ) * third_power( ( LeafTK ) ) + 0.6 * sigmaf * sheatg + Rhoag * Ce * Leg * Waf * Mg * ( 0.6 * sigmaf * rn / dOne ) * dqf;
	...
	)
	{
		...
		if ( Material( Construct( ConstrNum ).LayerPoint( 1 ) ).EcoRoofCalculationMethod == 1 ) {
			...
			SoilHydroConductivityTop = SoilConductivitySaturation * ( std::pow( RelativeSoilSaturationTop, lambda ) ) * second_power( ( 1.0 - std::pow( ( 1.0 - std::pow( ( RelativeSoilSaturationTop ), ( n / ( n - 1.0 ) ) ) ), ( ( n - 1.0 ) / n ) ) ) );
			...
			SoilHydroConductivityRoot = SoilConductivitySaturation * ( std::pow( RelativeSoilSaturationRoot, lambda ) ) * second_power( ( 1.0 - std::pow( ( 1.0 - std::pow( ( RelativeSoilSaturationRoot ), ( n / ( n - 1.0 ) ) ) ), ( ( n - 1.0 ) / n ) ) ) );
