...
namespace HVACCooledBeam {
			...
			if ( CurZoneEqNum > 0 ) {
				...
				if ( PltSizCoolNum > 0 ) {
					...
					if ( DesCoilLoad > 0.0 ) {
						...
						WaterVel = WaterVolFlowPerBeam / ( Pi * power( ( CoolBeam( CBNum ).InDiam ), 2 ) / 4.0 );
						if ( FinalZoneSizing( CurZoneEqNum ).ZoneTempAtCoolPeak > 0.0 ) {
						...
						for ( Iter = 1; Iter <= 100; ++Iter ) {
							IndAirFlowPerBeamL = CoolBeam( CBNum ).K1 * power( DT, CoolBeam( CBNum ).n ) + CoolBeam( CBNum ).Kin * DesAirFlowPerBeam / LengthX;
							ConvFlow = ( IndAirFlowPerBeamL / CoolBeam( CBNum ).a0 ) * RhoAir;
							if ( WaterVel > MinWaterVel ) {
								K = CoolBeam( CBNum ).a * power( DT, CoolBeam( CBNum ).n1 ) * power( ConvFlow, CoolBeam( CBNum ).n2 ) * power( WaterVel, CoolBeam( CBNum ).n3 );
							} else {
								K = CoolBeam( CBNum ).a * power( DT, CoolBeam( CBNum ).n1 ) * power( ConvFlow, CoolBeam( CBNum ).n2 ) * power( MinWaterVel, CoolBeam( CBNum ).n3 ) * ( WaterVel / MinWaterVel );
							}
	...
	)
	{
		...
		for ( Iter = 1; Iter <= 200; ++Iter ) {
			...
			IndFlow = CoolBeam( CBNum ).K1 * power( DT, CoolBeam( CBNum ).n ) + CoolBeam( CBNum ).Kin * CoolBeam( CBNum ).BeamFlow / CoolBeam( CBNum ).BeamLength;
			CoilFlow = ( IndFlow / CoolBeam( CBNum ).a0 ) * StdRhoAir;
			WaterVel = CWFlowPerBeam / ( rho * Pi * power( ( CoolBeam( CBNum ).InDiam ), 2 ) / 4.0 );
			if ( WaterVel > MinWaterVel ) {
				K = CoolBeam( CBNum ).a * power( DT, CoolBeam( CBNum ).n1 ) * power( CoilFlow, CoolBeam( CBNum ).n2 ) * power( WaterVel, CoolBeam( CBNum ).n3 );
			} else {
				K = CoolBeam( CBNum ).a * power( DT, CoolBeam( CBNum ).n1 ) * power( CoilFlow, CoolBeam( CBNum ).n2 ) * power( MinWaterVel, CoolBeam( CBNum ).n3 ) * ( WaterVel / MinWaterVel );
			}
