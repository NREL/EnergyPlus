...
namespace PurchasedAirManager {
	...
	SizePurchasedAir( int const PurchAirNum )
	{
		...
		if ( CurZoneEqNum > 0 ) {
			if ( ! IsAutoSize && ! ZoneSizingRunDone ) { // Simulation continue
				...
				if ( IsAutoSize ) {
					...
					ReportSizingOutput( PurchAir( PurchAirNum ).cObjectName, PurchAir( PurchAirNum ).Name, "Design Size Maximum Heating Air Flow Rate [m3/s]", MaxHeatVolFlowRateDes );
				} else {
					if ( PurchAir( PurchAirNum ).MaxHeatVolFlowRate > 0.0 && MaxHeatVolFlowRateDes > 0.0 ) {
		...
		if ( CurZoneEqNum > 0 ) {
			if ( ! IsAutoSize && ! ZoneSizingRunDone ) { // Simulation continue
				...
				if ( IsAutoSize ) {
					...
					ReportSizingOutput( PurchAir( PurchAirNum ).cObjectName, PurchAir( PurchAirNum ).Name, "Design Size Maximum Cooling Air Flow Rate [m3/s]", MaxCoolVolFlowRateDes );
				} else {
					if ( PurchAir( PurchAirNum ).MaxCoolVolFlowRate > 0.0 && MaxCoolVolFlowRateDes > 0.0 ) {
		...
		if ( CurZoneEqNum > 0 ) {
			if ( ! IsAutoSize && ! ZoneSizingRunDone ) { // Simulation continue
				...
				DesignLoad = PsyCpAirFnWTdb( OutletHumRat, 0.5 * ( MixedAirTemp + OutletTemp ) ) * FinalZoneSizing( CurZoneEqNum ).DesHeatMassFlow * ( OutletTemp - MixedAirTemp ); // *&^unique^&* "SizePurchasedAir"
		...
		if ( CurZoneEqNum > 0 ) {
			if ( ! IsAutoSize && ! ZoneSizingRunDone ) { // Simulation continue
				...
				DesignLoad = FinalZoneSizing( CurZoneEqNum ).DesCoolMassFlow * ( PsyHFnTdbW( MixedAirTemp, MixedAirHumRat ) - PsyHFnTdbW( OutletTemp, OutletHumRat ) ); // *&^unique^&* "SizePurchasedAir"
	...
	)
	{
		...
		if ( UnitOn ) {
			...
			if ( PurchAir( PurchAirNum ).OutdoorAir ) {
				CpAir = PsyCpAirFnWTdb( Node( OANodeNum ).HumRat, Node( OANodeNum ).Temp ); // *&^unique^&* "CalcPurchAirLoads"
			...
			if ( ( MinOASensOutput >= QZnCoolSP ) && ( TempControlType( ActualZoneNum ) != SingleHeatingSetPoint ) ) {
				...
				if ( ( ( PurchAir( PurchAirNum ).CoolingLimit == LimitFlowRate ) || ( PurchAir( PurchAirNum ).CoolingLimit == LimitFlowRateAndCapacity ) ) && ( OAMassFlowRate > PurchAir( PurchAirNum ).MaxCoolMassFlowRate ) ) {
					...
					if ( PurchAir( PurchAirNum ).EconomizerType != NoEconomizer ) {
						if ( ( ( PurchAir( PurchAirNum ).EconomizerType == DifferentialDryBulb ) && ( Node( OANodeNum ).Temp < Node( PurchAir( PurchAirNum ).ZoneRecircAirNodeNum ).Temp ) ) || ( ( PurchAir( PurchAirNum ).EconomizerType == DifferentialEnthalpy ) && ( Node( OANodeNum ).Enthalpy < Node( PurchAir( PurchAirNum ).ZoneRecircAirNodeNum ).Enthalpy ) ) ) {
							...
							CpAir = PsyCpAirFnWTdb( ZoneAirHumRat( ActualZoneNum ), Node( ZoneNodeNum ).Temp ); // *&^unique^&* "CalcPurchAirLoads"
				...
				if ( SupplyMassFlowRate > 0.0 ) {
					...
					SupplyEnthalpy = PsyHFnTdbW( SupplyTemp, SupplyHumRat ); // *&^unique^&* "CalcPurchAirLoads"
					...
					if ( ( PurchAir( PurchAirNum ).CoolingLimit == LimitCapacity ) || ( PurchAir( PurchAirNum ).CoolingLimit == LimitFlowRateAndCapacity ) ) {
						CpAir = PsyCpAirFnWTdb( MixedAirHumRat, MixedAirTemp ); // *&^unique^&* "CalcPurchAirLoads"
						...
						if ( CoolSensOutput >= PurchAir( PurchAirNum ).MaxCoolTotCap ) {
							...
							SupplyTemp = PsyTdbFnHW( SupplyEnthalpy, SupplyHumRat ); // *&^unique^&* "CalcPurchAirLoads"
							// This is the cooling mode, so SupplyTemp can't be more than MixedAirTemp
							SupplyTemp = min( SupplyTemp, MixedAirTemp );
					...
					if ( SELECT_CASE_var == None ) {
						...
						SupplyEnthalpy = max( SupplyEnthalpy, PsyHFnTdbW( SupplyTemp, 0.00001 ) ); // *&^unique^&* "CalcPurchAirLoads"
	...
	)
	{
		...
		if ( PurchAir( PurchAirNum ).OutdoorAir && ( OAMassFlowRate > 0.0 ) ) {
			...
			if ( HeatRecOn ) {
				...
				OAAfterHtRecEnthalpy = PsyHFnTdbW( OAAfterHtRecTemp, OAAfterHtRecHumRat ); // *&^unique^&* "CalcPurchAirMixedAir"
			...
			if ( SupplyMassFlowRate > OAMassFlowRate ) {
				...
				MixedAirTemp = PsyTdbFnHW( MixedAirEnthalpy, MixedAirHumRat ); // *&^unique^&* "CalcPurchAirMixedAir"
			} else {
				RecircMassFlowRate = 0.0;
