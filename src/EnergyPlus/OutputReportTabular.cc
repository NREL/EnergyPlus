		...
		for ( iInObj = 1; iInObj <= OutputTableBinnedCount; ++iInObj ) {
			...
			for ( iTable = 1; iTable <= curNumTables; ++iTable ) {
				...
				if ( BinStatistics( repIndex ).n > 1 ) {
					if ( BinStatistics( repIndex ).sum2 > ( ( second_power( BinStatistics( repIndex ).sum ) ) / BinStatistics( repIndex ).n ) ) {
						repStDev = std::sqrt( ( BinStatistics( repIndex ).sum2 - ( ( second_power( BinStatistics( repIndex ).sum ) ) / BinStatistics( repIndex ).n ) ) / ( BinStatistics( repIndex ).n - 1 ) );
					} else {
						repStDev = 0.0;
					}
