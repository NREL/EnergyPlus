...
//		static bool GetAirModelData( true ); // Used to "get" all air model data
	...
	GetRoomAirflowNetworkData( bool & ErrorsFound ) // True if errors found during this get input routine
	{
			...
			// SUBROUTINE LOCAL VARIABLE DECLARATIONS:
			int NumOfRoomAirflowNetControl;
			...
			if ( !RoomAirflowNetworkModelUsed ) {
				return;
			}
			...
			if ( NumOfRoomAirflowNetControl == 0 ) {
				ShowSevereError( "No " + cCurrentModuleObject + " objects found" );
				ErrorsFound = true;
				return;
			}
...
/*				RoomAirflowNetworkZoneInfo( ZoneNum ).AvailSched = cAlphaArgs( 3 );
				RoomAirflowNetworkZoneInfo( ZoneNum ).AvailSchedID = GetScheduleIndex( cAlphaArgs( 3 ) );
				if ( RoomAirflowNetworkZoneInfo( ZoneNum ).AvailSchedID == 0 ) {
...
/*								if ( SameString( "YES", cAlphaArgs( 1 + gainsLoop * 3 ) ) ) {
									RoomAirflowNetworkZoneInfo( ZoneNum ).Node( RAFNNodeNum ).IntGain( gainsLoop ).UseRoomAirModelTempForGains = true;
								}
								else if ( SameString( "NO", cAlphaArgs( 1 + gainsLoop * 3 ) ) ) {
									RoomAirflowNetworkZoneInfo( ZoneNum ).Node( RAFNNodeNum ).IntGain( gainsLoop ).UseRoomAirModelTempForGains = false;
								}
								else {
									ShowSevereError( "GetRoomAirflowNetworkData: Invalid " + cAlphaFieldNames( 1 + gainsLoop * 3 ) + " = " + cAlphaArgs( 1 + gainsLoop * 3 ) );
									ShowContinueError( "Entered in " + cCurrentModuleObject + " = " + cAlphaArgs( 1 ) );
									ShowContinueError( "incorrect type of internal gain" );
									ErrorsFound = true;
								} */
		...
		)
	{
			...
			// PURPOSE OF THIS FUNCTION:
			// This function looks up the given RoomAirNode name and returns RoomAir node
			// number.If not found, the return value will be zero.
			...
			// Return value
			int RoomAirNodeNum; // Node number
