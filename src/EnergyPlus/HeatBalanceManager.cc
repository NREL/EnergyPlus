...
namespace HeatBalanceManager {
	...
	GetMaterialData( bool & ErrorsFound ) // set to true if errors found in input
	{
		...
		for ( Loop = 1; Loop <= W5GlsMatAlt; ++Loop ) {
			...
			ReflectivitySol = power( ( ( MaterialProps( 2 ) - 1.0 ) / ( MaterialProps( 2 ) + 1.0 ) ), 2 );
			ReflectivityVis = power( ( ( MaterialProps( 4 ) - 1.0 ) / ( MaterialProps( 4 ) + 1.0 ) ), 2 );
			TransmittivitySol = std::exp( -MaterialProps( 3 ) * MaterialProps( 1 ) );
			TransmittivityVis = std::exp( -MaterialProps( 5 ) * MaterialProps( 1 ) );
			Material( MaterNum ).Trans = TransmittivitySol * ( power( ( 1.0 - ReflectivitySol ), 2 ) ) / ( 1.0 - power( ( ReflectivitySol * TransmittivitySol ), 2 ) );
			Material( MaterNum ).ReflectSolBeamFront = ReflectivitySol * ( 1.0 + ( power( ( 1.0 - ReflectivitySol ), 2 ) ) * ( power( TransmittivitySol, 2 ) ) / ( 1.0 - power( ( ReflectivitySol * TransmittivitySol ), 2 ) ) );
			Material( MaterNum ).ReflectSolBeamBack = Material( MaterNum ).ReflectSolBeamFront;
			Material( MaterNum ).TransVis = TransmittivityVis * ( power( ( 1.0 - ReflectivityVis ), 2 ) ) / ( 1.0 - power( ( ReflectivityVis * TransmittivityVis ), 2 ) );

			Material( MaterNum ).ReflectVisBeamFront = ReflectivityVis * ( 1.0 + ( power( ( 1.0 - ReflectivityVis ), 2 ) ) * ( power( TransmittivityVis, 2 ) ) / ( 1.0 - power( ( ReflectivityVis * TransmittivityVis ), 2 ) ) );
		...
		for ( Loop = 1; Loop <= TotScreens; ++Loop ) {
			...
			if ( MaterialProps( 5 ) > 0.0 ) {
				...
				if ( MaterialProps( 6 ) / MaterialProps( 5 ) >= 1.0 ) {
					...
					Material( MaterNum ).Trans = power( ( 1.0 - MaterialProps( 6 ) / MaterialProps( 5 ) ), 2 );
				}
			} else {
				ErrorsFound = true;
				ShowSevereError( CurrentModuleObject + "=\"" + MaterialNames( 1 ) + "\", Illegal value." );
		...
		for ( Loop = 1; Loop <= TotScreensEQL; ++Loop ) {
			...
			if ( Material( MaterNum ).ScreenWireSpacing > 0.0 ) {
				if ( Material( MaterNum ).ScreenWireDiameter / Material( MaterNum ).ScreenWireSpacing >= 1.0 ) {
					...
					//  Calculate direct normal transmittance (open area fraction)
					Openness = power( ( 1.0 - Material( MaterNum ).ScreenWireDiameter / Material( MaterNum ).ScreenWireSpacing ), 2 );
	...
	ReportWarmupConvergence()
	{
		...
		if ( ! WarmupFlag ) { // Report out average/std dev
			...
			for ( ZoneNum = 1; ZoneNum <= NumOfZones; ++ZoneNum ) {
				...
				StdDevZoneLoad = 0.0;
				for ( Num = 1; Num <= CountWarmupDayPoints; ++Num ) {
					TempZoneRptStdDev( Num ) = power( ( TempZoneRpt( Num, ZoneNum ) - AverageZoneTemp ), 2 );
					LoadZoneRptStdDev( Num ) = power( ( LoadZoneRpt( Num, ZoneNum ) - AverageZoneLoad ), 2 );
				}
	...
	SetupSimpleWindowGlazingSystem( int & MaterNum )
	{
		...
		if ( Material( MaterNum ).SimpleWindowUfactor > 4.5 ) {
			...
			if ( Material( MaterNum ).SimpleWindowSHGC < 0.7206 ) {
				...
				Material( MaterNum ).Trans = 0.939998 * power( Material( MaterNum ).SimpleWindowSHGC, 2 ) + 0.20332 * Material( MaterNum ).SimpleWindowSHGC;
			} else { // >= 0.7206
			...
			if ( Material( MaterNum ).SimpleWindowSHGC <= 0.15 ) {
				...
				Material( MaterNum ).Trans = 0.085775 * ( power( Material( MaterNum ).SimpleWindowSHGC, 2 ) ) + 0.963954 * Material( MaterNum ).SimpleWindowSHGC - 0.084958;
			}
		} else { // interpolate. 3.4 <= Ufactor <= 4.5

			if ( Material( MaterNum ).SimpleWindowSHGC < 0.7206 ) {
				TsolHiSide = 0.939998 * power( Material( MaterNum ).SimpleWindowSHGC, 2 ) + 0.20332 * Material( MaterNum ).SimpleWindowSHGC;
			} else { // >= 0.7206
			...
			if ( Material( MaterNum ).SimpleWindowSHGC <= 0.15 ) {
				...
				TsolLowSide = 0.085775 * ( power( Material( MaterNum ).SimpleWindowSHGC, 2 ) ) + 0.963954 * Material( MaterNum ).SimpleWindowSHGC - 0.084958;
			}
		...
		if ( Material( MaterNum ).SimpleWindowUfactor > 4.5 ) {
			...
			Ris = 1.0 / ( 29.436546 * power( DeltaSHGCandTsol, 3 ) - 21.943415 * power( DeltaSHGCandTsol, 2 ) + 9.945872 * DeltaSHGCandTsol + 7.426151 );
			Ros = 1.0 / ( 2.225824 * DeltaSHGCandTsol + 20.577080 );
		} else if ( Material( MaterNum ).SimpleWindowUfactor < 3.4 ) {

			Ris = 1.0 / ( 199.8208128 * power( DeltaSHGCandTsol, 3 ) - 90.639733 * power( DeltaSHGCandTsol, 2 ) + 19.737055 * DeltaSHGCandTsol + 6.766575 );
			Ros = 1.0 / ( 5.763355 * DeltaSHGCandTsol + 20.541528 );
		} else { // interpolate. 3.4 <= Ufactor <= 4.5
			//inside first
			RLowSide = 1.0 / ( 199.8208128 * power( DeltaSHGCandTsol, 3 ) - 90.639733 * power( DeltaSHGCandTsol, 2 ) + 19.737055 * DeltaSHGCandTsol + 6.766575 );
			RHiSide = 1.0 / ( 29.436546 * power( DeltaSHGCandTsol, 3 ) - 21.943415 * power( DeltaSHGCandTsol, 2 ) + 9.945872 * DeltaSHGCandTsol + 7.426151 );
		...
		if ( Material( MaterNum ).SimpleWindowVTinputByUser ) {
			...
			Material( MaterNum ).ReflectVisBeamBack = -0.7409 * power( Material( MaterNum ).TransVis, 3 ) + 1.6531 * power( Material( MaterNum ).TransVis, 2 ) - 1.2299 * Material( MaterNum ).TransVis + 0.4545;
			...
			Material( MaterNum ).ReflectVisBeamFront = -0.0622 * power( Material( MaterNum ).TransVis, 3 ) + 0.4277 * power( Material( MaterNum ).TransVis, 2 ) - 0.4169 * Material( MaterNum ).TransVis + 0.2399;
