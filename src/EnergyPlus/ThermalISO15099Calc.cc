...
namespace ThermalISO15099Calc {
	...
	)
	{
		...
		if ( SELECT_CASE_var == 0 ) { //ISO 15099
			...
			hcout = std::pow( ( second_power( ( 0.84 * std::pow( ( tw - tex ), 0.33 ) ) ) + second_power( ( acoef * std::pow( ws, bexp ) ) ) ), 0.5 );
		...
		if ( SHGCCalc > 0 ) {
			...
			if ( NeedUnshadedRun ) {
				...
				if ( WriteDebugOutput && ( UnshadedDebug == 1 ) ) {
					...
					gio::write( InArgumentsFile, "*" ) << "UNSHADED RUN:";
					gio::write( InArgumentsFile, "*" );
	...
	)
	{
		...
		for ( i = 1; i <= nlayer; ++i ) {
			...
			Rf( i ) = Radiation( k );
			Rb( i ) = Radiation( k + 1 );
			Ebf( i ) = StefanBoltzmann * fourth_power( theta( k ) );
			Ebb( i ) = StefanBoltzmann * fourth_power( theta( k + 1 ) );
		}
		//end if
		...
		if ( ( dir == 0.0 ) && ( nlayer > 1 ) ) {
			...
			if ( IsShadingLayer( LayerType( 1 ) ) ) {
				ShadeEmisRatioOut = qr_gap_out / ( emis( 3 ) * StefanBoltzmann * ( fourth_power( theta( 3 ) ) - fourth_power( trmout ) ) );
			...
			if ( IsShadingLayer( LayerType( nlayer ) ) ) {
				ShadeEmisRatioIn = qr_gap_in / ( emis( 2 * nlayer - 2 ) * StefanBoltzmann * ( fourth_power( trmin ) - fourth_power( theta( 2 * nlayer - 2 ) ) ) );
	...
	)
	{
		...
		for ( i = 1; i <= nlayer; ++i ) {
			...
			Ebf( i ) = StefanBoltzmann * fourth_power( theta( j - 1 ) );
			Ebb( i ) = StefanBoltzmann * fourth_power( theta( j ) );
		}
	...
	)
	{
		...
		if ( wsi > 0.0 ) { // main IF
			...
			gr = GravityConstant * third_power( height ) * delt * second_power( dens ) / ( tmean * second_power( visc ) );
	...
	)
	{
		...
		for ( i = 1; i <= nlayer - 1; ++i ) {
			...
			if ( presure( i + 1 ) > VacuumPressure ) {
				...
				// Calculate grashoff number:
				// The grashoff number is the Rayleigh Number (equation 5.29) in SPC142 divided by the Prandtl Number (prand):
				ra = GravityConstant * third_power( gap( i ) ) * delt * cp * second_power( dens ) / ( tmean * visc * con );
				Rayleigh( i ) = ra;
	...
	)
	{
		...
		for ( i = 1; i <= nlayer - 1; ++i ) {
			...
			if ( SupportPillar( i ) == YES_SupportPillar ) {
				...
				cpa = 2.0 * aveGlassConductivity * PillarRadius( i ) / ( ( second_power( PillarSpacing( i ) ) ) * ( 1.0 + 2.0 * gap( i ) / ( Pi * PillarRadius( i ) ) ) );
	...
	)
	{
		...
		if ( ( tilt >= 0.0 ) && ( tilt < 60.0 ) ) { //ISO/DIS 15099 - chapter 5.3.3.1
			...
			Nu1 = std::pow( ( 1.0 + seventh_power( ( ( 0.0936 * std::pow( ra, 0.314 ) ) / ( 1.0 + G ) ) ) ), ( 0.1428571 ) ); //equation 45
	...
	)
	{
		...
		if ( SDLayerIndex == 1 ) {
			...
			rayl = GravityConstant * third_power( gap_NOSD ) * delt * cp * second_power( dens ) / ( tmean * visc * con );
			asp = height / gap_NOSD;
			// determine the Nusselt number:
