...
namespace FuelCellElectricGenerator {
		...
		for ( thisConstit = 1; thisConstit <= FuelCell( GeneratorNum ).AirSup.NumConstituents; ++thisConstit ) {
			...
			if ( gasID > 0 ) {
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NISTShomate ) {
					...
					tempCp += ( ( A + B * Tsho + C * power( Tsho, 2 ) + D * power( Tsho, 3 ) + E / ( power( Tsho, 2 ) ) ) * FuelCell( GeneratorNum ).AirSup.ConstitMolalFract( thisConstit ) );
				}
				...
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NASAPolynomial ) {
					...
					tempCp += ( A1 + A2 * Tkel + A3 * power( Tkel, 2 ) + A4 * power( Tkel, 3 ) + A5 * power( Tkel, 4 ) ) * RinKJperMolpK * FuelCell( GeneratorNum ).AirSup.ConstitMolalFract( thisConstit );
		...
		for ( thisConstit = 1; thisConstit <= FuelCell( GeneratorNum ).AirSup.NumConstituents; ++thisConstit ) {
			...
			if ( gasID > 0 ) {
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NISTShomate ) {
					...
					HairI = ( A * Tsho + B * ( power( Tsho, 2 ) ) / 2.0 + C * ( power( Tsho, 3 ) ) / 3.0 + D * ( power( Tsho, 4 ) ) / 4.0 - E / Tsho + F - H );
				...
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NASAPolynomial ) {
					...
					tempHair += ( ( ( A1 + A2 * Tkel / 2.0 + A3 * power( Tkel, 2 ) / 3.0 + A4 * power( Tkel, 3 ) / 4.0 + A5 * power( Tkel, 4 ) / 5.0 + A6 / Tkel ) * RinKJperMolpK * Tkel ) - GasPhaseThermoChemistryData( gasID ).StdRefMolarEnthOfForm ) * FuelCell( GeneratorNum ).AirSup.ConstitMolalFract( thisConstit );
				}
			}
		}
		...
		for ( thisConstit = 1; thisConstit <= FuelSupply( FuelCell( GeneratorNum ).FuelSupNum ).NumConstituents; ++thisConstit ) {
			...
			if ( gasID > 0 ) {
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NISTShomate ) {
					...
					tempCp += ( ( A + B * Tsho + C * power( Tsho, 2 ) + D * power( Tsho, 3 ) + E / ( power( Tsho, 2 ) ) ) * FuelSupply( FuelCell( GeneratorNum ).FuelSupNum ).ConstitMolalFract( thisConstit ) );
				}
				...
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NASAPolynomial ) {
					...
					tempCp += ( A1 + A2 * Tkel + A3 * power( Tkel, 2 ) + A4 * power( Tkel, 3 ) + A5 * power( Tkel, 4 ) ) * RinKJperMolpK * FuelSupply( FuelCell( GeneratorNum ).FuelSupNum ).ConstitMolalFract( thisConstit );
		...
		for ( thisConstit = 1; thisConstit <= FuelSupply( FuelCell( GeneratorNum ).FuelSupNum ).NumConstituents; ++thisConstit ) {
			...
			if ( gasID > 0 ) {
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NISTShomate ) {
					...
					HfuelI = ( A * Tsho + B * ( power( Tsho, 2 ) ) / 2.0 + C * ( power( Tsho, 3 ) ) / 3.0 + D * ( power( Tsho, 4 ) ) / 4.0 - E / Tsho + F - H );
				...
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NASAPolynomial ) {
					...
					tempHfuel += ( ( ( A1 + A2 * Tkel / 2.0 + A3 * power( Tkel, 2 ) / 3.0 + A4 * power( Tkel, 3 ) / 4.0 + A5 * power( Tkel, 4 ) / 5.0 + A6 / Tkel ) * RinKJperMolpK * Tkel ) - GasPhaseThermoChemistryData( gasID ).StdRefMolarEnthOfForm ) * FuelSupply( FuelCell( GeneratorNum ).FuelSupNum ).ConstitMolalFract( thisConstit );
				}
			}
		}
		...
		for ( thisConstit = 1; thisConstit <= 5; ++thisConstit ) {
			...
			if ( gasID > 0 ) {
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NISTShomate ) {
					...
					tempHprodGases += ( ( A * Tsho + B * ( power( Tsho, 2 ) ) / 2.0 + C * ( power( Tsho, 3 ) ) / 3.0 + D * ( power( Tsho, 4 ) ) / 4.0 - E / Tsho + F - H ) * FuelCell( GeneratorNum ).FCPM.ConstitMolalFract( thisConstit ) );
				}
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NASAPolynomial ) {
					A1 = GasPhaseThermoChemistryData( gasID ).NASA_A1;
					A2 = GasPhaseThermoChemistryData( gasID ).NASA_A2;
					...
					tempHprodGases += ( ( ( A1 + A2 * Tkel / 2.0 + A3 * power( Tkel, 2 ) / 3.0 + A4 * power( Tkel, 3 ) / 4.0 + A5 * power( Tkel, 4 ) / 5.0 + A6 / Tkel ) * RinKJperMolpK * Tkel ) - GasPhaseThermoChemistryData( gasID ).StdRefMolarEnthOfForm ) * FuelCell( GeneratorNum ).FCPM.ConstitMolalFract( thisConstit );
				}
			} // gasid > 0
		}
		...
		for ( thisConstit = 1; thisConstit <= isize( FuelCell( GeneratorNum ).FCPM.GasLibID ); ++thisConstit ) {
			...
			if ( gasID > 0 ) {
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NISTShomate ) {
					...
					tempCp += ( ( A + B * Tsho + C * power( Tsho, 2 ) + D * power( Tsho, 3 ) + E / ( power( Tsho, 2 ) ) ) * FuelCell( GeneratorNum ).FCPM.ConstitMolalFract( thisConstit ) );
				}
				...
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NASAPolynomial ) {
					...
					tempCp += ( A1 + A2 * Tkel + A3 * power( Tkel, 2 ) + A4 * power( Tkel, 3 ) + A5 * power( Tkel, 4 ) ) * RinKJperMolpK * FuelCell( GeneratorNum ).FCPM.ConstitMolalFract( thisConstit );
		...
		for ( thisConstit = 1; thisConstit <= isize( FuelCell( GeneratorNum ).AuxilHeat.GasLibID ); ++thisConstit ) {
			...
			if ( gasID > 0 ) {
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NISTShomate ) {
					...
					tempCp += ( ( A + B * Tsho + C * power( Tsho, 2 ) + D * power( Tsho, 3 ) + E / ( power( Tsho, 2 ) ) ) * FuelCell( GeneratorNum ).AuxilHeat.ConstitMolalFract( thisConstit ) );
				}
				...
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NASAPolynomial ) {
					...
					tempCp += ( A1 + A2 * Tkel + A3 * power( Tkel, 2 ) + A4 * power( Tkel, 3 ) + A5 * power( Tkel, 4 ) ) * RinKJperMolpK * FuelCell( GeneratorNum ).AuxilHeat.ConstitMolalFract( thisConstit );
		...
		for ( thisConstit = 1; thisConstit <= isize( FuelCell( GeneratorNum ).ExhaustHX.GasLibID ); ++thisConstit ) {
			...
			if ( gasID > 0 ) {
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NISTShomate ) {
					...
					tempCp += ( ( A + B * Tsho + C * power( Tsho, 2 ) + D * power( Tsho, 3 ) + E / ( power( Tsho, 2 ) ) ) * FuelCell( GeneratorNum ).ExhaustHX.ConstitMolalFract( thisConstit ) );
				}
				...
				if ( GasPhaseThermoChemistryData( gasID ).ThermoMode == NASAPolynomial ) {
					...
					tempCp += ( A1 + A2 * Tkel + A3 * power( Tkel, 2 ) + A4 * power( Tkel, 3 ) + A5 * power( Tkel, 4 ) ) * RinKJperMolpK * FuelCell( GeneratorNum ).ExhaustHX.ConstitMolalFract( thisConstit );
	...
	)
	{
		...
		HGasWater = A * Tsho + B * ( power( Tsho, 2 ) ) / 2.0 + C * ( power( Tsho, 3 ) ) / 3.0 + D * ( power( Tsho, 4 ) ) / 4.0 - E / Tsho + F; //- H
	...
	)
	{
		...
		HLiqWater = A * Tsho + B * ( power( Tsho, 2 ) ) / 2.0 + C * ( power( Tsho, 3 ) ) / 3.0 + D * ( power( Tsho, 4 ) ) / 4.0 - E / Tsho + F; //- H
	...
	)
	{
		...
		Cp = A + B * Tsho + C * power( Tsho, 2 ) + D * power( Tsho, 3 ) + E / ( power( Tsho, 2 ) );
	...
	CalcFuelCellGenHeatRecovery( int const Num ) // Generator number
	{
		...
		if ( SELECT_CASE_var == FixedEffectiveness ) { //Method 1
			...
			UAeff = FuelCell( Num ).ExhaustHX.hxs0 + FuelCell( Num ).ExhaustHX.hxs1 * NdotWater + FuelCell( Num ).ExhaustHX.hxs2 * ( power( NdotWater, 2 ) ) + FuelCell( Num ).ExhaustHX.hxs3 * NdotGas + FuelCell( Num ).ExhaustHX.hxs4 * ( power( NdotGas, 2 ) );
			...
			hgas = FuelCell( Num ).ExhaustHX.h0gas * power( ( NdotGas / FuelCell( Num ).ExhaustHX.NdotGasRef ), FuelCell( Num ).ExhaustHX.nCoeff ); //Eq. 48

			hwater = FuelCell( Num ).ExhaustHX.h0Water * power( ( NdotWater / FuelCell( Num ).ExhaustHX.NdotWaterRef ), FuelCell( Num ).ExhaustHX.mCoeff ); //Eq. 48
			...
			if ( MdotWater != 0.0 ) {
				...
				NdotWaterCond = ( TcondThresh - TwaterIn ) * ( hxl1 * ( NdotWaterVapor / NdotGas ) + hxl2 * power( ( NdotWaterVapor / NdotGas ), 2 ) );
