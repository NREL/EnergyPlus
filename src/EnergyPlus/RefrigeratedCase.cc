	...
	CalculateWalkIn( int const WalkInID ) // Absolute pointer to  Walk In
	{
		...
		for ( ZoneID = 1; ZoneID <= WalkIn( WalkInID ).NumZones; ++ZoneID ) {
			...
			if ( StockDoorArea > 0.0 || GlassDoorArea > 0.0 ) {
				...
				EnthalpyZoneAir = PsyHFnTdbRhPb( ZoneDryBulb, ZoneRHFrac, OutBaroPress ); // *&^unique^&* RoutineName
	...
	)
	{
		...
		if ( DefrostDripDownSchedule == 1.0 ) {
			...
			if ( SensLoadRequested <= 0.0 ) { //No load so assume control keeps off, except that scheduled defrost still occurs
				...
				ZoneMixedAirEnthalpy = PsyHFnTdbRhPb( ZoneMixedAirDryBulb, ZoneMixedAirRHFrac, OutBaroPress ); // *&^unique^&* TrackMessage
				...
				{ auto const SELECT_CASE_var( WarehouseCoil( CoilID ).VerticalLocation );
		...
		if ( AirVolumeFlowMax > 0.0 ) {
			...
			if ( WarehouseCoil( CoilID ).RatingType == RatedCapacityTotal ) {
				...
				if ( SensibleCapacityMax > 0.0 ) {
					...
					ExitEnthalpyEstimate = PsyHFnTdbRhPb( ExitTemperatureEstimate, 1.0, OutBaroPress ); // *&^unique^&* TrackMessage
