...
class TowerControl(EnergyPlusPlugin):
    ...
    def on_inside_hvac_system_iteration_loop(self) -> int:
        if self.need_to_get_handles:
            self.outdoor_air_temp_handle = self.api.exchange.get_variable_handle("Site Outdoor Air Temperature",
                                                                                 "Environment")
        ...
        if self.api.exchange.get_variable_value(self.outdoor_air_temp_handle) < 6.0:
            ...
            self.api.exchange.set_actuator_value(self.chiller_condenser_loop_handle, self.api.exchange.reset_actuator())
            self.api.exchange.set_actuator_value(self.chiller_cond_pump_handle, self.api.exchange.reset_actuator())
