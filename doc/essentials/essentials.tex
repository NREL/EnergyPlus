...
normal EnergyPlus input files, but since it is using Ruby, it has
...
input object in series. Each input object starts with the kind of
input object, and each value for each field follows while being separated
by commas. The end of the input object is indicated by a semi-colon.
Comments are indicated by an exclamation point ``!'' and anything
...
IP (inch-pound) units can be used. It can initialize an input object
using the default values and has indications when values outside the
acceptable range are used. The main screen of the IDF editor is shown
below. Full details of the IDF Editor can be found in the Auxiliary
Programs document under the ``Creating Input Files'' section.
...
report of the bin and monthly data in a comma, tab, or space delimited
