...
Solar energy being absorbed on the exterior wall and roof surfaces
as well as entering a window varies by the sun position for each period
...
and heat that is absorbed by the surfaces and released slowly over
...
the building, and ultimately their operation accounts for a large
portion of the energy consumed in a building. For these reasons and
more, what might first appear as something that can be calculated
...
\subsection{Questions that BEM can answer}
...
building energy modeling. It described a methodology to apply building
...
to quantify the impact of design decision s at the point in time which
...
as energy and load simulation tools. BLAST was developed by \href{https://www.erdc.usace.army.mil/Locations/CERL/}{Construction Engineering Research Laboratory (CERL)}
and \href{https://illinois.edu/}{the University of Illinois} which
...
\end{itemize}
The example files are a great resource to learn more about how to
use EnergyPlus The example files are in the directory ExampleFiles.
...
approach is very simple but limited in the flexibility that it affords.
It is suitable for implementing measures related to internal loads,
constructions, and simple efficiency changes but is probably not the
appropriate tool for more complicated measures.
...
that describes these approaches to scripting was made during the 2018
...
parametric analysis, a few specific tools have been developed. \href{http://nrel.github.io/OpenStudio-user-documentation/reference/parametric_studies/}{OpenStudio Parametric Analysis}
...
user interface to describe the building. However, it can be run standalone
...
\includegraphics[width=6in]{eplaunch3}
...
documented on \href{https://ep-launch.readthedocs.io/en/}{readthedocs}
or in the docs folder on GitHub. EP-Launch works on Windows, MacOS,
...
the input object, and each value for each field follows while being
...
It is a text-based file format. The JSON format has wide industry
usage and is supported in just about every modern programming language.
It is a field-value style format using brackets and colons to indicate
the hierarchy and commas to separate each field and value pair. The
...
directory. It is a Windows-only program and was not designed to run
on Linux or MacOS. It is specially designed for editing IDF files
...
is currently being developed independently and will have many of the
...
systems are not behaving as expected. With an input file representing
...
to run a design day (see SizingPeriod:DesignDay) or a subset of the
year (see RunPeriod). This approach speeds up the simulation time
itself, and if used, please remember to recheck the .ERR file when
running an annual simulation for the first time.
...
with EnergyPlus contains these kinds of details and to use them, the
...
material is shown with a thickness along with the conductivity, density,
specific heat and other factors. These thicknesses should match the
thicknesses shown in the detailed cross-sections. But when it comes
to specifying the walls themselves in three-dimensional space, the
walls should be entered assuming zero thickness. Once each surface
...
For most modern buildings the choice of inside vs. outside vs. centerline
should have little impact on results so many modelers just pick one
a let the volumes be off a little. Using centerlines throughout the
model splits the difference. Or some modelers use outer edges for
exterior walls and then use centerlines for interior walls. If you
are modeling a very thick wall, such as an old stone building, then
you also have thermal mass considerations. If you use the outside
edges there will be too much mass, inside will be too little. Again,
centerline will split the difference and will be very close to the
...
\subsubsection*{Available Outputs Created by First Simulation}
...
can aid in the setting up input files or extracting or converting
...
\item CSV, TAB, or TXT -- tabulated results in a comma, tab, or space delimited
...
report of bin and monthly data in a comma, tab, or space delimited
or HTML format or XML format
...
document.
\end{itemize}
\includegraphics[width=5in]{IDFVersionUpdater}
\begin{itemize}
...
\end{itemize}
Finally, two other input objects should appear in your input file
and are included in almost all example files:
\begin{itemize}
...
temperature algorithm, specifies the average temperature for each
month of the year.
...
\subsection{Surface Construction Elements}
...
gap. Note it cannot be used for windows.
...
in windows and skylights have a significant impact on the heat transfer
...
\subsection{Thermal Zones and Surfaces}
...
\includegraphics[width=6in]{coordinatesystem}Figure X. EnergyPlus
Coordinate System.
\begin{itemize}
...
more) essentially identical are each simulated. The coordinates can
be just set to zero if the world coordinate system is used (see GlobalGeometryRules
below).
...
It has a field for a multiplier although usually the multiplier is
...
Casting shadows on the building, especially on to windows, can significantly
...
objects to help map them closer to the mental representation of real
surfaces in buildings. These input objects are effectively just different
ways to represent the same information as the BuildingSurface:Detailed,
FenestrationSurface:Detailed, and Shading:Zone:Detailed input objects.
These objects are seldom created by graphical user interfaces, so
it is unlikely that you will see them. Some of these objects also
...
one that changes relative to each zone.
...
the frame usually significantly impacts the overall performance of
a window.
\end{itemize}
\includegraphics[width=5in]{window_frame_and_divider}Figure X - A
window with frame and divider
\begin{itemize}
...
else in the model - often used to capture the effect of interior floors
...
these items that produce heat within a building is called internal
...
\subsection{Advanced Construction, Surface, Zone Concepts}
...
surfaces when the building level heat transfer algorithm should not
...
for complex windows, a way to override the longwave radiation with
...
\subsection{Zone Airflow}

The Zone Airflow input output group provides a way to model the airflow
...
by EnergyPlus; instead, a preprocessor called ExpandObjects turns
these HVACTemplate input objects into the standard input objects.
This can be used as a learning tool since you can review the .expidf
file that contains the standard input objects or it can be used for
long term modeling especially if you focus is not on HVAC related
energy efficiency measures. The primary disadvantage of using the
HVACTemplate input objects is that only a small subset of possible
configurations can be modeled using them. The subset of configurations
that can be used was carefully chosen to be some of the most common
HVAC configurations. If the HVACTemplate input objects do not support
the HVAC configuration you are considering; you need to model it with
the standard EnergyPlus input objects. When using HVACTemplate input
objects, no regular standard EnergyPlus input objects related to HVAC
should be used (see the InputOutputReference for some exceptions to
this). The HVACTemplates can model:
\begin{itemize}
...
systems
\end{itemize}
The Input Output Reference includes a list of exactly which objects
...
their connections with one another and their control systems. The
previous section described the HVACTemplates which simplify the HVAC
input for EnergyPlus for common configurations. 

An EnergyPlus HVAC simulation consists of various components that
are connected physically in the actual system by ducts, piping, etc.
Every component in an HVAC system must have an inlet and outlet \textquotedblleft node.\textquotedblright{}
...
are constructed by defining these loops and also defining the components
on the loops. The figure below shows a generic example of the loop-node
concept. Loop nodes are a key defining feature in EnergyPlus. As a
result, it is recommended that one of the first steps taken in defining
an HVAC system in EnergyPlus be the definition of a node diagram or
map. This is helpful for visualization of the entire system. 

\includegraphics[width=6in]{NodeDiagram}
...
(for a 2 deck system). An outdoor air subsystem can be included in
the supply side for ventilation and relief air.
...
side of this loop can also include mixers, flow splitters, and a bypass.
...
the demand side, this loop can also include mixers, flow splitters,
and a bypass.
\end{itemize}
...
air equipment like window air conditioners, unit heaters, fan coil
systems, and outdoor air units as well as other related input objects
...
object for the special case of flor control with thermal storage systems
\end{itemize}
...
\item Non-Zone Equipment - describe the LoadProfile:Plant input object that
allows the load on the plant to be specified by schedule if already
known
...
types of outputs, but initially the most important to understand is
...
the output meters appears in a file with a .mdd extension.
\end{itemize}
...
This approach lets you more deeply understand the way the item is
simulated by seeing how output variables and related driving output
...
The output from this input objects shows up in the .mtr file which
...
file and the output appears in a file with the .sln extension
...
\end{itemize}
Environmental reporting which adds new meters showing the related
to emissions is triggered by the use of the following three input
objects
\begin{itemize}
...
the normal error file. All of these are described in the \textquotedblleft Input/Output
...
The input objects used by EnergyPlus for cost estimating calculations
are as follows:
\begin{itemize}
\item ComponentCost:LineItem - provides inputs related to determining the
construction cost and can be associated with several different objects
and the cost estimate is based on factors such as cost per area or
cost per unit of output capacity. It has not been applied to all types
of input objects so for many scenarios; construction costing would
need to be performed outside of EnergyPlus.
...
the computation of estimated utility bills allows an understanding
of how much money is going to be saved to help offset higher initial
costs of high-performance building. Due to the many different utility
...
depending not only on energy and demand but the timing of each and
the relationship between the two. Overall, the goals of the UtilityCost
input objects of EnergyPlus are to make simple tariffs easy to model
and complex tariffs possible to model. The input objects used by EnergyPlus
for the calculation of monthly and annual utility bill are as follows:
\begin{itemize}
\item UtilityCost:Tariff - sets the parameters for the overall tariff such
as the name, associated output meter, conversion factors, demand computation,
...
of Economic Charges.
\item UtilityCost:Charge:Block - defines a most complicated but common type
...
\end{itemize}
\includegraphics[width=5in]{tariff-charges}Figure X Hierarchy of
Economic Charges
...
all have different initial and reoccurring future costs. The input
objects used for performing a life-cycle cost analysis are:
\begin{itemize}
...
\subsection{Other Groups of Input Objects}
...
objects that are then processed by the SLAB and BASEMENT programs
...
\item Refrigeration - describes supermarket refrigeration systems 
