...
Jet temperature rise & \$\textbackslashDelta \{T\_J\}\$ & °C & Volume-averaged temperature variations in the jet region, over the same volume used to define the jet velocity average. \tabularnewline
Recirculation zone temperature rise & \$\textbackslashDelta \{T\_R\}\$ & °C & Volume-average temperature variations in the recirculation region. The average is calculated over the cuboidal volume placed in each recirculation containing the volumetric sensible heat gains (see main text). \tabularnewline
\bottomrule
\end{longtable}
...
\toprule 
~ & Y & X & a & b & R \tabularnewline \midrule
\endhead
Jet velocity & \$\{U\_J\}/\{U\_\{in\}\}\$ & \$\textbackslashsqrt \{\{A\_\{in\}\}/\{A\_\{RM\}\}\} \textbackslashcdot \textbackslashleft( \{\{V\_\{J,m\}\}/\{U\_\{in\}\}\} \textbackslashright)\$ & 1.6173 & 0.1466 & 0.8343 \tabularnewline
Recirculation zone velocity & \$\{U\_R\}/\{U\_\{in\}\}\$ & \$\textbackslashsqrt \{\{A\_\{in\}\}/\{A\_\{RM\}\}\} \textbackslashcdot \textbackslashleft( \{\{V\_\{J,m\}\}/\{U\_\{in\}\}\} \textbackslashright)\$ & 0.8911 & 0.0393 & 0.6464 \tabularnewline
Recirculation zone flow rate & \$\{Q\_R\}/\{U\_\{in\}\}\$ & \$\textbackslashsqrt \{\{A\_\{in\}\}\{A\_\{RM\}\}\} \textbackslashcdot \textbackslashleft( \{\{V\_\{J,m\}\}/\{U\_\{in\}\}\} \textbackslashright)\$ & 0.4444 & 0.1751 & 0.6761 \tabularnewline
Jet temperature rise & \$\textbackslashDelta \{T\_J\}\$ & \$\textbackslashfrac\{\{q'\{'\_\{RM\}\}\}\}\{\{\textbackslashrho \{c\_p\}\{Q\_\{in\}\}\}\}\$ & 0.8254 & 0 & n/a \tabularnewline
Recirculation zone temperature rise & \$\textbackslashDelta \{T\_R\}\$ & \$\textbackslashfrac\{\{q'\{'\_\{RM\}\}\}\}\{\{\textbackslashrho \{c\_p\}\{Q\_\{in\}\}\}\}\$ & 1.2734 & 0 & n/a \tabularnewline
\bottomrule
\end{longtable}
...
\(\sqrt {{A_{in}}/{A_{RM}}} \left( {{V_{J,m}}/{U_{in}}} \right)\) \({U_J}/{U_{in}}\) \includegraphics{media/image2652.png}

Figure 140. Jet velocity correlation.

\(\sqrt {{A_{in}}/{A_{RM}}} \left( {{V_{J,m}}/{U_{in}}} \right)\) \({U_R}/{U_{in}}\) \includegraphics{media/image2655.png}

Figure 141. Recirculation region velocity correlation.

\(\sqrt {{A_{in}}{A_{RM}}} \left( {{V_{J,m}}/{U_{in}}} \right)\) \({Q_R}/{U_{in}}\) \includegraphics{media/image2658.png}
...
% table 48
\begin{longtable}[c]{p{1.72in}p{1.5in}p{2.77in}}
...
A & m & Inlet area \tabularnewline
A & m & Room cross-sectional area \tabularnewline
b & - & Intercept of correlation line of best fit \tabularnewline
c & J/kg/K & Specific heat capacity of air \tabularnewline
C & - & Confinement scaling factor, (A  /A  ) \tabularnewline
C & - & Correlation constant for jet velocity \tabularnewline
C & - & Correlation constant for recirculation velocity \tabularnewline
C & - & Correlation constant for recirculation flow rate \tabularnewline
...
Q & m  /s & Inlet flow rate, A   U \tabularnewline
~~~~~~~~~~~ \$\{Q\_R\}\$ & m  /s & Maximum recirculation region flow rate \tabularnewline
U & m/s & Inlet velocity, average over inlet area \tabularnewline
U & m/s & Maximum centerline velocity of jet \tabularnewline
U & m/s & Uniform inlet velocity of jet \tabularnewline
~~~~~~~~~~~ \$\{U\_J\}\$ & m/s & Volume-averaged jet velocity \tabularnewline
~~~~~~~~~~~ \$\{U\_R\}\$ & m/s & Average recirculation region velocity over plane of maximum flow \tabularnewline
V & m/s & Velocity scale \tabularnewline
V & m/s & Average of U   over room depth \tabularnewline
W & m & Room width \tabularnewline
x & m & Coordinate along room depth, 0 ≤ x ≤ D \tabularnewline
...
~~~~~~~~~~~ \$\textbackslashDelta \{T\_J\}\$ & °C & Average jet temperature variation \tabularnewline
\$\textbackslashDelta \{T\_R\}\$ & °C & Average recirculation region temperature variation \tabularnewline
r & kg/m & Density of air \tabularnewline
φ & ° & Wind direction relative to façade normal \tabularnewline
\bottomrule
\end{longtable}
...
\begin{equation}
{F_i}{C_{z,i}}\frac{{d{T_{z,i}}}}{{dt}} = \sum\limits_{j = 1}^{{N_{i,g}}} {{F_{gain,j}{\dot Q}_{i,j}}}  + \sum\limits_{j = 1}^{{N_{i,sur}}} {{h_{i,j}}} {A_j}\left( {{T_{s,j}} - {T_{z,i}}} \right) + \sum\limits_{j = 1}^{{N_{i,AFN}}} {{{\dot m}_{AFN i,j}}} {C_p}\left( {{T_{z,j}} - {T_{z,i}}} \right)\, + \sum\limits_{j = 1}^{{N_{i,HVAC}}} {{{F_{HVAC,j}}{\dot m}_{sup,i,j}}} {C_p}\left( {{T_{sup_j}} - {T_{z,i}}} \right)
\end{equation}
...
\begin{equation}
{F_i}{M_{z,i}}\frac{{d{W_{z,i}}}}{{dt}} = \sum\limits_{j = 1}^{{N_{i,g}}} {{F_{gain,j}{\dot Q}_{i,j}}}  + \sum\limits_{j = 1}^{{N_{i,sur}}} {{h_{M,j}}} {A_j}\left( {{W_{s,j}} - {W_{z,i}}} \right) + \sum\limits_{j = 1}^{{N_{i,AFN}}} {{{\dot m}_{AFN i,j}}} \left( {{W_{z,j}} - {W_{z,i}}} \right)\, + \sum\limits_{j = 1}^{{N_{i,HVAC}}} {{{F_{HVAC,j}}{\dot m}_{sup,i,j}}} \left( {{W_{sup_j}} - {W_{z,i}}} \right)
\end{equation}
