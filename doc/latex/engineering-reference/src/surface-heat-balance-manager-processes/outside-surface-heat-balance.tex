...
where: \(_{ }\) = Absorbed direct and diffuse solar (short wavelength) radiation heat flux. \(_{ }\) = Net long wavelength (thermal) radiation flux exchange with the air and surroundings. \(_{ }\) = Convective flux exchange with outside air. \(_{ }\) = Conduction heat flux (q/A) into the wall.
...
\(_{ }\)~is calculated using procedures presented later in this manual and includes both direct and diffuse incident solar radiation absorbed by the surface face.~ This is influenced by location, surface facing angle and tilt, surface face material properties, weather conditions, etc.
...
\emph{T\(_{t}\)} = weather file air temperature (measured at the meteorological station)

\emph{z\(_{t}\)} = height above ground of the air temperature sensor at the meteorological station.

The default value for \emph{z\(_{t}\)} for air temperature measurement is 1.5 m above ground.~ This value can be overridden by using the Site:WeatherStation object.
...
\emph{a} = wind speed profile exponent at the site

\emph{d} = wind speed profile boundary layer thickness at the site

\emph{z\(_{t}\)} = height above ground of the wind speed sensor at the meteorological station

\emph{V\(_{t}\)} = wind speed measured at the meteorological station

\emph{a\(_{t}\)} = wind speed profile exponent at the meteorological station

\emph{d\(_{t}\)} = wind speed profile boundary layer thickness at the meteorological station.

The wind speed profile coefficients \emph{a}, \emph{d}, \emph{a\(_{t}\)}, and \emph{d\(_{t}\)}, are variables that depend on the roughness characteristics of the surrounding terrain.~ Typical values for \emph{a} and d are shown in the following table:
...
The terrain types above map to the options in the \emph{Terrain} field of the Building object.~ The \emph{Terrain} field can be overridden with specific values for \emph{a} and d by using the Site:HeightVariation object.

The default value for \emph{z\(_{t}\)} for wind speed measurement is 10 m above ground.~ The default values for \emph{a\(_{t}\)} and \emph{d\(_{t}\)} are 0.14 and 270 m, respectively, because most meteorological stations are located in an open field.~ These values can be overridden by using the Site:WeatherStation object.
...
When the AdaptiveConvectionAlgorithm is used, there is a second, deeper level of control available for selecting among a larger variety of \emph{h\(_{t}\)} equations and also defining custom equations using curve or table objects.~ These options are described in this section.
...
A & Surface area of the surface & m & /=0 \tabularnewline
h & Surface exterior convective heat transfer coefficient & W/(m  K) & - \tabularnewline
h & Forced convective heat transfer coefficient & W/(m  K) & - \tabularnewline
h & Natural convective heat transfer coefficient & W/(m  K) & - \tabularnewline
P & Perimeter of surface & m & - \tabularnewline
R & Surface roughness multiplier & - & - \tabularnewline
T & Local outdoor air temperature calculated at the height above ground of the surface centroid & °C & - \tabularnewline
T & Outside surface temperature & °C & - \tabularnewline
DT & Temperature difference between the surface and air, & °C & - \tabularnewline
V & Local wind speed calculated at the height above ground of the surface centroid & m/s & - \tabularnewline
W & Wind direction modifier & - & - \tabularnewline
f & Angle between the ground outward normal and the surface outward normal & degree & - \tabularnewline
...
\toprule 
Variable & Description & Units & Range \tabularnewline \midrule \endhead
A & Constant & W/(m  K(m/s) & - \tabularnewline
B & Constant & - & - \tabularnewline
C & Turbulent natural convection constant & W/(m  K  ) & - \tabularnewline
h & Surface exterior convective heat transfer coefficient & W/(m  K) & - \tabularnewline
T & Outside surface temperature & °C/K & - \tabularnewline
DT & Temperature difference between the surface and air & °C/K & - \tabularnewline
\bottomrule
\end{longtable}
...
\toprule 
Wind Direction & Ct & a & b \tabularnewline \midrule
\endhead
(Units) & W/m  K & W/m  K(m/s) & - \tabularnewline
...
b & Constant & - & - \tabularnewline
h & Surface exterior convective heat transfer coefficient & W/(m  K) & - \tabularnewline
h & Convective heat transfer coefficient for very smooth surfaces (glass) & W/(m  K) & - \tabularnewline
h & Natural convective heat transfer coefficient & W/(m  K) & - \tabularnewline
R & Surface roughness multiplier & - & - \tabularnewline
T & Outside surface temperature & °C/K & - \tabularnewline
DT & Temperature difference between the surface and air, & °C/K & - \tabularnewline
f & Angle between the ground outward normal and the surface outward normal & radian & - \tabularnewline
\bottomrule
\end{longtable}
...
% table 12
\begin{longtable}[c]{p{0.25in}p{0.75in}p{0.75in}p{0.75in}p{1.75in}p{1.75in}}
\caption{Adaptive Convection Algorithm Details \label{table:adaptive-convection-algorithm-details}}\\
\toprule 
\# & Surface Classification & Heat Flow Direction & Wind Direction & h Models & h Models \tabularnewline \midrule \endhead
...
Where \emph{V\(_{m}\)} is the air velocity at the location of the weather station and θ is the angle of incidence between the wind and the surface in degrees.~ The following equations are used for horizontal (roof) surfaces (key: EmmelRoof):
