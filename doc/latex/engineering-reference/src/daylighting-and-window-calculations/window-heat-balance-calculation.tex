...
\toprule 
\textbf{Mathematical variable} & \textbf{Description} & \textbf{Units} & \textbf{FORTRAN \quad variable} \tabularnewline \midrule
\endhead
N & Number of glass layers & - & nlayer \tabularnewline \midrule
$\alpha$ & Stefan-Boltzmann constant & ~ & sigma \tabularnewline \midrule
$\varepsilon$\(_{i}\) & Emissivity of face i & - & emis \tabularnewline \midrule
k\(_{i}\) & Conductance of glass layer i & W/m\(^{2}\)-K & scon \tabularnewline \midrule
h\(_{o}\), h\(_{i}\) & Outside, inside air film convective conductance & W/m\(^{2}\)-K & hcout, hcout \tabularnewline \midrule
h\(_{i}\) & Conductance of gap j & W/m\(^{2}\)-K & hgap \tabularnewline \midrule
T\(_{o}\), T\(_{i}\) & Outdoor and indoor air temperatures & K & tout, tin \tabularnewline \midrule
E\(_{o}\), E\(_{i}\) & Exterior, interior long-wave radiation incident on window & W/m\(^{2}\) & outir, rmir \tabularnewline \midrule
$\theta$\(_{i}\) & Temperature of face i & K & thetas \tabularnewline \midrule
S\(_{i}\) & Radiation (short-wave, and long-wave from zone internal sources) absorbed by face i & W/m\(^{2}\) & AbsRadGlassFace \tabularnewline \midrule
I\(^{ext}_{bm}\) & Exterior beam normal solar irradiance & W/m\(^{2}\) & BeamSolarRad \tabularnewline \midrule
I\(^{ext}_{dif}\) & Exterior diffuse solar irradiance on glazing & W/m\(^{2}\) & - \tabularnewline \midrule
I\(^{int}_{sw}\) & Interior short-wave radiation (from lights and from reflected diffuse solar) incident on glazing from inside & W/m\(^{2}\) & QS \tabularnewline \midrule
I\(^{int}_{lw}\) & Long-wave radiation from lights and equipment incident on glazing from inside & W/m\(^{2}\) & QL \tabularnewline \midrule
$\varphi$ & Angle of incidence & radians & - \tabularnewline \midrule
A\(^{f}_{j}\) & Front beam solar absorptance of glass layer j & - & - \tabularnewline \midrule
A\(^{f,diff}_{j}\), A\(^{b,diff}_{j}\) & Front and back diffuse solar absorptance of glass layer j & - & AbsDiff, AbsDiffBack \tabularnewline \midrule
A, B & Matrices used to solve glazing heat balance equations & W/m\(^{2}\), W/m\(^{2}\)-K & Aface, Bface \tabularnewline \midrule
h\(_{r,i}\) & Radiative conductance for face i & W/m\(^{2}\)-K & hr(i) \tabularnewline \midrule
...
\endhead
\$\textbackslashbar h\$ & Area-weighted net conductance of glazing including edge-of-glass effects & W/m  -K & - \tabularnewline
A & Area of center-of-glass region & m & CenterGlArea \tabularnewline
A & Area of frame edge region & m & FrameEdgeArea \tabularnewline
A & Area of divider edge region & m & DividerEdgeArea \tabularnewline
A & Total glazing area & m & Surface\%Area \tabularnewline
h & Conductance of center-of-glass region (without air films) & W/m  -K & - \tabularnewline
h & Conductance of frame edge region (without air films) & W/m  -K & - \tabularnewline
h & Conductance of divider edge region (without air films) & W/m  -K & - \tabularnewline
h & Convective conductance of gap k & W/m  -K & - \tabularnewline
h & Radiative conductance of gap k & W/m  -K & - \tabularnewline
η & Area ratio & - & - \tabularnewline
α & Conductance ratio & - & FrEdgeToCenterGlCondRatio, DivEdgeToCenterGlCondRatio \tabularnewline
\bottomrule
\end{longtable}
...
\emph{A\(_{g}\)} = area of center-of-glass region
...
\$\{h\_\{c,k\}\} = \$ convective conductance of the \emph{k\(^{h}\)} gap

\$\{h\_\{r,k\}\} = \$ radiative conductance of the \emph{k\(^{h}\)} gap
...
\$\{\varepsilon\_i\},\{\varepsilon\_j\} = \$ emissivity of the faces bounding the gap

\$\{\theta\_i\},\{\theta\_j\} = \$ temperature of faces bounding the gap (K)
...
Mathematical variable & Description & Units & FORTRAN variable \tabularnewline \midrule
\endhead
Q & IR from the exterior surround absorbed by outside frame surfaces & W & - \tabularnewline
Q & IR emitted by outside frame surfaces & W & - \tabularnewline
Q & Convection from outside air to outside frame surfaces & W & - \tabularnewline
Q & Conduction through frame from inside frame surfaces to outside frame surfaces & W & - \tabularnewline
Q & Solar radiation plus outside glass IR absorbed by outside of frame & W & - \tabularnewline
Q & Diffuse solar absorbed by outside frame surfaces, per unit frame face area & W/ m & - \tabularnewline
Q & Beam solar absorbed by outside frame surfaces, per unit frame face area & W/ m & - \tabularnewline
I & Diffuse solar incident on window & W/ m & - \tabularnewline
I & Direct normal solar irradiance & W/ m & - \tabularnewline
α & Solar absorptance of frame & - & FrameSolAbsorp \tabularnewline
R & Front diffuse solar reflectance of glazing & - & ~ \tabularnewline
R & Front beam solar reflectance of glazing & - & ~ \tabularnewline
cos(β  ) & Cosine of angle of incidence of beam solar on frame outside face & ~ & CosIncAng \tabularnewline
Cos(β  ) & Cosine of angle of incidence of beam solar on frame projection parallel to window x-axis & - & CosIncAngHorProj \tabularnewline
Cos(β  ) & Cosine of angle of incidence of beam solar on frame projection parallel to window y-axis & - & CosIncAngVertProj \tabularnewline
f & Fraction of window that is sunlit & - & SunlitFrac \tabularnewline
A & Area of frame’s outside face (same as area of frame’s inside face) & m & - \tabularnewline
A  , A & Area of frame’s outside and inside projection faces & m & - \tabularnewline
F & Form factor of frame’s outside or inside face for IR & - & - \tabularnewline
F  , F & Form factor of frame outside projection for exterior IR; form factor of frame inside projection for interior IR & - & - \tabularnewline
E & Exterior IR incident on window plane & W/m & outir \tabularnewline
E & Interior IR incident on window plane & W/m & SurroundIRfromParentZone \tabularnewline
ε  , ε & Outside, inside frame surface emissivity & - & FrameEmis \tabularnewline
θ  , θ & Frame outside, inside surface temperature & K & FrameTempSurfOut, FrameTempSurfIn \tabularnewline
T  , T & Outdoor and indoor air temperatures & K & tout, tin \tabularnewline
h  , h & Frame outside and inside air film convective conductance & W/m2-K & HOutConv, HInConv \tabularnewline
k & Effective inside-surface to outside-surface conductance of frame per unit area of frame projected onto window plane & W/m  -K & FrameConductance, FrameCon \tabularnewline
S & Q  /A & W/m  -K & FrameQRadOutAbs \tabularnewline
S & Interior short-wave radiation plus interior IR from internal sources absorbed by inside of frame divided by A & W/m  -K & FrameQRadInAbs \tabularnewline
η  , η & A  /A  , A  /A & - & - \tabularnewline
...
w  , w & Frame width, divider width & m & FrameWidth, DividerWidth \tabularnewline
p  , p & Frame outside, inside projection & m & FrameProjectionOut, FrameProjectionIn \tabularnewline
N  , N & Number of horizontal, vertical dividers & - & HorDividers, VertDividers \tabularnewline
T  , T & Frame outside, inside radiative temperature & K & TOutRadFr, TInRadFr \tabularnewline
h  , h & Frame outside, inside surface radiative conductance & W/m  -K & HOutRad, HInRad \tabularnewline
...
\emph{τ\(_{h}\)} = IR diffuse transmittance of shading device

\emph{ε\(_{h}\)} = diffuse emissivity of shading device

\emph{ρ\(_{h}\)} = IR diffuse reflectance of shading device ( = 1 - ( \emph{τ\(_{h}\)} + \emph{ε\(_{h}\)}))

\emph{θ\(_{5}\)} = temperature of the surface of the shading layer that faces the gap (K).

The term 1 -- \emph{ρ\(_{4}\) ρ\(_{h}\)} accounts for the inter-reflection of IR radiation between glass and shading layer.
...
\emph{k\(_{h}\)} = shading layer conductance (W/m\(^{2}\)K).

\emph{θ\(_{6}\)} = temperature of shading layer surface facing the zone air (K).

\emph{S\(_{1}\)} = solar radiation plus short-wave radiation from lights plus IR radiation from lights and zone equipment absorbed by the gap-side face of the shading layer (W/m\(^{2}\)K).
...
\emph{S\(_{2}\)} = solar radiation plus short-wave radiation from lights plus IR radiation from lights and zone equipment absorbed by the zone-side face of the shading layer (W/m\(^{2}\)K).
...
\caption{  Vertical section (a) and perspective view (b) of glass layer and interior shading layer showing variables used in the gap airflow analysis. The opening areas *A\$\_\{t\}\$*, *A\$\_\{p\}\$*, *A\$\_\{l\}\$*, *A\$\_\{r\}\$* and *A\$\_\{h\}\$* are shown schematically. \protect \label{fig:vertical-section-a-and-perspective-view-b-of}}
\end{figure}
...
\emph{ρ\(_{0}\)} = density of air at temperature \emph{T\(_{0}\)} (kg/m\(^{3}\))
...
\emph{φ} = tilt angle of window (vertical = 90\(^{o}\))

\emph{T\(_{p}\)} = effective mean temperature of the gap air (K)

\emph{T\(_{n}\)} = gap inlet temperature ( = zone air temperature for interior shading) (K)
...
where~ \emph{ρ}is the gap air density evaluated at \emph{T\(_{p}\)} (kg/m\(^{3}\)).
...
where \emph{~μ}~ is the viscosity of air at temperature \emph{T\(_{p}\)} (Pa-s).
...
Here, the inlet pressure drop factor,* Z\(_{n}\)\emph{, and the outlet pressure drop factor,}Z\(_{t}\)*, are given by
...
\begin{array}{l}{Z_{in}} = {\left( {\frac{{{A_{gap}}}}{{0.66{A_{eq,in}}}} - 1} \right)^2}\\\{Z_{out}} = {\left( {\frac{{{A_{gap}}}}{{0.60{A_{eq,out}}}} - 1} \right)^2}\end{array}

where

\emph{A\(_{n}\)} = equivalent inlet opening area (m\(^{2}\))

\emph{A\(_{t}\)} = equivalent outlet opening area (m\(^{2}\))

\emph{A\(_{p}\)} = cross-sectional area of the gap = \emph{sW}(m\(^{2}\))

If~ T\(_{p}\) \textgreater{} T\(_{n}\)
...
A\(_{t}\) = area of the bottom opening (m\(^{2}\))

A\(_{p}\) = area of the top opening (m\(^{2}\))
...
Figure~\ref{fig:examples-of-openings-for-an-interior-shading} shows examples of A\(_{t}\), A\(_{p}\), A\(_{l}\) and A\(_{r}\) for different shading device configurations.~ These areas range from zero to a maximum value equal to the associated shade/screen/blind-to-glass cross-sectional area; i.e., A\(_{t}\) and A\(_{p}\) ≤ sW,~ A\(_{l}\) and A\(_{r}\) ≤ sH.
...
The choice of the root of the quadratic equation is dictated by the requirement that \emph{v} = 0 if \emph{T\(_{n}\)} = \emph{T\(_{p}\)}.
...
At start of iteration, guess \emph{T\(_{p}\)} as ((\emph{T\(_{l}\)} + \emph{T\(_{h}\)})/2 + \emph{T\(_{n}\)})/2. Thereafter use value from previous iteration.

Get still-air conductance, \emph{h\(_{c}\)}, by calling WindowGasConductance and NusseltNumber.

Get \emph{v} from Equation

Get \emph{h\(_{v}\)} from Equation

Get \emph{T\(_{e}\)}from Equation

Get \emph{T\(_{t}\)} from Equation

Get new value of \emph{T\(_{p}\)}from Equation

The values of \emph{h\(_{v}\)} and \emph{T\(_{p}\)} so determined are then used in the window heat balance equations to find new values of the face temperatures of the glass and shading layers. These temperatures are used in turn to get new values of \emph{h\(_{v}\)} and \emph{T\(_{p}\)} until the whole iterative process converges.
...
\$\{T\_\{gap,1\}\} = \$ ~effective mean air temperature in gap 1 (K)

\$\{h\_\{cv,1\}\} = \$ ~convective heat transfer coefficient from glass or shading layer to gas in gap 1 (W/m\(^{2}\)K)
...
\$\{T\_\{gap,2\}\} = \$ ~effective mean air temperature in gap 2 (K)

\$\{h\_\{cv,2\}\} = \$ ~convective heat transfer coefficient from glass or shading layer to gas in gap 2 (W/m\(^{2}\)K)
...
\$\{h\_\{c,1\}\},\{h\_\{c,2\}\} = \$ ~~~~ surface-to-surface heat transfer coefficients for gap \#1 and \#2,~ respectively,~ when these gaps are non-vented (closed).
...
\begin{array}{l}\Delta {p_{B,i}} = \frac{{{\rho_{gap,}}_i}}{2}{v^2}\\\Delta {p_{HP,i}} = 12{\mu_{gap,}}_i\frac{H}{{{s^2}}}\\\Delta {p_{Z,i}} = \frac{{{\rho_{gap,}}_i{v^2}}}{2}({Z_{in,i}} + {Z_{out,i}})\end{array}
...
\emph{h\(_{v}\)} = convective heat transfer coefficient from glass to gap air (W/m2K).
...
\emph{A\(_{p}\)} = gap cross-sectional area in direction of flow (m2)
...
\emph{T\(_{n}\)}\(_{ }\) = ~ gap air inlet temperature (\emph{T\(_{i}\)} if the airflow source is indoor air, \emph{T\(_{o}\)} if the airflow source is outside air) (K)
...
\emph{C\(_{i}\)}\(_{ }\) = heat capacity of the indoor air (J/kg-K)

\emph{C\(_{t}\)} = heat capacity of the gap outlet air (J/kg-K)
...
where \emph{A\(_{p}\)} = \emph{sW} is the cross-sectional area of the gap on either side of the shading device. It is assumed that the shading device is centered between the two panes of glass so that the airflow, \emph{F}, is divided equally between the two gaps.
...
\emph{C\(_{t}\)}\(_{ }\) = heat capacity of the outlet air evaluated at \emph{T\(_{t}\)} (J/kg-K)
...
a\(_{1}\), a\(_{2}\) = Accommodation coefficients of the gas molecules with the two surfaces.~ These values depend on the temperature, surface conditions, etc.~ For the present configuration and conditions, it is expected that \emph{a} is approximately 0.5.~ If conservative value is needed than value of 1.0 could be used.~ With a\(_{1}\), a\(_{2}\) = 0.5, a = 0.333

g~~~~ = Specific heat ratio, g\(_{r}\) = 1.40.
...
e\(_{1}\)~~~ = emissivity of the first facing glass surface, {[} - {]}

e\(_{2}\)~~~ = emissivity of the second facing glass surface, {[} - {]}
...
Clear Glass (e\(_{1}\) = e\(_{2}\) = 0.84): C\(_{D}\) = 3.4 W/m\(^{2}\)K

Hard Coat Low-e (e\(_{1}\) or e\(_{2}\) = 0.15): C\(_{D}\) = 0.68 W/m\(^{2}\)K

Soft Coat Low-e (e\(_{1}\) or e\(_{2}\) = 0.04): C\(_{D}\) = 0.19 W/m\(^{2}\)K

\textbf{\emph{Note:}} \emph{C\(_{D}\) of 0.09 is theoretically possible using best low-e technology today (i.e.,} \emph{e\(_{1}\) or} \emph{e\(_{2}\) = 0.02).}
...
\emph{t}\(_{s}\) = glass thickness; {[}m{]}

\emph{k}\(_{s}\) = glass conductivity; \emph{k}\(_{s}\) = 1 W/(m·K)

\emph{R}\(_{s}\) = 0.003 m\(^{2}\)K/W (for 3 mm glass pane)
...
For the square array of support pillars (Collins and Fischer-Cripps 1991) proposes the following formula for their conductance, \emph{C}\(_{a}\):
...
Δ\emph{P\(_{i}\)} = \emph{P\(_{)}\)} - \emph{P\(_{)}\)} (for i-th pane) {[}Force per unit Area; SI: Pa, IP: psi{]}

Δ\emph{P\(_{i}\)} = \emph{P\(_{)}\)} - \emph{P\(_{a}\)} (first pane) {[}Force per unit Area; SI: Pa, IP: psi{]}\emph{~~~~~~~~~~~~~~~~~~}

Δ\emph{P\(_{i}\)} = \emph{P\(_{a}\) - P\(_{)}\)} (last pane) {[}Force per unit Area; SI: Pa, IP: psi{]}
...
\emph{P\(_{i}\)} = Initial pressure. Applies to all gaps as a single value (input data - measured or otherwise) {[}Force per unit Area; SI: Pa, IP: psi{]}

\emph{T\(_{i}\)} = Initial temperature. Applies to all gaps as a single value (input data - measured or otherwise) {[}Degree Temperature; SI: K, IP: R{]}

\emph{V\(_{)}\)} = Initial volume for i-th gap. {[}Length*Length*Length; SI: m\(^{3}\), IP: in\(^{3}\){]}
...
\emph{T\(_{)}\)} = temperature of the gap between two glass panes (calculated using center of glazing thermal calculation algorithm, as described in ISO 15099 (ISO 2003). This value is first calculated using non-deflected state and is recalculated after the resulting deflection is calculated. This process is repeated until temperature at next iteration does not differ by more than 0.1 ºC

\emph{V\(_{)}\)} = volume of the IGU gap in deflected state {[}Lentgh*Length*Length; SI: m\(^{3}\), IP: in\(^{3}\){]}
...
\emph{L\(_{)}\) = Mean gap ``i'' width after incorporating glazing deflection. This mean gap width is used to recalculate thermal performance of deflected IGU.}

\(\overline {{L_{D,i}}}\) \textbf{\emph{~}}* = mean glazing deflection for each pane ``i''.*
...
At the end of calculations, program will calculate and return maximum deflection value for each pane (i.e., center of glazing deflection). If we label maximum deflection of each pane as \emph{L\(_{x}\)}, we can calculate this value by substituting \emph{x} = \emph{W}/2 and \emph{y} = \emph{H}/2 in equation to determine deflection at the center point. Therefore,
...
The procedure shown above generally applies to the 3 or more layer glazing system, with the exception that neighboring pressures are no longer \emph{P\(_{a}\)}, but rather could be \emph{P\(_{a}\)} on one side and \emph{P\(_{p}\)} on the other, or have \emph{P\(_{p}\)} on both sides, as shown in Figure~\ref{fig:deflection-direction-convention} for gap ``\emph{i}''. Center of glazing thermal calculation will determine new temperature distribution, after deflection is calculated for each glazing and will be used to determine new \emph{P\(_{p}\)}, as per the procedure above.
...
Measured value is typically gap width at the point of maximum deflection, which we can label \emph{L\(_{)}\)}. For i-th measured gap the width is equal to:
...
If we label ratio of mean deflection and maximum deflection as R\(_{)}\), then:
...
\($ Important thing to note is that ratios (\)\{R\_\{\left( i \right)\}\}\$ ) for all gaps in glazing system are equal.
...
The beam and diffuse characterization of solar radiation demands an expanded set of solar optical properties (Wright and Kotey 2006). The quantities of interest for single layer are shown inFigure~\ref{fig:twelve-solar-properties-assigned-at-each}.
...
The apparent wire material emissivity can be taken as \emph{e}\(^{w}\) = 0.93 for common insect screens - screens with dark, rough, non-metallic wire surfaces. The corresponding infrared wire material transmittance is \emph{t}\(^{w}\) = 0.98. A lower infrared emissivity can be used for screens constructed with shiny metallic wire. For example, to model stainless-steel wire mesh use \emph{e}\(^{w}\) = 0.32 and \emph{t}\(^{w}\) = 0.81.
...
\emph{e\(^{f}\)}~~ = ~~~~~~~~~ composite indoor (room-side) longwave emissivity, (-)

\emph{e\(_{j}\)~~~ = ~~~~~~~~~} effective emissivity of layer j, (-)

\emph{t\(_{k}\)~~~ = ~~~~~~~~~} effective infrared transmittance of layer k (\emph{t\(_{1}\)} = 1)
...
\emph{T\(^{f}\)}~ = ~~~~~~~~~ inside surface temperature of the composite layer, °C (°F)

\emph{Q\(_{w}\)}~ = ~~~~~~~~~ infrared radiant gain from zone, W/m2 (Btu/h-ft2)

\emph{s}~~~ = ~~~~~~~~~ Stefan-Boltzmann constant, W/m2-K4 (Btu/h-ft2-R4)
...
\emph{Q\(_{v}\)~~~~~~~~~~~ = ~~~~~~~~~} total convective heat flux to zone from equivalent layer window thermal model, W/m\(^{2}\) (Btu/h-ft\(^{2}\)); includes open-channel gains and impact of inside surface convective coefficient
