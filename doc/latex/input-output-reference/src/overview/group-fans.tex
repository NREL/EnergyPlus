...
\caption{Fan Coefficient Values \protect \label{table:fan-coefficient-values}}\\
\toprule 
Type of Fan & Fan Coeff. 1 & Fan Coeff. 2 & Fan Coeff. 3 & Fan Coeff. 4 & Fan Coeff. 5 \tabularnewline \midrule
\endhead
...
HVAC,Average,Fan Runtime Fraction \protect\hyperlink{section-1}{}
...
\paragraph{\texorpdfstring{Fan Runtime Fraction \protect\hyperlink{section-1}{}}{Fan Runtime Fraction }}\label{fan-runtime-fraction}
...
PLF = C\(_{1}\) + C\(_{2}\)\(^{.}\) FF + C\(_{3}\)\(^{.}\) FF\(^{+}\) C\(_{4}\)\(^{.}\) FF\(^{3}\) + C\(_{5}\)\(^{.}\) FF\(^{4}\)
...
The required numeric Euler number (\emph{Eu\(_{x}\)}), which is also called the throttling or pressure coefficient, and is the ratio of pressure forces to inertial forces. The Euler number is determined from analyses of manufacturer s data using:

\begin{equation}
Eu = \frac{{\left( {\Delta {P_{fan}} * D_{fan}^4} \right)}}{{\left( {\rho  * Q_{fan}^2} \right)}}
\end{equation}

where \emph{P\(_{n}\)} is fan static pressure rise (Pa; see \emph{Fan Pressure Rise Curve Name} field), \emph{D\(_{n}\)} is wheel diameter (m), \emph{ρ} is the manufacturer s reference air density (kg/m\(^{3}\)), and \emph{Q\(_{n}\)} is airflow through the fan (m\(^{3}\)/sec). \emph{Eu\(_{x}\)} is calculated using any pair of pressure rise and airflow values that correspond with maximum fan static efficiency for the specified fan. Must be greater than zero.

\paragraph{Field: Maximum Dimensionless Fan Airflow}\label{field-maximum-dimensionless-fan-airflow}

The required numeric maximum dimensionless airflow (\(\psi_{max}\) ) through the fan, which corresponds to the maximum ratio between the airflow through the fan (\emph{Q\(_{n}\)}, m\(^{3}\)/sec) and the fan shaft rotational speed (\(\omega\) \(_{n}\)\emph{, rpm) for the specified fan wheel diameter (}D\(_{n}\)\emph{, m).}φ\(_{x}\)* is determined from manufacturers data using:

\[{\varphi_{\max }} = \frac{{30}}{{\pi D_{fan}^3}}*\max \left( {\frac{{{Q_{fan}}}}{{{\omega_{fan}}}}} \right)\] \(_{x}\)* occurs at minimum \emph{Eu}, which corresponds to maximum speed (high flow) with zero pressure rise. The factor (\(30/\pi\) ) converts revolutions per minute (rpm) to rad/s. Must be greater than zero.

\paragraph{Field: Motor Fan Pulley Ratio}\label{field-motor-fan-pulley-ratio}

The numeric dimensionless ratio of the motor pulley diameter to the fan pulley diameter (\emph{D\(_{y}\) / D\(_{y}\)}). If specified, must be greater than zero. This ratio can be adjusted to account for belt slip if the fractional slip is known (multiply the drive ratio with no slip by 1+s, where s is the belt fractional slip). Default is 1.0 if field is blank (leave blank if no belt; i.e., direct drive). Can be autosized (assumes no slip).
...
The numeric dimensionless factor (\emph{F\(_{t}\)}) used to multiply the specified or autosized fan shaft maximum output torque (\(_{x}\)*). If specified, minimum value is 1.0. Default is 1.0 if field is blank.

\paragraph{Field: Belt Fractional Torque Transition}\label{field-belt-fractional-torque-transition}

The numeric transition point (\emph{x\(_{s}\)}) between performance curves for Regions 1 and 2 for the drive belt normalized part-load efficiency. Must be between 0.0 and 1.0. Default is 0.167 (corresponds to generic V-belt) if field is blank.

\paragraph{Field: Motor Maximum Speed}\label{field-motor-maximum-speed}

The required numeric maximum rotational speed of the fan motor shaft (\emph{ω\(_{x}\)}) in revolutions per minute (rpm). Typical values for motors supplied by 60 Hz power are near 900, 1200, 1800, and 3600 rpm. Must be greater than zero.

\paragraph{Field: Maximum Motor Output Power}\label{field-maximum-motor-output-power}

The required numeric maximum output power (input power to the fan drive belt) by the motor (\emph{H\(_{x}\)}, W). If specified, must be greater than zero. Can be autosized. In the case of direct drive, \emph{H\(_{x}\)} corresponds to the maximum fan shaft power (\emph{H\(_{x}\)}).
...
The numeric dimensionless sizing factor (\emph{F\(_{r}\)}) used to multiply the specified or autosized fan motor output power (\emph{H\(_{x}\)}). If specified, minimum value is 1.0. Default is 1.0.
...
The numeric dimensionless factor (\emph{F\(_{D}\)}) used to multiply the specified or autosized motor maximum input power (\emph{H\(_{x}\)}). If specified, minimum value is 1.0. Default is 1.0 if field is blank.

\paragraph{Field: Fan Pressure Rise Curve Name}\label{field-fan-pressure-rise-curve-name}

The required alpha name of the fan total pressure rise performance curve (ref: Curve:FanPressureRise in Performance Curves) that parameterizes the variation of fan total pressure rise (\emph{P\(_{t}\)}, Pa) as a function of volumetric flow through the fan (\emph{Q\(_{n}\)}, m\(^{3}\)/s) and duct static pressure set point (\emph{P\(_{m}\)}, Pa). The fan outlet velocity pressure is subtracted from the output of this curve to determine fan static pressure rise, which is then used to calculate a dimensionless Euler number at each time step. The Euler number is in turn used to determine fan efficiency, speed, and torque (the Euler number is defined in the \emph{Euler Number at Maximum Fan Static Efficiency} field). This curve should be valid for the range of volumetric flows, distribution system leakage, duct static pressures, and static pressures surrounding the ducts anticipated for the simulation period.

\paragraph{Field: Duct Static Pressure Reset Curve Name}\label{field-duct-static-pressure-reset-curve-name}

The required alpha name of the performance curve that parameterizes the variation of the duct static pressure set point (\emph{P\(_{m}\)}, Pa) as a function of volumetric flow through the fan (\emph{Q\(_{n}\)}, m\(^{3}\)/s), which is used so that the resistance associated with VAV box damper operation is reduced.
...
For an ad hoc linear duct static pressure reset scheme, the relation (ref: Curve:Linear in Performance Curves) between duct static pressure (\emph{P\(_{m}\)}, Pa) and flow through the fan (\emph{Q\(_{n}\)}, m\(^{3}\)/s) for \emph{Q\(_{n}\)} ≤ \emph{Q\(_{n}\)} ≤ \emph{Q\(_{x}\)} is:
...
The minimum and maximum fan airflows (\emph{Q\(_{n}\)} and \emph{Q\(_{x}\)}) correspond respectively to the minimum and maximum duct static pressure set points (\emph{P\(_{n}\)} and \emph{P\(_{x}\)}).
...
The required alpha name of the exponential-modified skew normal performance curve (ref: Curve:ExponentialSkewNormal in Performance Curves) that parameterizes the normalized fan static efficiency (\emph{-\(_{n}\)} (\emph{x\(_{n}\)}) \emph{/ -\(_{x}\)}) at each time step for the stall region of the fan performance map as a function of \emph{x\(_{n}\)} (see \emph{Normalized Fan Static Efficiency Curve Name Non-Stall Region} field). In this region, \emph{x\(_{n}\)}\textgreater{}0.
...
The required alpha name of the sigmoid performance curve (ref: Curve:Sigmoid in Performance Curves) that parameterizes the normalized dimensionless airflow through the fan (\emph{φ} (\emph{x\(_{n}\)}) \emph{/ φ\(_{x}\)}) at each time step for the normal operating (non-stall) region of the fan performance map as a function of \emph{x\(_{n}\)}, which is defined as log-base-10 of Eu at the fan flow and pressure rise operating point divided by Eu at maximum fan static efficiency {[}\emph{log\(_{0}\)}(\emph{Eu / Eu\(_{x}\)}){]}. In this region, \emph{x\(_{n}\)} \textless{} = 0.

The output of this curve is used to calculate the dimensionless airflow \emph{φ} (\emph{x\(_{n}\)}) at each time step by modifying \emph{φ\(_{x}\)} (see \emph{Maximum Dimensionless Fan Airflow} field). This curve should have a maximum of 1.0 and should be valid for the range of volumetric flows and fan pressure rises anticipated for the simulation period.
...
The required alpha name of the sigmoid performance curve (ref: Curve:Sigmoid in Performance Curves) that parameterizes the normalized dimensionless airflow through the fan (\emph{φ} (\emph{x\(_{n}\)}) \emph{/ φ\(_{x}\)}) at each time step for the stall region of the fan performance map as a function of \emph{x\(_{n}\)} (see \emph{Normalized Dimensionless Airflow Curve Name Non-Stall Region} field). In this region, \emph{x\(_{n}\)} \textgreater{}0.

The output of this curve is used to calculate the dimensionless airflow \emph{φ} (\emph{x\(_{n}\)}) at each time step by modifying \emph{φ\(_{x}\)} (see \emph{Maximum Dimensionless Fan Airflow} field). This curve should have a maximum of 1.0 and should be valid for the range of volumetric flows and fan pressure rises anticipated for the simulation period.

\paragraph{Field: Maximum Belt Efficiency Curve Name}\label{field-maximum-belt-efficiency-curve-name}

The alpha name of the quartic polynomial performance curve (ref: Curve:Quartic in Performance Curves) that determines the maximum fan drive belt efficiency in logarithmic space (\emph{-\(_{n}\)}) as a function of \emph{x\(_{x}\)}. The curve is:

\emph{\({\eta_{belt,\max ,\ln }} = {C_1} + {C_2} \cdot {x_{belt,\max }} + {C_3} \cdot x_{_{belt,\max }}^2 + {C_4} \cdot x_{_{belt,\max }}^3 + {C_5} \cdot x_{_{belt,\max }}^4\)}

where \emph{x\(_{x}\)} * = ln\emph{(}F\(_{t}\)*H\(_{x}\)\emph{) with}H\(_{x}\)* expressed in terms of hp.

\emph{Note that -\(_{x}\) = exp}(\emph{-\(_{n}\)}).

The output of this curve must be greater than zero and less than or equal to 1.0. If \emph{-\(_{x}\)} is known, it is represented by coefficient \emph{C\(_{1}\)} (* = ln\emph{(}-\(_{x}\)\emph{)). In this case, coefficients}C\(_{2}\)* through \emph{C\(_{5}\)} are set to zero. If this field is left blank (e.g., there is no belt), the model assumes that the output of the modifier curve is 1.0 for the entire simulation (maximum belt efficiency = 1.0).
...
The alpha name of the single rectangular hyperbola type 2 performance curve (ref: Curve:RectangularHyperbola2 in Performance Curves) that determines the normalized (par-load) fan drive belt efficiency (\emph{-\(_{t}\)} (\emph{x}\(_{t}\)) / \emph{-\(_{x}\)}) as a function of \emph{x\(_{t}\)}. Normalized belt efficiency is represented by a segmented curve with three different regions. The curve for Region 1 (0 \textless{} = \emph{x\(_{t}\)} \textless{} \emph{x\(_{s}\)}) is:

\emph{-\(_{t}\)}(\emph{x}\(_{t}\))/* -\(_{x}\) = \emph{(}C\(_{1}\) -* x\(_{t}\)\emph{)}/* (\emph{C\(_{2}\)+x\(_{t}\)}) \emph{+ C\(_{3}\) -* x\(_{t}\)}

where \emph{x\(_{t}\)} * = * \(_{t}\)* / \(_{x}\);* \(_{t}\)* is the belt output torque that corresponds to the calculated power input to the fan shaft (\emph{H\(_{n}\)}, W) by the drive belt and the calculated fan shaft speed (\emph{ω\(_{n}\)}, rpm).

The output of this curve is used to calculate the belt efficiency \emph{-\(_{t}\)} (\emph{x\(_{t}\)}) in Region 1 at each time step by modifying \emph{-\(_{x}\)} (see \emph{Maximum Belt Efficiency} \emph{Curve Name} field). The output of this curve must be greater than zero and less than or equal to 1.0 and should be valid for the range of volumetric flows and fan pressure rises anticipated for the simulation period.

If this field is left blank, the model assumes that the output of the modifier curve is 1.0 for the entire simulation (i.e., constant belt efficiency at \emph{-\(_{x}\)} in Region 1).
...
The alpha name of the exponential decay performance curve (ref: Curve:ExponentialDecay in Performance Curves) that determines the normalized (part-load) fan drive belt efficiency (\emph{-\(_{t}\)} (\emph{x}\(_{t}\)) / \emph{-\(_{x}\)}) as a function of \emph{x\(_{t}\)}. Normalized belt efficiency is represented by a segmented curve with three different regions. The curve for Region 2 (\emph{x\(_{s}\)} \textless{} = \emph{x\(_{t}\)} \textless{} = 1) is:

\emph{-\(_{t}\)} (\emph{x}\(_{t}\)) / \emph{-\(_{x}\) = C\(_{1}\) + C\(_{2}\) * exp}\(^{*}\)\emph{3\(^{x}\)belt}\(^{)}\)

where \emph{x\(_{t}\)} * = * \(_{t}\)* / \(_{x}\);* \(_{t}\)* is the belt output torque that corresponds to the calculated power input to the fan shaft (\emph{H\(_{n}\)}, W) by the drive belt and the calculated fan shaft speed (\emph{ω\(_{n}\)}, rpm).

The output of this curve is used to calculate the belt efficiency \emph{-\(_{t}\)} (\emph{x\(_{t}\)}) in Region 2 at each time step by modifying \emph{-\(_{x}\)} (see \emph{Maximum Belt Efficiency} \emph{Curve Name} field). The output of this curve must be greater than zero and less than or equal to 1.0 and should be valid for the range of volumetric flows and fan pressure rises anticipated for the simulation period.

If this field is left blank, the model assumes that the output of the modifier curve is 1.0 for the entire simulation (i.e., constant belt efficiency at \emph{-\(_{x}\)} in Region 2).
...
The alpha name of the single rectangular hyperbola type 2 performance curve (ref: Curve:RectangularHyperbola2 in Performance Curves) that determines the normalized (part-load) fan drive belt efficiency (\emph{-\(_{t}\)} (\emph{x}\(_{t}\)) / \emph{-\(_{x}\)}) as a function of \emph{x\(_{t}\)}. Normalized belt efficiency is represented by a segmented curve with three different regions. The curve for Region 3 (\emph{x\(_{t}\)} \textgreater{} 1) is:

\emph{-\(_{t}\)} (\emph{x}\(_{t}\)) / \emph{-\(_{x}\) =}(\emph{C\(_{1}\) -* x\(_{t}\)}) \emph{/} (\emph{C\(_{2}\)+x\(_{t}\)}) \emph{+ C\(_{3}\) -* x\(_{t}\)}

where \emph{x\(_{t}\)} * = * \(_{t}\)* / \(_{x}\);* \(_{t}\)* is the belt output torque that corresponds to the calculated power input to the fan shaft (\emph{H\(_{n}\)}, W) by the drive belt and the calculated fan shaft speed (\emph{ω\(_{n}\)}, rpm).

The output of this curve is used to calculate the belt efficiency \emph{-\(_{t}\)} (\emph{x\(_{t}\)}) in Region 3 at each time step by modifying \emph{-\(_{x}\)} (see \emph{Maximum Belt Efficiency} \emph{Curve Name} field). The output of this curve must be greater than zero and less than or equal to 1.0 and should be valid for the range of volumetric flows and fan pressure rises anticipated for the simulation period.

If this field is left blank, the model assumes that the output of the modifier curve is 1.0 for the entire simulation (i.e., constant belt efficiency at \emph{-\(_{x}\)} in Region 3).

\paragraph{Field: Maximum Motor Efficiency Curve Name}\label{field-maximum-motor-efficiency-curve-name}

The alpha name of the single rectangular hyperbola type 1 performance curve (ref: Curve: RectangularHyperbola1 in Performance Curves) that determines the maximum fan motor efficiency (\emph{-\(_{x}\)}) as a function of \emph{x\(_{x}\)}. The curve is:

\emph{-\(_{x}\) =}(\emph{C\(_{1}\) * x\(_{x}\)}) \emph{/} (\emph{C\(_{2}\) + x\(_{x}\)}) \emph{+ C\(_{3}\)}

where \emph{x\(_{x}\) = ln}(\emph{F\(_{r}\) * H\(_{x}\)}) with \emph{H\(_{x}\)} expressed in terms of hp.\emph{H\(_{x}\)} is the maximum output power from the motor to the belt, which corresponds to the calculated maximum power input to the fan shaft (\emph{H\(_{x}\)}, W).

The output of this curve must be greater than zero and less than or equal to 1.0. If \emph{-\(_{x}\)} is known, it is represented by coefficient \emph{C\(_{3}\)}. In this case, coefficients \emph{C\(_{1}\)} and \emph{C\(_{2}\)} are set to zero.
...
The alpha name of the single rectangular hyperbola type 2 performance curve (ref: Curve:RectangularHyperbola2 in Performance Curves) that determines the normalized (part-load) fan motor efficiency (\emph{-\(_{r}\)} (\emph{x}\(_{r}\)) / \emph{-\(_{x}\)}) as a function of the motor load fraction \emph{x\(_{r}\)}. The curve is:

\emph{-\(_{r}\)} (\emph{x\(_{r}\)}) \emph{/} \emph{-\(_{x}\) =}(*C\(_{1}\) * x**\(_{r}\)\emph{)}/* (*C\(_{2}\) + x**\(_{r}\)\emph{)}+* (*C\(_{3}\) -* x**\(_{r}\)*)

where \emph{x\(_{r}\) = H\(_{t}\)} / \emph{H\(_{x}\). H\(_{t}\)} is the calculated output power from the motor to the belt (W), which corresponds to the calculated power input to the fan shaft (\emph{H\(_{n}\)}, W).

The output of this curve is used to calculate the motor efficiency (\emph{-\(_{r}\)} (\emph{x\(_{r}\)})) at each time step by modifying \emph{-\(_{x}\)} (see \emph{Maximum Motor Efficiency Curve Name} field). The output of this curve must be greater than zero and less than or equal to 1.0 and should be valid for the range of volumetric flows and fan pressure rises anticipated for the simulation period.

If this field is left blank, the model assumes that the output of the modifier curve is 1.0 for the entire simulation (i.e., constant motor efficiency at \emph{-\(_{x}\)}).

\paragraph{Field: VFD Efficiency Curve Name}\label{field-vfd-efficiency-curve-name}

The alpha name of the single rectangular hyperbola type 2 performance curve (e.g., Curve:RectangularHyperbola2 in Performance Curves) that determines the VFD efficiency (\emph{-\(_{D}\)} (\emph{x}\(_{D}\))) as a function of the fractional input power of the motor or fractional motor speed (\emph{x\(_{D}\)}). An example of the curve is:

\emph{-\(_{D}\)} (\emph{x\(_{D}\)}) * = \emph{(*C\(_{1}\) * x**\(_{D}\)})\emph{/* (}C\(_{2}\)+x\textbf{\(_{D}\)\emph{)}+ C\(_{3}\) * x}\(_{D}\)*

where \emph{x\(_{D}\) = H\(_{r}\)} / \emph{H\(_{x}\)} or \emph{ω\(_{r}\) / ω\(_{x}\)}

The output of this curve is used to calculate the VFD efficiency \emph{-\(_{D}\)} (\emph{x\(_{D}\)}) at each time step. The output of this curve must be greater than zero and less than or equal to 1.0 and should be valid for the range of volumetric flows and fan pressure rises anticipated for the simulation period.
