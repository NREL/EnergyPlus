...
This field represents the time zone of the facility (relative to Greenwich Mean Time or the 0\(^{h}\) meridian). Time zones west of GMT (e.g.~North America) are represented as negative; east of GMT as positive. Non-whole hours can be represented in decimal (e.g.~6:30 is 6.5).
...
% table 2
\begin{longtable}[c]{p{1.5in}p{1.62in}p{2.87in}}
...
WetBulbProfileDefaultMultipliers & Wetbulb or DewPoint at Maximum Dry-Bulb & N/A (unused) \tabularnewline
WetBulbProfileMultiplierSchedule & Wetbulb or DewPoint at Maximum Dry-Bulb & Fractions of wet-bulb daily range (0 – 1) \tabularnewline
WetBulbProfileDifferenceSchedule & Wetbulb or DewPoint at Maximum Dry-Bulb & Difference between maximum and hour/timestep wet-bulb temperature, C \tabularnewline
RelativeHumiditySchedule & N/A & Relative humidity (\%, 0 – 100) \tabularnewline
\bottomrule
\end{longtable}
...
\item
  Zone,Average,Sizing Period Site Humidity Condition Schedule Value \protect\hyperlink{section-1}{}
\item
  Zone,Average,Sizing Period Site Drybulb Temperature Range Modifier Schedule Value \protect\hyperlink{section-1}{}
\item
...
\paragraph{\texorpdfstring{Sizing Period Site Humidity Condition Schedule Value \protect\hyperlink{section-1}{}}{Sizing Period Site Humidity Condition Schedule Value }}\label{sizing-period-site-humidity-condition-schedule-value-1}
...
\paragraph{\texorpdfstring{Sizing Period Site Drybulb Temperature Range Modifier Schedule Value \protect\hyperlink{section-1}{}}{Sizing Period Site Drybulb Temperature Range Modifier Schedule Value }}\label{sizing-period-site-drybulb-temperature-range-modifier-schedule-value}
...
\subsection{SizingPeriod:WeatherFileConditionType}\label{sizingperiodweatherfileconditiontype}
...
Note: EnergyPlus processed weather files available on the EnergyPlus web site: http://www.energyplus.gov/cfm/weather\_data.cfm have neither special days specified nor daylight saving period. However, DDY (Design Day) files produced from the ASHRAE Design Conditions that accompany the EPW files may include a DaylightSavingPeriod object for certain locations.
...
Note: EnergyPlus processed weather files available on the EnergyPlus web site: http://www.energyplus.gov/cfm/weather\_data.cfm have neither special days specified nor daylight saving period.

\textbf{Note:~ For EnergyPlus Output:Variable and Output:Meter reporting, the time stamps are always in standard time. When daylight saving time is active, scheduled loads and controls will shift one hour relative to standard time.}
...
Each numeric field is the monthly ground temperature (degrees Celsius) used for the indicated month (January = 1\(^{t}\) field, February = 2\(^{d}\) field, etc.)
...
Each numeric field is the monthly surface ground temperature (degrees Celsius) used for the indicated month (January = 1\(^{t}\) field, February = 2\(^{d}\) field, etc.)
...
Each numeric field is the monthly deep ground temperature (degrees Celsius) used for the indicated month (January = 1\(^{t}\) field, February = 2\(^{d}\) field, etc.)
...
Each numeric field is the monthly average reflectivity of the ground used for the indicated month (January = 1\(^{t}\) field, February = 2\(^{d}\) field, etc.)
...
\begin{equation}
{\rm{GroundReflectanc}}{{\rm{e}}_{{\rm{used}}}} = {\rm{GroundReflectance}} \bullet Modifie{r_{Snow}}
\end{equation}
...
\begin{equation}
{\rm{DaylightingGroundReflectanc}}{{\rm{e}}_{{\rm{used}}}} = {\rm{GroundReflectance}} \bullet Modifie{r_{Snow}}
\end{equation}
...
\begin{equation}
Amount\,(m/hour) = \,\left\{ {\begin{array}{*{20}{c}}{Light = 0.0125}\\\{Moderate = 0.052}\\\{Heavy = 0.1}\end{array}} \right.
\end{equation}
...
\item
  Zone,Average,Site Rain Status \protect\hyperlink{section-1}{}
\item
  Zone,Average,Site Snow on Ground Status \protect\hyperlink{section-1}{}
\item
...
\item
  Zone,Average,Site Daylighting Model Sky Clearness \protect\hyperlink{section-1}{}
\item
  Zone,Average,Sky Brightness for Daylighting Calculation \protect\hyperlink{section-1}{}
\item
  Zone,Average,Site Daylight Saving Time Status \protect\hyperlink{section-1}{}
\item
  Zone,Average,Site Day Type Index \protect\hyperlink{section-1}{}
\item
...
\begin{equation}
Groundreflectedsolar = \left( {Beamsolar\cdot COS(SolarAltitudeAngle) + Diffusesolar} \right)\cdot {\rm{Groundreflectanc}}{{\rm{e}}_{{\rm{month}}}}
\end{equation}
...
\paragraph{\texorpdfstring{Site Rain Status \protect\hyperlink{section-1}{}}{Site Rain Status }}\label{site-rain-status}
...
\paragraph{\texorpdfstring{Site Snow on Ground Status \protect\hyperlink{section-1}{}}{Site Snow on Ground Status }}\label{site-snow-on-ground-status}
...
\paragraph{\texorpdfstring{Site Daylight Saving Time Status \protect\hyperlink{section-1}{}}{Site Daylight Saving Time Status }}\label{site-daylight-saving-time-status}
...
\paragraph{\texorpdfstring{Site Day Type Index \protect\hyperlink{section-1}{}}{Site Day Type Index }}\label{site-day-type-index}
