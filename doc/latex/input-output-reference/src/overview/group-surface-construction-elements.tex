...
\begin{itemize}
\item
  Zone,Sum,CondFD Inner Solver Loop Iteration Count \protect\hyperlink{section-1}{}
\item
  Zone,Average,CondFD Surface Temperature Node \textless{}1\textgreater{} {[}C{]}
\end{itemize}

\paragraph{\texorpdfstring{CondFD Inner Solver Loop Iteration Count \protect\hyperlink{section-1}{}}{CondFD Inner Solver Loop Iteration Count }}\label{condfd-inner-solver-loop-iteration-count}
...
The relative humidity of the x\(^{h}\) coordinate. The relative humidity is entered as fraction, not in percent.

\paragraph{Field: Moisture Content x}\label{field-moisture-content-x}

The Moisture Content of the x\(^{h}\) coordinate. The units are {[}kg/m3{]}
...
The moisture content of the x\(^{h}\) point. The units are {[}kg/m3{]}.

\paragraph{Field: Liquid Transport Coefficient x}\label{field-liquid-transport-coefficient-x}

The Liquid Transport Coefficient of the x\(^{h}\) point. The units are {[}m2/s{]}.
...
The moisture content of the x\(^{h}\) pair. The relative humidity is entered as fraction, not in percent.

\paragraph{Field: Vapor Diffusion Resistance Factor \#x}\label{field-vapor-diffusion-resistance-factor-x}

The Liquid Transport Coefficient of the x\(^{h}\) pair.
...
The Thermal Conductivity of the x\(^{h}\) coordinate. The units are {[}W/m-K{]}
...
So for example to output the temperature of the 10\(^{h}\) cell in a surface, eg ``East Wall'' would require the following output variable.
...
This is a factor that corrects for the presence of dirt on the glass. The program multiplies the fields ``Solar Transmittance at Normal Incidence'' and ``Visible Transmittance at Normal Incidence'' by this factor if the material is used as the outer glass layer of an exterior window or glass door.\protect\hyperlink{ux5fftn1}{{[}1{]}} If the material is used as an inner glass layer (in double glazing, for example), the dirt correction factor is not applied because inner glass layers are assumed to be clean. Using a material with dirt correction factor \textless{} 1.0 in the construction for an interior window will result in an error message.
...
\toprule 
Type of Location & Angle of Glazing \tabularnewline \midrule
\endhead
Vertical & 45 & Horizontal \tabularnewline
Non-industrial & 0.9 & 0.8 & 0.7 \tabularnewline
Industrial & 0.7 & 0.6 & 0.5 \tabularnewline
Very Dirty & 0.6 & 0.5 & 0.4 \tabularnewline
\bottomrule
\end{longtable}
...
This is a factor that corrects for the presence of dirt on the glass. It multiplies the solar and visible transmittance at normal Incidence (which the program calculates from the input values of thickness, solar index of refraction, solar extinction coefficient, etc.) if the material is used as the outer glass layer of an exterior window or glass door. If the material is used as an inner glass layer (in double glazing, for example), the dirt correction factor is not applied because inner glass layers are assumed to be clean. Using a material with dirt correction factor \textless{} 1.0 in the construction for an interior window will result in an error message.
...
Effective long-wave emissivity. Assumed same on both sides of shade. We can approximate this~ effective emissivity, \emph{ε\(_{f}\),}~ as follows. Let \emph{η} be the ``openness'' the shade, i.e.,~ the ratio of the area of openings in the shade to the overall shade area (see Field: Air-Flow Permeability, below). Let the emissivity of~ the shade material be ε. Then

\begin{equation}
{\varepsilon_{{\rm{eff}}}} \approx \varepsilon \left( {1 - \eta } \right)
\end{equation}

For most non-metallic materials \emph{ε}is about 0.9.

\paragraph{Field: Thermal Transmittance}\label{field-thermal-transmittance}

Effective long-wave transmittance. Assumed independent of incidence angle. We can approximate this~ effective long-wave transmittance, \emph{τ\(_{f}\),}~ as follows. Let \emph{η} be the ``openness'' the shade, i.e.,~ the ratio of the area of openings in the shade to the overall shade area. Let the long-wave transmittance of the shade material be \emph{τ}. Then
...
In this case the opening multiplier is calculated as the smallest distance between the shading device and the frame (d\(_{p}\)), divided by the gap width (S).~ There are three possible cases for the position of a shading device the on indoor/outdoor side (see Figure 15).

\includegraphics{media/image036.png}~~~~ \includegraphics{media/image038.png}

Figure 15. Three cases for the D\(_{p}\) calculation for an indoor/outdoor shade: Case a) A shading device between the frame; Case b) A shading device outside the frame, covering the frame; Case c) a shading device outside the frame, not covering the frame.

In the case where the distance between the frame and the shading device is bigger than the gap width, the d\(_{p}\) multiplier is equal to one. Therefore, the calculation of the D\(_{p}\) opening multiplier is:

A\(_{p}\) = min(d\(_{p}\)/S, 1)

\textbf{Shading device between glass layers}

In this case the opening multiplier is calculated as the smallest distance between the shading device and the frame or spacer (d\(_{p}\)), divided by the smaller gap width (the minimum of (S\(_{1}\) andS\(_{2}\))).

\includegraphics{media/image039.png}~~~~~~~~~ \includegraphics{media/image040.png}

Figure 16. Calculation of Dtop for a shading device between glass layers

The D\(_{p}\) opening multiplier for a between glass shade is calculated as:

A\(_{p}\) = min(d\(_{p}\)/S\(_{n}\), 1)

Where

S\(_{n}\) = min(S\(_{1}\), S\(_{2}\))

\paragraph{Field: Bottom Opening Multiplier}\label{field-bottom-opening-multiplier-1}

The bottom opening multiplier (d\(_{t}\)) is calculated in the same way as the top opening multiplier, with the rules applied to the bottom of the shading device.

\paragraph{Field: Left Side Opening Multiplier}\label{field-left-side-opening-multiplier-1}

The left side opening multiplier (d\(_{t}\)) is calcuated in the same way as the top opening multiplier, with the rules applied to the left side of the shading device.

\paragraph{Field: Right Side Opening Multiplier}\label{field-right-side-opening-multiplier-1}

The right side opening multiplier (d\(_{t}\))~ is calcuated in the same way as the top opening multiplier, with the rules applied to the right side of the shading device.
...
\textbf{\emph{Field: Window Glass Spectral Data Set Name}}
...
\item
  Zone,Average,Green Roof Soil Root Moisture Ratio \protect\hyperlink{section-1}{}
\item
  Zone,Average,Green Roof Soil Near Surface Moisture Ratio \protect\hyperlink{section-1}{}
\item
...
\subsubsection{\texorpdfstring{Green Roof Soil Root Moisture Ratio \protect\hyperlink{section-1}{}}{Green Roof Soil Root Moisture Ratio }}\label{green-roof-soil-root-moisture-ratio}

Mean value of root moisture (m\(^{3}\)/m\(^{3}\))

\subsubsection{\texorpdfstring{Green Roof Soil Near Surface Moisture Ratio \protect\hyperlink{section-1}{}}{Green Roof Soil Near Surface Moisture Ratio }}\label{green-roof-soil-near-surface-moisture-ratio}
...
\caption{Window construction with and without a between-glass shading layer. Shown are gap widths *g*, *g\$\_\{1\}\$* and *g\$\_\{2\}\$*, and shading layer width, *w*. An error will result if *g\$\_\{1\}\$+g\$\_\{2\}\$+w* is not equal to *g*, where *w* is zero for a blind and greater than zero for a shade. \protect \label{fig:window-construction-with-and-without-a}}
\end{figure}
...
\paragraph{HAMT Surface Relative Humidity Cell \textless{}N\textgreater{} {[}\%{]}}\label{hamt-surface-relative-humidity-cell-n-1}
...
\subsection{Matix:TwoDimension}\label{matixtwodimension}
...
\textbf{~ ! Note -- Window5DataFile.dat is presumed to be in the ``run'' folder where EnergyPlus.exe is}
...
\textbf{~ C:\textbackslash{}EnergyPlusData\textbackslash{}DataSets\textbackslash{}MyWindow.dat;}
