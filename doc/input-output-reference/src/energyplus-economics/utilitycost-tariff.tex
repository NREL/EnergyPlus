...
If no value is entered, QuarterHour is assumed. The choice may be overridden based on the value of \hyperref[timestep]{Timestep} so that they are consistent. If the \hyperref[timestep]{Timestep} is 6, i.e.~10 minute timesteps, then QuarterHour is not used and instead HalfHour is used instead. If \hyperref[timestep]{Timestep} is 3, i.e.~20 minute timesteps, then QuarterHour and HalfHour are not used and instead FullHour is used instead. Day and Week are primarily used by fuel utilities.
