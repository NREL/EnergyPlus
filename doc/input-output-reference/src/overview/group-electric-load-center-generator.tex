...
\paragraph{Field: Battery Conservation Method}\label{field-battery-conservation-method}

This field specifies how the amount of energy stored will be managed.  There are two options: Basic and EnergyAndCurrent.  In the Basic option which is the default, the battery only controls the remaining storage in amp-hours.  Due to potential differences in the charging and discharging curves as well as potentially differing rates of charging and discharging, it is possible for this model to conserve storage in amp-hours but not conserve energy stored in Joules.  As a result, it is possible to run a simulation where the amp-hours are conserved but the total energy in Joules discharged from the battery exceeds the sum of the charged energy and the initial energy state.  The Basic option ignores this situation and simply controls amp-hours.  To avoid such a situation where energy in Joules is not conserved, the EnergyAndCurrent option controls both the stored amp-hours and Joules.  If either drops to zero, then both are zeroed out.  This conserves both amp-hours and Joules and avoids a violation of the conservation of energy.

\paragraph{Field: Initial Energy Stored in Battery}\label{field-initial-energy-stored-in-battery}

When the Battery Conservation Method (see previous input field) is set to CurrentAndEnergy by the user, an initial amount of storage in Joules is required in order to monitor both the amp-hours and the Joules stored in the battery at the beginning of the simulation.  Users must enter a value for this field that corresponds to the storage in amp-hours as defined by the maximum amount of storage, fractional state of storage, and number of batteries in fields shown above.  Note that the storage represented by this input is the total of all of the batteries in Joules.  If the user has set the Battery Conservation Method to Basic (or left that field blank), this field for initial energy stored in the battery is ignored and not used.
...
ElectricLoadCenter:Storage:Battery,
  ...
  EnergyAndCurrent,      !- Battery Conservation Method
  4000000;               !- Initial Energy Stored in Battery {J}
...
\item
  HVAC,Average,Electric Storage Battery Charge State Change {[}Ah{]}
\item
...
\item
  HVAC,Sum,Electric Storage Discharge Energy {[}J{]}
\item
  HVAC,Sum,Electric Storage Battery Charge State Energy {[}J{]}
\item
  HVAC,Sum,Electric Storage Battery Charge State Energy Change {[}J{]}
\item
...
\paragraph{Electric Storage Battery Charge State Change {[}Ah{]}}\label{electric-storage-battery-charge-state-change-ah}

The state change of charge reports the amount of change in the charge stored in the battery at a point of time. This change is from the previous time step and like the charge state is for an individual battery.
...
\paragraph{Electric Storage Battery Charge State Energy {[}J{]}}\label{electric-storage-battery-charge-state-energy-j}

This output reports the current amount of energy stored in the battery system at the end of the current time step in Joules.  When the Battery Conservation Method is set to Basic, this number could report as negative giving an indication that more energy has been discharged from the battery than was used to charge the battery.  Negative numbers for this output result from differences in the charging and discharging voltage.  To avoid negative energy storage, the user can set the Battery Conservation Method to CurrentAndEnergy which avoids negative numbers for either the storage in Ah or J.

\paragraph{Electric Storage Battery Charge State Energy Change {[}J{]}}\label{electric-storage-battery-charge-state-energy-change-j}

This output reports the change in the amount of energy stored in the battery system from the end of the previous time step to at the end of the current time step in Joules.  Positive numbers indicate charging while negative numers denote discharging during a particular time step.
