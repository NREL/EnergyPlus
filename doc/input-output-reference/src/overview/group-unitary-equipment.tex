...
This alpha field defines the name of the HVAC system node where the bypass air enters the zone mixer or return plenum. This name should match the name of the inlet node in the \hyperref[airloophvaczonemixer]{AirLoopHVAC:ZoneMixer} or \hyperref[airloophvacreturnplenum]{AirLoopHVAC:ReturnPlenum} associated with this system. This node name must be different from the system's air outlet node or bypass duct splitter node name.
...
6)~~~When the Plenum or Mixer Inlet Node Name is specified, this node name must connect to either a \hyperref[airloophvaczonemixer]{AirLoopHVAC:ZoneMixer} or \hyperref[airloophvacreturnplenum]{AirLoopHVAC:ReturnPlenum} and the \hyperref[airloophvacoutdoorairsystem]{AirLoopHVAC:OutdoorAirSystem} may then be used to control the outdoor air flow rates and, therefore, the inlet condition to the unit.
