...
\item
  To control a zone to a specified indoor level of contaminants. When the zone is served by an \hyperref[airloophvac]{AirLoopHVAC}, other zones served by the same AirLoopHVAC that do not have an available controller may use this controller's setpoint as described below. Currently, the available contaminant controls are carbon dioxide and generic contaminant controls. The specified carbon dioxide setpoint is used to calculate the required outdoor airflow rate through the HVAC system to reach the setpoint. The \hyperref[airloophvac]{AirLoopHVAC} system outdoor flow rate is realized by the \hyperref[controllermechanicalventilation]{Controller:MechanicalVentilation} object with System Outdoor Air Method = IndoorAirQualityProcedure or IndoorAirQualityProcedureCombined. The specified generic contaminant setpoint is used to calculate the required outdoor airflow rate through the HVAC system to reach the setpoint. The AirLoopHVAC system outdoor flow rate is realized by the Controller:MechanicalVentilation object with System Outdoor Air Method = IndoorAirQualityProcedureGenericContaminant or IndoorAirQualityProcedureCombined.
\item
...
For the first purpose above, a zone with an available ZoneControl:ContaminantController uses the setpoint specified by its own controller. If the zone does not have an available controller, EnergyPlus searches the ZoneControl:ContaminantController objects in processing order and uses the setpoint from the first available controller connected to an \hyperref[airloophvac]{AirLoopHVAC} that also serves the zone. For IDF input, processing order is the order in which the controller objects occur in the file; for epJSON input, controller instances are processed in object-name order. The selection therefore does not depend on the internal zone index. This behavior applies separately to carbon dioxide and generic contaminant control, using the corresponding availability and setpoint schedules. If no available controller is found on an AirLoopHVAC serving the zone, no contaminant-control outdoor air demand is calculated for that zone. For example, assume an AirLoopHVAC serves zones 1 through 5 and the controller for zone 2 is processed before the controller for zone 4. While both controllers are available, zones 1, 3, and 5 use the setpoint from the zone 2 controller, and zones 2 and 4 use their own setpoints. If the zone 2 controller is unavailable, zones 1, 2, 3, and 5 use the setpoint from the zone 4 controller while that controller is available. Specify a controller for each zone that requires a distinct setpoint so that the result does not depend on controller processing order.

The same ZoneControl:ContaminantController inputs support the following applications when the Outdoor Air Method is specified in the \hyperref[designspecificationoutdoorair]{DesignSpecification:OutdoorAir} object.
...
This required field contains the name of a schedule that defines the zone carbon dioxide concentration setpoint as a function of time. The units for the carbon dioxide setpoint are ppm, and the schedule values must be between 0 and 2000 ppm. When Carbon Dioxide Concentration = Yes in the \hyperref[zoneaircontaminantbalance]{ZoneAirContaminantBalance} object, this setpoint is used to calculate the outdoor air mass flow rate required to meet the zone setpoint. The resulting flow rate is used when System Outdoor Air Method = IndoorAirQualityProcedure or IndoorAirQualityProcedureCombined in the \hyperref[controllermechanicalventilation]{Controller:MechanicalVentilation} object and when Outdoor Air Method = IndoorAirQualityProcedure in the \hyperref[designspecificationoutdoorair]{DesignSpecification:OutdoorAir} object. The setpoint is also used by \hyperref[availabilitymanagerhybridventilation]{AvailabilityManager:HybridVentilation} when Ventilation Control Mode = 7 (carbon dioxide control).
...
This field contains the name of a schedule that defines the zone generic contaminant concentration setpoint as a function of time. The units for the generic contaminant setpoint are ppm, and the schedule values must be \textgreater{}= 0. This field is required when Generic Contaminant Concentration = Yes in the \hyperref[zoneaircontaminantbalance]{ZoneAirContaminantBalance} object; there is no default setpoint when the field is omitted. The setpoint is used to calculate the outdoor air mass flow rate required to meet the zone setpoint. The resulting flow rate is used when System Outdoor Air Method = IndoorAirQualityProcedureGenericContaminant or IndoorAirQualityProcedureCombined in the \hyperref[controllermechanicalventilation]{Controller:MechanicalVentilation} object.
