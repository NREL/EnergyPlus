...
This field controls whether to overwrite the scheduled cooling setpoint temperature when the adaptive setpoint is lower than the user input one. This field allows controlling how the program should behave during conflicts that may occur because the scheduled values may include night setup of the cooling setpoint which is warmer than the single value for the entire day generated by the adaptive models. Enter \textbf{OverwriteWhenEnergySaving} here to overwrite the operative setpoint only when the generated adaptive setpoint is higher than the original one. Enter \textbf{AlwaysOverwrite} to overwrite the operative setpoint under any circumstance when the object is defined.
