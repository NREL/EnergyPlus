...
Coil:Heating:Desuperheater,
  ...
  Coil:Cooling:DX, !- Heating Source Type
...
Following is an example input for a Coil:Cooling:DX coil.
...
\textbf{MultiMode} - activate enhanced dehumidification mode as needed and operate to meet the sensible load. If a sensible load exists, the system will operate to meet that sensible load and may not meet the latent load. If no sensible load exists, and Run on Latent Load = Yes, the system will operate to meet the entire latent load. This option is used to model DX equipment with a switchable option such as subcool reheat. It is valid only with Cooling coil type = Coil:Cooling:DX, CoilSystem:Cooling:DX:HeatExchangerAssisted, or Coil:Cooling:DX (last coil option must reference an enhanced dehumidification Coil:Cooling:DX:CurveFit:OperatingMode object). If the Run on Latent Load option below is set to Yes, this option may require the use of a heating coil and heating coil outlet air temperature set point manager downstream of this cooling coil to maintain the temperature set point.

\textbf{CoolReheat} - cool beyond the dry-bulb setpoint as required to meet the humidity setpoint. It is valid only with Cooling coil type = Coil:Cooling:DX, or Coil:Cooling:DX that references an enhanced dehumidification Coil:Cooling:DX:CurveFit:OperatingMode object. This option requires the use of a heating coil and heating coil outlet air temperature set point manager downstream of this cooling coil to maintain the temperature set point.
...
\textbf{\emph{Coil Type = Coil:Cooling:DX}}
...
\textbf{\emph{Coil types = Coil:Cooling:DX \& CoilSystem:Cooling:DX:HeatExchangerAssisted}}
...
\end{itemize}
\item
  DX cooling coil object (Coil:Cooling:DX)
\end{itemize}
...
This alpha field denotes the type of DX cooling coil being modeled. The only valid choice is Coil:Cooling:DX.
...
This alpha field defines the space cooling (SC) mode in an ASIHP, which must be given. The SC mode refers to the name of a Coil:Cooling:DX object, which contains all the performance curves and rated values to describe the space cooling operation.
