...
Zone name is ignored.

\paragraph{Field: Surface \textless{}\#\textgreater{} Name}\label{field-surface-name-002}

This field is the name of a surface in the radiant enclosure seen by the person. All surfaces listed should be in the same radiant enclosure. This should also be the same radiant enclosure as the People instance which references this ComfortViewFactorAngles object. If not, a warning will be issued, but the simulation will proceed with the specified surfaces.
...
\item[Watts/Area] With this choice, the method used will be a factor per floor area of the zone. (The Watts per Zone Floor Area field should be filled).
...
This input is the maximum water volumetric flow rate in m\(^{3}\)/s going between the pool and the water heating equipment. This along with the pool setpoint temperature and the heating plant equipment outlet temperature will establish the maximum heat addition to the pool. If a bypass branch is present and in parallel with the swimming pool, the actual flow rate to the pool will be varied in an attempt to reach the desired pool water setpoint temperature (see Setpoint Temperature Schedule below). This variation of flow rate at a given inlet temperature enables the pool to get the correct heat addition from the water loop to achieve the setpoint temperature and the pool outlet temperature will be the same as the pool water temperature. In cases where there is not a bypass present/in parallel with the pool, the flow rate to the pool component is set by the loop, and an implied bypass and three-way valve is assumed so that the flow rate to the pool component stays constant.  In this case, the outlet temperature from the pool is actual the mixed temperature of water coming from the pool and the implied bypass and will be between the inlet temperature and the pool water temperature.
