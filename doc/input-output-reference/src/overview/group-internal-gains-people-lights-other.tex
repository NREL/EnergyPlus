...
\toprule
Fixture No. & Luminaire Feature & Return Air Fraction & Fraction Radiant & Fraction Visible & \(f_{\mathrm{convected}}\) \tabularnewline
\midrule
\endhead
...
\begin{equation}
  (\mathrm{Return Air Fraction})_{\mathrm{calculated}} = C_{1} - C_{2} \times T_{\mathrm{plenum}}
\end{equation}

where T\(_{\mathrm{plenum}}\) is the previous-time-step value of the return plenum air temperature (C),

and C\(_{1}\) and C\(_{2}\) are the values of the coefficients entered in the next two fields.

To compensate for the change in the return air fraction relative to its input value, the program modifies Fraction Radiant and \(f_{\mathrm{convected}}\) by a scale factor such that

\begin{equation}
  (\mathrm{Return Air Fraction})_{\mathrm{calculated}} + (\mathrm{Fraction Radiant})_{\mathrm{modified}} + (f_{\mathrm{convected}})_{\mathrm{modified}} + (\mathrm{Fraction Visible})_{\mathrm{input}} = 1.0
\end{equation}
...
\begin{equation}
  f_{\mathrm{convected}} = 1.0 - (\mathrm{Fraction Latent} + \mathrm{Fraction Radiant} + \mathrm{Fraction Lost})
\end{equation}
...
\begin{equation}
  f_{convected} = 1.0 - (\mathrm{Fraction Latent} + \mathrm{Fraction Radiant} + \mathrm{Fraction Lost})
\end{equation}
...
You will get an error message if \(\mathrm{Fraction Latent} + \mathrm{Fraction Radiant} + \mathrm{Fraction Lost}\) exceeds 1.0.
...
\(C_{\mathrm{cutoff}}\) = Cutoff concentration at which emission ceases {[}ppm{]}
...
\(\mathrm{F}_{R}\) = Schedule or control signal value at time \(t\) {[}-{]}
