...
This field is the ``standard effective temperature'' (SET) calculated using the Pierce two-node thermal comfort model.  Note that if a user wishes that have SET reported that it must be done using the Pierce two-node model and the user must select "Pierce" as one of the Thermal Comfort model types as shown above in the input syntax for the People statement.
