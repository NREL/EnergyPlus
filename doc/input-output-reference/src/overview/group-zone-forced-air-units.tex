...
The name of the plenum exhaust air node connected to the ideal loads object. When an AirloopHVAC:ZoneReturnPlenum is connected to the Ideal Loads Air System the Zone Supply Air Node Name should be the same as one of the zone air inlet nodes and the Zone Exhaust Air Node Name should be the same as one of the zone air exhaust nodes for the zone this component is serving and one of the AirloopHVAC:ReturnPlenum outlet or induced nodes. In the AirloopHVAC:ZoneReturnPlenum object, one of the plenum exhaust nodes is listed as the plenum Outlet Node Name (required) while the remaining nodes, if needed, are listed in the Induced Air Outlet Node or NodeList Name field.
