...
The name of the HVAC system node from which the fan coil unit draws its indoor air. This should be either one of zone exhaust nodes (Ref. \hyperref[zonehvacequipmentconnections]{ZoneHVAC:EquipmentConnections}) for the zone which the fan coil unit is serving or one of induced nodes (Ref. \hyperref[airloophvacreturnplenum]{AirLoopHVAC:ReturnPlenum}) to draw air from a return plenum, when the zone return node is connected to a return plenum.
