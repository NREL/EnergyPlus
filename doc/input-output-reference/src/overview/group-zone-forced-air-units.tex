...
\textbf{Notes:} The ideal loads system uses the zone return node or an optional zone exhaust node to extract air from the zone. When the AirloopHVAC:ReturnPlenum is used the inlet of the ideal loads system is connected to the plenum exhaust node while the outlet of the ideal loads system is connected to a zone inlet node and the node connected between the zone and plenum is specified as the zone exhaust node. Every zone served by an HVAC component must have a return air node, even though this node may not be connected to anything.
...
The name of the plenum outlet (or induced) air node connected to the ideal loads object. When an AirloopHVAC:ReturnPlenum is connected to the Ideal Loads Air System the Zone Supply Air Node Name should be the same as one of the zone air inlet nodes. The Zone Exhaust Air Node Name should be the same as one of the zone air exhaust nodes and one of the AirloopHVAC:ReturnPlenum inlet nodes. In the AirloopHVAC:ReturnPlenum object, for the first ideal loads air system, the System Inlet Air Node Name is entered as the plenum Outlet Node Name (required field) while all remaining ideal loads air systems, if used, list the System Inlet Air Node Name in the Induced Air Outlet Node or NodeList Name field (see Engineering Reference for schematic).
