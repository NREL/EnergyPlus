...
The name of the plenum exhaust air node connected to the ideal loads object. When an AirloopHVAC:ZoneReturnPlenum is connected to the Ideal Loads Air System the Zone Supply Air Node Name should be the same as one of the zone air inlet nodes and the Zone Exhaust Air Node Name should be the same as one of the zone air exhaust nodes for the zone this component is serving. This node name is required if ZoneHVAC:IdealLoadsAirSystem is used in a zone which also has other forced air HVAC equipment. Otherwise this node name is optional but recommended. In the AirloopHVAC:ZoneReturnPlenum object, one of the plenum exhaust nodes is listed as the plenum Outlet Node Name while the remaining nodes, if needed, are listed in the Induced Air Outlet Node or NodeList Name field.
