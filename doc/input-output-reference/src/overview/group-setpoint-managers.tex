...
If the Control Variable input field is set to MaximumTemperature, then the setpoint will be set to a setpoint value specified at Setpoint at Outdoor Low Temperature input field, else if the Control Variable input field is set to MinimumTemperature, then the setpoint will be set to a setpoint value specified in Setpoint at Outdoor High Temperature input field, else if the Control Variable input field is set to Temperature, then the setpoint will be set to a setpoint value determined based on the current time outdoor air temperature, and the setpoint values specified at Setpoint Outdoor Low and High Temperature input fields.
