...
\paragraph{Field: GHE:CalculationModelName}

There are two options for this input; UHFcalc or UBHWTcalc. The default is
UHFcalc, where a g-function is computed with the uniform heat extraction rate
boundary condition. The g-function computed with a uniform borehole wall
temperature boundary condition is much closer to reality. The UBHWTcalc calls
the third party tool cpgfunctionEP. The UBHWT g-function calculation of
cpgfunctionEP is tuned using an adaptive discretization scheme that approximates
g-functions computed by the uniform inlet fluid temperature g-function.
...
\begin{lstlisting}
  GroundHeatExchanger:System,
    ...
    UBHWTcalc,          !- g-Function Calculation Model Name
    GHE-Array;
