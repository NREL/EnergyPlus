...
used instead. A flag ``Use Precipitation'' is added in RoofIrrigation input. When
this flag is "Yes", the actual irrigation amount will count both the irrigation
from the pipes and the rainfall. Otherwise, it will only consider the
irrigation from pipes. When rainfall data in the weather file is used, a
...
Use Precipitation is a Yes/No choice field. If it is set to "Yes", then the
actual irrigation amount will include the rainfall quantity. Otherwise, it
will only count the irrigation from the pipes. The default is not to include
rainfall as part of the irrigation.
