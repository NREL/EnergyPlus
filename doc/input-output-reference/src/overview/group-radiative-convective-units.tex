...
This field is the rated standard water flow rate in kg/s which is published as part of the manufacturer's literature. It is used by the manufacturers when determining the rated capacity (see next field). The default value is 0.063kg/s. If it is blank or zero, the default value is assumed. This input is used to calculate UA when the heating capacity is determined from a positive Heating Design Capacity, Capacity Per Floor Area, or Fraction of Autosized Heating Design Capacity. When Heating Design Capacity is autosized, the Maximum Water Flow Rate is used instead.
