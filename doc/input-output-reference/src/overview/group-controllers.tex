...
Input for this field is the minimum outdoor air flow rate for the system in cubic meters per second. This field may be autosized. If a Controller Mechanical Ventilation Name is specified, then this field will autosize to zero.
...
Choosing \textbf{TimeOfDay} means the economizer will be fully open whenever the Economizer Control Schedule is greater than zero. This control type does not modulate to the mixed air temperature; it is either at the minimum or the maximum.

The default for this field is \textbf{NoEconomizer}.

In addition, for all economizer control types listed above, the controller checks the Economizer Control Schedule and the user-entered values for the upper limit of dry-bulb temperature, enthalpy limit, humidity ratio limit and dewpoint limit. The outdoor air flow rate is set to minimum if any of these entered limits are exceeded or the schedule is zero.
...
If the Minimum Outdoor Air Flow Rate is autosized, it will size to zero when a valid Controller:MechanicalVentilation object is entered.  The Minimum Fraction of Outdoor Air Schedule can reduce the outdoor air flow rate set by the Controller:MechanicalVentilation.

\paragraph{Field: Economizer Control Schedule Name}\label{economizer-control-schedule-name}

This alpha field is the name of a schedule which controls the availability of the economizer for all economizer control types. Schedule values greater than 0 enable the economizer and enables high humidity control. If this field is blank, the schedule defaults to 1.0 at all times.
...
If the Controller:OutdoorAir Minimum Outdoor Air Flow Rate is autosized, it will size to zero when a valid Controller:MechanicalVentilation object is entered.
