...
\item
  Zone,Average,Zone Exfiltration Heat Transfer Rate {[}W{]}
\item
  Zone,Average,Zone Exfiltration Sensible Heat Transfer Rate {[}W{]}
\item
  Zone,Average,Zone Exfiltration Latent Heat Transfer Rate {[}W{]}
\item
  Zone,Average,Zone Exhaust Air Heat Transfer Rate {[}W{]}
\item
  Zone,Average,Zone Exhaust Air Sensible Heat Transfer Rate {[}W{]}
\item
  Zone,Average,Zone Exhaust Air Latent Heat Transfer Rate {[}W{]}
\item
...
These output variables represent the heat emitted to ambient from exfiltration in watts. The exfiltration rate is calculated by solving a mass flow balance on the zone including infiltration, ventilation, outdoor air mixing, zone-to-zone mixing, and all of the zone inlet, exhaust, and return nodes. The latent parts are determined by taking the difference between the total and the sensible rate. The positive values indicate the zone injects heat to the environment, while the negative values indicate the building extracts heat from the environment. (This output variable is only generated if the user has set a computer system environment variable DisplayAdvancedReportVariables equal to "yes".)
...
These output variables represent the heat emitted to ambient from exhaust air in watts. The zone exhaust air flow rate is aggregated from the zone exhaust nodes. The positive values indicate the building injects heat to the environment, while the negative values indicate the building extracts heat from the environment. (This output variable is only generated if the user has set a computer system environment variable DisplayAdvancedReportVariables equal to "yes".)

\subsubsection{Site Total Zone Exfiltration Heat Loss {[}W{]}}\label{site-total-exfiltration-heat-transfer-rate-w}

These output variables represent the total amount of heat emitted to ambient from all zones by exfiltration in watts. The positive values indicate the building injects heat to the environment, while the negative values indicate the building extracts heat from the environment.

\subsubsection{Site Total Zone Exhaust Air Heat Loss {[}W{]}}\label{site-total-exhaust-heat-transfer-rate-w}

These output variables represent the total amount of heat emitted to ambient from all zones by exhaust air in watts. The positive values indicate the building injects heat to the environment, while the negative values indicate the building extracts heat from the environment.
...
This is thermal radiation heat transfer rate between the outside face and the air mass surrounding the surface, calculated by the coeffient.
