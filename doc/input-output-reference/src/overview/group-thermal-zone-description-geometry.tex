...
The Zone Air Heat Balance System Air Transfer Rate is the sum, in watts, of heat transferred to the zone air by HVAC forced-air systems and air terminal units. Such HVAC systems are connected to the zone by an inlet node (see ZoneHVAC:EquipmentConnections input field called Zone Air Inlet Node or Node List Name) This field is not multiplied by zone or group multipliers. The Zone Air Heat Balance System Air Transfer Rate may not agree exactly with equipment-level delivered energy transfer rate when the zone thermostat temperature setpoint is changing significantly (e.g. during thermostat setback and setup) over a timestep, but the energy will balance out over time. 
...
This field represents the sensible heating rate in Watts that is actually supplied by the system to that zone for the timestep reported. This is calculated and reported from the Correct step in the Zone Predictor-Corrector module. This field is not multiplied by zone or group multipliers. The zone air sensible heating rate may not agree exactly with equipment-level delivered heating rate when the zone thermostat temperature setpoint is changing significantly (e.g. during thermostat setback and setup) over a timestep, but the energy will balance out over time.

\subsubsection{Zone Air System Sensible Cooling Rate {[}W{]}}\label{zone-air-system-sensible-cooling-rate-w-1}

This field represents the sensible cooling rate in Watts that is actually supplied by the system to that zone for the timestep reported. This is calculated and reported from the Correct step in the Zone Predictor-Corrector module. This field is not multiplied by zone or group multipliers. The zone air sensible cooling rate may not agree exactly with equipment-level delivered cooling rate when the zone thermostat temperature setpoint is changing significantly (e.g. during thermostat setback and setup) over a timestep, but the energy will balance out over time.
