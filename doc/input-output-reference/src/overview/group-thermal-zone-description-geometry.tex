...
\subsubsection{Surface Internal User Specified Location Temperature {[}C{]}}\label{surface-internal-user—specified-location-temperature-c}

When a surface has an internal source or sink (defined using Construction:InternalSource) then this output is available for the temperature within the construction at the location requested by the user.  It is defined using the Construction:InternalSource input as after a particular layer.  This output will calculate the interior temperature at that location.
