...
The CarrollMRT options seemed to have a much more significant impact on the natural gas usage, and the total water and the times were similar to the runtimes using ScriptF. So for this example, CarrollMRT doesn’t seem to be a right choice. In this case, the computatoinal times are not very different than the ScriptF instances, but the energy usage is further away from the Normal baseline. The electricity usage differences for Runs 2 through 8 are small compared to Run 1 (the normal case), and are less than 0.27\% different. Natural gas usage has a more significant impact of 1.4\% to 1.9\%, and the water usage changes from 0.36\% to 0.88\%. From an energy perspective, these impacts for the ScriptF runs 2 through 8 are probably tolerable. 
...
The number of hours any zone is oscillating for Run 1 the Normal baseline case using Script F is only 2.92 hours, with almost none of the hours in the dead band and nearly all during occupancy. This changes when considering the other ScriptF cases using Mode01 to Mode07 (Run 2 to Run 8).  For Mode01, the oscillating hours increase but are still less than 8 hours per year, while more than doubling, on an absolute basis, a small change. The change is even more minor when considering the hours with the occupancy, which for Mode02 (Run 2) even decreased by a small amount. The oscillation in the deadband for Mode02 (Run 2) increased about 6 hours. For Mode02 to Mode04, there is an increase of about 2 hours for each oscillating hour categories (the total, occupancy, and deadband), though they are considered to be a relatively minor change especially when comparing further with the succeeding Mode05 and Mode07.For Mode05 to Mode07, the oscillating hours increase significantly, by more than one order of magnitude to aroudn 155 to 158 hours per year. The numbers are also reflected in the oscillating hours during occupancy and the oscillating hours during dead band.  For the CarrollMRT cases, similar trend can be observed when applying Mode01, and from Mode02 to Mode07.  The most significant change is Mode02, which for both ScriptF and CarrollMRT raise the oscillating hours to about 155--158. This is a substantial change that does indicate a significant shift in the way the simulation is behaving. The oscillation could be moderately tamed by applying any of the subsequent modes (Modes03 to Modes07), without too much loss in the simulation speed. On the other hand, however, the warnings for all the seventeen cases were not too much different from each other, all ranging from 46 to 54. No severe errors were reported for any of these simulations, so no column is shown for them, and the column for warnings shows a small increase. After investigating the changes, the change in the count warning messages was not considered important.

The decision on which mode to use is up to the individual modelers. However for these simulations runs, it can be clearly divided into three tiers based on the simulation speed (computation time) and the numerical stability (oscillation hours): the normal run tier, which gains the least (the best) oscillations, but take the longest computation time; the second tier include Mode02 to Mode04, where the simulatoin can be cut to roughly to one-third of the Normal run baseline, but with a minor penalty for increased ocsillation hours; the third tier, which can achieve an superior running time advantage (reaching around one-fifth to one-seventh of normal baseline), but with significant oscillation hours panlty of about one order of mangintude higher. For simulations conducted in these test modes, Runs 5 (Mode04) is recommended as a balanced option for speed and numerical stability (oscillations). It does not use direct coil solution, uses ScriptF, and Mode04. This simulation time also shows a slight advantages over the other cases. It saves time compared to Mode01-Mode03 and has the oscillating hours are still quite tamed. Further, it does not add too much additional overrides, which should be avoided if not adding value.  For extreme speed benifit, higher modes such as Mode05 to Mode07 definitely showed an advantaged as the compuatational time is only about half of that for Mode04, although substantially higher oscillating hours will be inccured as a downside.
