...
\toprule 
Demand Window & Applicable Number of Timesteps per Hour \tabularnewline \midrule
\endhead
...
% table 1
\begin{longtable}[c]{@{}ll@{}}
\caption{Values for ``Terrain'' \protect \label{table:values-for-terrain}}\\
\toprule 
Terrain Type Value & Terrain Description \tabularnewline \midrule
\endhead
...
1) \textbf{Shadowing surfaces}. These are surfaces like overhangs or neighboring buildings entered with Shading:Site, Shading:Building, Shading:Site:Detailed, Shading:Building:Detailed, Shading:Overhang, Shading:Overhang:Projection, Shading:Fin, Shading:Fin:Projection or Shading:Zone:Detailed objects. See Figure~\ref{fig:solar-reflection-from-shadowing-surfaces.}.
...
This field specifies the name of a schedule that contains outdoor air carbon dioxide level values in units of ppm. One source of monthly average CO\(_{2}\) levels in the atmosphere is available at a NOAA ftpsite as .
