...
\paragraph{Field: Construction Assignment Set Name}\label{building-field-construction-assignment-set-name}

This optional field references a \hyperref[constructionassignmentset]{ConstructionAssignmentSet} object. It provides inherited construction assignments for any surface or sub-surface in the building whose \emph{Construction Name} field is left blank and which does not inherit a set from its Space (ref: the \emph{Construction Assignment Set Name} field on the \hyperref[space]{Space} object, which takes precedence over this one). If a surface's construction cannot be resolved through an explicit assignment, its Space's set, or this Building-level set, a fatal error results.
...
\begin{equation}
    \textrm{PLR} = \frac{\textrm{SystemLoad} - \textrm{OutputOff}}{\textrm{OutputFull} - \textrm{OutputOff}}
\end{equation}
...
\begin{equation}
    \textrm{SpeedRatio} = \frac{\textrm{SystemLoad} - \textrm{OutputFull}_{i-1}}{\textrm{OutputFull}_{i} - \textrm{OutputFull}_{i-1}}
\end{equation}
...
\begin{equation}
    \textrm{PLR} = \frac{\textrm{DesOutTemp} - \textrm{InletTemp}}{\textrm{OutTempFull} - \textrm{InletTemp}}
\end{equation}
...
\begin{equation}
    \textrm{SpeedRatio} = \frac{\textrm{DesOutTemp} - \textrm{OutTempFull}_{i-1}}{\textrm{OutTempFull}_{i} - \textrm{OutTempFull}_{i-1}}
\end{equation}
...
A custom meter allows the user to group variables or meters onto a virtual meter that can be used just like a normal meter created by EnergyPlus. For consistency, the items being grouped must all be similar. A Meter:Custom cannot reference another Meter:Custom or a Meter:CustomDecrement.
...
A key name field is used when the following field specifies an output variable.
Use ``*'' (or leave the field blank) to assign all instances of the output variable to the meter.

You can also use regular expressions to assign only the instances whose key matches the pattern.
For example, to include only the lights energy for zones whose names begin with ``SPACE'', you can use ``SPACE.*''.
...
Note that because the Key Name field does not preserve case, all alphabetic characters in the pattern are converted to uppercase before matching, effectively making this a case-insensitive regex as if ``(?i)<pattern>'' was used.

Additionally, no regular expression can contain a ``,'' because the Input Processor would interpret it as a field separator.

\paragraph{Field: Output Variable or Meter Name \#}\label{field-output-variable-or-meter-name}

This field must be a valid output variable name or a valid meter name. If a Meter:Custom references another Meter:Custom or a Meter:CustomDecrement it will generate a warning and not produce any output.
...
The decrement custom meter is very similar to the custom meter specification but additionally allows a predefined meter to be used as the ``source'' meter and the remaining items subtract from that predefined meter. A Meter:CustomDecrement cannot reference another Meter:CustomDecrement, either as the Source Meter Name or in the group(s) Key Name-Output Variable/Meter Name fields, though it may reference a Meter:Custom.
...
This name specifies the meter that will be used as the main source for the decrement custom meter. The remainder of the fields are subtracted from the value of this meter to create the meter value named above. The Source Meter is not changed in any way by including this custom meter. This field may reference a Meter:Custom, but if it references another Meter:CustomDecrement it will generate an error.
...
This field must be a valid output variable name or a valid meter name. Additionally, it must be contained on the Source Meter. Note that, if an error occurs, only the Variable in error will show -- confusing things if what was entered was a meter name. This field may reference a Meter:Custom, but if it references another Meter:CustomDecrement it will generate a warning and not produce any output. For output variables, only summed variables are valid. For example from the rdd output, any Sum type variable is valid:
\begin{lstlisting}
 Zone,Sum,Exterior Equipment Electricity Energy [J]
\end{lstlisting}
...
Meter:Custom,
  SPACE 1-3 Lights,           !- Name
  Electricity,                !- Resource Type
  SPACE[1-3]-1,               !- Key Name 1
  Lights Electricity Energy;  !- Output Variable or Meter Name 1
\end{lstlisting}
