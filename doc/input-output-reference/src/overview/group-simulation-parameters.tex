...
The field has two options: \textbf{ScheduledShading} and \textbf{InternalCalculation}. The field indicates whether the external shading is pre-calculated and imported as schedules, which overwrite the EnergyPlus calculated array of the surface sunlit fraction. If \textbf{ScheduledShading} is chosen, all external shading calculation would be performed externally (EnergyPlus internal calculations of external shading will be turned off) and be imported during the initialization of an EnergyPlus run with the schedule object defined by the \textbf{SurfaceProperty:LocalEnvironment} object of this surface. If some exterior surfaces do not have their \textbf{SurfaceProperty:LocalEnvironment} objects, no shading is assigned on those exterior surfaces. The default choice is \textbf{InternalCalculation}, indicating EnergyPlus will do the external shading calculations and no external results will be used.
...
\paragraph{Field: Disable Shading Within A Zone Group}\label{field-disable-shading-within-a-zone-group}
This fields specifies during shading calculation, for all surfaces in a targeted Zone Group, whether or not (\textbf{Yes} or \textbf{No} ) the shading effect by other exterior surfaces of all zones in the target Zone Group is disabled. If \textbf{Yes} is chosen, the zones in a self-shading group will not self-shade each other, but they will cast shadows on all other zones in the model. Any zones not listed in a self-shading group will shade any zone in the model. If either Disable Shading Within A Zone Group = Yes or Disable Shading Between Zone Groups = Yes, but not both, Self-Shading Groups should be defined. If both Disable Shading Within A Zone Group and Disable Shading Between Zone Groups are Yes, all self-shading effect by all zones will be disabled.

\paragraph{Field: Disable Shading Between Zone Groups}\label{field-disable-shading-between-zone-groups}
This fields specifies during shading calculation, for all surfaces in a targeted Zone Group, whether or not (\textbf{Yes} or \textbf{No} ) the shading effect by all exterior surfaces of all other zones apart from the target Zone Group is disabled. If \textbf{Yes} is chosen, the zones in a self-shading group will still self-shade each other, but they will not cast shadows on all other zones in the model. Any zones not listed in a self-shading group will shade any zone in the model. If either Disable Shading Within A Zone Group = Yes or Disable Shading Between Zone Groups = Yes, but not both, Self-Shading Groups should be defined. If both Disable Shading Within A Zone Group and Disable Shading Between Zone Groups are Yes, all self-shading effect by all zones will be disabled.

\paragraph{Field: Shading Group ZoneList Name}\label{field-shading-group-zoneList-name}
The shading zones group specifies the self-shading group defined in \textbf{field:Disable Shading Within A Zone Group} and \textbf{field:Disable Shading Between Zone Groups}.
