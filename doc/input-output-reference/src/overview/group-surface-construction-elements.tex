...
surfaces. The model uses ``actual'' convective mass transfer
...
This moisture model will be used when the appropriate EMPD moisture
...
where \(T\) is the temperature and \(P_{ambient}\) the ambient
atmospheric pressure.

\paragraph{Fields: Moisture equilibrium
	constants}\label{fields-moisture-equilibrium-constants}

The next four fields, coefficients ``a'', ``b'', ``c'', and ``d'', help
define the sorption isotherm curve used for building materials under
equilibrium conditions. They are used to define the relationship between
...
relative humidity variation. The surface penetration depth can be
...
sorption curve). Density, input elsewhere, is :
...
from Woods et al., 2014 (see Engineering Reference). Density is
kg/m\^{}3:
...
\paragraph{EMPD Surface Layer Moisture Content
	{[}kg/m3{]}}\label{empd-surface-layer-moisture-content-kgm3}
...
\paragraph{EMPD Deep Layer Moisture Content
	{[}kg/m3{]}}\label{empd-deep-layer-moisture-content-kgm3}
...
The left side opening multiplier (d\(_{left}\)) is calcuated in the same way as the top opening multiplier, with the rules applied to the left side of the shading device.

\paragraph{Field: Right Side Opening Multiplier}\label{field-right-side-opening-multiplier-1}

The right side opening multiplier (d\(_{right}\)) is calcuated in the same way as the top opening multiplier, with the rules applied to the right side of the shading device.
