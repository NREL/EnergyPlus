...
The absorptance values in this object impart surface properties to the construction and should be applied to the thermally significant inner and outer layers in the overall assembly.~ For an opaque construction, only the outside-face properties of the outermost layer and the inside-face properties of the innermost layer affect the surface heat balances; absorptance values on intermediate layers are not surface properties. Attempting to trick the program by modeling thin ``paint'' layers to apply surface properties is not a good idea; the models were not intended to support such strategies.
...
This field specifies the bulk thermal absorptance: the fraction of incident longwave radiation (wavelengths greater than 2.5~$\mu{}m$) absorbed by the material. When Thermal Absorptance Inside Face is blank, this value is used at both faces. When that optional field is entered, this value applies to the outside face. Thermal absorptance is used in longwave radiant exchange and the surface heat balances; by Kirchhoff's law, thermal emissivity and emittance equal thermal absorptance. The value must be greater than 0.0 and no greater than 0.99999. The default is 0.9.

\paragraph{Field: Solar Absorptance}\label{field-solar-absorptance}

This field specifies the bulk solar absorptance: the fraction of incident solar radiation (approximately 0.3 to 2.537~$\mu{}m$) absorbed by the material. When Solar Absorptance Inside Face is blank, this value is used at both faces. When that optional field is entered, this value applies to the outside face. For an opaque material, absorptance equals 1.0 minus reflectance. The value must be between 0.0 and 1.0. The default is 0.7.

\paragraph{Field: Visible Absorptance}\label{field-visible-absorptance}

This field specifies the bulk visible absorptance: the fraction of photopically weighted visible radiation (approximately 0.37 to 0.78~$\mu{}m$) absorbed by the material. When Visible Absorptance Inside Face is blank, this value is used at both faces. When that optional field is entered, this value applies to the outside face. The value affects visible-radiation and daylighting calculations and, for an opaque material, equals 1.0 minus visible reflectance. The value must be between 0.0 and 1.0. The default is 0.7.

\paragraph{Field: Thermal Absorptance Inside Face}\label{field-thermal-absorptance-inside-face}

This optional field specifies the thermal absorptance used at the inside face when this material is the innermost layer of a construction. When blank, it inherits the Thermal Absorptance value above. The value must be greater than 0.0 and no greater than 0.99999.

\paragraph{Field: Solar Absorptance Inside Face}\label{field-solar-absorptance-inside-face}

This optional field specifies the solar absorptance used at the inside face when this material is the innermost layer of a construction. When blank, it inherits the Solar Absorptance value above. The value must be between 0.0 and 1.0.

\paragraph{Field: Visible Absorptance Inside Face}\label{field-visible-absorptance-inside-face}

This optional field specifies the visible absorptance used at the inside face when this material is the innermost layer of a construction. When blank, it inherits the Visible Absorptance value above. The value must be between 0.0 and 1.0.

Here are two IDF examples:
...
In the example above, the three existing absorptance fields are bulk values and therefore apply to both faces.

\begin{lstlisting}
Material,
  Material Diff Absorptances,  !- Material Name
  Rough,                       !- Roughness
  0.05,                        !- Thickness {m}
  2.5,                         !- Conductivity {W/M*K}
  1500.0,                      !- Density {Kg/M**3}
  888.8,                       !- Specific Heat {J/Kg*K}
  0.9000000,                   !- Thermal Absorptance
  0.7000000,                   !- Solar Absorptance
  0.7000000,                   !- Visible Absorptance
  0.6000000,                   !- Thermal Absorptance Inside Face
  0.4500000,                   !- Solar Absorptance Inside Face
  0.4500000;                   !- Visible Absorptance Inside Face
\end{lstlisting}

In the second example, the three existing absorptance fields apply to the outside face and the three optional fields apply to the inside face.
...
This field has the same bulk and face-specific behavior as Thermal Absorptance for the \hyperref[material]{Material} object. It is used at both faces when Thermal Absorptance Inside Face is blank and at the outside face when the optional inside-face value is entered. The value must be greater than 0.0 and no greater than 0.99999. The default is 0.9.

\paragraph{Field: Solar Absorptance}\label{field-solar-absorptance-1}

This field has the same bulk and face-specific behavior as Solar Absorptance for the \hyperref[material]{Material} object. It is used at both faces when Solar Absorptance Inside Face is blank and at the outside face when the optional inside-face value is entered. The value must be between 0.0 and 1.0. The default is 0.7.

\paragraph{Field: Visible Absorptance}\label{field-visible-absorptance-1}

This field has the same bulk and face-specific behavior as Visible Absorptance for the \hyperref[material]{Material} object. It is used at both faces when Visible Absorptance Inside Face is blank and at the outside face when the optional inside-face value is entered. The value must be between 0.0 and 1.0. The default is 0.7.

\paragraph{Field: Thermal Absorptance Inside Face}\label{field-thermal-absorptance-inside-face-1}

This optional field specifies the inside-face thermal absorptance. When blank, it inherits the Thermal Absorptance value above. The value must be greater than 0.0 and no greater than 0.99999.

\paragraph{Field: Solar Absorptance Inside Face}\label{field-solar-absorptance-inside-face-1}

This optional field specifies the inside-face solar absorptance. When blank, it inherits the Solar Absorptance value above. The value must be between 0.0 and 1.0.

\paragraph{Field: Visible Absorptance Inside Face}\label{field-visible-absorptance-inside-face-1}

This optional field specifies the inside-face visible absorptance. When blank, it inherits the Visible Absorptance value above. The value must be between 0.0 and 1.0.
...
\begin{lstlisting}
Material:NoMass,
  R13LAYER with diff abs, !- Material Name
  Rough,                  !- Roughness
  2.290965,               !- Resistance {M**2K/W}
  0.9000000,              !- Thermal Absorptance
  0.7500000,              !- Solar Absorptance
  0.7500000,              !- Visible Absorptance
  0.2000000,              !- Thermal Absorptance Inside Face
  0.2000000,              !- Solar Absorptance Inside Face
  0.2000000;              !- Visible Absorptance Inside Face
\end{lstlisting}
...
This object defines variable thermal and solar absorptance controls for the outside and inside faces of an opaque exterior heat-transfer surface. The referenced material must be the outermost construction layer for its outside-face controls to apply and the innermost layer for its inside-face controls to apply. Controls on interzone surfaces or on other construction layers are ignored with a warning.

Each face has one control signal---surface temperature, surface received solar radiation, zone heating/cooling mode, or a schedule---shared by its thermal and solar absorptance controls. Thermal and solar absorptance may reference different functions or schedules, and either property may remain constant. The existing Control Signal and function/schedule fields control the outside face. The optional Inside Face fields independently control the inside face. If Control Signal Inside Face is blank, the inside-face absorptances remain at the constant values defined by the referenced material; they do not follow the outside-face controls.

Variable-absorptance coatings can also be modeled with EMS.
...
This required field selects the outside-face control signal: surface temperature, surface received solar radiation, zone heating/cooling mode, or a schedule. For ``Scheduled'', specify at least one outside-face absorptance schedule. For any other signal, specify at least one outside-face absorptance function. Thermal and solar absorptance use the same signal but may use different functions or schedules. The default is SurfaceTemperature.

\paragraph{Field: Thermal Absorptance Function Name}\label{field-thermal-absorptance-function-name}

The name of a Curve or a Table:Lookup object describing the relationship between the control signal and the thermal absorptance at the outside face or exterior side.

\paragraph{Field: Thermal Absorptance Schedule Name}\label{field-thermal-absorptance-schedule-name}

The name of a Schedule object that overrides the outside-face thermal absorptance when Control Signal is Scheduled. If neither this field nor Thermal Absorptance Function Name is defined, the outside-face thermal absorptance remains constant.

\paragraph{Field: Solar Absorptance Function Name}\label{field-solar-absorptance-function-name}

The name of a Curve or a Table:Lookup object describing the relationship between the control signal and the solar absorptance at the outside face or exterior side.

\paragraph{Field: Solar Absorptance Schedule Name}\label{field-solar-absorptance-schedule-name}

The name of a Schedule object that overrides the outside-face solar absorptance when Control Signal is Scheduled. If neither this field nor Solar Absorptance Function Name is defined, the outside-face solar absorptance remains constant.

\paragraph{Field: Control Signal Inside Face}\label{field-control-signal-inside-face}

This optional field selects the inside-face control signal using the same choices as Control Signal. For ``Scheduled'', specify at least one inside-face absorptance schedule. For any other signal, specify at least one inside-face absorptance function. If this field is blank, all inside-face absorptances remain at the constant values defined by the referenced material. This preserves the behavior of input files created before inside-face variable absorptance controls were available.

\paragraph{Field: Thermal Absorptance Function Name Inside Face}\label{field-thermal-absorptance-function-name-inside-face}

The name of a Curve or a Table:Lookup object describing the relationship between the control signal and the thermal absorptance at the inside face or interior side.

\paragraph{Field: Thermal Absorptance Schedule Name Inside Face}\label{field-thermal-absorptance-schedule-name-inside-face}

The name of a Schedule object that overrides the inside-face thermal absorptance when Control Signal Inside Face is Scheduled. If neither this field nor Thermal Absorptance Function Name Inside Face is defined, the inside-face thermal absorptance remains constant.

\paragraph{Field: Solar Absorptance Function Name Inside Face}\label{field-solar-absorptance-function-name-inside-face}

The name of a Curve or a Table:Lookup object describing the relationship between the control signal and the solar absorptance at the inside face or interior side.

\paragraph{Field: Solar Absorptance Schedule Name Inside Face}\label{field-solar-absorptance-schedule-name-inside-face}

The name of a Schedule object that overrides the inside-face solar absorptance when Control Signal Inside Face is Scheduled. If neither this field nor Solar Absorptance Function Name Inside Face is defined, the inside-face solar absorptance remains constant.

Here are two IDF examples of this object:

\begin{lstlisting}
  MaterialProperty:VariableAbsorptance,
    variableThermalR13LAYER, !- Name
    R13LAYER_vary,           !- Reference Material Name
    SurfaceTemperature,      !- Control Signal
    ThermalAbsTempTable,     !- Thermal Absorptance Function Name
    ,                        !- Thermal Absorptance Schedule Name
    SolarAbsTempCurve,       !- Solar Absorptance Function Name
    ;                        !- Solar Absorptance Schedule Name
\end{lstlisting}

In the example above, only the outside face varies. The inside-face absorptances remain at the constant values defined by the referenced material.

\begin{lstlisting}
  MaterialProperty:VariableAbsorptance,
    variableAbsR13LAYERdiff, !- Name
    R13LAYER_vary_diff,      !- Reference Material Name
    SurfaceTemperature,      !- Control Signal
    ThermalAbsTempTable,     !- Thermal Absorptance Function Name
    ,                        !- Thermal Absorptance Schedule Name
    SolarAbsTempCurve,       !- Solar Absorptance Function Name
    ...
    ThermalAbsSched,         !- Inside Face Thermal Absorptance Schedule Name
    ,                        !- Inside Face Solar Absorptance Function Name
    SolarAbsSched;           !- Inside Face Solar Absorptance Schedule Name
\end{lstlisting}

In the second example, the outside face follows the temperature functions while the inside face follows the thermal and solar schedules.
...
\begin{equation}
{\varepsilon_{{\mathrm{eff}}}} \approx \varepsilon \left( {1 - \eta } \right)
\end{equation}
...
Effective long-wave transmittance. Assumed independent of incidence angle. We can approximate this effective long-wave transmittance, \(T_{\mathrm{eff}}\) as follows. Let \(\eta\) be the ``openness'' of the shade, i.e., the ratio of the area of openings in the shade to the overall shade area. Let the long-wave transmittance of the shade material be \(T\). Then

\begin{equation}
T_{\mathrm{eff}} \approx \eta + T \left( 1 - \eta \right)
\end{equation}

For most materials \(T\) is very close to zero, which gives

\begin{equation}
T_{\mathrm{eff}} \approx \eta
\end{equation}
