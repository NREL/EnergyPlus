...
\mathrm{QuadraticLinear} \to Eff ={}& {A_0} + {A_1}\cdot PLR + {A_2}\cdot PL{R^2} + {A_3}\cdot {T_w} + {A_4}\cdot PLR\cdot {T_w} + {A_5}\cdot PL{R^2}\cdot {T_w}\\
\begin{split}
\mathrm{Bicubic} \to Eff ={}& A_0 + A_1 \cdot PLR + A_2 \cdot PLR^2 + A_3 \cdot T_w + A_4 T_w^2 + A_5 \cdot PLR \cdot T_w + A_6 \cdot PLR^3 +\\
&A_7 \cdot T_w^3 + A_8 \cdot PLR^2 \cdot T_w + A_9 \cdot PLR \cdot T_w^2
\end{split}
\end{align}
\noindent where
...
This numeric field contains the maximum design water volumetric flow rate in \si{\meter\cubic\per\second}. This should be the largest flow rate than can be heated to the design outlet temperature. This field is autosizable.
...
This numeric field contains the outlet temperature upper limit. If this field is left blank, the default value is \SI{99.9}{\degreeCelcius}.
...
\noindent One of the following blocks will be applicable based on fuel type:
