...
Next, the EIR-formulated model objects are described. Although the model follows the pattern of having a separate input object for cooling and heating, the inputs are essentially identical, so they are described together.
...
\paragraph{Field: Companion Coil Name}\label{wwhp_eir_inputs_companion_name}

This optional alpha field contains the name of a companion coil.  If the current coil is heating type then this is expected to include the paired cooling coil, and vice-versa.  The companion coil is used for sizing if plant sizing data is unavailable, and during the simulation to ensure simultaneous operation of the two coils provides a warning to the user.
...
\paragraph{Field: Capacity Function of Temperature Curve Name}\label{wwhp_eir_inputs_capft}
...
\paragraph{Field: Electric Input to Output Ratio Function of Temperature Curve Name}\label{wwhp_eir_inputs_eirft}
...
\paragraph{Field: Electric Input to Output Ratio Function of Part Load Ratio Curve Name}\label{wwhp_eir_inputs_eirfplr}
...
\begin{lstlisting}
  ...
  HeatPump:WaterToWater:EIR:Cooling,
    ...
    Heating Coil,            !- Companion Coil Name
    0.005,                   !- Load Side Design Volume Flow Rate {m3/s}
    0.003,                   !- Source Side Design Volume Flow Rate {m3/s}
    75000,                   !- Reference Capacity
    4.0,                     !- Reference COP
    ,                        !- Sizing Factor
    CapCurveFuncTemp2,       !- Heating Capacity Function of Temperature Curve Name
    EIRCurveFuncTemp2,       !- Electric Input to Heating Output Ratio Function of Temperature Curve Name
    EIRCurveFuncPLR2;        !- Electric Input to Heating Output Ratio Function of Part Load Ratio Curve Name
\end{lstlisting}
...
\begin{itemize}
\item
  HVAC,Average,EIR WWHP Load Side Heat Transfer {[}W{]}
\item
  HVAC,Average,EIR WWHP Source Side Heat Transfer {[}W{]}
\item
  HVAC,Average,EIR WWHP Power Usage {[}W{]}
\item
  HVAC,Average,EIR WWHP Load Side Inlet Temperature {[}°C{]}
\item
  HVAC,Average,EIR WWHP Load Side Outlet Temperature {[}°C{]}
\item
  HVAC,Average,EIR WWHP Source Side Inlet Temperature {[}°C{]}
\item
  HVAC,Average,EIR WWHP Source Side Outlet Temperature {[}°C{]}
\end{itemize}

\paragraph{EIR WWHP Load Side Heat Transfer {[}W{]}}\label{eir_wwhp_outputs_q_load}

This output variable represents the average load side heat transfer occurring for this coil during the specified output reporting window.

\paragraph{EIR WWHP Source Side Heat Transfer {[}W{]}}\label{eir_wwhp_outputs_q_source}
...
\paragraph{EIR WWHP Power Usage {[}W{]}}\label{eir_wwhp_outputs_power_usage}

This output variable represents the average power usage of this coil during the specified reporting window.

\paragraph{EIR WWHP Load Side Inlet Temperature {[}°C{]}}\label{eir_wwhp_outputs_t_load_in}

This output variable represents the inlet temperature on the load side connection of this coil.

\paragraph{EIR WWHP Load Side Outlet Temperature {[}°C{]}}\label{eir_wwhp_outputs_t_load_out}

This output variable represents the outlet temperature on the load side connection of this coil.

\paragraph{EIR WWHP Source Side Inlet Temperature {[}°C{]}}\label{eir_wwhp_outputs_t_source_in}

This output variable represents the inlet temperature on the source side connection of this coil.

\paragraph{EIR WWHP Source Side Outlet Temperature {[}°C{]}}\label{eir_wwhp_outputs_t_source_out}
