...
\item
  Hybrid Model: Internal Thermal Mass table is only shown when the field \textit{Calculate Zone Internal Mass} is set to YES in the \hyperref[inputs-hm]{HybridModel:Zone} object. This report includes Hybrid Modeling for Internal Mass and Temperature Capacitance Multiplier values for each zone.
\end{itemize}
...
\item
  Exterior Fenestration Shaded State has values shown for exterior fenestration that have a shaded state by using \hyperref[windowpropertyshadingcontrol]{WindowShadingControl}.  Note that for windows without frames/dividers defined only the values of the U-Factor, SHGC, and visible transmittance for the glass only when in the shaded state are shown. For windows with frames/dividers defined, the U-Factor, SHGC, and visible transmittance are shown for both the glass only and the full assembly when in the shaded state.
\item
...
\paragraph{HVAC Topology}\label{hvac-topology}
...
\paragraph{Resilience Summaries}\label{resilience-summaries}
...
\begin{itemize}
\item \textbf{Thermal Resilience Summary}: The Thermal Resilience Summary provides the indoor thermal resilience metrics
unmet hours and occupant$·$hours summary report. This includes: Heat Index
...
\item \textbf{CO2 Resilience Summary}: The CO2 Resilience Summary provides the indoor CO2 level unmet hours,
...
\item \textbf{Visual Resilience Summary}: The Visual Resilience Summary provides the indoor illuminance level unmet hours
...
\item
  Refrigeration:Electricity (SumOrAverage)
\item
...
\item
  Humidifier:Electricity (ValueWhenMaxMin)
\item
  HeatRecovery:Electricity (ValueWhenMaxMin)
\item
  WaterSystems:Electricity (ValueWhenMaxMin)
\item
  Refrigeration:Electricity (ValueWhenMaxMin)
\item
  Cogeneration:Electricity (ValueWhenMaxMin)
\end{itemize}
...
\paragraph{HeatEmissionsMonthly}\label{HeatEmissionsMonthly}
