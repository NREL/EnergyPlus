...
The fraction of full-load driven torque for the belt (\(\tau_{fan) / \tau_{belt,max}\)), which is typically called ``belt load'', is thus \emph{τ\(_{fan}\)} divided by the belt torque capacity (\(\tau_{belt,max}\)). For a particular belt type and cross-section, belt torque capacity can be determined from manufacturer's information such as a shaft speed versus power chart.
