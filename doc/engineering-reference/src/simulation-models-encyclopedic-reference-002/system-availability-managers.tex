...
·~~~~~~~~Thermostat on/off tolerance \emph{T\(_{l}\)} (degrees C);
...
a.~~~~For control types \emph{Cycle On Any} and \emph{Cycle On Any -- Zone Fans Only} the manger looks at each zone served by the air loop and detects whether the zone temperature at the thermostat is greater than the cooling setpoint plus ½\emph{T\(_{l}\)} or less than the heating setpoint minus ½\emph{T\(_{l}\)}. If it is, \emph{AvailStatus} is set to \emph{CycleOn} (or \emph{CycleOnZoneFansOnly}). If not, \emph{AvailStatus} is set to \emph{NoAction}.
...
·~~~~~~~~temperature difference for turning the system on (\emph{DeltaT\(_{n}\)});

·~~~~~~~~temperature difference for turning the system off (\emph{DeltaT\(_{f}\)}).
...
\emph{DeltaT} = \emph{T\(_{e}\)} -- \emph{T\(_{e}\)}

If \emph{DeltaT} \textgreater{} = \emph{DeltaT\(_{n}\)} ~then

\emph{AvailStatus = CycleOn}

~ ~~~~Else if \emph{DeltaT} \textless{} = \emph{DeltaT\(_{f}\)} ~then
...
If \emph{T\(_{e}\)} \textgreater{} = \emph{T\(_{u}\)} then
...
If \emph{T\(_{e}\)} \textless{} = \emph{T\(_{l}\)}~ then
...
\subsection{\texorpdfstring{\textbf{Optimum Start Controls}}{Optimum Start Controls}}\label{optimum-start-controls}
...
Where, on \emph{i}\(^{h}\) day

\emph{t\(_{i}\)} = start/recovery time,

\emph{T\(_{i}\)} = ~~~~~~~~~ zone temperature

\emph{T\(_{i}\)} = ~~~~~~~~~~ outdoor temperature
...
\emph{T\(_{c}\)} and \emph{T\(_{c}\)} are setpoint temperatures during unoccupied (setback) and occupied periods.
...
\emph{t\(_{)}\)} = recovery time on (\emph{i}-1)\(^{h}\) day

\emph{q\(_{)}\)} = ~~~~~~~~~ energy extracted or added during last time step

\emph{q\(_{x}\)} = ~~~~~~~~ maximum capacity of the equipments 
