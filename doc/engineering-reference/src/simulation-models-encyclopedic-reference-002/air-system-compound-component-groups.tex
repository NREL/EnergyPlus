...
\section{Fuel-Fired Air to Water Heat PUmps}

This section describes the EIR formulated model for Fuel-Fired plant loop heat pumps, which is usually configured as either an air-source object. The object is similar to a conventional air-to-water heat pump water heater, except that the heat pump uses absorption cycles driven by fuel combustion, rather than using electricity based vaport compression cycles. The object names for the fuel-fired plant loop heat pumps are HeatPump:AirToWater:FuelFired:Heating and HeatPump:AirToWater:FuelFired:Cooling. In general, these heat pump objects are treated a similar way as the other (air-to-water) heat pump models by the plant loop. When dealing with the heating and cooling modules of the equipment, it used a similar paradigm that an EIR (electric-driven) plant heat pump used: even though a heat pump is generally a single load coil and a single source coil with a reversing valve, in EnergyPlus, the paradigm is to split the operation into two separate coils, a heating and a cooling. It is certainly possible to connect the load side of both of these to a single plant loop if the controls are established properly. The equipment is mainly used for air-source operation, an outdoor air node is used on the source-side inlet node.

The underlying gas-fire absorption heat pump model uses equation-fit models with manufacture provided parameters. The implemented model is as follows: 

\subsection{Equipment Capacity}
The equipment's heating and cooling capacities are derived from rating capacity and a capacity function of tempereatures. 
\begin{equation}
\mathrm{GAHP Heating/Cooling Capacity} = \mathrm{Rated Heating/Cooling Capacity} \times \mathrm{CAPFT}
\end{equation}
...
\subsection{Part Load Ratio}
Partial Load Ratio is the ratio for the actual heating/cooling load over the full capaticy load. In real absorption machines, the PLR is usually between 0.2 and 1.0. 
\begin{equation}
\mathrm{PLR} = \frac{\mathrm{Load}}{\mathrm{Capacity}}, 0.2 \leq \mathrm{PLR} \leq 1
\end{equaiton}

\subsection{Fuel Usage}
\begin{equation}
\mathrm{Fuel Usage} = \frac{\mathrm{Load} \times \mathrm{EIRFT} \times \mathrm{EIRFPLR} \times \mathrm{EIRDEFROST}}{\mathrm{CRF}}
\end{equation}

\subsection{EIR Temperature Correction}
...
\subsection{Defrosting Correction (Heating mode only)}
The defrost fuel usage correction is modeled using the ambient air temperature as the independent varable. The inputs allows a cubic (or lower order) function. For example, one possible curve correlation can look like the following:  
\begin{equation}
...
\subsection{Cycling Ratio Correction}
The cyclying ratio correction is also used for fuel usage calculations. 
\begin{equation}
...
\subsection{Electricty Usage}

The electricity usage for the fuel-fired absorption heat pump is modeled using EIR (Electricity based relation) input curves. The two part of electricity usage is the ``auxiliary electricty'' usage and the ``standby electricity usage'':
\begin{equation}
\mathrm{Electricity usage} = \mathrm{EIR}_{\mathrm{auxiliary}} \times \mathrm{Load} + \mathrm{Electricty}_{\mathrm{Standby}},
\end{equation}
where $\mathrm{EIR}_{\mathrm{auxiliary}}$ is supplied by the curve input and $\mathrm{Electricty}_{\mathrm{Standby}}$ is supplied by the equipment standby electricity input. 
