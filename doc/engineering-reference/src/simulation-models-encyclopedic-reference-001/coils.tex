...
\emph{h\(_{ADP}\)} = enthalpy of saturated air at the coil apparatus dewpoint, J/kg
...
For a given coil geometry, the bypass factor is only a function of air mass flow rate. The model calculates the parameter A\(_{o}\) in equation (460) based on BF\(_{rated}\) and the rated air mass flow rate. With A\(_{o}\) known, the coil BF can be determined for non-rated air flow rates.
...
If the model determines that the cooling coil is dry (ω\(_{in}\) \textless{} ω\(_{ADP}\)), then equations (455) and (456) are invalid since they are functions of entering wet-bulb temperature. Under dry-coil conditions, coil performance is a function of dry-bulb temperature rather than wet-bulb temperature. In this case, the model recalculates the performance of the DX cooling unit using the calculation procedure described above but with ω\(_{in\\ =}\) ω\(_{dry,}\) where ω\(_{dry}\) is the inlet air humidity ratio at the coil dry-out point (SHR = 1.0).
...
Figure~\ref{fig:concepts-of-moisture-buildup-and-evaporation} also shows several parameters that are used in the latent degradation model. The ratio of the coil's moisture holding capacity (M\(_{o}\)) and the steady-state latent capacity (\({\mathop Q\limits^ \bullet_L}\) ) is defined as \emph{t\(_{wet}\)} : the nominal time for moisture to fall from the coil (ignoring transient effects at startup and starting with a dry coil). The ratio of the initial moisture evaporation rate (\({\mathop Q\limits^ \bullet_e}\) ) and the steady-state latent capacity (\({\mathop Q\limits^ \bullet_L}\) ) is defined as \(\gamma\) . Both \emph{t\(_{wet}\)}and \(\gamma\) ~at the rated air volume flow rate and temperature conditions are required model inputs. Two other model inputs are the Maximum ON/OFF Cycling Rate (cycles per hour, \emph{N\(_{max}\)}) and the time constant (\(\tau\) , in seconds) for the cooling coil's latent capacity to reach steady state after startup. The development of the latent degradation model is fully described by Henderson and Rengarajan (1996). The model implemented in EnergyPlus is for their ``linear decay'' evaporation model.

During the simulation, all of the steady-state calculations described previously in equations (449) through (462) are completed. The latent degradation model then modifies the steady-state sensible heat ratio for the coil as shown below. The value of \emph{t\(_{wet}\)} at the current air volume flow rate and entering air conditions is first calculated based on the rated value of \emph{t\(_{wet}\)} entered by the user:
...
\emph{t\(_{wet}\)}~ = nominal time for condensate removal to begin at the current airflow and entering air conditions, starting with a dry coil (sec)

\emph{t\(_{wet,rated}\)} = nominal time for condensate removal to begin at the coil's rated airflow and entering air conditions, starting with a dry coil (sec)
...
\emph{t\(_{wet,max}\)} = maximum allowed value for \emph{t\(_{wet}\)} (9999.0 sec)
...
\emph{T\(_{db,i}\)} = dry-bulb temperature of the air entering the cooling coil, °C

\emph{T\(_{wb,i}\)} = wet-bulb temperature of the air entering the cooling coil, °C

\emph{T\(_{db,rated}\)} = dry-bulb temperature of air entering the cooling coil at rated conditions (26.7°C)

\emph{T\(_{wb,rated}\)} = wet-bulb temperature of air entering the cooing coil at rated conditions (19.4°C)
...
\emph{t\(_{on}\)} = duration of cooling coil on-cycle (sec)

\emph{N\(_{max}\)} = maximum on/off cycles per hour (cph)

\emph{X} = cooling coil runtime fraction (-)

\emph{t\(_{off}\)} = duration of cooling coil off-cycle (sec)
...
\emph{t\(_{on,heating}\)} = duration of heating coil on-cycle (sec)

\emph{X\(_{,heating}\)} = heating coil runtime fraction (-)

\emph{t\(_{off,heating}\)} = duration of heating coil off-cycle (sec)
...
\emph{PLF\(_{0.5}\)} = Part Load Fraction Correlation Curve evaluated at a part load ratio (PLR) of 0.5 (dimensionless)

\emph{EER\(_{TestB}\)} = Energy efficiency ratio with 19.44°C wet-bulb temperature air entering the cooling coil, 27.78°C dry-bulb temperature air entering the air-cooled (outdoor) condenser, and rated air volume flow through the cooling coil (W/W)
...
Q\(_{coil,latent}\)~~~~~~~ = effective latent cooling capacity {[}W{]}
...
\emph{q\(_{c}\)}(T\(_{j}\))/N = ~ = the ratio of space cooling capacity provided by the unit during periods of the space cooling season when the outdoor temperature fell within the range represented by bin temperature \emph{T\(_{j}\)} to the total number of hours in the cooling season (N), (W)

\emph{e\(_{c}\)}(T\(_{j}\))/N = the ratio of the electrical energy consumed by the unit during periods of the space cooling season when the outdoor temperature fell within the range represented by bin temperature Tj to the total number of hours in the cooling season (N), W.

\emph{j} ~~~ ~ = the bin number, dimensionless. For cooling season j ranges from 1 to 8.

\emph{T\(_{j}\)}~~~ = ~~~~~~~~ outdoor air bin temperature, °C. Outdoor temperatures are ``binned'' such that calculations are only performed based one temperature within the bin. Bins of 2.8 °C with 8 cooling season bin temperatures being 19.44°C, 22.22°C, 25.0°C, 27.78°C, 30.56°C, 33.33°C, 36.11°C, 38.89°C.

The steady-state cooling capacity delivered and the electric power inputs when the DX coil is operating at minimum compressor speed (\emph{k = 1}), and outdoor air temperature \emph{T\(_{j}\)}, are determined by linear interpolation using the values of B1 and F1 tests as follows:
...
The steady-state cooling capacities and electric power inputs at the minimum speed and test condition \emph{B\(_{1}\)} and \emph{F\(_{1}\)} are calculated from the minimum (low) speed performance curves as follows:
...
The steady-state cooling capacity delivered and the electric power inputs when the DX cooling coil is operating at maximum (high) compressor speed (\emph{k = 2}), and outdoor air temperature \emph{T\(_{j}\)}, are determined as follows:
...
The steady-state cooling capacities and electric power input at the maximum speed and test condition \emph{A\(_{2}\)} and \emph{B\(_{2}\)} are determined from the maximum (high) speed performance curves as follows:
...
\(CAPFT_{TestA2}^{k = 2}\) ~ = cooling capacity modifier curve for temperature at maximum (high) compressor speed and \emph{A}\(_{2}\) test condition, (-)
...
\emph{BL(T\(_{j}\))}~~~~~~~~~ = the building space cooling load corresponding to outdoor temperature of \emph{T\(_{j}\)}, (W).
...
\emph{Case 3}:~ The steady-state cooling capacity when the unit is operating continuously at maximum (high) compressor speed capacity at outdoor air temperature \emph{T\(_{j}\)}. That is the building cooling load is greater or equal to the available capacity at maximum (high) compressor speed:

\begin{equation}
BL({T_j}) \ge \dot Q_c^{k = 2}({T_j})
\end{equation}

For units when operating continuously at maximum (high) compressor speed (k = 2) at temperature \emph{T\(_{j}\)}, the delivered cooling capacity and electric power inputs are calculated as follows:
...
\toprule 
Source: AHRI Standard 210-240, 2008; Table 5, Page 65 \tabularnewline
\midrule
\endfirsthead
...
Test Description & Air Entering Indoor Unit Temperature (F / C) &  & Air Entering Outdoor Unit Temperature (F / C) &  &  &  \tabularnewline
 & Dry Bulb & Wet Bulb & Dry Bulb & Wet Bulb & Compressor Capacity & Cooling Air Volume Rate \tabularnewline
...
\subsection{Gas Air Heating Coil}\label{gas-air-heating-coil}
...
\emph{h\(_{outlet,full\_Stage\_n-1}\)} = specific enthalpy of the coil outlet air during full-load operation at Stage n-1 (no cycling) {[}J/kg{]}
...
T\(_{off}\)~ = ~~~~~~~~ the outdoor temperature when the compressor is automatically shut off, °C. (If no such temperature exists, Tj is always greater than T\(_{off}\) and T\(_{on}\)).

T\(_{on}\)~ = ~~~~~~~~ the outdoor temperature when the compressor is automatically turned back on, if applicable, following an automatic shut-off, °C.
