...
\emph{HR\(_{n}\)~~~~~~ =} enthalpies evaluated at a constant humidity ratio, the minimum humidity ratio of the unitary system exiting air or the air in the control zone

\emph{Mass Flow Rate\(_{f}\)} = air mass flow rate through the unitary system with the cooling coil OFF, kg/s
...
where: {\$\mathop m\limits\^{} \bullet \(_{N}\)\(</span>  = air mass flow rate through unitary system when the cooling coil is ON (kg/s) <span>\)\mathop m\limits\^{} \bullet \(_{F}\)\$}~ = air mass flow rate through unitary system when no cooling or heating is needed (kg/s)
...
\[FullCoolOutpu{t_{Highest\;Speed}} = ({\dot m_{HighestSpeed}}){({h_{out,full\;load}} - {h_{control\;zone}})_{HR\min }} - {\Delta_{sen,\;HighestSpeed}}\] \[NoCoolOutput = ({\dot m_{CoilOff}}){({h_{out,\;coil\;off}} - {h_{control\;zone}})_{HR\min }} - {\Delta_{sen,\;coil\;off}}\]
...
\emph{HR\(_{n}\) =} the minimum humidity ratio of the unitary system exiting air or the air leaving the control zone {[}kg/kg{]}
...
\emph{UnitarySystemOutput\(_{o}\)} = unitary system delivered sensible capacity between two consecutive speeds at a specific ratio {[}W{]}
...
\{SR\_\{n\}\} = speed ratio of the water coil unitary system at speed n, (-)
...
\{FullLoadOutput\_\{n-1\}\} = fully load system output at fan speed level n-1, (W)

\{FullLoadOutput\_\{n\}\} = fully load system output at fan speed level n, (W)
...
\emph{h\(_{l}\)}\(_{ }\) = enthalpy of air exiting the furnace (J/kg)
...
\({\dot V_{mode}}\) ~~~~~~~~~~~~~~~ = User-specified system volumetric flow rate in cooling, heating, or no cooling or heating mode, m\(^{3}\)/s
...
\begin{array}{rl}
    {\mathop m\limits^ \bullet_{water_removed,Proc}} & = \,{\mathop m\limits^ \bullet_{Proc,in}}\left( {{w_{Proc,\,\,in}} - {w_{Proc,\,\,out}}} \right) \\
    {m_{water_removed,Proc}}                         & = {\mathop m\limits^ \bullet_{water_removed,Proc}}*\,\,TimeStepSys\,\,*\,\,3600.
  \end{array}
