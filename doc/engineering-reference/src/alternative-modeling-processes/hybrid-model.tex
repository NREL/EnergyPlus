...
\begin{equation}
\text{Infiltration} = \text{Infiltration}_\text{design} F_\text{schedule} [A+B|T_z -T_o|+C (\text{WindSpeed})+D(\text{Windspeed}^2 )]
\end{equation}
...
\begin{equation}
T_z^t = \\ \frac {\Sigma Q_i +\Sigma(h_i A_i T_{si}) + \Sigma (\dot{m}_i C_p T_{zi}) + \dot{m}_{inf} C_p T_o + \dot{m}_{sys} C_p T_{sup}^t - \frac {C_z} {\delta t}(-3T_z^{t-\delta t} + \frac {3} {2} T_z^{t-2\delta t} - \frac {1} {3} T_z^{t-3\delta t})} { \frac {11} {6} \frac {C_z} {\delta t} + \Sigma (h_i A_i)  + \Sigma (\dot{m}_i C_p) + \dot{m}_{inf} C_p + \dot{m}_{sys} C_p}
\end{equation}
...
\begin{equation}
\dot{m}_{inf} C_p = \frac {\Sigma Q_i + \Sigma (h_i A_i T_{si}) + \Sigma (\dot{m}_i C_p T_{zi}) + \dot{m}_{sys} C_p T_{sup}^t - \frac {C_z} {\delta t} (-3T_z^{t-\delta t} + \frac {3} {2} T_z^{t-2\delta t} - \frac {1} {3} T_z^{t-3\delta t}) - T_z^t [ \frac {11} {6} \frac {C_z} {\delta t} + \Sigma (h_i A_i) + \Sigma (\dot{m}_i C_p) + \dot{m}_{sys} C_p]} {T_z^t - T_o}
\end{equation}
