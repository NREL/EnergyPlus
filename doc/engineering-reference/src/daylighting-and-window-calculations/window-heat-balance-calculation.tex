...
Openness can be determined by optical measurement at normal incidence, A\(_{o}\) = {$\tau}\(_{bb}\) ($\theta = 0$), but in the case of insect screens A\(_{o}\) can reliably be calculated knowing wire diameter (\emph{d}), and wire spacing (\emph{s}) as follows:
...
\emph{$\tau$\(_{k}\)~~~ = ~~~~~~~~~} effective infrared transmittance of layer k (\emph{$\tau$\(_{1}\)} = 1)
