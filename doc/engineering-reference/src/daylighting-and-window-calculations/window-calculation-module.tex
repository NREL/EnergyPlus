...
\caption{Variables in Window Calculations \label{table:variables-in-window-calculations}}\\
\toprule 
\textbf{Mathematical variable} & \textbf{Description} & \textbf{Units} & \textbf{FORTRAN \quad variable} \tabularnewline \midrule \endhead
T & Transmittance & - & - \tabularnewline \midrule
R & Reflectance & - & - \tabularnewline \midrule
R\(^{f}\), R\(^{b}\) & Front reflectance, back reflectance & - & - \tabularnewline \midrule
T\(_{i,j}\) & Transmittance through glass layers i to j & - & - \tabularnewline \midrule
T\(^{dir}_{gl}\) & Direct transmittance of glazing & - & - \tabularnewline \midrule
R\(^{f}_{i,j}\), R\(^{b}_{i,j}\) & Front reflectance, back reflectance from glass layers i to j & - & - \tabularnewline \midrule
R\(^{dir}_{gl,f}\), R\(^{dir}_{gl,b}\) & Direct front and back reflectance of glazing & - & - \tabularnewline \midrule
A\(^{f}_{i}\), A\(^{b}_{i}\) & Front~ absorptance, back absorptance of layer i & - & - \tabularnewline \midrule
N & Number of glass layers & - & Nlayer \tabularnewline \midrule
$\lambda$ & Wavelength & microns & Wle \tabularnewline \midrule
E\(_{s}\) ($\lambda$) & Solar spectral irradiance function & W/m\(^{2}\)-micron & E \tabularnewline \midrule
V($\lambda$) & Photopic response function of the eye & - & y30 \tabularnewline \midrule
$\varphi$ & Angle of incidence (angle between surface normal and direction of incident beam radiation) & Rad & Phi \tabularnewline \midrule
$\tau$ & Transmittivity or transmittance & - & tf0 \tabularnewline \midrule
$\rho$ & Reflectivity or reflectance & - & rf0, rb0 \tabularnewline \midrule
$\alpha$ & Spectral absorption coefficient & m\(^{-1}\) & - \tabularnewline \midrule
d & Glass thickness & M & Material\%Thickness \tabularnewline \midrule
n & Index of refraction & - & ngf, ngb \tabularnewline \midrule
$\kappa$ & Extinction coefficient & - & - \tabularnewline \midrule
$\beta$ & Intermediate variable & - & betaf, betab \tabularnewline \midrule
P, p & A general property, such as transmittance & - & - \tabularnewline \midrule
$\tau$\(_{sh}\) & Shade transmittance & - & Material\%Trans \tabularnewline \midrule
$\rho$\(_{sh}\) & Shade reflectance & - & Material\%ReflectShade \tabularnewline \midrule
$\alpha$\(_{sh}\) & Shade absorptance & - & Material\%AbsorpSolar \tabularnewline \midrule
$\tau$\(_{bl}\) $\rho$\(_{bl}\) $\alpha$\(_{bl}\) & Blind transmittance, reflectance, absorptance & - & - \tabularnewline \midrule
Q, G, J & Source, irradiance and radiosity for blind optical properties calculation & W/m\(^{2}\) & - \tabularnewline \midrule
F\(_{ij}\) & View factor between segments i and j & - & - \tabularnewline \midrule
f\(_{switch}\) & Switching factor & - & SwitchFac \tabularnewline \midrule
R($\lambda$) & Photopic response function of the eye & - & y30 \tabularnewline \midrule
$\varphi$' & Relative azimuth angle (angle between screen surface normal and vertical plane through sun, Ref. Figure 87) & Rad & SunAzimuthToScreenNormal \tabularnewline \midrule
$\alpha$' & Relative altitude angle (angle between screen surface horizontal normal plane and direction of incident beam radiation, Ref. Figure 87) & Rad & SunAltitudeToScreenNormal \tabularnewline \midrule
$\rho$\(_{sc}\) & Beam-to-diffuse solar reflectance of screen material & - & Screens\%ReflectCylinder \tabularnewline \midrule
$\gamma$ & Screen material aspect ratio & - & Screens\%ScreenDiameterTo SpacingRatio \tabularnewline \midrule
Α & Spectral absorption coefficient & m\(^{-1}\) & - \tabularnewline \midrule
D & Glass thickness & M & Material\%Thickness \tabularnewline \midrule
N & Index of refraction & - & ngf, ngb \tabularnewline \midrule
Κ & Extinction coefficient & - & - \tabularnewline \midrule
Β & Intermediate variable & - & betaf, betab \tabularnewline \midrule
P, p & A general property, such as transmittance & - & - \tabularnewline \midrule
\bottomrule
\end{longtable}
...
\begin{array}{l}r = {\left( {\frac{{1.526 - 1}}{{1.526 + 1}}} \right)^2}\\\tau  = 0.93974\\R = 0.07846\end{array}
...
\caption{Diagram of Transmittance and Reflectance Correlations Used based on U and SHGC \protect \label{fig:diagram-of-transmittance-and}}
\end{figure}
...
If a glazing layer has optical properties that are roughly constant with wavelength, the wavelength-dependent values of \emph{T\(_{i}\)} , \emph{R\(^{f}\)\(_{i}\)} and \emph{R\(^{b}\)\(_{i}\)} in Eqs. to can be replaced with constant values for that layer.
...
\toprule 
\({\tau_{dir,dir}}\) & Direct-to-diffuse transmittance (same for front and back of slat) \tabularnewline \midrule
\({\tau_{dif,dir}}\) & Diffuse-to-diffuse transmittance (same for front and back of slat) \tabularnewline \midrule
\(\rho_{dir,dif}^f\), \(\rho_{dir,dif}^b\) & Front and back direct-to-diffuse reflectance \tabularnewline \midrule
...
The edge correction factor for diffuse incident radiation is calculated by averaging this value of \emph{f\(_{e}\)} over profile angles, \emph{φ\(_{s}\)}, from --90\(^{O}\) to +90\(^{O}\).
...
The second component of the window screen transmittance model is an empirical algorithm that accounts for the inward reflection of incident beam radiation off the screen material surface. The calculation procedure for the screen's transmittance via beam reflection, \emph{T\(_{t}\)} (α', φ') is as follows:
...
\begin{array}{rl}
IF \left( \delta \gt \delta_{max} \right) \; T_{scatt} \left( \alpha ',\varphi ' \right) =& 0.2 \rho_{sc} T_{scattmax} \left( 1 - \gamma \right) \left( 1 + \left( Peak_{ratio} - 1 \right) e^{\frac{- \left| \delta - \delta_{max} \right|^{2.5}}{600}} \right) \\
&- 0.2 \rho_{sc} T_{scattmax} \left( 1 - \gamma \right) \left( \max \left( 0.0, \frac{\delta  - \delta_{max}}{90. - {delta_{max}}} \right) \right) \\
IF \left( \delta \gt \delta_{max} \right) \; T_{scatt}^{vis} \left( \alpha ',\varphi ' \right) =& 0.2 \rho_{sc}^{vis} T_{scattmax}^{vis} \left( 1 - \gamma \right) \left( 1 + \left( Peak_{ratio}^{vis} - 1 \right) e^{\frac{- \left| \delta - \delta_{max} \right|^{2.5}}{600}} \right) \\
...
\begin{array}{l}T_{sys}^{dir,all}(\alpha ',\varphi ',\phi ) = T_{sc}^{dir,dir}(\alpha ',\varphi ')T_{gl}^{dir}\left( \phi  \right) + (T_{sc}^{dir,dir}(\alpha ',\varphi ')R_{gl,f}^{dir}R_{sc}^{dir,dif}(\alpha ',\varphi ') + T_{sc}^{dir,dif}(\alpha ',\varphi '))T_{gl}^{dif}\sum\limits_{n = 0}^\infty  {{{(R_{gl,f}^{dif}R_{sc}^{dif,dif})}^n}} \\\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\; - or - \end{array}
...
Substituting equation for \emph{S}\(^{)}\)(\textbf{\emph{p}}\(^{)}\), we obtain a propagation equation for outside radiation passing through the window and arriving at surface element 2:
...
In this equation we have dropped the explicit time dependence, but \textbf{\emph{p}}\(^{)}\), \emph{q}\(^{)}\), \emph{I}\(^{)}\), and \emph{I}\(^{)}\) are time-varying, while DW\(^{)}\) is simply the constant angular size of the sun.
...
\caption{Determination of array sizes \label{table:determination-of-array-sizes}}\\
\toprule 
\multicolumn{2}{c}{Parameters} \tabularnewline \midrule
N\(_{Basis}\) & Number of elements in the (incoming or outgoing) basis \tabularnewline \midrule
N\(_{Sun}\) & Number of basis directions that may be sun directions (depends on fenestration orientation \tabularnewline \midrule
N\(^{(Gnd)}_{Sun}\) & Number of sun directions that give significantly different ground irradiation conditions, as seen by fenestration \tabularnewline \midrule
N\(_{Sf}\) & Number of reflecting surfaces viewable by fenestration (depends on fenestration orientation) \tabularnewline \midrule
N\(^{(Sf,n)}_{Sun}\) & Number of time steps for which surface \emph{n} is sunlit (depends on orientation of surface \emph{n}; determined during shading calculation) \tabularnewline \midrule
N\(^{(Sf,n)}_{Refl}\) & Number of basis directions that may be reflected sun directions from surface \emph{n} (depends on orientation of fenestration and surface \emph{n}). \tabularnewline \midrule
N\(_{IntSurf}\) & Number of interior surfaces in the zone containing the fenestration \tabularnewline \midrule
N\(_{Layers}\) & Number of thermal layers in the fenestration system \tabularnewline \midrule
\multicolumn{2}{c}{Arrays} \tabularnewline \midrule
A\(^{F,l}_{i}\), A\(^{B,l}_{i}\) & Absorptance vector element; N\(_{Basis}\) \tabularnewline \midrule
T\(_{ij}\) & Transmittance matrix element; N\(_{Basis}\) X N\(_{Basis}\) \tabularnewline \midrule
V\(^{(Sky)}_{i}\) & Sky viewed fraction; one-dimensional, N\(_{Basis}\) \tabularnewline \midrule
V\(^{(Sf,n)}_{i}\) & Fraction of surface \emph{n} viewed; N\(_{Basis}\) X N\(_{Sf}\) \tabularnewline \midrule
$\rho$\(^{(n)}\), $\rho$\(^{(sp,n)}\) & Surface \emph{n} diffuse, specular reflectance; N\(_{Sf}\) (already stored by E+) \tabularnewline \midrule
U\(^{(D,n)}_{i\,Sun(tsh)}\) & Fraction of the image of $\Delta\Omega$\(_{i}\) on surface \emph{n} that views the sun when it is in direction Sun(tsh); N\(_{Basis}\) X N\(_{Sf}\) X N\(^{(Sf,n)}_{Sun}\) \tabularnewline \midrule
V\(^{(Gnd)}_{i}\) & Fraction of $\Delta\Omega$\(_{i}\) that views ground; N\(_{Basis}\) \tabularnewline \midrule
U\(^{(Sky, Gnd)}_{i}\) & Fraction of sky radiation received by the image of $\Delta\Omega$\(_{i}\) on the ground; N\(_{Basis}\) \tabularnewline \midrule
U\(^{(D,Gnd)}_{i\,Sun(tsh)}\) & Fraction of direct solar radiation for sun direction Sun(tsh) received by image of $\Delta\Omega$\(_{i}\) on ground; N\(^{Gnd}_{Sim}\) X N\(_{Basis}\) \tabularnewline \midrule
V\(^{(D)}_{i\,s(t)}\) & Fraction of fenestration area irradiated by direct solar radiation for direction \emph{i}, given that sun angle is s(\emph{t}); N\(_{Sun}\) X N\(_{Basis}\) \tabularnewline \midrule
F\(^{(k)}_{j}\) & Fraction of radiation in direction \emph{j} leaving fenestration interior that arrives at surface \emph{k}; N\(_{Basis}\) X N\(_{IntSurf}\) \tabularnewline \midrule
Z\(^{(Sky), k}\) & Sky irradiation factor; N\(_{IntSurf}\) \tabularnewline \midrule
Z\(^{(sp,Sf,n),k}_{r(t)}\) & Exterior surface specular irradiation factor; N\(^{(Sf,n)}_{Sun}\) X N\(_{Sf}\) X N\(_{IntSurf}\) \tabularnewline \midrule
Z\(^{(Sun,Sf,n),k}_{s(t)}\) & Exterior surface direct-diffuse irradiation factor; N\(^{(Sf,n)}_{Sun}\) X N\(_{Sf}\) X N\(_{IntSurf}\) \tabularnewline \midrule
Z\(^{(Sky,Sf,n),k}\) & Exterior surface sky irradiation factor; N\(_{Sf}\) X N\(_{IntSurf}\) \tabularnewline \midrule
Z\(^{(D,Gnd),k}_{s(t)}\) & Ground-reflected direct solar irradiaton factor (given sun direction s(\emph{t})); N\(^{(Gnd)}_{Sun}\) X N\(_{IntSurf}\) \tabularnewline \midrule
Z\(^{(Sky,Gnd),k}\) & Ground-reflected diffuse solar irradiation factor; N\(_{IntSurf}\) \tabularnewline \midrule
Z\(^{(Sun),k}_{s(t)}\) & Direct solar irradiation factor; N\(_{Sun}\) X N\(_{IntSurf}\) \tabularnewline \midrule
K\(^{(Sky),l}\) & Sky absorption factor; N\(_{Layers}\) \tabularnewline \midrule
K\(^{(sp,Sf,n),l}_{r(t)}\) & Exterior surface specular absorption factor; N\(_{Sf}\) X N\(^{(Sf,n)}_{Refl}\) X N\(_{Layers}\) \tabularnewline \midrule
K\(^{(Sun,Sf,n),l}_{s(t)}\) & Exterior surface diffusely reflected direct sun absorption factor; N\(_{Sf}\) X N\(^{(Sf,n)}_{Sun}\) X N\(_{Layers}\) \tabularnewline \midrule
K\(^{(Sky,Sf,n),l}\) & Exterior surface reflected sky radiation absorption factor; N\(_{Sf}\) X N\(_{Layers}\) \tabularnewline \midrule
K\(^{(D,Gnd),l}_{s(t)}\) & Ground-reflected direct solar absorption factor; N\(^{(Gnd)}_{Sun}\) X N\(_{Layers}\) \tabularnewline \midrule
K\(^{(Sky,Gnd),l}\) & Ground-reflected sky radiation absorption factor; N\(_{Layers}\) \tabularnewline \midrule
