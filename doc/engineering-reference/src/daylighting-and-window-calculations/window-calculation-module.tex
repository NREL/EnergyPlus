...
There may not be exact agreement between the performance indices echoed out and
those input in the model. This is expected with the model and the result of a
number of factors. For example, when there is conflict between the SHGC and the
U that are not physical and compromises need to be made. In versions up till
9.5.0, the reported U value is limited to no higher than about 5.8W/m\(^{2}\)\(\cdot\)K
when input is allowed to go up to U-7 W/m\(^{2}\)\(\cdot\)K. In later versions,
this mis-match of the input and the reported U-factors among exterior windows
are resolved with the application of an adjustment ratio. The adjustment ratio
is computed as a ratio between the input U and the initially computed nominal (or
effective) U in function CalcNominalWindowCond. In general, the simple window
model is intended to generate a physically-reasonable glazing that approximates
the input entered as well as possible. But the model is not always be able to
