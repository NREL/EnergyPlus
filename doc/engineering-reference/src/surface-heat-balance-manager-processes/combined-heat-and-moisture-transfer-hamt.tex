...
The moisture content of the material w and the vapor diffusion resistance
...
Equation~\ref{eq:HAMTMoistureBalanceEquationDiscreteCell} can be rearranged
...
This is simply the gradient of moisture sorption isotherm at the RH of the
material.
...
As an illustration of the use of the Heat and Moisture Transfer (HAMT)
...
\subsubsection{User-Defined Constant Airflow }
...
(\IP{0.0038}{\CFM\per\ft\squared}) for very airtight buildings to
\SI{0.33}{\meter\cubed\per\hour\per\meter\squared}
(\IP{0.018}{\CFM\per\ft\squared}) for buildings without requirements on their
...
(\IP{0.00022}{\CFM\per\ft\squared\per\pascal}) for very airtight buildings
to \SI{0.06}{\meter\cubed\per\meter\squared\per\hour\per\pascal}
(\IP{0.0033}{\CFM\per\ft\squared\per\pascal}) for standard construction.
