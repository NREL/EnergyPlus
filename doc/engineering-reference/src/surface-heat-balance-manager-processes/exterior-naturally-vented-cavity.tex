...
\(q''_{conv,cav}\) ~is modeled using the classical formulation: \(q''_{conv} = h_{cp}(T_{air} - T_{o))\) ~ where h\(_{p}\), is the convection coefficient.~ The value for h\(_{p}\) is obtained from correlations used for window gaps from ISO (2003) standard 15099.
