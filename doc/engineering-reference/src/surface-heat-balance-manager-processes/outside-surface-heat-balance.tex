...
\caption{Nomenclature List of Variables \label{table:nomenclature-list-of-variables}}\\
\toprule 
\textbf{Mathematical variable} & \textbf{Description} & \textbf{Units} & \textbf{Range} \tabularnewline \midrule \endhead
$q''_{LWR}$ & Exterior surface longwave radiation flux & $W/m^2$ & --- \tabularnewline \midrule
$h_r$ & Linearized radiative heat transfer coefficient to air temperature & $W/m^2.K$ & --- \tabularnewline \midrule
$T_{surf}$ & Surface Outside face temperatures & $K$ & --- \tabularnewline \midrule
$T_{air}$ & Outside air temperature & $K$ & --- \tabularnewline \midrule
$T_{gnd}$ & Environmental ground surface temperature & $K$ & --- \tabularnewline \midrule
$T_{sky}$ & Sky Effective temperature & $K$ & --- \tabularnewline \midrule
$F_{gnd}$ & view factor of wall surface to ground surface & --- & 0--1 \tabularnewline \midrule
$F_{sky}$ & View factor of wall surface to sky & --- & 0--1 \tabularnewline \midrule
$F_{air}$ & View factor of wall surface to air & --- & 0--1 \tabularnewline \midrule
$\varepsilon$ & Surface long-wave emissivity & --- & 0--1 \tabularnewline \midrule
...
F\(_{d}\) = view factor of wall surface to ground surface temperature

F\(_{y}\) = view factor of wall surface to sky temperature

F\(_{r}\) = view factor of wall surface to air temperature

T\(_{f}\) = outside surface temperature

T\(_{d}\) = ground surface temperature

T\(_{y}\) = sky temperature

T\(_{r}\) = air temperature
...
where \emph{$\phi$} is the tilt angle of the surface.~ The view factor to the sky is further split between sky and air radiation by:
...
\caption{  Atmospheric Variables at Two Different Altitudes above Ground Level \protect \label{table:atmospheric-variables-at-two-different}}\\
\toprule 
...
\caption{Wind Speed Profile Coefficients (ASHRAE Fundamentals 2005). \label{table:wind-speed-profile-coefficients-ashrae}}\\
\toprule
\textbf{Terrain Description}       & \textbf{Exponent, $\alpha$} & \textbf{Boundary Layer Thickness, $\delta$ (m)} \tabularnewline
\midrule
\endhead
Flat, open country        & 0.14 & 270 \tabularnewline \midrule
Rough, wooded country     & 0.22 & 370 \tabularnewline \midrule
Towns and cities          & 0.33 & 460 \tabularnewline \midrule
Ocean                     & 0.10 & 210 \tabularnewline \midrule
...
% table 6
\begin{longtable}[c]{@{}lllll@{}}
\caption{  Roughness Coefficients D, E, and F. \protect \label{table:roughness-coefficients-d-e-and-f.}}\\
\toprule 
\textbf{Roughness Index} & \textbf{D} & \textbf{E} & \textbf{F} & \textbf{Example Material} \tabularnewline \midrule
\endhead
1 (Very Rough) & 11.58 & 5.894 & 0.0 & Stucco \tabularnewline \midrule
2 (Rough) & 12.49 & 4.065 & 0.028 & Brick \tabularnewline \midrule
3 (Medium Rough) & 10.79 & 4.192 & 0.0 & Concrete \tabularnewline \midrule
4 (Medium Smooth) & 8.23 & 4.0 & -0.057 & Clear pine \tabularnewline \midrule
5 (Smooth) & 10.22 & 3.1 & 0.0 & Smooth Plaster \tabularnewline \midrule
...
\caption{Nomenclature List of Variables. \label{table:nomenclature-list-of-variables.-001}}\\
\toprule 
\textbf{Variable} & \textbf{Description} & \textbf{Units} & \textbf{Range} \tabularnewline \midrule \endhead
A & Surface area of the surface & m\(^{2}\) & /=0 \tabularnewline \midrule
h\(_{c}\) & Surface exterior convective heat transfer coefficient & W/(m\(^{2}\)K) & - \tabularnewline \midrule
h\(_{f}\) & Forced convective heat transfer coefficient & W/(m\(^{2}\)K) & - \tabularnewline \midrule
h\(_{n}\) & Natural convective heat transfer coefficient & W/(m\(^{2}\)K) & - \tabularnewline \midrule
P & Perimeter of surface & m & - \tabularnewline \midrule
R\(_{f}\) & Surface roughness multiplier & - & - \tabularnewline \midrule
T\(_{air}\) & Local outdoor air temperature calculated at the height above ground of the surface centroid & °C & - \tabularnewline \midrule
T\(_{so}\) & Outside surface temperature & °C & - \tabularnewline \midrule
$\Delta$T & Temperature difference between the surface and air, & °C & - \tabularnewline \midrule
V\(_{z}\) & Local wind speed calculated at the height above ground of the surface centroid & m/s & - \tabularnewline \midrule
W\(_{f}\) & Wind direction modifier & - & - \tabularnewline \midrule
$\phi$ & Angle between the ground outward normal and the surface outward normal & degree & - \tabularnewline \midrule
...
\caption{  Surface Roughness Multipliers (Walton 1981). \protect \label{table:surface-roughness-multipliers-walton-1981.}}\\
\toprule 
\textbf{Roughness Index} & \textbf{Rf} & \textbf{Example Material} \tabularnewline \midrule
\endhead
1 (Very Rough) & 2.17 & Stucco \tabularnewline \midrule
2 (Rough) & 1.67 & Brick \tabularnewline \midrule
3 (Medium Rough) & 1.52 & Concrete \tabularnewline \midrule
4 (Medium Smooth) & 1.13 & Clear pine \tabularnewline \midrule
5 (Smooth) & 1.11 & Smooth Plaster \tabularnewline \midrule
...
\caption{Nomenclature List of Variables. \label{table:nomenclature-list-of-variables.-002}}\\
\toprule 
\textbf{Variable} & \textbf{Description} & \textbf{Units} & \textbf{Range} \tabularnewline \midrule \endhead
A & Constant & W/(m\(^{2}\)K(m/s)\(^{b}\) & - \tabularnewline \midrule
B & Constant & - & - \tabularnewline \midrule
C\(_{t}\) & Turbulent natural convection constant & W/(m\(^{2}\)K\(^{4/3}\)) & - \tabularnewline \midrule
h\(_{c}\) & Surface exterior convective heat transfer coefficient & W/(m\(^{2}\)K) & - \tabularnewline \midrule
T\(_{so}\) & Outside surface temperature & °C/K & - \tabularnewline \midrule
...
\caption{MoWiTT Coefficients (Yazdanian and Klems 1994, Booten et al. 2012) \protect \label{table:mowitt-coefficients-yazdanian-and-klems-1994}}\\
\toprule 
\textbf{Wind Direction} & \textbf{C\(_{t}\)} & \textbf{a} & \textbf{b} \tabularnewline \midrule
\endhead
...
% table 11
\begin{longtable}[c]{p{0.5in}p{3.0in}p{1.5in}p{1.0in}}
\caption{  Nomenclature List of Variables. \label{table:nomenclature-list-of-variables.-003}}\\
\toprule 
\textbf{Variable} & \textbf{Description} & \textbf{Units} & \textbf{Range} \tabularnewline \midrule \endhead
a & Constant & W/(m\(^{2}\)K(m/s)\(^{b}\) & - \tabularnewline \midrule
b & Constant & - & - \tabularnewline \midrule
h\(_{c}\) & Surface exterior convective heat transfer coefficient & W/(m\(^{2}\)K) & - \tabularnewline \midrule
h\(_{c,glass}\) & Convective heat transfer coefficient for very smooth surfaces (glass) & W/(m\(^{2}\)K) & - \tabularnewline \midrule
h\(_{n}\) & Natural convective heat transfer coefficient & W/(m\(^{2}\)K) & - \tabularnewline \midrule
R\(_{f}\) & Surface roughness multiplier & - & - \tabularnewline \midrule
T\(_{so}\) & Outside surface temperature & °C/K & - \tabularnewline \midrule
$\Delta$T & Temperature difference between the surface and air, & °C/K & - \tabularnewline \midrule
$\Phi$ & Angle between the ground outward normal and the surface outward normal & radian & - \tabularnewline \midrule
\bottomrule
\end{longtable}
...
\caption{Adaptive Convection Algorithm Details \label{table:adaptive-convection-algorithm-details}}\\
\toprule 
\textbf{\#} & \textbf{Surface Classification} & \textbf{Heat Flow Direction} & \textbf{Wind Direction} & \textbf{h Models} & \textbf{h Models} \tabularnewline \midrule \endhead
1 & Roof Stable & Down & Any & TARPWindward MoWiTTWindward DOE2Windward NusseltJurges BlockenWindward EmmelRoof ClearRoof & WaltonStableHorizontalOrTilt AlamdariStableHorizontal \tabularnewline \midrule
2 & Roof Unstable & Up & Any & TARPWindward MoWiTTWindward DOE2Windward NusseltJurges BlockenWindward EmmelRoof ClearRoof & WaltonUnstableHorizontalOrTilt AlamdariUnstableHorizontal \tabularnewline \midrule
3 & Vertical Wall Windward & Any & Windward & TARPWindward DOE2Windward MoWiTTWindward NusseltJurges McAdams Mitchell BlockenWindward EmmelVertical & ASHRAEVerticalWall AlamdariHammondVerticalWall FohannoPolidoriVerticalWall ISO15099Windows \tabularnewline \midrule
...
Where \emph{V\(_{m}\)} is the air velocity at the location of the weather station and $\theta$ is the angle of incidence between the wind and the surface in degrees.~ The following equations are used for horizontal (roof) surfaces (key: EmmelRoof):
