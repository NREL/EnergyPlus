...
For no temperature difference OR a vertical surface, the TARP algorithm used:

\begin{equation}
h = 1.31{\left| {\Delta T} \right|^{\frac{1}{3}}}
\end{equation}

Typically, a vertical surface would have a cosine of the tilt angle that yields zero.  However, with numerical round-off, a zero cosine is unlikely.  Rather than apply a computationally expensive absolute value check against a small number frequently throughout a run, EnergyPlus chooses to simply treat the vertical surface like reduced convection shown below.  The result for h is essentially the same for the reduced convection and vertical surface correlation when the cosine of the tilt is zero. It also does the same for a temperature difference of zero since again the result is the same for both (both yield a zero value for h).
...
For ($\Delta$T \textgreater{} 0.0 AND an upward facing surface)~ OR~ ($\Delta$T \textless{} 0.0 AND an downward facing surface) or a vertical surface, a reduced convection correlation is used:
