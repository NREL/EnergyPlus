...
\begin{longtable}{@{}lp{4in}@{}}
  ...
  VAV &
  \(\begin{array}{rl}
    T_{cc,exit} &= T_{cool,supply} \\ 
    \dot{V}_{cc,air} &= \frac{\dot{m}_{cc,air,peak}}{\rho_{air}}
  \end{array}\) \tabularnewline
  Bypass &
  \(\begin{array}{rl}
    T_{cc,exit} &= T_{cool,supply} \\ 
    \dot{V}_{cc,air} &=
      \dot{V}_{cc,air,max}\cdot\max \left(0, \min \left(1,
        \frac{T_{mix,at-peak}-T_{sup,avg}}{T_{mix,at-peak}-T_{cc,exit}} \right) \right)
  \end{array}\) \tabularnewline
  VT &
  \(\begin{array}{rl}
    T_{cc,exit} &= \max\left(T_{cool,supply}, T_{sup,avg}\right) \\ 
    \dot{V}_{cc,air} &= \dot{V}_{cc,air,max}
  \end{array}\) \tabularnewline
  OnOff &
  \(\begin{array}{rl}
    T_{cc,exit} &= T_{cool,supply} \\
    \dot{V}_{cc,air} &=\dot{V}_{sys,air,max}
  \end{array}\) \tabularnewline
\bottomrule
\end{longtable}
...
where,
\( C_{p,w} \) is the specific heat of load side loop fluid at standard conditions (5.05 \(^{o}\)C);

\( \rho_w \) is the density of load side loop fluid at standard conditions (5.05 \(^{o}\)C);
...
\begin{equation}
  \dot{V}_{max,hw} = \dot{Q}_{coil,des} / (c_{p,w} \cdot \rho_w \cdot \Delta T_{loop,des})
\end{equation}
