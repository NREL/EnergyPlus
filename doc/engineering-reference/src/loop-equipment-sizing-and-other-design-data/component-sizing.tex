...
For duct type = \emph{heating}, \(\dot{V}_{fan,max} = \text{DesHeatVolFlowsys}\)
...
\paragraph{\texorpdfstring{\textbf{Coil in main air stream, no preconditioning of outside air} }{Coil in main air stream, no preconditioning of outside air }}\label{coil-in-main-air-stream-no-preconditioning-of-outside-air}
...
The oustide air fraction is calculated as (where V\(_{r}\) is calculated as above)
...
Where the inputs to those functions are the coil inlet design conditions. For coils in terminal units these are set at the system level to the system design supply air temperature. For zonal units they are set to design return air, mixed air, or outside air as appropriate to the unit. T\(_{s}\) is set to the zone cooling design supply air temperature as specified in the \emph{Zone:Sizing} inputs. W\(_{s}\) is set to the zone cooling design supply air humidity ratio as specified in the \emph{Zone:Sizing} inputs.

\subsubsection{\texorpdfstring{Design Water Flow Rate (m\(^{3}\)/s) - System Coils}{Design Water Flow Rate (m\^{}\{3\}/s) - System Coils}}\label{design-water-flow-rate-m3s---system-coils}
...
Where \(\Delta T_{w,des}\) is just the \emph{Loop Design Temperature Difference} user input from \emph{Sizing:Plant} (if the coil is in the outside air stream, 1/2 the \emph{Loop Design Temperature Difference} is used). The design coil load \emph{Load\(_{s}\)} is calculated from:
...
\def\labelenumi{\arabic{enumi}.}
\item
  Coil in outside air stream: T\(_{s}\) = T\(_{l}\) (the specified Precool Design Temperature from the Sizing:System object).
\item
...
For all other cases T\(_{s}\) is set to T\(_{s}\) (the zone design supply air temperature as specified in Sizing:Zone).
...
\begin{lstlisting}
-   Get the dewpoint temperature at W$_{s}$: $T_{dp,in} = \text{PsyTdpFnWPb}\left(W_{air,in,des},P_{air,std}\right)$
\end{lstlisting}

\begin{lstlisting}
-   If T$_{n}$ < = T$_{s}$ set W$_{s}$ = W$_{s}$. Otherwise set W$_{s}$ = min(PsyWFnTdbRhPb(T$_{s}$,0.9,P$_{d}$),W$_{s}$)
\end{lstlisting}
...
\emph{N\(_{w}\)} = \textbf{Max}(\emph{N\(_{w}\)},3)
...
\emph{A\(_{e}\)} = 4.4 *\emph{D\(_{e}\)} *\emph{N\(_{s}\)} *\emph{N\(_{w}\)}

Where \emph{D\(_{e}\)} is the tube inside diameter.

\subsubsection{Tube Outside Surf Area}\label{tube-outside-surf-area}

\emph{A\(_{e}\)} = 4.1 *\emph{D\(_{e}\)} *\emph{N\(_{s}\)} *\emph{N\(_{w}\)}

Where \emph{D\(_{e}\)} is the tube outside diameter.

\subsubsection{Coil Depth}\label{coil-depth}

\emph{Depth\(_{l}\)} = \emph{Depth\(_{g}\)} * \emph{N\(_{s}\)}
...
\begin{equation}
{T_{WB,ratio}} = {\raise0.7ex\hbox{{\left( {{T_{WB,air,in,des}} + 273.15\,C} \right)}} \!\mathord{\left/ {\vphantom {{\left( {{T_{WB,air,in,des}} + 273.15\,C} \right)} {283.15\,C}}}\right.}\!\lower0.7ex\hbox{{283.15\,C}}}
\end{equation}

\begin{equation}
{T_{S,ratio}} = {\raise0.7ex\hbox{{\left( {29.44\,C + 273.15\,C} \right)}} \!\mathord{\left/ {\vphantom {{\left( {29.44\,C + 273.15\,C} \right)} {283.15\,C}}}\right.}\!\lower0.7ex\hbox{{283.15\,C}}}
\end{equation}

where:

\$\{T\_\{WB,ratio\}\} = \$ ratio of load-side inlet air wet-bulb temperature in Kelvin to a reference temperature

\$\{T\_\{S,ratio\}\} = \$ ratio of source-side inlet water temperature in Kelvin to a reference temperature
...
The 4\(^{h}\) and 5\(^{h}\) coefficient (TCC4 and TCC5) used in the above equation are multipliers for the load-side and source-side flow ratios, respectively. For sizing, these ratios are assumed to be 1.
...
\begin{equation}
{T_{DB,ratio}} = {\raise0.7ex\hbox{{\left( {{T_{DB,air,in,des}} + 273.15\,C} \right)}} \!\mathord{\left/ {\vphantom {{\left( {{T_{DB,air,in,des}} + 273.15\,C} \right)} {283.15\,C}}}\right.}\!\lower0.7ex\hbox{{283.15\,C}}}
\end{equation}
...
The 5\(^{h}\) and 6\(^{h}\) coefficient (SCC5 and SCC6) used in the above equation are multipliers for the load-side and source-side flow ratios, respectively. For sizing, these ratios are assumed to be 1.
...
\begin{array}{l}IF({T_{DB,in}}\, > \,{T_{DB,out}})THEN\\\,\,\,\,\,{\mathop Q\limits^\cdot_{coil,des,sensible}} = {\raise0.7ex\hbox{{{m_{air,des}}C{p_{air,des}}\left( {{T_{DB,in}} - {T_{DB,out}}} \right)}} \!\mathord{\left/ {\vphantom {{{m_{air,des}}C{p_{air,des}}\left( {{T_{DB,in}} - {T_{DB,out}}} \right)} {SensCapTempModFac}}}\right.}\!\lower0.7ex\hbox{{SensCapTempModFac}}}\\ELSE\\\,\,\,\,\,{\mathop Q\limits^\cdot_{coil,des,sensible}} = {\raise0.7ex\hbox{{{m_{air,des}}C{p_{air,des}}\left( {24 - {T_{DB,out}}} \right)}} \!\mathord{\left/ {\vphantom {{{m_{air,des}}C{p_{air,des}}\left( {24 - {T_{DB,out}}} \right)} {SensCapTempModFac}}}\right.}\!\lower0.7ex\hbox{{SensCapTempModFac}}}\\ENDIF\end{array}
...
\emph{If H\(_{n}\) \textgreater{} H\(_{t}\) Then}
...
\emph{T\(_{r}\) = DesHeatCoilInTemp\(_{e}\)}

If the coil is part of an induction unit take into account the induced air:

\emph{Frac\(_{w}\)} = \emph{MinFlowFrac\(_{e}\)}

\emph{T\(_{r}\)} = \emph{DesHeatCoilInTemp\(_{e}\)} * \emph{Frac\(_{w}\)} +

\emph{ZoneTempAtHeatPeak\(_{e}\)} *(1- \emph{Frac\(_{w}\)})

\emph{T\(_{r}\) = HeatDesTemp\(_{e}\)}

W\emph{\(_{r}\) = HeatDesHumRat\(_{e}\)}
...
Here \emph{c\(_{r}\)} is calculated at the outlet humidity and the average of the inlet and outlet temperatures.
...
\emph{Q\(_{s}\)} = \emph{HeatCap\(_{s}\)}

The required inputs for the simple coil model are:

\emph{T\(_{r}\)} = \emph{HeatMixTemp\(_{s}\)}

\emph{W\(_{r}\)} = \emph{HeatMixHumRat\(_{s}\)}

\emph{T\(_{r}\)} = \emph{ExitTemp\(_{s}\)}
...
\emph{W\(_{r}\)} = \emph{DesHeatCoilInHumRat\(_{e}\)}
...
The steam density (\({\rho_{steam}}\) ) is for saturated steam at 100 degC (101325.0 Pa) and \emph{h\(_{g}\)} is the latent heat of vaporization of water at 100 degC (101325.0 Pa). \emph{C\(_{w}\)} is the heat capacity of saturated water (condensate) at 100 degC (101325.0 Pa) and \(\Delta {T_{sc}}\) is the Degree of Subcooling defined in the Coil:Heating:Steam object input. The design coil load \emph{Load\(_{s}\)} is calculated from:

\begin{equation}
Loa{d_{coil,des}} = {\dot m_{air,des}}({c_{p,air}})({T_{air,coil,des,out}} - {T_{air,coil,des,in}})
\end{equation}

The design air mass flow rate depends on the location of the coil (duct type). For duct type = \emph{main,} the flow rate is set to \emph{r\(_{r}\)} *\emph{DesMainVolFlow\(_{s}\)} *\emph{MinSysAirFlowRatio}. If the coil is in a cooling duct the flow rate is set to \emph{r\(_{r}\)} *\emph{DesCoolVolFlow\(_{s}\)} *\emph{MinSysAirFlowRatio}. If the coil is in a heating duct the flow rate is set to \emph{r\(_{r}\)} *\emph{DesHeatVolFlow\(_{s}\)}. If the coil is in any other kind of duct, the flow rate is set to \emph{r\(_{r}\)} *\emph{DesMainVolFlow\(_{s}\)}.

For sizing, the design outlet air temperature (\emph{T\(_{t}\)}) is the Central Heating Design Supply Air Temperature specified in the Sizing:System object.
...
\emph{T\(_{n}\)} = \emph{HeatOutTemp\(_{s}\)} (the outdoor air temperature at the design heating peak)
...
\tightlist
\item
  Sizing based on minimum outdoor air flow. The outdoor air fraction is calculated as \emph{Frac}\(_{a}\) = \emph{DesOutAirVolFlow\(_{s}\)} / \emph{DesVolFlow}. \emph{DesVolFlow} is \({{{{\mathop m\limits^ \bullet }_{air,des}}} \mathord{\left/ {\vphantom {{{{\mathop m\limits^ \bullet }_{air,des}}} {{\rho_{air}}}}} \right. } {{\rho_{air}}}}\) \emph{.}
\end{enumerate}

\emph{T\(_{n}\)} = \emph{Frac}\(_{a}\)* * HeatOutTemp\(_{s}\)* + (1.- \emph{Frac\(_{a}\)}) *\emph{HeatRetTemp\(_{s}\)} (see Table~\ref{table:system-sizing-data}. System Sizing Data)

\paragraph{Zone Coils}\label{zone-coils-2}

If the coil is part of an \emph{AirTerminal:SingleDuct:*} unit \\
(e.g., \emph{AirTerminal:SingleDuct:ConstantVolume:Reheat, AirTerminal:SingleDuct:VAV:Reheat, AirTerminal:SingleDuct:SeriesPIU:Reheat, etc.)}, the maximum steam flow rate is set equal to the terminal unit's maximum steam flow rate. Otherwise (e.g., the zone-level coil is part of \emph{ZoneHVAC:PackagedTerminalAirConditioner, ZoneHVAC:UnitVentilator, ZoneHVAC:UnitHeater or ZoneHVAC:VentilatedSlab}) the calculation is similar to that at the system level. A design load is calculated:
...
\({\dot m_{air,des}}\) = \emph{DesHeatMassFlow\(_{e}\)} (see Table~\ref{table:zone-sizing-data}. Zone Sizing Data)

\emph{T\(_{n}\)} = \emph{DesHeatCoilInTemp\(_{e}\)} (see Table~\ref{table:zone-sizing-data})

\emph{T\(_{t}\)} = \emph{HeatDesTemp\(_{e}\)} (user input from Sizing:Zone object)

\({c_{p,air}}\) = Specific heat of air (evaluated at the average of inlet and outlet air temperatures, and at the zone heating design supply air humidity ratio \emph{HeatDesHumRat\(_{e}\)} {[}user input from Sizing:Zone object{]})

\begin{equation}
{\dot V_{coil,steam,max}}\,\,\, = \,\,\,\,\,\frac{{Loa{d_{coil,des}}}}{{{\rho_{steam}}\left( {{h_{fg}} + {c_{p,w}}\cdot \Delta {T_{sc}}} \right)}}
\end{equation}

The terms in the denominator of this equation (\emph{ρ\(_{m}\)}, \emph{h\(_{g}\)}, etc.) are evaluated in the same way as described above for steam System Coils.
...
\emph{Q\(_{s}\)} = \emph{C\(_{r}\)} * \emph{DesHeatMassFlow\(_{e}\)} *(\emph{T\(_{r}\)}-\emph{T\(_{r}\)})
...
\emph{T\(_{x}\)} = \emph{CoolMixTemp\(_{s}\)}

\emph{W\(_{x}\)} = \emph{CoolMixHumRat\(_{s}\)}

\emph{T\(_{p}\)} = \emph{CoolSupTemp\(_{s}\)}

\emph{W\(_{p}\)} = \emph{CoolSupHumRat\(_{s}\)}

\emph{T\(_{e}\)} = \emph{CoolOutTemp\(_{s}\)}

\emph{r\(_{r}\)} = \emph{PsyRhoAirFnPbTdbW}(\emph{p\(_{d}\)}, \emph{T\(_{x}\)},\emph{W\(_{x}\)})

\emph{h\(_{x}\)} = \emph{PsyHFnTdbW}(\emph{T\(_{x}\)},\emph{W\(_{x}\)})

\emph{h\(_{p}\)} = \emph{PsyHFnTdbW}(\emph{T\(_{p}\)},\emph{W\(_{p}\)})

\emph{T\(_{b}\)} = \emph{PsyTwbFnTdbWPb}(\emph{T\(_{x}\)},\emph{W\(_{x}\)}, \emph{p\(_{d}\)})

\emph{CapModFac} = \emph{CurveValue}(CCapFTemp,\emph{T\(_{b}\)},\emph{T\(_{e}\)})

\begin{equation}
CCappeak = \rho air\cdot \dot Vair,rated\cdot (hmix - hsup)
\end{equation}

\emph{CCap\(_{d}\)} = \emph{CCap\(_{k}\)}/ \emph{CapModFac}
...
If \emph{FlowCapRatio} \textless{} \emph{FlowCapRatio\(_{n}\)} then

\begin{equation}
CCaprated = \dot Vair,rated/FlowCapRatiomin
\end{equation}

If \emph{FlowCapRatio} \textgreater{} \emph{FlowCapRatio\(_{x}\)} then
...
\emph{FlowCapRatio\(_{n}\)} = 0.00004027 m\(^{3}\)/s per watt (300 cfm/ton)

And

\emph{FlowCapRatio\(_{x}\)} = 0.00006041 m\(^{3}\)/s per watt (450 cfm/ton)
...
\emph{FlowCapRatio\(_{n}\)} = 0.00001677 m\(^{3}\)/s per Watt (125 cfm/ton)

And

\emph{FlowCapRatio\(_{x}\)} = 0.00003355 m\(^{3}\)/s per Watt (250 cfm/ton)
...
\emph{T\(_{x}\)} = \emph{DesCoolCoilInTemp\(_{e}\)}

\emph{W\(_{x}\)} = \emph{DesCoolCoilInHumRat\(_{e}\)}

\emph{T\(_{p}\)} = \emph{CoolDesTemp\(_{e}\)}

\emph{W\(_{p}\)} = \emph{CoolDesHumRat\(_{e}\)}

\emph{T\(_{e}\)} = \emph{T\(_{e}\),\(_{k}\)}
...
\emph{T\(_{d}\)} = 26.6667 \(^{o}\)C (80 \(^{o}\)F)

\emph{W\(_{d}\)} = 0.01125 (corresponds to 80 \(^{o}\)F drybulb, 67 \(^{o}\)F wetbulb)

\emph{C\(_{r}\)} = \emph{PsyCpAirFnWTdb}(\emph{W\(_{d}\)}, \emph{T\(_{d}\)})
...
\emph{h\(_{d}\)} = \emph{PsyHFnTdbW}(\emph{T\(_{d}\)}, \emph{W\(_{d}\)})

\emph{h\(_{p}\)} = \emph{PsyHFnTdbW}(\emph{T\(_{p}\)}, \emph{W\(_{p}\)})

\emph{Dh}\(_{p}\) = \emph{h\(_{d}\)}-\emph{h\(_{p}\)}

\emph{DQs\(_{p}\)} = \emph{C\(_{r}\)} *(\emph{T\(_{d}\)}-\emph{T\(_{p}\)})

\emph{SHR\(_{d}\)} = \emph{DQs\(_{p}\)}/\emph{Dh}\(_{p}\)
...
\emph{Eff\(_{t}\)} is the motor efficiency, often the default value of 0.9. This is defined as the shaft power output in Watts divided by the electric power in Watts.

\emph{V\(_{m}\)} is the design volume flow rate in m\(^{3}\)/s.
...
\emph{H\(_{m}\)} the nominal head, or pressure rise across the pump, is an input in Pascals.
...
\emph{C\(_{w}\)} is the specific heat of water at 5 \(^{o}\)C;

\emph{r\(_{w}\)} is the density of water at standard conditions (5.05 \(^{o}\)C);

\emph{DT\(_{s}\)} is the chilled water loop design temperature rise;
...
\emph{C\(_{w}\)} is the specific heat of water at design condenser inlet temperature;

\emph{r\(_{w}\)} is the density of water at standard conditions (5.05 \(^{o}\)C);

\emph{DT\(_{s}\)} is the chilled water loop design temperature rise;

\emph{COP\(_{m}\)} is the chiller nominal COP.
...
\emph{C\(_{w}\)} is the specific heat of water at the boiler design outlet temperature;

\emph{r\(_{w}\)} is the density of water at standard conditions (5.05 \(^{o}\)C);

\emph{DT\(_{s}\)} is the hot water loop design temperature decrease;
...
\emph{C\(_{w}\) is the specific heat of water at average temperature ((100+20)/2 = 60.0} * degC), (J/kgK);*
...
\emph{h\(_{g}\) is the latent heat of vaporization of water at 100.0 degC, (J/kg);}
...
where 5.382 *10\(^{8}\) is m\(^{3}\)/s per watt corresponds to the rule-of-thumb of sizing the tower flow rate at 3 gallons per minute per ton. For the CoolingTower:VariableSpeed:Merkel model with NominalCapacity input method, the user can input the value used to scale design water flow rate from nominal capacity and the default is 5.382 *10\(^{8}\) m\(^{3}\)/s/W.
...
\emph{C\(_{w}\)} is the specific heat of water at the condenser loop design exit temperature;

\emph{r\(_{w}\)} is the density of water at standard conditions (5.05 \(^{o}\)C);

\emph{DT\(_{s}\)} is the condenser water loop design temperature rise;
...
r\(_{r}\) is the density of air at standard conditions.
...
\emph{T\(_{r}\)} = 35 \(^{o}\)C (95 \(^{o}\)F design air inlet temperature)

\emph{T\(_{b}\)} = 25.6 \(^{o}\)C (78 \(^{o}\)F design air inlet wetbulb temperature)

\emph{W\(_{n}\)} is calculated from the entering air drybulb and wetbulb.
...
\emph{T\(_{r}\)} = \emph{T\(_{s}\)}+\emph{DT\(_{s}\)}

\emph{NominalCapacity}

\emph{T\(_{r}\)} = 35 \(^{o}\)C (95 \(^{o}\)F design inlet water temperature).
...
\begin{itemize}
\item
  C\(_{w}\)* is the specific heat of water at the condenser loop design exit temperature;
\item
  \begin{itemize}
  \tightlist
  \item
...
\emph{T\(_{}\)} = Design entering water temperature provided by the user

\emph{T\(_{}\)} = Design air inlet temperature provided by the user
...
\emph{T\(_{b}\)} = Design air inlet wetbulb temperature provided by the user
...
where 5.382 *10\(^{8}\) is m\(^{3}\)/s per watt corresponds to the rule-of-thumb of sizing the fluid cooler flow rate at 3 gallons per minute per ton.
...
\emph{T\(_{}\)} = 35 \(^{o}\)C (95 \(^{o}\)F design air inlet temperature)
...
\emph{T\(_{n}\)} = \emph{DesHeatCoilInTemp\(_{e}\)}

\emph{T\(_{t}\)} = \emph{HeatDesTemp\(_{e}\)} \(_{}\)
...
\emph{c\(_{r}\)} is evaluated at the average of the inlet \& outlet temperatures and the coil outlet humidity ratio.

\subsubsection{Maximum Cold Water Flow}\label{maximum-cold-water-flow}

\emph{T\(_{n}\)} = \emph{DesColdCoilInTemp\(_{e}\)}

\emph{T\(_{t}\)} = \emph{ColdDesTemp\(_{e}\)}

\emph{W\(_{n}\)} = \emph{DesCoolCoilInHumRat\(_{e}\)}

\emph{W\(_{t}\)} = \emph{CoolDesHumRat\(_{e}\)}

\emph{H\(_{n}\)} = \emph{PsyHFnTdbW}(\emph{T\(_{n}\)}, \emph{W\(_{n}\)})

\emph{H\(_{t}\)} = \emph{PsyHFnTdbW}(\emph{T\(_{t}\)}, \emph{W\(_{t}\)}) \(_{}\)
...
\emph{c\(_{h}\) is specific heat of hot water at reference condition (60 degC), J/kgK}
...
\emph{c\(_{c}\) is specific heat of hot water at reference condition (5 degC), J/kgK}
