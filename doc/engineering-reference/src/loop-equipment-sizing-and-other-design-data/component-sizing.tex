...
The design fan temperature rise (\emph{C}) due to fan heat is added to the air steam during sizing calculations when appropriate and is typically used for water cooling coil sizing. The calculation uses a straight-forward inversion of the classic \(Q = \dot{m}*Cp*\Delta{T}\) equation as:
...
\begin{itemize}
\item
  \(T_{air,in,des} = T_{mix,cool,at-peak} + T_{fan,heat,des}\) (the mixed air temperature at the design cooling peak plus the design fan temperature rise due to fan heat - see Section \ref{design-fan-heat})
\item
...
\item
  \(T_{air,in,des} = f_{oa}T_{precool} + \left(1-f_{oa}\right)T_{ret,cool,at-peak} + T_{fan,heat,des}\) (Precool temperature is the specified Precool Design Temperature from System:Sizing Manager; T\_ret\_cool\_at-peak is the return temperature at the system cooling peak load and \(T_{fan,heat,des}\) is the design fan temperature rise due to fan heat - see Section \ref{design-fan-heat})
\item
...
\item
  Coil in main air stream, no preconditioning of outside air: \(T_{air,in,des} = T_{mix,cool,at-peak} + T_{fan,heat,des}\) (the mixed air temperature at the cooling design peak plus the design fan temperature rise due to fan heat - see Section \ref{design-fan-heat}).
\item
  Coil in main air stream, outside air preconditioned. The outside air fraction is calculated as \(f_{oa} = \dot V_{air,out,des}/\dot V_{cc,air}\) , where \(\dot V_{cc,air}\) is calculated above. Then \(T_{air,in,des} = f_{oa}T_{precool}+\left(1-f_{oa}\right)T_{ret,cool,at-peak} + T_{fan,heat,des}\) , where \(T_{precool}\) is the specified \emph{Precool Design Temperature} from \emph{System:Sizing}, \(T_{ret,cool,at-peak}\) is the return temperature at the system cooling peak load and \(T_{fan,heat,des}\) is the design fan temperature rise due to fan heat - see Section \ref{design-fan-heat}.
\end{itemize}

\subsubsection{Design Air Inlet Temperature - Zone Coils}\label{design-air-inlet-temperature---zone-coils}

The design inlet temperature depends on whether the coil is in a terminal unit or a zonal unit, and where the coil is positioned within the unit. In each case, the design fan temperature rise is added to coil inlet temperature regardless of configuration (e.g., blow-through or draw-through) since fan heat in either case results in a higher design coil load - see Section \ref{design-fan-heat}.  

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  For the AirTerminal:SingleDuct:ConstantVolume:FourPipeInduction terminal unit the design inlet temperature is set to the zone temperature at the time of the zone cooling peak, since the coil is located in the induced air stream: \(T_{air,in,des} = T_{zone,cool,peak} + T_{fan,heat,des}\)
\item
  For fan coil units the design inlet temperature is set to the mixed air temperature: \(T_{air,in,des} = f_{oa}T_{oa,coolpeak} + \left(1-f_{oa}\right)T_{z,coolpeak} + T_{fan,heat,des}\) , where \(f_{oa} = \rho_a \dot V_{z,oa,des} / \dot m_{z,cool,des}\)
\item
  In all other cases the design inlet temperature is set to the zone design cooling coil inlet temperature which is calculated in the zone sizing simulation and is basically the same calculation as the fan coil unit. The design fan temperature rise due to fan heat is also included.
\end{enumerate}
...
\begin{equation}
  \dot{Q}_{coil,des,sensible} = \frac{\dot{m}_{air,des}C_{p,air,des}\PB{T_{DB,in}-T_{DB,out}}}{SensCapTempModFac} +\dot{Q}_{fan,heat,dex}
\end{equation}
...
\( C_{p,w} \) is the specific heat of evaporator loop fluid at 5 \(^{o}\)C;

\( \rho_w \) is the density of evaporator loop fluid at standard conditions (5.05 \(^{o}\)C);
...
\( \rho_w \) is the density of condenser loop fluid at standard conditions (5.05 \(^{o}\)C);
...
\( C_{p,w} \) is the specific heat of load side loop fluid at standard conditions (5.05 \(^{o}\)C for cooling, 60.0 \(^{o}\)C for heating);

\( \rho_w \) is the density of load side loop fluid at standard conditions (5.05 \(^{o}\)C for cooling, 60.0 \(^{o}\)C for heating);
...
\begin{equation}
\dot Vdes = \dot V_{loop,des} \cdot f_{size}
\end{equation}
