...
\begin{equation}
N_{tube/row} = \operatorname{Int} ({\mathrm{13750}}\cdot \dot Vcoil,water,max)
\end{equation}
...
\begin{equation}
\textit{A\(_{tube,total\, inside}\)} = 4.4 *\textit{D\(_{tube,inside}\)} *\textit{N\(_{tube\, rows}\)} *\textit{N\(_{tubes/row}\)}
\end{equation}
...
\begin{equation}
\textit{A\(_{tube,outside}\)} = 4.1 *\textit{D\(_{tube,outside}\)} *\textit{N\(_{tube\, rows}\)} *\textit{N\(_{tubes/row}\)}
\end{equation}
...
\begin{equation}
\textit{Depth\(_{coil}\)} = \textit{Depth\(_{tube\, spacing}\)} * \textit{N\(_{tube\, rows}\)}
\end{equation}
...
\begin{equation}
TotCapTempModFra{c_{NominalSpeed}} = {\mathrm{a}} + {\mathrm{b*}}W{B_i} + c*WB_i^2 + {\mathrm{d*EWT}} + e*EW{T^2} + f*W{B_i}*EWT
\end{equation}
...
\begin{equation}
\textit{Q\(_{coil,des}\)} = \textit{HeatCap\(_{sys}\)}
\end{equation}

The required inputs for the simple coil model are:

\begin{equation}
\textit{T\(_{in,air}\)} = \textit{HeatMixTemp\(_{sys}\)}
\end{equation}

\begin{equation}
\textit{W\(_{in,air}\)} = \textit{HeatMixHumRat\(_{sys}\)}
\end{equation}

\begin{equation}
\textit{T\(_{in,water}\)} = \textit{ExitTemp\(_{plt,hw,des}\)}
\end{equation}
...
\begin{equation}
\textit{\(\dot m_{in,air} = \rho{air} \cdot DesMainVolFlow_{sys}\)}
\end{equation}

for duct type = \emph{cooling}

\begin{equation}
\textit{\(\dot m_{in,air} = \rho_{air} \cdot DesCoolVolFlowsys\)}
\end{equation}
...
\tightlist
\item
  Sizing based on minimum outdoor air flow. The outdoor air fraction is calculated as \emph{Frac}\(_{oa}\) = \emph{DesOutAirVolFlow\(_{sys}\)} / \emph{DesVolFlow}. \emph{DesVolFlow} is \({\frac{{{{\mathop m\limits^ \bullet }_{air,des}}}}{{\rho_{air}}}}\).
\end{enumerate}
...
\begin{equation}
\textit{Cap\(_{nom}\)} = \textit{HeatCap\(_{sys}\)}
\end{equation}
...
\begin{equation}
\textit{T\(_{in,air}\) = DesHeatCoilInTemp\(_{zone}\)}
\end{equation}

If the coil is part of an induction unit take into account the induced air:

\begin{equation}
\textit{Frac\(_{minflow}\)} = \textit{MinFlowFrac\(_{zone}\)}
\end{equation}
...
\begin{equation}
\textit{ZoneTempAtHeatPeak\(_{zone}\)} *(1- \textit{Frac\(_{minflow}\)})
\end{equation}

\begin{equation}
\textit{T\(_{out,air}\) = HeatDesTemp\(_{zone}\)}
\end{equation}

\begin{equation}
W\textit{\(_{out,air}\) = HeatDesHumRat\(_{zone}\)}
\end{equation}

\begin{equation}
\textit{Q\(_{coil,des}\)} = \textit{C\(_{p,air}\)} * \textit{DesHeatMassFlow\(_{zone}\)} *(\textit{T\(_{out,air}\)}-\textit{T\(_{in,air}\)})
\end{equation}
...
\begin{equation}
\textit{T\(_{mix}\)} = \textit{CoolMixTemp\(_{sys}\)}
\end{equation}

\begin{equation}
\textit{W\(_{mix}\)} = \textit{CoolMixHumRat\(_{sys}\)}
\end{equation}

\begin{equation}
\textit{T\(_{sup}\)} = \textit{CoolSupTemp\(_{sys}\)}
\end{equation}

\begin{equation}
\textit{W\(_{sup}\)} = \textit{CoolSupHumRat\(_{sys}\)}
\end{equation}

\begin{equation}
\textit{T\(_{outside}\)} = \textit{CoolOutTemp\(_{sys}\)}
\end{equation}

\begin{equation}
\textit{r\(_{air}\)} = \textit{PsyRhoAirFnPbTdbW}(\textit{p\(_{air,std}\)}, \textit{T\(_{mix}\)},\textit{W\(_{mix}\)})
\end{equation}

\begin{equation}
\textit{h\(_{mix}\)} = \textit{PsyHFnTdbW}(\textit{T\(_{mix}\)},\textit{W\(_{mix}\)})
\end{equation}

\begin{equation}
\textit{h\(_{sup}\)} = \textit{PsyHFnTdbW}(\textit{T\(_{sup}\)},\textit{W\(_{sup}\)})
\end{equation}

\begin{equation}
\textit{T\(_{mix,wb}\)} = \textit{PsyTwbFnTdbWPb}(\textit{T\(_{mix}\)},\textit{W\(_{mix}\)}, \textit{p\(_{air,std}\)})
\end{equation}

\begin{equation}
\textit{CapModFac} = \textit{CurveValue}(CCapFTemp,\textit{T\(_{mix,wb}\)},\textit{T\(_{outside}\)})
\end{equation}
...
\begin{equation}
\textit{CCap\(_{rated}\)} = \textit{CCap\(_{peak}\)}/ \textit{CapModFac}
\end{equation}
...
\begin{equation}
\textit{T\(_{mix}\)} = \textit{DesCoolCoilInTemp\(_{zone}\)}
\end{equation}

\begin{equation}
\textit{W\(_{mix}\)} = \textit{DesCoolCoilInHumRat\(_{zone}\)}
\end{equation}

\begin{equation}
\textit{T\(_{sup}\)} = \textit{CoolDesTemp\(_{zone}\)}
\end{equation}

\begin{equation}
\textit{W\(_{sup}\)} = \textit{CoolDesHumRat\(_{zone}\)}
\end{equation}

\begin{equation}
\textit{T\(_{outside}\)} = \textit{T\(_{outside}\),\(_{desday,peak}\)}
\end{equation}
...
\begin{equation}
\textit{FlowCapRatio\(_{min}\)} = 0.00004027 m^{3}/s per watt (300 cfm/ton)
\end{equation}

and

\begin{equation}
\textit{FlowCapRatio\(_{max}\)} = 0.00006041 m^{3}/s per watt (450 cfm/ton)
\end{equation}
...
\begin{equation}
\textit{FlowCapRatio\(_{min}\)} = 0.00001677 m^{3}/s per Watt (125 cfm/ton)
\end{equation}

and

\begin{equation}
\textit{FlowCapRatio\(_{max}\)} = 0.00003355 m^{3}/s per Watt (250 cfm/ton)
\end{equation}
...
\begin{equation}
TotCapTempModFra{c_{NominalSpeed}} = {\mathrm{a}} + {\mathrm{b*}}W{B_i} + c*WB_i^2 + {\mathrm{d*}}D{B_o} + e*D{B_o}{T^2} + f*W{B_i}*D{B_o}
\end{equation}
...
The Loop Demand Side design flow rate,\({\dot V_{Dmd,des}}\) , is set equal to the Loop Supply Side design flow rate multiplied by the supply to demand side ratio of the loop side specific heat (Cp), temperature (Dt), and density (rho) product. This ratio accounts for differences in plant design delta temperatures, fluid density, and fluid heat capacity (e.g., water versus glycol). If Sizing:Plant objects are not present for both loops connected to the heat exchanger then this ratio is equal to 1.

\begin{equation}
{\dot V_{Dmd,des}} = {\dot V_{Sup,des} * {((Cp,sup * Dt,sup * rho,sup) / (Cp,dem * Dt,dem * rho,dem))}}
\end{equation}

The design heat transfer capacity and UA for the heat exchanger are calculated using the design temperatures for the two plant loops. The loop design temperature difference for the Loop Supply Side, \(\Delta {T_{SupLoop,Des}}\) , is entered in a Sizing:Plant object and is used to determine a nominal capacity for autosized UA. For autosized UA, \(c_p\) and \(\rho\) are calculated at an initialization temperature of 5.05 C. When UA is not autosized, the reference capacity is based on the user-entered UA and the temperature difference between the loop set point temperatures, or the average of the loop high and low set point temperatures for DualSetpointDeadband control. The following equations are used to calculate the reference capacity for autosized UA and for user-entered UA:
...
\begin{equation}
\dot Q = UA \Delta {T_{SupLoop,Setpoint - DemLoop,Setpoint}}
\end{equation}
...
A reference capacity for the heat exchanger is determined from either the design flow rate and supply loop design temperature difference or the user-entered UA and heat exchanger temperature difference. When UA is autosized the loop temperature difference is obtained from the inputs in Sizing:Plant objects. If UA is not autosized then the loop's setpoint temperatures are used (if the loop's load scheme is DualSetpointDeadband then the average of the high and low setpoints are used). This capacity is reported and may be used for controls based on operation scheme.
...
\begin{equation}
\textit{T\(_{coil,in}\)} = \textit{DesColdCoilInTemp\(_{zone}\)}
\end{equation}

\begin{equation}
\textit{T\(_{coil,out}\)} = \textit{ColdDesTemp\(_{zone}\)}
\end{equation}

\begin{equation}
\textit{W\(_{coil,in}\)} = \textit{DesCoolCoilInHumRat\(_{zone}\)}
\end{equation}

\begin{equation}
\textit{W\(_{coil,out}\)} = \textit{CoolDesHumRat\(_{zone}\)}
\end{equation}

\begin{equation}
\textit{H\(_{coil,in}\)} = \textit{PsyHFnTdbW}(\textit{T\(_{coil,in}\)}, \textit{W\(_{coil,in}\)})
\end{equation}

\begin{equation}
\textit{H\(_{coil,out}\)} = \textit{PsyHFnTdbW}(\textit{T\(_{coil,out}\)}, \textit{W\(_{coil,out}\)})
\end{equation}
...
\begin{equation}
\dot V_{OA,cooling} = {\mathrm{MIN}}(MinO{A_{zone}},\dot V_{SA,cooling})
\end{equation}
...
\begin{equation}
\dot V_{OA,~no~cool~or~heat} = {\mathrm{MIN}}(MinO{A_{zone}},\dot V_{SA,~no~cool~or~heat})
\end{equation}
...
\begin{equation}
{\dot V_{air,\max ,{\mathrm{terminal}}}} = Max(DesCoolVolFlow_{zone},DesHeatVolFlow_{zone})
\end{equation}
...
For the series PIU terminal units, the maximum primary air flow rate is sized to the same value as the maximum total air flow rate. For the parallel PIU the secondary air flow rate is sized to the difference between the maximum primary air flow rate and the minimum primary air flow rate.
...
\begin{equation}
{\dot V_{fan,\max }} = MAX({\mathrm{DesOutAirVolFlow, 0}}{\mathrm{.5*DesMainVolFlow)}}
\end{equation}
...
\begin{equation}
  \dot{V}_{\mathrm{SecCoil}} = \dot{V}_{\mathrm{PriHeatCoil}} \cdot \text{ScalingFactor}
\end{equation}
