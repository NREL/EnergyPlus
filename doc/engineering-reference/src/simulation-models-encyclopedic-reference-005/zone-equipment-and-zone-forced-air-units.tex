...
\begin{itemize}
 \item x1 = outside air temperature (dry bulb) ($^{\circ}$C) = TOSA
 \item x2 = outside humidity ratio (gH2O/g dry air) = OSA
 \item x3 = return air temperature (dry bulb) ($^{\circ}$C) = TRA
 \item x4 = return air humidity ratio  (gH2O/g dry air) = RA
 \item x5 = normalized supply air mass flow rate (–) = mSA/mSA max
 \item x6 = outside air fraction (–) =OSAF
\end{itemize}

Dependent variables (yi) for the model include:
\begin{itemize}
 \item y1 = supply air temperature (dry bulb) ($^{\circ}$C) =TSA
 \item y2 = supply air humidity ratio (gH2O/g dry air) = SA
 \item y3 = total electric power (kJ/kg) = W'elec/mSA max
 \item y4 = supply fan electric power (kJ/kg) = W'fan elec/mSA max
 \item y5 = second fuel consumption (kJ/kg) = W'fuel/mSA max
 \item y6 = third fuel consumption (kJ/kg) = W'fuel/mSA max
 \item y7 = water consumption (liters/kg) = V'water/mSA max
...
\begin{itemize}
    \item $\dot{Q}_{SystemSensible}(t)$ = system sensible cooling capacity for time step
    \item $\dot{Q}_{SystemLatent}(t)$ = system latent cooling capacity for time step
    \item $\dot{Q}_{SystemSensible}(t)$ = zone sensible cooling capacity for time step
    \item $\dot{Q}_{ZoneLatent}(t)$ = zone latent cooling capacity for time step
    \item $\overline{\dot{m}_{SA}(t)}$ = time weighted average supply air mass flow rate for time step
    \item $C_{p,RA}(t)$ = specific heat capacity of return air for time step
    \item $C_{p,OA}(t)$ = specific heat capacity of outdoor air for time step
    \item $\overline{C_{p,SA}}(t)$ = supply air mass weighted average specific heat capacity of supply air for time step
    \item $\lambda$ = latent heat of vaporization for water
    \item $\overline{OAF}(t)$ = supply air mass weighted average outdoor air fraction for time step
    \item $T_{RA}(t)$ = return air temperature for time step
    \item $T_{OA}(t)$ = outdoor air temperature for time step
    \item $\overline{T_{SA}}(t)$ = supply air mass weighted average supply air temperature for time step
    \item $\omega_{RA}(t)$ = return air humidity ratio for time step
    \item $\omega_{OA}(t)$ = outdoor air humidity ratio for time step
    \item $\overline{\omega_{SA}}(t)$ = supply air mass weighted average supply air humidity ratio for time step
\end{itemize}
