...
\begin{equation}
{Q_{r}}=3.96*f\(_{l}\)*{[}(0.01*T\(_{a}\))\(^{4}\)-(0.01*T\(_{a}\))\(^{4}\){]}}
\end{equation}
...
\begin{equation}
{E_{rsw}} = 0.42(H - 58.2) for \(H > 58.2\)
\end{equation}

\begin{equation}
{E_{rsw}} = 0 for \(H \le 58.2\)
\end{equation}

\begin{equation}
{E_{f}}=0.00305*(5733.0-6.99H-P\(_{a}\))}
\end{equation}
...
In the EnergyPlus code, when SkinTemp is greater than SkinTempSet (33.7�C), then SkinThermSigWarm = SkinTemp - SkinTempSet and 
...
In the EnergyPlus code, when AvgBodyTemp is greater than AvgBodyTempSet(36.49�C), then BodyThermSigWarm = AvgBodyTemp - AvgBodyTempSet and BodyThermSigCold = 0.0. Otherwise, when AvgBodyTemp is less than or equal to AvgBodyTempSet, then BodyThermSigWarm = 0.0 and BodyThermSigCold = AvgBodyTempSet-AvgBodyTemp.

The skin flood flow rate in L/hr•m\(^{2}\) is calculated using:
...
In the Pierce model, T\(_{cl}\) in �C is estimated by each iteration using following equation:
...
\begin{equation}
For \({E_{sw}} \le {E_{\max }}\), {E_{sk}} = {E_{sw}} + (1 - {w_{rsw}}){E_{diff}}
\end{equation}

\begin{equation}
For \({E_{sw}} > {E_{\max }}\), {E_{sk}} = {E_{\max }}
\end{equation}
