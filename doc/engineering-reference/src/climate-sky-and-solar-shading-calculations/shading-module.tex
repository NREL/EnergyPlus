...
\begin{equation}
VMULT(ZoneNum) = \frac{1}{{\sum\limits_{i = 1}^{{N_{surf}}} {AbsIntSur{f_i}*{A_i}} }}{\mathrm{    [}}{{\mathrm{m}}^{{\mathrm{ - 2}}}}]
\end{equation}
...
\begin{equation}
EnclSolDB(ZoneNum) = BTOTZone - BABSZone{\mathrm{    [}}{{\mathrm{m}}^{\mathrm{2}}}]
\end{equation}
...
\emph{BABSZone} is given by the following sum\footnote{If Solar Distribution = FullInteriorAndExterior in the Building object, the program calculates where beam solar from exterior windows falls inside the zone. Otherwise, all beam solar is assumed to fall on the floor.} (see Figure~\ref{fig:vertical-section-through-a-two-zone-building}):
...
\begin{equation}
\begin{array}{l}QS(ZoneNum)*\alpha_{l,back}^{dif}(SurfNum) + \\BeamSolarRad*\frac{{\alpha_{l,back}^{beam}(SurfNum)}}{{A(SurfNum)}}\sum\limits_{i = 1}^{{N_{extwin}}} {TB{m_i}*Aoverla{p_i}(SurfNum)} *CosIn{c_i}{\mathrm{ [W/}}{{\mathrm{m}}^{\mathrm{2}}}]\end{array}
\end{equation}
...
\begin{equation}
QS(ZoneNum)*{\tau ^{dif}}(SurfNum)*A(SurfNum){\mathrm{    [W]}}
\end{equation}
...
\begin{equation}
BeamSolarRad*{\tau ^{beam}}(SurfNum)\sum\limits_{i = 1}^{{N_{extwin}}} {TB{m_i}*Aoverla{p_i}(SurfNum)*CosIn{c_i}{\mathrm{    [W}}]}
\end{equation}
...
\begin{equation}
QS(ZoneNum) \to QS(ZoneNum) + [{\mathrm{beam solar from adjacent zones]}}*VMULT(ZoneNum)
\end{equation}
...
\begin{equation}
{\mathrm{GroundReflectedSolar = (BeamSolar}} \bullet {\mathrm{COS(SunZenithAngle) + DiffuseSolar)}} \bullet {\mathrm{GroundReflectance}}
\end{equation}
...
\begin{equation}
{\mathrm{GroundReflectanc}}{{\mathrm{e}}_{{\mathrm{used}}}} = {\mathrm{GroundReflectance}} \bullet SolarModifie{r_{Snow}}
\end{equation}
...
\begin{equation}
{\mathrm{DaylightingGroundReflectanc}}{{\mathrm{e}}_{{\mathrm{used}}}} = {\mathrm{GroundReflectance}} \bullet DaylightingModifie{r_{Snow}}
\end{equation}
