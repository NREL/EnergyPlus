...
All input data for the inlet and supply side mixer air terminal units is stored in the data structure \emph{SysATMixer}.
...
For a mixer terminal unit connected to the supply side of a zoneHVAC equipment, the conditions and flow rates of the primary air and secondary air streams are known, are taken from the inlet nodes' data. The balance equations are then used to calculate the outlet flow rate and conditions.

\subsubsection{Simulation and Control}\label{simulation-and-control-1-002}

The terminal mixer model is invoked from within the zone HVAC model. Basically the terminal mixer becomes a subcomponent of the zone HVAC equipment model. This allows the zone unit to allow for the mixing of central supply air with its inlet or supply air stream of the zoneHVAC equipment in calculating how much cooling or heating it needs to do in order to meet the zone load.
