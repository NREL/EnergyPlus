...
\(Q_{conv}\) = Convective heat transfer rate (W/m²)

\(h\) = Convection heat transfer coefficient (W/m²·°C)

\(T_p\) = Pool water temperature (°C)

\(T_a\) = Air temperature over pool (°C)
...
\(m_{fw}\) = Mass flow rate of fresh water (kg/s)

\(c_w\) = Specific heat of water (J/kg·°C)

\(T_p\) = Pool water temperature (°C)

\(T_{fw}\) = Fresh water supply temperature (°C)
...
\(T_{hw}\) = Heated water supply temperature (°C)
...
\(m_w\) = mass of pool water (kg)

\(cp\) = specific heat of water (J/kg·°C)

\(\Delta t\) = time step length (s)

\(T_{set}\) = desired pool water temperature (°C)

\(T_{old}\) = temperature of water at the last time step (°C)

\(m_p\) = needed mass flow rate of water from the plant (kg/s)

\(T_{in}\) = inlet water temperature from the plant (°C)
